...
ArchiveManager::FileDescription *ArchiveManager::add_file(UnixTime ts, bool key_block) {
  ...
  index_->begin_transaction().ensure();
  {
    ...
    index_
        ->set(create_serialize_tl_object<tos_api::db_archive_index_key>().as_slice(),
              create_serialize_tl_object<tos_api::db_archive_index_value>(std::move(t), std::move(tk)).as_slice())
        .ensure();
  }
  {
    index_
        ->set(create_serialize_tl_object<tos_api::db_archive_package_key>(ts, key_block).as_slice(),
              create_serialize_tl_object<tos_api::db_archive_package_value>(
                  ts, key_block, std::vector<tl_object_ptr<tos_api::db_archive_package_firstBlock>>(), false)
                  .as_slice())
        .ensure();
  }
  index_->commit_transaction().ensure();
...
void ArchiveManager::load_package(UnixTime ts, bool key_block) {
  auto key = create_serialize_tl_object<tos_api::db_archive_package_key>(ts, key_block);
  ...
  auto R = fetch_tl_object<tos_api::db_archive_package_value>(value, true);
  R.ensure();
  auto x = R.move_as_ok();
...
void ArchiveManager::update_desc(FileDescription &desc, ShardIdFull shard, BlockSeqno seqno, LogicalTime lt) {
  ...
  std::vector<tl_object_ptr<tos_api::db_archive_package_firstBlock>> vec;
  for (auto &e : desc.first_blocks) {
    vec.push_back(create_tl_object<tos_api::db_archive_package_firstBlock>(e.first.workchain, e.first.shard,
                                                                           e.second.seqno, e.second.lt));
  }
  index_->begin_transaction().ensure();
  index_
      ->set(create_serialize_tl_object<tos_api::db_archive_package_key>(desc.unix_time, desc.key_block).as_slice(),
            create_serialize_tl_object<tos_api::db_archive_package_value>(desc.unix_time, desc.key_block,
                                                                          std::move(vec), false)
                .as_slice())
      .ensure();
  index_->commit_transaction().ensure();
...
void ArchiveManager::start_up() {
  ...
  std::string value;
  auto v = index_->get(create_serialize_tl_object<tos_api::db_archive_index_key>().as_slice(), value);
  v.ensure();
  if (v.move_as_ok() == td::KeyValue::GetStatus::Ok) {
    auto R = fetch_tl_object<tos_api::db_archive_index_value>(value, true);
    R.ensure();
    auto x = R.move_as_ok();
