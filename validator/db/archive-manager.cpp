...
void ArchiveManager::get_temp_file_short_bounded_cont(FileReference ref_id, PackageId idx,
                                                      td::uint64 maximum_data_size,
  ...
  auto P = td::PromiseCreator::lambda(
      [SelfId = actor_id(this), ref_id, maximum_data_size, idx = get_prev_temp_file_desc_idx(idx),
       promise = std::move(promise)](td::Result<td::BufferSlice> R) mutable {
        if (R.is_ok()) {
          promise.set_value(R.move_as_ok());
...
void ArchiveManager::get_file(ConstBlockHandle handle, FileReference ref_id,
                              td::Promise<td::BufferSlice> promise) {
  get_file_bounded(std::move(handle), std::move(ref_id), Package::max_data_size(), std::move(promise));
}

void ArchiveManager::get_file_bounded(ConstBlockHandle handle, FileReference ref_id, td::uint64 maximum_data_size,
                                      td::Promise<td::BufferSlice> promise) {
  if (handle->moved_to_archive()) {
    ...
    if (f.is_ok()) {
      td::actor::send_closure(f.ok()->file_actor_id(), &ArchiveSlice::get_file_bounded, std::move(handle),
                              std::move(ref_id), maximum_data_size, std::move(promise));
      return;
    }
  }
  if (handle->handle_moved_to_archive()) {
  ...
  get_temp_file_short_bounded_cont(std::move(ref_id), get_max_temp_file_desc_idx(), maximum_data_size,
                                   std::move(promise));
}
...
void ArchiveManager::delete_package(PackageId id, td::Promise<td::Unit> promise) {
  auto key = create_serialize_tl_object<tos_api::db_files_package_key>(id.id, id.key, id.temp);
  ...
  auto R = fetch_tl_object<tos_api::db_files_package_value>(value, true);
  R.ensure();
  auto x = R.move_as_ok();
...
const ArchiveManager::FileDescription *ArchiveManager::add_file_desc(ShardIdFull shard, PackageId id, BlockSeqno seqno,
  ...
  std::vector<tl_object_ptr<tos_api::db_files_package_firstBlock>> vec;
  for (auto &e : desc.first_blocks) {
    vec.push_back(create_tl_object<tos_api::db_files_package_firstBlock>(e.first.workchain, e.first.shard,
  ...
  // add package info to list of packages
  {
    ...
    index_
        ->set(create_serialize_tl_object<tos_api::db_files_index_key>().as_slice(),
              create_serialize_tl_object<tos_api::db_files_index_value>(std::move(t), std::move(tk), std::move(tt))
                  .as_slice())
        .ensure();
  }
  ...
  {
    index_
        ->set(create_serialize_tl_object<tos_api::db_files_package_key>(id.id, id.key, id.temp).as_slice(),
              create_serialize_tl_object<tos_api::db_files_package_value>(id.id, id.key, id.temp, std::move(vec), false)
                  .as_slice())
        .ensure();
  }
  index_->commit_transaction().ensure();
...
void ArchiveManager::update_desc(FileMap &f, const FileDescription &desc, ShardIdFull shard, BlockSeqno seqno,
  ...
  index_->begin_transaction().ensure();
  index_
      ->set(create_serialize_tl_object<tos_api::db_files_package_key>(desc.id.id, desc.id.key, desc.id.temp).as_slice(),
            create_serialize_tl_object<tos_api::db_files_package_value>(desc.id.id, desc.id.key, desc.id.temp,
                                                                        std::move(vec), false)
                .as_slice())
      .ensure();
  index_->commit_transaction().ensure();
...
void ArchiveManager::start_up() {
  ...
  std::string value;
  auto v = index_->get(create_serialize_tl_object<tos_api::db_files_index_key>().as_slice(), value);
  v.ensure();
  if (v.move_as_ok() == td::KeyValue::GetStatus::Ok) {
    auto R = fetch_tl_object<tos_api::db_files_index_value>(value, true);
    R.ensure();
    auto x = R.move_as_ok();
...

  {
    ...
    while (it != key_files_.end()) {
      if (it->first.id <= masterchain_seqno) {
        ...
        index_
            ->erase(create_serialize_tl_object<tos_api::db_files_package_key>(it2->second.id.id, it2->second.id.key,
                                                                              it2->second.id.temp)
                        .as_slice())
            .ensure();
