...
void ArchiveSlice::add_handle(BlockHandle handle, td::Promise<td::Unit> promise) {
  ...
  R.ensure();
  tl_object_ptr<tos_api::db_lt_desc_value> v;
  bool add_shard = false;
  if (R.move_as_ok() == td::KeyValue::GetStatus::Ok) {
    auto F = fetch_tl_object<tos_api::db_lt_desc_value>(td::BufferSlice{value}, true);
    F.ensure();
    v = F.move_as_ok();
  } else {
    v = create_tl_object<tos_api::db_lt_desc_value>(1, 1, 0, 0, 0);
    add_shard = true;
  }
  ...
  auto db_value = create_serialize_tl_object<tos_api::db_lt_el_value>(create_tl_block_id(handle->id()),
                                                                      handle->logical_time(), handle->unix_time());
  auto db_key = get_db_key_lt_el(handle->id().shard_full(), v->last_idx_++);
  auto status_key = create_serialize_tl_object<tos_api::db_lt_status_key>();
  ...
  if (add_shard) {
    ...
    if (G.move_as_ok() == td::KeyValue::GetStatus::NotFound) {
      ...
      auto F = fetch_tl_object<tos_api::db_lt_status_value>(value, true);
      F.ensure();
      auto f = F.move_as_ok();
      idx = f->total_shards_;
    }
  ...
  if (add_shard) {
    auto shard_key = create_serialize_tl_object<tos_api::db_lt_shard_key>(idx);
    auto shard_value =
        create_serialize_tl_object<tos_api::db_lt_shard_value>(handle->id().id.workchain, handle->id().id.shard);
    kv_->set(status_key.as_slice(), create_serialize_tl_object<tos_api::db_lt_status_value>(idx + 1)).ensure();
...
void ArchiveSlice::get_block_common(AccountIdPrefixFull account_id,
                                    std::function<td::int32(tos_api::db_lt_desc_value &)> compare_desc,
                                    std::function<td::int32(tos_api::db_lt_el_value &)> compare, bool exact,
  ...
  for (td::uint32 len = 0; len <= 60; len++) {
    ...
    f = true;
    auto G = fetch_tl_object<tos_api::db_lt_desc_value>(value, true);
    G.ensure();
    auto g = G.move_as_ok();
    if (compare_desc(*g) > 0) {
      continue;
    ...
    while (r - l > 1) {
      ...
      auto E = fetch_tl_object<tos_api::db_lt_el_value>(td::BufferSlice{value}, true);
      E.ensure();
      auto e = E.move_as_ok();
...
td::BufferSlice ArchiveSlice::get_db_key_lt_desc(ShardIdFull shard) {
  return create_serialize_tl_object<tos_api::db_lt_desc_key>(shard.workchain, shard.shard);
}

td::BufferSlice ArchiveSlice::get_db_key_lt_el(ShardIdFull shard, td::uint32 idx) {
  return create_serialize_tl_object<tos_api::db_lt_el_key>(shard.workchain, shard.shard, idx);
}

td::BufferSlice ArchiveSlice::get_db_key_block_info(BlockIdExt block_id) {
  return create_serialize_tl_object<tos_api::db_blockdb_key_value>(create_tl_block_id(block_id));
}
...
void ArchiveSlice::iterate_block_handles(std::function<void(const BlockHandleInterface &)> f) {
  before_query();
  td::uint32 range_start = tos_api::db_blockdb_key_value::ID;
  td::uint32 range_end = tos_api::db_blockdb_key_value::ID + 1;
  kv_->for_each_in_range(td::Slice{(char *)&range_start, 4}, td::Slice{(char *)&range_end, 4},
                         [&](td::Slice key, td::Slice value) -> td::Status {
                           auto r_key = fetch_tl_object<tos_api::db_blockdb_key_value>(key, true);
                           if (r_key.is_error()) {
                             return td::Status::OK();
                           }
...
td::Result<ArchiveSlice::PackageInfo *> ArchiveSlice::choose_package(BlockSeqno masterchain_seqno,
  ...
  if (!shard_separated_) {
    ...
    shard_prefix = tos::shard_prefix(shard_prefix, shard_split_depth_);
  }
...
void ArchiveSlice::truncate_shard(BlockSeqno masterchain_seqno, ShardIdFull shard, td::uint32 cutoff_seqno,
  ...
  for (int i = g->first_idx_; i < g->last_idx_; i++) {
    ...
    F.ensure();
    CHECK(F.move_as_ok() == td::KeyValue::GetStatus::Ok);
    auto E = fetch_tl_object<tos_api::db_lt_el_value>(value, true);
    E.ensure();
    auto e = E.move_as_ok();
...
void ArchiveSlice::truncate(BlockSeqno masterchain_seqno, ConstBlockHandle handle, td::Promise<td::Unit> promise) {
  ...
  for (int i = 0; i < f->total_shards_; i++) {
    auto shard_key = create_serialize_tl_object<tos_api::db_lt_shard_key>(i);
    R = kv_->get(shard_key, value);
    R.ensure();
    CHECK(R.move_as_ok() == td::KeyValue::GetStatus::Ok);

    auto G = fetch_tl_object<tos_api::db_lt_shard_value>(value, true);
    G.ensure();
    auto g = G.move_as_ok();
