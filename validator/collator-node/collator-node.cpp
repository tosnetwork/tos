...
void CollatorNode::start_up() {
  ...
  td::actor::send_closure(adnl_, &adnl::Adnl::subscribe, local_id_,
                          adnl::Adnl::int_to_bytestring(tos_api::collatorNode_generateBlock::ID),
                          std::make_unique<Cb>(actor_id(this)));
  td::actor::send_closure(adnl_, &adnl::Adnl::subscribe, local_id_,
                          adnl::Adnl::int_to_bytestring(tos_api::collatorNode_ping::ID),
                          std::make_unique<Cb>(actor_id(this)));
...
void CollatorNode::receive_query(adnl::AdnlNodeIdShort src, td::BufferSlice data,
  ...
  auto r_ping = fetch_tl_object<tos_api::collatorNode_ping>(data, true);
  if (r_ping.is_ok()) {
...
void CollatorNode::process_ping(adnl::AdnlNodeIdShort src, tos_api::collatorNode_ping& ping,
                                td::Promise<td::BufferSlice> promise) {
  LOG(DEBUG) << "got ping from " << src;
  TRY_STATUS_PROMISE(promise, check_out_of_sync());
  TRY_STATUS_PROMISE_PREFIX(promise, mc_config_status_.clone(), "unsupported mc config: ");
  auto pong = create_tl_object<tos_api::collatorNode_pong>();
  if (ping.flags_ & tos_api::collatorNode_pong::VERSION_MASK) {
    pong->flags_ |= tos_api::collatorNode_pong::VERSION_MASK;
