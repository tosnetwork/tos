...
Result<Hash> owner_execution_check(td::Ref<vm::Cell> state, td::Ref<vm::Cell> block, const Anchor& anchor,
                                  const ChainContext& chain, const Update& update, const Identity& current,
                                  std::uint64_t lt, std::uint16_t index) {
  return executed(std::move(state), std::move(block), anchor, chain, update, current, lt, index);
}
Result<td::Ref<vm::Cell>> owner_canonical_proof(const vm::MerkleProofBuilder& used) {
  return canonical_proof(used);
}
