...
std::uint16_t code(const Error& error) {
  ...
  if (e == "api-bad-request" || e == "chunk-conflict" || e == "chunk-index")
    return 1;
  if (e == "unauthorized" || e == "authority-refused" || e == "api-unauthorized" || e == "local-unauthorized" ||
  ...
  if (e == "sign-key-context" || e == "snapshot-validity" || e == "new-key-validity")
    return 6;
  if (e == "preparation-conflict" || e == "pending-conflict" || e == "operation-reservation-conflict" ||
      e == "cached-request-conflict" || e == "duty-conflict" || e == "candidate-conflict" ||
      e == "finalize-skip-conflict")
    return 7;
  if (e == "fenced" || e == "primitive-reservation-fence")
    return 8;
  if (e == "provider-key-capacity" || e == "fence-exhausted" || e == "journal-exhausted")
    return 9;
  if (e == "storage-unavailable" || e == "storage-quota" || e == "object-quota" || e == "journal-witness-mismatch" ||
      e == "witness-unavailable" || e == "principal-storage-quota" || e == "global-storage-quota" ||
      e == "storage-clock-regression" || e == "storage-clock-overflow")
    return 10;
  if (e == "result-uncertain" || e == "primitive-already-claimed")
    return 11;
  if (e == "history-unavailable" || e == "provider-history-unavailable" || e == "snapshot-missing" ||
...
Result<HttpResponse> failure(std::uint8_t method, const Hash& id, std::uint16_t code, std::uint8_t state) {
  bool retry = (method == 1 || method == 2 || method == 6 || method >= 8) && code >= 10 && code <= 12;
  auto raw = encode(ApiError{id, method, code, static_cast<std::uint8_t>(retry), state, {}});
  if (!raw.ok())
    return raw.error();
  auto json = encode_transport_frame({method, id, raw.value(), true, true});
  if (!json.ok())
    return json.error();
  return HttpResponse{code == 1 ? 400u : code == 2 ? 403u : 200u, std::string(api_media_type), json.value()};
}
...
#define ANCHOR_CASE(N, T)     \
  ...
  switch (method) {
    ANCHOR_CASE(8, GetProfileRequest)
    ANCHOR_CASE(9, GetPolicyRequest)
    ANCHOR_CASE(10, GetRegistryRequest)
    ANCHOR_CASE(11, GetKeyRequest)
    ANCHOR_CASE(12, GetCertificateRequest)
    ANCHOR_CASE(13, VerifyCertificateRequest)
    default:
