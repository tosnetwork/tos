...
Result<bool> native_session_identity_confirms(td::Ref<vm::Cell> masterchain_state, const Anchor& anchor,
  ...
  // The committee this session would run under, derived rather than described.
  // What the state admits on its own was already applied here; what was not is
  // everything the registry decides, and the identity below cannot stand in for
  // it -- that identity is a hash of keys, addresses and weights, so it agrees
  // whether or not the registry ever issued the bindings the set carries.
  //
  // The catchain sequence comes from the set itself so that the committee is
  // derived for the session this set would actually run, and not for a number
  // named twice.
  auto committee = NativeCommittee::derive(masterchain_state, anchor, chain, shard,
                                           validator_set->get_catchain_seqno(), budget);
  if (!committee.ok())
    return committee.error();
  // And the roster: the members this manager would seat have to be the ones
  // derivation selected, in that order. Derivation refuses a selected member
  // with no binding, so that rule is here through this comparison rather than
  // restated beside it.
