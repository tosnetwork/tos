...
void ValidatorManagerImpl::new_block_broadcast(BlockBroadcast broadcast, bool signatures_checked,
                                               BroadcastSource source, td::Promise<td::Unit> promise) {
  ...
  promise = [SelfId = actor_id(this), promise = std::move(promise), block_id = broadcast.block_id,
             cc_seqno = broadcast.sig_set->get_catchain_seqno(), is_final = broadcast.sig_set->is_final(),
             sig_set = std::move(sig_set)](td::Result<td::Unit> R) mutable {
    if (R.is_ok() && is_final) {
      td::actor::ask(SelfId, &ValidatorManagerImpl::validated_accepted_block_broadcast, block_id, cc_seqno).detach();
      if (!block_id.is_masterchain()) {
        td::actor::ask(SelfId, &ValidatorManagerImpl::generate_shard_block_description, block_id, std::move(sig_set))
            .detach();
      }
    }
...
td::actor::Task<> ValidatorManagerImpl::generate_shard_block_description(BlockIdExt block_id,
  ...
  if (!shard_client_handle_ || shard_client_handle_->unix_time() <= (UnixTime)td::Clocks::system() - 60) {
    VLOG(VALIDATOR_INFO) << "Failed to generate shard block description for " << block_id.to_str() << " : out of sync";
    co_return td::Unit{};
  }
  auto shard_desc = last_masterchain_state_->get_shard_from_config(block_id.shard_full());
  if (shard_desc.not_null() && shard_desc->top_block_id().seqno() >= block_id.seqno()) {
    VLOG(VALIDATOR_INFO) << "Don't generate shard block description for " << block_id.to_str() << " : too old block";
    co_return td::Unit{};
  }
  ...
  auto r_desc =
      co_await validator::generate_shard_block_description(block_id, sig_set, td::Timestamp::in(30.0), actor_id(this))
          .wrap();
  if (r_desc.is_error()) {
    VLOG(VALIDATOR_WARNING) << "Failed to generate shard block description for " << block_id.to_str() << " : "
                            << r_desc.error();
    co_return td::Unit{};
  }
  VLOG(VALIDATOR_DEBUG) << "Generated shard block description for " << block_id.to_str();
...
td::actor::Task<> ValidatorManagerImpl::new_block_candidate_broadcast(BlockIdExt block_id, CatchainSeqno cc_seqno,
  ...
  if (!db_event_publisher_.empty() && is_valid_nonfinal_group(block_id.shard_full(), cc_seqno)) {
    ...
    td::actor::ask(db_event_publisher_, &DbEventPublisher::publish,
                   create_tl_object<tos_api::db_event_blockCandidateReceived>(create_tl_block_id(block_id)))
        .detach();
  }
  co_return td::Unit{};
}
...
static td::actor::Task<> check_finality_signatures(BlockIdExt block_id, Ref<block::BlockSignatureSet> sig_set,
                                                   std::vector<Ref<block::ValidatorSet>> validator_sets) {
  ...
  td::Status last_error = td::Status::Error("no validator set for claimed catchain seqno");
  for (auto &validator_set : validator_sets) {
    auto status = try_val_set(std::move(validator_set));
    if (status.is_ok()) {
      co_return td::Unit{};
    }
...
td::actor::Task<> ValidatorManagerImpl::new_block_finality_broadcast(BlockFinalityBroadcast finality,
  ...
  if (finality.sig_set.is_null()) {
    VLOG(VALIDATOR_WARNING) << "dropping block finality broadcast without signatures: " << finality.block_id.to_str();
    co_return td::Unit{};
  }
  ...
  const auto shard = finality.block_id.shard_full();
  const auto claimed_catchain_seqno = finality.sig_set->get_catchain_seqno();
  const auto claimed_validator_set_hash = finality.sig_set->get_validator_set_hash();
  const auto current_catchain_seqno = state->get_shard_cc_seqno(shard);
  const std::vector<PendingFinalityAuthoritySet> *authority_sets = nullptr;
  if ((source_peer || !finality.sig_set->is_pq()) &&
      pending_finality_coordinate_is_admissible(state->get_shard_from_config(shard, true).not_null(),
                                                current_catchain_seqno, claimed_catchain_seqno)) {
    authority_sets = &pending_finality_authority_memo_.get({shard, claimed_catchain_seqno}, [&] {
      std::vector<PendingFinalityAuthoritySet> sets;
      auto append_matching_set = [&](const Ref<block::ValidatorSet> &set) {
        if (set.is_null() || set->get_catchain_seqno() != claimed_catchain_seqno) {
          return;
        }
        ...
        sets.push_back({set->get_validator_set_hash(), canonical_validator_transport_roots(std::move(roots)), set});
      };
      append_matching_set(state->get_next_validator_set(shard, claimed_catchain_seqno));
      append_matching_set(state->get_validator_set(shard, claimed_catchain_seqno));
      return sets;
    });
  }
  ...
  bool remote_validator_capacity = false;
  std::vector<Ref<block::ValidatorSet>> validator_sets;
  if (authority_sets != nullptr) {
    if (!finality.sig_set->is_pq()) {
      validator_sets.reserve(authority_sets->size());
    }
    for (const auto &set : *authority_sets) {
      if (!finality.sig_set->is_pq()) {
        validator_sets.push_back(set.validator_set);
      }
      if (source_peer && set.validator_set_hash == claimed_validator_set_hash &&
          std::find(set.roots.begin(), set.roots.end(), *source_peer) != set.roots.end()) {
        remote_validator_capacity = true;
      }
    }
  }
  if (!finality.sig_set->is_pq()) {
  ...
  std::optional<std::size_t> local_signature_bytes;
  if (!source_peer) {
    ...
    local_signature_bytes = signature_bytes.move_as_ok();
  }
  auto ingress = prepare_pending_finality_ingress(source_peer ? &*source_peer : nullptr, finality.received_bytes,
  ...
  // reference into the actor-owned LRU is retained across co_await.
  bool validator_capacity = ingress.sender.local || remote_validator_capacity;
  ...
  // and next, and an exact-shard lookup rejects invented descendants which
  // inherit their containing shard's cc_seqno. The attacker-claimed set hash is
  // compared with the computed hash stored inside an entry and cannot create
  // cache keys. A global eight-entry LRU bounds memory even when arrivals cycle
  ...
  if (signatures_verified && !block_id.is_masterchain() && incoming_is_final) {
    process_accepted_nonfinal_block(block_id, finality.sig_set->get_catchain_seqno()).start().detach();
  }
...
void ValidatorManagerImpl::try_process_pending_block_finality(BlockIdExt block_id) {
  ...
  if (block.is_error()) {
    VLOG(VALIDATOR_WARNING) << "failed to parse pending block candidate " << block_id.to_str() << ": "
                            << block.move_as_error();
    if (block_id.is_masterchain()) {
  ...
  class Cb : public adnl::Adnl::Callback {
   private:
    ...
    void receive_query(adnl::AdnlNodeIdShort src, adnl::AdnlNodeIdShort dst, td::BufferSlice data,
      ...
      td::actor::send_closure(id_, &ValidatorManagerImpl::run_ext_query, src, std::move(data), std::move(promise));
    }
...
void ValidatorManagerImpl::run_ext_query(adnl::AdnlNodeIdShort source, td::BufferSlice data,
  ...
  td::optional<tl_object_ptr<lite_api::liteServer_waitMasterchainSeqno>> wait;
  if (E.is_ok()) {
    wait = E.move_as_ok();
  }

  if (data.size() < sizeof(td::int32)) {
    promise.set_error(td::Status::Error(ErrorCode::protoviolation, "liteserver query has no function tag"));
    return;
  }
...
void ValidatorManagerImpl::finish_ext_query(td::Result<td::BufferSlice> result, td::Promise<td::BufferSlice> promise) {
  lite_server_admission_.release();
  td::BufferSlice data;
  if (result.is_error()) {
    auto status = result.move_as_error();
    data = create_serialize_tl_object<lite_api::liteServer_error>(status.code(), status.message().c_str());
  } else {
    data = result.move_as_ok();
  }
...
void ValidatorManagerImpl::create_celldb_streaming_writer_unsafe_for_tests_only(
  ...
  // forwards to the Db actor (RootDb), which routes through CellDb
  // to CellDbIn::create_streaming_writer_unsafe_for_tests_only_async.
  // The writer is paired with a CellDbReader snapshot at the call
  // site and drives
...
void ValidatorManagerImpl::import_persistent_state_streaming(PersistentStateImportRequest request,
  ...
  td::actor::send_closure(db_, &Db::import_persistent_state_streaming, std::move(request), std::move(promise));
}
...
void ValidatorManagerImpl::new_block_cont(BlockHandle handle, td::Ref<ShardState> state,
  ...
  if (!db_event_publisher_.empty() && !handle->id().is_masterchain()) {
    ...
    td::actor::ask(db_event_publisher_, &DbEventPublisher::publish,
                   create_tl_object<tos_api::db_event_blockApplied>(create_tl_block_id(handle->id())))
        .detach();
  }
}
...
class StatsCallback final : public tos::stats::Callback {
 public:
 ...
 private:
  class Recorder final : public tos::stats::Recorder {
   public:
    Recorder(td::actor::ActorId<ValidatorManagerImpl> manager) : manager_(manager) {
    }

    void add(const tl_object_ptr<tos_api::Object> &data) override {
...
td::actor::Task<> ValidatorManagerImpl::finish_start_up() {
  ...
  if (S.is_error()) {
    LOG(ERROR) << "Initial advancing of masterchain: " << S.move_as_error();
  }
  if (!out_of_sync()) {
  ...
  if (allow_validate_) {
    for (const auto &[shard, prev] : new_shards) {
      ...
      for (auto local_adnl_id : observer_adnl_ids) {
        ...
        if (!entry.started) {
          LOG(INFO) << "Started observer group " << shard.to_str() << "." << entry.cc_seqno << " at " << local_adnl_id;
          td::actor::send_closure(entry.actor, &IValidatorGroup::start, prev, last_masterchain_block_id_);
          entry.started = true;
        }
...
void ValidatorManagerImpl::update_shard_client_block_handle(BlockHandle handle, td::Ref<MasterchainState> state,
  ...
  if (!db_event_publisher_.empty()) {
    ...
    td::actor::ask(db_event_publisher_, &DbEventPublisher::publish,
                   create_tl_object<tos_api::db_event_blockApplied>(create_tl_block_id(shard_client_handle_->id())))
        .detach();
  }
  for (auto &c : collator_nodes_) {
...
void ValidatorManagerImpl::get_collation_manager_stats(
    td::Promise<tl_object_ptr<tos_api::engine_validator_collationManagerStats>> promise) {
  class Cb : public td::actor::Actor {
   public:
    explicit Cb(td::Promise<tl_object_ptr<tos_api::engine_validator_collationManagerStats>> promise)
        : promise_(std::move(promise)) {
    }

    void got_stats(tl_object_ptr<tos_api::engine_validator_collationManagerStats_localId> s) {
    ...
    void dec_pending() {
      ...
      --pending_;
      if (pending_ == 0) {
        promise_.set_result(create_tl_object<tos_api::engine_validator_collationManagerStats>(std::move(result_)));
        stop();
      }
    }

   private:
    td::Promise<tl_object_ptr<tos_api::engine_validator_collationManagerStats>> promise_;
    size_t pending_ = 1;
    std::vector<tl_object_ptr<tos_api::engine_validator_collationManagerStats_localId>> result_;
  };
  ...
  for (auto &[_, actor] : collation_managers_) {
    ...
    td::actor::send_closure(
        ...
        [callback](td::Result<tl_object_ptr<tos_api::engine_validator_collationManagerStats_localId>> R) {
          if (R.is_error()) {
