...
void FullNodeFastSyncOverlay::process_telemetry_broadcast(
    adnl::AdnlNodeIdShort src, const tl_object_ptr<tos_api::validator_telemetry> &telemetry) {
  ...
  class Callback : public overlay::Overlays::Callback {
   public:
    ...
    void receive_query(adnl::AdnlNodeIdShort src, overlay::OverlayIdShort overlay_id, td::BufferSlice data,
      ...
      // Match custom overlays: return an explicit unsupported-query error.
  ...
  if (shard_.is_masterchain()) {
    class TelemetryCallback : public ValidatorTelemetry::Callback {
     public:
      ...
      void send_telemetry(tl_object_ptr<tos_api::validator_telemetry> telemetry) override {
...
void FullNodeFastSyncOverlay::get_stats_extra(td::Promise<std::string> promise) {
  auto res = create_tl_object<tos_api::engine_validator_fastSyncOverlayStats>();
