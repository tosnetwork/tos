...
class ValidatorManagerImpl : public ValidatorManager {
 ...
 public:
  ...
  void get_node_consensus_status(td::Promise<NodeConsensusStatus> promise) override;
  ...
  ValidatorManagerImpl(PublicKeyHash local_id, td::Ref<ValidatorManagerOptions> opts, ShardIdFull shard_id,
                       ...
                       std::string export_candidate, std::string import_candidate, std::string query_result_path)
      : local_id_(local_id)
      , opts_(std::move(opts))
      , db_root_(db_root)
      ...
      , query_result_path_(std::move(query_result_path)) {
  }
 ...
 public:
  ...
  void get_collation_manager_stats(
      td::Promise<tl_object_ptr<tos_api::engine_validator_collationManagerStats>> promise) override {
    UNREACHABLE();
  }
