...
td::Status CollatorsList::unpack(const tos_api::engine_validator_collatorsList& obj) {
  shards.clear();
  self_collate = false;
...
td::Status ShardBlockVerifierConfig::unpack(const tos_api::engine_validator_shardBlockVerifierConfig& obj) {
  shards.clear();
  for (const auto& shard_obj : obj.shards_) {
    Shard shard;
