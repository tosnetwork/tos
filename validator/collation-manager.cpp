...
void CollationManager::collate_shard_block(ShardIdFull shard, BlockIdExt min_masterchain_block_id,
  ...
  td::Promise<td::BufferSlice> P2 = [=, SelfId = actor_id(this), P = std::move(P),
    ...
    auto r_error = fetch_tl_object<tos_api::collatorNode_error>(data, true);
    if (r_error.is_ok()) {
...
void CollationManager::get_stats(
    td::Promise<tl_object_ptr<tos_api::engine_validator_collationManagerStats_localId>> promise) {
  auto stats = create_tl_object<tos_api::engine_validator_collationManagerStats_localId>();
  stats->adnl_id_ = local_id_.bits256_value();
  for (ShardInfo& s : shards_) {
    auto obj = create_tl_object<tos_api::engine_validator_collationManagerStats_shard>();
  ...
  for (auto& [id, collator] : collators_) {
    auto obj = create_tl_object<tos_api::engine_validator_collationManagerStats_collator>();
  ...
  for (auto& [id, collator] : collators_) {
    ...
    if (collator.ping_at.is_in_past()) {
      ...
      td::BufferSlice query =
          create_serialize_tl_object<tos_api::collatorNode_ping>(tos_api::collatorNode_pong::VERSION_MASK);
...
void CollationManager::got_pong(adnl::AdnlNodeIdShort id, td::Result<td::BufferSlice> R) {
  ...
  auto r_pong = [&]() -> td::Result<tl_object_ptr<tos_api::collatorNode_pong>> {
    ...
    return fetch_tl_object<tos_api::collatorNode_pong>(data, true);
  }();
  ...
  if (r_pong.is_error()) {
    ...
    collator.version = pong->flags_ & tos_api::collatorNode_pong::VERSION_MASK ? pong->version_ : 0;
