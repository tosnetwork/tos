...
struct Neighbour {
  ...
  void update_proto_version(tos_api::tosNode_capabilities &q);
...
class FullNodeShardImpl : public FullNodeShard {
 public:
  ...
  void process_query(adnl::AdnlNodeIdShort src, tos_api::tosNode_getArchiveSlice &query,
                     td::Promise<td::BufferSlice> promise);
  void process_query(adnl::AdnlNodeIdShort src, tos_api::tosNode_getOutMsgQueueProof &query,
                     td::Promise<td::BufferSlice> promise);
  ...
  void process_broadcast(PublicKeyHash src, tos_api::tosNode_ihrMessageBroadcast &query);
  ...
  void sign_overlay_certificate(PublicKeyHash signed_key, td::uint32 expiry_at, td::uint32 max_size,
                                td::Promise<td::BufferSlice> promise) override;
  void import_overlay_certificate(PublicKeyHash signed_key, std::shared_ptr<tos::overlay::Certificate> cert,
  ...
  bool has_valid_certificate_for_source(const PublicKeyHash &source,
                                        const std::shared_ptr<tos::overlay::Certificate> &cert, td::uint32 payload_size,
                                        bool is_fec) const;
 ...
 private:
  ...
  std::shared_ptr<tos::overlay::Certificate> cert_;
  std::shared_ptr<tos::overlay::Certificate> adnl_source_cert_;
