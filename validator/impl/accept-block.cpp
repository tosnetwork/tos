...
void AcceptBlockQuery::written_block_data() {
  ...
  if (handle_->inited_signatures() || is_fake_ || !signatures_->is_final() || is_fork_) {
...
void AcceptBlockQuery::find_known_ancestors() {
  ...
  CHECK(config);
  auto shard = tos::ShardIdFull(id_);
  auto ancestor = config->get_shard_hash(shard, false);
  if (ancestor.is_null()) {
    ancestor = config->get_shard_hash(tos::shard_child(shard, true));
    auto ancestor2 = config->get_shard_hash(tos::shard_child(shard, false));
    ...
    ancestors_.emplace_back(std::move(ancestor));
  } else if (tos::shard_is_parent(ancestor->shard(), shard)) {
...
void AcceptBlockQuery::require_proof_link(BlockIdExt id) {
  ...
  CHECK(tos::ShardIdFull(id) == tos::ShardIdFull(id_));
...
void AcceptBlockQuery::got_proof_link(BlockIdExt id, Ref<ProofLink> proof) {
  ...
  if (id.id.seqno == ancestors_seqno_ + 1) {
    ...
    // intermediate link
    if (link_prev_.size() != 1 || tos::ShardIdFull(link_prev_[0].id) != tos::ShardIdFull(id_) ||
...
void AcceptBlockQuery::send_broadcasts() {
  ...
  VLOG(VALIDATOR_DEBUG) << "send_broadcasts block_mode=" << block_broadcast_mode_
