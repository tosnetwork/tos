...
ValidateQuery::ValidateQuery(BlockCandidate candidate, ValidateParams params,
    ...
    , shard_pfx_len_(tos::shard_prefix_length(shard_))
...
bool ValidateQuery::init_parse() {
  ...
  int shard_pfx_len = tos::shard_prefix_length(shard);
...
bool ValidateQuery::compute_next_state() {
  ...
  tos::BlockId hdr_id{tos::ShardIdFull(id), info.seq_no};
  if (hdr_id != id_.id) {
...
bool ValidateQuery::unpack_one_prev_state(block::ShardState& ss, BlockIdExt blkid, Ref<vm::Cell> prev_state_root) {
  auto res = ss.unpack_state_ext(blkid, std::move(prev_state_root), global_id_, mc_seqno_, after_split_,
                                 after_split_ | after_merge_, [this](tos::BlockSeqno mc_seqno) {
...
bool ValidateQuery::unpack_next_state() {
  ...
  auto res = ns_.unpack_state_ext(id_, state_root_, global_id_, mc_seqno_, before_split_, false,
                                  [](tos::BlockSeqno mc_seqno) { return true; });
  if (res.is_error()) {
...
bool ValidateQuery::request_neighbor_queues() {
  ...
  for (tos::BlockId blk_id : neighbor_list) {
    ...
    auto shard_ptr = new_shard_conf_->get_shard_hash(tos::ShardIdFull(blk_id));
...
void ValidateQuery::got_neighbor_out_queue(int i, td::Result<Ref<MessageQueue>> res, td::PerfLogAction token) {
  try {
    ...
    do {
      ...
      // for the neighbor shard is tracked as V-008.
 ...
 */
void ValidateQuery::after_get_aux_shard_state(tos::BlockIdExt blkid, td::Result<Ref<ShardState>> res,
                                              td::PerfLogAction token) {
  token.finish(res);
...
bool ValidateQuery::check_one_shard(const block::McShardHash& info, const block::McShardHash* sibling,
  ...
  if (info.next_validator_shard_ != shard.shard) {
    return reject_query("new shard configuration for shard "s + shard.to_str() +
                        ...
                        tos::ShardIdFull{shard.workchain, info.next_validator_shard_}.to_str());
  }
  ...
  unsigned depth = tos::shard_prefix_length(shard);
...
bool ValidateQuery::check_shard_layout() {
  ...
  if (!new_shard_conf_->process_sibling_shard_hashes([self = this, &new_top_shard_blocks, &wc_set, &wc_id, &wc_info,
        ...
        if (wc_id != cur.workchain()) {
          wc_id = cur.workchain();
          if (wc_id == tos::workchainInvalid || wc_id == tos::masterchainId) {
 ...
 */
bool ValidateQuery::fix_one_processed_upto(block::MsgProcessedUpto& proc, tos::ShardIdFull owner, bool allow_cur) {
    ...
    if (tos::shard_intersects(nb.shard(), shard_)) {
      ...
      if (nb.shard() == prev_shard) {
        if (prev_shard == shard_) {
          ...
          nb2.blk_.id.shard = tos::shard_sibling(shard_.shard);
...
bool ValidateQuery::unpack_precheck_value_flow(Ref<vm::Cell> value_flow_root) {
  ...
  if (is_masterchain()) {
    ...
    create_fee = (basechain_create_fee_ >> tos::shard_prefix_length(shard_));
  } else {
 ...
 */
bool ValidateQuery::precheck_one_transaction(td::ConstBitPtr acc_id, tos::LogicalTime trans_lt,
                                             Ref<vm::CellSlice> trans_csr, tos::Bits256& prev_trans_hash,
                                             tos::LogicalTime& prev_trans_lt, unsigned& prev_trans_lt_len,
                                             tos::Bits256& acc_state_hash) {
...
bool ValidateQuery::precheck_one_account_block(td::ConstBitPtr acc_id, Ref<vm::CellSlice> acc_blk_root) {
  ...
  tos::Bits256 acc_state_hash = hash_upd.old_hash;
  try {
 ...
 */
Ref<vm::Cell> ValidateQuery::lookup_transaction(const tos::StdSmcAddress& addr, tos::LogicalTime lt) const {
...
bool ValidateQuery::precheck_one_message_queue_update(td::ConstBitPtr out_msg_id, Ref<vm::CellSlice> old_value,
  ...
  if (new_value.not_null()) {
    ...
    tos::LogicalTime enqueued_lt = new_value->prefetch_ulong(64);
  ...
  if (old_value.not_null()) {
    ...
    tos::LogicalTime enqueued_lt = old_value->prefetch_ulong(64);
 ...
 */
bool ValidateQuery::update_max_processed_lt_hash(tos::LogicalTime lt, const tos::Bits256& hash) {
 ...
 * @returns True if the update was successful, false otherwise.
 */
bool ValidateQuery::update_min_enqueued_lt_hash(tos::LogicalTime lt, const tos::Bits256& hash) {
...
bool ValidateQuery::check_in_msg(td::ConstBitPtr key, Ref<vm::CellSlice> in_msg) {
  ...
  tos::StdSmcAddress src_addr, dest_addr;
  tos::WorkchainId src_wc, dest_wc;
  ...
  if (tag != block::gen::InMsg::msg_import_ext) {
    ...
    if (transaction.not_null() && !tos::shard_contains(shard_, dest_prefix)) {
    ...
    if (tag != block::gen::InMsg::msg_import_deferred_tr && transaction.is_null() &&
        tos::shard_contains(shard_, dest_prefix)) {
      return reject_query("inbound internal message with hash "s + key.to_hex(256) + " has destination address " +
...
bool ValidateQuery::check_out_msg(td::ConstBitPtr key, Ref<vm::CellSlice> out_msg) {
  ...
  tos::AccountIdPrefixFull src_prefix, dest_prefix, cur_prefix, next_prefix;
  td::RefInt256 fwd_fee, orig_fwd_fee;
  tos::LogicalTime import_lt = ~0ULL;
  unsigned long long created_lt = 0;
  int mode = 0, in_tag = -2;
  bool is_short = false;
  // initial checks and unpack
  switch (tag) {
    case block::gen::OutMsg::msg_export_ext: {
      ...
      if (!tos::shard_contains(shard_, src_prefix)) {
  ...
  if (is_short) {
    ...
    if (tag == block::gen::OutMsg::msg_export_new_defer) {
      ...
      if (!tos::shard_contains(shard_, cur_prefix)) {
    ...
    if (transaction.not_null() && !tos::shard_contains(shard_, src_prefix)) {
 ...
 */
bool ValidateQuery::check_neighbor_outbound_message(Ref<vm::CellSlice> enq_msg, tos::LogicalTime lt,
...
bool ValidateQuery::CheckAccountTxs::try_check() {
  try {
    ...
    tos::LogicalTime min_trans_lt = min_trans.to_ulong(), max_trans_lt = max_trans.to_ulong();
    if (!trans_dict.check_for_each_extra(
            [this, &account, min_trans_lt, max_trans_lt](Ref<vm::CellSlice> value, Ref<vm::CellSlice> extra,
              ...
              REJECT_UNLESS(key_len == 64);
              tos::LogicalTime lt = key.get_uint(64);
              extra.clear();
...
bool ValidateQuery::check_new_state() {
  ...
  tos::BlockSeqno my_mc_seqno = is_masterchain() ? id_.seqno() : mc_seqno_;
  tos::BlockSeqno ref_mc_seqno =
 ...
 */
bool ValidateQuery::check_one_prev_dict_update(tos::BlockSeqno seqno, Ref<vm::CellSlice> old_val_extra,
                                               Ref<vm::CellSlice> new_val_extra) {
