...
td::Status ShardStateQ::init() {
  ...
  if (!id.is_valid() || get_shard() != tos::ShardIdFull(id) || get_seqno() != info.seq_no) {
...
td::Result<td::Ref<ShardState>> ShardStateQ::merge_with(const ShardState& with) const {
  ...
  if (shard1 == shard2 || !tos::shard_is_sibling(shard1, shard2)) {
  ...
  auto m = Ref<ShardStateQ>{
      true,
      tos::BlockIdExt{blkid.id.workchain, tos::shard_parent(blkid.id.shard),
                      std::max(blkid.seqno(), other.blkid.seqno()), tos::Bits256::zero(), tos::Bits256::zero()},
      root};
...
td::Result<std::pair<td::Ref<ShardState>, td::Ref<ShardState>>> ShardStateQ::split() const {
  ...
  ls.blkid.id.shard = tos::shard_child(blkid.id.shard, true);
  rs.blkid.id.shard = tos::shard_child(blkid.id.shard, false);
...
Ref<block::ValidatorSet> MasterchainStateQ::get_total_validator_set(int next) const {
  ...
  return Ref<block::ValidatorSet>{true, 0, tos::ShardIdFull{}, std::move(nodes)};
}
...
std::vector<Ref<McShardHash>> MasterchainStateQ::get_shards() const {
  ...
  std::vector<tos::BlockId> shard_ids = config_->get_shard_hash_ids(true);
  std::vector<Ref<McShardHash>> v;
  for (const auto& b : shard_ids) {
    v.emplace_back(config_->get_shard_hash(tos::ShardIdFull(b)));
...
bool MasterchainStateQ::get_old_mc_block_id(tos::BlockSeqno seqno, tos::BlockIdExt& blkid,
                                            tos::LogicalTime* end_lt) const {
  return config_ && config_->get_old_mc_block_id(seqno, blkid, end_lt);
}

bool MasterchainStateQ::check_old_mc_block_id(const tos::BlockIdExt& blkid, bool strict) const {
