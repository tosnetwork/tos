 ...
 */
std::string show_shard(tos::WorkchainId workchain, tos::ShardId shard) {
  char tmp[128];
 ...
 * @returns A string representation of the shard.
 */
std::string show_shard(const tos::BlockId blk_id) {
 ...
 */
std::string show_shard(const tos::ShardIdFull blk_id) {
  return show_shard(blk_id.workchain, blk_id.shard);
}
 ...
 */
void Collator::after_get_aux_shard_state(tos::BlockIdExt blkid, td::Result<Ref<ShardState>> res,
...
bool Collator::unpack_last_mc_state() {
  ...
  report_version_ = config_->has_capability(tos::capReportVersion);
  short_dequeue_records_ = config_->has_capability(tos::capShortDequeue);
  store_out_msg_queue_size_ = config_->has_capability(tos::capStoreOutMsgQueueSize);
  msg_metadata_enabled_ = config_->has_capability(tos::capMsgMetadata);
  deferring_messages_enabled_ = config_->has_capability(tos::capDeferMessages);
...
bool Collator::request_neighbor_msg_queues() {
  ...
  for (tos::BlockId blk_id : neighbor_list) {
    if (blk_id.seqno == 0 && blk_id.shard_full() != shard_) {
      continue;
    }
    auto shard_ptr = shard_conf_->get_shard_hash(tos::ShardIdFull(blk_id));
...
void Collator::got_neighbor_msg_queue(unsigned i, Ref<OutMsgQueueProof> res) {
  ...
  // the neighbor shard is tracked as V-009.
...
bool Collator::unpack_one_last_state(block::ShardState& ss, BlockIdExt blkid, Ref<vm::Cell> prev_state_root) {
  auto res = ss.unpack_state_ext(blkid, std::move(prev_state_root), global_id_, prev_mc_block_seqno, after_split_,
                                 after_split_ | after_merge_, [self = this](tos::BlockSeqno mc_seqno) {
                                   Ref<MasterchainStateQ> state;
...
bool Collator::split_last_state(block::ShardState& ss) {
  ...
  CHECK(after_split_);
  auto sib_shard = tos::shard_sibling(shard_);
...
bool Collator::add_trivial_neighbor_after_merge() {
  ...
  for (std::size_t i = 0; i < n; i++) {
    ...
    if (tos::shard_intersects(nb.shard(), shard_)) {
      ++found;
      LOG(DEBUG) << "neighbor #" << i << " : " << nb.blk_.to_str() << " intersects our shard " << shard_.to_str();
      if (!tos::shard_is_parent(shard_, nb.shard()) || found > 2) {
...
bool Collator::add_trivial_neighbor() {
  ...
  tos::ShardIdFull prev_shard = descr_ref->shard();
  // Possible cases are:
    ...
    if (tos::shard_intersects(nb.shard(), shard_)) {
      ...
      if (nb.shard() == prev_shard) {
        if (prev_shard == shard_) {
          ...
          cs = 1;
        } else if (tos::shard_is_parent(nb.shard(), shard_)) {
          ...
          nb2.blk_.id.shard = tos::shard_sibling(get_shard());
      ...
      } else if (tos::shard_is_parent(nb.shard(), shard_) && shard_ == prev_shard) {
        ...
        int l = tos::shard_prefix_length(shard_);
        ...
        cs = 3;
      } else if (tos::shard_is_parent(shard_, nb.shard()) && shard_ == prev_shard) {
...
bool Collator::do_preinit() {
  ...
  new_id = tos::BlockId{shard_, new_block_seqno};
  CHECK(!config_);
...
bool Collator::adjust_shard_config() {
  ...
  for (const auto& wpair : wset) {
    tos::WorkchainId wc = wpair.first;
...
bool Collator::import_new_shard_top_blocks() {
  ...
  ShardIdFull prev_shard{tos::workchainInvalid, ~0ULL};
 ...
 */
bool Collator::fix_one_processed_upto(block::MsgProcessedUpto& proc, const tos::ShardIdFull& owner) {
  ...
  if (state.is_null()) {
    return fatal_error(
        -666, PSTRING() << "cannot obtain masterchain state with seqno " << seqno << " (originally required "
                        ...
                        << tos::ShardIdFull{owner.workchain, proc.shard}.to_str() << " owned by " << owner.to_str());
  }
...
bool Collator::init_lt() {
  ...
  tos::LogicalTime align = config_->get_lt_align(), incr = align - start_lt % align;
 ...
 */
bool Collator::dequeue_message(Ref<vm::Cell> msg_envelope, tos::LogicalTime delivered_lt) {
 ...
 */
void Collator::update_max_lt(tos::LogicalTime lt) {
  CHECK(lt >= start_lt);
  if (lt > max_lt) {
    max_lt = lt;
  }
 ...
 */
bool Collator::update_last_proc_int_msg(const std::pair<tos::LogicalTime, tos::Bits256>& new_lt_hash) {
...
bool Collator::create_ticktock_transactions(int mask) {
  tos::LogicalTime req_lt = mask == 2 ? start_lt + 1 : max_lt;
 ...
 */
bool Collator::is_our_address(tos::AccountIdPrefixFull addr_pfx) const {
  return tos::shard_contains(shard_, addr_pfx);
}
 ...
 * @returns True if the address belongs to the current shard, False otherwise.
 */
bool Collator::is_our_address(const tos::StdSmcAddress& addr) const {
  return tos::shard_contains(get_shard(), addr);
}
...
int Collator::process_one_new_message(block::NewOutMsg msg, bool enqueue_only, Ref<vm::Cell>* is_special) {
  ...
  if (!is_special &&
      !update_last_proc_int_msg(std::pair<tos::LogicalTime, tos::Bits256>(msg.lt, msg.msg->get_hash().bits()))) {
...
bool Collator::enqueue_transit_message(Ref<vm::Cell> msg, Ref<vm::Cell> old_msg_env,
                                       tos::AccountIdPrefixFull prev_prefix, tos::AccountIdPrefixFull cur_prefix,
                                       tos::AccountIdPrefixFull dest_prefix, td::RefInt256 fwd_fee_remaining,
...
bool Collator::precheck_inbound_message(Ref<vm::CellSlice> enq_msg, tos::LogicalTime lt) {
  tos::LogicalTime enqueued_lt = 0;
 ...
 */
bool Collator::process_inbound_message(Ref<vm::CellSlice> enq_msg, tos::LogicalTime lt, td::ConstBitPtr key,
                                       int src_nb_idx) {
  const auto& src_nb = neighbors_.at(src_nb_idx);
  tos::LogicalTime enqueued_lt = enq_msg->prefetch_ulong(64);
  auto msg_env = enq_msg->prefetch_ref();
  ...
  if (!update_last_proc_int_msg(std::pair<tos::LogicalTime, tos::Bits256>(lt, env.msg->get_hash().bits()))) {
  ...
  if (!tos::shard_contains(src_nb.shard(), cur_prefix)) {
  ...
  bool our = tos::shard_contains(shard_, cur_prefix);
  bool to_us = tos::shard_contains(shard_, dest_prefix);
 ...
 */
bool store_ext_blk_ref_to(vm::CellBuilder& cb, const tos::BlockIdExt& id_ext, tos::LogicalTime end_lt) {
 ...
 * @returns True if the reference was successfully stored, false otherwise.
 */
bool store_ext_blk_ref_to(vm::CellBuilder& cb, const tos::BlockIdExt& id_ext, Ref<vm::Cell> blk_root) {
...
static int update_one_shard(block::McShardHash& info, const block::McShardHash* sibling,
                            const block::WorkchainInfo* wc_info, tos::UnixTime now,
  ...
  if (wc_info && !info.before_split_) {
    ...
    unsigned depth = tos::shard_prefix_length(info.shard());
...
bool Collator::update_shard_config(const block::WorkchainSet& wc_set, const block::CatchainValidatorsConfig& ccvc,
  ...
  WorkchainId wc_id{tos::workchainInvalid};
  Ref<block::WorkchainInfo> wc_info;
  tos::BlockSeqno& min_seqno = min_ref_mc_seqno_;
...
bool Collator::create_mc_state_extra() {
  ...
  if (cfg0.not_null()) {
    tos::StdSmcAddress new_config_addr;
 ...
 */
bool Collator::try_fetch_new_config(const tos::StdSmcAddress& cfg_addr, Ref<vm::Cell>& new_config) {
 ...
 */
bool Collator::update_min_mc_seqno(tos::BlockSeqno some_mc_seqno) {
...
bool Collator::create_block_candidate() {
  ...
  auto new_block_id_ext = tos::BlockIdExt{tos::BlockId{shard_, new_block_seqno}, new_block->get_hash().bits(),
...
td::Status Collator::register_external_message(Ref<ExtMessage> ext_msg, int priority) {
  ...
  if (!tos::shard_contains(shard_, dest_prefix)) {
