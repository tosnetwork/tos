...
void LiteQuery::run_query(td::BufferSlice data, td::actor::ActorId<ValidatorManager> manager,
  ...
  td::actor::create_actor<LiteQuery>("litequery", std::move(data), std::move(manager), std::move(cache),
                                     std::move(source), std::move(promise))
      .release();
}
...
void LiteQuery::start_up() {
  ...
  auto F = fetch_tl_object<tos::lite_api::Function>(query_, true);
  if (F.is_error()) {
  ...
  lite_api::downcast_call(
      *query_obj_,
      td::overloaded(
          ...
          [&](lite_api::liteServer_getBlock& q) { this->perform_getBlock(tos::create_block_id(q.id_)); },
          [&](lite_api::liteServer_getBlockHeader& q) {
            this->perform_getBlockHeader(tos::create_block_id(q.id_), q.mode_);
          },
          [&](lite_api::liteServer_getState& q) { this->perform_getState(tos::create_block_id(q.id_)); },
          [&](lite_api::liteServer_getAccountState& q) {
            this->perform_getAccountState(tos::create_block_id(q.id_), static_cast<WorkchainId>(q.account_->workchain_),
                                          q.account_->id_, 0);
          },
          ...
          [&](lite_api::liteServer_getOneTransaction& q) {
            this->perform_getOneTransaction(tos::create_block_id(q.id_),
          ...
          [&](lite_api::liteServer_getShardInfo& q) {
            this->perform_getShardInfo(tos::create_block_id(q.id_),
                                       ShardIdFull{q.workchain_, static_cast<ShardId>(q.shard_)}, q.exact_);
          },
          [&](lite_api::liteServer_getAllShardsInfo& q) {
            this->perform_getAllShardsInfo(tos::create_block_id(q.id_));
          },
          [&](lite_api::liteServer_lookupBlock& q) {
            this->perform_lookupBlock(tos::create_block_id_simple(q.id_), q.mode_, q.lt_, q.utime_);
          },
          [&](lite_api::liteServer_lookupBlockWithProof& q) {
            this->perform_lookupBlockWithProof(tos::create_block_id_simple(q.id_), tos::create_block_id(q.mc_block_id_),
                                               q.mode_, q.lt_, q.utime_);
          },
          [&](lite_api::liteServer_listBlockTransactions& q) {
            this->perform_listBlockTransactions(tos::create_block_id(q.id_), q.mode_, q.count_,
          ...
          [&](lite_api::liteServer_listBlockTransactionsExt& q) {
            this->perform_listBlockTransactionsExt(tos::create_block_id(q.id_), q.mode_, q.count_,
                                                   (q.mode_ & 128) ? q.after_->account_ : td::Bits256::zero(),
                                                   static_cast<LogicalTime>((q.mode_ & 128) ? (q.after_->lt_) : 0));
          },
          [&](lite_api::liteServer_getConfigParams& q) {
            this->perform_getConfigParams(tos::create_block_id(q.id_), (q.mode_ & 0xffff) | 0x10000, q.param_list_);
          },
          [&](lite_api::liteServer_getConfigAll& q) {
            this->perform_getConfigParams(tos::create_block_id(q.id_), (q.mode_ & 0xffff) | 0x20000);
          },
          [&](lite_api::liteServer_getBlockProof& q) {
            this->perform_getBlockProof(tos::create_block_id(q.known_block_),
                                        q.mode_ & 1 ? tos::create_block_id(q.target_block_) : tos::BlockIdExt{},
                                        q.mode_);
          },
          [&](lite_api::liteServer_getValidatorStats& q) {
            this->perform_getValidatorStats(tos::create_block_id(q.id_), q.mode_, q.limit_,
                                            q.mode_ & 1 ? q.start_after_ : td::Bits256::zero(),
                                            q.mode_ & 4 ? q.modified_after_ : 0);
          },
          [&](lite_api::liteServer_runSmcMethod& q) {
            this->perform_runSmcMethod(tos::create_block_id(q.id_), static_cast<WorkchainId>(q.account_->workchain_),
          ...
          [&](lite_api::liteServer_getLibrariesWithProof& q) {
            this->perform_getLibrariesWithProof(tos::create_block_id(q.id_), q.mode_, q.library_list_);
          },
...
void LiteQuery::perform_getMasterchainInfo(int mode) {
  ...
  td::actor::send_closure_later(manager_, &tos::validator::ValidatorManager::get_last_liteserver_state_block,
                                [Self = actor_id(this), return_state = bool(acc_state_promise_),
                                 mode](td::Result<std::pair<Ref<tos::validator::MasterchainState>, BlockIdExt>> res) {
                                  if (res.is_error()) {
...
void LiteQuery::gotMasterchainInfoForAccountState(Ref<tos::validator::MasterchainState> mc_state, BlockIdExt blkid,
                                                  int mode) {
...
void LiteQuery::continue_getMasterchainInfo(Ref<tos::validator::MasterchainState> mc_state, BlockIdExt blkid,
  ...
  auto b = (mode == -1) ? tos::create_serialize_tl_object<tos::lite_api::liteServer_masterchainInfo>(
                              tos::create_tl_lite_block_id(blkid), mc_state_q->root_hash(), std::move(zs_tl))
                        : tos::create_serialize_tl_object<tos::lite_api::liteServer_masterchainInfoExt>(
                              mode, ls_version, ls_capabilities, tos::create_tl_lite_block_id(blkid),
...
void LiteQuery::perform_getBlock(BlockIdExt blkid) {
  ...
  td::actor::send_closure(manager_, &ValidatorManager::get_block_data_for_litequery, blkid,
                          [Self = actor_id(this), blkid](td::Result<Ref<tos::validator::BlockData>> res) {
                            if (res.is_error()) {
                              td::actor::send_closure(Self, &LiteQuery::abort_query, res.move_as_error());
                            } else {
...
void LiteQuery::perform_getBlockHeader(BlockIdExt blkid, int mode) {
  ...
  td::actor::send_closure(manager_, &ValidatorManager::get_block_data_for_litequery, blkid,
                          [Self = actor_id(this), blkid, mode](td::Result<Ref<tos::validator::BlockData>> res) {
                            if (res.is_error()) {
                              td::actor::send_closure(Self, &LiteQuery::abort_query, res.move_as_error());
                            } else {
...
void LiteQuery::continue_getBlockHeader(BlockIdExt blkid, int mode, Ref<tos::validator::BlockData> block) {
  ...
  // send answer
  auto b = tos::create_serialize_tl_object<tos::lite_api::liteServer_blockHeader>(tos::create_tl_lite_block_id(blkid),
...
void LiteQuery::continue_getState(BlockIdExt blkid, Ref<tos::validator::ShardState> state) {
  ...
  auto b = tos::create_serialize_tl_object<tos::lite_api::liteServer_blockState>(
      tos::create_tl_lite_block_id(blkid), state->root_hash(), file_hash, std::move(data));
  finish_query(std::move(b));
}
...
void LiteQuery::perform_sendMessage(td::BufferSlice data) {
  ...
  td::actor::send_closure(
      manager_, &ValidatorManager::new_external_message_query, std::move(data), std::move(source_),
      td::PromiseCreator::lambda(
          [Self = actor_id(this), cache = cache_, cache_key = cache_key_](td::Result<td::Unit> res) mutable {
            if (res.is_error()) {
              ...
              auto b = tos::create_serialize_tl_object<tos::lite_api::liteServer_sendMsgStatus>(1);
...
void LiteQuery::perform_getAccountState(BlockIdExt blkid, WorkchainId workchain, StdSmcAddress addr, int mode) {
  ...
  if (workchain == blkid.id.workchain &&
      !tos::shard_contains(blkid.shard_full(), extract_addr_prefix(workchain, addr))) {
  ...
  if (blkid.id.workchain != masterchainId) {
    ...
    LOG(DEBUG) << "sending a get_last_liteserver_state_block query to manager";
    td::actor::send_closure_later(
        manager_, &tos::validator::ValidatorManager::get_last_liteserver_state_block,
        [Self = actor_id(this)](td::Result<std::pair<Ref<tos::validator::MasterchainState>, BlockIdExt>> res) -> void {
          if (res.is_error()) {
            td::actor::send_closure(Self, &LiteQuery::abort_query, res.move_as_error());
          } else {
            auto pair = res.move_as_ok();
...
void LiteQuery::continue_getAccountState_0(Ref<tos::validator::MasterchainState> mc_state, BlockIdExt blkid) {
  LOG(DEBUG) << "obtained last masterchain block = " << blkid.to_str();
  base_blk_id_ = blkid;
...
void LiteQuery::perform_getLibraries(std::vector<td::Bits256> library_list) {
  ...
  if (library_list.size() > 16) {
    LOG(DEBUG) << "too many libraries requested, returning only first 16";
  ...
  td::actor::send_closure_later(
      manager_, &tos::validator::ValidatorManager::get_last_liteserver_state_block,
      [Self = actor_id(this),
       library_list](td::Result<std::pair<Ref<tos::validator::MasterchainState>, BlockIdExt>> res) -> void {
        if (res.is_error()) {
          td::actor::send_closure(Self, &LiteQuery::abort_query, res.move_as_error());
        } else {
          auto pair = res.move_as_ok();
...
void LiteQuery::continue_getLibraries(Ref<tos::validator::MasterchainState> mc_state, BlockIdExt blkid,
  ...
  if (false) {
    ...
    for (auto k : *lib_dict) {
      ...
      LOG(DEBUG) << "library " << k.first.to_hex(256) << ": \n" << oss.str();
    }
  }

  std::vector<tos::tl_object_ptr<tos::lite_api::liteServer_libraryEntry>> a;
  for (const auto& hash : library_list) {
    LOG(DEBUG) << "looking for library " << hash.to_hex();
    auto libres = config->lookup_library(hash);
    if (libres.is_null()) {
      LOG(DEBUG) << "library lookup result is null";
      continue;
    }
    ...
    a.push_back(tos::create_tl_object<tos::lite_api::liteServer_libraryEntry>(hash, data.move_as_ok()));
  }
  auto b = tos::create_serialize_tl_object<tos::lite_api::liteServer_libraryResult>(std::move(a));
  finish_query(std::move(b));
}
...
void LiteQuery::continue_getLibrariesWithProof(std::vector<td::Bits256> library_list, int mode) {
  LOG(DEBUG) << "obtained masterchain block = " << base_blk_id_.to_str();
  CHECK(mc_state_.not_null());
  ...
  std::vector<tos::tl_object_ptr<tos::lite_api::liteServer_libraryEntry>> result;
  ...
  for (const auto& hash : result_hashes) {
    ...
    result.push_back(tos::create_tl_object<tos::lite_api::liteServer_libraryEntry>(hash, std::move(libdata)));
  }

  auto b = tos::create_serialize_tl_object<tos::lite_api::liteServer_libraryResultWithProof>(
      tos::create_tl_lite_block_id(base_blk_id_), mode, std::move(result), state_proof_boc.move_as_ok(),
...
void LiteQuery::perform_getOneTransaction(BlockIdExt blkid, WorkchainId workchain, StdSmcAddress addr, LogicalTime lt) {
  ...
  if (!tos::shard_contains(blkid.shard_full(), extract_addr_prefix(workchain, addr))) {
...
void LiteQuery::continue_getAccountState() {
  ...
  if (!blkid.is_valid()) {
    ...
    LOG(DEBUG) << "getAccountState(" << acc_workchain_ << ":" << acc_addr_.to_hex()
              << ") query completed (unknown workchain/shard)";
    auto b = tos::create_serialize_tl_object<tos::lite_api::liteServer_accountState>(
        tos::create_tl_lite_block_id(base_blk_id_), tos::create_tl_lite_block_id(blkid), proof.move_as_ok(),
...
void LiteQuery::finish_runSmcMethod(td::BufferSlice shard_proof, td::BufferSlice state_proof, Ref<vm::Cell> acc_root,
  ...
  if (acc_root.is_null()) {
    // no such account
    LOG(DEBUG) << "runSmcMethod(" << acc_workchain_ << ":" << acc_addr_.to_hex()
              << ") query completed: account state is empty";
    auto b = tos::create_serialize_tl_object<tos::lite_api::liteServer_runMethodResult>(
        mode, tos::create_tl_lite_block_id(base_blk_id_), tos::create_tl_lite_block_id(blk_id_), std::move(shard_proof),
  ...
  if (!(tlb::unpack_cell(pb.root(), acc) && tlb::csr_unpack(std::move(acc.storage), store) &&
    ...
    LOG(DEBUG) << "error unpacking account state, or account is frozen or uninitialized";
...
void LiteQuery::continue_getOneTransaction() {
  LOG(DEBUG) << "completing getOneTransaction() query";
  CHECK(block_.not_null());
  ...
  LOG(DEBUG) << "getOneTransaction(" << acc_workchain_ << ":" << acc_addr_.to_hex() << "," << trans_lt_
            << ") query completed";
  auto b = tos::create_serialize_tl_object<tos::lite_api::liteServer_transactionInfo>(
      tos::create_tl_lite_block_id(blk_id_), proof_boc.move_as_ok(), std::move(data));
  finish_query(std::move(b));
}
...
void LiteQuery::continue_getTransactions(unsigned remaining, bool exact) {
  LOG(DEBUG) << "continue getTransactions() : " << remaining << " remaining";
  bool redo = true;
  while (remaining && redo && trans_lt_ && block_.not_null()) {
    redo = false;
    if (!tos::shard_contains(block_->block_id().shard_full(), tos::extract_addr_prefix(acc_workchain_, acc_addr_))) {
  ...
  LOG(DEBUG) << "sending get_block_by_lt_from_db() query to manager for " << acc_workchain_ << ":" << acc_addr_.to_hex()
             << " " << trans_lt_;
  td::actor::send_closure_later(
      manager_, &ValidatorManager::get_block_by_lt_for_litequery, tos::extract_addr_prefix(acc_workchain_, acc_addr_),
      trans_lt_, [Self = actor_id(this), remaining, manager = manager_](td::Result<ConstBlockHandle> res) {
        if (res.is_error()) {
          td::actor::send_closure(Self, &LiteQuery::abort_getTransactions, res.move_as_error(), tos::BlockIdExt{});
        } else {
...
void LiteQuery::continue_getTransactions_2(BlockIdExt blkid, Ref<BlockData> block, unsigned remaining) {
  LOG(DEBUG) << "getTransactions() : loaded block " << blkid.to_str();
  --pending_;
  CHECK(!pending_);
  CHECK(block.not_null());
...
void LiteQuery::abort_getTransactions(td::Status error, tos::BlockIdExt blkid) {
  LOG(DEBUG) << "getTransactions() : got error " << error.message() << " from manager";
  if (roots_.empty()) {
    if (blkid.is_valid()) {
...
void LiteQuery::finish_getTransactions() {
  ...
  auto b = tos::create_serialize_tl_object<tos::lite_api::liteServer_transactionList>(std::move(a), res.move_as_ok());
  finish_query(std::move(b));
}
...
void LiteQuery::load_prevKeyBlock(tos::BlockIdExt blkid, td::Promise<std::pair<BlockIdExt, Ref<BlockQ>>> promise) {
  td::actor::send_closure_later(manager_, &tos::validator::ValidatorManager::get_last_liteserver_state_block,
...
void LiteQuery::continue_loadPrevKeyBlock(tos::BlockIdExt blkid,
  ...
  LOG(DEBUG) << "continuing load_prevKeyBlock(" << blkid.to_str() << ") query with a state for "
            << base_blk_id_.to_str();
...
void LiteQuery::finish_loadPrevKeyBlock(tos::BlockIdExt blkid, td::Result<Ref<BlockData>> res,
                                        td::Promise<std::pair<BlockIdExt, Ref<BlockQ>>> promise) {
...
void LiteQuery::continue_getConfigParams(int mode, std::vector<int> param_list) {
  LOG(DEBUG) << "completing getConfigParams(" << base_blk_id_.to_str() << ", " << mode << ", <list of "
...
void LiteQuery::continue_getShardInfo(ShardIdFull shard, bool exact) {
  LOG(DEBUG) << "completing getShardInfo(" << shard.to_str() << ") query";
...
void LiteQuery::continue_getAllShardsInfo() {
  LOG(DEBUG) << "completing getAllShardsInfo() query";
...
void LiteQuery::continue_lookupBlockWithProof_getHeaderProof(Ref<tos::validator::BlockData> block,
  ...
  if (include_prev) {
    ...
    for (auto& p : prev) {
      if (tos::shard_contains(p.shard_full(), req_prefix)) {
        prev_blkid = p;
      }
    }
...
void LiteQuery::continue_lookupBlockWithProof_gotPrevBlockData(Ref<BlockData> prev_block,
  ...
  if (!blk_id_.is_masterchain()) {
    tos::AccountIdPrefixFull pfx{tos::masterchainId, tos::shardIdAll};
    td::actor::send_closure_later(
...
void LiteQuery::continue_lookupBlockWithProof_getMcBlockPrev(
  ...
  auto b = tos::create_serialize_tl_object<tos::lite_api::liteServer_lookupBlockResult>(
      tos::create_tl_lite_block_id(blk_id_), mode_, tos::create_tl_lite_block_id(base_blk_id_alt_),
...
void LiteQuery::perform_lookupBlock(BlockId blkid, int mode, LogicalTime lt, UnixTime utime) {
  ...
  LOG(DEBUG) << "performing a lookupBlock(" << blkid.to_str() << ", " << mode << ", " << lt << ", " << utime
            << ") query";
...
void LiteQuery::finish_listBlockTransactions(int mode, int req_count) {
  LOG(DEBUG) << "completing a listBlockTransactions(" << base_blk_id_.to_str() << ", " << mode << ", " << req_count
  ...
  bool eof = false;
  tos::LogicalTime reverse = (mode & 64) ? ~0ULL : 0;
  try {
    block::gen::Block::Record blk;
    block::gen::BlockExtra::Record extra;
...
void LiteQuery::finish_listBlockTransactionsExt(int mode, int req_count) {
  LOG(DEBUG) << "completing a listBlockTransactionsExt(" << base_blk_id_.to_str() << ", " << mode << ", " << req_count
            << ", " << acc_addr_.to_hex() << ", " << trans_lt_ << ") liteserver query";
  constexpr int max_answer_transactions = 256;
  CHECK(block_.not_null());
  auto block_root = block_->root_cell();
...
void LiteQuery::perform_getBlockProof(tos::BlockIdExt from, tos::BlockIdExt to, int mode) {
  if (!(mode & 1)) {
    to.invalidate_clear();
  }
  LOG(DEBUG) << "performing a getBlockProof(" << mode << ", " << from.to_str() << ", " << to.to_str() << ") query";
  ...
  if (mode & 1) {
    ...
    td::actor::send_closure_later(manager_, &tos::validator::ValidatorManager::get_shard_client_state, false,
...
void LiteQuery::continue_getBlockProof(tos::BlockIdExt from, tos::BlockIdExt to, int mode, BlockIdExt baseblk,
  ...
  LOG(DEBUG) << "continuing getBlockProof(" << mode << ", " << from.to_str() << ", " << to.to_str()
...
bool LiteQuery::construct_proof_chain(tos::BlockIdExt id) {
  CHECK(chain_);
...
bool LiteQuery::adjust_last_proof_link(tos::BlockIdExt cur, Ref<vm::Cell> block_root) {
  CHECK(chain_);
  if (!(chain_->last_link_incomplete() && chain_->last_link().to.seqno())) {
    return true;
  }
...
bool LiteQuery::construct_proof_link_forward(tos::BlockIdExt cur, tos::BlockIdExt next) {
  LOG(DEBUG) << "constructing a forward proof link from " << cur.to_str() << " to " << next.to_str();
...
bool LiteQuery::construct_proof_link_forward_cont(tos::BlockIdExt cur, tos::BlockIdExt next) {
  LOG(DEBUG) << "continue constructing a forward proof link from " << cur.to_str() << " to " << next.to_str();
...
bool LiteQuery::construct_proof_link_back(tos::BlockIdExt cur, tos::BlockIdExt next) {
  LOG(DEBUG) << "constructing a backward proof link from " << cur.to_str() << " to " << next.to_str();
  CHECK(chain_);
  if (!(cur.is_masterchain_ext() && next.is_masterchain_ext() && mc_state0_->check_old_mc_block_id(cur) &&
        mc_state0_->check_old_mc_block_id(next))) {
...
bool LiteQuery::construct_proof_link_back_cont(tos::BlockIdExt cur, tos::BlockIdExt next) {
  LOG(DEBUG) << "continue constructing a backward proof link from " << cur.to_str() << " to " << next.to_str();
...
bool LiteQuery::finish_proof_chain(tos::BlockIdExt id) {
  CHECK(chain_);
  LOG(DEBUG) << "finish constructing block proof chain from " << chain_->from.to_str() << " to " << chain_->to.to_str()
  ...
  try {
    ...
    chain_->to = id;
    // serialize answer
    std::vector<tos::tl_object_ptr<lite_api::liteServer_BlockLink>> a;
    for (auto& link : chain_->links) {
      ...
      if (link.is_fwd) {
        ...
        a.push_back(create_tl_object<lite_api::liteServer_blockLinkForward>(
            link.is_key, tos::create_tl_lite_block_id(link.from), tos::create_tl_lite_block_id(link.to),
...
void LiteQuery::continue_getValidatorStats(int mode, int limit, Bits256 start_after, UnixTime min_utime) {
  LOG(DEBUG) << "completing getValidatorStats(" << base_blk_id_.to_str() << ", " << mode << ", " << limit << ", "
            << start_after.to_hex() << ", " << min_utime << ") liteserver query";
  Ref<vm::Cell> proof1;
  if (!make_mc_state_root_proof(proof1)) {
    return;
...
void LiteQuery::finish_getBlockOutMsgQueueSize() {
  LOG(DEBUG) << "completing getBlockOutNsgQueueSize() query";
...
void LiteQuery::finish_getDispatchQueueInfo(StdSmcAddress after_addr, int max_accounts) {
  LOG(DEBUG) << "completing getDispatchQueueInfo() query";
  bool with_proof = mode_ & 1;
  Ref<vm::Cell> state_root = state_->root_cell();
  vm::MerkleProofBuilder pb;
  if (with_proof) {
...
void LiteQuery::finish_getDispatchQueueMessages(StdSmcAddress addr, LogicalTime lt, int max_messages) {
  LOG(DEBUG) << "completing getDispatchQueueMessages() query";
