...
td::actor::ActorOwn<FullNode> FullNode::create(
  ...
  return td::actor::create_actor<FullNodeImpl>("fullnode", adnl_id, zero_state_file_hash, opts, keyring, adnl,
                                               rldp2, quic, dht, overlays, validator_manager, client, db_root,
                                               std::move(started_promise));
}

FullNodeConfig::FullNodeConfig(const tl_object_ptr<tos_api::engine_validator_fullNodeConfig> &obj)
    : ext_messages_broadcast_disabled_(obj->ext_messages_broadcast_disabled_) {
}

tl_object_ptr<tos_api::engine_validator_fullNodeConfig> FullNodeConfig::tl() const {
  return create_tl_object<tos_api::engine_validator_fullNodeConfig>(ext_messages_broadcast_disabled_);
}
...
CustomOverlayParams CustomOverlayParams::fetch(const tos_api::engine_validator_customOverlay &f) {
