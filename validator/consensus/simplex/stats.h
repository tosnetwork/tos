...
using voted = tos_api::consensus_simplex_stats_voted;
using certObserved = tos_api::consensus_simplex_stats_certObserved;
...
class MetricCollector final : public consensus::stats::MetricCollector {
 public:
  MetricCollector(ValidatorSessionId session_id, PublicKeyHash self_id, std::unique_ptr<tos::stats::Recorder> recorder);
 ...
 private:
  ...
  std::unique_ptr<tos::stats::Recorder> recorder_;
};
