...
//    mismatched session, a non-canonical name, a path separator, or a NUL, so a
//    corrupt record can never target an arbitrary NAME);
//  - build the path under the consensus root, destroy the RocksDB, remove the
//    directory, and confirm removal by stat.
// Returns true ONLY on confirmed removal (an already-absent directory counts as
// gone). This mirrors the existing observer sweep deleter and the bridge's
