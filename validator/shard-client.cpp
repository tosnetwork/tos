...
td::actor::Task<> ShardClient::initialize() {
  ...
  if (init_mode_) {
    ...
    mc_state = td::Ref<MasterchainState>{
...
td::actor::Task<> ShardClient::apply_all_shards(td::Ref<MasterchainState> mc_state) {
...
td::actor::Task<> ShardClient::wait_shard_states(td::Ref<MasterchainState> mc_state) {
  ...
  std::vector<td::actor::StartedTask<td::Ref<ShardState>>> tasks;
...
td::actor::Task<td::Ref<MasterchainState>> ShardClient::wait_mc_state(BlockHandle handle) {
  while (true) {
    ...
    co_return td::Ref<MasterchainState>{R.move_as_ok()};
  }
}
...
td::actor::Task<> ShardClient::force_update_shard_client_ex(BlockHandle handle, td::Ref<MasterchainState> state) {
  CHECK(!init_mode_);
  CHECK(!started_);
  CHECK(masterchain_block_handle_);
