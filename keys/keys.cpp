...
PublicKeyHash::PublicKeyHash(const tl_object_ptr<tos_api::PublicKey> &value) {
  value_ = get_tl_object_sha_bits256(value);
}

PublicKey::PublicKey(const tl_object_ptr<tos_api::PublicKey> &id) {
  tos_api::downcast_call(
      *const_cast<tos_api::PublicKey *>(id.get()),
      td::overloaded([&](const tos_api::pub_ed25519 &obj) { this->pub_key_ = pubkeys::Ed25519{obj}; },
                     [&](const tos_api::pub_aes &obj) { this->pub_key_ = pubkeys::AES{obj}; },
                     [&](const tos_api::pub_unenc &obj) { this->pub_key_ = pubkeys::Unenc{obj}; },
                     [&](const tos_api::pub_overlay &obj) { this->pub_key_ = pubkeys::Overlay{obj}; }));
}
...
tl_object_ptr<tos_api::PublicKey> PublicKey::tl() const {
  tl_object_ptr<tos_api::PublicKey> res;
...
td::Result<PublicKey> PublicKey::import(td::Slice s) {
  TRY_RESULT(x, fetch_tl_object<tos_api::PublicKey>(s, true));
  return PublicKey{x};
}
...
tl_object_ptr<tos_api::PublicKey> privkeys::Ed25519::pub_tl() const {
  ...
  if (r_public_key.is_error()) {
    ...
    return create_tl_object<tos_api::pub_ed25519>(X);
  }
}
...
PrivateKey::PrivateKey(const tl_object_ptr<tos_api::PrivateKey> &id) {
  tos_api::downcast_call(
      *const_cast<tos_api::PrivateKey *>(id.get()),
      td::overloaded([&](const tos_api::pk_ed25519 &obj) { this->priv_key_ = privkeys::Ed25519{obj}; },
                     [&](const tos_api::pk_aes &obj) { this->priv_key_ = privkeys::AES{obj}; },
                     [&](const tos_api::pk_unenc &obj) { this->priv_key_ = privkeys::Unenc{obj}; },
                     [&](const tos_api::pk_overlay &obj) { this->priv_key_ = privkeys::Overlay{obj}; }));
}
...
tl_object_ptr<tos_api::PrivateKey> PrivateKey::tl() const {
  tl_object_ptr<tos_api::PrivateKey> res;
