...
if [ -f "./server" ]; then
    ...
    cp server /var/tos-work/db/keyring/$SERVER_ID1
fi
...
if [ -f "./client" ]; then
    ...
    sed -e "s/CONSOLE-PORT/\"$(printf "%q" $CONSOLE_PORT)\"/g" -e "s~SERVER-ID~\"$(printf "%q" $SERVER_ID2)\"~g" -e "s~CLIENT-ID~\"$(printf "%q" $CLIENT_ID2)\"~g" /var/tos-work/scripts/control.template > control.new
    sed -e "s~\"control\"\ \:\ \[~$(printf "%q" $(cat control.new))~g" /var/tos-work/db/config.json > config.json.new
    mv config.json.new /var/tos-work/db/config.json
fi
...
if [ -z "$LITESERVER" ]; then
    ...
    if [ -f "./liteserver" ]; then
        ...
        cp liteserver /var/tos-work/db/keyring/$LITESERVER_ID1
        ...
        sed -e "s~\"liteservers\"\ \:\ \[~$LITESERVERS~g" /var/tos-work/db/config.json > config.json.liteservers
