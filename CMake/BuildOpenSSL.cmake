...
set(TOS_THIRD_PARTY_SOURCE_DIR "${TOS_SOURCE_DIR}/third-party")
set(TOS_THIRD_PARTY_BINARY_DIR "${CMAKE_BINARY_DIR}/third-party")

if (NOT OPENSSL_CRYPTO_LIBRARY)
  set(OPENSSL_SOURCE_DIR ${TOS_THIRD_PARTY_SOURCE_DIR}/openssl)

  if (ANDROID)
    set(OPENSSL_INSTALL_DIR ${TOS_ANDROID_THIRD_PARTY_DIR}/crypto/${TOS_ANDROID_OPENSSL_DIR})
    set(OPENSSL_BINARY_DIR ${OPENSSL_INSTALL_DIR})
    set(OPENSSL_BUILD_DIR ${TOS_THIRD_PARTY_BINARY_DIR}/openssl-android-${TOS_ANDROID_OPENSSL_DIR})
    set(OPENSSL_BUILD_ROOT ${TOS_THIRD_PARTY_BINARY_DIR})
    set(OPENSSL_CONFIGURE_SCRIPT ${OPENSSL_BUILD_DIR}/Configure)
    set(OPENSSL_INCLUDE_DIR ${OPENSSL_INSTALL_DIR}/include)
    set(OPENSSL_ANDROID_ENV
      ${CMAKE_COMMAND} -E env
      ANDROID_NDK_ROOT=${TOS_ANDROID_NDK_ROOT}
      ANDROID_NDK_HOME=${TOS_ANDROID_NDK_ROOT}
      ANDROID_NDK=${TOS_ANDROID_NDK_ROOT}
      PERL=/usr/bin/perl
      CC=${TOS_ANDROID_CC}
      CXX=${TOS_ANDROID_CXX}
      AR=${TOS_ANDROID_AR}
      RANLIB=${TOS_ANDROID_RANLIB}
      NM=${TOS_ANDROID_NM}
      PATH=${TOS_ANDROID_NDK_BIN}:$ENV{PATH}
    )
    ...
    if (TOS_ANDROID_ARCH STREQUAL "arm")
      set(OPENSSL_CONFIGURE_TARGET android-arm)
    elseif (TOS_ANDROID_ARCH STREQUAL "arm64")
      set(OPENSSL_CONFIGURE_TARGET android-arm64)
    elseif (TOS_ANDROID_ARCH STREQUAL "x86")
      set(OPENSSL_CONFIGURE_TARGET android-x86)
    elseif (TOS_ANDROID_ARCH STREQUAL "x86_64")
      set(OPENSSL_CONFIGURE_TARGET android-x86_64)
    else()
      message(FATAL_ERROR "Unsupported Android arch for OpenSSL: ${TOS_ANDROID_ARCH}")
    endif()
    set(CMD ${OPENSSL_ANDROID_ENV}
      /usr/bin/perl ./Configure ${OPENSSL_CONFIGURE_TARGET} --prefix=${OPENSSL_INSTALL_DIR}
      no-shared no-dso no-unit-test no-tests no-apps enable-quic --libdir=lib -D__ANDROID_API__=${TOS_ANDROID_API})
  else()
    set(OPENSSL_BINARY_DIR ${TOS_THIRD_PARTY_BINARY_DIR}/openssl)
    set(OPENSSL_BUILD_DIR ${TOS_THIRD_PARTY_BINARY_DIR}/openssl-src)
  ...
  if (MSVC)
    set(OPENSSL_BINARY_DIR ${TOS_THIRD_PARTY_BINARY_DIR}/openssl)
    set(OPENSSL_BUILD_DIR ${TOS_THIRD_PARTY_BINARY_DIR}/openssl-src-msvc)
  ...
  elseif (USE_EMSCRIPTEN OR EMSCRIPTEN)
    set(OPENSSL_BINARY_DIR ${TOS_THIRD_PARTY_BINARY_DIR}/openssl-emscripten)
    set(OPENSSL_BUILD_DIR ${TOS_THIRD_PARTY_BINARY_DIR}/openssl-src-emscripten)
    ...
    if (ANDROID)
      ...
      set(OPENSSL_ARCH_EXPECTED "${TOS_ANDROID_ABI};${TOS_ANDROID_API};${TOS_ANDROID_CC}")
