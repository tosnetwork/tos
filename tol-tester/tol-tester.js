// Usage: `node tol-tester.js tests_dir` OR `node tol-tester.js tests_dir file_pattern`
// from current dir, providing some env (see getenv() calls).
// This is a JS version of tol-tester.py to test Tol compiled to WASM.
...
const TOLFIFTLIB_MODULE = getenv('TOLFIFTLIB_MODULE')
const TOLFIFTLIB_WASM = getenv('TOLFIFTLIB_WASM')
const FIFT_EXECUTABLE = getenv('FIFT_EXECUTABLE')
const FIFT_LIBS_FOLDER = getenv('FIFTPATH')
const STDLIB_FOLDER = __dirname + '/../crypto/smartcont/tol-stdlib'
const TMP_DIR = os.tmpdir()

class CmdLineOptions {
    constructor(/**string[]*/ argv) {
        if (argv.length < 3) {
            print("Usage: node tol-tester.js tests_dir [file_pattern]")
            process.exit(1)
        }
...
class TolCompilationFailedError extends Error {
...
class TolCompilationSucceededError extends Error {
}
...
class TolTestCaseInputOutput {
    ...
    constructor(/**string*/ method_id_str, /**string*/ input_str, /**string*/ output_str) {
        ...
        for (let in_arg of input_str.split(' ')) {
            if (in_arg.length === 0)
                continue
            else if (in_arg.startsWith("x{") || TolTestCaseInputOutput.reJustNumber.test(in_arg))
                processed_inputs.push(in_arg)
            else if (TolTestCaseInputOutput.reMathExpr.test(in_arg))
...
class TolTestCaseFifCodegen {
    ...
    try_match_pattern(/**number*/ pattern_offset, /**string[]*/ fif_output, /**number*/ offset) {
        ...
        if (line_pattern !== "...") {
            if (!TolTestCaseFifCodegen.does_line_match(line_pattern, line_output))
                return false
    ...
    static does_line_match(/**string*/ line_pattern, /**string*/ line_output) {
        const [cmd_pattern, comment_pattern] = TolTestCaseFifCodegen.split_line_to_cmd_and_comment(line_pattern)
        const [cmd_output, comment_output] = TolTestCaseFifCodegen.split_line_to_cmd_and_comment(line_output.trim())
...
class TolTestFile {
    constructor(/**string*/ tol_filename, /**string*/ artifacts_folder) {
        this.line_idx = 0
        this.tol_filename = tol_filename
        this.artifacts_folder = artifacts_folder
        this.compilation_should_fail = false
        /** @type {TolTestCaseStderr[]} */
        this.stderr_includes = []
        /** @type {TolTestCaseInputOutput[]} */
        this.input_output = []
        /** @type {TolTestCaseFifCodegen[]} */
        this.fif_codegen = []
        /** @type {TolTestCaseExpectedHash | null} */
        this.expected_hash = null
        /** @type {Object} */
        this.path_mappings = {}
        /** @type {boolean} */
        this.enable_tol_lines_comments = false
        /** @type {number} */
        this.pivot_typeid = 128
    }

    parse_input_from_tol_file() {
        const lines = fs.readFileSync(this.tol_filename, 'utf-8').split(/\r?\n/)
        this.line_idx = 0
        ...
        while (this.line_idx < lines.length) {
            ...
            if (line.startsWith('@testcase')) {
                ...
                this.input_output.push(new TolTestCaseInputOutput(s[1], s[2], s[3]))
            } else if (line.startsWith('@compilation_should_fail')) {
                this.compilation_should_fail = true
            } else if (line.startsWith('@stderr_avoid')) {
                this.stderr_includes.push(new TolTestCaseStderr(this.parse_string_value(lines), true))
            } else if (line.startsWith('@stderr')) {
                this.stderr_includes.push(new TolTestCaseStderr(this.parse_string_value(lines), false))
            } else if (line.startsWith("@fif_codegen_avoid")) {
                this.fif_codegen.push(new TolTestCaseFifCodegen(this.parse_string_value(lines), true))
            } else if (line.startsWith("@fif_codegen_enable_comments")) {
                this.enable_tol_lines_comments = true
            } else if (line.startsWith("@fif_codegen")) {
                this.fif_codegen.push(new TolTestCaseFifCodegen(this.parse_string_value(lines), false))
            } else if (line.startsWith("@code_hash")) {
                this.expected_hash = new TolTestCaseExpectedHash(this.parse_string_value(lines, false)[0])
            } else if (line.startsWith("@path_mapping")) {
                let eq_pos = line.indexOf('=')
                this.path_mappings[line.substring(14, eq_pos)] = line.substring(eq_pos+1).replace('{DIR}', path.dirname(this.tol_filename)).replace(/[\\\/]+$/, '')
            }
            this.line_idx++
        }
    ...
    async run_and_check() {
        const wasmModule = await compileWasm(TOLFIFTLIB_MODULE, TOLFIFTLIB_WASM)
        let res = compileFile(wasmModule, this.tol_filename, this.enable_tol_lines_comments, this.path_mappings)
        let exit_code = res.status === 'ok' ? 0 : 1
        let stderr = res.message || res.stderr
        let stdout = ''

        if (exit_code === 0 && this.compilation_should_fail)
            throw new TolCompilationSucceededError("compilation succeeded, but it should have failed")
        ...
        if (exit_code !== 0 && !this.compilation_should_fail)
            throw new TolCompilationFailedError(`tol exit_code = ${exit_code}`, stderr)
...
async function run_all_tests(/**string[]*/ tests) {
    for (let ti = 0; ti < tests.length; ++ti) {
        let tol_filename = tests[ti]
        print(`Running test ${ti + 1}/${tests.length}: ${path.basename(tol_filename)}`)

        let artifacts_folder = path.join(TMP_DIR, tol_filename)
        let testcase = new TolTestFile(tol_filename, artifacts_folder)

        try {
            if (!fs.existsSync(artifacts_folder))
                fs.mkdirSync(artifacts_folder, {recursive: true})
            testcase.parse_input_from_tol_file()
            ...
            if (e instanceof ParseInputError) {
                ...
                process.exit(2)
            } else if (e instanceof TolCompilationFailedError) {
                print("  Error compiling tol:", e.message)
                print("  stderr:")
...
function compileFile(mod, filename, withSrcLineComments, pathMappings) {
    // see tol-wasm.cpp: typedef void (*WasmFsReadCallback)(int, char const*, char**, char**)
    const callbackPtr = mod.addFunction((kind, dataPtr, destContents, destError) => {
        switch (kind) {   // enum ReadCallback::Kind in C++
            case 0:       // realpath
                ...
                if (!relativeFilename.endsWith('.tol') && !relativeFilename.endsWith('.fif')) {
                    relativeFilename += '.tol'
                }
    ...
    const responsePtr = mod._tol_compile(configPtr, callbackPtr);

    return JSON.parse(copyFromCString(mod, responsePtr));
}

async function compileWasm(tolFiftLibJsFileName, tolFiftLibWasmFileName) {
    const wasmModule = require(tolFiftLibJsFileName)
    const wasmBinary = new Uint8Array(fs.readFileSync(tolFiftLibWasmFileName))
