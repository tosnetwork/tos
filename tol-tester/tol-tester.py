# Usage: `tol-tester.py tests_dir` OR `tol-tester.py tests_dir file_pattern`
# from current dir, providing some env (see getenv() calls).
# Every .tol file should provide /* testcase description in a comment */, consider tests/ folder.
#
# Tests for Tol can be
...
#
# Note, that there is also tol-tester.js to test Tol compiled to WASM.
...
TOL_EXECUTABLE = getenv("TOL_EXECUTABLE", "tol")
...
class CmdLineOptions:
    def __init__(self, argv: List[str]):
        if len(argv) < 2:
            print("Usage: tol-tester.py tests_dir [file_pattern]", file=sys.stderr)
            exit(1)
    ...
    def find_tests(self) -> List[str]:
        ...
        for f in all_children_of_tests_dir:
            if not f.endswith(".tol") and f != "imports":
...
class TolCompilationFailedError(Exception):
    def __init__(self, message: str, stderr: str):
        super().__init__(message)
        self.stderr = stderr


class TolCompilationSucceededError(Exception):
    pass
...
class TolTestCaseInputOutput:
    ...
    def __init__(self, method_id_str: str, input_str: str, output_str: str):
        ...
        for in_arg in input_str.split(" "):
            if len(in_arg) == 0:
                continue
            elif in_arg.startswith("x{") or TolTestCaseInputOutput.reJustNumber.fullmatch(in_arg):
                processed_inputs.append(in_arg)
            elif in_arg.startswith("cell{"):
                processed_inputs.append("<b " + in_arg.replace("cell{", "x{") + " s, b>")
            elif TolTestCaseInputOutput.reMathExpr.fullmatch(in_arg):
...
class TolTestCaseFifCodegen:
    ...
    See tests/codegen_check_demo.tol of how it looks.
    ...
    And one more word about //comments. Tol inserts them into fift output.
    ...
    def try_match_pattern(self, pattern_offset: int, fif_output: List[str], offset: int) -> bool:
        ...
        if line_pattern != "...":
            if not TolTestCaseFifCodegen.does_line_match(line_pattern, line_output):
                return False
    ...
    def does_line_match(line_pattern: str, line_output: str) -> bool:
        cmd_pattern, comment_pattern = TolTestCaseFifCodegen.split_line_to_cmd_and_comment(line_pattern)
        cmd_output, comment_output = TolTestCaseFifCodegen.split_line_to_cmd_and_comment(line_output.strip())
...
class TolTestFile:
    def __init__(self, tol_filename: str, artifacts_folder: str):
        self.line_idx = 0
        self.tol_filename = tol_filename
        self.artifacts_folder = artifacts_folder
        self.compilation_should_fail = False
        self.stderr_includes: List[TolTestCaseStderr] = []
        self.input_output: List[TolTestCaseInputOutput] = []
        self.fif_codegen: List[TolTestCaseFifCodegen] = []
        self.expected_hash: TolTestCaseExpectedHash | None = None
        self.more_cmd_line_options: List[str] = []
        self.enable_tol_lines_comments = False
        self.pivot_typeid = 128

    def parse_input_from_tol_file(self):
        with open(self.tol_filename, "r") as fd:
        ...
        while self.line_idx < len(lines):
            ...
            if line.startswith("@testcase"):
                ...
                self.input_output.append(TolTestCaseInputOutput(s[1], s[2], s[3]))
            elif line.startswith("@compilation_should_fail"):
                self.compilation_should_fail = True
            elif line.startswith("@stderr_avoid"):
                self.stderr_includes.append(TolTestCaseStderr(self.parse_string_value(lines), True))
            elif line.startswith("@stderr"):
                self.stderr_includes.append(TolTestCaseStderr(self.parse_string_value(lines), False))
            elif line.startswith("@fif_codegen_avoid"):
                self.fif_codegen.append(TolTestCaseFifCodegen(self.parse_string_value(lines), True))
            elif line.startswith("@fif_codegen_enable_comments"):
                self.enable_tol_lines_comments = True
            elif line.startswith("@fif_codegen"):
                self.fif_codegen.append(TolTestCaseFifCodegen(self.parse_string_value(lines), False))
            elif line.startswith("@code_hash"):
                self.expected_hash = TolTestCaseExpectedHash(self.parse_string_value(lines, False)[0])
            elif line.startswith("@path_mapping"):
                self.more_cmd_line_options += ["--path-mapping", line[14:].replace('{DIR}', os.path.dirname(self.tol_filename))]
    ...
    def run_and_check(self):
        cmd_args = [TOL_EXECUTABLE, "-o", self.get_compiled_fif_filename()] + self.more_cmd_line_options
        if not self.enable_tol_lines_comments:
            cmd_args = cmd_args + ["--no-line-comments"]
        res = subprocess.run(cmd_args + [self.tol_filename], capture_output=True, timeout=10)
        exit_code = res.returncode
        stderr = str(res.stderr, "utf-8")
        stdout = str(res.stdout, "utf-8")

        if exit_code == 0 and self.compilation_should_fail:
            raise TolCompilationSucceededError("compilation succeeded, but it should have failed")
        ...
        if exit_code != 0 and not self.compilation_should_fail:
            raise TolCompilationFailedError("tol exit_code = %d" % exit_code, stderr)
        ...
        gas_used = sum(map(int, TolTestCaseInputOutput.reGasUsed.findall(stderr)))
...
def run_all_tests(tests: List[str]):
    ...
    for ti in range(len(tests)):
        tol_filename = tests[ti]
        print("Running test %d/%d: %s" % (ti + 1, len(tests), os.path.basename(tol_filename)), file=sys.stderr)

        artifacts_folder = os.path.join(TMP_DIR, tol_filename)
        testcase = TolTestFile(tol_filename, artifacts_folder)
        try:
            ...
            exit(2)
        except TolCompilationFailedError as e:
            print("  Error compiling tol:", e, file=sys.stderr)
            print("  stderr:", file=sys.stderr)
            print(e.stderr.rstrip(), file=sys.stderr)
            exit(2)
        except TolCompilationSucceededError as e:
