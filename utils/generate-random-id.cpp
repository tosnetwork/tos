...
int main(int argc, char *argv[]) {
  tos::PrivateKey pk;
  td::optional<tos::adnl::AdnlAddressList> addr_list;
  ...
  p.add_checked_option('k', "key", "path to private key to import", [&](td::Slice key) {
    ...
    TRY_RESULT_PREFIX_ASSIGN(pk, tos::PrivateKey::import(data.as_slice()), "failed to import private key: ");
    return td::Status::OK();
  });
  p.add_checked_option('a', "addr-list", "addr list to sign", [&](td::Slice key) {
    ...
    tos::tl_object_ptr<tos::tos_api::adnl_addressList> addr_list_tl;
    TRY_STATUS_PREFIX(td::from_json(addr_list_tl, std::move(as_json_value)), "bad addr list TL: ");
    TRY_RESULT_PREFIX_ASSIGN(addr_list, tos::adnl::AdnlAddressList::create(addr_list_tl), "bad addr list: ");
    return td::Status::OK();
  });
  ...
  if (pk.empty()) {
    pk = tos::privkeys::Ed25519::random();
  }
  ...
  if (mode == "id") {
    ...
    std::cout << v << std::endl;
    v = td::json_encode<std::string>(td::ToJson(tos::adnl::AdnlNodeIdShort{short_key}.tl()));
    std::cout << v << std::endl;
  } else if (mode == "adnl") {
    if (!addr_list) {
    ...
    auto x = tos::create_tl_object<tos::tos_api::adnl_node>(pub_key.tl(), addr_list.value().tl());
    auto e = pk.create_decryptor().move_as_ok();
    auto r = e->sign(tos::serialize_tl_object(x, true).as_slice()).move_as_ok();
    ...
    td::BufferSlice to_sign = tos::serialize_tl_object(
        tos::dht::DhtNode{tos::adnl::AdnlNodeIdFull{pub_key}, addr_list.value(), -1, network_id, td::BufferSlice{}}
            .tl(),
        true);
    auto e = pk.create_decryptor().move_as_ok();
    auto signature = e->sign(to_sign.as_slice()).move_as_ok();
    auto node =
        tos::dht::DhtNode{tos::adnl::AdnlNodeIdFull{pub_key}, addr_list.value(), -1, network_id, std::move(signature)};
    ...
    std::cout << name << " " << tos::adnl::AdnlNodeIdShort{n}.serialize() << std::endl;
  } else {
