...
class PrepareLsSliceConfig : public td::actor::Actor {
 public:
  void start_up() override {
    ...
    if (!start_mc_seqno && !end_mc_seqno) {
      auto slice = create_tl_object<tos_api::liteserver_descV2_sliceSimple>();
      if (shards.empty()) {
    ...
    tos_api::liteclient_config_global gc;
    tos_api::from_json(gc, gc_j.get_object()).ensure();
    ...
    slice_timed_ = create_tl_object<tos_api::liteserver_descV2_sliceTimed>();
    ++pending_;
  ...
  static tl_object_ptr<tos_api::liteserver_descV2_shardInfo> parse_header(const lite_api::liteServer_blockHeader& obj,
                                                                          bool is_start) {
    auto res = create_tl_object<tos_api::liteserver_descV2_shardInfo>();
  ...
  void got_shards_info(tl_object_ptr<lite_api::liteServer_blockHeader> mc_header,
    ...
    for (auto id : ids) {
      BlockIdExt block_id = sh_conf.get_shard_hash(tos::ShardIdFull(id))->top_block_id();
  ...
  void print_result(const tos_api::liteserver_descV2_Slice& result) {
 ...
 private:
  ...
  tl_object_ptr<tos_api::liteserver_descV2_sliceTimed> slice_timed_;
  size_t pending_ = 0;

  void dec_pending() {
    --pending_;
    if (pending_ == 0) {
      auto cmp = [](const tl_object_ptr<tos_api::liteserver_descV2_shardInfo>& a,
                    const tl_object_ptr<tos_api::liteserver_descV2_shardInfo>& b) {
...
int main(int argc, char* argv[]) {
  ...
  p.add_option('C', "global-config", "global TOS configuration file (used to fetch shard configuration)",
