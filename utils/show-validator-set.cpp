...
int main(int argc, char** argv) {
  ...
  p.add_checked_option('w', "workchain", "workchain id", [&](td::Slice arg) {
    TRY_RESULT_ASSIGN(workchain, td::to_integer_safe<tos::WorkchainId>(arg));
    return td::Status::OK();
  });
  ...
  p.add_checked_option('c', "cc-seqno", "catchain seqno", [&](td::Slice arg) {
    TRY_RESULT_ASSIGN(cc_seqno, td::to_integer_safe<tos::CatchainSeqno>(arg));
    return td::Status::OK();
  });
  ...
  if (filename.empty() || workchain == tos::workchainInvalid) {
    std::cerr << "Usage: show-validator-set -f <key-block.boc> -w <workchain> -s <shard-hex> -c <cc-seqno>"
              << std::endl;
    return 1;
  }

  tos::ShardIdFull shard_id{workchain, static_cast<tos::ShardId>(shard)};
