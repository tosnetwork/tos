...
class ProxyLiteserver : public td::actor::Actor {
 public:
  ...
  td::Status prepare_local_config() {
    ...
    if (r_conf_data.is_ok()) {
      ...
      TRY_STATUS_PREFIX(tos_api::from_json(*config_, conf_json.get_object()), "json does not fit TL scheme: ");
  ...
  td::Status parse_global_config() {
    ...
    TRY_STATUS_PREFIX(tos_api::from_json(gc, global_config_json.get_object()), "Failed to parse global config: ");
 ...
 private:
  ...
  tl_object_ptr<tos_api::proxyLiteserver_config> config_ = create_tl_object<tos_api::proxyLiteserver_config>();
