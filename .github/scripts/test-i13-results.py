...
with tempfile.TemporaryDirectory(prefix="i13-ci-observer-") as temporary:
    ...
    for label, tests, success in [
        ("six", registry["tests"], True), ("zero", [], False),
        ("missing", registry["tests"][:-1], False),
        ("duplicate", [registry["tests"][0]] * 6, False),
    ]:
    ...
    if len(successful_tests) != 6 or any(t.get('status') != 'run' for t in successful_tests):
        raise AssertionError('positive observer must see six actual run statuses')
    # Keep all six actual names so failure cannot be rejected merely by count.
    ...
    try:
        ...
        if len(failed_tests) != 6 or {t.get('name') for t in failed_tests} != set(NAMES):
            raise AssertionError('forced failure must retain all six registered names')
