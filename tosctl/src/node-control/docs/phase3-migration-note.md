...
### TON References Cleaned in Contract Code
...
|-----------|--------|-----------------|
| Wallet versions (V1R3, V3R2, V4R2, V5R1) | Code cells from TON, needs TOS verification | Deploy-test on TOS testnet |
...
| Single nominator contract | Code from TON, needs TOS verification | Deploy-test on TOS testnet |
...
TOS uses the same TVM and wallet contract standards inherited from TON:

- **Supported wallets:** V1R3, V3R2, V4R2, V5R1
- **Code cells:** Embedded from TON mainnet contracts — must be validated on TOS testnet
- **Address derivation:** Same algorithm (StateInit hash) — deterministic given same code + data
- **Subwallet ID:** Default `698983191` (TON convention) — TOS may define a different default
