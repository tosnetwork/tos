...
fn rejected_funding_is_returned_to_sender_not_orphaned() {
    // BUG-001: jettons are credited to the escrow's own wallet before the
    // escrow runs, and the transfer_notification is non-bounceable, so a throw
    // on a mismatch cannot un-credit them and would orphan them permanently.
    // Every funding the escrow does not accept must instead be returned to the
    // funder (exactly one outbound transfer), leaving the escrow able to still
    // accept a correct funding afterwards.
    let mut fixture = Fixture::new();
    let buyer = fixture.buyer.address().clone();
...
fn funding_after_the_deadline_is_returned_not_orphaned() {
    ...
    // funding_deadline through message-routing latency. It must be returned to
    // the buyer, not thrown away, and must not consume the awaiting_funding
    // state.
    let mut fixture = Fixture::new();
    let buyer = fixture.buyer.address().clone();
    fixture.send(&buyer, fixture.accept_body(3)).expect_success();
