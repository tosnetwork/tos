...
use crate::error::ToscenterError;
use crate::models::{
...
impl ApiClientV2 {
    ...
    ) -> Result<Vec<RawTransaction>, ToscenterError> {
    ...
    pub async fn get_masterchain_block_signatures(
        &self,
        seqno: u32,
    ) -> Result<MasterchainBlockSignatures, ToscenterError> {
    ...
    ) -> Result<BlocksShardBlockProof, ToscenterError> {
    ...
    pub async fn get_shards(&self, seqno: u32) -> Result<BlocksShards, ToscenterError> {
        let seqno_string = seqno.to_string();
        let seqno_ref = seqno_string.as_str();
    ...
    pub async fn get_block_transactions(
        ...
        count: Option<u32>,
    ) -> Result<BlocksTransactions, ToscenterError> {
        let mut params: Vec<(&str, String)> = vec![
            ("workchain", workchain.to_string()),
            ("shard", shard.to_string()),
            ("seqno", seqno.to_string()),
    ...
    pub async fn get_block_header(
        ...
        root_hash: Option<&str>,
        file_hash: Option<&str>,
    ) -> Result<BlocksHeader, ToscenterError> {
        let mut params: Vec<(&str, String)> = vec![
            ("workchain", workchain.to_string()),
            ("shard", shard.to_string()),
            ("seqno", seqno.to_string()),
    ...
    pub async fn try_locate_tx(
        ...
        destination: &str,
        created_lt: u64,
    ) -> Result<RawTransaction, ToscenterError> {
        let params = [
            ("source", source),
    ...
    pub async fn get_config_param(
        ...
        config_id: u32,
        seqno: Option<u32>,
    ) -> Result<ConfigInfo, ToscenterError> {
    ...
    pub async fn run_get_method(
        ...
        method: &str,
        stack: &[&str],
    ) -> Result<SmcRunResult, ToscenterError> {
    ...
    pub async fn send_boc(&self, boc: &str) -> Result<Success, ToscenterError> {
    ...
    pub async fn send_boc_return_hash(
        &self,
        boc: &str,
    ) -> Result<RawExtMessageInfo, ToscenterError> {
        let body = serde_json::json!({
            "boc": boc,
        });
    ...
    /// # Returns
    ///
    /// A `Result` containing `SendQueryResponse` on success, or `ToscenterError` on failure.
    ...
    /// # Returns
    ///
    /// A `Result` containing `EstimateFeeResponse` on success, or `ToscenterError` on failure.
    ...
    ) -> Result<QueryFees, ToscenterError> {
        let mut request_body = serde_json::json!({
            "address": address,
        });
