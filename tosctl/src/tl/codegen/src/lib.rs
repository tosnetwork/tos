...
fn reformat(filename: &Path) {
    ...
    if !cfg!(feature = "reformat") {
        if !WARNING_PRINTED.swap(true, Ordering::Relaxed) {
            println!(
                "use feature \"reformat\" in tl_api cargo.toml for tl_codegen \
    ...
    const INSTALL_INSTRUCTIONS: &str = "It's not an issue, the building will proceed. \
        If you wish to develop using tl_api, you can install rustfmt by running command: \
...
impl TypeIR {
    ...
    fn io_turbofish(&self, with_tlobject: bool) -> TokenStream {
        let ty = match self.wire_kind {
            WireKind::Flags => quote!(crate::tos::Flags),
            WireKind::TypeParameter(_) if with_tlobject => quote!(crate::tos::TLObject),
...
impl Constructor<TypeIR, FieldIR> {
    ...
    fn deserialize_tlobject_generics(&self) -> TokenStream {
        self.generics(|_| quote!(crate::tos::TLObject))
    }
    ...
    fn as_struct_deserialize(&self) -> TokenStream {
        ...
        for f in &self.fields {
            ...
            let mut read_op = if f.ty.needs_determiner {
                ...
                if bare {
                    quote!(<Vec<#tokens> as crate::tos::VectoredBare<#tokens>>::deserialize(_de)?)
                } else {
                    quote!(<Vec<#tokens> as crate::tos::VectoredBoxed<#tokens>>::deserialize(_de)?)
                }
            } else {
                ...
                if f.ty.field_type().to_string() == "crate :: tos :: bytes" {
                    ...
                    return quote!(
                        impl crate::Signing for #name {
                            fn signature_mut(&mut self) -> &mut crate::tos::bytes { &mut self.signature }
                        }
                    );
                } else {
                    break;
        ...
        for f in self.fields.iter() {
            ...
            let field = if f.ty.is_flags() {
                ...
                let tokens = if f.ty.needs_determiner {
                    let (tokens, bare) = f.ty.contained();
                    if bare {
                        quote!((inner as &dyn crate::tos::VectoredBare<#tokens>).serialize(_ser)?;)
                    } else {
                        quote!((inner as &dyn crate::tos::VectoredBoxed<#tokens>).serialize(_ser)?;)
                    }
                } else {
                ...
                let (tokens, bare) = f.ty.contained();
                if bare {
                    quote!((#local_name as &dyn crate::tos::VectoredBare<#tokens>).serialize(_ser)?;)
                } else {
                    quote!((#local_name as &dyn crate::tos::VectoredBoxed<#tokens>).serialize(_ser)?;)
                }
            } else {
    ...
    fn as_dynamic_deserializer(&self) -> Option<TokenStream> {
        ...
        let mut ty = if &self.original_variant == "manual.gzip_packed" {
            quote!(crate::tos::TransparentGunzip)
        ...
        if !self.type_parameters.is_empty() {
            let generics = self.type_parameters.iter().map(|_| quote!(crate::tos::TLObject));
