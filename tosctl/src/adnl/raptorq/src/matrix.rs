use crate::{
    gf2::add_assign_binary, iterators::OctetIter, octet::Octet, octets::BinaryOctetVec,
    util::get_both_ranges,
};
#[cfg(not(feature = "std"))]
use alloc::vec::Vec;
...
impl BinaryMatrix for DenseBinaryMatrix {
    fn new(height: usize, width: usize, _: usize) -> DenseBinaryMatrix {
        ...
        DenseBinaryMatrix { height, width, elements }
    }
    ...
    fn query_non_zero_columns(&self, row: usize, start_col: usize) -> Vec<usize> {
        (start_col..self.width).filter(|col| self.get(row, *col) != Octet::zero()).collect()
    }
