use crate::{
    arraymap::{U16ArrayMap, U32VecMap, UndirectedGraph},
    graph::ConnectedComponentGraph,
    matrix::BinaryMatrix,
    octet::Octet,
    octet_matrix::DenseOctetMatrix,
    octets::BinaryOctetVec,
    operation_vector::SymbolOps,
    symbol::Symbol,
    systematic_constants::{
        num_hdpc_symbols, num_intermediate_symbols, num_ldpc_symbols, num_pi_symbols,
    },
    util::get_both_indices,
};
#[cfg(not(feature = "std"))]
use alloc::vec::Vec;
...
impl FirstPhaseRowSelectionStats {
    ...
    pub fn recompute_row<T: BinaryMatrix>(&mut self, row: usize, matrix: &T) {
        ...
        self.ones_histogram.decrement(self.ones_per_row.get(row) as usize);
    ...
    fn rebuild_connected_components<T: BinaryMatrix>(
        ...
        for key in graph.nodes() {
            ...
            while !node_queue.is_empty() {
                ...
                self.col_graph.add_node(node as usize, connected_component_id);
    ...
    fn first_phase_graph_substep<T: BinaryMatrix>(
        ...
        let node =
            self.col_graph.get_node_in_largest_connected_component(self.start_col, self.end_col);
...
impl<T: BinaryMatrix> IntermediateSymbolDecoder<T> {
    ...
    fn first_phase_swap_columns_substep(
        ...
        if r == 1 {
            ...
            let col = self
                ...
                .find_map(|(col, value)| if value != Octet::zero() { Some(col) } else { None })
                .unwrap();
    ...
    fn first_phase(&mut self) -> Option<Vec<RowOp>> {
        ...
        while self.i + self.u < self.L {
            ...
            row_ops.push(RowOp::Swap { row1: temp, row2: chosen_row });
            ...
            let ones_in_column =
                self.A.get_ones_in_column(temp, self.i + 1, self.A.height() - num_hdpc_rows);
            ...
            for row in ones_in_column {
                ...
                row_ops.push(RowOp::AddAssign { src: temp, dest: row });
                if r == 1 {
            ...
            let pi_octets = self.A.get_sub_row_as_octets(temp, self.A.width() - (self.u + r - 1));
            ...
            .filter_map(|x| match x {
                RowOp::AddAssign { src, dest } => {
                    ...
                    if mapping[*src] < self.i && mapping[*dest] < self.i {
                        Some(RowOp::AddAssign { src: mapping[*src], dest: mapping[*dest] })
                    } else {
                        None
                    }
                }
    ...
    fn record_mul_row(&mut self, i: usize, beta: Octet) {
        ...
        self.deferred_D_ops.push(SymbolOps::MulAssign { dest: self.d[i], scalar: beta });
    ...
    fn record_fma_rows(&mut self, i: usize, iprime: usize, beta: Octet) {
        ...
        if beta == Octet::one() {
            self.deferred_D_ops.push(SymbolOps::AddAssign { dest: self.d[iprime], src: self.d[i] });
        } else {
            self.debug_symbol_mul_ops += 1;
    ...
    fn swap_columns(&mut self, j: usize, jprime: usize, start_row: usize) {
        ...
        self.A_hdpc_rows.as_mut().unwrap().swap_columns(j, jprime, 0);
