...
pub struct ImmutableListMap {
    // offset of u32::MAX indicates that the key is not present
    offsets: Vec<u32>,
    values: Vec<u32>,
}
...
impl ImmutableListMapBuilder {
    pub fn new(num_keys: usize) -> ImmutableListMapBuilder {
        ImmutableListMapBuilder { entries: vec![], num_keys }
    }
...
impl UndirectedGraph {
    ...
    pub fn build(&mut self) {
        ...
        for (index, (node, _)) in self.edges.iter().enumerate() {
            if last_node != *node {
                last_node = *node;
                self.node_edge_starting_index.insert(last_node as usize, index as u32);
            }
        }
    }
...
impl U16ArrayMap {
    pub fn new(start_key: usize, end_key: usize) -> U16ArrayMap {
        U16ArrayMap { offset: start_key, elements: vec![0; end_key - start_key] }
    }
...
impl U32VecMap {
    pub fn new(start_key: usize) -> U32VecMap {
        U32VecMap { offset: start_key, elements: vec![0; 1] }
    }

    pub fn with_capacity(start_key: usize, end_key: usize) -> U32VecMap {
        U32VecMap { offset: start_key, elements: vec![0; end_key - start_key] }
    }

    fn grow_if_necessary(&mut self, index: usize) {
        if index >= self.elements.len() {
            self.elements.extend(vec![0; index - self.elements.len() + 1]);
        }
    }
