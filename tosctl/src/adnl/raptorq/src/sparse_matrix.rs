use crate::{
    arraymap::{ImmutableListMap, ImmutableListMapBuilder},
    iterators::OctetIter,
    matrix::BinaryMatrix,
    octet::Octet,
    octets::BinaryOctetVec,
    sparse_vec::SparseBinaryVec,
...
impl BinaryMatrix for SparseBinaryMatrix {
    ...
    fn get(&self, i: usize, j: usize) -> Octet {
        ...
        if self.width - j <= self.num_dense_columns {
            ...
            return self.sparse_elements[physical_i].get(physical_j).unwrap_or_else(Octet::zero);
        }
    }
    ...
    fn get_row_iter(&self, row: usize, start_col: usize, end_col: usize) -> OctetIter<'_> {
        ...
        OctetIter::new_sparse(start_col, end_col, sparse_elements, &self.physical_col_to_logical)
    }
    ...
    fn get_ones_in_column(&self, col: usize, start_row: usize, end_row: usize) -> Vec<u32> {
        ...
        let mut rows = vec![];
        for physical_row in self.sparse_columnar_values.as_ref().unwrap().get(physical_col) {
    ...
    fn hint_column_dense_and_frozen(&mut self, i: usize) {
        ...
        let physical_i = self.logical_col_to_physical[i] as usize;
        for maybe_present_in_row in
            self.sparse_columnar_values.as_ref().unwrap().get(physical_i as u16)
        {
