use raptorq::{
    extended_source_block_symbols, generate_constraint_matrix, BinaryMatrix,
    IntermediateSymbolDecoder, Octet, SparseBinaryMatrix, Symbol,
};
...
fn main() {
    for elements in [10, 100, 1000, 10000, 40000, 56403].iter() {
        ...
        println!("Initial memory usage: {}KB", decoder.get_non_symbol_bytes() / 1024);
        decoder.execute();
        println!(
