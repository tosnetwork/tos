...
use super::{engine::{storage::fetch_stack, Engine}, gas::gas_state::Gas, types::Instruction};
...
extern "C" {
    fn tos_rust_mldsa44_native_verify(sig: *const u8, message: *const u8, message_len: usize,
        context: *const u8, context_len: usize, public_key: *const u8) -> i32;
}

fn read_bytes(engine: &mut Engine, mut cell: Cell, limit: usize) -> Result<Vec<u8>> {
    let mut result = Vec::new();
    loop {
        if cell.level() != 0 { fail!(ExceptionCode::CellUnderflow, "PQ cells must have level zero"); }
        ...
        if slice.cell_type() != CellType::Ordinary || slice.level() != 0
            || slice.remaining_bits() % 8 != 0 || refs > 1 || size > CHUNK_BYTES
        ...
        if refs == 0 { return Ok(result); }
        cell = slice.reference(0)?;
    }
}

pub(super) fn execute_pq_mldsa44(engine: &mut Engine) -> Status {
    // An unavailable instruction charges only the invalid-instruction base,
    // before any stack inspection, exactly as the native version wrapper does.
    ...
    if engine.cc.stack.depth() < 4 { fail!(ExceptionCode::StackUnderflow); }
    ...
    let status = unsafe { tos_rust_mldsa44_native_verify(signature.as_ptr(), message.as_ptr(),
        message.len(), context.as_ptr(), context.len(), key.as_ptr()) };
    let valid = match status {
        0 => true,
        -6 => false, // The pinned backend's MLD_ERR_INVALID_SIGNATURE.
        _ => { fail!(ExceptionCode::FatalError, "ML-DSA verifier backend failure"); }
    };
