...
use std::{fs, env};
use tos_vm::{executor::{Engine, BehaviorModifiers, gas::gas_state::Gas},
    stack::{Stack, StackItem, savelist::SaveList}, error::tvm_exception_code};
...
fn main() -> anyhow::Result<()> {
    ...
    for line in data.lines() {
        ...
        let version=f[1].parse::<u32>()?; let budget=f[2].parse::<i64>()?;
        let repeats=f[4].parse::<usize>()?;
        anyhow::ensure!((1..=11).contains(&repeats) && budget>=0,"invalid limits");
        let mut stack=Stack::new();
        for _ in 0..repeats { for field in &f[6..10] {
            match *field {
                "skip"=>{}, "int"=>{stack.push(StackItem::int(1));},
                value=>{let raw=hex::decode(value)?; stack.push(StackItem::Cell(read_single_root_boc(raw)?));}
            }
        }}
        let mut code=BuilderData::new();
        for i in 0..repeats {code.append_raw(&[0xf9,0x31,0],24)?;if i+1<repeats{code.append_raw(&[0x30],8)?;}}
        let mut engine=Engine::with_capabilities(0).setup_checked(code.into_cell()?, SaveList::new(), stack,
            Gas::test_with_limit(budget),vec![])?;
        engine.set_block_version(version);
        let mut modifiers=BehaviorModifiers::default();modifiers.chksig_always_succeed=f[3]=="1";
        ...
        let gas=engine.gas_used(); let committed=engine.is_committed_state();
