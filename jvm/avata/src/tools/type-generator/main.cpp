...
using namespace vm;
using namespace avata::tools::typegenerator;
    ...
    if (f.aliasName.size() > 0) {
      if (fields.find(f.aliasName) == fields.end()) {
        if (fields.find(f.field->name) != fields.end()) {
          ...
          // TODO: this currently works around how avata uses an object (either
...
void writeClassAccessors(Output* out, Module& module, Class* cl)
{
  ...
  if (cl->arrayField) {
    ...
    out->write("> ");
    out->write(obfuscate(f->name));
    out->write("() { return avata::util::Slice<");
    if (isFieldGcVisible(module, f)) {
      out->write("const ");
    }
    out->write(cppFieldType(module, f));
...
int main(int ac, char** av)
{
  ...
  class MyAllocator : public avata::util::Alloc {
   public:
