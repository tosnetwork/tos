  ...
  *t = static_cast<Thread*>(m->localThread->get());
  if (*t) {
    if (version <= JNI_VERSION_1_6) {
      ...
      return AVATA_JNI_EVERSION;
    }
  } else {
    return AVATA_JNI_EDETACHED;
  }
}
                                         ...
                                         jint mode)
{
  ...
  if (mode == 0 or mode == AVATA_JNI_COMMIT) {
    if (size) {
      memcpy((*array)->body().begin(), p, size);
    }
  }

  if (mode == 0 or mode == AVATA_JNI_ABORT) {
...
size_t parseSize(const char* s)
{
  ...
  if (errno or end == s or *end) {
    return 0;
  }

  uint64_t max = static_cast<uint64_t>(~static_cast<size_t>(0));
...
extern "C" AVATA_EXPORT jint JNICALL JNI_GetDefaultJavaVMInitArgs(void*)
{
  return 0;
}
...
extern "C" AVATA_EXPORT jstring JNICALL JVM_GetTemporaryDirectory(JNIEnv* e UNUSED)
{
  // Unimplemented
...
extern "C" AVATA_EXPORT jboolean JNICALL JVM_KnownToNotExist(JNIEnv* e UNUSED, jobject loader UNUSED, jstring classname UNUSED)
{
  // Unimplemented
  abort();
}

extern "C" AVATA_EXPORT jintArray JNICALL JVM_GetResourceLookupCache(JNIEnv* e UNUSED, jobject loader UNUSED, jstring resourcename UNUSED)
{
  // Unimplemented
  abort();
}
