...
class Context {
 public:
  Context(System* system, size_t limit)
      : system(system),
        client(0),
        count(0),
        limit(limit),
  ...
  if (Verbose) {
    if (count > c->limit) {
      if (not c->limitWasExceeded) {
        ...
        fprintf(stderr, "heap limit %zu exceeded: %zu\n", c->limit, count);
      }
    } else if (c->limitWasExceeded) {
      c->limitWasExceeded = false;
      fprintf(
          stderr, "heap limit %zu no longer exceeded: %zu\n", c->limit, count);
    }
  }
...
void* allocate(Context* c, size_t size, bool limit)
{
  ...
  if ((not limit) or (size <= c->limit and c->count <= c->limit - size)) {
...
class MyHeap : public Heap {
 public:
  MyHeap(System* system, size_t limit) : c(system, limit)
  {
  }
...
Heap* makeHeap(System* system, size_t limit)
{
