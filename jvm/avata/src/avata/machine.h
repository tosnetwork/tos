...
class Machine {
 public:
  ...
  static const unsigned ContractHelperGasCostCount = 14;
...
AVATA_EXPORT void vmPrintTrace(vm::Thread* t);

AVATA_EXPORT void vmfPrintTrace(vm::Thread* t, FILE* out);
...
inline void NO_RETURN throw_(Thread* t, GcThrowable* e)
{
  ...
  if (objectClass(t, e) == type(t, GcOutOfMemoryError::Type)) {
    if (not t->m->dumpedHeapOnOOM) {
      ...
      const char* path = findProperty(t, "avata.heap.dump");
      if (path) {
...
{
  if (t->m->errorLog == 0) {
    const char* path = findProperty(t, "avata.error.log");
    if (path) {
...
AVATA_EXPORT void* vmAddressFromLine(vm::Thread* t,
                                     vm::object m,
                                     unsigned line);
