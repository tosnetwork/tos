...
class Zone : public avata::util::AllocOnly {
 public:
  class Segment {
   public:
  ...
  Zone(avata::util::Allocator* allocator, size_t minimumFootprint)
      : allocator(allocator),
        segment(0),
 ...
 private:
  ...
  bool tryEnsure(unsigned space)
  {
    if (segment == 0 or segment->position + space > segment->size) {
      unsigned size = padToPage(
          avata::util::max(
              space,
              avata::util::max(minimumFootprint,
  ...
  avata::util::Allocator* allocator;
  Segment* segment;
