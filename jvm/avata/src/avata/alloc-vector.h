...
class Vector {
 public:
  Vector(avata::util::Aborter* a,
         avata::util::Alloc* allocator,
  ...
  void ensure(size_t space)
  {
    if (position + space > data.count) {
      ...
      size_t newCapacity = avata::util::max(
          position + space, avata::util::max(minimumCapacity, data.count * 2));
      if (data.begin()) {
        data.resize(allocator, newCapacity);
      } else {
        data = avata::util::Slice<uint8_t>::alloc(allocator, newCapacity);
      }
    }
  }
  ...
  avata::util::Aborter* a;
  avata::util::Alloc* allocator;
  avata::util::Slice<uint8_t> data;
  size_t position;
  size_t minimumCapacity;
};
