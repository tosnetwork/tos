    ...
    bool wait(System::Thread* context, int64_t time, bool clearInterrupted)
    {
      ...
      if (owner_ == t) {
...

        {
          ...
          bool hasDeadline = time and time < INT64_C(31536000000000000);
          int64_t then = hasDeadline ? s->now() + time : 0;
          timespec ts = {static_cast<long>(then / 1000),
                         static_cast<long>((then % 1000) * 1000 * 1000)};

          while (not interrupted and (t->flags & Notified) == 0) {
            int rv UNUSED;
            if (hasDeadline) {
              rv = pthread_cond_timedwait(&(t->condition), &(t->mutex), &ts);
              expect(s, rv == 0 or rv == ETIMEDOUT or rv == EINTR);
              if (rv == ETIMEDOUT) {
                break;
              }
            } else {
              rv = pthread_cond_wait(&(t->condition), &(t->mutex));
...
AVATA_EXPORT System* makeSystem(bool reentrant)
{
