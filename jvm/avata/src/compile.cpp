 ...
 public:
  ...
  MyThread(Machine* m,
      ...
      : Thread(m, javaThread, parent),
        ...
        trace(0),
        reference(0),
        arch(parent ? parent->arch : avata::codegen::makeArchitectureNative(
                                         m->system,
                                         useNativeFeatures)),
        transition(0),
        traceContext(0),
  ...
  CallTrace* trace;
  Reference* reference;
  avata::codegen::Architecture* arch;
  Context* transition;
...
class PoolElement : public avata::codegen::Promise {
 public:
...
class TraceElement : public avata::codegen::TraceHandler {
 public:
  ...
  virtual void handleTrace(avata::codegen::Promise* address,
...
class TraceElementPromise : public avata::codegen::Promise {
 public:
...
class BootContext {
 public:
  ...
  GcTriple* calls;
  avata::codegen::DelayedPromise* addresses;
  avata::codegen::DelayedPromise* addressSentinal;
  Zone* zone;
  OffsetResolver* resolver;
  JavaVM* hostVM;
};
 ...
 public:
  ...
  class MyClient : public Compiler::Client {
   public:
    ...
    virtual intptr_t getThunk(avata::codegen::lir::UnaryOperation, unsigned)
    {
      abort(t);
    }

    virtual intptr_t getThunk(avata::codegen::lir::BinaryOperation op,
                              unsigned size,
                              unsigned resultSize)
    {
      if (size == 8) {
        switch (op) {
        case avata::codegen::lir::Absolute:
          assertT(t, resultSize == 8);
          return local::getThunk(t, absoluteLongThunk);

        case avata::codegen::lir::FloatNegate:
          assertT(t, resultSize == 8);
          return local::getThunk(t, negateDoubleThunk);

        case avata::codegen::lir::FloatSquareRoot:
          assertT(t, resultSize == 8);
          return local::getThunk(t, squareRootDoubleThunk);

        case avata::codegen::lir::Float2Float:
          assertT(t, resultSize == 4);
          return local::getThunk(t, doubleToFloatThunk);

        case avata::codegen::lir::Float2Int:
          if (resultSize == 8) {
        ...
        case avata::codegen::lir::Int2Float:
          if (resultSize == 8) {
        ...
        case avata::codegen::lir::FloatAbsolute:
          assertT(t, resultSize == 4);
    ...
    virtual intptr_t getThunk(avata::codegen::lir::TernaryOperation op,
                              unsigned size,
                              unsigned,
                              bool* threadParameter)
    {
      *threadParameter = false;
      ...
      if (size == 8) {
        switch (op) {
        case avata::codegen::lir::Divide:
          *threadParameter = true;
          return local::getThunk(t, divideLongThunk);

        case avata::codegen::lir::Remainder:
          *threadParameter = true;
          return local::getThunk(t, moduloLongThunk);

        case avata::codegen::lir::FloatAdd:
          return local::getThunk(t, addDoubleThunk);

        case avata::codegen::lir::FloatSubtract:
          return local::getThunk(t, subtractDoubleThunk);

        case avata::codegen::lir::FloatMultiply:
          return local::getThunk(t, multiplyDoubleThunk);

        case avata::codegen::lir::FloatDivide:
          return local::getThunk(t, divideDoubleThunk);

        case avata::codegen::lir::FloatRemainder:
          return local::getThunk(t, moduloDoubleThunk);

        case avata::codegen::lir::JumpIfFloatEqual:
        case avata::codegen::lir::JumpIfFloatNotEqual:
        case avata::codegen::lir::JumpIfFloatLess:
        case avata::codegen::lir::JumpIfFloatGreater:
        case avata::codegen::lir::JumpIfFloatLessOrEqual:
        case avata::codegen::lir::JumpIfFloatGreaterOrUnordered:
        case avata::codegen::lir::JumpIfFloatGreaterOrEqualOrUnordered:
          return local::getThunk(t, compareDoublesGThunk);

        case avata::codegen::lir::JumpIfFloatGreaterOrEqual:
        case avata::codegen::lir::JumpIfFloatLessOrUnordered:
        case avata::codegen::lir::JumpIfFloatLessOrEqualOrUnordered:
  ...
  MyThread* thread;
  Zone zone;
  avata::codegen::Assembler* assembler;
  MyClient client;
  avata::codegen::Compiler* compiler;
  GcMethod* method;
  BootContext* bootContext;
...
avata::util::FixedAllocator* codeAllocator(MyThread* t);
...
class Frame {
 public:
  ...
  ir::Value* append(object o)
  {
    ...
    if (bc) {
      avata::codegen::Promise* p = new (bc->zone)
          avata::codegen::ListenPromise(t->m->system, bc->zone);
  ...
  avata::codegen::Promise* addressPromise(avata::codegen::Promise* p)
  {
    BootContext* bc = context->bootContext;
    if (bc) {
      bc->addresses = new (bc->zone) avata::codegen::DelayedPromise(
  ...
  ir::Value* addressOperand(avata::codegen::Promise* p)
  {
    return c->promiseConstant(p, ir::Type::iptr());
  }

  ir::Value* absoluteAddressOperand(avata::codegen::Promise* p)
  {
    return context->bootContext
               ? c->binaryOp(
                     ...
                     c->promiseConstant(
                         new (&context->zone) avata::codegen::OffsetPromise(
                             p,
  ...
  *targetIp = 0;
  while (*targetIp == 0) {
    ...
    if (method) {
      ...
      if (avata::codegen::TailCalls and argumentFootprint > alignment) {
...
void compileDirectInvoke(MyThread* t,
                         ...
                         bool tailCall,
                         bool useThunk,
                         avata::codegen::Promise* addressPromise)
{
  avata::codegen::Compiler* c = frame->c;

  unsigned flags
      = (avata::codegen::TailCalls and tailCall ? Compiler::TailJump : 0);
  unsigned traceFlags;
  ...
  if (emptyMethod(t, target) and (not classNeedsInit(t, target->class_()))) {
    ...
    if (bc) {
      if ((target->class_() == frame->context->method->class_()
          ...
          and (not(avata::codegen::TailCalls and tailCall
                   and (target->flags() & ACC_NATIVE)))) {
        avata::codegen::Promise* p = new (bc->zone)
            avata::codegen::ListenPromise(t->m->system, bc->zone);
...
{
  return avata::codegen::TailCalls
...
loop:
  ...
  while (ip < code->length()) {
    ...
    if (DebugInstructions) {
      ...
      avata::jvm::debug::printInstruction(code->body().begin(), startingIp);
      fprintf(stderr, "\n");
    }
    ...
    case lookupswitch: {
      ...
      if (pairCount) {
        ...
        for (int32_t i = 0; i < pairCount; ++i) {
          ...
          avata::codegen::Promise* p = c->poolAppend(key);
          if (i == 0) {
            start = p;
          }
    ...
    case tableswitch: {
      ...
      for (int32_t i = 0; i < top - bottom + 1; ++i) {
        ...
        avata::codegen::Promise* p = c->poolAppendPromise(
...
GcArray* translateExceptionHandlerTable(MyThread* t,
                                        ...
                                        intptr_t end)
{
  avata::codegen::Compiler* c = context->compiler;
...
void finish(MyThread* t, FixedAllocator* allocator, Context* context)
{
  ...
  BootContext* bc = context->bootContext;
  if (bc) {
    for (avata::codegen::DelayedPromise* p = bc->addresses;
         p != bc->addressSentinal;
         p = p->next) {
      p->basis = new (bc->zone)
          avata::codegen::ResolvedPromise(p->basis->value());
    }
  }
...
#endif // not AVATA_AOT_ONLY

void updateCall(MyThread* t,
                avata::codegen::lir::UnaryOperation op,
                void* returnAddress,
                void* target)
{
      ...
      if (unwindContext and unwindContext->continuation()) {
        ...
        if (compileRoots(t)->rewindMethod() == 0) {
          ...
          GcMethod* method = resolveMethod(
              ...
              "avata/Continuations",
              "rewind",
              "(Ljava/lang/Runnable;Lavata/Callback;Ljava/lang/Object;"
...

  {
    ...
    if (compileRoots(t)->windMethod() == 0) {
      GcMethod* method = resolveMethod(
          ...
          "Ljava/lang/Runnable;)Lavata/Continuations$UnwindResult;");
...
class MyProcessor : public Processor {
 public:
        ...
        dynamicTableSize(0)
  {
    ...
    using namespace avata::codegen::runtime;
                ...
                const char* spec)
{
  ...
  if (not open) {
    open = true;
    const char* path = findProperty(t, "avata.jit.log");
    if (path) {
  ...
  if (updateCaller) {
    ...
    if (node->flags() & TraceElement::LongCall) {
      if (node->flags() & TraceElement::TailCall) {
        op = avata::codegen::lir::AlignedLongJump;
      } else {
        op = avata::codegen::lir::AlignedLongCall;
      }
    } else if (node->flags() & TraceElement::TailCall) {
      op = avata::codegen::lir::AlignedJump;
    } else {
      op = avata::codegen::lir::AlignedCall;
    }
...
using avata::codegen::OperandInfo;
namespace lir = avata::codegen::lir;

void compileCall(MyThread* t, Context* c, ThunkIndex index, bool call = true)
{
  avata::codegen::Assembler* a = c->assembler;
  ...
  if (processor(t)->bootImage) {
    ...
    lir::Constant proc(new (&c->zone) avata::codegen::ResolvedPromise(
...
{
  Context context(t);
  avata::codegen::Assembler* a = context.assembler;
                              ...
                              const char* baseName)
{
  ...
  avata::codegen::ResolvedPromise indexPromise(index);
  ...
  avata::codegen::ResolvedPromise promise(thunk);
