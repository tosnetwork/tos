...
extern "C" JNIEXPORT void JNICALL
    Java_java_lang_Runtime_kill(JNIEnv* e UNUSED,
                                jclass,
                                jlong pid,
                                jint in,
                                jint out,
...
#else
  std::wstring culture = AvataInterop::GetCurrentUICulture();
  char* cultureName
...
#error "unknown architecture"
  ...
  add(e,
      ...
      index++,
      L"java.io.tmpdir=%ls",
      AvataInterop::GetTemporaryFolder().c_str());
  ...
  add(e,
      ...
      index++,
      L"user.dir=%ls",
      AvataInterop::GetInstalledLocation().c_str());
  ...
  add(e,
      ...
      index++,
      L"user.home=%ls",
      AvataInterop::GetDocumentsLibraryLocation().c_str());
...
{
  // The Java-level System.currentTimeMillis() traps in the TOS profile.  Keep
  // this legacy JNI entry deterministic in case it is reached by future
  // classpath changes.
  return 0;
}
