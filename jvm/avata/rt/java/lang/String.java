  ...
  implements Comparable<String>, CharSequence, Serializable
{
  ...
  private static char[] decodeBytes(byte[] bytes, int offset, int length,
                                    String charsetName)
    throws UnsupportedEncodingException
  {
    ...
    if (charsetName.equalsIgnoreCase(UTF_8_ENCODING)) {
      ...
      if (c == null) {
        throw new RuntimeException("unable to parse bytes as " + charsetName);
      }
      return c;
    } else if (charsetName.equalsIgnoreCase(ISO_8859_1_ENCODING)
               || charsetName.equalsIgnoreCase(LATIN_1_ENCODING)) {
      return Iso88591.decode(bytes, offset, length);
    } else {
