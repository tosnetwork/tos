Avata - A lightweight Java Virtual Machine (JVM)
================================================
...
[![Build Status](https://travis-ci.org/ReadyTalk/avata.png?branch=master)](https://travis-ci.org/ReadyTalk/avata)

Quick Start
-----------

These are examples of building Avata on various operating systems for
...
#### on Linux:
    ...
    $ make
    $ build/linux-x86_64/avata -cp build/linux-x86_64/test Hello

#### on Mac OS X:
    $ export JAVA_HOME=$(/usr/libexec/java_home)
    $ make
    $ build/macosx-x86_64/avata -cp build/macosx-x86_64/test Hello

#### on Windows (Cygwin):
    $ git clone git@github.com:ReadyTalk/win64.git ../win64
    $ export JAVA_HOME="/cygdrive/c/Program Files/Java/jdk1.7.0_45"
    $ make
    $ build/windows-x86_64/avata -cp build/windows-x86_64/test Hello

#### on FreeBSD:
    $ export JAVA_HOME=/usr/local/openjdk7
    $ gmake
    $ build/freebsd-x86_64/avata -cp build/freebsd-x86_64/test Hello


Introduction
------------

Avata is a lightweight virtual machine and class library designed to
...
Avata can currently target the following platforms:
...
avata.Continuations methods callWithCurrentContinuation and
...
default Avata class library.  See "Building with the OpenJDK Class
...
example of an Xcode project for iOS which uses Avata.
...
directory containing the avata directory)
...
Avata targets MSVC 11 and above (it uses c++ features not available in older versions).
...
By default, Avata uses its own lightweight class library.  However,
...
features beyond that subset, you may want to tell Avata to use
...
This will build Avata as a conventional JVM (e.g. libjvm.so) which
...
remain installed for Avata to work, and you can run applications like
this:

    $ build/linux-x86_64-openjdk/avata-dynamic -cp /path/to/my/application \
...
application is simplified:
    ...
    $ build/linux-x86_64-openjdk-src/avata -cp /path/to/my/application \
        com.example.MyApplication
...
an LZMA-enabled version:
    ...
    $ upx --lzma --best build/linux-x86_64-openjdk-src/avata
...
[app.mk](https://github.com/ReadyTalk/avata-swt-examples/blob/master/app.mk)
in the _avata-swt-examples_ project for an example of using Avata,
OpenJDK, ProGuard, and UPX in concert.

Here are some examples of how to install OpenJDK and build Avata with
it on various OSes:
...
---------------------------------------
As an alternative to both the Avata and OpenJDK class libaries, you
can also build with the Android class library. Now it should work on Linux, OS X and Windows.

The simpliest way to build Avata with Android classpath is to use `avata-pack` project: https://github.com/bigfatbrowncat/avata-pack

Avata-pack consists of Avata itself with some Android components (such as libcore and icu4c).
...
Installing Avata is as simple as copying the executable to the desired
directory:

    $ cp build/${platform}-${arch}/avata ~/bin/
...
stand-alone executable out of a Java application using Avata.
...
__1.__ Build Avata, create a new directory, and populate it with the
VM object files and bootstrap classpath jar.

    $ make
    $ mkdir hello
    $ cd hello
    $ ar x ../build/${platform}-${arch}/libavata.a
...
If you've built Avata using the `lzma` option, you may optionally
...
VM object files.
    ...
    $ mkdir hello
    $ cd hello
    $ ar x ../build/linux-x86_64-bootimage/libavata.a
...
this function via the "avata.bootimage" property.
...
The Avata project is not affiliated with Oracle.
