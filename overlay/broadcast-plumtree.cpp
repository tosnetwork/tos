...
tl_object_ptr<tos_api::overlay_plumtreeStatsRecord> clone_stats_record(
    const tos_api::overlay_plumtreeStatsRecord &record) {
  return create_tl_object<tos_api::overlay_plumtreeStatsRecord>(
...
td::Bits256 compute_broadcast_id(td::uint32 flags, const td::Bits256 &full_data_hash, td::uint32 full_data_size,
                                 td::uint32 part_size) {
  return get_tl_object_sha_bits256(create_tl_object<tos_api::overlay_broadcastPlumtreeFec_id>(
...
td::BufferSlice make_fec_payload_to_sign(const td::Bits256 &broadcast_id, double timestamp, td::uint32 part_index,
  ...
  return create_serialize_tl_object<tos_api::overlay_broadcastPlumtreeFec_toSign>(
...
td::BufferSlice make_simple_payload_to_sign(const td::Bits256 &broadcast_id, double timestamp, td::uint32 tree_index,
  ...
  return create_serialize_tl_object<tos_api::overlay_broadcastPlumtreeSimple_toSign>(
...
class BroadcastsPlumtree::Impl {
 public:
  ...
  td::actor::Task<> process_fec_payload(OverlayImpl *overlay, adnl::AdnlNodeIdShort from,
                                        tl_object_ptr<tos_api::overlay_broadcastPlumtreeFec> msg,
  ...
  td::actor::Task<> process_simple_payload(OverlayImpl *overlay, adnl::AdnlNodeIdShort from,
                                           tl_object_ptr<tos_api::overlay_broadcastPlumtreeSimple> msg,
                                           PlumtreePayloadOrigin origin = PlumtreePayloadOrigin::Push,
                                           const MissingPartKey *expected_key = nullptr);
  td::actor::Task<> process_ihave(OverlayImpl *overlay, adnl::AdnlNodeIdShort from,
                                  tl_object_ptr<tos_api::overlay_broadcastPlumtreeIHave> msg);
  void process_repair_query(OverlayImpl *overlay, adnl::AdnlNodeIdShort from,
                            tos_api::overlay_repairPlumtreePart &query, td::Promise<td::BufferSlice> promise);
  ...
  td::actor::Task<> process_prune(OverlayImpl *overlay, adnl::AdnlNodeIdShort from,
                                  tl_object_ptr<tos_api::overlay_broadcastPlumtreePrune> msg);
  td::actor::Task<> process_useful(OverlayImpl *overlay, adnl::AdnlNodeIdShort from,
                                   tl_object_ptr<tos_api::overlay_broadcastPlumtreeUseful> msg);
  td::actor::Task<> process_stats_push(OverlayImpl *overlay, adnl::AdnlNodeIdShort from,
                                       tl_object_ptr<tos_api::overlay_plumtreeStatsPush> msg);
  std::vector<tl_object_ptr<tos_api::overlay_plumtreeStatsRecord>> stats_collect_records() const;
 ...
 private:
  ...
  bool send_control(OverlayImpl *overlay, const adnl::AdnlNodeIdShort &dst,
                    tl_object_ptr<tos_api::overlay_Broadcast> control);
...
bool BroadcastsPlumtree::Impl::is_inactive_eager_peer(adnl::AdnlNodeIdShort peer, td::Timestamp now,
  ...
  if (now - PLUMTREE_EAGER_PEER_INACTIVITY_TTL < it->second.last_active_at) {
    return false;
  }
...
void BroadcastsPlumtree::Impl::send_repair_requests(OverlayImpl *overlay, const MissingPartKey &key,
  ...
  while (missing.sent_repair_targets < missing.repair_targets.size()) {
    ...
    auto query = create_serialize_tl_object<tos_api::overlay_repairPlumtreePart>(broadcast_id, td::Clocks::system(),
...
td::Result<td::BufferSlice> BroadcastsPlumtree::Impl::prepare_fec_payload_for_peer(OverlayImpl *overlay,
  ...
  auto wire = create_serialize_tl_object<tos_api::overlay_broadcastPlumtreeFec>(
...
td::Result<td::BufferSlice> BroadcastsPlumtree::Impl::prepare_simple_payload_for_peer(
  ...
  auto wire = create_serialize_tl_object<tos_api::overlay_broadcastPlumtreeSimple>(
...
void BroadcastsPlumtree::Impl::send_ihave_to(OverlayImpl *overlay, PlumtreePartState &part,
  ...
  if (send_control(overlay, dst,
                   create_tl_object<tos_api::overlay_broadcastPlumtreeIHave>(
...
void BroadcastsPlumtree::Impl::send_prune(OverlayImpl *overlay, const adnl::AdnlNodeIdShort &dst,
  ...
  send_control(overlay, dst,
               create_tl_object<tos_api::overlay_broadcastPlumtreePrune>(broadcast_id, td::Clocks::system(),
                                                                         static_cast<std::int32_t>(part_index),
                                                                         static_cast<std::int32_t>(tree_index)));
}
...
void BroadcastsPlumtree::Impl::send_useful(OverlayImpl *overlay, const adnl::AdnlNodeIdShort &dst,
  ...
  send_control(overlay, dst,
               create_tl_object<tos_api::overlay_broadcastPlumtreeUseful>(broadcast_id, td::Clocks::system(),
...
td::actor::Task<> BroadcastsPlumtree::Impl::process_fec_payload(
    OverlayImpl *overlay, adnl::AdnlNodeIdShort from, tl_object_ptr<tos_api::overlay_broadcastPlumtreeFec> msg,
...
td::actor::Task<> BroadcastsPlumtree::Impl::process_simple_payload(
    OverlayImpl *overlay, adnl::AdnlNodeIdShort from, tl_object_ptr<tos_api::overlay_broadcastPlumtreeSimple> msg,
    PlumtreePayloadOrigin origin, const MissingPartKey *expected_key) {
  if (from.is_zero()) {
...
void BroadcastsPlumtree::Impl::process_repair_query(OverlayImpl *overlay, adnl::AdnlNodeIdShort from,
                                                    tos_api::overlay_repairPlumtreePart &query,
  ...
  if (!has_state(broadcast_id)) {
    if (overlay->is_delivered(broadcast_id)) {
      promise.set_value(create_serialize_tl_object<tos_api::overlay_broadcastNotFound>());
    } else {
...
td::actor::Task<> BroadcastsPlumtree::Impl::process_repair_response(OverlayImpl *overlay, adnl::AdnlNodeIdShort from,
  ...
  auto bcast = fetch_tl_object<tos_api::overlay_Broadcast>(std::move(data), true);
  if (bcast.is_error()) {
    co_return bcast.move_as_error_prefix("invalid Plumtree repair response: ");
  }
  auto obj = bcast.move_as_ok();
  switch (obj->get_id()) {
    case tos_api::overlay_broadcastPlumtreeFec::ID:
      co_await process_fec_payload(overlay, from, move_tl_object_as<tos_api::overlay_broadcastPlumtreeFec>(obj),
                                   PlumtreePayloadOrigin::RepairResponse, &expected_key);
      break;
    case tos_api::overlay_broadcastPlumtreeSimple::ID:
      co_await process_simple_payload(overlay, from, move_tl_object_as<tos_api::overlay_broadcastPlumtreeSimple>(obj),
                                      PlumtreePayloadOrigin::RepairResponse, &expected_key);
      break;
    case tos_api::overlay_broadcastNotFound::ID:
      break;
    default:
...
td::actor::Task<> BroadcastsPlumtree::Impl::process_useful(
    OverlayImpl *overlay, adnl::AdnlNodeIdShort from, tl_object_ptr<tos_api::overlay_broadcastPlumtreeUseful> msg) {
  if (from.is_zero()) {
    co_return td::Status::Error(ErrorCode::protoviolation, "missing Plumtree immediate sender");
  }
  auto control = get_control_fields(*msg);
...
void BroadcastsPlumtree::Impl::stats_tick(OverlayImpl *overlay) {
  ...
  if (!stats_epoch_.snapshot_done && stats_epoch_.snapshot_at.is_in_past()) {
    ...
    stats_epoch_.store[src] = create_tl_object<tos_api::overlay_plumtreeStatsRecord>(
...
std::vector<tl_object_ptr<tos_api::overlay_plumtreeStatsRecord>> BroadcastsPlumtree::Impl::stats_collect_records()
    const {
  std::vector<tl_object_ptr<tos_api::overlay_plumtreeStatsRecord>> records;
...
void BroadcastsPlumtree::Impl::stats_forward(OverlayImpl *overlay, const tos_api::overlay_plumtreeStatsRecord &record,
                                             const adnl::AdnlNodeIdShort &from) {
  for (const auto &dst : slots_[PLUMTREE_SIMPLE_TREE_INDEX].eager) {
    if (dst != from) {
      send_control(overlay, dst, create_tl_object<tos_api::overlay_plumtreeStatsPush>(clone_stats_record(record)));
    }
  }
}
...
void BroadcastsPlumtree::Impl::gc(OverlayImpl *overlay) {
  ...
  while (!broadcasts_.empty()) {
    ...
    CHECK(bcast);
    if (broadcast_expired_before < bcast->first_seen_at) {
      break;
    }
...
std::vector<tl_object_ptr<tos_api::overlay_plumtreeStatsRecord>> BroadcastsPlumtree::collect_stats_records() const {
