...
void OverlayManager::register_overlay(adnl::AdnlNodeIdShort local_id, OverlayIdShort overlay_id,
  ...
  if (it == overlays_.end()) {
    td::actor::send_closure(adnl_, &adnl::Adnl::subscribe, local_id,
                            adnl::Adnl::int_to_bytestring(tos_api::overlay_message::ID),
                            std::make_unique<AdnlCallback>(actor_id(this)));
    td::actor::send_closure(adnl_, &adnl::Adnl::subscribe, local_id,
                            adnl::Adnl::int_to_bytestring(tos_api::overlay_query::ID),
                            std::make_unique<AdnlCallback>(actor_id(this)));
    td::actor::send_closure(adnl_, &adnl::Adnl::subscribe, local_id,
                            adnl::Adnl::int_to_bytestring(tos_api::overlay_messageWithExtra::ID),
                            std::make_unique<AdnlCallback>(actor_id(this)));
    td::actor::send_closure(adnl_, &adnl::Adnl::subscribe, local_id,
                            adnl::Adnl::int_to_bytestring(tos_api::overlay_queryWithExtra::ID),
                            std::make_unique<AdnlCallback>(actor_id(this)));
  }
...
void OverlayManager::receive_message(adnl::AdnlNodeIdShort src, adnl::AdnlNodeIdShort dst, td::BufferSlice data) {
  ...
  tl_object_ptr<tos_api::overlay_messageExtra> extra;
  td::Slice parsed_data = data;
  auto R = fetch_tl_prefix<tos_api::overlay_messageWithExtra>(parsed_data, true);
  if (R.is_ok()) {
    overlay_id = OverlayIdShort{R.ok()->overlay_};
    extra = std::move(R.ok()->extra_);
  } else {
    auto R2 = fetch_tl_prefix<tos_api::overlay_message>(parsed_data, true);
    if (R2.is_ok()) {
...
void OverlayManager::receive_query(adnl::AdnlNodeIdShort src, adnl::AdnlNodeIdShort dst, td::BufferSlice data,
  ...
  tl_object_ptr<tos_api::overlay_messageExtra> extra;
  td::Slice parsed_data = data;
  auto R = fetch_tl_prefix<tos_api::overlay_queryWithExtra>(parsed_data, true);
  if (R.is_ok()) {
    overlay_id = OverlayIdShort{R.ok()->overlay_};
    extra = std::move(R.ok()->extra_);
  } else {
    auto R2 = fetch_tl_prefix<tos_api::overlay_query>(parsed_data, true);
    if (R2.is_ok()) {
      overlay_id = OverlayIdShort{R2.ok()->overlay_};
    } else {
...
void OverlayManager::send_query_via(adnl::AdnlNodeIdShort dst, adnl::AdnlNodeIdShort src, OverlayIdShort overlay_id,
  ...
  auto extra = create_tl_object<tos_api::overlay_messageExtra>();
  extra->flags_ = 0;
  ...
  td::BufferSlice serialized_query =
      (extra_flags ? create_serialize_tl_object_suffix<tos_api::overlay_queryWithExtra>(
                         query.as_slice(), overlay_id.tl(), std::move(extra))
                   : create_serialize_tl_object_suffix<tos_api::overlay_query>(query.as_slice(), overlay_id.tl()));
...
void OverlayManager::send_message_via(adnl::AdnlNodeIdShort dst, adnl::AdnlNodeIdShort src, OverlayIdShort overlay_id,
  ...
  td::BufferSlice serialized_message =
      (extra_flags ? create_serialize_tl_object_suffix<tos_api::overlay_messageWithExtra>(
                         object.as_slice(), overlay_id.tl(), std::move(extra))
                   : create_serialize_tl_object_suffix<tos_api::overlay_message>(object.as_slice(), overlay_id.tl()));
...
void OverlayManager::save_to_db(adnl::AdnlNodeIdShort local_id, OverlayIdShort overlay_id,
  ...
  std::vector<tl_object_ptr<tos_api::overlay_node>> nodes_vec;
  for (auto &n : nodes) {
    nodes_vec.push_back(n.tl());
  }
  auto obj = create_tl_object<tos_api::overlay_nodes>(std::move(nodes_vec));

  auto key = create_hash_tl_object<tos_api::overlay_db_key_nodes>(local_id.bits256_value(), overlay_id.bits256_value());
  db_.set(key, create_serialize_tl_object<tos_api::overlay_db_nodes>(std::move(obj)), [](td::Result<td::Unit>) {});
}

void OverlayManager::get_stats(td::Promise<tl_object_ptr<tos_api::engine_validator_overlaysStats>> promise) {
  class Cb : public td::actor::Actor {
   public:
    Cb(td::Promise<tl_object_ptr<tos_api::engine_validator_overlaysStats>> promise) : promise_(std::move(promise)) {
    }
    void incr_pending() {
      pending_++;
    }
    void decr_pending() {
      if (!--pending_) {
        std::sort(res_.begin(), res_.end(),
                  [](const tl_object_ptr<tos_api::engine_validator_overlayStats> &a,
                     const tl_object_ptr<tos_api::engine_validator_overlayStats> &b) {
        ...
        promise_.set_result(create_tl_object<tos_api::engine_validator_overlaysStats>(std::move(res_)));
        stop();
      }
    }
    void receive_answer(tl_object_ptr<tos_api::engine_validator_overlayStats> res) {
      if (res) {
   ...
   private:
    std::vector<tl_object_ptr<tos_api::engine_validator_overlayStats>> res_;
    size_t pending_{1};
    td::Promise<tl_object_ptr<tos_api::engine_validator_overlaysStats>> promise_;
  };
  ...
  for (auto &a : overlays_) {
    for (auto &b : a.second) {
      ...
      td::actor::send_closure(b.second.overlay, &Overlay::get_stats,
                              [act](td::Result<tl_object_ptr<tos_api::engine_validator_overlayStats>> R) {
                                if (R.is_ok()) {
...
td::BufferSlice Certificate::to_sign(OverlayIdShort overlay_id, PublicKeyHash issued_to) const {
  if (flags_ == cert_default_flags(max_size_)) {
    return create_serialize_tl_object<tos_api::overlay_certificateId>(overlay_id.tl(), issued_to.tl(), expire_at_,
                                                                      max_size_);
  } else {
    return create_serialize_tl_object<tos_api::overlay_certificateIdV2>(overlay_id.tl(), issued_to.tl(), expire_at_,
                                                                        max_size_, flags_);
  }
}
...
td::Result<std::shared_ptr<Certificate>> Certificate::create(const tl_object_ptr<tos_api::overlay_Certificate> &cert) {
  std::shared_ptr<Certificate> res;
  tos_api::downcast_call(*cert.get(),
                         td::overloaded([&](tos_api::overlay_emptyCertificate &obj) { res = nullptr; },
                                        [&](tos_api::overlay_certificate &obj) {
                                        ...
                                        [&](tos_api::overlay_certificateV2 &obj) {
                                          res = std::make_shared<Certificate>(PublicKey{obj.issued_by_}, obj.expire_at_,
                                                                              static_cast<td::uint32>(obj.max_size_),
...
tl_object_ptr<tos_api::overlay_Certificate> Certificate::tl() const {
  if (flags_ == cert_default_flags(max_size_)) {
    return create_tl_object<tos_api::overlay_certificate>(issued_by_.get<PublicKey>().tl(), expire_at_, max_size_,
                                                          signature_.clone_as_buffer_slice());
  }
  return create_tl_object<tos_api::overlay_certificateV2>(issued_by_.get<PublicKey>().tl(), expire_at_, max_size_,
                                                          flags_, signature_.clone_as_buffer_slice());
}

tl_object_ptr<tos_api::overlay_Certificate> Certificate::empty_tl() {
  return create_tl_object<tos_api::overlay_emptyCertificate>();
}

OverlayMemberCertificate::OverlayMemberCertificate(const tos_api::overlay_MemberCertificate *cert) {
  if (!cert) {
    expire_at_ = std::numeric_limits<td::int32>::max();
    return;
  }
  if (cert->get_id() == tos_api::overlay_emptyMemberCertificate::ID) {
    expire_at_ = std::numeric_limits<td::int32>::max();
    return;
  }
  CHECK(cert->get_id() == tos_api::overlay_memberCertificate::ID);
  const auto *real_cert = static_cast<const tos_api::overlay_memberCertificate *>(cert);
