...
class Certificate {
 public:
  ...
  tl_object_ptr<tos_api::overlay_Certificate> tl() const;
  const PublicKey &issuer() const;
  const PublicKeyHash issuer_hash() const;
  const td::SharedSlice &signature() const;

  static td::Result<std::shared_ptr<Certificate>> create(const tl_object_ptr<tos_api::overlay_Certificate> &cert);
  static tl_object_ptr<tos_api::overlay_Certificate> empty_tl();
...
class OverlayMemberCertificate {
 public:
  ...
  explicit OverlayMemberCertificate(const tos_api::overlay_MemberCertificate *cert);
  ...
  tl_object_ptr<tos_api::overlay_MemberCertificate> tl() const {
    if (empty()) {
      return create_tl_object<tos_api::overlay_emptyMemberCertificate>();
    }
    return create_tl_object<tos_api::overlay_memberCertificate>(signed_by_.tl(), flags_, slot_, expire_at_,
  ...
  td::BufferSlice to_sign_data(const adnl::AdnlNodeIdShort &node) const {
    return tos::create_serialize_tl_object<tos::tos_api::overlay_memberCertificateId>(node.tl(), flags_, slot_,
                                                                                      expire_at_);
  }
  ...
  class Callback {
   public:
    ...
    virtual void receive_query(adnl::AdnlNodeIdShort src, OverlayIdShort overlay_id, td::BufferSlice data,
      ...
      // Default to a structured error rather than silently dropping the
      // promise. Concrete overlay implementations that do not support
      // queries should follow the same contract.
...
inline StringBuilder &operator<<(StringBuilder &stream, const tos::overlay::OverlayIdShort &value) {
