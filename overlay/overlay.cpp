...
void OverlayImpl::process_query(adnl::AdnlNodeIdShort src, tos_api::overlay_getRandomPeers &query,
...
void OverlayImpl::process_query(adnl::AdnlNodeIdShort src, tos_api::overlay_ping &query,
                                td::Promise<td::BufferSlice> promise) {
  promise.set_value(create_serialize_tl_object<tos_api::overlay_pong>());
}

void OverlayImpl::process_query(adnl::AdnlNodeIdShort src, tos_api::overlay_getBroadcast &query,
                                td::Promise<td::BufferSlice> promise) {
  broadcasts_simple_.process_query(src, query, std::move(promise));
}

void OverlayImpl::process_query(adnl::AdnlNodeIdShort src, tos_api::overlay_getBroadcastList &query,
                                td::Promise<td::BufferSlice> promise) {
...
void OverlayImpl::process_query(adnl::AdnlNodeIdShort src, tos_api::overlay_repairPlumtreePart &query,
                                td::Promise<td::BufferSlice> promise) {
  if (!opts_.enable_plumtree_broadcast_) {
...
/*void OverlayImpl::process_query(adnl::AdnlNodeIdShort src, adnl::AdnlQueryId query_id, tos_api::overlay_customQuery
&query) { callback_->receive_query(src, query_id, id_, std::move(query.data_));
}
*/

void OverlayImpl::receive_query(adnl::AdnlNodeIdShort src, tl_object_ptr<tos_api::overlay_messageExtra> extra,
  ...
  VLOG(OVERLAY_EXTRA_DEBUG) << this << "query from " << src << ": " << tos_api::to_string(Q);

  tos_api::downcast_call(*Q.get(), [&](auto &object) { this->process_query(src, object, std::move(promise)); });
}
...
td::actor::Task<> OverlayImpl::process_broadcast(adnl::AdnlNodeIdShort message_from,
                                                 tl_object_ptr<tos_api::overlay_broadcastNotFound> bcast) {
...
td::actor::Task<> OverlayImpl::process_broadcast(adnl::AdnlNodeIdShort message_from,
                                                 tl_object_ptr<tos_api::overlay_broadcastTwostepSimple> bcast) {
  if (opts_.twostep_broadcast_sender_.empty()) {
...
td::actor::Task<> OverlayImpl::process_broadcast(adnl::AdnlNodeIdShort message_from,
                                                 tl_object_ptr<tos_api::overlay_broadcastTwostepFec> bcast) {
  if (opts_.twostep_broadcast_sender_.empty()) {
    co_return td::Status::Error("twostep broadcasts are not enabled");
  }
  co_await broadcasts_twostep_.process_broadcast(this, message_from, std::move(bcast));
...
td::actor::Task<> OverlayImpl::process_broadcast(adnl::AdnlNodeIdShort message_from,
                                                 tl_object_ptr<tos_api::overlay_broadcastPlumtreeFec> bcast) {
  if (!opts_.enable_plumtree_broadcast_) {
...
td::actor::Task<> OverlayImpl::process_broadcast(adnl::AdnlNodeIdShort message_from,
                                                 tl_object_ptr<tos_api::overlay_broadcastPlumtreeSimple> bcast) {
  if (!opts_.enable_plumtree_broadcast_) {
    co_return td::Status::Error("Plumtree broadcasts are not enabled");
  }
  if (peer_list_.local_member_flags_ & OverlayMemberFlags::DoNotReceiveBroadcasts) {
...
void OverlayImpl::receive_message(adnl::AdnlNodeIdShort src, tl_object_ptr<tos_api::overlay_messageExtra> extra,
  ...
  auto X = fetch_tl_object<tos_api::overlay_Broadcast>(data.clone(), true);
  if (X.is_error()) {
  ...
  auto Q = X.move_as_ok();
  tos_api::downcast_call(*Q, [self = this, &Q, &src](auto &object) {
...
void OverlayImpl::alarm() {
  ...
  if (update_throughput_at_.is_in_past()) {
    ...
    VLOG(OVERLAY_WARNING) << "Overlay_traffic"
...
void OverlayImpl::receive_dht_nodes(dht::DhtValue v) {
  ...
  auto R = fetch_tl_object<tos_api::overlay_nodes>(v.value().clone(), true);
  if (R.is_ok()) {
    auto r = R.move_as_ok();
    VLOG(OVERLAY_INFO) << this << ": received " << r->nodes_.size() << " nodes from overlay";
    VLOG(OVERLAY_EXTRA_DEBUG) << this << ": nodes: " << tos_api::to_string(r);
...
void OverlayImpl::update_dht_nodes(OverlayNode node) {
  ...
  auto nodes = create_tl_object<tos_api::overlay_nodes>(std::vector<tl_object_ptr<tos_api::overlay_node>>());
...
void OverlayImpl::send_broadcast(PublicKeyHash send_as, td::uint32 flags, td::BufferSlice data) {
  ...
  if (!has_valid_broadcast_certificate(send_as, data.size(), false,
                                       flags & Overlays::BroadcastFlagAnySender())) {
...
void OverlayImpl::get_stats(td::Promise<tl_object_ptr<tos_api::engine_validator_overlayStats>> promise) {
  auto res = create_tl_object<tos_api::engine_validator_overlayStats>();
  ...
  iterate_all_peers([&](const adnl::AdnlNodeIdShort &key, const OverlayPeer &peer) {
    auto node_obj = create_tl_object<tos_api::engine_validator_overlayStatsNode>();
  ...
  res->stats_.push_back(
      create_tl_object<tos_api::engine_validator_oneStat>("neighbours_cnt", PSTRING() << neighbours_cnt()));
  res->stats_.push_back(create_tl_object<tos_api::engine_validator_oneStat>("plumtree_neighbours_cnt",
...
tl_object_ptr<tos_api::engine_validator_overlayStatsTraffic> TrafficStats::tl() const {
  return create_tl_object<tos_api::engine_validator_overlayStatsTraffic>(out_bytes, in_bytes, out_packets, in_packets);
}
...
tl_object_ptr<tos_api::engine_validator_overlayStatsBroadcasts> BroadcastsLimiter::tl(PublicKeyHash src, double now) {
  init_stats(now);
  return create_tl_object<tos_api::engine_validator_overlayStatsBroadcasts>(
