...
void OverlayImpl::add_peers(const tl_object_ptr<tos_api::overlay_nodes> &nodes, bool received_from_peer) {
...
void OverlayImpl::add_peers(const tl_object_ptr<tos_api::overlay_nodesV2> &nodes, bool received_from_peer) {
  for (auto &n : nodes->nodes_) {
    auto N = OverlayNode::create(n);
    if (N.is_ok()) {
      add_peer(N.move_as_ok(), received_from_peer);
