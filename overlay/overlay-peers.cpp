...
td::Status OverlayImpl::validate_peer_certificate(const adnl::AdnlNodeIdShort &node,
                                                  tos_api::overlay_MemberCertificate *cert) {
...
td::actor::Task<> OverlayImpl::process_pending_peer(OverlayNode node) {
  ...
  for (size_t attempt = 0; attempt < 3; ++attempt) {
    ...
    td::actor::send_closure(manager_, &OverlayManager::send_query, node.adnl_id_short(), local_id_, overlay_id_,
                            ...
                            create_serialize_tl_object<tos_api::overlay_ping>());
...
void OverlayImpl::add_peers(const tl_object_ptr<tos_api::overlay_nodes> &nodes, bool verified,
...
void OverlayImpl::receive_random_peers(adnl::AdnlNodeIdShort src, td::Result<td::BufferSlice> R, double elapsed) {
  ...
  auto R2 = fetch_tl_object<tos_api::overlay_nodes>(R.move_as_ok(), true);
  if (R2.is_error()) {
...
void OverlayImpl::send_random_peers_cont(adnl::AdnlNodeIdShort src, OverlayNode node,
  ...
  if (promise) {
    auto Q = create_tl_object<tos_api::overlay_nodes>(std::move(vec));
    ...
    auto Q =
        create_tl_object<tos_api::overlay_getRandomPeers>(create_tl_object<tos_api::overlay_nodes>(std::move(vec)));
...
void OverlayImpl::ping_random_peers() {
  ...
  for (const adnl::AdnlNodeIdShort &peer : peers) {
    ...
    td::actor::send_closure(manager_, &OverlayManager::send_query, peer, local_id_, overlay_id_, "overlay ping",
                            std::move(P), td::Timestamp::in(5.0), create_serialize_tl_object<tos_api::overlay_ping>());
  }
}
...
void OverlayImpl::update_neighbours(td::uint32 nodes_to_change, bool allow_delete) {
  ...
  update_list(
      peer_list_.neighbours_, max_neighbours(), nodes_to_change, [](OverlayPeer *peer) { return peer->is_neighbour(); },
      [](OverlayPeer *peer, bool value) { peer->set_neighbour(value); }, "neighbour", false);
  if (max_plumtree_neighbours() > 0) {
    auto plumtree_nodes_to_change = nodes_to_change == 0 ? 0 : std::max<td::uint32>(nodes_to_change, 5);
    // Sample this list from the entire member set: members flagged as not
    // receiving broadcasts still take their share of the slots, which keeps
    // the announce fan-out from concentrating on the receiving members. The
    // flag is honored at send time instead.
    update_list(
        peer_list_.plumtree_neighbours_, max_plumtree_neighbours(), plumtree_nodes_to_change,
        [](OverlayPeer *peer) { return peer->is_plumtree_neighbour(); },
        [](OverlayPeer *peer, bool value) { peer->set_plumtree_neighbour(value); }, "Plumtree neighbour", true);
  }
}
...
void OverlayImpl::receive_nodes_from_db(tl_object_ptr<tos_api::overlay_nodes> tl_nodes) {
  if (overlay_type_ != OverlayType::FixedMemberList) {
...
bool OverlayImpl::check_src_peer(const adnl::AdnlNodeIdShort &src,
                                 const tos_api::overlay_MemberCertificate *certificate) {
