...
static Overlay::BroadcastHash compute_broadcast_id(PublicKeyHash source, const fec::FecType &fec_type,
                                                   ...
                                                   td::uint32 flags) {
  return get_tl_object_sha_bits256(create_tl_object<tos_api::overlay_broadcastFec_id>(
...
td::Status BroadcastFecPart::run(OverlayImpl *overlay, BroadcastFec &bcast) {
  ...
  TRY_STATUS(bcast.add_part(
      seqno_, data_.clone(),
      create_serialize_tl_object<tos_api::overlay_broadcastFecShort>(
          source_.tl(), cert_ ? cert_->tl() : Certificate::empty_tl(), broadcast_hash_, part_data_hash_, seqno_,
          signature_.clone()),
      create_serialize_tl_object<tos_api::overlay_broadcastFec>(
...
td::BufferSlice BroadcastFecPart::to_sign() {
  auto obj = create_tl_object<tos_api::overlay_broadcast_toSign>(part_hash_, date_);
