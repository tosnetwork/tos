...
void store(const tos::ZeroStateIdExt &zero_state_id, StorerT &storer) {
  using td::store;
  using toslib::store;
...
void parse(tos::ZeroStateIdExt &zero_state_id, ParserT &parser) {
  using td::parse;
  using toslib::parse;
...
template <class StorerT>
void store(const tos::BlockId &block_id, StorerT &storer) {
  using td::store;
  using toslib::store;
...
template <class ParserT>
void parse(tos::BlockId &block_id, ParserT &parser) {
  using td::parse;
  using toslib::parse;
...
template <class StorerT>
void store(const tos::BlockIdExt &block_id, StorerT &storer) {
  using td::store;
  using toslib::store;
...
template <class ParserT>
void parse(tos::BlockIdExt &block_id, ParserT &parser) {
  using td::parse;
  using toslib::parse;
...
struct LastBlockState {
  tos::ZeroStateIdExt zero_state_id;
  tos::BlockIdExt last_key_block_id;
  tos::BlockIdExt last_block_id;
  td::int64 utime{0};
  tos::BlockIdExt init_block_id;
...
class LastBlock : public td::actor::Actor {
 ...
 private:
  ...
  void do_check_init_block(tos::BlockIdExt from, tos::BlockIdExt to);
  ...
  void on_masterchain_info(td::Result<tos::tos_api::object_ptr<tos::lite_api::liteServer_masterchainInfo>> r_info);
  void do_get_last_block();
  void on_block_proof(tos::BlockIdExt from,
                      td::Result<tos::tos_api::object_ptr<tos::lite_api::liteServer_partialBlockProof>> r_block_proof);
  ...
  void update_zero_state(tos::ZeroStateIdExt zero_state_id, td::Slice source);

  bool update_mc_last_block(tos::BlockIdExt mc_block_id);
  bool update_mc_last_key_block(tos::BlockIdExt mc_key_block_id);
  void update_utime(td::int64 utime);
  bool update_init_block(tos::BlockIdExt init_block_id);
