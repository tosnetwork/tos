...
void LastBlock::sync_loop() {
  ...
  update_zero_state(tos::ZeroStateIdExt(config_.zero_state_id.id.workchain, config_.zero_state_id.root_hash,
...
void LastBlock::do_get_last_block() {
  ...
  client_.send_query(
      tos::lite_api::liteServer_getBlockProof(0, create_tl_lite_block_id(state_.last_key_block_id), nullptr),
...
td::Result<std::unique_ptr<block::BlockProofChain>> LastBlock::process_block_proof(
    tos::BlockIdExt from,
    td::Result<tos::tos_api::object_ptr<tos::lite_api::liteServer_partialBlockProof>> r_block_proof) {
  ...
  TRY_RESULT_PREFIX(chain, TRY_VM(process_block_proof(from, std::move(block_proof))),
                    ToslibError::ValidateBlockProof());
  return std::move(chain);
}

td::Result<std::unique_ptr<block::BlockProofChain>> LastBlock::process_block_proof(
    tos::BlockIdExt from, tos::tos_api::object_ptr<tos::lite_api::liteServer_partialBlockProof> block_proof) {
...
void LastBlock::on_block_proof(
  ...
  if (!chain) {
    get_last_block_state_ = QueryState::Empty;
    on_sync_error(ToslibError::ValidateBlockProof().move_as_error_suffix("empty proof chain"));
    return;
  }
  update_state(*chain);
  if (chain->complete) {
    if (chain->to.seqno() + 100 < max_seqno_ && max_seqno_ != 0) {
      get_last_block_state_ = QueryState::Empty;
      on_sync_error(ToslibError::LiteServerNetwork().move_as_error_suffix(
...
void LastBlock::on_init_block_proof(
    tos::BlockIdExt from, tos::BlockIdExt to,
    td::Result<tos::tos_api::object_ptr<tos::lite_api::liteServer_partialBlockProof>> r_block_proof) {
  ...
  if (chain->complete && chain->to != to) {
    check_init_block_state_ = QueryState::Empty;
    on_sync_error(ToslibError::ValidateBlockProof().move_as_error_suffix(
...
void LastBlock::on_masterchain_info(
    td::Result<tos::tos_api::object_ptr<tos::lite_api::liteServer_masterchainInfo>> r_info) {
  if (r_info.is_ok()) {
...
void LastBlock::update_zero_state(tos::ZeroStateIdExt zero_state_id, td::Slice source) {
  if (has_fatal_error()) {
    return;
  }
  ...
  on_fatal_error(ToslibError::ValidateZeroState(PSLICE() << "Masterchain zerostate mismatch: expected: "
                                                         << state_.zero_state_id.to_str() << ", found "
                                                         << zero_state_id.to_str() << " from " << source));
}

bool LastBlock::update_mc_last_block(tos::BlockIdExt mc_block_id) {
  if (has_fatal_error()) {
    return false;
  }
...
bool LastBlock::update_mc_last_key_block(tos::BlockIdExt mc_key_block_id) {
  if (has_fatal_error()) {
    return false;
  }
...
bool LastBlock::update_init_block(tos::BlockIdExt init_block_id) {
  if (has_fatal_error()) {
    return false;
  }
...
void LastBlock::tear_down() {
  on_sync_error(ToslibError::Cancelled());
}
