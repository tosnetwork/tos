...
class ToslibCli : public td::actor::Actor {
 public:
  struct Options {
  ...
  ToslibCli(Options options) : options_(std::move(options)) {
  }

 private:
  Options options_;
  td::actor::ActorOwn<td::TerminalIO> io_;
  td::actor::ActorOwn<toslib::ToslibClient> client_;
  ...
  struct Address {
    toslib_api::object_ptr<toslib_api::accountAddress> address;
    std::string public_key;
    td::SecureString secret;
    auto input_key(td::Slice password = "") const {
      return !secret.empty() ? make_object<toslib_api::inputKeyRegular>(
                                   make_object<toslib_api::key>(public_key, secret.copy()), td::SecureString(password))
                             : nullptr;
    }
    auto toslib_api() const {
      return make_object<toslib_api::accountAddress>(address->account_address_);
    }
  };

  std::map<std::uint64_t, td::Promise<toslib_api::object_ptr<toslib_api::Object>>> query_handlers_;
  ...
  void start_up() override {
    class Cb : public td::TerminalIO::Callback {
     public:
      void line_cb(td::BufferSlice line) override {
        td::actor::send_closure(id_, &ToslibCli::parse_line, std::move(line));
      }
      Cb(td::actor::ActorShared<ToslibCli> id) : id_(std::move(id)) {
      }

     private:
      td::actor::ActorShared<ToslibCli> id_;
    };
    ...
    class ToslibCb : public toslib::ToslibCallback {
     public:
      ToslibCb(td::actor::ActorShared<ToslibCli> id) : id_(std::move(id)) {
      }
      void on_result(std::uint64_t id, toslib_api::object_ptr<toslib_api::Object> result) override {
        send_closure(id_, &ToslibCli::on_toslib_result, id, std::move(result));
      }
      void on_error(std::uint64_t id, toslib_api::object_ptr<toslib_api::error> error) override {
        send_closure(id_, &ToslibCli::on_toslib_error, id, std::move(error));
      }
    ...
    ref_cnt_++;
    client_ = td::actor::create_actor<toslib::ToslibClient>("Toslib", td::make_unique<ToslibCb>(actor_shared(this, 1)));
    ...
    if (options_.use_callbacks_for_network) {
      auto config = toslib::Config::parse(options_.config).move_as_ok();
    ...
    auto config = !options_.config.empty()
                      ? make_object<toslib_api::config>(options_.config, options_.name,
                                                        options_.use_callbacks_for_network, options_.ignore_cache)
                      : nullptr;

    toslib_api::object_ptr<toslib_api::KeyStoreType> ks_type;
    if (options_.in_memory) {
      ks_type = make_object<toslib_api::keyStoreTypeInMemory>();
    } else {
      ks_type = make_object<toslib_api::keyStoreTypeDirectory>(options_.key_dir);
    }
    send_query(make_object<toslib_api::init>(make_object<toslib_api::options>(std::move(config), std::move(ks_type))),
               [&](auto r_ok) {
                 ...
                 if (r_ok.is_ok()) {
                   ...
                   load_channnels();
                   td::TerminalIO::out() << "Toslib is inited\n";
                   if (options_.one_shot) {
                     td::actor::send_closure(actor_id(this), &ToslibCli::parse_line, td::BufferSlice(options_.cmd));
                   }
                 }
               });
  }
  ...
  void rwallet_address(td::ConstParser& parser, td::Promise<td::Unit> promise) {
    ...
    TRY_RESULT_PROMISE(
        promise, addr,
        sync_send_query(make_object<toslib_api::getAccountAddress>(
            make_object<toslib_api::rwallet_initialAccountState>(address.public_key, public_key, wallet_id_ - 1), 1,
            -1)));
  ...
  void rwallet_init(td::ConstParser& parser, td::Promise<td::Unit> promise) {
    ...
    auto public_key = parser.read_word().str();
    auto initial_state =
        make_object<toslib_api::rwallet_initialAccountState>(address.public_key, public_key, wallet_id_ - 1);
    TRY_RESULT_PROMISE(
        promise, addr,
        sync_send_query(make_object<toslib_api::getAccountAddress>(
            make_object<toslib_api::rwallet_initialAccountState>(address.public_key, public_key, wallet_id_ - 1), 1,
    ...
    auto config = make_object<toslib_api::rwallet_config>();
    config->start_at_ = start_at;
    for (auto limit : limits) {
      config->limits_.push_back(make_object<toslib_api::rwallet_limit>(limit.first, limit.second));
    }
    auto action =
        make_object<toslib_api::actionRwallet>(make_object<toslib_api::rwallet_actionInit>(std::move(config)));
    send_query(make_object<toslib_api::createQuery>(address.input_key(), std::move(addr), 60, std::move(action),
                                                    std::move(initial_state)),
               promise.send_closure(actor_id(this), &ToslibCli::transfer2, false));
  }
  ...
  struct Channel {
    ...
    auto to_address() {
      return make_object<toslib_api::accountAddress>(address);
    }
    auto to_init_state() {
      return make_object<toslib_api::pchan_initialAccountState>(make_object<toslib_api::pchan_config>(
          alice_public_key, make_object<toslib_api::accountAddress>(alice_address), bob_public_key,
          make_object<toslib_api::accountAddress>(bob_address), init_timeout, close_timeout, channel_id));
    }
  };
  void store_channels() {
  ...
  td::Status do_pchan_create(td::ConstParser& parser, bool gen_channel_id) {
    ...
    TRY_RESULT(addr, sync_send_query(make_object<toslib_api::getAccountAddress>(channel.to_init_state(), -1, 0)));
  ...
  void pchan_promise_make2(toslib_api::object_ptr<toslib_api::pchan_promise> ans, td::Promise<td::Unit> promise) {
    td::TerminalIO::out() << "Signature (base64url):" << td::base64url_encode(ans->signature_) << "\n";
    send_query(make_object<toslib_api::pchan_packPromise>(std::move(ans)), promise.wrap([](auto&& ans) {
  ...
  void pchan_promise_make(td::ConstParser& parser, td::Promise<td::Unit> promise) {
    ...
    send_query(make_object<toslib_api::pchan_signPromise>(
                   addr.input_key(),
                   make_object<toslib_api::pchan_promise>("", promise_A.nano, promise_B.nano, chan.channel_id)),
               promise.send_closure(actor_id(this), &ToslibCli::pchan_promise_make2));
  }
  void pchan_promise_check(td::ConstParser& parser, td::Promise<td::Unit> promise) {
    ...
    send_query(make_object<toslib_api::pchan_validatePromise>(
                   public_key, make_object<toslib_api::pchan_promise>(std::move(signature), promise_A.nano,
  ...
  void pchan_init_2(Address addr, td::int32 pchan_id, td::int64 value,
                    toslib_api::object_ptr<toslib_api::query_info> query, td::Promise<td::Unit> promise) {
    std::vector<toslib_api::object_ptr<toslib_api::msg_message>> messages;
    messages.push_back(make_object<toslib_api::msg_message>(
        channels_[pchan_id].to_address(), "", value, std::vector<toslib_api::object_ptr<toslib_api::extraCurrency>>{},
        make_object<toslib_api::msg_dataRaw>(query->body_, query->init_state_), -1));
    auto action = make_object<toslib_api::actionMsg>(std::move(messages), true);
    send_query(
        make_object<toslib_api::createQuery>(addr.input_key(), std::move(addr.address), 60, std::move(action), nullptr),
        promise.send_closure(actor_id(this), &ToslibCli::transfer2, false));
  }
  ...
  void pchan_close2(td::int32 pchan_id, Address addr, toslib_api::object_ptr<toslib_api::pchan_promise> pchan_promise,
                    td::Promise<td::Unit> promise) {
    auto action = make_object<toslib_api::actionPchan>(
        make_object<toslib_api::pchan_actionClose>(0, 0, std::move(pchan_promise)));
    //send_query(make_object<toslib_api::createQuery>(addr.input_key(), channels_[pchan_id].to_address(), 60,
    //std::move(action), channels_[pchan_id].to_init_state()),
  ...
  void pchan_close(td::int32 pchan_id, td::ConstParser& parser, td::Promise<td::Unit> promise) {
    ...
    TRY_RESULT_PROMISE_PREFIX(promise, packed_promise, base64url_decode(parser.read_word()), "promise");
    send_query(make_object<toslib_api::pchan_unpackPromise>(td::SecureString(packed_promise)),
               promise.send_closure(actor_id(this), &ToslibCli::pchan_close2, pchan_id, std::move(addr)));
  }
  ...
  void do_dns_resolve(std::string name, td::Bits256 category, td::int32 ttl,
                      toslib_api::object_ptr<toslib_api::dns_resolved> resolved, td::Promise<td::Unit> promise) {
    if (resolved->entries_.empty()) {
      td::TerminalIO::out() << "No dns entries found\n";
      promise.set_value(td::Unit());
      return;
    }
    if (resolved->entries_[0]->entry_->get_id() == toslib_api::dns_entryDataNextResolver::ID && ttl != 0) {
      td::TerminalIO::out() << "Redirect resolver\n";
      auto entry = toslib_api::move_object_as<toslib_api::dns_entryDataNextResolver>(resolved->entries_[0]->entry_);
      send_query(make_object<toslib_api::dns_resolve>(std::move(entry->resolver_), name, category, ttl),
                 promise.send_closure(actor_id(this), &ToslibCli::do_dns_resolve, name, category, 0));
      return;
    }
  ...
  void dns_resolve(td::ConstParser& parser, td::Promise<td::Unit> promise) {
    ...
    std::vector<toslib_api::object_ptr<toslib_api::dns_entry>> entries;
  ...
  void dns_cmd(td::Slice cmd, td::ConstParser& parser, td::Promise<td::Unit> promise) {
    ...
    std::vector<tos::ManualDns::ActionExt> actions_ext;
    if (cmd == "cmd") {
      TRY_RESULT_PROMISE_ASSIGN(promise, actions_ext, tos::ManualDns::parse(parser.read_all()));
    } else if (cmd == "cmdfile") {
      TRY_RESULT_PROMISE(promise, file_data, td::read_file(parser.read_word().str()));
      TRY_RESULT_PROMISE_ASSIGN(promise, actions_ext, tos::ManualDns::parse(file_data));
    }

    std::vector<toslib_api::object_ptr<toslib_api::dns_Action>> actions;
    for (auto& action : actions_ext) {
      if (action.name.empty()) {
        actions.push_back(make_object<toslib_api::dns_actionDeleteAll>());
        td::TerminalIO::out() << "Delete all dns entries\n";
      } else if (action.category.is_zero()) {
        actions.push_back(make_object<toslib_api::dns_actionDelete>(action.name, td::Bits256::zero()));
        td::TerminalIO::out() << "Delete all dns enties with name: " << action.name << "\n";
      } else if (!action.data) {
        actions.push_back(make_object<toslib_api::dns_actionDelete>(action.name, action.category));
        ...
        TRY_RESULT_PROMISE(promise, data, toslib::to_toslib_api(action.data.value()));
        ...
        td::TerminalIO::out() << "Set dns entry: " << action.name << ":" << action.category << " " << sb.as_cslice()
                              << "\n";
        actions.push_back(make_object<toslib_api::dns_actionSet>(
            make_object<toslib_api::dns_entry>(action.name, action.category, std::move(data))));
      }
    }

    auto action = make_object<toslib_api::actionDns>(std::move(actions));

    td::Slice password;  // empty by default

    auto key = !address.secret.empty() ? make_object<toslib_api::inputKeyRegular>(
                                             make_object<toslib_api::key>(address.public_key, address.secret.copy()),
                                             td::SecureString(password))
                                       : nullptr;
    send_query(make_object<toslib_api::createQuery>(std::move(key), std::move(address.address), 60, std::move(action),
                                                    nullptr),
               promise.send_closure(actor_id(this), &ToslibCli::transfer2, false));
  }

  void remote_time(td::Promise<td::Unit> promise) {
    send_query(make_object<toslib_api::liteServer_getInfo>(), promise.wrap([](auto&& info) {
  ...
  void send_file(td::Slice name, td::Promise<td::Unit> promise) {
    ...
    send_query(make_object<toslib_api::raw_sendMessage>(std::move(data)), promise.wrap([](auto&& info) {
  ...
  void save_account(td::Slice cmd, td::Slice path, td::Slice address, td::Promise<td::Unit> promise) {
    ...
    send_query(make_object<toslib_api::smc_load>(std::move(addr.address)),
               promise.send_closure(actor_id(this), &ToslibCli::save_account_2, cmd.str(), path.str(), address.str()));
  }

  void save_account_2(std::string cmd, std::string path, std::string address,
                      toslib_api::object_ptr<toslib_api::smc_info> info, td::Promise<td::Unit> promise) {
    auto with_query = [&, self = this](auto query, auto log) {
      send_query(std::move(query),
                 promise.send_closure(actor_id(self), &ToslibCli::save_account_3, std::move(path), std::move(log)));
    };
    if (cmd == "saveaccount") {
      with_query(make_object<toslib_api::smc_getState>(info->id_), PSTRING() << "StateInit of account " << address);
    } else if (cmd == "saveaccountcode") {
      with_query(make_object<toslib_api::smc_getCode>(info->id_), PSTRING() << "Code of account " << address);
    } else if (cmd == "saveaccountdata") {
      with_query(make_object<toslib_api::smc_getData>(info->id_), PSTRING() << "Data of account " << address);
    } else {
      promise.set_error(td::Status::Error("Unknown query"));
    }
  }

  void save_account_3(std::string path, std::string log, toslib_api::object_ptr<toslib_api::tvm_cell> cell,
                      td::Promise<td::Unit> promise) {
  ...
  void sync(td::Promise<td::Unit> promise, bool update_last) {
    send_query(make_object<toslib_api::sync>(), promise.wrap([&, update_last](auto&& block) {
  ...
  td::Result<toslib_api::object_ptr<toslib_api::tvm_StackEntry>> parse_stack_entry(td::Slice str) {
    ...
    if (str[0] == '"') {
      ...
      return make_object<toslib_api::tvm_stackEntrySlice>(
          make_object<toslib_api::tvm_slice>(vm::std_boc_serialize(cb.finalize()).ok().as_slice().str()));
    }
    if (l >= 3 && (str[0] == 'x' || str[0] == 'b') && str[1] == '{' && str.back() == '}') {
      ...
      return make_object<toslib_api::tvm_stackEntrySlice>(make_object<toslib_api::tvm_slice>(
    ...
    return make_object<toslib_api::tvm_stackEntryNumber>(make_object<toslib_api::tvm_numberDecimal>(dec_string(num)));
  }

  td::Result<std::vector<toslib_api::object_ptr<toslib_api::tvm_StackEntry>>> parse_stack(td::ConstParser& parser,
                                                                                          td::Slice end_token) {
    std::vector<toslib_api::object_ptr<toslib_api::tvm_StackEntry>> stack;
    while (true) {
      auto word = parser.read_word();
      ...
      if (word == "[") {
        ...
        stack.push_back(
            make_object<toslib_api::tvm_stackEntryTuple>(make_object<toslib_api::tvm_tuple>(std::move(elements))));
      } else if (word == "(") {
        TRY_RESULT(elements, parse_stack(parser, ")"));
        stack.push_back(
            make_object<toslib_api::tvm_stackEntryList>(make_object<toslib_api::tvm_list>(std::move(elements))));
      } else {
  ...
  static void store_entry(td::StringBuilder& sb, toslib_api::tvm_StackEntry& entry) {
    downcast_call(entry, td::overloaded(
                             [&](toslib_api::tvm_stackEntryCell& cell) {
                             ...
                             [&](toslib_api::tvm_stackEntrySlice& cell) {
                             ...
                             [&](toslib_api::tvm_stackEntryNumber& cell) { sb << cell.number_->number_; },
                             [&](toslib_api::tvm_stackEntryTuple& cell) {
                               sb << "(";
                             ...
                             [&](toslib_api::tvm_stackEntryList& cell) {
                               sb << "[";
                             ...
                             [&](toslib_api::tvm_stackEntryUnsupported& cell) { sb << "<UNSUPPORTED>"; }));
  }
  ...
  void run_method(td::ConstParser& parser, td::Promise<td::Unit> promise) {
    ...
    toslib_api::object_ptr<toslib_api::smc_MethodId> method;
    if (std::all_of(method_str.begin(), method_str.end(), [](auto c) { return c >= '0' && c <= '9'; })) {
      method = make_object<toslib_api::smc_methodIdNumber>(td::to_integer<td::int32>(method_str.str()));
    } else {
      method = make_object<toslib_api::smc_methodIdName>(method_str.str());
    }
    ...
    auto to_run = make_object<toslib_api::smc_runGetMethod>(0 /*fixme*/, std::move(method), std::move(stack));

    send_query(make_object<toslib_api::smc_load>(std::move(addr.address)),
               promise.send_closure(actor_id(this), &ToslibCli::run_method_2, std::move(to_run)));
  }

  void run_method_2(toslib_api::object_ptr<toslib_api::smc_runGetMethod> to_run,
                    toslib_api::object_ptr<toslib_api::smc_info> info, td::Promise<td::Unit> promise) {
    to_run->id_ = info->id_;
    send_query(std::move(to_run), promise.send_closure(actor_id(this), &ToslibCli::run_method_3));
  }
  void run_method_3(toslib_api::object_ptr<toslib_api::smc_runResult> info, td::Promise<td::Unit> promise) {
    td::StringBuilder sb;
  ...
  void set_validate_config(td::Slice cmd, td::Slice path, td::Slice name, bool use_callback, bool ignore_cache,
    ...
    auto config = make_object<toslib_api::config>(std::move(data), name.str(), use_callback, ignore_cache);
    if (cmd == "setconfig") {
      send_query(make_object<toslib_api::options_setConfig>(std::move(config)), promise.wrap([](auto&& info) {
        td::TerminalIO::out() << "Config is set\n";
        return td::Unit();
      }));
    } else {
      send_query(make_object<toslib_api::options_validateConfig>(std::move(config)), promise.wrap([](auto&& info) {
  ...
  void on_adnl_result(td::uint64 id, td::Result<td::BufferSlice> res) {
    if (res.is_ok()) {
      ...
      send_query(make_object<toslib_api::onLiteServerQueryResult>(id, res.move_as_ok().as_slice().str()),
                 [](auto r_ok) { LOG_IF(ERROR, r_ok.is_error()) << r_ok.error(); });
    } else {
      send_query(make_object<toslib_api::onLiteServerQueryError>(
                     id, make_object<toslib_api::error>(res.error().code(), res.error().message().str())),
                 [](auto r_ok) { LOG_IF(ERROR, r_ok.is_error()) << r_ok.error(); });
    }
  }
  ...
  void on_toslib_result(std::uint64_t id, toslib_api::object_ptr<toslib_api::Object> result) {
    if (id == 0) {
      switch (result->get_id()) {
        case toslib_api::updateSendLiteServerQuery::ID: {
          auto update = toslib_api::move_object_as<toslib_api::updateSendLiteServerQuery>(std::move(result));
          ...
          send_closure(raw_client_, &liteclient::ExtClient::send_query, "query", td::BufferSlice(update->data_),
                       ...
                       [actor_id = actor_id(this), id = update->id_](td::Result<td::BufferSlice> res) {
                         send_closure(actor_id, &ToslibCli::on_adnl_result, id, std::move(res));
                       });
          return;
        }
        case toslib_api::updateSyncState::ID: {
          auto update = toslib_api::move_object_as<toslib_api::updateSyncState>(std::move(result));
          switch (update->sync_state_->get_id()) {
            case toslib_api::syncStateDone::ID: {
              td::TerminalIO::out() << "synchronization: DONE in "
                                    << td::format::as_time(td::Time::now() - sync_started_.at()) << "\n";
              sync_started_ = {};
              break;
            }
            case toslib_api::syncStateInProgress::ID: {
              if (!sync_started_) {
                sync_started_ = td::Timestamp::now();
              }
              auto progress = toslib_api::move_object_as<toslib_api::syncStateInProgress>(update->sync_state_);
  ...
  void on_toslib_error(std::uint64_t id, toslib_api::object_ptr<toslib_api::error> error) {
    auto it = query_handlers_.find(id);
    if (it == query_handlers_.end()) {
      return;
    }
  ...
  void send_query(toslib_api::object_ptr<QueryT> query, td::Promise<typename QueryT::ReturnType> promise) {
    if (is_closing_) {
      return;
    }
    toslib_api::object_ptr<toslib_api::Function> func = std::move(query);
    if (block_mode_ == BlockMode::Manual && func->get_id() != toslib_api::sync::ID) {
      if (!current_block_) {
    ...
    td::actor::send_closure(client_, &toslib::ToslibClient::request, query_id, std::move(func));
    query_handlers_[query_id] =
        [promise = std::move(promise)](td::Result<toslib_api::object_ptr<toslib_api::Object>> r_obj) mutable {
          if (r_obj.is_error()) {
            return promise.set_error(r_obj.move_as_error());
          }
          promise.set_value(tos::move_tl_object_as<typename QueryT::ReturnType::element_type>(r_obj.move_as_ok()));
        };
  }

  template <class QueryT>
  td::Result<typename QueryT::ReturnType> sync_send_query(toslib_api::object_ptr<QueryT> query) {
    if (is_closing_) {
      return td::Status::Error("Closing");
    }
    auto r_obj = toslib::ToslibClient::static_request(std::move(query));
    if (r_obj->get_id() == toslib_api::error::ID) {
      auto err = tos::move_tl_object_as<toslib_api::error>(std::move(r_obj));
      return td::Status::Error(err->code_, err->message_);
    }
    return tos::move_tl_object_as<typename QueryT::ReturnType::element_type>(r_obj);
  }

  td::Status validate_address(td::Slice addr) {
    TRY_STATUS(sync_send_query(make_object<toslib_api::unpackAccountAddress>(addr.str())));
    return td::Status::OK();
  }

  void unpack_address(td::Slice addr) {
    send_query(make_object<toslib_api::unpackAccountAddress>(addr.str()),
  ...
  void set_bounceable(td::Slice addr, bool bounceable) {
    send_query(make_object<toslib_api::unpackAccountAddress>(addr.str()), [addr = addr.str(), bounceable,
      ...
      this->send_query(make_object<toslib_api::packAccountAddress>(std::move(parsed_addr)), [](auto r_addr) mutable {
        if (r_addr.is_error()) {
  ...
  void generate_key(td::SecureString entropy, td::SecureString password) {
    ...
    send_query(make_object<toslib_api::createNewKey>(std::move(password_copy), td::SecureString() /*mnemonic password*/,
                                                     std::move(entropy)),
  ...
  void do_delete_all_keys() {
    send_query(make_object<toslib_api::deleteAllKeys>(), [](auto r_res) {
      if (r_res.is_error()) {
  ...
  auto with_account_state(int version, std::string public_key, td::uint32 wallet_id, F&& f) {
    if (version == 4) {
      return f(make_object<toslib_api::wallet_highload_v1_initialAccountState>(public_key, wallet_id));
    }
    if (version == 5) {
      return f(make_object<toslib_api::wallet_highload_v2_initialAccountState>(public_key, wallet_id));
    }
    if (version == 6) {
      return f(make_object<toslib_api::dns_initialAccountState>(public_key, wallet_id));
    }
    return f(make_object<toslib_api::wallet_v3_initialAccountState>(public_key, wallet_id));
  }
    ...
    auto r_addr = [&, self = this](td::int32 version, td::int32 revision) {
      auto do_request = [revision, self](auto x) {
        return self->sync_send_query(
            make_object<toslib_api::getAccountAddress>(std::move(x), revision, self->workchain_id_));
      };
  ...
  td::Result<Address> to_account_address(td::Slice key, bool need_private_key) {
    ...
    if (r_key_i.is_ok()) {
      ...
      if (address) {
        ...
        res.address = make_object<toslib_api::accountAddress>(address.unwrap());
        return std::move(res);
      }
    ...
    Address res;
    res.address = make_object<toslib_api::accountAddress>(key.str());
    return std::move(res);
  }
  ...
  void delete_key(td::Slice key) {
    ...
    send_query(make_object<toslib_api::deleteKey>(
                   make_object<toslib_api::key>(keys_[key_i].public_key, keys_[key_i].secret.copy())),
  ...
  void import_key_pem(td::Slice filename, td::Promise<td::Unit> promise) {
    ...
    send_query(make_object<toslib_api::importPemKey>(td::SecureString(), td::SecureString("cucumber"),
                                                     make_object<toslib_api::exportedPemKey>(std::move(data))),
  ...
  void import_key_raw(td::Slice filename, td::Promise<td::Unit> promise) {
    ...
    send_query(make_object<toslib_api::importUnencryptedKey>(
                   td::SecureString(), make_object<toslib_api::exportedUnencryptedKey>(std::move(data))),
               promise.wrap([&](auto&& key) {
                 LOG(ERROR) << to_string(key);
                 KeyInfo info;
                 info.public_key = key->public_key_;
  ...
  void export_key(std::string cmd, std::string key, size_t key_i, td::Slice password) {
    if (cmd == "exportkey") {
      send_query(make_object<toslib_api::exportKey>(make_object<toslib_api::inputKeyRegular>(
                     make_object<toslib_api::key>(keys_[key_i].public_key, keys_[key_i].secret.copy()),
                     td::SecureString(password))),
      ...
      send_query(make_object<toslib_api::exportPemKey>(
                     make_object<toslib_api::inputKeyRegular>(
                         make_object<toslib_api::key>(keys_[key_i].public_key, keys_[key_i].secret.copy()),
                         td::SecureString(password)),
  ...
  void import_key(std::vector<td::SecureString> words, td::Slice password) {
    send_query(make_object<toslib_api::importKey>(td::SecureString(password), td::SecureString(""),
                                                  make_object<toslib_api::exportedKey>(std::move(words))),
  ...
  void get_state(td::Slice key, td::Promise<td::Unit> promise) {
    ...
    send_query(make_object<toslib_api::getAccountState>(
                   tos::move_tl_object_as<toslib_api::accountAddress>(std::move(address.address))),
  ...
  void get_state_by_transaction(td::ConstParser& parser, td::Promise<td::Unit> promise) {
    ...
    auto address_str = address.address->account_address_;
    auto transaction_id = std::make_unique<toslib_api::internal_transactionId>(lt, std::move(hash));
    send_query(make_object<toslib_api::getAccountStateByTransaction>(
                   tos::move_tl_object_as<toslib_api::accountAddress>(std::move(address.address)),
                   tos::move_tl_object_as<toslib_api::internal_transactionId>(std::move(transaction_id))),
               promise.wrap([address_str](auto&& state) {
                 td::TerminalIO::out() << "Address: " << address_str << "\n";
                 print_full_account_state(state);
                 return td::Unit();
  ...
  void get_config_param(td::ConstParser& parser, td::Promise<td::Unit> promise) {
    ...
    send_query(make_object<toslib_api::getConfigParam>(0, param),
  ...
  void get_history(td::Slice key, td::Promise<td::Unit> promise) {
    ...
    send_query(make_object<toslib_api::getAccountState>(
                   tos::move_tl_object_as<toslib_api::accountAddress>(std::move(address.address))),
               promise.send_closure(td::actor::actor_id(this), &ToslibCli::get_history2, key.str()));
  }
  ...
  void guess_revision(td::Slice key, td::Promise<td::Unit> promise) {
    ...
    with_account_state(options_.wallet_version, keys_[key_i].public_key, wallet_id_, [&](auto state) {
      send_query(make_object<toslib_api::guessAccountRevision>(std::move(state), 0), promise.wrap([](auto revisions) {
  ...
  void guess_account(td::Slice key, td::Slice init_public_key, td::Promise<td::Unit> promise) {
    TRY_RESULT_PROMISE(promise, address, to_account_address(key, false));
    send_query(make_object<toslib_api::guessAccount>(address.public_key, init_public_key.str()),
  ...
  void run_get_masterchain_block_signatures(td::Slice seqno_s, td::Promise<td::Unit> promise) {
    ...
    send_query(make_object<toslib_api::blocks_getMasterchainBlockSignatures>(seqno), promise.wrap([](auto signatures) {
      tos::toslib_api::downcast_call(
          *signatures,
          td::overloaded(
              [&](const toslib_api::blocks_blockSignatures& obj) {
              ...
              [&](const toslib_api::blocks_blockSignatures_simplex& obj) {
  ...
  void run_get_out_msg_queue_sizes(td::Promise<td::Unit> promise) {
    send_query(make_object<toslib_api::blocks_getOutMsgQueueSizes>(0, 0, 0),
               promise.wrap([](toslib_api::object_ptr<toslib_api::blocks_outMsgQueueSizes>&& f) {
                 td::TerminalIO::out() << "Outbound message queue sizes:" << std::endl;
                 for (const auto& shard : f->shards_) {
                   td::TerminalIO::out() << tos::BlockId{shard->id_->workchain_, (tos::ShardId)shard->id_->shard_,
                                                         (tos::BlockSeqno)shard->id_->seqno_}
                                                .to_str()
  ...
  void run_show_transactions(td::ConstParser& parser, td::Promise<td::Unit> promise) {
    ...
    auto id = make_object<toslib_api::internal_transactionId>(lt, hash);
    send_query(make_object<toslib_api::raw_getTransactionsV2>(
                   nullptr, tos::move_tl_object_as<toslib_api::accountAddress>(std::move(address.address)),
                   std::move(id), count, false),
               promise.wrap([](tos::tl_object_ptr<toslib_api::raw_transactions>&& result) -> td::Result<td::Unit> {
  ...
  void get_history2(td::Slice key, td::Result<toslib_api::object_ptr<toslib_api::fullAccountState>> r_state,
                    td::Promise<td::Unit> promise) {
    ...
    send_query(make_object<toslib_api::raw_getTransactions>(
                   std::move(input_key), tos::move_tl_object_as<toslib_api::accountAddress>(std::move(address.address)),
                   std::move(state->last_transaction_id_)),
               promise.wrap([](auto res) {
                 td::StringBuilder sb;
                 for (toslib_api::object_ptr<toslib_api::raw_transaction>& t : res->transactions_) {
                   td::int64 balance = 0;
                   ...
                   auto print_msg_data = [](td::StringBuilder& sb,
                                            toslib_api::object_ptr<toslib_api::msg_Data>& msg_data) {
                     if (!msg_data) {
                       return;
                     }
                     sb << " ";
                     downcast_call(*msg_data,
                                   td::overloaded([&](toslib_api::msg_dataRaw& raw) { sb << "<unknown message>"; },
                                                  [&](toslib_api::msg_dataText& raw) { sb << "{" << raw.text_ << "}"; },
                                                  [&](toslib_api::msg_dataEncryptedText& raw) { sb << "<encrypted>"; },
                                                  [&](toslib_api::msg_dataDecryptedText& raw) {
  ...
  void transfer(td::ConstParser& parser, td::Slice cmd, td::Promise<td::Unit> cmd_promise) {
    ...
    auto add_message = [&](td::ConstParser& parser) {
      ...
      if (use_encryption) {
        data = make_object<toslib_api::msg_dataDecryptedText>(message.str());
      } else {
        data = make_object<toslib_api::msg_dataText>(message.str());
      }
      messages.push_back(make_object<toslib_api::msg_message>(
          std::move(address.address), "", amount.nano, std::vector<toslib_api::object_ptr<toslib_api::extraCurrency>>{},
          std::move(data), -1));
      return td::Status::OK();
    };
    ...
    toslib_api::object_ptr<toslib_api::InputKey> key =
        !from_address.secret.empty()
            ? make_object<toslib_api::inputKeyRegular>(
                  make_object<toslib_api::key>(from_address.public_key, from_address.secret.copy()),
                  td::SecureString(password))
            : nullptr;
    if (use_fake_key) {
      key = make_object<toslib_api::inputKeyFake>();
    }

    bool allow_send_to_uninited = force;

    send_query(make_object<toslib_api::createQuery>(
                   std::move(key), std::move(from_address.address), 60,
                   make_object<toslib_api::actionMsg>(std::move(messages), allow_send_to_uninited), nullptr),
               cmd_promise.send_closure(actor_id(this), &ToslibCli::transfer2, estimate_fees));
  }

  void transfer2(bool estimate_fees, td::Result<toslib_api::object_ptr<toslib_api::query_info>> r_info,
                 td::Promise<td::Unit> cmd_promise) {
    TRY_RESULT_PROMISE(cmd_promise, info, std::move(r_info));
    if (estimate_fees) {
      send_query(make_object<toslib_api::query_estimateFees>(info->id_, true), cmd_promise.wrap([](auto&& info) {
        td::TerminalIO::out() << "Extimated fees: " << to_string(info);
        return td::Unit();
      }));
    } else {
      send_query(make_object<toslib_api::query_send>(info->id_), cmd_promise.wrap([](auto&& info) {
  ...
  void get_hints(td::Slice prefix) {
    auto obj = toslib::ToslibClient::static_request(make_object<toslib_api::getBip39Hints>(prefix.str()));
    if (obj->get_id() == toslib_api::error::ID) {
      return;
    }
...
int main(int argc, char* argv[]) {
  ...
  td::OptionParser p;
  ToslibCli::Options options;
  p.set_description("cli wrapper around toslib");
  ...
  p.add_option('V', "version", "show toslib-cli build information", [&]() {
    std::cout << "toslib-cli build information: [ Commit: " << GitMetadata::CommitSHA1()
  ...
  scheduler.run_in_context([&] { td::actor::create_actor<ToslibCli>("console", options).release(); });
  scheduler.run();
  return 0;
}
