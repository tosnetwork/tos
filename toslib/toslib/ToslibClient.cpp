...
#include "ToslibClient.h"

template <class Type>
using lite_api_ptr = tos::lite_api::object_ptr<Type>;
template <class Type>
...
toslib_api::object_ptr<toslib_api::options_configInfo> to_toslib_api(const ToslibClient::FullConfig& full_config) {
  return toslib_api::make_object<toslib_api::options_configInfo>(full_config.wallet_id,
                                                                 full_config.rwallet_init_public_key);
}

td::Result<toslib_api::object_ptr<toslib_api::blocks_BlockSignatures>> to_toslib_api(
    const tos::BlockIdExt& blk, td::Ref<block::BlockSignatureSet> sig_set) {
  if (!sig_set->is_final()) {
...
class ToslibQueryActor : public td::actor::Actor {
 public:
  ToslibQueryActor(td::actor::ActorShared<ToslibClient> client) : client_(std::move(client)) {
  }
 ...
 private:
  td::actor::ActorShared<ToslibClient> client_;
};

toslib_api::object_ptr<toslib_api::error> status_to_toslib_api(const td::Status& status) {
  return toslib_api::make_object<toslib_api::error>(status.code(), status.message().str());
}

static block::AccountState create_account_state(tos::tl_object_ptr<tos::lite_api::liteServer_accountState> from) {
  block::AccountState res;
  res.blk = tos::create_block_id(from->id_);
  res.shard_blk = tos::create_block_id(from->shardblk_);
  res.shard_proof = std::move(from->shard_proof_);
  res.proof = std::move(from->proof_);
  res.state = std::move(from->state_);
  return res;
}
static block::AccountState create_account_state(tos::tl_object_ptr<tos::lite_api::liteServer_runMethodResult>& from) {
  block::AccountState res;
  res.blk = tos::create_block_id(from->id_);
  res.shard_blk = tos::create_block_id(from->shardblk_);
  res.shard_proof = std::move(from->shard_proof_);
...
toslib_api::object_ptr<toslib_api::internal_transactionId> empty_transaction_id() {
  return toslib_api::make_object<toslib_api::internal_transactionId>(0, std::string(32, 0));
}

toslib_api::object_ptr<toslib_api::internal_transactionId> to_transaction_id(const block::AccountState::Info& info) {
  return toslib_api::make_object<toslib_api::internal_transactionId>(info.last_trans_lt,
...
td::Result<toslib_api::object_ptr<toslib_api::tvm_cell>> to_toslib_tvm_cell(td::Ref<vm::Cell> cell) {
  TRY_RESULT(bytes, to_bytes(std::move(cell)));
  return toslib_api::make_object<toslib_api::tvm_cell>(std::move(bytes));
}

td::Result<std::vector<toslib_api::object_ptr<toslib_api::extraCurrency>>> parse_extra_currencies_or_throw(
    const td::Ref<vm::Cell> dict_root) {
  std::vector<toslib_api::object_ptr<toslib_api::extraCurrency>> result;
  ...
  if (!dict.check_for_each([&](td::Ref<vm::CellSlice> value, td::ConstBitPtr key, int n) {
        ...
        result.push_back(toslib_api::make_object<toslib_api::extraCurrency>(id, amount));
        return true;
      })) {
...
td::Result<std::vector<toslib_api::object_ptr<toslib_api::extraCurrency>>> parse_extra_currencies(
    const td::Ref<vm::Cell>& dict_root) {
  return TRY_VM(parse_extra_currencies_or_throw(dict_root));
}

td::Result<td::Ref<vm::Cell>> to_extra_currenctes_dict(
    const std::vector<toslib_api::object_ptr<toslib_api::extraCurrency>>& extra_currencies) {
...
td::Status check_enough_extra_currencies(const td::Ref<vm::Cell>& balance, const td::Ref<vm::Cell>& amount) {
  ...
  if (!v) {
    return ToslibError::NotEnoughFunds();
  }
  return td::Status::OK();
}
...
td::Result<block::PublicKey> get_public_key(td::Slice public_key) {
  TRY_RESULT_PREFIX(address, block::PublicKey::parse(public_key), ToslibError::InvalidPublicKey());
  return address;
}
...
td::Result<block::PublicKey> public_key_from_bytes(td::Slice bytes) {
  TRY_RESULT_PREFIX(key_bytes, block::PublicKey::from_bytes(bytes), ToslibError::Internal());
  return key_bytes;
}
...
td::Result<tos::pchan::Config> to_pchan_config(const toslib_api::pchan_initialAccountState& pchan_state) {
  tos::pchan::Config config;
...
class AccountState {
 public:
  ...
  td::Result<toslib_api::object_ptr<toslib_api::raw_accountState>> to_raw_accountState() const {
    ...
    return toslib_api::make_object<toslib_api::raw_accountState>(std::move(code), std::move(data), raw().frozen_hash);
  }

  td::Result<toslib_api::object_ptr<toslib_api::raw_fullAccountState>> to_raw_fullAccountState() const {
    auto state = get_smc_state();
    std::string code;
    if (state.code.not_null()) {
      TRY_RESULT_ASSIGN(code, to_bytes(state.code));
    ...
    return toslib_api::make_object<toslib_api::raw_fullAccountState>(
        get_balance(), std::move(extra_currencies), std::move(code), std::move(data), to_transaction_id(raw().info),
        to_toslib_api(raw().block_id), raw().frozen_hash, get_sync_time());
  }

  td::Result<toslib_api::object_ptr<toslib_api::wallet_v3_accountState>> to_wallet_v3_accountState() const {
    if (wallet_type_ != WalletV3) {
      return ToslibError::AccountTypeUnexpected("WalletV3");
    }
    auto wallet = tos::WalletV3(get_smc_state());
    TRY_RESULT(seqno, wallet.get_seqno());
    TRY_RESULT(wallet_id, wallet.get_wallet_id());
    return toslib_api::make_object<toslib_api::wallet_v3_accountState>(static_cast<td::uint32>(wallet_id),
                                                                       static_cast<td::uint32>(seqno));
  }
  td::Result<toslib_api::object_ptr<toslib_api::wallet_v4_accountState>> to_wallet_v4_accountState() const {
    if (wallet_type_ != WalletV4) {
      return ToslibError::AccountTypeUnexpected("WalletV4");
    }
    auto wallet = tos::WalletV4(get_smc_state());
    TRY_RESULT(seqno, wallet.get_seqno());
    TRY_RESULT(wallet_id, wallet.get_wallet_id());
    return toslib_api::make_object<toslib_api::wallet_v4_accountState>(static_cast<td::uint32>(wallet_id),
                                                                       static_cast<td::uint32>(seqno));
  }
  td::Result<toslib_api::object_ptr<toslib_api::wallet_highload_v1_accountState>> to_wallet_highload_v1_accountState()
      const {
    if (wallet_type_ != HighloadWalletV1) {
      return ToslibError::AccountTypeUnexpected("HighloadWalletV1");
    }
    auto wallet = tos::HighloadWallet(get_smc_state());
    TRY_RESULT(seqno, wallet.get_seqno());
    TRY_RESULT(wallet_id, wallet.get_wallet_id());
    return toslib_api::make_object<toslib_api::wallet_highload_v1_accountState>(static_cast<td::uint32>(wallet_id),
                                                                                static_cast<td::uint32>(seqno));
  }
  td::Result<toslib_api::object_ptr<toslib_api::wallet_highload_v2_accountState>> to_wallet_highload_v2_accountState()
      const {
    if (wallet_type_ != HighloadWalletV2) {
      return ToslibError::AccountTypeUnexpected("HighloadWalletV2");
    }
    auto wallet = tos::HighloadWalletV2(get_smc_state());
    TRY_RESULT(wallet_id, wallet.get_wallet_id());
    return toslib_api::make_object<toslib_api::wallet_highload_v2_accountState>(static_cast<td::uint32>(wallet_id));
  }
  td::Result<toslib_api::object_ptr<toslib_api::rwallet_accountState>> to_rwallet_accountState() const {
    if (wallet_type_ != RestrictedWallet) {
      return ToslibError::AccountTypeUnexpected("RestrictedWallet");
    }
    auto wallet = tos::RestrictedWallet::create(get_smc_state());
    TRY_RESULT(seqno, wallet->get_seqno());
    TRY_RESULT(wallet_id, wallet->get_wallet_id());
    TRY_RESULT(balance, wallet->get_balance(raw_.balance, raw_.info.gen_utime));
    TRY_RESULT(config, wallet->get_config());

    auto api_config = toslib_api::make_object<toslib_api::rwallet_config>();
    api_config->start_at_ = config.start_at;
    for (auto& limit : config.limits) {
      api_config->limits_.push_back(toslib_api::make_object<toslib_api::rwallet_limit>(limit.first, limit.second));
    }

    return toslib_api::make_object<toslib_api::rwallet_accountState>(wallet_id, seqno, balance, std::move(api_config));
  }
  td::Result<toslib_api::object_ptr<toslib_api::pchan_accountState>> to_payment_channel_accountState() const {
    if (wallet_type_ != PaymentChannel) {
      return ToslibError::AccountTypeUnexpected("PaymentChannel");
    }
    auto pchan = tos::PaymentChannel::create(get_smc_state());
    ...
    toslib_api::object_ptr<toslib_api::pchan_State> tl_state;
    info.state.visit(td::overloaded(
        [&](const tos::pchan::StateInit& state) {
          tl_state = toslib_api::make_object<toslib_api::pchan_stateInit>(
              state.signed_A, state.signed_B, state.min_A, state.min_B, state.A, state.B, state.expire_at);
        },
        [&](const tos::pchan::StateClose& state) {
          tl_state = toslib_api::make_object<toslib_api::pchan_stateClose>(
              state.signed_A, state.signed_B, state.promise_A, state.promise_B, state.A, state.B, state.expire_at);
        },
        [&](const tos::pchan::StatePayout& state) {
          tl_state = toslib_api::make_object<toslib_api::pchan_statePayout>(state.A, state.B);
        }));

    using toslib_api::make_object;
    return toslib_api::make_object<toslib_api::pchan_accountState>(
        toslib_api::make_object<toslib_api::pchan_config>(
            a_key.serialize(true), make_object<toslib_api::accountAddress>(info.config.a_addr.rserialize(true)),
            b_key.serialize(true), make_object<toslib_api::accountAddress>(info.config.b_addr.rserialize(true)),
            info.config.init_timeout, info.config.close_timeout, info.config.channel_id),
        std::move(tl_state), info.description);
  }

  td::Result<toslib_api::object_ptr<toslib_api::dns_accountState>> to_dns_accountState() const {
    if (wallet_type_ != ManualDns) {
      return ToslibError::AccountTypeUnexpected("ManualDns");
    }
    TRY_RESULT(wallet_id, tos::ManualDns(get_smc_state()).get_wallet_id());
    return toslib_api::make_object<toslib_api::dns_accountState>(static_cast<td::uint32>(wallet_id));
  }

  td::Result<toslib_api::object_ptr<toslib_api::AccountState>> to_accountState() const {
    auto f = [](auto&& r_x) -> td::Result<toslib_api::object_ptr<toslib_api::AccountState>> {
  ...
  td::Result<toslib_api::object_ptr<toslib_api::fullAccountState>> to_fullAccountState() const {
    TRY_RESULT(account_state, to_accountState());
    TRY_RESULT(extra_currencies, parse_extra_currencies(get_extra_currencies()));
    return toslib_api::make_object<toslib_api::fullAccountState>(
        toslib_api::make_object<toslib_api::accountAddress>(get_address().rserialize(true)), get_balance(),
        std::move(extra_currencies), to_transaction_id(raw().info), to_toslib_api(raw().block_id), get_sync_time(),
        std::move(account_state), get_wallet_revision());
  }

  td::Result<toslib_api::object_ptr<toslib_api::tvm_cell>> to_shardAccountCell() const {
    ...
    auto cell = vm::CellBuilder()
                    ...
                    .finalize();
    return to_toslib_tvm_cell(cell);
  }
  ...
  tos::BlockIdExt get_block_id() const {
  ...
  WalletType guess_type_by_init_state(toslib_api::InitialAccountState& initial_account_state) {
    ...
    downcast_call(
        ...
        td::overloaded(
            [](auto& x) {},
            [&](toslib_api::wallet_v3_initialAccountState& v3wallet) {
              for (auto revision : tos::SmartContractCode::get_revisions(tos::SmartContractCode::WalletV3)) {
                auto init_data = to_init_data(v3wallet);
                if (init_data.is_error()) {
                  continue;
                }
                auto wallet = tos::WalletV3::create(init_data.move_as_ok(), revision);
                if (!(wallet->get_address(tos::masterchainId) == address_ ||
                      wallet->get_address(tos::basechainId) == address_)) {
                  continue;
                }
            ...
            [&](toslib_api::wallet_v4_initialAccountState& v4wallet) {
              for (auto revision : tos::SmartContractCode::get_revisions(tos::SmartContractCode::WalletV4)) {
                auto init_data = to_init_data(v4wallet);
                if (init_data.is_error()) {
                  continue;
                }
                auto wallet = tos::WalletV4::create(init_data.move_as_ok(), revision);
                if (!(wallet->get_address(tos::masterchainId) == address_ ||
                      wallet->get_address(tos::basechainId) == address_)) {
                  continue;
                }
            ...
            [&](toslib_api::rwallet_initialAccountState& rwallet) {
              for (auto revision : tos::SmartContractCode::get_revisions(tos::SmartContractCode::RestrictedWallet)) {
                auto r_init_data = to_init_data(rwallet);
                if (r_init_data.is_error()) {
                  continue;
                }
                auto wallet = tos::RestrictedWallet::create(r_init_data.move_as_ok(), revision);
                if (!(wallet->get_address(tos::masterchainId) == address_ ||
                      wallet->get_address(tos::basechainId) == address_)) {
                  continue;
                }
            ...
            [&](toslib_api::pchan_initialAccountState& pchan) {
              for (auto revision : tos::SmartContractCode::get_revisions(tos::SmartContractCode::PaymentChannel)) {
                auto r_conf = to_pchan_config(pchan);
                if (r_conf.is_error()) {
                  continue;
                }
                auto conf = r_conf.move_as_ok();
                auto wallet = tos::PaymentChannel::create(conf, revision);
                if (!(wallet->get_address(tos::masterchainId) == address_ ||
                      wallet->get_address(tos::basechainId) == address_)) {
                  continue;
                }
  ...
  WalletType guess_type_by_public_key(td::Ed25519::PublicKey& key) {
    ...
    tos::WalletInterface::DefaultInitData init_data{key.as_octet_string(), wallet_id};
    auto o_revision = tos::WalletV3::guess_revision(address_, init_data);
    if (o_revision) {
      wallet_type_ = WalletType::WalletV3;
      wallet_revision_ = o_revision.value();
      set_new_state(tos::WalletV3::get_init_state(wallet_revision_, init_data));
      return wallet_type_;
    }
    o_revision = tos::WalletV4::guess_revision(address_, init_data);
    if (o_revision) {
      wallet_type_ = WalletType::WalletV4;
      wallet_revision_ = o_revision.value();
      set_new_state(tos::WalletV4::get_init_state(wallet_revision_, init_data));
      return wallet_type_;
    }
    o_revision = tos::HighloadWalletV2::guess_revision(address_, init_data);
    if (o_revision) {
      wallet_type_ = WalletType::HighloadWalletV2;
      wallet_revision_ = o_revision.value();
      set_new_state(tos::HighloadWallet::get_init_state(wallet_revision_, init_data));
      return wallet_type_;
    }
    o_revision = tos::HighloadWallet::guess_revision(address_, init_data);
    if (o_revision) {
    ...
    o_revision = tos::ManualDns::guess_revision(address_, key, wallet_id);
    if (o_revision) {
      wallet_type_ = WalletType::ManualDns;
      wallet_revision_ = o_revision.value();
      auto dns = tos::ManualDns::create(key, wallet_id, wallet_revision_);
  ...
  WalletType guess_type_default(td::Ed25519::PublicKey& key) {
    ...
    tos::WalletV3::InitData init_data(key.as_octet_string(), wallet_id_ + address_.workchain);
    set_new_state(tos::WalletV3::get_init_state(0, init_data));
    wallet_type_ = WalletType::WalletV3;
    return wallet_type_;
  }

  tos::SmartContract::State get_smc_state() const {
  ...
  void set_new_state(tos::SmartContract::State state) {
    raw_.code = std::move(state.code);
    raw_.data = std::move(state.data);
    raw_.state = tos::GenericAccount::get_init_state(raw_.code, raw_.data);
    has_new_state_ = true;
  }
 ...
 private:
  ...
  WalletType guess_type() {
    ...
    auto o_revision = tos::WalletV3::guess_revision(code_hash);
    if (o_revision) {
      wallet_type_ = WalletType::WalletV3;
      wallet_revision_ = o_revision.value();
      return wallet_type_;
    }
    o_revision = tos::WalletV4::guess_revision(code_hash);
    if (o_revision) {
      wallet_type_ = WalletType::WalletV4;
      wallet_revision_ = o_revision.value();
      return wallet_type_;
    }
    o_revision = tos::HighloadWalletV2::guess_revision(code_hash);
    if (o_revision) {
      wallet_type_ = WalletType::HighloadWalletV2;
      wallet_revision_ = o_revision.value();
      return wallet_type_;
    }
    o_revision = tos::HighloadWallet::guess_revision(code_hash);
    if (o_revision) {
      wallet_type_ = WalletType::HighloadWalletV1;
      wallet_revision_ = o_revision.value();
      return wallet_type_;
    }
    o_revision = tos::ManualDns::guess_revision(code_hash);
    if (o_revision) {
      wallet_type_ = WalletType::ManualDns;
      wallet_revision_ = o_revision.value();
      return wallet_type_;
    }
    o_revision = tos::PaymentChannel::guess_revision(code_hash);
    if (o_revision) {
      wallet_type_ = WalletType::PaymentChannel;
      wallet_revision_ = o_revision.value();
      return wallet_type_;
    }
    o_revision = tos::RestrictedWallet::guess_revision(code_hash);
    if (o_revision) {
      wallet_type_ = WalletType::RestrictedWallet;
      wallet_revision_ = o_revision.value();
      return wallet_type_;
...
class Query {
 public:
  ...
  struct Fee {
    ...
    td::int64 gas_fee{0};
    td::int64 fwd_fee{0};
    auto to_toslib_api() const {
      return toslib_api::make_object<toslib_api::fees>(in_fwd_fee, storage_fee, gas_fee, fwd_fee);
    }
  };
  ...
  td::Result<std::pair<Fee, std::vector<Fee>>> estimate_fees(bool ignore_chksig, const LastConfigState& state,
    ...
    bool is_masterchain = raw_.source->get_address().workchain == tos::masterchainId;
    ...
    auto smc = tos::SmartContract::create(raw_.source->get_smc_state());
    ...
    for (auto& destination : raw_.destinations) {
      bool dest_is_masterchain = destination && destination->get_address().workchain == tos::masterchainId;
...
class GetTransactionHistory : public td::actor::Actor {
 public:
  GetTransactionHistory(ExtClientRef ext_client_ref, block::StdAddress address, tos::LogicalTime lt, tos::Bits256 hash,
 ...
 private:
  ...
  void with_transactions(
      td::Result<tos::lite_api::object_ptr<tos::lite_api::liteServer_transactionList>> r_transactions) {
  ...
  td::Status do_with_transactions(
    ...
    TRY_RESULT_PREFIX(info, TRY_VM(do_with_transactions(std::move(transactions))), ToslibError::ValidateTransactions());
    promise_.set_value(std::move(info));
    return td::Status::OK();
  }

  td::Result<block::TransactionList::Info> do_with_transactions(
      tos::lite_api::object_ptr<tos::lite_api::liteServer_transactionList> transactions) {
    std::vector<tos::BlockIdExt> blkids;
    for (auto& id : transactions->ids_) {
      blkids.push_back(tos::create_block_id(std::move(id)));
    }
    return do_with_transactions(std::move(blkids), std::move(transactions->transactions_));
  }

  td::Result<block::TransactionList::Info> do_with_transactions(std::vector<tos::BlockIdExt> blkids,
  ...
  void start_up() override {
    ...
    client_.send_query(
        tos::lite_api::liteServer_getTransactions(
            count_, tos::create_tl_object<tos::lite_api::liteServer_accountId>(address_.workchain, address_.addr), lt_,
            hash_),
        [self = this](auto r_transactions) { self->with_transactions(std::move(r_transactions)); });
  }
  void hangup() override {
    check(ToslibError::Cancelled());
  }
};
...
class RemoteRunSmcMethod : public td::actor::Actor {
 ...
 private:
  ...
  void with_run_method_result(
      td::Result<tos::tl_object_ptr<tos::lite_api::liteServer_runMethodResult>> r_run_method_result) {
  ...
  td::Status do_with_run_method_result(
    ...
    TRY_RESULT_PREFIX(state, TRY_VM(do_with_run_method_result(std::move(run_method_result))),
                      ToslibError::ValidateAccountState());
    promise_.set_value(std::move(state));
    stop();
    return td::Status::OK();
  }
  td::Result<int_api::RemoteRunSmcMethod::ReturnType> do_with_run_method_result(
      tos::tl_object_ptr<tos::lite_api::liteServer_runMethodResult> run_method_result) {
...
class GetRawAccountState : public td::actor::Actor {
 public:
  GetRawAccountState(ExtClientRef ext_client_ref, block::StdAddress address, td::optional<tos::BlockIdExt> block_id,
 ...
 private:
  block::StdAddress address_;
  td::optional<tos::BlockIdExt> block_id_;
  td::Promise<RawAccountState> promise_;
  td::actor::ActorShared<> parent_;
  ExtClient client_;

  void with_account_state(td::Result<tos::tl_object_ptr<tos::lite_api::liteServer_accountState>> r_account_state) {
    check(do_with_account_state(std::move(r_account_state)));
  }

  td::Status do_with_account_state(
      td::Result<tos::tl_object_ptr<tos::lite_api::liteServer_accountState>> r_raw_account_state) {
  ...
  td::Result<RawAccountState> do_with_account_state(
      tos::tl_object_ptr<tos::lite_api::liteServer_accountState> raw_account_state) {
  ...
  void with_block_id() {
    client_.send_query(
        tos::lite_api::liteServer_getAccountState(
            tos::create_tl_lite_block_id(block_id_.value()),
            tos::create_tl_object<tos::lite_api::liteServer_accountId>(address_.workchain, address_.addr)),
...
class GetMasterchainBlockSignatures : public td::actor::Actor {
 public:
  GetMasterchainBlockSignatures(ExtClientRef ext_client_ref, tos::BlockSeqno seqno, td::actor::ActorShared<> parent,
                                td::Promise<toslib_api_ptr<toslib_api::blocks_BlockSignatures>>&& promise)
      : block_id_short_(tos::masterchainId, tos::shardIdAll, seqno)
      , parent_(std::move(parent))
      , promise_(std::move(promise)) {
    client_.set_client(ext_client_ref);
  }
  ...
  void got_last_block(tos::BlockIdExt id) {
    last_block_ = id;
    prev_block_id_short_ = block_id_short_;
    prev_block_id_short_.seqno--;
    client_.send_query(
        tos::lite_api::liteServer_lookupBlock(1, tos::create_tl_lite_block_id_simple(prev_block_id_short_), 0, 0),
        [SelfId = actor_id(this)](td::Result<lite_api_ptr<tos::lite_api::liteServer_blockHeader>> R) {
          if (R.is_error()) {
            td::actor::send_closure(SelfId, &GetMasterchainBlockSignatures::abort, R.move_as_error());
          } else {
            td::actor::send_closure(SelfId, &GetMasterchainBlockSignatures::got_prev_block_id,
                                    tos::create_block_id(R.ok()->id_));
          }
        });
  }

  void got_prev_block_id(tos::BlockIdExt id) {
    prev_block_id_ = id;
    ...
    client_.send_query(
        tos::lite_api::liteServer_getBlockProof(0x1001, tos::create_tl_lite_block_id(last_block_),
                                                tos::create_tl_lite_block_id(prev_block_id_)),
        [SelfId = actor_id(this)](td::Result<lite_api_ptr<tos::lite_api::liteServer_partialBlockProof>> R) {
          if (R.is_error()) {
            td::actor::send_closure(SelfId, &GetMasterchainBlockSignatures::abort, R.move_as_error());
          } else {
  ...
  void got_prev_proof(lite_api_ptr<tos::lite_api::liteServer_partialBlockProof> proof) {
    ...
    client_.send_query(
        tos::lite_api::liteServer_lookupBlock(1, tos::create_tl_lite_block_id_simple(block_id_short_), 0, 0),
        [SelfId = actor_id(this)](td::Result<lite_api_ptr<tos::lite_api::liteServer_blockHeader>> R) {
          if (R.is_error()) {
            td::actor::send_closure(SelfId, &GetMasterchainBlockSignatures::abort, R.move_as_error());
          } else {
  ...
  void got_proof(lite_api_ptr<tos::lite_api::liteServer_partialBlockProof> proof) {
    auto R = liteclient::deserialize_proof_chain(std::move(proof));
    if (R.is_error()) {
      abort(R.move_as_error());
      return;
    ...
    promise_.set_result(to_toslib_api(block_id_, chain->last_link().sig_set));
    stop();
  }
 ...
 private:
  tos::BlockId block_id_short_;
  td::actor::ActorShared<> parent_;
  td::Promise<toslib_api_ptr<toslib_api::blocks_BlockSignatures>> promise_;
  ExtClient client_;
  tos::BlockIdExt block_id_;
  tos::BlockId prev_block_id_short_;
  tos::BlockIdExt prev_block_id_;
  tos::BlockIdExt last_block_;
};

class GetShardBlockProof : public td::actor::Actor {
 public:
  GetShardBlockProof(ExtClientRef ext_client_ref, tos::BlockIdExt id, tos::BlockIdExt from,
                     td::actor::ActorShared<> parent,
                     td::Promise<toslib_api_ptr<toslib_api::blocks_shardBlockProof>>&& promise)
  ...
  void got_from_block(tos::BlockIdExt from) {
    from_ = from;
    CHECK(from_.is_masterchain_ext());
    client_.send_query(
        tos::lite_api::liteServer_getShardBlockProof(tos::create_tl_lite_block_id(id_)),
        [SelfId = actor_id(this)](td::Result<lite_api_ptr<tos::lite_api::liteServer_shardBlockProof>> R) {
          if (R.is_error()) {
            td::actor::send_closure(SelfId, &GetShardBlockProof::abort, R.move_as_error());
          } else {
  ...
  void got_shard_block_proof(lite_api_ptr<tos::lite_api::liteServer_shardBlockProof> result) {
    ...
    tos::BlockIdExt cur_id = mc_id_;
    try {
      for (auto& link : result->links_) {
        tos::BlockIdExt prev_id = create_block_id(link->id_);
        td::BufferSlice proof = std::move(link->proof_);
        auto R = vm::std_boc_deserialize(proof);
        if (R.is_error()) {
          abort(ToslibError::InvalidBagOfCells("proof"));
          return;
        }
        ...
        if (cur_id.is_masterchain()) {
          ...
          std::vector<tos::BlockIdExt> prev;
          tos::BlockIdExt mc_blkid;
          bool after_split;
    ...
    client_.send_query(
        tos::lite_api::liteServer_getBlockProof(0x1001, tos::create_tl_lite_block_id(from_),
                                                tos::create_tl_lite_block_id(mc_id_)),
        [SelfId = actor_id(this)](td::Result<lite_api_ptr<tos::lite_api::liteServer_partialBlockProof>> R) {
          if (R.is_error()) {
            td::actor::send_closure(SelfId, &GetShardBlockProof::abort, R.move_as_error());
          } else {
  ...
  void got_mc_proof(lite_api_ptr<tos::lite_api::liteServer_partialBlockProof> result) {
    ...
    std::vector<tos::tl_object_ptr<toslib_api::blocks_shardBlockLink>> links;
    std::vector<tos::tl_object_ptr<toslib_api::blocks_blockLinkBack>> mc_proof;
    for (const auto& p : links_) {
      links.push_back(
          tos::create_tl_object<toslib_api::blocks_shardBlockLink>(to_toslib_api(p.first), p.second.as_slice().str()));
    }
    if (chain->link_count() == 1) {
      ...
      mc_proof.push_back(tos::create_tl_object<toslib_api::blocks_blockLinkBack>(
          link.is_key, to_toslib_api(link.from), to_toslib_api(link.to), dest_proof.as_slice().str(),
          proof.as_slice().str(), state_proof.as_slice().str()));
    }

    promise_.set_result(tos::create_tl_object<toslib_api::blocks_shardBlockProof>(
        to_toslib_api(from_), to_toslib_api(mc_id_), std::move(links), std::move(mc_proof)));
    stop();
  }
 ...
 private:
  tos::BlockIdExt id_, from_, mc_id_;
  td::actor::ActorShared<> parent_;
  td::Promise<toslib_api_ptr<toslib_api::blocks_shardBlockProof>> promise_;
  ExtClient client_;
  std::vector<std::pair<tos::BlockIdExt, td::BufferSlice>> links_;
};

class GetOutMsgQueueSizes : public td::actor::Actor {
 public:
  GetOutMsgQueueSizes(ExtClientRef ext_client_ref, std::vector<tos::BlockIdExt> blocks, td::actor::ActorShared<> parent,
                      td::Promise<toslib_api_ptr<toslib_api::blocks_outMsgQueueSizes>>&& promise)
  ...
  void start_up() override {
    ...
    for (size_t i = 0; i < blocks_.size(); ++i) {
      client_.send_query(
          tos::lite_api::liteServer_getBlockOutMsgQueueSize(1, tos::create_tl_lite_block_id(blocks_[i]), true),
          [SelfId = actor_id(this), i](td::Result<lite_api_ptr<tos::lite_api::liteServer_blockOutMsgQueueSize>> R) {
            if (R.is_error()) {
    ...
    client_.send_query(
        tos::lite_api::liteServer_getOutMsgQueueSizes(1, tos::masterchainId, tos::shardIdAll),
        [SelfId = actor_id(this)](td::Result<lite_api_ptr<tos::lite_api::liteServer_outMsgQueueSizes>> R) {
          if (R.is_error()) {
            td::actor::send_closure(SelfId, &GetOutMsgQueueSizes::abort, R.move_as_error());
          } else {
  ...
  void got_block_queue_size(size_t i, lite_api_ptr<tos::lite_api::liteServer_blockOutMsgQueueSize> f) {
    try {
      auto S = [&, this]() -> td::Status {
        ...
        tos::Bits256 state_hash = state_root->get_hash().bits();
  ...
  void dec_pending() {
    if (--pending_ == 0) {
      std::vector<toslib_api::object_ptr<toslib_api::blocks_outMsgQueueSize>> shards;
      for (size_t i = 0; i < blocks_.size(); ++i) {
        shards.push_back(toslib_api::make_object<toslib_api::blocks_outMsgQueueSize>(
            to_toslib_api(blocks_[i]), static_cast<td::uint32>(sizes_[i])));
      }
      promise_.set_result(
          toslib_api::make_object<toslib_api::blocks_outMsgQueueSizes>(std::move(shards), ext_msg_queue_size_limit_));
      stop();
    }
  }
 ...
 private:
  std::vector<tos::BlockIdExt> blocks_;
  td::actor::ActorShared<> parent_;
  td::Promise<toslib_api_ptr<toslib_api::blocks_outMsgQueueSizes>> promise_;
  ExtClient client_;
...
td::Status check_block_transactions_proof(lite_api_ptr<tos::lite_api::liteServer_blockTransactions>& bTxes,
                                          int32_t mode, tos::LogicalTime start_lt, td::Bits256 start_addr,
                                          td::Bits256 root_hash, int req_count);
td::Status check_lookup_block_proof(lite_api_ptr<tos::lite_api::liteServer_lookupBlockResult>& result, int mode,
                                    tos::BlockId blkid, tos::BlockIdExt client_mc_blkid, td::uint64 lt,
                                    td::uint32 utime);

class RunEmulator : public ToslibQueryActor {
 public:
  RunEmulator(ExtClientRef ext_client_ref, int_api::GetAccountStateByTransaction request,
              td::actor::ActorShared<ToslibClient> parent, td::Promise<td::unique_ptr<AccountState>>&& promise)
      : ToslibQueryActor(std::move(parent)), request_(std::move(request)), promise_(std::move(promise)) {
    client_.set_client(ext_client_ref);
  }
 ...
 private:
  ...
  void get_block_id(td::Promise<FullBlockId>&& promise) {
    auto shard_id = tos::shard_prefix(request_.address.addr, 60);
    auto blkid = tos::BlockId(request_.address.workchain, shard_id, 0);
    ...
    client_.with_last_block([self = this, promise = std::move(promise), shard_id, blkid, mode,
      ...
      auto query = tos::lite_api::liteServer_lookupBlockWithProof(
          mode, tos::create_tl_lite_block_id_simple(blkid), tos::create_tl_lite_block_id(trusted_mc_block_id), lt, 0);
      self->client_.send_query(
          std::move(query),
          [self, promise = std::move(promise), shard_id, blkid, mode, lt, trusted_mc_block_id](
              td::Result<toslib_api::object_ptr<tos::lite_api::liteServer_lookupBlockResult>> lookup_result_r) mutable {
            ...
            auto block_id = tos::create_block_id(lookup_result->id_);
            self->client_.send_query(
                tos::lite_api::liteServer_getBlockHeader(tos::create_tl_lite_block_id(block_id), 0xffff),
                promise.wrap([shard_id, block_id](
                                 td::Result<toslib_api::object_ptr<tos::lite_api::liteServer_blockHeader>> header_r)
                                 -> td::Result<FullBlockId> {
                  TRY_RESULT(header, std::move(header_r));
                  if (block_id != tos::create_block_id(header->id_)) {
                  ...
                  try {
                    ...
                    if (tos::RootHash{virt_root->get_hash().bits()} != block_id.root_hash) {
                      return td::Status::Error("block header has incorrect root hash");
                    }

                    std::vector<tos::BlockIdExt> prev_blocks;
                    tos::BlockIdExt mc_block_id;
                    bool after_split;
                    td::Status status =
  ...
  void get_mc_state_root(td::Promise<td::Ref<vm::Cell>>&& promise) {
    TRY_RESULT_PROMISE(promise, lite_block, to_lite_api(*to_toslib_api(block_id_.mc)));
    auto block = tos::create_block_id(lite_block);
    client_.send_query(tos::lite_api::liteServer_getConfigAll(0b11'11111111, std::move(lite_block)),
  ...
  td::Status get_transactions(std::int64_t lt) {
    TRY_RESULT(lite_block, to_lite_api(*to_toslib_api(block_id_.id)));
    auto after = tos::lite_api::make_object<tos::lite_api::liteServer_transactionId3>(request_.address.addr, lt);
    auto mode = 0b10100111;
    constexpr int req_count = 256;
    auto query = tos::lite_api::liteServer_listBlockTransactions(std::move(lite_block), mode, req_count,
                                                                 std::move(after), false, true);

    client_.send_query(
        std::move(query), [self = this, mode, lt, root_hash = block_id_.id.root_hash](
                              td::Result<lite_api_ptr<tos::lite_api::liteServer_blockTransactions>> maybe_bTxes) {
  ...
  void inc() {
    ...
    try {
      ...
      tos::UnixTime now = account_state_->get_sync_time();
      bool is_special = address.workchain == tos::masterchainId && config->is_special_smartcontract(address.addr);
...
void ToslibClient::proxy_request(td::int64 query_id, std::string data) {
  on_update(toslib_api::make_object<toslib_api::updateSendLiteServerQuery>(query_id, data));
}

void ToslibClient::init_ext_client() {
  if (use_callbacks_for_network_) {
    class Callback : public ExtClientOutbound::Callback {
     public:
      explicit Callback(td::actor::ActorShared<ToslibClient> parent, td::uint32 config_generation)
          : parent_(std::move(parent)), config_generation_(config_generation) {
      }

      void request(td::int64 id, std::string data) override {
        send_closure(parent_, &ToslibClient::proxy_request, (id << 16) | (config_generation_ & 0xffff),
                     std::move(data));
      }

     private:
      td::actor::ActorShared<ToslibClient> parent_;
...
void ToslibClient::update_last_block_state(LastBlockState state, td::uint32 config_generation) {
...
void ToslibClient::update_sync_state(LastBlockSyncState state, td::uint32 config_generation) {
  if (config_generation != config_generation_) {
    return;
  }
  switch (state.type) {
    case LastBlockSyncState::Done:
      on_update(
          toslib_api::make_object<toslib_api::updateSyncState>(toslib_api::make_object<toslib_api::syncStateDone>()));
      break;
    case LastBlockSyncState::InProgress:
      on_update(
          toslib_api::make_object<toslib_api::updateSyncState>(toslib_api::make_object<toslib_api::syncStateInProgress>(
...
void ToslibClient::init_last_block(LastBlockState state) {
  ref_cnt_++;
  class Callback : public LastBlock::Callback {
   public:
    Callback(td::actor::ActorShared<ToslibClient> client, td::uint32 config_generation)
        : client_(std::move(client)), config_generation_(config_generation) {
    }
    void on_state_changed(LastBlockState state) override {
      send_closure(client_, &ToslibClient::update_last_block_state, std::move(state), config_generation_);
    }
    void on_sync_state_changed(LastBlockSyncState sync_state) override {
      send_closure(client_, &ToslibClient::update_sync_state, std::move(sync_state), config_generation_);
    }
...
void ToslibClient::init_last_config() {
  ref_cnt_++;
  class Callback : public LastConfig::Callback {
   public:
    Callback(td::actor::ActorShared<ToslibClient> client) : client_(std::move(client)) {
    }
...
void ToslibClient::on_result(td::uint64 id, toslib_api::object_ptr<toslib_api::Object> response) {
  VLOG_IF(toslib_query, id != 0) << "Toslib answer query " << td::tag("id", id) << " " << to_string(response);
  VLOG_IF(toslib_query, id == 0) << "Toslib update " << to_string(response);
  if (response->get_id() == toslib_api::error::ID) {
    callback_->on_error(id, toslib_api::move_object_as<toslib_api::error>(response));
    return;
  }
  callback_->on_result(id, std::move(response));
}
void ToslibClient::on_update(object_ptr<toslib_api::Object> response) {
  on_result(0, std::move(response));
}

void ToslibClient::make_any_request(toslib_api::Function& function, QueryContext query_context,
...
void ToslibClient::request(td::uint64 id, toslib_api::object_ptr<toslib_api::Function> function) {
  ...
  if (function == nullptr) {
    ...
    return on_result(id, toslib_api::make_object<toslib_api::error>(400, "Request is empty"));
  }
  ...
  if (state_ == State::Closed) {
    return on_result(id, toslib_api::make_object<toslib_api::error>(400, "toslib is closed"));
  }
  if (state_ == State::Uninited) {
    if (!is_uninited_request(function->get_id())) {
      return on_result(id, toslib_api::make_object<toslib_api::error>(400, "library is not inited"));
    }
  }

  ref_cnt_++;
  using Object = toslib_api::object_ptr<toslib_api::Object>;
  td::Promise<Object> promise = [actor_id = actor_id(this), id, tmp = actor_shared(this)](td::Result<Object> r_result) {
    toslib_api::object_ptr<toslib_api::Object> result;
    if (r_result.is_error()) {
      result = status_to_toslib_api(r_result.error());
    } else {
...
toslib_api::object_ptr<toslib_api::Object> ToslibClient::static_request(
    toslib_api::object_ptr<toslib_api::Function> function) {
  VLOG(toslib_query) << "Toslib got static query " << to_string(function);
  if (function == nullptr) {
    LOG(ERROR) << "Receive empty static request";
    return toslib_api::make_object<toslib_api::error>(400, "Request is empty");
  }

  auto response = downcast_call2<toslib_api::object_ptr<toslib_api::Object>>(
      *function, [](auto& request) { return ToslibClient::do_static_request(request); });
  VLOG(toslib_query) << "  answer static query " << to_string(response);
  return response;
}

bool ToslibClient::is_static_request(td::int32 id) {
  switch (id) {
    case toslib_api::runTests::ID:
    case toslib_api::getAccountAddress::ID:
    case toslib_api::packAccountAddress::ID:
    case toslib_api::unpackAccountAddress::ID:
    case toslib_api::getBip39Hints::ID:
    case toslib_api::setLogStream::ID:
    case toslib_api::getLogStream::ID:
    case toslib_api::setLogVerbosityLevel::ID:
    case toslib_api::getLogVerbosityLevel::ID:
    case toslib_api::getLogTags::ID:
    case toslib_api::setLogTagVerbosityLevel::ID:
    case toslib_api::getLogTagVerbosityLevel::ID:
    case toslib_api::addLogMessage::ID:
    case toslib_api::encrypt::ID:
    case toslib_api::decrypt::ID:
    case toslib_api::kdf::ID:
    case toslib_api::msg_decryptWithProof::ID:
      return true;
    default:
      return false;
  }
}
bool ToslibClient::is_uninited_request(td::int32 id) {
  switch (id) {
    case toslib_api::init::ID:
    case toslib_api::close::ID:
      return true;
    default:
      return false;
  }
...
toslib_api::object_ptr<toslib_api::Object> ToslibClient::do_static_request(const toslib_api::runTests& request) {
  auto& runner = td::TestsRunner::get_default();
  if (!request.dir_.empty()) {
    td::chdir(request.dir_).ignore();
  }
  runner.run_all();
  return toslib_api::make_object<toslib_api::ok>();
}

td::Result<block::StdAddress> get_account_address(const toslib_api::raw_initialAccountState& raw_state,
                                                  td::int32 revision, tos::WorkchainId workchain_id) {
  TRY_RESULT_PREFIX(code, vm::std_boc_deserialize(raw_state.code_), ToslibError::InvalidBagOfCells("raw_state.code"));
  TRY_RESULT_PREFIX(data, vm::std_boc_deserialize(raw_state.data_), ToslibError::InvalidBagOfCells("raw_state.data"));
  return tos::GenericAccount::get_address(workchain_id,
                                          tos::GenericAccount::get_init_state(std::move(code), std::move(data)));
}
...
td::Result<block::StdAddress> get_account_address(const toslib_api::pchan_initialAccountState& pchan_state,
                                                  td::int32 revision, tos::WorkchainId workchain_id) {
  TRY_RESULT(config, to_pchan_config(pchan_state));
  return tos::PaymentChannel::create(config, revision)->get_address(workchain_id);
}
td::Result<block::StdAddress> get_account_address(const toslib_api::rwallet_initialAccountState& rwallet_state,
                                                  td::int32 revision, tos::WorkchainId workchain_id) {
  TRY_RESULT(init_data, to_init_data(rwallet_state));
  return tos::RestrictedWallet::create(init_data, revision)->get_address(workchain_id);
}

td::Result<td::Bits256> get_adnl_address(td::Slice adnl_address) {
  TRY_RESULT_PREFIX(address, td::adnl_id_decode(adnl_address),
                    ToslibError::InvalidField("adnl_address", "can't decode"));
  return address;
}

static td::optional<tos::SmartContractCode::Type> get_wallet_type(toslib_api::InitialAccountState& state) {
  return downcast_call2<td::optional<tos::SmartContractCode::Type>>(
      state,
      td::overloaded(
          [](const toslib_api::raw_initialAccountState&) { return td::optional<tos::SmartContractCode::Type>(); },
          [](const toslib_api::wallet_v3_initialAccountState&) { return tos::SmartContractCode::WalletV3; },
          [](const toslib_api::wallet_v4_initialAccountState&) { return tos::SmartContractCode::WalletV4; },
          [](const toslib_api::wallet_highload_v1_initialAccountState&) {
            return tos::SmartContractCode::HighloadWalletV1;
          },
          [](const toslib_api::wallet_highload_v2_initialAccountState&) {
            return tos::SmartContractCode::HighloadWalletV2;
          },
          [](const toslib_api::rwallet_initialAccountState&) { return tos::SmartContractCode::RestrictedWallet; },
          [](const toslib_api::pchan_initialAccountState&) { return tos::SmartContractCode::PaymentChannel; },
          [](const toslib_api::dns_initialAccountState&) { return tos::SmartContractCode::ManualDns; }));
}

toslib_api::object_ptr<toslib_api::Object> ToslibClient::do_static_request(
    const toslib_api::getAccountAddress& request) {
  if (!request.initial_account_state_) {
    return status_to_toslib_api(ToslibError::EmptyField("initial_account_state"));
  }
  auto o_type = get_wallet_type(*request.initial_account_state_);
  if (o_type) {
    auto status = tos::SmartContractCode::validate_revision(o_type.value(), request.revision_);
    if (status.is_error()) {
      return status_to_toslib_api(ToslibError::InvalidRevision());
    }
  }
  ...
  if (r_account_address.is_error()) {
    return status_to_toslib_api(r_account_address.error());
  }
...
td::Status ToslibClient::do_request(toslib_api::guessAccountRevision& request,
  ...
  std::vector<toslib_api::object_ptr<toslib_api::InitialAccountState>> states;
  states.push_back(std::move(request.initial_account_state_));
  for (auto& initial_account_state : states) {
    if (!initial_account_state) {
      return ToslibError::EmptyField("initial_account_state");
    }
    ...
    auto revisions = tos::SmartContractCode::get_revisions(type);
    auto workchains = std::vector<tos::WorkchainId>{request.workchain_id_};
        ...
        *initial_account_state, [&revisions, &targets, &workchains, &type](const auto& state) {
          for (auto workchain : workchains) {
            for (auto revision : revisions) {
              ...
              Target target;
              target.can_be_empty = type != tos::SmartContractCode::Type::RestrictedWallet;
...
td::Status ToslibClient::do_request(toslib_api::guessAccount& request,
                                    td::Promise<object_ptr<toslib_api::accountRevisionList>>&& promise) {
  std::vector<Target> targets;
  struct Source {
    toslib_api::object_ptr<toslib_api::InitialAccountState> init_state;
    tos::WorkchainId workchain_id;
  };
  ...
  sources.push_back(Source{toslib_api::make_object<toslib_api::rwallet_initialAccountState>(
                               rwallet_init_public_key, request.public_key_, wallet_id_ + tos::masterchainId),
                           tos::masterchainId});
  sources.push_back(Source{toslib_api::make_object<toslib_api::wallet_v3_initialAccountState>(
                               request.public_key_, wallet_id_ + tos::masterchainId),
                           tos::masterchainId});
  sources.push_back(Source{toslib_api::make_object<toslib_api::wallet_v3_initialAccountState>(
                               request.public_key_, wallet_id_ + tos::basechainId),
                           tos::basechainId});
  sources.push_back(Source{toslib_api::make_object<toslib_api::wallet_v4_initialAccountState>(
                               request.public_key_, wallet_id_ + tos::masterchainId),
                           tos::masterchainId});
  sources.push_back(Source{toslib_api::make_object<toslib_api::wallet_v4_initialAccountState>(
                               request.public_key_, wallet_id_ + tos::basechainId),
  ...
  for (Source& source : sources) {
    ...
    auto type = o_type.unwrap();
    auto revisions = tos::SmartContractCode::get_revisions(type);
    auto workchains = std::vector<tos::WorkchainId>{source.workchain_id};
          ...
          for (auto workchain : workchains) {
            for (auto revision : revisions) {
              ...
              target.can_be_uninited =
                  type == tos::SmartContractCode::Type::WalletV3 && revision == 2 && workchain == tos::basechainId;
              target.can_be_empty = type != tos::SmartContractCode::Type::RestrictedWallet || target.can_be_uninited;
              target.address = address;
...
td::Status ToslibClient::guess_revisions(std::vector<Target> targets,
                                         td::Promise<object_ptr<toslib_api::accountRevisionList>>&& promise) {
  auto actor_id = actor_id_++;
  class GuessRevisions : public ToslibQueryActor {
   public:
    GuessRevisions(td::actor::ActorShared<ToslibClient> client, td::optional<tos::BlockIdExt> block_id,
                   std::vector<Target> targets, td::Promise<std::vector<td::unique_ptr<AccountState>>> promise)
        : ToslibQueryActor(std::move(client))
        , block_id_(std::move(block_id))
   ...
   private:
    ...
    void hangup() override {
      promise_.set_error(ToslibError::Cancelled());
      return;
    }
  ...
  actors_[actor_id] = td::actor::create_actor<GuessRevisions>(
      ...
      promise.wrap([](auto&& v) mutable {
        std::vector<toslib_api::object_ptr<toslib_api::fullAccountState>> res;
        for (auto& x : v) {
        ...
        return toslib_api::make_object<toslib_api::accountRevisionList>(std::move(res));
      }));
  return td::Status::OK();
}

toslib_api::object_ptr<toslib_api::Object> ToslibClient::do_static_request(
    const toslib_api::unpackAccountAddress& request) {
  auto r_account_address = get_account_address(request.account_address_);
  if (r_account_address.is_error()) {
    return status_to_toslib_api(r_account_address.move_as_error());
  }
  auto account_address = r_account_address.move_as_ok();
  return toslib_api::make_object<toslib_api::unpackedAccountAddress>(
      account_address.workchain, account_address.bounceable, account_address.testnet,
      account_address.addr.as_slice().str());
}

toslib_api::object_ptr<toslib_api::Object> ToslibClient::do_static_request(
    const toslib_api::packAccountAddress& request) {
  if (!request.account_address_) {
    return status_to_toslib_api(ToslibError::EmptyField("account_address"));
  }
  if (request.account_address_->addr_.size() != 32) {
    return status_to_toslib_api(ToslibError::InvalidField("account_address.addr", "must be 32 bytes long"));
  }
  ...
  return toslib_api::make_object<toslib_api::accountAddress>(addr.rserialize(true));
}

toslib_api::object_ptr<toslib_api::Object> ToslibClient::do_static_request(toslib_api::getBip39Hints& request) {
  return toslib_api::make_object<toslib_api::bip39Hints>(
      td::transform(Mnemonic::word_hints(td::trim(td::to_lower_inplace(request.prefix_))), [](auto& x) { return x; }));
}

td::Status ToslibClient::do_request(const toslib_api::init& request,
                                    td::Promise<object_ptr<toslib_api::options_info>>&& promise) {
  if (state_ != State::Uninited) {
    return td::Status::Error(400, "Toslib is already inited");
  }
  if (!request.options_) {
    return ToslibError::EmptyField("options");
  }
  if (!request.options_->keystore_type_) {
    return ToslibError::EmptyField("options.keystore_type");
  }

  auto r_kv = downcast_call2<td::Result<td::unique_ptr<KeyValue>>>(
      *request.options_->keystore_type_,
      td::overloaded(
          [](toslib_api::keyStoreTypeDirectory& directory) { return KeyValue::create_dir(directory.directory_); },
          [](toslib_api::keyStoreTypeInMemory& inmemory) { return KeyValue::create_inmemory(); }));
...
class MasterConfig {
 public:
  ...
  td::optional<Config> by_root_hash(const tos::RootHash& root_hash) const {
 ...
 private:
  ...
  std::map<tos::RootHash, std::shared_ptr<const Config>> by_root_hash_;
};
...
td::Result<ToslibClient::FullConfig> ToslibClient::validate_config(toslib_api::object_ptr<toslib_api::config> config) {
  if (!config) {
    return ToslibError::EmptyField("config");
  }
  if (config->config_.empty()) {
    return ToslibError::InvalidConfig("config is empty");
  }
  TRY_RESULT_PREFIX(new_config, Config::parse(std::move(config->config_)),
                    ToslibError::InvalidConfig("can't parse config"));

  if (new_config.lite_servers.empty() && !config->use_callbacks_for_network_) {
    return ToslibError::InvalidConfig("no lite clients");
  }
  ...
  if (o_master_config) {
    ...
    if (!name.empty() && !new_config.name.empty() && new_config.name != name && name == "mainnet") {
      return ToslibError::InvalidConfig(PSLICE() << "Invalid blockchain_id: expected '" << name << "'");
    }
  }

  if (o_master_config && o_master_config.value().zero_state_id != new_config.zero_state_id) {
    return ToslibError::InvalidConfig("zero_state differs from embedded zero_state");
  }

  if (o_master_config && o_master_config.value().hardforks != new_config.hardforks) {
    return ToslibError::InvalidConfig("hardforks differs from embedded hardforks");
  }
  ...
  auto zero_state = tos::ZeroStateIdExt(new_config.zero_state_id.id.workchain, new_config.zero_state_id.root_hash,
  ...
  if (config->ignore_cache_ || r_state.is_error()) {
    ...
    if (state.zero_state_id != zero_state) {
      ...
      return ToslibError::InvalidConfig("zero_state differs from cached zero_state");
    }
    if (state.vert_seqno > vert_seqno) {
      LOG(ERROR) << "Stored vert_seqno is bigger than one in config: " << state.vert_seqno << " vs " << vert_seqno;
      return ToslibError::InvalidConfig("vert_seqno in cached state is bigger");
    }
    if (state.vert_seqno < vert_seqno) {
      state.zero_state_id = zero_state;
      state.last_block_id = new_config.zero_state_id;
      state.last_key_block_id = new_config.zero_state_id;
      state.init_block_id = tos::BlockIdExt{};
...
void ToslibClient::set_config(FullConfig full_config) {
...
td::Status ToslibClient::do_request(const toslib_api::close& request,
  ...
  state_ = State::Closed;
  source_.cancel();
  promise.set_value(toslib_api::make_object<toslib_api::ok>());
  return td::Status::OK();
}

td::Status ToslibClient::do_request(toslib_api::options_validateConfig& request,
                                    td::Promise<object_ptr<toslib_api::options_configInfo>>&& promise) {
  TRY_RESULT(config, validate_config(std::move(request.config_)));
  auto res = to_toslib_api(config);
  promise.set_value(std::move(res));
  return td::Status::OK();
}

td::Status ToslibClient::do_request(toslib_api::options_setConfig& request,
                                    td::Promise<object_ptr<toslib_api::options_configInfo>>&& promise) {
  if (!request.config_) {
    return ToslibError::EmptyField("config");
  }
...
struct ToRawTransactions {
  ...
  td::Result<toslib_api::object_ptr<toslib_api::raw_message>> to_raw_message_or_throw(td::Ref<vm::Cell> cell) {
    ...
    auto get_data = [body = std::move(body), body_cell = std::move(body_cell),
                     ...
                     this](td::Slice salt) mutable -> td::Result<toslib_api::object_ptr<toslib_api::msg_Data>> {
      ...
      if (try_decode_messages_ && body->size() >= 32) {
        ...
        if (type == 0 || type == tos::WalletInterface::EncryptedCommentOp) {
          td::Status status;
          ...
          if (r_body_message.is_ok()) {
            if (type == 0) {
              data = toslib_api::make_object<toslib_api::msg_dataText>(r_body_message.move_as_ok());
            } else {
              auto encrypted_message = r_body_message.move_as_ok();
              auto r_decrypted_message = [&]() -> td::Result<std::string> {
                if (!private_key_) {
                  return ToslibError::EmptyField("private_key");
                }
              ...
              if (r_decrypted_message.is_ok()) {
                data = toslib_api::make_object<toslib_api::msg_dataDecryptedText>(r_decrypted_message.move_as_ok());
              } else {
                data = toslib_api::make_object<toslib_api::msg_dataEncryptedText>(encrypted_message);
              }
            }
          }
        }
      ...
      if (!data) {
        ...
        data = toslib_api::make_object<toslib_api::msg_dataRaw>(std::move(body_boc), std::move(init_state_boc));
      }
      return data;
    };
    ...
    switch (tag) {
      case block::gen::CommonMsgInfo::int_msg_info: {
        ...
        return toslib_api::make_object<toslib_api::raw_message>(
            msg_hash, toslib_api::make_object<toslib_api::accountAddress>(src),
            toslib_api::make_object<toslib_api::accountAddress>(std::move(dest)), balance, std::move(extra_currencies),
      ...
      case block::gen::CommonMsgInfo::ext_in_msg_info: {
        ...
        return toslib_api::make_object<toslib_api::raw_message>(
            msg_hash, toslib_api::make_object<toslib_api::accountAddress>(),
            toslib_api::make_object<toslib_api::accountAddress>(std::move(dest)), 0,
            std::vector<toslib_api::object_ptr<toslib_api::extraCurrency>>{}, 0, 0, 0, std::move(body_hash),
            std::move(data));
      }
      case block::gen::CommonMsgInfo::ext_out_msg_info: {
        ...
        return toslib_api::make_object<toslib_api::raw_message>(
            msg_hash, toslib_api::make_object<toslib_api::accountAddress>(src),
            toslib_api::make_object<toslib_api::accountAddress>(), 0,
            std::vector<toslib_api::object_ptr<toslib_api::extraCurrency>>{}, 0, 0, created_lt, std::move(body_hash),
            std::move(data));
      }
    }
  ...
  td::Result<toslib_api::object_ptr<toslib_api::raw_message>> to_raw_message(td::Ref<vm::Cell> cell) {
    return TRY_VM(to_raw_message_or_throw(std::move(cell)));
  }

  td::Result<toslib_api::object_ptr<toslib_api::raw_transaction>> to_raw_transaction_or_throw(
      block::Transaction::Info&& info) {
    std::string data;

    toslib_api::object_ptr<toslib_api::raw_message> in_msg;
    std::vector<toslib_api::object_ptr<toslib_api::raw_message>> out_msgs;
    td::int64 fees = 0;
    td::int64 storage_fee = 0;
    td::string address;
    ...
    return toslib_api::make_object<toslib_api::raw_transaction>(
        toslib_api::make_object<toslib_api::accountAddress>(std::move(address)), info.now, data,
        toslib_api::make_object<toslib_api::internal_transactionId>(info.prev_trans_lt,
                                                                    info.prev_trans_hash.as_slice().str()),
        fees, storage_fee, fees - storage_fee, std::move(in_msg), std::move(out_msgs));
  }

  td::Result<toslib_api::object_ptr<toslib_api::raw_transaction>> to_raw_transaction(block::Transaction::Info&& info) {
    return TRY_VM(to_raw_transaction_or_throw(std::move(info)));
  }

  td::Result<toslib_api::object_ptr<toslib_api::raw_transactions>> to_raw_transactions(
      block::TransactionList::Info&& info) {
    std::vector<toslib_api::object_ptr<toslib_api::raw_transaction>> transactions;
    ...
    auto transaction_id =
        toslib_api::make_object<toslib_api::internal_transactionId>(info.lt, info.hash.as_slice().str());
    for (auto& transaction : transactions) {
      std::swap(transaction->transaction_id_, transaction_id);
    }

    return toslib_api::make_object<toslib_api::raw_transactions>(std::move(transactions), std::move(transaction_id));
  }
  ...
  td::Result<toslib_api::object_ptr<toslib_api::raw_transaction>> to_raw_transaction_or_throw(
    ...
    return toslib_api::make_object<toslib_api::raw_transaction>(
        toslib_api::make_object<toslib_api::accountAddress>(std::move(address)), info.now, data,
        toslib_api::make_object<toslib_api::internal_transactionId>(info.lt, info.hash.as_slice().str()), fees,
        storage_fee, fees - storage_fee, std::move(in_msg), std::move(out_msgs));
  }

  td::Result<toslib_api::object_ptr<toslib_api::raw_transaction>> to_raw_transaction(
      block::BlockTransaction::Info&& info) {
    return TRY_VM(to_raw_transaction_or_throw(std::move(info)));
  }

  td::Result<std::vector<toslib_api::object_ptr<toslib_api::raw_transaction>>> to_raw_transactions(
...
auto to_any_promise(td::Promise<toslib_api::object_ptr<toslib_api::ok>>&& promise) {
  return promise.wrap([](auto x) { return toslib_api::make_object<toslib_api::ok>(); });
}
...
td::Status ToslibClient::do_request(const toslib_api::raw_sendMessageReturnHash& request,
  ...
  make_request(int_api::SendMessage{std::move(body)}, promise.wrap([hash = std::move(hash),
    ...
    return toslib_api::make_object<toslib_api::raw_extMessageInfo>(std::move(hash), hash_norm.as_slice().str());
  }));
  return td::Status::OK();
}

td::Status ToslibClient::do_request(const toslib_api::raw_createAndSendMessage& request,
                                    td::Promise<object_ptr<toslib_api::ok>>&& promise) {
  if (!request.destination_) {
    return ToslibError::EmptyField("destination");
  }
...
td::Status ToslibClient::do_request(toslib_api::raw_getAccountState& request,
                                    td::Promise<object_ptr<toslib_api::raw_fullAccountState>>&& promise) {
  if (!request.account_address_) {
    return ToslibError::EmptyField("account_address");
  }
...
td::Status ToslibClient::do_request(toslib_api::raw_getAccountStateByTransaction& request,
                                    td::Promise<object_ptr<toslib_api::raw_fullAccountState>>&& promise) {
  if (!request.account_address_) {
    return ToslibError::EmptyField("account_address");
  }
  if (!request.transaction_id_) {
    return ToslibError::EmptyField("transaction_id");
  }
  TRY_RESULT(account_address, get_account_address(request.account_address_->account_address_));
...
td::Result<KeyStorage::InputKey> from_toslib(toslib_api::inputKeyRegular& input_key) {
  if (!input_key.key_) {
    return ToslibError::EmptyField("key");
  }
...
td::Result<KeyStorage::InputKey> from_toslib(toslib_api::InputKey& input_key) {
  return downcast_call2<td::Result<KeyStorage::InputKey>>(
      input_key, td::overloaded([&](toslib_api::inputKeyRegular& input_key) { return from_toslib(input_key); },
                                [&](toslib_api::inputKeyFake&) { return KeyStorage::fake_input_key(); }));
}

td::Status ToslibClient::do_request(toslib_api::raw_getTransactions& request,
                                    td::Promise<object_ptr<toslib_api::raw_transactions>>&& promise) {
  if (!request.account_address_) {
    return ToslibError::EmptyField("account_address");
  }
  if (!request.from_transaction_id_) {
    return ToslibError::EmptyField("from_transaction_id");
  }
  TRY_RESULT(account_address, get_account_address(request.account_address_->account_address_));
  td::optional<td::Ed25519::PrivateKey> private_key;
  if (request.private_key_) {
    TRY_RESULT(input_key, from_toslib(*request.private_key_));
...
td::Status ToslibClient::do_request(toslib_api::raw_getTransactionsV2& request,
                                    td::Promise<object_ptr<toslib_api::raw_transactions>>&& promise) {
  if (!request.account_address_) {
    return ToslibError::EmptyField("account_address");
  }
...
td::Status ToslibClient::do_request(const toslib_api::getAccountState& request,
...
td::Status ToslibClient::do_request(const toslib_api::getAccountStateByTransaction& request,
                                    td::Promise<object_ptr<toslib_api::fullAccountState>>&& promise) {
  if (!request.account_address_) {
    return ToslibError::EmptyField("account_address");
  }
...
td::Status ToslibClient::do_request(const toslib_api::getShardAccountCell& request,
...
td::Status ToslibClient::do_request(const toslib_api::getShardAccountCellByTransaction& request,
                                    td::Promise<object_ptr<toslib_api::tvm_cell>>&& promise) {
  if (!request.account_address_) {
    return ToslibError::EmptyField("account_address");
  }
...
td::Result<tos::ManualDns::EntryData> to_dns_entry_data(toslib_api::dns_EntryData& entry_data) {
  using R = td::Result<tos::ManualDns::EntryData>;
  return downcast_call2<R>(
      entry_data,
      td::overloaded(
          [&](toslib_api::dns_entryDataUnknown& unknown) -> R { return tos::ManualDns::EntryData(); },
          [&](toslib_api::dns_entryDataNextResolver& next_resolver) -> R {
            if (!next_resolver.resolver_) {
              return ToslibError::EmptyField("resolver");
            }
            TRY_RESULT(resolver, get_account_address(next_resolver.resolver_->account_address_));
            return tos::ManualDns::EntryData::next_resolver(std::move(resolver));
          },
          [&](toslib_api::dns_entryDataSmcAddress& smc_address) -> R {
            if (!smc_address.smc_address_) {
              return ToslibError::EmptyField("smc_address");
            }
            TRY_RESULT(address, get_account_address(smc_address.smc_address_->account_address_));
            return tos::ManualDns::EntryData::smc_address(std::move(address));
          },
          [&](toslib_api::dns_entryDataAdnlAddress& adnl_address) -> R {
            if (!adnl_address.adnl_address_) {
              return ToslibError::EmptyField("adnl_address");
            }
            TRY_RESULT(address, get_adnl_address(adnl_address.adnl_address_->adnl_address_));
            return tos::ManualDns::EntryData::adnl_address(std::move(address));
          },
          [&](toslib_api::dns_entryDataStorageAddress& storage_address) -> R {
            return tos::ManualDns::EntryData::storage_address(storage_address.bag_id_);
          },
          [&](toslib_api::dns_entryDataText& text) -> R { return tos::ManualDns::EntryData::text(text.text_); }));
}

class GenericCreateSendGrams : public ToslibQueryActor {
 public:
  GenericCreateSendGrams(td::actor::ActorShared<ToslibClient> client, toslib_api::createQuery query,
                         td::optional<tos::BlockIdExt> block_id, td::Promise<td::unique_ptr<Query>>&& promise)
      : ToslibQueryActor(std::move(client))
 ...
 private:
  ...
  std::vector<tos::ManualDns::Action> dns_actions_;
  ...
  td::Result<Action> to_action(const toslib_api::msg_message& message) {
    if (!message.destination_) {
      return ToslibError::EmptyField("message.destination");
    }
    Action res;
    TRY_RESULT(destination, get_account_address(message.destination_->account_address_));
    res.destination = destination;
    if (message.amount_ < 0) {
      return ToslibError::InvalidField("amount", "can't be negative");
    }
    ...
    auto status = downcast_call2<td::Status>(
        *message.data_, td::overloaded(
                            ...
                            [&](toslib_api::msg_dataText& text) {
                              res.message = text.text_;
                              res.should_encrypt = false;
                              res.is_encrypted = false;
                              return td::Status::OK();
                            },
                            [&](toslib_api::msg_dataDecryptedText& text) {
                              res.message = text.text_;
                              if (!has_private_key_) {
                                return ToslibError::EmptyField("input_key");
                              }
                              res.should_encrypt = true;
                              res.is_encrypted = true;
                              return td::Status::OK();
                            },
                            [&](toslib_api::msg_dataEncryptedText& text) {
                              res.message = text.text_;
                              res.should_encrypt = false;
                              res.is_encrypted = true;
                              return td::Status::OK();
    ...
    if (res.message.size() > tos::WalletV3Traits::max_message_size) {
      return ToslibError::MessageTooLong();
    }
    TRY_STATUS(std::move(status));
    return std::move(res);
  }

  td::Result<tos::ManualDns::Action> to_dns_action(toslib_api::dns_Action& action) {
    using R = td::Result<tos::ManualDns::Action>;
    return downcast_call2<R>(action, td::overloaded(
                                         [&](toslib_api::dns_actionDeleteAll& del_all) -> R {
                                           return tos::ManualDns::Action{"", td::Bits256::zero(), {}};
                                         },
                                         [&](toslib_api::dns_actionDelete& del) -> R {
                                           return tos::ManualDns::Action{del.name_, del.category_, {}};
                                         },
                                         [&](toslib_api::dns_actionSet& set) -> R {
                                           if (!set.entry_) {
                                             return ToslibError::EmptyField("entry");
                                           }
                                           if (!set.entry_->entry_) {
                                             return ToslibError::EmptyField("entry.entry");
                                           }
                                           TRY_RESULT(entry_data, to_dns_entry_data(*set.entry_->entry_));
                                           TRY_RESULT(data_cell, entry_data.as_cell());
                                           return tos::ManualDns::Action{set.entry_->name_, set.entry_->category_,
                                                                         std::move(data_cell)};
                                         }));
  }

  td::Status parse_action(toslib_api::Action& action) {
    return downcast_call2<td::Status>(action,
                                      td::overloaded([&](toslib_api::actionNoop& cell) { return td::Status::OK(); },
                                                     [&](toslib_api::actionMsg& cell) {
                                                       allow_send_to_uninited_ = cell.allow_send_to_uninited_;
                                                       for (auto& from_action : cell.messages_) {
                                                         if (!from_action) {
                                                           return ToslibError::EmptyField("message");
                                                         }
                                                     ...
                                                     [&](toslib_api::actionPchan& cell) {
                                                       pchan_action_ = true;
                                                       return td::Status::OK();
                                                     },
                                                     [&](toslib_api::actionRwallet& cell) {
                                                       rwallet_action_ = true;
                                                       return td::Status::OK();
                                                     },
                                                     [&](toslib_api::actionDns& cell) {
                                                       for (auto& from_action : cell.actions_) {
                                                         if (!from_action) {
                                                           return ToslibError::EmptyField("action");
                                                         }
  ...
  td::Status do_start_up() {
    if (query_.timeout_ < 0 || query_.timeout_ > 300) {
      return ToslibError::InvalidField("timeout", "must be between 0 and 300");
    }
    if (!query_.address_) {
      return ToslibError::EmptyField("address");
    }
    if (!query_.action_) {
      return ToslibError::EmptyField("action");
    }
    ...
    if (has_private_key_) {
      TRY_RESULT(input_key, from_toslib(*query_.private_key_));
      is_fake_key_ = query_.private_key_->get_id() == toslib_api::inputKeyFake::ID;
  ...
  td::Status do_dns_loop() {
    ...
    auto dns = tos::ManualDns::create(source_->get_smc_state());
  ...
  td::Status do_pchan_loop(td::Ref<tos::PaymentChannel> pchan, toslib_api::actionPchan& action) {
    if (!action.action_) {
      return ToslibError::EmptyField("action");
    }
    ...
    if (!is_alice && !is_bob) {
      return ToslibError::InvalidField("private_key", "invalid for this smartcontract");
    }
    auto status = downcast_call2<td::Status>(
        *action.action_,
        td::overloaded(
            [&](toslib_api::pchan_actionTimeout& timeout) {
              auto builder = tos::pchan::MsgTimeoutBuilder();
              if (is_alice) {
            ...
            [&](toslib_api::pchan_actionInit& init) {
              auto builder = tos::pchan::MsgInitBuilder()
                                 .inc_A(init.inc_A_)
                                 .inc_B(init.inc_B_)
                                 .min_A(init.min_A_)
                                 .min_B(init.min_B_)
            ...
            [&](toslib_api::pchan_actionClose& close) {
              if (!close.promise_) {
                return ToslibError::EmptyField("promise");
              }

              tos::pchan::SignedPromiseBuilder sbuilder;
              ...
              if (is_alice && !sbuilder.check_signature(close.promise_->signature_,
                ...
                return ToslibError::InvalidSignature();
              }
              ...
              auto builder = tos::pchan::MsgCloseBuilder()
  ...
  td::Status do_pchan_loop() {
    ...
    auto pchan = tos::PaymentChannel::create(source_->get_smc_state());

    return downcast_call2<td::Status>(
        *query_.action_, td::overloaded([&](toslib_api::actionNoop& cell) { return td::Status::OK(); },
                                        [&](auto& cell) { return td::Status::Error(); },
                                        [&](toslib_api::actionPchan& cell) { return do_pchan_loop(pchan, cell); }));
  }

  td::Status do_rwallet_action(td::Ref<tos::RestrictedWallet> rwallet, toslib_api::actionRwallet& action) {
    if (!action.action_) {
      return ToslibError::EmptyField("action");
    }
  ...
  td::Status do_rwallet_action() {
    ...
    auto rwallet = tos::RestrictedWallet::create(source_->get_smc_state());
    return downcast_call2<td::Status>(
        *query_.action_,
        td::overloaded([&](auto& cell) { return td::Status::Error("UNREACHABLE"); },
                       [&](toslib_api::actionRwallet& cell) { return do_rwallet_action(rwallet, cell); }));
  }
  ...
  td::Status do_loop() {
    ...
    switch (source_->get_wallet_type()) {
      case AccountState::Empty:
        return ToslibError::AccountNotInited();
      case AccountState::Unknown:
        return ToslibError::AccountTypeUnknown();
      default:
        break;
    }

    if (!source_->is_wallet()) {
      return ToslibError::AccountActionUnsupported("wallet action");
    }
    ...
    if (source_->get_wallet_type() == AccountState::RestrictedWallet) {
      auto r_unlocked_balance = tos::RestrictedWallet::create(source_->get_smc_state())
    ...
    for (auto& action : actions_) {
      ...
      if (action.body.not_null()) {
        ...
        if (!o_public_key) {
          auto smc = tos::SmartContract::create(destination->get_smc_state());
          auto r_public_key = tos::GenericAccount::get_public_key(destination->get_smc_state());
          if (r_public_key.is_ok()) {
            o_public_key = r_public_key.move_as_ok();
          }
        }
        ...
        if (!o_public_key) {
          return ToslibError::MessageEncryption("Cannot get public key of destination (possibly unknown wallet type)");
        }
    ...
    auto with_wallet = [&](auto&& wallet) {
      ...
      if (wallet.get_max_gifts_size() < gifts.size()) {
        return ToslibError::MessageTooLong();  // TODO: other error
      }
...
td::int64 ToslibClient::register_query(td::unique_ptr<Query> query) {
  auto query_id = ++next_query_id_;
  queries_[query_id] = std::move(query);
  return query_id;
}

td::Result<toslib_api::object_ptr<toslib_api::query_info>> ToslibClient::get_query_info(td::int64 id) {
  ...
  return toslib_api::make_object<toslib_api::query_info>(id, it->second->get_valid_until(),
                                                         it->second->get_body_hash().as_slice().str(),
                                                         std::move(message_body), std::move(init_state));
}

void ToslibClient::finish_create_query(td::Result<td::unique_ptr<Query>> r_query,
...
td::Status ToslibClient::do_request(toslib_api::createQuery& request,
                                    td::Promise<object_ptr<toslib_api::query_info>>&& promise) {
  auto id = actor_id_++;
...
td::Status ToslibClient::do_request(toslib_api::msg_decrypt& request,
                                    td::Promise<object_ptr<toslib_api::msg_dataDecryptedArray>>&& promise) {
  if (!request.input_key_) {
    return ToslibError::EmptyField("input_key");
  }
  if (!request.data_) {
    return ToslibError::EmptyField("data");
  }
  TRY_RESULT(input_key, from_toslib(*request.input_key_));
  using ReturnType = toslib_api::object_ptr<toslib_api::msg_dataDecrypted>;
  make_request(
      ...
      promise.wrap([elements = std::move(request.data_)](auto key) mutable {
        ...
        auto new_elements = td::transform(std::move(elements->elements_), [&private_key](auto msg) -> ReturnType {
          auto res = toslib_api::make_object<toslib_api::msg_dataDecrypted>();
          if (!msg) {
            return res;
          }
          if (!msg->data_) {
          ...
          return downcast_call2<ReturnType>(
              *res->data_,
              td::overloaded(
                  ...
                  [&res, &private_key, &msg](toslib_api::msg_dataEncryptedText& encrypted) -> ReturnType {
                    auto r_decrypted =
                    ...
                    return toslib_api::make_object<toslib_api::msg_dataDecrypted>(
                        decrypted.proof.as_slice().str(),
                        toslib_api::make_object<toslib_api::msg_dataDecryptedText>(decrypted.data.as_slice().str()));
                  }));
        });
        return toslib_api::make_object<toslib_api::msg_dataDecryptedArray>(std::move(new_elements));
      }));
  return td::Status::OK();
}

td::Status ToslibClient::do_request(const toslib_api::raw_createQuery& request,
                                    td::Promise<object_ptr<toslib_api::query_info>>&& promise) {
  if (!request.destination_) {
    return ToslibError::EmptyField("destination");
  }
...
td::Status ToslibClient::do_request(const toslib_api::query_getInfo& request,
                                    td::Promise<object_ptr<toslib_api::query_info>>&& promise) {
  promise.set_result(get_query_info(request.id_));
  return td::Status::OK();
}

void ToslibClient::query_estimate_fees(td::int64 id, bool ignore_chksig, td::Result<LastConfigState> r_state,
                                       td::Promise<object_ptr<toslib_api::query_fees>>&& promise) {
  auto it = queries_.find(id);
  if (it == queries_.end()) {
    promise.set_error(ToslibError::InvalidQueryId());
    return;
  }
  ...
  TRY_RESULT_PROMISE_PREFIX(promise, fees, TRY_VM(it->second->estimate_fees(ignore_chksig, state, libraries)),
                            ToslibError::Internal());
  promise.set_value(toslib_api::make_object<toslib_api::query_fees>(
      fees.first.to_toslib_api(), td::transform(fees.second, [](auto& x) { return x.to_toslib_api(); })));
}

td::Status ToslibClient::do_request(const toslib_api::query_estimateFees& request,
                                    td::Promise<object_ptr<toslib_api::query_fees>>&& promise) {
...
td::Status ToslibClient::do_request(const toslib_api::query_send& request,
                                    td::Promise<object_ptr<toslib_api::ok>>&& promise) {
  auto it = queries_.find(request.id_);
  if (it == queries_.end()) {
    return ToslibError::InvalidQueryId();
...
td::Status ToslibClient::do_request(toslib_api::query_forget& request,
                                    td::Promise<object_ptr<toslib_api::ok>>&& promise) {
...
td::int64 ToslibClient::register_smc(td::unique_ptr<AccountState> smc) {
  auto smc_id = ++next_smc_id_;
  smcs_[smc_id] = std::move(smc);
  return smc_id;
}

td::Result<toslib_api::object_ptr<toslib_api::smc_info>> ToslibClient::get_smc_info(td::int64 id) {
  auto it = smcs_.find(id);
  if (it == smcs_.end()) {
    return ToslibError::InvalidSmcId();
  }
  return toslib_api::make_object<toslib_api::smc_info>(id);
}

void ToslibClient::finish_load_smc(td::unique_ptr<AccountState> smc,
                                   td::Promise<object_ptr<toslib_api::smc_info>>&& promise) {
  auto id = register_smc(std::move(smc));
  promise.set_result(get_smc_info(id));
}

td::Status ToslibClient::do_request(const toslib_api::smc_load& request,
                                    td::Promise<object_ptr<toslib_api::smc_info>>&& promise) {
  if (!request.account_address_) {
    return ToslibError::EmptyField("account_address");
  }
  TRY_RESULT(account_address, get_account_address(request.account_address_->account_address_));
  make_request(int_api::GetAccountState{std::move(account_address), query_context_.block_id.copy(), {}},
               promise.send_closure(actor_id(this), &ToslibClient::finish_load_smc));
  return td::Status::OK();
}

td::Status ToslibClient::do_request(const toslib_api::smc_loadByTransaction& request,
                                    td::Promise<object_ptr<toslib_api::smc_info>>&& promise) {
  if (!request.account_address_) {
    return ToslibError::EmptyField("account_address");
  }
...
td::Status ToslibClient::do_request(const toslib_api::smc_forget& request,
                                    td::Promise<object_ptr<toslib_api::ok>>&& promise) {
...
td::Status ToslibClient::do_request(const toslib_api::smc_getCode& request,
                                    td::Promise<object_ptr<toslib_api::tvm_cell>>&& promise) {
  auto it = smcs_.find(request.id_);
  if (it == smcs_.end()) {
    return ToslibError::InvalidSmcId();
  ...
  promise.set_result(to_toslib_tvm_cell(code));
  return td::Status::OK();
}

td::Status ToslibClient::do_request(const toslib_api::smc_getData& request,
                                    td::Promise<object_ptr<toslib_api::tvm_cell>>&& promise) {
  auto it = smcs_.find(request.id_);
  if (it == smcs_.end()) {
    return ToslibError::InvalidSmcId();
  ...
  promise.set_result(to_toslib_tvm_cell(data));
  return td::Status::OK();
}

td::Status ToslibClient::do_request(const toslib_api::smc_getState& request,
                                    td::Promise<object_ptr<toslib_api::tvm_cell>>&& promise) {
  auto it = smcs_.find(request.id_);
  if (it == smcs_.end()) {
    return ToslibError::InvalidSmcId();
...
td::Status ToslibClient::do_request(const toslib_api::smc_getRawFullAccountState& request,
                                    td::Promise<object_ptr<toslib_api::raw_fullAccountState>>&& promise) {
  auto it = smcs_.find(request.id_);
  if (it == smcs_.end()) {
    return ToslibError::InvalidSmcId();
...
auto to_toslib_api(const vm::StackEntry& entry, int& limit)
    -> td::Result<toslib_api::object_ptr<toslib_api::tvm_StackEntry>> {
  if (limit <= 0) {
    return td::Status::Error(PSLICE() << "TVM stack size exceeds limit");
  }
  switch (entry.type()) {
    case vm::StackEntry::Type::t_int:
      return toslib_api::make_object<toslib_api::tvm_stackEntryNumber>(
          toslib_api::make_object<toslib_api::tvm_numberDecimal>(dec_string(entry.as_int())));
    case vm::StackEntry::Type::t_slice: {
      TRY_RESULT(bytes, to_bytes(vm::CellBuilder().append_cellslice(entry.as_slice()).finalize()));
      return toslib_api::make_object<toslib_api::tvm_stackEntrySlice>(
          toslib_api::make_object<toslib_api::tvm_slice>(std::move(bytes)));
    }
    case vm::StackEntry::Type::t_cell: {
      TRY_RESULT(cell, to_toslib_tvm_cell(entry.as_cell()));
      return toslib_api::make_object<toslib_api::tvm_stackEntryCell>(std::move(cell));
    }
    case vm::StackEntry::Type::t_null:
    case vm::StackEntry::Type::t_tuple: {
      std::vector<toslib_api::object_ptr<toslib_api::tvm_StackEntry>> elements;
      if (is_list(entry)) {
        auto node = entry;
        while (node.type() == vm::StackEntry::Type::t_tuple) {
          TRY_RESULT(tl_entry, to_toslib_api(node.as_tuple()->at(0), --limit));
          elements.push_back(std::move(tl_entry));
          node = node.as_tuple()->at(1);
        }
        return toslib_api::make_object<toslib_api::tvm_stackEntryList>(
            toslib_api::make_object<toslib_api::tvm_list>(std::move(elements)));

      } else {
        for (auto& element : *entry.as_tuple()) {
          TRY_RESULT(tl_entry, to_toslib_api(element, --limit));
          elements.push_back(std::move(tl_entry));
        }
        return toslib_api::make_object<toslib_api::tvm_stackEntryTuple>(
            toslib_api::make_object<toslib_api::tvm_tuple>(std::move(elements)));
      }
    }

    default:
      return toslib_api::make_object<toslib_api::tvm_stackEntryUnsupported>();
  }
};

auto to_toslib_api(const td::Ref<vm::Stack>& stack)
    -> td::Result<std::vector<toslib_api::object_ptr<toslib_api::tvm_StackEntry>>> {
  int stack_limit = 8000;
  std::vector<toslib_api::object_ptr<toslib_api::tvm_StackEntry>> tl_stack;
  for (auto& entry : stack->as_span()) {
    TRY_RESULT(tl_entry, to_toslib_api(entry, --stack_limit));
    tl_stack.push_back(std::move(tl_entry));
  }
  return tl_stack;
}

td::Result<vm::StackEntry> from_toslib_api(toslib_api::tvm_StackEntry& entry) {
  // TODO: error codes
  // downcast_call
  return downcast_call2<td::Result<vm::StackEntry>>(
      entry,
      td::overloaded(
          [&](toslib_api::tvm_stackEntryUnsupported& cell) { return td::Status::Error("Unsuppored stack entry"); },
          [&](toslib_api::tvm_stackEntrySlice& cell) -> td::Result<vm::StackEntry> {
            if (!cell.slice_) {
              return ToslibError::EmptyField("slice");
            }
          ...
          [&](toslib_api::tvm_stackEntryCell& cell) -> td::Result<vm::StackEntry> {
            if (!cell.cell_) {
              return ToslibError::EmptyField("cell");
            }
            TRY_RESULT(res, vm::std_boc_deserialize(cell.cell_->bytes_));
            return vm::StackEntry{std::move(res)};
          },
          [&](toslib_api::tvm_stackEntryTuple& tuple) -> td::Result<vm::StackEntry> {
            if (!tuple.tuple_) {
              return ToslibError::EmptyField("tuple");
            }
            std::vector<vm::StackEntry> elements;
            for (auto& element : tuple.tuple_->elements_) {
              if (!element) {
                return ToslibError::EmptyField("tuple.elements");
              }
              TRY_RESULT(new_element, from_toslib_api(*element));
              elements.push_back(std::move(new_element));
            }
            return td::Ref<vm::Tuple>(true, std::move(elements));
          },
          [&](toslib_api::tvm_stackEntryList& list) -> td::Result<vm::StackEntry> {
            if (!list.list_) {
              return ToslibError::EmptyField("list");
            }
            vm::StackEntry tail;
            for (auto& element : td::reversed(list.list_->elements_)) {
              if (!element) {
                return ToslibError::EmptyField("list.elements");
              }
              TRY_RESULT(new_element, from_toslib_api(*element));
          ...
          [&](toslib_api::tvm_stackEntryNumber& number) -> td::Result<vm::StackEntry> {
            if (!number.number_) {
              return ToslibError::EmptyField("number");
            }
...
td::Status ToslibClient::do_request(const toslib_api::smc_getLibraries& request,
                                    td::Promise<object_ptr<toslib_api::smc_libraryResult>>&& promise) {
  if (request.library_list_.size() > 16) {
    promise.set_error(ToslibError::InvalidField("library_list", ": too many libraries requested, 16 maximum"));
  }
  if (query_context_.block_id) {
    ...
    client_.with_last_block([this, promise = std::move(promise),
      ...
      if (r_last_block.is_error()) {
        promise.set_error(r_last_block.move_as_error_prefix(ToslibError::Internal("get last block failed ")));
      } else {
...
void ToslibClient::get_libraries(tos::BlockIdExt blkid, std::vector<td::Bits256> library_list,
                                 td::Promise<object_ptr<toslib_api::smc_libraryResult>>&& promise) {
  sort(library_list.begin(), library_list.end());
  library_list.erase(unique(library_list.begin(), library_list.end()), library_list.end());

  std::vector<object_ptr<toslib_api::smc_libraryEntry>> result_entries;
  ...
  for (auto& library_hash : library_list) {
    if (libraries.key_exists(library_hash)) {
      ...
      result_entries.push_back(toslib_api::make_object<toslib_api::smc_libraryEntry>(library_hash, library_content));
    } else {
  ...
  if (not_cached_hashes.empty()) {
    promise.set_value(toslib_api::make_object<toslib_api::smc_libraryResult>(std::move(result_entries)));
    return;
  }

  auto missed_lib_ids = not_cached_hashes;
  client_.send_query(
      tos::lite_api::liteServer_getLibrariesWithProof(tos::create_tl_lite_block_id(blkid), 1,
                                                      std::move(missed_lib_ids)),
      promise.wrap([self = this, blkid, result_entries = std::move(result_entries), not_cached_hashes](
                       td::Result<tos::lite_api::object_ptr<tos::lite_api::liteServer_libraryResultWithProof>>
                           r_libraries) mutable -> td::Result<toslib_api::object_ptr<toslib_api::smc_libraryResult>> {
        ...
        try {
          ...
          for (auto& hash : not_cached_hashes) {
            ...
            if (csr.is_null()) {
              ...
              if (std::any_of(libraries->result_.begin(), libraries->result_.end(),
                ...
                return ToslibError::Internal("library is included in response but it's not found in proof");
              }
              continue;
            }
            block::gen::LibDescr::Record libdescr;
            if (!tlb::csr_unpack(csr, libdescr)) {
              return ToslibError::Internal("cannot unpack LibDescr record");
            }
            ...
            if (lib_it == libraries->result_.end()) {
              return ToslibError::Internal("library is found in proof but not in response");
            }
            auto& lib = *lib_it;
            auto contents = vm::std_boc_deserialize(lib->data_);
            if (!contents.is_ok() || contents.ok().is_null()) {
              return ToslibError::Internal(PSLICE() << "cannot deserialize library cell " << lib->hash_.to_hex());
            }
          ...
          return toslib_api::make_object<toslib_api::smc_libraryResult>(std::move(result_entries));
        } catch (vm::VmError& err) {
          return ToslibError::Internal(PSLICE()
...
td::Status ToslibClient::do_request(const toslib_api::smc_getLibrariesExt& request,
  ...
  for (auto& x : request.list_) {
    ...
    downcast_call(*x, td::overloaded([&](toslib_api::smc_libraryQueryExt_one& one) { request_libs.insert(one.hash_); },
                                     [&](toslib_api::smc_libraryQueryExt_scanBoc& scan) {
  ...
  for (size_t i = 0; i < not_cached.size(); i += 16) {
    ...
    client_.send_query(
        tos::lite_api::liteServer_getLibraries(
  ...
  ig.add_promise(promise.wrap([self = this, libs = std::move(request_libs)](td::Unit&&) {
    ...
    return tos::create_tl_object<toslib_api::smc_libraryResultExt>(dict_boc.as_slice().str(), std::move(libs_ok),
...
td::Status ToslibClient::do_request(const toslib_api::smc_runGetMethod& request,
  ...
  if (!request.method_) {
    return ToslibError::EmptyField("method");
  }

  td::Ref<tos::SmartContract> smc(true, it->second->get_smc_state());
  tos::SmartContract::Args args;
  downcast_call(*request.method_,
                td::overloaded([&](toslib_api::smc_methodIdNumber& number) { args.set_method_id(number.number_); },
                               [&](toslib_api::smc_methodIdName& name) { args.set_method_id(name.name_); }));
  td::Ref<vm::Stack> stack(true);
  td::Status status;
  for (auto& entry : request.stack_) {
    if (!entry) {
      return ToslibError::EmptyField("stack");
    }
    TRY_RESULT(e, from_toslib_api(*entry));
  ...
  client_.with_last_config([self = this, smc = std::move(smc), args = std::move(args),
    ...
    if (code.not_null()) {
      ...
      if (libraryList.size() > 0) {
        ...
        self->client_.send_query(
            tos::lite_api::liteServer_getLibraries(std::move(libraryList)),
...
void ToslibClient::process_new_libraries(
...
void ToslibClient::perform_smc_execution(td::Ref<tos::SmartContract> smc, tos::SmartContract::Args args,
  ...
  if (res.missing_library) {
    ...
    promise.set_value(toslib_api::make_object<toslib_api::smc_runResult>(res.gas_used, std::move(res_stack), res.code));
  }
}

td::Result<toslib_api::object_ptr<toslib_api::dns_EntryData>> to_toslib_api(
    const tos::ManualDns::EntryData& entry_data) {
  td::Result<toslib_api::object_ptr<toslib_api::dns_EntryData>> res;
  if (entry_data.data.empty()) {
    return ToslibError::Internal("Unexpected empty EntryData");
  }
  entry_data.data.visit(td::overloaded(
      [&](const tos::ManualDns::EntryDataText& text) {
        res = toslib_api::make_object<toslib_api::dns_entryDataText>(text.text);
      },
      [&](const tos::ManualDns::EntryDataNextResolver& resolver) {
        res = toslib_api::make_object<toslib_api::dns_entryDataNextResolver>(
            toslib_api::make_object<toslib_api::accountAddress>(resolver.resolver.rserialize(true)));
      },
      [&](const tos::ManualDns::EntryDataAdnlAddress& adnl_address) {
        res = toslib_api::make_object<toslib_api::dns_entryDataAdnlAddress>(
            toslib_api::make_object<toslib_api::adnlAddress>(
                td::adnl_id_encode(adnl_address.adnl_address.as_slice()).move_as_ok()));
      },
      [&](const tos::ManualDns::EntryDataSmcAddress& smc_address) {
        res = toslib_api::make_object<toslib_api::dns_entryDataSmcAddress>(
            toslib_api::make_object<toslib_api::accountAddress>(smc_address.smc_address.rserialize(true)));
      },
      [&](const tos::ManualDns::EntryDataStorageAddress& storage_address) {
        res = toslib_api::make_object<toslib_api::dns_entryDataStorageAddress>(storage_address.bag_id);
      }));
  return res;
}
...
void ToslibClient::finish_dns_resolve(std::string name, td::Bits256 category, td::int32 ttl,
  ...
  if (entries.size() == 1 && entries[0].partially_resolved && ttl > 0) {
    ...
    if (suffix != got_name) {
      TRY_STATUS_PROMISE(promise, ToslibError::Internal("domain is not a suffix of the query"));
    }
  ...
  for (auto& entry : entries) {
    TRY_RESULT_PROMISE(promise, entry_data, to_toslib_api(entry.data));
...
void ToslibClient::do_dns_request(std::string name, td::Bits256 category, td::int32 ttl,
  ...
  TRY_RESULT_PROMISE(new_promise, args, tos::DnsInterface::resolve_args(name, category, address));
...
td::Status ToslibClient::do_request(toslib_api::pchan_signPromise& request,
  ...
  make_request(int_api::GetPrivateKey{std::move(input_key)},
               promise.wrap([promise = std::move(request.promise_)](auto key) mutable {
                 auto private_key = td::Ed25519::PrivateKey(std::move(key.private_key));
                 promise->signature_ = tos::pchan::SignedPromiseBuilder()
...
td::Status ToslibClient::do_request(toslib_api::pchan_validatePromise& request,
                                    td::Promise<object_ptr<toslib_api::ok>>&& promise) {
  if (!request.promise_) {
    return ToslibError::EmptyField("promise");
  }
  TRY_RESULT(key_bytes, get_public_key(request.public_key_));
  auto key = td::Ed25519::PublicKey(td::SecureString(key_bytes.key));
  bool is_ok = tos::pchan::SignedPromiseBuilder()
...
td::Status ToslibClient::do_request(toslib_api::pchan_packPromise& request,
                                    td::Promise<object_ptr<toslib_api::data>>&& promise) {
  if (!request.promise_) {
    return ToslibError::EmptyField("promise");
  }
  promise.set_result([&]() -> td::Result<object_ptr<toslib_api::data>> {
    TRY_RESULT(bytes, to_bytes(tos::pchan::SignedPromiseBuilder()
                                   .promise_A(request.promise_->promise_A_)
                                   .promise_B(request.promise_->promise_B_)
                                   .channel_id(request.promise_->channel_id_)
                                   .signature(td::SecureString(request.promise_->signature_))
                                   .finalize()));
    return toslib_api::make_object<toslib_api::data>(td::SecureString(std::move(bytes)));
  }());
  return td::Status::OK();
}

td::Status ToslibClient::do_request(toslib_api::pchan_unpackPromise& request,
                                    td::Promise<object_ptr<toslib_api::pchan_promise>>&& promise) {
  TRY_RESULT_PREFIX(body, vm::std_boc_deserialize(request.data_), ToslibError::InvalidBagOfCells("data"));
  tos::pchan::SignedPromise spromise;
  if (!spromise.unpack(body)) {
    return ToslibError::InvalidField("data", "Can't unpack as a promise");
  }
  promise.set_value(toslib_api::make_object<toslib_api::pchan_promise>(
...
td::Status ToslibClient::do_request(const toslib_api::createNewKey& request,
  ...
  promise.set_value(toslib_api::make_object<toslib_api::key>(key_bytes.serialize(true), std::move(key.secret)));
  return td::Status::OK();
}

td::Status ToslibClient::do_request(const toslib_api::exportKey& request,
  ...
  promise.set_value(toslib_api::make_object<toslib_api::exportedKey>(std::move(exported_key.mnemonic_words)));
  return td::Status::OK();
}

td::Status ToslibClient::do_request(const toslib_api::deleteKey& request,
                                    td::Promise<object_ptr<toslib_api::ok>>&& promise) {
  if (!request.key_) {
    return ToslibError::EmptyField("key");
  }
  ...
  TRY_STATUS_PREFIX(key_storage_.delete_key(key), ToslibError::KeyUnknown());
  promise.set_value(toslib_api::make_object<toslib_api::ok>());
  return td::Status::OK();
}

td::Status ToslibClient::do_request(const toslib_api::deleteAllKeys& request,
                                    td::Promise<object_ptr<toslib_api::ok>>&& promise) {
  TRY_STATUS_PREFIX(key_storage_.delete_all_keys(), ToslibError::Internal());
  promise.set_value(toslib_api::make_object<toslib_api::ok>());
  return td::Status::OK();
}

td::Status ToslibClient::do_request(const toslib_api::importKey& request,
                                    td::Promise<object_ptr<toslib_api::key>>&& promise) {
  if (!request.exported_key_) {
    return ToslibError::EmptyField("exported_key");
  }
...
td::Status ToslibClient::do_request(const toslib_api::exportPemKey& request,
  ...
  promise.set_value(toslib_api::make_object<toslib_api::exportedPemKey>(std::move(exported_pem_key.pem)));
  return td::Status::OK();
}

td::Status ToslibClient::do_request(const toslib_api::importPemKey& request,
                                    td::Promise<object_ptr<toslib_api::key>>&& promise) {
  if (!request.exported_key_) {
    return ToslibError::EmptyField("exported_key");
  }
...
td::Status ToslibClient::do_request(const toslib_api::exportEncryptedKey& request,
  ...
  promise.set_value(toslib_api::make_object<toslib_api::exportedEncryptedKey>(std::move(exported_key.data)));
  return td::Status::OK();
}

td::Status ToslibClient::do_request(const toslib_api::importEncryptedKey& request,
                                    td::Promise<object_ptr<toslib_api::key>>&& promise) {
  if (!request.exported_encrypted_key_) {
    return ToslibError::EmptyField("exported_encrypted_key");
  }
...
td::Status ToslibClient::do_request(const toslib_api::exportUnencryptedKey& request,
  ...
  promise.set_value(toslib_api::make_object<toslib_api::exportedUnencryptedKey>(std::move(exported_key.data)));
  return td::Status::OK();
}
td::Status ToslibClient::do_request(const toslib_api::importUnencryptedKey& request,
                                    td::Promise<object_ptr<toslib_api::key>>&& promise) {
...
td::Status ToslibClient::do_request(const toslib_api::changeLocalPassword& request,
                                    td::Promise<object_ptr<toslib_api::key>>&& promise) {
  if (!request.input_key_) {
    return ToslibError::EmptyField("input_key");
  }
  ...
  promise.set_value(toslib_api::make_object<toslib_api::key>(key.public_key.as_slice().str(), std::move(key.secret)));
  return td::Status::OK();
}

td::Status ToslibClient::do_request(const toslib_api::onLiteServerQueryResult& request,
                                    td::Promise<object_ptr<toslib_api::ok>>&& promise) {
...
td::Status ToslibClient::do_request(const toslib_api::onLiteServerQueryError& request,
                                    td::Promise<object_ptr<toslib_api::ok>>&& promise) {
  if (ext_client_outbound_.empty()) {
    return ToslibError::InvalidQueryId();
  }
  ...
  if (!request.error_) {
    return ToslibError::EmptyField("error");
  }
  send_closure(ext_client_outbound_, &ExtClientOutbound::on_query_result, request.id_ >> 16,
               td::Status::Error(request.error_->code_, request.error_->message_)
                   .move_as_error_prefix(ToslibError::LiteServerNetwork()),
               to_any_promise(std::move(promise)));
  return td::Status::OK();
}

toslib_api::object_ptr<toslib_api::Object> ToslibClient::do_static_request(toslib_api::setLogStream& request) {
  auto result = Logging::set_current_stream(std::move(request.log_stream_));
  if (result.is_ok()) {
    return toslib_api::make_object<toslib_api::ok>();
  } else {
    return toslib_api::make_object<toslib_api::error>(400, result.message().str());
  }
}
toslib_api::object_ptr<toslib_api::Object> ToslibClient::do_static_request(const toslib_api::getLogStream& request) {
  auto result = Logging::get_current_stream();
  if (result.is_ok()) {
    return result.move_as_ok();
  } else {
    return toslib_api::make_object<toslib_api::error>(400, result.error().message().str());
  }
}
toslib_api::object_ptr<toslib_api::Object> ToslibClient::do_static_request(
    const toslib_api::setLogVerbosityLevel& request) {
...
toslib_api::object_ptr<toslib_api::Object> ToslibClient::do_static_request(
    const toslib_api::setLogTagVerbosityLevel& request) {
...
toslib_api::object_ptr<toslib_api::Object> ToslibClient::do_static_request(
    const toslib_api::getLogVerbosityLevel& request) {
  return toslib_api::make_object<toslib_api::logVerbosityLevel>(Logging::get_verbosity_level());
}
toslib_api::object_ptr<toslib_api::Object> ToslibClient::do_static_request(
    const toslib_api::getLogTagVerbosityLevel& request) {
  auto result = Logging::get_tag_verbosity_level(request.tag_);
  if (result.is_ok()) {
    return toslib_api::make_object<toslib_api::logVerbosityLevel>(result.ok());
  } else {
    return toslib_api::make_object<toslib_api::error>(400, result.error().message().str());
  }
}
toslib_api::object_ptr<toslib_api::Object> ToslibClient::do_static_request(const toslib_api::getLogTags& request) {
  return toslib_api::make_object<toslib_api::logTags>(Logging::get_tags());
}
toslib_api::object_ptr<toslib_api::Object> ToslibClient::do_static_request(const toslib_api::addLogMessage& request) {
  Logging::add_message(request.verbosity_level_, request.text_);
  return toslib_api::make_object<toslib_api::ok>();
}

toslib_api::object_ptr<toslib_api::Object> ToslibClient::do_static_request(const toslib_api::encrypt& request) {
  return toslib_api::make_object<toslib_api::data>(
      SimpleEncryption::encrypt_data(request.decrypted_data_, request.secret_));
}

toslib_api::object_ptr<toslib_api::Object> ToslibClient::do_static_request(const toslib_api::decrypt& request) {
  auto r_data = SimpleEncryption::decrypt_data(request.encrypted_data_, request.secret_);
  if (r_data.is_ok()) {
    return toslib_api::make_object<toslib_api::data>(r_data.move_as_ok());
  } else {
    return status_to_toslib_api(r_data.error().move_as_error_prefix(ToslibError::KeyDecrypt()));
  }
}

toslib_api::object_ptr<toslib_api::Object> ToslibClient::do_static_request(const toslib_api::kdf& request) {
  auto max_iterations = 10000000;
  if (request.iterations_ < 0 || request.iterations_ > max_iterations) {
    return status_to_toslib_api(
        ToslibError::InvalidField("iterations", PSLICE() << "must be between 0 and " << max_iterations));
  }
  return toslib_api::make_object<toslib_api::data>(
...
toslib_api::object_ptr<toslib_api::Object> ToslibClient::do_static_request(
    const toslib_api::msg_decryptWithProof& request) {
  if (!request.data_) {
    return status_to_toslib_api(ToslibError::EmptyField("data"));
  }
  if (!request.data_->data_) {
    return status_to_toslib_api(ToslibError::EmptyField("data.data"));
  }
  if (!request.data_->source_) {
    return status_to_toslib_api(ToslibError::EmptyField("data.source"));
  }
  using ReturnType = toslib_api::object_ptr<toslib_api::msg_Data>;
  return downcast_call2<ReturnType>(
      *request.data_->data_,
      td::overloaded([&request](auto&) { return std::move(request.data_->data_); },
                     [&request](toslib_api::msg_dataEncryptedText& encrypted) -> ReturnType {
                       ...
                       auto decrypted = r_decrypted.move_as_ok();
                       return toslib_api::make_object<toslib_api::msg_dataDecryptedText>(decrypted.as_slice().str());
                     }));
}
...
td::Status ToslibClient::do_request(int_api::RemoteRunSmcMethod request,
                                    td::Promise<int_api::RemoteRunSmcMethod::ReturnType>&& promise) {
  auto actor_id = actor_id_++;
...
td::Status ToslibClient::do_request(int_api::GetPrivateKey request, td::Promise<KeyStorage::PrivateKey>&& promise) {
...
td::Status ToslibClient::do_request(int_api::SendMessage request, td::Promise<td::Unit>&& promise) {
  client_.send_query(tos::lite_api::liteServer_sendMessage(vm::std_boc_serialize(request.message).move_as_ok()),
                     to_any_promise(std::move(promise)));
  return td::Status::OK();
}

td::Status ToslibClient::do_request(const toslib_api::liteServer_getInfo& request,
                                    td::Promise<object_ptr<toslib_api::liteServer_info>>&& promise) {
  client_.send_query(tos::lite_api::liteServer_getVersion(), promise.wrap([](auto&& version) {
    return toslib_api::make_object<toslib_api::liteServer_info>(version->now_, version->version_,
...
auto to_bits256(td::Slice data, td::Slice name) -> td::Result<td::Bits256> {
  if (data.size() != 32) {
    return ToslibError::InvalidField(name, "wrong length (not 32 bytes)");
  }
  return td::Bits256(data.ubegin());
}

td::Status ToslibClient::do_request(toslib_api::withBlock& request,
                                    td::Promise<object_ptr<toslib_api::Object>>&& promise) {
  if (!request.id_) {
    return ToslibError::EmptyField("id");
  }
  if (!request.function_) {
    return ToslibError::EmptyField("function");
  }
  ...
  tos::BlockIdExt block_id(request.id_->workchain_, request.id_->shard_, request.id_->seqno_, root_hash, file_hash);
...
void ToslibClient::get_config_param(int32_t param, int32_t mode, tos::BlockIdExt block,
                                    td::Promise<object_ptr<toslib_api::configInfo>>&& promise) {
  std::vector<int32_t> params = {param};
  client_.send_query(
      tos::lite_api::liteServer_getConfigParams(mode, tos::create_tl_lite_block_id(block), std::move(params)),
      promise.wrap([param, block](auto r_config) -> td::Result<object_ptr<toslib_api::configInfo>> {
        auto state = block::check_extract_state_proof(block, r_config->state_proof_.as_slice(),
                                                      r_config->config_proof_.as_slice());
        if (state.is_error()) {
          return state.move_as_error_prefix(ToslibError::ValidateConfig());
        }
        auto config = block::Config::extract_from_state(std::move(state.move_as_ok()), 0);
        if (config.is_error()) {
          return config.move_as_error_prefix(ToslibError::ValidateConfig());
        }
        auto config_ptr = config.move_as_ok();
        toslib_api::configInfo config_result;
        TRY_RESULT_PREFIX_ASSIGN(config_result.config_, to_toslib_tvm_cell(config_ptr->get_config_param(param)),
                                 ToslibError::ValidateConfig());
        return toslib_api::make_object<toslib_api::configInfo>(std::move(config_result));
      }));
}

td::Status ToslibClient::do_request(const toslib_api::getConfigParam& request,
                                    td::Promise<object_ptr<toslib_api::configInfo>>&& promise) {
  if (query_context_.block_id) {
...
void ToslibClient::get_config_all(int32_t mode, tos::BlockIdExt block,
                                  td::Promise<object_ptr<toslib_api::configInfo>>&& promise) {
  client_.send_query(tos::lite_api::liteServer_getConfigAll(mode, tos::create_tl_lite_block_id(block)),
                     promise.wrap([block](auto r_config) -> td::Result<object_ptr<toslib_api::configInfo>> {
                       auto state = block::check_extract_state_proof(block, r_config->state_proof_.as_slice(),
                                                                     r_config->config_proof_.as_slice());
                       if (state.is_error()) {
                         return state.move_as_error_prefix(ToslibError::ValidateConfig());
                       ...
                       TRY_RESULT_PREFIX_ASSIGN(config_result.config_,
                                                to_toslib_tvm_cell(config.move_as_ok()->get_root_cell()),
                                                ToslibError::ValidateConfig());
                       return toslib_api::make_object<toslib_api::configInfo>(std::move(config_result));
                     }));
}

td::Status ToslibClient::do_request(const toslib_api::getConfigAll& request,
                                    td::Promise<object_ptr<toslib_api::configInfo>>&& promise) {
  if (query_context_.block_id) {
...
td::Status ToslibClient::do_request(const toslib_api::blocks_getShards& request,
  ...
  client_.send_query(tos::lite_api::liteServer_getAllShardsInfo(std::move(block)),
                     promise.wrap([req_blk_id](lite_api_ptr<tos::lite_api::liteServer_allShardsInfo>&& all_shards_info)
                                      -> td::Result<object_ptr<toslib_api::blocks_shards>> {
                       auto blk_id = tos::create_block_id(all_shards_info->id_);
                       ...
                       try {
                         ...
                         if (tos::RootHash{virt_root->get_hash().bits()} != blk_id.root_hash) {
                         ...
                         toslib_api::blocks_shards shards;
                         for (auto& id : ids) {
                           auto ref = sh_conf.get_shard_hash(tos::ShardIdFull(id));
                           if (ref.not_null()) {
                             shards.shards_.push_back(to_toslib_api(ref->top_block_id()));
                           }
                         }
                         return toslib_api::make_object<toslib_api::blocks_shards>(std::move(shards));
                       } catch (vm::VmError& err) {
...
td::Status ToslibClient::do_request(const toslib_api::blocks_lookupBlock& request,
  ...
  client_.with_last_block([self = this, blkid, lite_block = std::move(lite_block), mode = request.mode_,
    ...
    self->client_.send_query(
        tos::lite_api::liteServer_lookupBlockWithProof(
...
td::Status check_lookup_block_proof(lite_api_ptr<tos::lite_api::liteServer_lookupBlockResult>& result, int mode,
                                    ...
                                    td::uint32 utime) {
  try {
    tos::BlockIdExt cur_id = tos::create_block_id(result->mc_block_id_);
    ...
    try {
      for (auto& link : result->shard_links_) {
        ...
        if (R.is_error()) {
          return ToslibError::InvalidBagOfCells("proof");
        }
    ...
    if (header_r.is_error()) {
      return ToslibError::InvalidBagOfCells("header");
    }
    ...
    if (cur_id != tos::create_block_id(result->id_)) {
    ...
    if (mode & 1) {
      ...
      if (prev_header_r.is_error()) {
        return ToslibError::InvalidBagOfCells("prev_headers");
      }
      ...
      if (!tos::shard_contains(prev[prev_idx].id.shard, blkid.shard)) {
...
auto to_toslib_api(const tos::lite_api::liteServer_transactionId& txid)
    -> toslib_api_ptr<toslib_api::blocks_shortTxId> {
  return toslib_api::make_object<toslib_api::blocks_shortTxId>(txid.mode_, txid.account_.as_slice().str(), txid.lt_,
...
td::Status check_block_transactions_proof(lite_api_ptr<tos::lite_api::liteServer_blockTransactions>& bTxes,
                                          int32_t mode, tos::LogicalTime start_lt, td::Bits256 start_addr,
                                          td::Bits256 root_hash, int req_count) {
  if ((mode & tos::lite_api::liteServer_listBlockTransactions::WANT_PROOF_MASK) == 0) {
    return td::Status::OK();
  }
  constexpr int max_answer_transactions = 256;
  bool reverse_mode = mode & tos::lite_api::liteServer_listBlockTransactions::REVERSE_ORDER_MASK;
  ...
  try {
    ...
    bool eof = false;
    tos::LogicalTime reverse = reverse_mode ? ~0ULL : 0;
    tos::LogicalTime trans_lt = static_cast<tos::LogicalTime>(start_lt);
...
td::Status ToslibClient::do_request(const toslib_api::blocks_getTransactions& request,
  ...
  bool check_proof = request.mode_ & tos::lite_api::liteServer_listBlockTransactions::WANT_PROOF_MASK;
  bool reverse_mode = request.mode_ & tos::lite_api::liteServer_listBlockTransactions::REVERSE_ORDER_MASK;
  bool has_starting_tx = request.mode_ & tos::lite_api::liteServer_listBlockTransactions::AFTER_MASK;

  td::Bits256 start_addr;
  tos::LogicalTime start_lt;
  tos::lite_api::object_ptr<tos::lite_api::liteServer_transactionId3> after;
  if (has_starting_tx) {
    if (!request.after_) {
    ...
    after = tos::lite_api::make_object<tos::lite_api::liteServer_transactionId3>(start_addr, start_lt);
  } else {
  ...
  client_.send_query(
      ...
      promise.wrap([block_id, root_hash, req_count = request.count_, start_addr, start_lt,
        ...
        r.id_ = to_toslib_api(*bTxes->id_);
        r.req_count_ = bTxes->req_count_;
        r.incomplete_ = bTxes->incomplete_;
        for (auto& id : bTxes->ids_) {
          r.transactions_.push_back(to_toslib_api(*id));
        }
        return toslib_api::make_object<toslib_api::blocks_transactions>(std::move(r));
      }));
  return td::Status::OK();
}

td::Status ToslibClient::do_request(const toslib_api::blocks_getTransactionsExt& request,
                                    td::Promise<object_ptr<toslib_api::blocks_transactionsExt>>&& promise) {
  if (!request.id_) {
    return ToslibError::EmptyField("id");
  }
  TRY_RESULT(block, to_lite_api(*request.id_))
  bool check_proof = request.mode_ & tos::lite_api::liteServer_listBlockTransactionsExt::WANT_PROOF_MASK;
  bool reverse_mode = request.mode_ & tos::lite_api::liteServer_listBlockTransactionsExt::REVERSE_ORDER_MASK;
  bool has_starting_tx = request.mode_ & tos::lite_api::liteServer_listBlockTransactionsExt::AFTER_MASK;
  ...
  auto block_id = tos::create_block_id(block);
  client_.send_query(
      tos::lite_api::liteServer_listBlockTransactionsExt(std::move(block), request.mode_, request.count_,
                                                         std::move(after), reverse_mode, check_proof),
      promise.wrap([block_id, check_proof, reverse_mode, start_addr, start_lt,
                    req_count = request.count_](lite_api_ptr<tos::lite_api::liteServer_blockTransactionsExt>&& bTxes)
                       -> td::Result<toslib_api::object_ptr<toslib_api::blocks_transactionsExt>> {
        ...
        if (raw_transactions.is_error()) {
          return raw_transactions.move_as_error_prefix("Error occured while creating toslib_api::raw_transaction: ");
        }

        toslib_api::blocks_transactionsExt r;
        r.id_ = to_toslib_api(*bTxes->id_);
        r.req_count_ = bTxes->req_count_;
        r.incomplete_ = bTxes->incomplete_;
        r.transactions_ = raw_transactions.move_as_ok();
        return toslib_api::make_object<toslib_api::blocks_transactionsExt>(std::move(r));
      }));
  return td::Status::OK();
}

td::Status ToslibClient::do_request(const toslib_api::blocks_getBlockHeader& request,
  ...
  TRY_RESULT(req_blk_id, to_block_id(*request.id_));
  client_.send_query(tos::lite_api::liteServer_getBlockHeader(std::move(lite_block), 0xffff),
                     promise.wrap([req_blk_id](lite_api_ptr<tos::lite_api::liteServer_blockHeader>&& hdr)
                                      -> td::Result<toslib_api::object_ptr<toslib_api::blocks_header>> {
                         ...
                         try {
                           ...
                           if (res.is_error()) {
                             ...
                             if (!(tlb::unpack_cell(virt_root, blk) && tlb::unpack_cell(blk.info, info))) {
                               ...
                               toslib_api::blocks_header header;
                               header.id_ = to_toslib_api(blk_id);
                               ...
                               for (auto& id : prev) {
                                 header.prev_blocks_.push_back(to_toslib_api(id));
                               }
                               return toslib_api::make_object<toslib_api::blocks_header>(std::move(header));
                             }
                           }
                         } catch (vm::VmError& err) {
...
td::Status ToslibClient::do_request(const toslib_api::blocks_getMasterchainBlockSignatures& request,
...
td::Status ToslibClient::do_request(const toslib_api::blocks_getShardBlockProof& request,
  ...
  if (request.mode_ & 1) {
    if (!request.from_) {
      return ToslibError::EmptyField("from");
    }
...
td::Status ToslibClient::do_request(const toslib_api::blocks_getOutMsgQueueSizes& request,
                                    td::Promise<object_ptr<toslib_api::blocks_outMsgQueueSizes>>&& promise) {
  auto req_mode = request.mode_;
  auto req_shard = tos::ShardIdFull{request.wc_, (tos::ShardId)request.shard_};
  ...
  client_.with_last_block(
      [=, self = this, promise = std::move(promise)](td::Result<LastBlockState> r_last_block) mutable {
        ...
        td::Promise P = [=, mc_blkid = last_block.last_block_id,
                         promise = std::move(promise)](td::Result<object_ptr<toslib_api::blocks_shards>> R) mutable {
          TRY_RESULT_PROMISE_PREFIX(promise, shards, std::move(R), "get shards failed: ");
          std::vector<tos::BlockIdExt> blocks;
          if (!(req_mode & 1) || tos::shard_intersects(mc_blkid.shard_full(), req_shard)) {
            blocks.push_back(mc_blkid);
          }
          for (const auto& shard : shards->shards_) {
            TRY_RESULT_PROMISE(promise, block_id, to_block_id(*shard));
            if (!(req_mode & 1) || tos::shard_intersects(block_id.shard_full(), req_shard)) {
        ...
        self->do_request(toslib_api::blocks_getShards(to_toslib_api(last_block.last_block_id)), std::move(P));
      });
  return td::Status::OK();
}

void ToslibClient::load_libs_from_disk() {
  LOG(DEBUG) << "loading libraries from disk cache";
  auto r_data = kv_->get("toslib.libcache");
  if (r_data.is_error()) {
    return;
  }
...
void ToslibClient::store_libs_to_disk() {  // NB: Dictionary.get_root_cell does not compute_root, and it is protected
  kv_->set("toslib.libcache", vm::std_boc_serialize(vm::CellBuilder().append_cellslice(libraries.get_root()).finalize())
                                  .move_as_ok()
                                  .as_slice());
...
td::Status ToslibClient::do_request(const int_api::ScanAndLoadGlobalLibs& request,
  ...
  client_.send_query(
      tos::lite_api::liteServer_getLibraries(std::move(to_load_list)),
...
template <class P>
td::Status ToslibClient::do_request(const toslib_api::runTests& request, P&&) {
  UNREACHABLE();
  return ToslibError::Internal();
}
template <class P>
td::Status ToslibClient::do_request(const toslib_api::getAccountAddress& request, P&&) {
  UNREACHABLE();
  return ToslibError::Internal();
}
template <class P>
td::Status ToslibClient::do_request(const toslib_api::packAccountAddress& request, P&&) {
  UNREACHABLE();
  return ToslibError::Internal();
}
template <class P>
td::Status ToslibClient::do_request(const toslib_api::unpackAccountAddress& request, P&&) {
  UNREACHABLE();
  return ToslibError::Internal();
}
template <class P>
td::Status ToslibClient::do_request(toslib_api::getBip39Hints& request, P&&) {
  UNREACHABLE();
  return ToslibError::Internal();
}
template <class P>
td::Status ToslibClient::do_request(toslib_api::setLogStream& request, P&&) {
  UNREACHABLE();
  return ToslibError::Internal();
}
template <class P>
td::Status ToslibClient::do_request(const toslib_api::getLogStream& request, P&&) {
  UNREACHABLE();
  return ToslibError::Internal();
}
template <class P>
td::Status ToslibClient::do_request(const toslib_api::setLogVerbosityLevel& request, P&&) {
  UNREACHABLE();
  return ToslibError::Internal();
}
template <class P>
td::Status ToslibClient::do_request(const toslib_api::setLogTagVerbosityLevel& request, P&&) {
  UNREACHABLE();
  return ToslibError::Internal();
}
template <class P>
td::Status ToslibClient::do_request(const toslib_api::getLogVerbosityLevel& request, P&&) {
  UNREACHABLE();
  return ToslibError::Internal();
}
template <class P>
td::Status ToslibClient::do_request(const toslib_api::getLogTagVerbosityLevel& request, P&&) {
  UNREACHABLE();
  return ToslibError::Internal();
}
template <class P>
td::Status ToslibClient::do_request(const toslib_api::getLogTags& request, P&&) {
  UNREACHABLE();
  return ToslibError::Internal();
}
template <class P>
td::Status ToslibClient::do_request(const toslib_api::addLogMessage& request, P&&) {
  UNREACHABLE();
  return ToslibError::Internal();
}
template <class P>
td::Status ToslibClient::do_request(const toslib_api::encrypt& request, P&&) {
  UNREACHABLE();
  return ToslibError::Internal();
}
template <class P>
td::Status ToslibClient::do_request(const toslib_api::decrypt& request, P&&) {
  UNREACHABLE();
  return ToslibError::Internal();
}
template <class P>
td::Status ToslibClient::do_request(const toslib_api::kdf& request, P&&) {
  UNREACHABLE();
  return ToslibError::Internal();
}
template <class P>
td::Status ToslibClient::do_request(const toslib_api::msg_decryptWithProof& request, P&&) {
  UNREACHABLE();
  return ToslibError::Internal();
}
}  // namespace toslib
