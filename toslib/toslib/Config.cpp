...
namespace toslib {
td::Result<tos::BlockIdExt> parse_block_id_ext(td::JsonObject &obj) {
  tos::WorkchainId zero_workchain_id;
  {
    TRY_RESULT(wc, obj.get_required_int_field("workchain"));
    zero_workchain_id = wc;
  }
  tos::ShardId zero_shard_id;  // uint64
  {
    TRY_RESULT(shard_id, obj.get_optional_long_field("shard"));
    zero_shard_id = static_cast<tos::ShardId>(shard_id);
  }
  tos::BlockSeqno zero_seqno;
  {
    TRY_RESULT(seqno, obj.get_required_int_field("seqno"));
    zero_seqno = seqno;
  }

  tos::RootHash zero_root_hash;
  {
    TRY_RESULT(hash_b64, obj.get_required_string_field("root_hash"));
    TRY_RESULT(hash, td::base64_decode(hash_b64));
    if (hash.size() * 8 != tos::RootHash::size()) {
      return td::Status::Error("Invalid config (8)");
    }
    zero_root_hash = tos::RootHash(td::ConstBitPtr(td::Slice(hash).ubegin()));
  }
  tos::FileHash zero_file_hash;
  {
    TRY_RESULT(hash_b64, obj.get_required_string_field("file_hash"));
    TRY_RESULT(hash, td::base64_decode(hash_b64));
    if (hash.size() * 8 != tos::FileHash::size()) {
      return td::Status::Error("Invalid config (9)");
    }
    zero_file_hash = tos::RootHash(td::ConstBitPtr(td::Slice(hash).ubegin()));
  }

  return tos::BlockIdExt(zero_workchain_id, zero_shard_id, zero_seqno, std::move(zero_root_hash),
...
td::Result<Config> Config::parse(std::string str) {
  ...
  Config res;
  tos::tos_api::liteclient_config_global conf;
  TRY_STATUS(tos::tos_api::from_json(conf, json.get_object()));
  ...
  res.zero_state_id = tos::create_block_id(conf.validator_->zero_state_);
  if (conf.validator_->init_block_) {
    res.init_block_id = tos::create_block_id(conf.validator_->init_block_);
  }

  for (auto &fork : conf.validator_->hardforks_) {
    res.hardforks.push_back(tos::create_block_id(fork));
  }
