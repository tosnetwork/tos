...
namespace toslib {
ExtClient::~ExtClient() {
  last_config_queries_.for_each([](auto id, auto &promise) { promise.set_error(ToslibError::Cancelled()); });
  last_block_queries_.for_each([](auto id, auto &promise) { promise.set_error(ToslibError::Cancelled()); });
  queries_.for_each([](auto id, auto &promise) { promise.set_error(ToslibError::Cancelled()); });
}
