...
static const std::map<td::Slice, int *> log_tags{ADD_TAG(toslib_query), ADD_TAG(last_block), ADD_TAG(last_config),
                                                 ADD_TAG(lite_server)};
#undef ADD_TAG

td::Status Logging::set_current_stream(toslib_api::object_ptr<toslib_api::LogStream> stream) {
  if (stream == nullptr) {
  ...
  switch (stream->get_id()) {
    case toslib_api::logStreamDefault::ID:
      td::log_interface = td::default_log_interface;
      return td::Status::OK();
    case toslib_api::logStreamFile::ID: {
      auto file_stream = toslib_api::move_object_as<toslib_api::logStreamFile>(stream);
    ...
    case toslib_api::logStreamEmpty::ID:
...
td::Result<toslib_api::object_ptr<toslib_api::LogStream>> Logging::get_current_stream() {
  std::lock_guard<std::mutex> lock(log_data().logging_mutex);
  if (td::log_interface == td::default_log_interface) {
    return toslib_api::make_object<toslib_api::logStreamDefault>();
  }
  if (td::log_interface == &log_data().null_log) {
    return toslib_api::make_object<toslib_api::logStreamEmpty>();
  }
  if (td::log_interface == &log_data().ts_log) {
    return toslib_api::make_object<toslib_api::logStreamFile>(log_data().file_log.get_path().str(),
