  ...
  while (true) {
    ...
    if (response.object) {
      ...
      if (response.object->get_id() == toslib_api::error::ID) {
        auto error = toslib_api::move_object_as<toslib_api::error>(response.object);
        return td::Result<ReturnTypePtr>(td::Status::Error(error->code_, error->message_));
      }
      return td::Result<ReturnTypePtr>(toslib_api::move_object_as<ReturnType>(response.object));
    }
  }
};
...
TEST(Toslib, InitClose) {
  ...
  auto dir = [](auto str) { return make_object<toslib_api::keyStoreTypeDirectory>(str); };
  {
    Client client;
    sync_send(client, make_object<toslib_api::close>()).ensure();
    sync_send(client, make_object<toslib_api::init>(make_object<toslib_api::options>(nullptr, dir("."))))
        .ensure_error();
  }
  {
    Client client;
    sync_send(client, make_object<toslib_api::init>(nullptr)).ensure_error();
    sync_send(client, make_object<toslib_api::init>(make_object<toslib_api::options>(cfg("fdajkfldsjkafld"), dir("."))))
        .ensure_error();
    sync_send(client, make_object<toslib_api::init>(make_object<toslib_api::options>(nullptr, dir("fdhskfds"))))
        .ensure_error();
    sync_send(client, make_object<toslib_api::init>(make_object<toslib_api::options>(nullptr, dir(".")))).ensure();
    sync_send(client, make_object<toslib_api::init>(make_object<toslib_api::options>(nullptr, dir("."))))
        .ensure_error();
    ...
    td::Slice bad_config = R"abc(
{
  ...
  "liteclients": [ ]
    ...
    sync_send(client, make_object<toslib_api::options_setConfig>(cfg(bad_config.str()))).ensure_error();
    auto address = sync_send(client, make_object<toslib_api::getAccountAddress>(
                                         make_object<toslib_api::wallet_v3_initialAccountState>(
                                             "Pubjns2gp7DGCnEH7EOWeCnb6Lw1akm538YYaz6sdLVHfRB2", 123),
                                         0, -1))
                       .move_as_ok();
    sync_send(client, make_object<toslib_api::getAccountState>(std::move(address))).ensure_error();
    sync_send(client, make_object<toslib_api::close>()).ensure();
    sync_send(client, make_object<toslib_api::close>()).ensure_error();
    sync_send(client, make_object<toslib_api::init>(make_object<toslib_api::options>(nullptr, dir("."))))
        .ensure_error();
  }
}
...
td::Slice to_data(const toslib::SimpleEncryptionV2::Decrypted &str) {
...
TEST(Toslib, SimpleEncryption) {
  test_encryption<SimpleEncryption>();
}

TEST(Toslib, SimpleEncryptionV2) {
  test_encryption<SimpleEncryptionV2>();
}

TEST(Toslib, SimpleEncryptionAsym) {
...
TEST(Toslib, ParseAddres) {
  using toslib_api::make_object;
  Client client;

  // init
  sync_send(client, make_object<toslib_api::init>(
                        make_object<toslib_api::options>(nullptr, make_object<toslib_api::keyStoreTypeDirectory>("."))))
      .ensure();

  sync_send(client, make_object<toslib_api::unpackAccountAddress>("hello")).ensure_error();
  auto addr =
      sync_send(client,
                make_object<toslib_api::unpackAccountAddress>("Ef9Tj6fMJP-OqhAdhKXxq36DL-HYSzCc3-9O6UNzqsgPfYFX"))
          .move_as_ok();
  ...
  auto addr_str = sync_send(client, make_object<toslib_api::packAccountAddress>(std::move(addr))).move_as_ok();
  ASSERT_EQ("Ef9Tj6fMJP-OqhAdhKXxq36DL-HYSzCc3-9O6UNzqsgPfYFX", addr_str->account_address_);
  auto addr_str2 = sync_send(client, make_object<toslib_api::packAccountAddress>(
                                         make_object<toslib_api::unpackedAccountAddress>(-1, false, false, raw)))
                       .move_as_ok();
  ASSERT_EQ("Uf9Tj6fMJP-OqhAdhKXxq36DL-HYSzCc3-9O6UNzqsgPfdyS", addr_str2->account_address_);
}

TEST(Toslib, ConfigParseBug) {
  td::Slice literal =
...
TEST(Toslib, EncryptionApi) {
  ...
  auto key = std::move(
      sync_send(client, make_object<toslib_api::kdf>(td::SecureString(password), td::SecureString("salt"), 100000))
          .move_as_ok()
          ->bytes_);
  auto encrypted = std::move(
      sync_send(client, make_object<toslib_api::encrypt>(td::SecureString(data), key.copy())).move_as_ok()->bytes_);
  auto decrypted =
      std::move(sync_send(client, make_object<toslib_api::decrypt>(encrypted.copy(), key.copy())).move_as_ok()->bytes_);
  ASSERT_EQ(data, decrypted);

  auto bad_key = std::move(sync_send(client, make_object<toslib_api::kdf>(td::SecureString(password + "BAD"),
                                                                          td::SecureString("salt"), 100000))
                               .move_as_ok()
                               ->bytes_);
  sync_send(client, make_object<toslib_api::decrypt>(encrypted.copy(), bad_key.copy())).ensure_error();
}
...
TEST(Toslib, KeysApi) {
  ...
  // init
  sync_send(client, make_object<toslib_api::init>(make_object<toslib_api::options>(
                        nullptr, make_object<toslib_api::keyStoreTypeDirectory>("testdir"))))
      .ensure();
  auto local_password = td::SecureString("local password");
  auto mnemonic_password = td::SecureString("mnemonic password");
  {
    auto key = sync_send(client, make_object<toslib_api::createNewKey>(local_password.copy(), td::SecureString{},
                                                                       td::SecureString{}))
                   .move_as_ok();
  }

  //createNewKey local_password:bytes mnemonic_password:bytes = Key;
  auto key = sync_send(client, make_object<toslib_api::createNewKey>(local_password.copy(), mnemonic_password.copy(),
                                                                     td::SecureString{}))
                 .move_as_ok();

  sync_send(client, make_object<toslib_api::exportKey>(make_object<toslib_api::inputKeyRegular>(
                        make_object<toslib_api::key>(key->public_key_, key->secret_.copy()),
  ...
  auto exported_key =
      sync_send(client,
                make_object<toslib_api::exportKey>(make_object<toslib_api::inputKeyRegular>(
                    make_object<toslib_api::key>(key->public_key_, key->secret_.copy()), local_password.copy())))
          .move_as_ok();
  ...
  auto new_key =
      sync_send(client,
                make_object<toslib_api::changeLocalPassword>(
  ...
  sync_send(client,
            make_object<toslib_api::exportKey>(make_object<toslib_api::inputKeyRegular>(
                make_object<toslib_api::key>(key->public_key_, new_key->secret_.copy()), local_password.copy())))
      .ensure_error();

  auto exported_key2 = sync_send(client, make_object<toslib_api::exportKey>(make_object<toslib_api::inputKeyRegular>(
                                             make_object<toslib_api::key>(key->public_key_, new_key->secret_.copy()),
                                             td::SecureString("tmp local password"))))
                           .move_as_ok();
  ...
  //sync_send(client, make_object<toslib_api::importKey>(new_local_password.copy(), mnemonic_password.copy(),
  //make_object<toslib_api::exportedKey>(copy_word_list())))
  //.ensure_error();

  {
    ...
    auto exported_encrypted_key =
        sync_send(client, make_object<toslib_api::exportEncryptedKey>(
                              make_object<toslib_api::inputKeyRegular>(
                                  make_object<toslib_api::key>(key->public_key_, new_key->secret_.copy()),
                                  td::SecureString("tmp local password")),
    ...
    sync_send(client,
              make_object<toslib_api::deleteKey>(make_object<toslib_api::key>(key->public_key_, key->secret_.copy())))
        .move_as_ok();

    sync_send(client, make_object<toslib_api::importEncryptedKey>(
                          new_local_password.copy(), wrong_export_password.copy(),
                          make_object<toslib_api::exportedEncryptedKey>(exported_encrypted_key->data_.copy())))
        .ensure_error();
  ...
  auto err1 = sync_send(
      client, make_object<toslib_api::importKey>(new_local_password.copy(), td::SecureString("wrong password"),
                                                 make_object<toslib_api::exportedKey>(copy_word_list())));
  if (err1.is_ok()) {
  ...
  err1.ensure_error();
  auto err2 =
      sync_send(client, make_object<toslib_api::importKey>(new_local_password.copy(), td::SecureString(),
                                                           make_object<toslib_api::exportedKey>(copy_word_list())));
  err2.ensure_error();
  ...
  auto r_exported_pem_key = sync_send(
      client,
      make_object<toslib_api::exportPemKey>(
          make_object<toslib_api::inputKeyRegular>(
              make_object<toslib_api::key>(key->public_key_, imported_key->secret_.copy()), new_local_password.copy()),
          pem_password.copy()));
  ...
  //sync_send(client, make_object<toslib_api::importPemKey>(
  //new_local_password.copy(), pem_password.copy(),
  //make_object<toslib_api::exportedPemKey>(exported_pem_key->pem_.copy())))
  //.ensure_error();
  sync_send(client, make_object<toslib_api::deleteKey>(
                        make_object<toslib_api::key>(imported_key->public_key_, imported_key->secret_.copy())))
      .move_as_ok();
  sync_send(client, make_object<toslib_api::importPemKey>(
  ...
  auto new_imported_key = sync_send(client, make_object<toslib_api::importPemKey>(
                                                new_local_password.copy(), pem_password.copy(),
                                                make_object<toslib_api::exportedPemKey>(exported_pem_key->pem_.copy())))
                              .move_as_ok();
  ...
  auto exported_raw_key =
      sync_send(client, make_object<toslib_api::exportUnencryptedKey>(make_object<toslib_api::inputKeyRegular>(
                            make_object<toslib_api::key>(key->public_key_, new_imported_key->secret_.copy()),
                            new_local_password.copy())))
          .move_as_ok();
  sync_send(client, make_object<toslib_api::deleteKey>(
                        make_object<toslib_api::key>(new_imported_key->public_key_, new_imported_key->secret_.copy())))
      .move_as_ok();
  td::Ed25519::PrivateKey pkey(exported_raw_key->data_.copy());
  auto raw_imported_key = sync_send(client, make_object<toslib_api::importUnencryptedKey>(new_local_password.copy(),
  ...
  std::vector<toslib_api::object_ptr<toslib_api::msg_dataEncrypted>> elements;
  ...
  elements.push_back(make_object<toslib_api::msg_dataEncrypted>(
      make_object<toslib_api::accountAddress>(addr.str()), make_object<toslib_api::msg_dataEncryptedText>(encrypted)));

  auto decrypted =
      sync_send(client, make_object<toslib_api::msg_decrypt>(
                            make_object<toslib_api::inputKeyRegular>(
                                make_object<toslib_api::key>(key->public_key_, raw_imported_key->secret_.copy()),
                                new_local_password.copy()),
                            make_object<toslib_api::msg_dataEncryptedArray>(std::move(elements))))
          .move_as_ok();
  ...
  auto decrypted2 = sync_send(client, make_object<toslib_api::msg_decryptWithProof>(
                                          proof, make_object<toslib_api::msg_dataEncrypted>(
                                                     make_object<toslib_api::accountAddress>(addr.str()),
                                                     make_object<toslib_api::msg_dataEncryptedText>(encrypted))))
                        .move_as_ok();
...
TEST(Toslib, ConfigCache) {
  ...
  sync_send(client,
            make_object<toslib_api::options_validateConfig>(make_object<toslib_api::config>(bad, "", true, false)))
      .ensure_error();

  sync_send(client,
            make_object<toslib_api::options_validateConfig>(make_object<toslib_api::config>(mainnet, "", true, false)))
      .ensure();
  sync_send(client,
            make_object<toslib_api::options_validateConfig>(make_object<toslib_api::config>(testnet, "", true, false)))
      .ensure();
  sync_send(client,
            make_object<toslib_api::options_validateConfig>(make_object<toslib_api::config>(custom, "", true, false)))
      .ensure();

  sync_send(client, make_object<toslib_api::options_validateConfig>(
                        make_object<toslib_api::config>(mainnet, "testnet", true, false)))
      .ensure_error();

  sync_send(client, make_object<toslib_api::options_setConfig>(
                        make_object<toslib_api::config>(testnet, "testnet", true, false)))
      .ensure();
  sync_send(client,
            make_object<toslib_api::options_setConfig>(make_object<toslib_api::config>(custom, "testnet", true, false)))
      .ensure_error();
}
