...
struct Key {
  ...
  toslib_api::object_ptr<toslib_api::InputKey> get_input_key() const {
    return toslib_api::make_object<toslib_api::inputKeyRegular>(
        toslib_api::make_object<toslib_api::key>(public_key, secret.copy()), td::SecureString("local"));
  }
  toslib_api::object_ptr<toslib_api::InputKey> get_fake_input_key() const {
    return toslib_api::make_object<toslib_api::inputKeyFake>();
  }
};
struct Wallet {
  std::string address;
  ...
  auto get_address() const {
    return toslib_api::make_object<toslib_api::accountAddress>(address);
  }
};
...
struct AccountState {
  ...
  std::string address;
  toslib_api::object_ptr<toslib_api::fullAccountState> state;
...
void sync(Client& client) {
  sync_send(client, make_object<toslib_api::sync>()).ensure();
}

static td::uint32 default_wallet_id{0};
std::string wallet_address(Client& client, const Key& key) {
  return sync_send(client,
                   make_object<toslib_api::getAccountAddress>(
                       make_object<toslib_api::wallet_v3_initialAccountState>(key.public_key, default_wallet_id), 0, 0))
      .move_as_ok()
      ->account_address_;
}

std::string highload_wallet_address(Client& client, const Key& key) {
  return sync_send(client, make_object<toslib_api::getAccountAddress>(
                               make_object<toslib_api::wallet_highload_v2_initialAccountState>(key.public_key,
                                                                                               default_wallet_id),
...
Wallet import_wallet_from_pkey(Client& client, std::string pkey, std::string password) {
  auto key = sync_send(client, make_object<toslib_api::importPemKey>(
                                   td::SecureString("local"), td::SecureString(password),
                                   make_object<toslib_api::exportedPemKey>(td::SecureString(pkey))))
                 .move_as_ok();
...
AccountState get_account_state(Client& client, std::string address) {
  auto state = sync_send(client, toslib_api::make_object<toslib_api::getAccountState>(
                                     toslib_api::make_object<toslib_api::accountAddress>(address)))
                   .move_as_ok();
  AccountState res;
  ...
  switch (state->account_state_->get_id()) {
    case toslib_api::uninited_accountState::ID:
      res.type = AccountState::Empty;
      break;
    case toslib_api::wallet_v3_accountState::ID:
      res.type = AccountState::Wallet;
      break;
    case toslib_api::dns_accountState::ID:
      res.type = AccountState::Dns;
      break;
    case toslib_api::pchan_accountState::ID:
...
td::Result<QueryId> create_send_grams_query(Client& client, const Wallet& source, std::string destination,
                                            ...
                                            bool fake = false) {
  std::vector<toslib_api::object_ptr<toslib_api::msg_message>> msgs;
  toslib_api::object_ptr<toslib_api::msg_Data> data;
  if (message.text) {
    if (message.encrypted) {
      data = toslib_api::make_object<toslib_api::msg_dataDecryptedText>(message.text.unwrap());
    } else {
      data = toslib_api::make_object<toslib_api::msg_dataText>(message.text.unwrap());
    }
  } else {
    data = toslib_api::make_object<toslib_api::msg_dataRaw>(message.raw.unwrap(), message.init_state.unwrap());
  }
  msgs.push_back(toslib_api::make_object<toslib_api::msg_message>(
      toslib_api::make_object<toslib_api::accountAddress>(destination), "", amount,
      std::vector<toslib_api::object_ptr<toslib_api::extraCurrency>>{}, std::move(data), -1));

  auto r_id =
      sync_send(client, toslib_api::make_object<toslib_api::createQuery>(
                            fake ? source.key.get_fake_input_key() : source.key.get_input_key(), source.get_address(),
                            timeout, toslib_api::make_object<toslib_api::actionMsg>(std::move(msgs), force), nullptr));
  TRY_RESULT(id, std::move(r_id));
  return QueryId{id->id_};
}

td::Result<QueryId> create_update_dns_query(Client& client, const Wallet& dns,
                                            std::vector<toslib_api::object_ptr<toslib_api::dns_Action>> entries) {
...
td::Result<QueryId> create_raw_query(Client& client, std::string source, std::string init_code, std::string init_data,
                                     std::string body) {
  auto r_id =
      sync_send(client, toslib_api::make_object<toslib_api::raw_createQuery>(
                            toslib_api::make_object<toslib_api::accountAddress>(source), init_code, init_data, body));
  TRY_RESULT(id, std::move(r_id));
  return QueryId{id->id_};
}

std::pair<Fee, Fee> query_estimate_fees(Client& client, QueryId query_id, bool ignore_chksig = false) {
  auto fees = sync_send(client, toslib_api::make_object<toslib_api::query_estimateFees>(query_id.id, ignore_chksig))
                  .move_as_ok();
  Fee second;
...
void query_send(Client& client, QueryId query_id) {
  sync_send(client, toslib_api::make_object<toslib_api::query_send>(query_id.id)).ensure();
}
QueryInfo query_get_info(Client& client, QueryId query_id) {
  auto info = sync_send(client, toslib_api::make_object<toslib_api::query_getInfo>(query_id.id)).move_as_ok();
...
td::Result<toslib_api::object_ptr<toslib_api::raw_transactions>> get_transactions(Client& client,
                                                                                  td::optional<const Wallet*> wallet,
                                                                                  std::string address,
                                                                                  const TransactionId& from) {
  auto got_transactions = sync_send(client, make_object<toslib_api::raw_getTransactions>(
                                                wallet ? wallet.value()->key.get_input_key() : nullptr,
                                                make_object<toslib_api::accountAddress>(address),
                                                make_object<toslib_api::internal_transactionId>(from.lt, from.hash)))
                              .move_as_ok();
  return std::move(got_transactions);
}

std::string read_text(toslib_api::msg_Data& data) {
  std::string text;
  downcast_call(data, td::overloaded([](auto& other) {}, [&](toslib_api::msg_dataText& data) { text = data.text_; },
                                     [&](toslib_api::msg_dataDecryptedText& data) { text = data.text_; }));
  return text;
}
...
Wallet create_empty_wallet(Client& client) {
  using toslib_api::make_object;
  auto key = sync_send(client, make_object<toslib_api::createNewKey>(td::SecureString("local"), td::SecureString(),
                                                                     td::SecureString()))
                 .move_as_ok();
  Wallet wallet{"", {key->public_key_, std::move(key->secret_)}};
  ...
  auto account_address =
      sync_send(
          client,
          make_object<toslib_api::getAccountAddress>(
              make_object<toslib_api::wallet_v3_initialAccountState>(wallet.key.public_key, default_wallet_id), 0, 0))
          .move_as_ok();
...
Wallet create_empty_dns(Client& client) {
  ...
  auto account_address =
      sync_send(client,
                make_object<toslib_api::getAccountAddress>(
                    make_object<toslib_api::dns_initialAccountState>(dns.key.public_key, default_wallet_id), 0, 0))
          .move_as_ok();
...
void test_multisig(Client& client, const Wallet& giver_wallet) {
  ...
  auto ms = tos::MultisigWallet::create();
...
void dns_resolve(Client& client, const Wallet& dns, std::string name) {
  ...
  auto resolved = sync_send(client, make_object<::tos::toslib_api::dns_resolve>(
...
void test_paychan(Client& client, const Wallet& giver_wallet) {
  ...
  auto account_address = sync_send(client, make_object<toslib_api::getAccountAddress>(get_initial_state(), -1, 0))
                             .move_as_ok()
                             ->account_address_;
  ...
  auto create_init_query = [&](auto& wallet, td::int64 inc_A, td::int64 inc_B) {
    auto action = make_object<toslib_api::actionPchan>(make_object<toslib_api::pchan_actionInit>(inc_A, inc_B, 0, 0));
  ...
  while (true) {
    ...
    auto pchan_state = toslib_api::move_object_as<toslib_api::pchan_accountState>(state.state->account_state_);
    if (pchan_state->state_->get_id() != toslib_api::pchan_stateClose::ID) {
      continue;
    }
  ...
  auto create_close_query = [&](auto& x_wallet, auto& y_wallet, td::int64 A, td::int64 B) {
    auto p = sync_send(client, make_object<pchan_signPromise>(y_wallet.key.get_input_key(),
                 ...
                 .move_as_ok();
    auto action = make_object<toslib_api::actionPchan>(make_object<toslib_api::pchan_actionClose>(0, 0, std::move(p)));
  ...
  while (true) {
    ...
    auto pchan_state = toslib_api::move_object_as<toslib_api::pchan_accountState>(state.state->account_state_);
    if (pchan_state->state_->get_id() != toslib_api::pchan_statePayout::ID) {
      continue;
    }
    LOG(INFO) << "Account type: " << state.type << "  " << state.sync_utime << "\n" << to_string(pchan_state->state_);
    auto payout = toslib_api::move_object_as<toslib_api::pchan_statePayout>(pchan_state->state_);
    payout_A = payout->A_;
    payout_B = payout->B_;
    break;
  }
...
void test_dns(Client& client, const Wallet& giver_wallet) {
  ...
  actions.push_back(make_object<dns_actionSet>(make_object<dns_entry>(
      "A", tos::DNS_NEXT_RESOLVER_CATEGORY, make_object<dns_entryDataNextResolver>(A_B.get_address()))));
  auto init_A = create_update_dns_query(client, A, std::move(actions)).move_as_ok();
  actions.push_back(make_object<dns_actionSet>(make_object<dns_entry>(
      "B", tos::DNS_NEXT_RESOLVER_CATEGORY, make_object<dns_entryDataNextResolver>(A_B_C.get_address()))));
...
int main(int argc, char* argv[]) {
  ...
  static_send(make_object<toslib_api::setLogTagVerbosityLevel>("toslib_query", 4)).ensure();
  auto tags = static_send(make_object<toslib_api::getLogTags>()).move_as_ok()->tags_;
  for (auto& tag : tags) {
    static_send(make_object<toslib_api::setLogTagVerbosityLevel>(tag, 4)).ensure();
  }

  Client client;
  {
    auto info = sync_send(client, make_object<toslib_api::init>(make_object<toslib_api::options>(
                                      make_object<toslib_api::config>(global_config_str, "", false, false),
                                      make_object<toslib_api::keyStoreTypeDirectory>(keystore_dir))))
                    .move_as_ok();
