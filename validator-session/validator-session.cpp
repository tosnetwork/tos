...
void ValidatorSessionImpl::process_blocks(std::vector<catchain::CatChainBlock *> blocks) {
  ...
  std::vector<tl_object_ptr<tos_api::validatorSession_round_Message>> msgs;
  ...
  if (generated_ && !sent_generated_) {
    ...
    msgs.emplace_back(create_tl_object<tos_api::validatorSession_message_submittedBlock>(
  ...
  for (auto &block : to_approve) {
    ...
    if (approved_.contains(id) && approved_[id].first <= td::Clocks::system()) {
      msgs.emplace_back(create_tl_object<tos_api::validatorSession_message_approvedBlock>(
  ...
  for (auto &B : pending_reject_) {
    msgs.emplace_back(
        create_tl_object<tos_api::validatorSession_message_rejectedBlock>(cur_round_, B.first, std::move(B.second)));
  }
  ...
  if (signed_) {
    ...
    if (found) {
      ...
      msgs.emplace_back(
          create_tl_object<tos_api::validatorSession_message_commit>(cur_round_, signed_block_, std::move(signature_)));
      cnt++;
    }
  }
  ...
  while (true) {
    ...
    bool stop = false;
    if (msg->get_id() == tos_api::validatorSession_message_empty::ID) {
      stop = true;
    }
    VLOG(VALIDATOR_SESSION_INFO) << this << ": applying action: " << msg.get();
    real_state_ = ValidatorSessionState::action(description(), real_state_, local_idx(), att, msg.get());
  ...
  auto payload = create_tl_object<tos_api::validatorSession_blockUpdate>(ts, std::move(msgs),
...
void ValidatorSessionImpl::preprocess_block(catchain::CatChainBlock *block) {
  ...
  if (block->payload().size() != 0 || deps.size() != 0) {
    auto R = fetch_tl_object<tos_api::validatorSession_blockUpdate>(block->payload().clone(), true);
    if (!R.is_error()) {
      auto B = R.move_as_ok();
...
void ValidatorSessionImpl::process_broadcast(PublicKeyHash src, td::BufferSlice data,
  ...
  if (is_overlay_broadcast) {
    auto R = fetch_tl_object<tos_api::validatorSession_optimisticCandidateBroadcast>(data, true);
    if (R.is_ok()) {
      if (src == local_id()) {
        return;
      }
...
void ValidatorSessionImpl::process_received_block(td::uint32 block_round, PublicKeyHash src, td::uint32 src_idx,
                                                  tl_object_ptr<tos_api::validatorSession_candidate> candidate,
...
void ValidatorSessionImpl::process_query(PublicKeyHash src, td::BufferSlice data,
  ...
  auto F = fetch_tl_object<tos_api::validatorSession_downloadCandidate>(std::move(data), true);
  if (F.is_error()) {
  ...
  auto P = td::PromiseCreator::lambda([promise = std::move(promise), src = f->id_->src_, round_id,
    ...
    if (R.is_error()) {
      ...
      auto c = R.move_as_ok();
      auto obj = create_tl_object<tos_api::validatorSession_candidate>(src, round_id, c.id.root_hash, std::move(c.data),
...
void ValidatorSessionImpl::generated_block(td::uint32 round, GeneratedCandidate c, double collation_time) {
  ...
  blocks_[block_id] = create_tl_object<tos_api::validatorSession_candidate>(
...
ValidatorSessionImpl::SentCandidateStats &ValidatorSessionImpl::send_candidate_broadcast(
  ...
  auto b = create_tl_object<tos_api::validatorSession_candidate>(
  ...
  if (optimistic_prev_candidate) {
    data = create_serialize_tl_object<tos_api::validatorSession_optimisticCandidateBroadcast>(
...
void ValidatorSessionImpl::get_broadcast_p2p(PublicKeyHash node, ValidatorSessionFileHash file_hash,
  ...
  auto obj = create_tl_object<tos_api::validatorSession_downloadCandidate>(
      round,
      create_tl_object<tos_api::validatorSession_candidateId>(src.tl(), root_hash, file_hash, collated_data_file_hash));
...
void ValidatorSessionImpl::check_action(td::uint32 att) {
  ...
  if (!requested_new_block_) {
    ...
    if (action && action->get_id() != tos_api::validatorSession_message_empty::ID) {
      request_new_block(false);
    }
  }
}
    ...
    if (x) {
      auto P = td::PromiseCreator::lambda([SelfId = actor_id(this), round = virtual_state_->cur_round_seqno(),
        ...
        if (R.is_error()) {
          ...
          auto B = R.move_as_ok();
          auto broadcast = create_tl_object<tos_api::validatorSession_candidate>(
...
void ValidatorSessionImpl::stats_process_action(td::uint32 node_id, tos_api::validatorSession_round_Message &action) {
  tos_api::downcast_call(
      action,
      td::overloaded(
          [&](const tos_api::validatorSession_message_submittedBlock &obj) {
            auto candidate_id =
          ...
          [&](const tos_api::validatorSession_message_approvedBlock &obj) {
            if (obj.candidate_ == skip_round_candidate_id()) {
              return;
            }
            process_approve(node_id, obj.round_, obj.candidate_);
          },
          [&](const tos_api::validatorSession_message_commit &obj) {
            if (obj.candidate_ == skip_round_candidate_id()) {
              return;
            }
