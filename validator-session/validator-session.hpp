...
class ValidatorSessionImpl : public ValidatorSession {
 private:
  ...
  std::map<ValidatorSessionCandidateId, tl_object_ptr<tos_api::validatorSession_candidate>> blocks_;
  ...
  ValidatorSessionStats::Producer *stats_get_candidate_stat_by_id(td::uint32 round,
                                                                  ValidatorSessionCandidateId candidate_id);
  void stats_process_action(td::uint32 node_id, tos_api::validatorSession_round_Message &action);
 ...
 public:
  ...
  void process_received_block(td::uint32 block_round, PublicKeyHash src, td::uint32 src_idx,
                              tl_object_ptr<tos_api::validatorSession_candidate> candidate, const BroadcastInfo &info,
                              bool is_overlay_broadcast, bool is_startup);
...
inline td::StringBuilder &operator<<(td::StringBuilder &sb,
                                     const tos::validatorsession::ValidatorSessionImpl *session) {
  sb << session->print_id();
  return sb;
}
