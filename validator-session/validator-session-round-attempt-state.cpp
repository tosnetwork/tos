...
const ValidatorSessionRoundAttemptState* ValidatorSessionRoundAttemptState::action(
    ...
    td::uint32 att, const tos_api::validatorSession_message_voteFor& act, const ValidatorSessionRoundState* round) {
...
const ValidatorSessionRoundAttemptState* ValidatorSessionRoundAttemptState::action(
    ValidatorSessionDescription& desc, const ValidatorSessionRoundAttemptState* state, td::uint32 src_idx,
    td::uint32 att, const tos_api::validatorSession_message_vote& act, const ValidatorSessionRoundState* round) {
  bool made;
...
const ValidatorSessionRoundAttemptState* ValidatorSessionRoundAttemptState::action(
    ValidatorSessionDescription& desc, const ValidatorSessionRoundAttemptState* state, td::uint32 src_idx,
    td::uint32 att, const tos_api::validatorSession_message_precommit& act, const ValidatorSessionRoundState* round) {
  bool made;
  return make_one(desc, state, src_idx, att, round, &act, made);
}
...
const ValidatorSessionRoundAttemptState* ValidatorSessionRoundAttemptState::action(
    ValidatorSessionDescription& desc, const ValidatorSessionRoundAttemptState* state, td::uint32 src_idx,
    td::uint32 att, const tos_api::validatorSession_message_empty& act, const ValidatorSessionRoundState* round) {
  bool made;
  return make_one(desc, state, src_idx, att, round, &act, made);
}
...
const ValidatorSessionRoundAttemptState* ValidatorSessionRoundAttemptState::try_vote(
    ValidatorSessionDescription& desc, const ValidatorSessionRoundAttemptState* state, td::uint32 src_idx,
    td::uint32 att, const ValidatorSessionRoundState* round, const tos_api::validatorSession_round_Message* act,
    bool& made) {
  made = false;
  ...
  if (act) {
    if (act->get_id() != tos_api::validatorSession_message_vote::ID) {
      VLOG(VALIDATOR_SESSION_WARNING) << "[validator session][node " << desc.get_source_id(src_idx) << "][" << act
                                      << "]: expected VOTE(" << block_id << ")";
    } else {
      auto x = static_cast<const tos_api::validatorSession_message_vote*>(act);
...
const ValidatorSessionRoundAttemptState* ValidatorSessionRoundAttemptState::try_precommit(
  ...
  if (act) {
    if (act->get_id() != tos_api::validatorSession_message_precommit::ID) {
      VLOG(VALIDATOR_SESSION_WARNING) << "[validator session][node " << desc.get_source_id(src_idx) << "][" << act
                                      << "]: expected PRECOMMIT(" << block_id << ")";
    } else {
      auto x = static_cast<const tos_api::validatorSession_message_precommit*>(act);
      if (x->candidate_ != block_id) {
        VLOG(VALIDATOR_SESSION_WARNING) << "[validator session][node " << desc.get_source_id(src_idx) << "][" << act
                                        << "]: expected PRECOMMIT(" << block_id << ")";
      }
...
const ValidatorSessionRoundAttemptState* ValidatorSessionRoundAttemptState::make_one(
  ...
  if (act && act->get_id() != tos_api::validatorSession_message_empty::ID) {
    VLOG(VALIDATOR_SESSION_WARNING) << "[validator session][node " << desc.get_source_id(src_idx) << "][" << act
...
const ValidatorSessionRoundAttemptState* ValidatorSessionRoundAttemptState::action(
    ValidatorSessionDescription& desc, const ValidatorSessionRoundAttemptState* state, td::uint32 src_idx,
    td::uint32 att, const tos_api::validatorSession_round_Message* act, const ValidatorSessionRoundState* round) {
...
tl_object_ptr<tos_api::validatorSession_round_Message> ValidatorSessionRoundAttemptState::create_action(
  ...
  if (!check_vote_received_from(src_idx)) {
    ...
    if (found) {
      ...
      return create_tl_object<tos_api::validatorSession_message_vote>(round->get_seqno(), seqno_, block_id);
    }
  }
  if (!check_precommit_received_from(src_idx)) {
    ...
    if (f) {
      ...
      return create_tl_object<tos_api::validatorSession_message_precommit>(round->get_seqno(), seqno_, block_id);
    }
  }

  return create_tl_object<tos_api::validatorSession_message_empty>(round->get_seqno(), seqno_);
}
