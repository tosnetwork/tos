...
inline td::StringBuilder &operator<<(td::StringBuilder &sb,
                                     const tos::validatorsession::ValidatorSession::PrintId &print_id) {
  sb << "[validatorsession " << print_id.instance_ << "@" << print_id.local_id_ << "]";
  return sb;
}

inline td::StringBuilder &operator<<(td::StringBuilder &sb, const tos::validatorsession::ValidatorSession *session) {
  sb << session->print_id();
  return sb;
}
