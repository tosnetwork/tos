...
struct SrcFile {
  ...
  bool is_stdlib_file;                  // is a part of Tol distribution, imported via "@stdlib/..."
  std::string realpath;                 // what "realpath" returned to locate (either abs path or what tol-js returns)
  std::string text;                     // file contents loaded into memory, every Token::str_val points inside it
  AnyV ast = nullptr;                   // when a file has been parsed, its ast_tol_file is kept here
...
// SrcRange is a "substring" in some loaded .tol source SrcFile.
