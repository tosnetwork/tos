...
void Optimizer::apply() {
  tol_assert(p_ > 0 && p_ <= l_ && q_ >= 0 && q_ <= optimize_depth && l_ <= optimize_depth);
  ...
  for (int i = 0, end_offset = start_offset; i < p_; ++i) {
    tol_assert(end_offset < static_cast<int>(asm_code.size()));
...
bool Optimizer::rewrite_const_push_xchgs() {
  ...
  for (int i = 1; i < p_; i++) {
    ...
    if (op_[i]->is_xchg(&a, &b)) {
      if (a == pos) {
        ...
        tol_assert(apply_op(t, oq_[q_]));
        ++q_;
      }
    } else {
      tol_assert(op_[i]->is_push(&a));
      tol_assert(a != pos);
      oq_[q_] = *op_[i];
      if (a > pos) {
        oq_[q_].a = a - 1;
  ...
  tol_assert(!pos);
  t.apply_push_newconst();
  tol_assert(t <= tr_[p_ - 1]);
  tol_assert(op_[0]->is_gconst());
  oq_[q_++] = *op_[0];
  return true;
}
