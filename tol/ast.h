...
/*
 *   Here we introduce AST representation of Tol source code.
 ...
 *   In Tol, all files are parsed into AST, and all semantic analysis is done at the AST level.
 *
...
// https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.2.1, §10.1, security review v2 closure).
//
...
// See https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.2.1.
//
...
// See https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.6 / §4.1 v3.
//
...
// (the raw, unparsed slice). See https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.2 v3.
...
//
// Per https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.8, externals do NOT have an InMessage envelope,
...
// from `crc16(name) | 0x10000` (see https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.5)
...
struct Vertex<ast_contract_declaration> final : ASTOtherVararg {
  ...
  int n_receive_external_blocks;      // Slice 2 Stage 6 (https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.8)
  ...
  // See https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.2 v3.
...
struct Vertex<ast_struct_field> final : ASTOtherVararg {
  ...
  // Slice 2 Stage 3 (https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.4): `@on(State1, State2)` annotation
...
// currently, Tol doesn't have "implements" or whatever, so struct declaration contains only body
...
template<>
// ast_tol_required_version is a preamble fixating compiler's version at the top of the file
// example: `tol 0.6`
// when compiler version mismatches, it means, that another compiler was earlier for that sources, a warning is emitted
struct Vertex<ast_tol_required_version> final : ASTOtherLeaf {
  std::string_view semver;

  Vertex(SrcRange range, std::string_view semver)
    : ASTOtherLeaf(ast_tol_required_version, range)
    , semver(semver) {}
};

template<>
// ast_import_directive is an import at the top of the file
// examples: `import "another.tol"` / `import "@stdlib/tvm-dicts"`
...
template<>
// ast_tol_file represents a whole parsed input .tol file
...
struct Vertex<ast_tol_file> final : ASTOtherVararg {
