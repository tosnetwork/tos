 ...
 *   https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-message-policy.md (v6) AND the per-receiver scope rule
 *   from https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.2.1 (v3, security
 *   review v2 closure):
 *
  ...
  void diagnose_scope_at_exit(const ScopeRecord& scope, AnyV anchor) {
    ...
    if (scope.saw_reply_emit.empty()) {
      err("inbound envelope carries `queryId` but the receiver emits no reply — "
          ...
          "See https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-message-policy.md \xc2\xa7""4.4 / "
          "https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md \xc2\xa7""3.2.1.")
  ...
  for (const SrcFile* file : G.all_src_files) {
    ...
    for (AnyV declaration : v_file->get_toplevel_declarations()) {
      if (declaration->kind == ast_contract_declaration) {
        err("internal: pipeline order violation: pipeline_check_query_id_propagation ran before pipeline_lower_contracts (contract `{}` still present at top level). See https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §10.1.",
