...
// If "print line comments" is true in settings, every asm instruction is preceded by an original line from Tol sources.
...
static void generate_output_func(std::ostream& os, FunctionPtr fun_ref) {
  tol_assert(fun_ref->is_code_function());
  ...
  output_asm_code_for_fun(
    ...
    G_settings.tol_src_as_line_comments
  );
...
void pipeline_generate_fif_output(std::ostream& os) {
  ...
  // Slice 2 Stage 6 (https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.7) — debug manifest
  ...
  // bounce-replay scripts) can recover which exact site fired.
  {
    ...
    if (!manifest.empty()) {
      ...
      os << "  // see https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.7\n";
