...
void Stack::assign_var(var_idx_t new_idx, var_idx_t old_idx) {
  int i = find(old_idx);
  tol_assert(i >= 0 && "variable not found in stack");
  if (new_idx != old_idx) {
...
void Stack::do_copy_var(AnyV origin, var_idx_t new_idx, var_idx_t old_idx) {
  ...
  if (find(old_idx, i + 1) < 0) {
    issue_push(origin, i);
    tol_assert(get(0).var_idx == old_idx);
  }
...
void Stack::enforce_state(AnyV origin, const StackLayoutVars& req_stack) {
  ...
  tol_assert(depth() == k);
  for (int i = 0; i < k; i++) {
    tol_assert(s[i].var_idx == req_stack[i]);
  }
}

void Stack::merge_const(const Stack& req_stack) {
  tol_assert(s.size() == req_stack.s.size());
  for (std::size_t i = 0; i < s.size(); i++) {
    tol_assert(s[i].var_idx == req_stack.s[i].var_idx);
...
void Stack::rearrange_top(AnyV origin, var_idx_t top_var_idx, bool last) {
  ...
  tol_assert(get(0).var_idx == top_var_idx);
}
  ...
  if (callxargs_count != -1 || ((mode & _InlineFunc) && o.retalt_)) {
    ...
    if (callxargs_count != -1) {
      if (callxargs_count <= 15) {
        ...
        tol_assert(callxargs_count <= 254);
...
bool Op::generate_code_step(Stack& stack, const OpList& parent_ops, size_t self_idx) {
  ...
  switch (cl) {
    ...
    case _Let: {
      tol_assert(left.size() == right.size());
      int i = 0;
    ...
    case _CallInd: {
      ...
      tol_assert(!arg_order || arg_order->size() == right.size());
      tol_assert(!ret_order || ret_order->size() == left.size());
      std::vector<var_idx_t> right1;
      if (args.size()) {
        tol_assert(args.size() == right.size());
        for (int i = 0; i < (int)right.size(); i++) {
          int j = arg_order ? arg_order->at(i) : i;
          const VarDescr& arg = args.at(j);
          if (!arg.is_unused()) {
            tol_assert(var_info[arg.idx] && !var_info[arg.idx]->is_unused());
            right1.push_back(arg.idx);
          }
        }
      } else {
        tol_assert(!arg_order);
        right1 = right;
      }
      std::vector<bool> last;
    ...
    case _SetGlob: {
      ...
      tol_assert(k >= 0);
      for (int i = 0; i < (int)right.size(); i++) {
        tol_assert(stack.s[k + i].var_idx == right[i]);
      }
      if (right.size() > 1) {
        stack.o << AsmOp::Tuple(origin, (int)right.size());
      }
    ...
    case _If: {
      ...
      tol_assert(stack.get(0).var_idx == x);
      stack.s.pop_back();
...
std::vector<AsmOp> CodeBlob::generate_asm_code(int mode) const {
  ...
  tol_assert(!ops.empty() && ops.front()->cl == Op::_Import);
