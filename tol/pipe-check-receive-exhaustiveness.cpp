...
static void warn_implicit_unknown_policy(V<ast_contract_declaration> contract) {
  ...
  err("contract `{}` has no visible unknown-opcode policy; opcodes outside the declared receive map use the implicit Protocol throw. "
      ...
      "See https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §5 / https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tol.tex.",
...
static void validate_implicit_protocol_suppressions(V<ast_contract_declaration> contract) {
  ...
  if (!contract->has_state_machine()) {
    if (contract->implicit_protocol_default) {
      err("`@implicit_protocol_default` requires a state-bearing contract; see https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §5")
        .fire(contract->implicit_protocol_default_range);
    }
    for (const ContractImplicitProtocolFor& suppression : contract->implicit_protocol_for) {
      err("`@implicit_protocol_for({}, {})` requires a state-bearing contract; see https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §5",
  ...
  for (const ContractImplicitProtocolFor& suppression : contract->implicit_protocol_for) {
    if (!declared_messages.count(suppression.message_name)) {
      err("`@implicit_protocol_for({}, {})` references message type `{}` that is not declared by any `receive(msg: ...)` in contract `{}`; see https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §5",
    ...
    if (!declared_states.count(suppression.state_name)) {
      err("`@implicit_protocol_for({}, {})` references state `{}` that is not declared in contract `{}`; see https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §5",
    ...
    if (!seen_pairs.insert(key).second) {
      err("duplicate `@implicit_protocol_for({}, {})` suppression; see https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §5",
          suppression.message_name, suppression.state_name)
        .fire(suppression.range);
    }
  }
