...
// G_settings is filled by tol-main or tol-wasm before calling tol_proceed()
...
TolCompilationResult tol_proceed(const std::string &entrypoint_filename) {
  ...
  try {
    pipeline_discover_and_parse_sources("@stdlib/common.tol", entrypoint_filename);
    ...
    // Slice 2 Stage 6 (https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.7): rewrite
    ...
    // https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-message-policy.md §4.4 hardening:
