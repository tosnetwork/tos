...
static int require_declared_state(StateCheckInfo& info, V<ast_identifier> state_identifier, std::string_view role) {
  ...
  if (it == info.state_to_idx.end()) {
    err("{} `{}` is not declared in `states:`; see https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.4", role, state_name)
  ...
  switch (expr->kind) {
    ...
    case ast_dot_access:
      if (is_storage_state_read(expr->as<ast_dot_access>())) {
        err("receiver code may not read compiler-reserved `storage.__state`; use `become` or `keep_state`. See https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.4.")
          .fire(expr);
      }
...
static StatementFlow analyze_statement(AnyV statement, StateCheckInfo& info, int current_state) {
  ...
  switch (statement->kind) {
    ...
    case ast_return_statement:
      err("state-bearing receivers must end with `become` or `keep_state`, not `return`; see https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.4")
        .fire(statement);
    case ast_throw_statement:
      return {true, false};
    default:
  ...
  for (AnyV item : block->get_items()) {
    if (flow.terminates) {
      if (flow.via_state_tail) {
        err("statements after `become` or `keep_state` are not permitted in the same control-flow path; see https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.4")
          .fire(item);
      }
      return flow;
    }
...
static StateCheckInfo build_state_info(V<ast_contract_declaration> contract) {
  ...
  for (int i = 0; i < contract->get_num_states(); ++i) {
    ...
    if (!inserted) {
      err("duplicate state `{}` in `states:` declaration; see https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.4", state_name)
  ...
  if (!contract->get_initial_state_identifier()) {
    err("state-bearing contract must declare exactly one `@initial state <State>`; see https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.4")
...
static void check_storage_reserved_state_field(V<ast_contract_declaration> contract) {
  ...
  if (StructFieldPtr field = storage_struct->find_field("__state")) {
    err("state-bearing contract storage struct may not declare compiler-reserved field `__state`; see https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.4")
...
static void check_reachability(StateCheckInfo& info) {
  ...
  for (int i = 0; i < static_cast<int>(info.states.size()); ++i) {
    if (!reachable[i]) {
      err("state `{}` is unreachable from `@initial`; see https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.4", info.states[i])
...
static void check_state_contract(V<ast_contract_declaration> contract) {
  ...
  for (int i = 0; i < contract->get_num_receives(); ++i) {
    ...
    // Slice 2 Stage 4 (https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.6): `@deploy` receivers and the
    ...
    if (receive->is_deploy) {
      if (contains_state_tail_statement(receive->get_body())) {
        err("`become` and `keep_state` are not permitted inside an `@deploy receive(...)` body; deployment has exactly one authoritative initial state; see https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.6")
    ...
    if (receive->is_unknown_opcode_catch_all) {
      ...
      if (contains_state_tail_statement(receive->get_body())) {
        err("`become` and `keep_state` are not permitted inside `receive(msg: UnknownOpcode)`; the catch-all body has no source state; see https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.2")
          .fire(receive->get_body());
      }
      continue;
    }
    if (!receive->has_state_clause()) {
      err("state-bearing contract receiver must declare `on <State>`; see https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.4")
        .fire(receive);
    }
    ...
    if (!flow.terminates) {
      err("state-bearing receiver must end every successful control-flow path with `become` or `keep_state`; see https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.4")
        .fire(receive->get_body());
    }
  }
...
static void check_non_state_contract(V<ast_contract_declaration> contract) {
  if (contract->get_initial_state_identifier()) {
    err("`@initial state` requires a `states:` declaration; see https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.4")
      .fire(contract->get_initial_state_identifier());
  }
  for (int i = 0; i < contract->get_num_receives(); ++i) {
    V<ast_receive_block> receive = contract->get_receive(i);
    // Slice 2 Stage 4 (https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.6): `become`/`keep_state` are also
    ...
    if (receive->has_state_clause()) {
      err("`receive(...) on <State>` requires a `states:` declaration; see https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.4")
        .fire(receive->state_identifier);
    }
    if (contains_state_tail_statement(receive->get_body())) {
      err("`become` and `keep_state` require a state-bearing contract; see https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.4")
        .fire(receive->get_body());
    }
  }
}
