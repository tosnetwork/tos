...
// LexingTrie is a prefix tree storing all available Tol language constructs.
...
// It's not valid in Tol, valid is `"...".crc32()` (just methods for `string`)
...
// E.g., for 'tol >0.5.0', '0.5.0' should be parsed specially to emit tok_semver.
// See TolLanguageGrammar::parse_next_chunk_special().
  ...
  static TokenType maybe_keyword(std::string_view str) {
    switch (str.size()) {
      ...
      case 16:
        ...
        // See https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.8.
...
//
// ----------------------------------------------------------------------
// Here we define a grammar of Tol.
// All valid chunks prefixes are stored in trie.
//

struct TolLanguageGrammar {
...
LexingTrie TolLanguageGrammar::trie;
...
void Lexer::next() {
  while (cur_token_idx == last_token_idx && !is_eof()) {
    update_location();
    if (!TolLanguageGrammar::parse_next_chunk(this)) {
...
void Lexer::next_special(TokenType parse_next_as, const char* str_expected) {
  tol_assert(cur_token_idx == last_token_idx);
  skip_spaces();
  update_location();
  if (!TolLanguageGrammar::parse_next_chunk_special(this, parse_next_as)) {
...
void Lexer::hack_replace_rshift_with_one_triangle() {
  ...
  tol_assert(cur_token.type == tok_rshift);
