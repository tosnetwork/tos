...
static bool parse_raw_address(std::string_view acc_string, int& workchain, tos::StdSmcAddress& addr) {
  ...
  td::Result<int> r_wc = td::to_integer_safe<tos::WorkchainId>(td::Slice(acc_string.data(), pos++));
  if (r_wc.is_error()) {
    return false;
  }
...
static ConstValExpression parse_vertex_call_to_compile_time_function(V<ast_function_call> v, std::string_view f_name) {
  ...
  if (receiver == TypeDataString::create()) {
    ...
    tol_assert(v->get_num_args() == !v->dot_obj_is_self);
  ...
  tol_assert(v->get_num_args() == 1);    // checked by type inferring
