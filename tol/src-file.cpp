...
SrcFile::SrcPosition SrcFile::convert_offset(int offset) const {
  ...
  // since original Tol src lines are now printed into Fift output, this is invoked for every asm instruction
...
std::string SrcFile::extract_short_name() const {
  ...
  std::string short_name = realpath.substr(last_slash + 1);    // "file.tol" (no path)

  if (is_stdlib_file) {   // not "common.tol", but "@stdlib/common"
    return "@stdlib/" + short_name.substr(0, short_name.size() - 4);
  }
  return short_name;
}
...
SrcRange::DecodedRange SrcRange::decode_offsets() const {
  const SrcFile* src_file = get_src_file();
  tol_assert(src_file && src_file->is_offset_valid(start_offset));
