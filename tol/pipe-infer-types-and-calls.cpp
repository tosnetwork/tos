...
// (in FunC, `forall` type just couldn't be unified with non-primitives; in Tol, generic T is expectedly inferred)
...
class InferTypesAndCallsAndFieldsVisitor final {
...
#ifdef TOL_DEBUG
    tol_assert(src->inferred_type != nullptr && !src->inferred_type->has_genericT_inside());
#endif
...
#ifdef TOL_DEBUG
    tol_assert(inferred_type != nullptr && !inferred_type->has_genericT_inside());
#endif
...
#ifdef TOL_DEBUG
    tol_assert(inferred_return_type != nullptr && !inferred_return_type->has_genericT_inside());
#endif
  ...
  static ExprFlow infer_local_vars_declaration(V<ast_local_vars_declaration> v, FlowContext&& flow, bool used_as_condition) {
    tol_assert(false);   // handled at assignment, because `var x = rhs` is an assignment
  }
  ...
  static void process_assignment_lhs_after_infer_rhs(AnyExprV lhs, TypePtr rhs_type, FlowContext& out_flow) {
    tol_assert(lhs->inferred_type != nullptr);
  ...
  ExprFlow infer_braced_expression(V<ast_braced_expression> v, FlowContext&& flow, bool used_as_condition, TypePtr hint) {
    ...
    for (AnyV item : v->get_block_statement()->get_items()) {
      if (auto v_return = item->try_as<ast_braced_yield_result>()) {
        tol_assert(implicit_result == nullptr);
  ...
  ExprFlow infer_reference(V<ast_reference> v, FlowContext&& flow, bool used_as_condition, TypePtr hint, FunctionPtr* out_f_called = nullptr) const {
    ...
    if (LocalVarPtr var_ref = v->sym->try_as<LocalVarPtr>()) {
      tol_assert(flow.smart_cast_exists(SinkExpression(var_ref)));   // all local vars are presented in flow
  ...
  ExprFlow infer_dot_access(V<ast_dot_access> v, FlowContext&& flow, bool used_as_condition, TypePtr hint, FunctionPtr* out_f_called = nullptr, AnyExprV* out_dot_obj = nullptr) {
    ...
    if (fun_ref->is_generic_function() && fun_ref->genericTs->size() == fun_ref->genericTs->n_from_receiver) {
      tol_assert(substitutedTs.typeT_at(0) != nullptr);
      fun_ref = check_and_instantiate_generic_function(v->range, fun_ref, std::move(substitutedTs));
    }
  ...
  ExprFlow infer_lambda_fun(V<ast_lambda_fun> v, FlowContext&& flow, bool used_as_condition, TypePtr hint) {
    ...
    tol_assert(v->lambda_ref == nullptr);
  ...
  FlowContext process_try_catch_statement(V<ast_try_catch_statement> v, FlowContext&& flow) {
    ...
    tol_assert(v->get_catch_expr()->size() == 2);
...
public:
  ...
  void start_visiting_function(FunctionPtr fun_ref, V<ast_function_declaration> v_function) {
    ...
    if (fun_ref->is_code_function()) {
      ...
      tol_assert(fun_ref->declared_return_type);
    }
...
static void infer_and_save_return_type_of_function(FunctionPtr fun_ref) {
  ...
  tol_assert(!fun_ref->is_generic_function() && !fun_ref->is_type_inferring_done());
