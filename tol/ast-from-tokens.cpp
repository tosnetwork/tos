...
static const char* slice2_deferred_msg() {
  return "deferred to future Slice 2 commit; see https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §10.1";
}

static bool is_slice2_deferred_statement(std::string_view name) {
  // Slice 2 Stage 6 (https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.7) ships `require(...)`;
  ...
  switch (kind) {
    ...
    case AnnotationKind::on_states: {
      if (!v_arg || v_arg->size() == 0) {
        err("expecting `(State1, State2, ...)` after {}; see https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.4", name).fire(range);
      }
      for (int i = 0; i < v_arg->size(); ++i) {
        AnyExprV item = v_arg->get_item(i);
        if (item->kind != ast_reference) {
          err("`@on(...)` arguments must be state identifiers; see https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.4").fire(item);
        }
      }
      break;
    }
...
static AnyV parse_struct_field(Lexer& lex) {
  ...
  for (auto v_annotation : field_annotations) {
    switch (v_annotation->kind) {
      case AnnotationKind::on_states: {
        if (!on_states.empty()) {
          err("`@on(...)` may be specified at most once per field; see https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.4")
            .fire(v_annotation);
        }
...
static bool is_deferred_contract_member_name(std::string_view name) {
  // Slice 2 Stage 6 (https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.8) ships `receive_external`;
...
// see https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.5
static AnyV parse_contract_get_fun_block(Lexer& lex, const std::vector<V<ast_annotation>>& annotations) {
  ...
  if (lex.tok() == tok_colon) {
    ...
    err("`get fun` must declare an explicit return type; see https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.5").fire(v_ident);
  }
  ...
  for (auto v_annotation : annotations) {
    switch (v_annotation->kind) {
      ...
      case AnnotationKind::custom:
        ...
        break;
      default:
        err("annotation `@{}` is not applicable to `get fun`; see https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.5", v_annotation->name).fire(v_annotation);
    }
  }
...
static AnyV parse_receive_block(Lexer& lex,
  ...
  // Slice 2 Stage 4 (https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.2 v3): the reserved literal type name
  ...
  if (auto leaf = msg_type->try_as<ast_type_leaf_text>(); leaf && leaf->text == "UnknownOpcode") {
    ...
    if (state_identifier != nullptr) {
      err("`receive(msg: UnknownOpcode)` cannot carry an `on <State>` clause; the catch-all body runs irrespective of contract state; see https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.2")
        .fire(state_identifier);
    }
    if (has_disclaim_query_id_annotation) {
      err("`@disclaim_query_id` cannot decorate `receive(msg: UnknownOpcode)`; the unknown-opcode catch-all has no parsed query_id surface; see https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.2")
        .fire(disclaim_annotation_range);
    }
    if (is_deploy) {
      err("`@deploy` cannot decorate `receive(msg: UnknownOpcode)`; deploy and unknown-opcode catch-all are distinct entry points; see https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.6")
        .fire(deploy_annotation_range);
    }
  }

  // Slice 2 Stage 4 (https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.6): `@deploy` cannot combine with
  // `on <State>` (deployment has exactly one initial state) nor with `@disclaim_query_id`.
  if (is_deploy) {
    if (state_identifier != nullptr) {
      err("`@deploy` cannot combine with `on <State>`; deployment has exactly one authoritative initial state; see https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.6")
        .fire(state_identifier);
    }
    if (has_disclaim_query_id_annotation) {
      err("`@deploy` cannot combine with `@disclaim_query_id`; the deploy receiver runs before storage exists and has its own scope rules; see https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.6")
        .fire(disclaim_annotation_range);
    }
  }
...
// Slice 2 Stage 6 (https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.8): parse a
...
static AnyV parse_receive_external_block(Lexer& lex) {
  ...
  lex.expect(tok_clpar, "`)`");
  if (lex.tok() == tok_identifier && lex.cur_str() == "on") {
    err("`receive_external(...)` cannot carry an `on State` clause; state-machine guards apply to internal receivers only; see https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.8").fire(lex.cur_range());
  }
  auto v_body = parse_block_statement(lex, true);
  range.end(v_body->range);
...
static AnyV parse_contract_declaration(Lexer& lex, const std::vector<V<ast_annotation>>& annotations) {
  ...
  std::vector<AnyV> receive_external_blocks;       // Slice 2 Stage 6 (https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.8)
  ...
  // Slice 2 Stage 4 (https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.2 v3): contract-level unknown-opcode mode.
  ...
  while (lex.tok() != tok_clbrace) {
    ...
    if (lex.tok() == tok_annotation_at) {
      if (lex.cur_str() == "@initial") {
        ...
        if (initial_state_identifier) {
          err("contract block may contain only one `@initial state` declaration; see https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.4").fire(lex.cur_range());
        }
        initial_state_identifier = parse_initial_state(lex);
        continue;
      }
      // Slice 2 Stage 7 (https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.2.1):
      ...
      if (lex.cur_str() == "@disclaim_query_id") {
        ...
        if (lex.tok() != tok_receive) {
          err("`@disclaim_query_id` is only valid immediately before a `receive(...)` block; see https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.2.1").fire(annotation_range);
        }
        if (!storage_type) {
      ...
      // Slice 2 Stage 4 (https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.6):
      ...
      if (lex.cur_str() == "@deploy") {
        ...
        if (lex.tok() != tok_receive) {
          err("`@deploy` is only valid immediately before a `receive(...)` block; see https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.6").fire(annotation_range);
        }
        if (!storage_type) {
          err("contract `storage:` declaration must appear before `receive(...)` blocks").fire(lex.cur_range());
        }
      ...
      // Slice 2 Stage 4 (https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.2 v3):
      ...
      if (lex.cur_str() == "@unknown_silent_drop") {
        ...
        if (unknown_set_explicitly) {
          err("contract block already declares `{}`; `@unknown_silent_drop` and `@unknown_throw(...)` are mutually exclusive; see https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.2",
      ...
      if (lex.cur_str() == "@unknown_throw") {
        ...
        if (lex.tok() != tok_oppar) {
          err("`@unknown_throw` requires a literal int argument: `@unknown_throw(N);`; see https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.2").fire(annotation_range);
        }
        lex.next();
        if (lex.tok() != tok_int_const) {
          err("`@unknown_throw(N)` requires a literal int; see https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.2").fire(lex.cur_range());
        }
      ...
      if (lex.cur_str() == "@implicit_protocol_default") {
        ...
        if (implicit_protocol_default) {
          err("contract block already declares `@implicit_protocol_default`; duplicate state-cross-product suppressions are not allowed; see https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §5")
            .fire(annotation_range);
        }
        if (lex.tok() == tok_oppar) {
          err("`@implicit_protocol_default` takes no arguments; write `@implicit_protocol_default;`; see https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §5")
            .fire(annotation_range);
        }
      ...
      if (lex.cur_str() == "@implicit_protocol_for") {
        ...
        lex.next();
        if (lex.tok() != tok_oppar) {
          err("`@implicit_protocol_for` requires `(MessageType, StateName)`; see https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §5")
            .fire(annotation_range);
        }
        lex.next();
    ...
    if (lex.tok() == tok_identifier && lex.cur_str() == "states") {
      ...
      if (!state_identifiers.empty()) {
        err("contract block may contain only one `states:` declaration; see https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.4").fire(lex.cur_range());
      }
    ...
    if (lex.tok() == tok_receive_external) {
      // Slice 2 Stage 6 (https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.8): external receivers
      // do not accept any annotations (no `@deploy`, no `@disclaim_query_id`, etc).
      if (!pending_member_annotations.empty()) {
        err("`receive_external(...)` blocks do not accept annotations; see https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.8").fire(pending_member_annotations.front());
      }
      if (!storage_type) {
    ...
    if (lex.tok() == tok_fun) {
      err("free-standing `fun` declarations are not permitted inside a contract block; see https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.1").fire(lex.cur_range());
    }
    if (lex.tok() == tok_identifier && is_deferred_contract_member_name(lex.cur_str())) {
      err("`{}` is {}", lex.cur_str(), slice2_deferred_msg()).fire(lex.cur_range());
    }
    err("contract blocks may contain only `storage:`, `states:`, `@initial state`, `@implicit_protocol_default`, `@implicit_protocol_for(...)`, `receive(...)`, `receive_external(...)`, and `get fun` declarations; see https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.1 / §3.5 / §3.8").fire(lex.cur_range());
  }
  ...
  if (receive_blocks.empty()) {
    // Slice 2 Stage 6 (https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.8): every contract is
  ...
  // Slice 2 Stage 4 (https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.2 / §3.6):
  ...
  for (AnyV r : receive_blocks) {
    ...
    if (rv->is_deploy) {
      if (first_deploy != nullptr) {
        err("contract `{}` declares more than one `@deploy receive(...)`; first declared at {}; see https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.6",
    ...
    if (rv->is_unknown_opcode_catch_all) {
      if (first_unknown_catch_all != nullptr) {
        err("contract `{}` declares more than one `receive(msg: UnknownOpcode)`; first declared at {}; see https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.2",
  ...
  if (first_unknown_catch_all != nullptr) {
    if (unknown_set_explicitly) {
      err("contract `{}` declares both `{}` and `receive(msg: UnknownOpcode)`; pick one of the two unknown-opcode handlers; see https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.2",
...
static void reject_contract_mixed_with_onInternalMessage(const std::vector<AnyV>& declarations) {
  ...
  for (AnyV v : declarations) {
    if (auto v_contract = v->try_as<ast_contract_declaration>()) {
      if (contract_decl) {
        err("Slice 2 Stage 1 supports one `contract` declaration per .tol file; see https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §10.1").fire(v_contract);
      }
  ...
  if (contract_decl && on_internal) {
    err("a .tol file cannot contain both `contract X { ... }` and top-level `fun onInternalMessage(...)`; see https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §6.3").fire(on_internal);
  }
  // Slice 2 Stage 6 (https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.8): the contract block synthesizes
  ...
  if (contract_decl && contract_decl->get_num_externals() > 0 && on_external) {
    err("a .tol file with `receive_external(...)` blocks cannot also declare top-level `fun onExternalMessage(...)`; the contract synthesizes it; see https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.8").fire(on_external);
  }
  if (contract_decl && file_scope_get_fun) {
    err("a .tol file with a `contract X { ... }` block cannot also declare top-level `get fun {}(...)`; move the get-method inside the contract; see https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.5",
