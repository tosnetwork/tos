  ...
  void visit(V<ast_dot_access> v) override {
    if (v->get_field_name() == "entries" &&
      ...
      err("direct access to `Slice3PendingReplyTable.entries` bypasses `(expected_responder, query_id)` reply binding "
          ...
          "See https://github.com/tosnetwork/doc/blob/main/tos-blockchain/tos-message-policy.md §4.4 / https://github.com/tosnetwork/doc/blob/main/tos-blockchain/tos-language-syntax-policy.md §5.")
        .collect(v, cur_f);
    }
    parent::visit(v);
  }
  ...
  void visit(V<ast_function_call> v) override {
    ...
    if (fun_ref && fun_ref->name == "slice3PendingReplyKeyHash" &&
      ...
      err("raw Slice 3 reply-key construction is warning-only for legacy code; manifest-backed stdlib reply APIs must use "
          ...
          "and optional `expected_reply_opcode`. See https://github.com/tosnetwork/doc/blob/main/tos-blockchain/tos-message-policy.md §4.4 / https://github.com/tosnetwork/doc/blob/main/tos-blockchain/tos-language-syntax-policy.md §5.")
        .warning(v, cur_f);
    }
    parent::visit(v);
  }
