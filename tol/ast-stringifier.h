...
class ASTStringifier final : public ASTVisitor {
  constexpr static std::pair<ASTNodeKind, const char*> name_pairs[] = {
    ...
    {ast_tol_required_version, "ast_tol_required_version"},
    {ast_import_directive, "ast_import_directive"},
    {ast_tol_file, "ast_tol_file"},
  };

  static_assert(std::size(name_pairs) == ast_tol_file + 1, "name_pairs needs to be updated");
...
public:
  ...
  void visit(AnyV v) override {
    switch (v->kind) {
      ...
      case ast_tol_required_version:         return handle_vertex(v->as<ast_tol_required_version>());
      case ast_import_directive:              return handle_vertex(v->as<ast_import_directive>());
      case ast_tol_file:                     return handle_vertex(v->as<ast_tol_file>());
      default:
