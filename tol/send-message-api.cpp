...
struct IR_AutoDeployAddress {
  ...
  IR_AutoDeployAddress(CodeBlob& code, AnyV origin, const std::vector<var_idx_t>& ir_vars) {
    ...
    tol_assert(ir_vars.size() == 1 + 3 + 3);
    tol_assert(t_stateInit && t_stateInit->get_width_on_stack() == (2+1) && t_stateInit->size() == 2);
    tol_assert(t_toShard && t_toShard->get_width_on_stack() == (2+1) && t_toShard->or_null);
...
std::vector<var_idx_t> generate_createMessage(FunctionPtr called_f, CodeBlob& code, AnyV origin, const std::vector<std::vector<var_idx_t>>& ir_options) {
  ...
  tol_assert(t_bounce && t_bounce->get_width_on_stack() == (1+1) && t_bounce->size() == 2);
  tol_assert(t_dest && t_dest->get_width_on_stack() == (1+3+3+1) && t_dest->size() == 4);
  tol_assert(t_value && t_value->get_width_on_stack() == (2+1) && t_value->size() == 2);
  ...
  tol_assert(offset == static_cast<int>(rvect.size()));
  ...
  if (body_size.max_bits == 0 && body_size.max_refs == 0) {
    tol_assert(ir_body.empty());
  } else if (body_store_as_ref) {
    tol_assert(ir_body.size() == 1);   // it was either an input cell or a automatically created one
...
std::vector<var_idx_t> generate_createExternalLogMessage(FunctionPtr called_f, CodeBlob& code, AnyV origin, const std::vector<std::vector<var_idx_t>>& args) {
  ...
  tol_assert(t_dest && t_dest->get_width_on_stack() == (2+1) && t_dest->size() == 3);
  tol_assert(t_topic && t_topic->get_width_on_stack() == (1+1) && t_topic->size() == 2);
...
std::vector<var_idx_t> generate_address_buildInAnotherShard(FunctionPtr called_f, CodeBlob& code, AnyV origin, const std::vector<std::vector<var_idx_t>>& args) {
  ...
  tol_assert(ir_shard_options.size() == 2);
