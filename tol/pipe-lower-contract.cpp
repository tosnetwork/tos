...
static void check_receive_opcode_prefix(V<ast_receive_block> receive, StructPtr message_struct) {
  ...
  if (prefix_len != 32) {
    err("contract receive message type `{}` must declare exactly a 32-bit opcode prefix; actual prefix length is {}. See https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.2.",
        message_struct->name, prefix_len)
      .fire(receive->message_type_node);
  }
}

// Slice 2 Stage 6 (https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.8): an external receiver's
...
static void check_receive_external_opcode_prefix(V<ast_receive_external_block> receive, StructPtr message_struct) {
  int prefix_len = message_struct->opcode.prefix_len;
  if (prefix_len != 32) {
    err("contract receive_external message type `{}` must declare exactly a 32-bit opcode prefix; actual prefix length is {}. See https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.8.",
        message_struct->name, prefix_len)
      .fire(receive->message_type_node);
  }
}
...
// Slice 2 Stage 4 (https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.6 / §4.1 v3): for state-bearing
...
static AnyV make_receive_scope_marker(
  ...
  if (receive->has_disclaim_query_id_annotation) {
    // Slice 2 Stage 7 (https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.2.1): the parser annotation
...
static AnyV make_receive_branch(
  ...
  // https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.2.1, §10.1.
...
// Slice 2 Stage 4 (https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.6 / §4.1 v3): build the @deploy
...
// Slice 2 Stage 4 (https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.2 v3): build the unknown-opcode tail.
...
static std::vector<AnyV> make_unknown_mode_tail(V<ast_contract_declaration> contract, V<ast_receive_block> unknown_catch_all_receive) {
  ...
  switch (contract->unknown_mode) {
    case ContractUnknownMode::default_protocol_throw:
      ...
      // Protocol-class application error code. See https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-message-policy.md §5.2 / §5.3.
...
// Slice 2 Stage 6 (https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.8): build one dispatch branch
...
// Slice 2 Stage 6 (https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.8): synthesize the
...
static V<ast_function_declaration> make_on_internal_function(
  ...
  // Slice 2 Stage 4 dispatch shape (https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §4.1 v3):
  ...
  // https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.2 v3 / §4.1 v3.
...
// `get fun` bodies must be free of these per https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.5
  ...
  void visit(V<ast_function_call> v) override {
    ...
    if (auto v_ref = callee->try_as<ast_reference>()) {
      ...
      if (is_forbidden_in_get_fun_top_level(name)) {
        err("`{}(...)` is not permitted in `get fun {}`; the get-method execution path is read-only and may not emit actions or commit storage; see https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.5",
...
// Slice 2 Stage 6 (https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.7): map every synthesized
...
static std::vector<AnyV> lower_contract(V<ast_contract_declaration> contract) {
  ...
  // Slice 2 Stage 4 (https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.6 / §4.1 v3): a state-bearing
  ...
  if (state_ctx.enabled && has_deploy && contract->get_initial_state_identifier() == nullptr) {
    err("`@deploy receive(...)` in a state-bearing contract requires `@initial state <Name>` to be declared so the deploy lowering knows which initial state tag to inject; see https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.6")
  ...
  // Slice 2 Stage 6 (https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.8): external receivers form a
  ...
  for (int i = 0; i < contract->get_num_externals(); ++i) {
    ...
    if (is_unknown_opcode_type_node(receive->message_type_node)) {
      if (first_external_unknown_catch_all != nullptr) {
        err("contract `{}` declares more than one `receive_external(msg: UnknownOpcode)`; first declared at {}; see https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.8",
  ...
  for (int i = 0; i < contract->get_num_get_funs(); ++i) {
    ...
    if (name == "loadData" || name == "save" || name == "onInternalMessage" || name == "onExternalMessage") {
      err("`get fun {}` shadows a contract-internal function generated by the lowering; pick a different name; see https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.5",
  ...
  // Slice 2 Stage 6 (https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.7): tag every synthesized
...
// See https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.5 / §10.1.
void check_contract_method_id_collisions() {
  ...
  for (FunctionPtr fun_ref : G.all_functions) {
    ...
    if (origin.empty()) {
      ...
      err("method_id collision: get fun `{}` (method_id {}) collides with get fun `{}` in contract `{}`; rename one or pin via @method_id(N); see https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/tos-language-syntax-policy.md §3.5",
