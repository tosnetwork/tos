...
static td::Result<std::string> compile_internal(char *config_json) {
  ...
  // note that `pathMappings` are handled on a client-side (in tol-js) only

  G_settings.verbosity = 0;
  G_settings.optimization_level = std::max(0, opt_level);
  G_settings.stack_layout_comments = stack_comments;
  G_settings.tol_src_as_line_comments = src_line_comments;
  ...
  TolCompilationResult result = tol_proceed(entrypoint_filename);
  ...
  // in tol-js particularly, .fif files are embedded into distribution, next to tol-stdlib/ folder
  ...
  // This is acceptable, because Fift compilation is very fast compared to tol_proceed (which is fully parallel).
  ...
  if (fift_result.is_error()) {
    ...
    json.key_value("tolVersion", JsonPrettyOutput::Unescaped{TOL_VERSION});
    json.end_object();
    return result_json_str.str();
  }
...
/// Callback used to retrieve file contents from a "not file system". See tol-js for implementation.
/// The callback must fill either destContents or destError.
/// The implementor must use malloc() for them and use free() after tol_compile returns.
/// callback_payload is an opaque pointer passed through from tol_compile(), for caller-side identification.
...
const char* tol_version() {
  std::ostringstream result_json_str;
  JsonPrettyOutput json(result_json_str);
  json.start_object();
  json.key_value("tolVersion", JsonPrettyOutput::Unescaped{TOL_VERSION});
  json.key_value("tolFiftLibCommitHash", JsonPrettyOutput::Unescaped{GitMetadata::CommitSHA1()});
  json.key_value("tolFiftLibCommitDate", JsonPrettyOutput::Unescaped{GitMetadata::CommitDate()});
  json.end_object();
  return strdup(result_json_str.str().c_str());
}

const char *tol_compile(char *config_json, WasmFsReadCallback callback, void* callback_payload) {
