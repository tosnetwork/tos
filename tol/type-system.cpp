...
TypePtr TypeDataGenericTypeWithTs::create(StructPtr struct_ref, AliasDefPtr alias_ref, std::vector<TypePtr>&& type_arguments) {
  if (struct_ref) {
    tol_assert(alias_ref == nullptr && struct_ref->is_generic_struct());
  } else {
    tol_assert(struct_ref == nullptr && alias_ref->is_generic_alias());
  }
...
int TypeDataArray::get_type_id() const {
  tol_assert(!has_genericT_inside());
...
int TypeDataGenericT::get_type_id() const {
  tol_assert(false);    // generics must have been instantiated in advance
}

int TypeDataGenericTypeWithTs::get_type_id() const {
  tol_assert(false);    // `Wrapper<T>` has to be resolved in advance
}
...
int TypeDataUnion::get_type_id() const {
  tol_assert(false);    // a union can not be inside a union
}
...
int TypeDataNotInferred::get_type_id() const {
  tol_assert(false);    // "not inferred" is an intermediate state only
}
