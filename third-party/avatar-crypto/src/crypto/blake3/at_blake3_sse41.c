...
compress_pre( vu_t        rows[4],
              ...
              uchar const block[ static AT_BLAKE3_BLOCK_SZ ],
              uint        block_len,
              ulong       ctr,
              uint        flags ) {
  rows[0] = vu_ld( cv   );
  rows[1] = vu_ld( cv+4 );
  rows[2] = vu( AT_BLAKE3_IV[0], AT_BLAKE3_IV[1], AT_BLAKE3_IV[2], AT_BLAKE3_IV[3] );
...
void
at_blake3_sse_compress1( uchar * restrict       out,
  ...
  AT_BLAKE3_TRACE(( "at_blake3_sse_compress1(out=%p,msg=%p,sz=%u,counter=%lu,flags=%02x)",
                    (void *)out, (void *)msg, msg_sz, counter, flags ));
  assert( msg_sz<=AT_BLAKE3_CHUNK_SZ );
  ...
  uint flag_mask = ~at_uint_if( flags&AT_BLAKE3_FLAG_PARENT,
                                AT_BLAKE3_FLAG_CHUNK_START|AT_BLAKE3_FLAG_CHUNK_END,
                                0U );

  uint block_flags = flags | (flag_mask & AT_BLAKE3_FLAG_CHUNK_START);
  if( AT_UNLIKELY( in_chain && !(flags&AT_BLAKE3_FLAG_CHUNK_START) ) ) {
    block_flags &= ~AT_BLAKE3_FLAG_CHUNK_START;
  }
  do {
    uint block_sz = at_uint_min( msg_sz, AT_BLAKE3_BLOCK_SZ );
    block_flags |= AT_BLAKE3_FLAG_CHUNK_END;
    block_flags &= (flag_mask & ~at_uint_if( msg_sz<=AT_BLAKE3_BLOCK_SZ, 0, (AT_BLAKE3_FLAG_CHUNK_END|AT_BLAKE3_FLAG_ROOT) ) );

    uchar tail[ AT_BLAKE3_BLOCK_SZ ] __attribute__((aligned(16)));
    uchar const * restrict block;
    if( AT_LIKELY( msg_sz>=AT_BLAKE3_BLOCK_SZ ) ) {
      block = msg;
    } else {
      vb_st( tail,    vu_zero() );
      ...
      at_memcpy( tail, msg, msg_sz );
      block = tail;
    }

    if( AT_UNLIKELY( out_chain && (block_flags & AT_BLAKE3_FLAG_CHUNK_END) ) ) {
      at_memcpy( out,       block, AT_BLAKE3_BLOCK_SZ    );
      at_memcpy( out_chain, cv,    AT_BLAKE3_OUTCHAIN_SZ );
      AT_BLAKE3_TRACE(( "at_blake3_sse_compress1: done (XOF mode)" ));
      return;
    }

    AT_BLAKE3_TRACE(( "at_blake3_sse_compress1: sz=%u counter=%u flags=%x", block_sz, (uint)counter, block_flags ));
  ...
  AT_BLAKE3_TRACE(( "at_blake3_sse_compress1: done" ));
}
