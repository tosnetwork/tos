#ifndef HEADER_at_src_util_shmem_at_shmem_h
#define HEADER_at_src_util_shmem_at_shmem_h
...
/* APIs for NUMA aware and page size aware manipulation of complex
   ...
   interoperate with the at_shmem_cfg command and control script for
   host configuration.  fd must be booted to use the APIs in this
   module. */

#include "../log/at_log.h"

/* AT_SHMEM_JOIN_MAX gives the maximum number of unique fd shmem regions
...
#define AT_SHMEM_JOIN_MAX (255UL)

/* AT_SHMEM_JOIN_MODE_* are used to specify how a memory region should
   be initially mapped into the thread group's local address space by
   at_shmem_join. */

#define AT_SHMEM_JOIN_MODE_READ_ONLY   (0)
#define AT_SHMEM_JOIN_MODE_READ_WRITE  (1)

/* AT_SHMEM_{NUMA,CPU}_MAX give the maximum number of numa nodes and
   logical cpus supported by at_shmem.
   AT_SHMEM_CPU_MAX>=AT_SHMEM_NUMA_MAX>0. */

#define AT_SHMEM_NUMA_MAX (1024UL)
#define AT_SHMEM_CPU_MAX  (1024UL)

/* AT_SHMEM_{UNKNOWN,NORMAL,HUGE,GIGANTIC}_{PAGE_LG_SZ,PAGE_SZ} give the
...
#define AT_SHMEM_UNKNOWN_LG_PAGE_SZ  (-1)
#define AT_SHMEM_NORMAL_LG_PAGE_SZ   (12)
#define AT_SHMEM_HUGE_LG_PAGE_SZ     (21)
#define AT_SHMEM_GIGANTIC_LG_PAGE_SZ (30)

#define AT_SHMEM_UNKNOWN_PAGE_SZ           (0UL)
#define AT_SHMEM_NORMAL_PAGE_SZ         (4096UL)
#define AT_SHMEM_HUGE_PAGE_SZ        (2097152UL)
#define AT_SHMEM_GIGANTIC_PAGE_SZ (1073741824UL)

/* AT_SHMEM_NAME_MAX gives the maximum number of bytes needed to hold
   the cstr with the name at_shmem region.  That is, a valid at_shmem
   region name will have a strlen in [1,AT_SHMEM_NAME_MAX).  (Harmonized
   with AT_LOG_NAME_MAX but this is not strictly required.) */

#define AT_SHMEM_NAME_MAX AT_LOG_NAME_MAX

/* AT_SHMEM_PAGE_SZ_CSTR_MAX is the size of a buffer large enough to
   hold an shmem page sz cstr (==at_strlen("gigantic")+1). */

#define AT_SHMEM_PAGE_SZ_CSTR_MAX (9UL)

/* at_shmem_private_key_t is for internal use (tmpl/at_map
   interoperability). */

struct at_shmem_private_key {
  char cstr[ AT_SHMEM_NAME_MAX ];
};

typedef struct at_shmem_private_key at_shmem_private_key_t;

/* A at_shmem_join_info_t used by various APIs to provide low level
   details about a join. */

struct at_shmem_join_info {
  ...
  int    mode;     /* Will be in AT_SHMEM_JOIN_MODE_{READ_ONLY,READ_WRITE}.  Attempting to execute and (if read-only) write in the
                      shmem region will fault the thread group. */
  uint   hash;     /* Will be (uint)at_hash( 0UL, name, AT_SHMEM_NAME_MAX ) */
  union {
    char                   name[ AT_SHMEM_NAME_MAX ]; /* cstr with the region name at join time (guaranteed '\0' terminated) */
    at_shmem_private_key_t key;                       /* For easy interoperability tmpl/at_map.h */
  };
};

typedef struct at_shmem_join_info at_shmem_join_info_t;

/* A at_shmem_joinleave_func_t is optionally used by at_shmem_join /
   at_shmem_leave to wrap / unwrap a shared memory region with
   additional thread group local context when it is mapped / unmapped. */

typedef void *
(*at_shmem_joinleave_func_t)( void *                       context,
                              at_shmem_join_info_t const * join_info );

/* A at_shmem_info_t used by various APIs to provide low level details
   of a shared memory region. */

struct at_shmem_info {
...
typedef struct at_shmem_info at_shmem_info_t;

AT_PROTOTYPES_BEGIN

/* User APIs **********************************************************/

/* at_shmem_{join,leave} joins/leaves the caller to/from a named fd
   ...
   (AT_SHMEM_JOIN_MODE_{READ_ONLY,READ_WRITE}).  Then the region is
   marked as MADVISE_DONTDUMP if the dump flag is 0.  Lastly, any user
   provided at_shmem_join_func_t is called on the mapping.

   A at_shmem_join_func_t is meant to do any additional local address
   ...
   (i.e. the context passed to at_shmem_join) and a pointer to
   ...
   at_shmem_leave is just the inverse of this.  It can fail for a few
   ...
   at_shmem_join/at_shmem_leave to join additional regions as necessary.
   ...
   join/leave cycles (e.g. at_shmem_join("region1") calls join_func
   region1_join("region1") which calls at_shmem_join("region2") which
   calls join_func region2_join("region2") which calls
   shmem_join("region1")).  Such cycles will be detected, logged and
   failed. */

void *
at_shmem_join( char const *               name,
               int                        mode,
               int                        dump,
               at_shmem_joinleave_func_t  join_func,
               void *                     context,
               at_shmem_join_info_t *     opt_info );

int
at_shmem_leave( void *                    join,
                at_shmem_joinleave_func_t leave_func,
                void *                    context );

/* FIXME: CONSIDER OPTION FOR SLIGHTLY MORE ALGO EFFICIENT LEAVE BY NAME
   VARIANT? */

/* at_shmem_join_query_by_{name,join,addr} queries if the cstr pointed
   ...
   are theoretically O(AT_SHMEM_JOIN_MAX) but still quite fast
   practically. */

int
at_shmem_join_query_by_name( char const *           name,
                             at_shmem_join_info_t * opt_info );

int
at_shmem_join_query_by_join( void const *           join,
                             at_shmem_join_info_t * opt_info );

int
at_shmem_join_query_by_addr( void const *           addr,
                             ulong                  sz,
                             at_shmem_join_info_t * opt_info );

/* at_shmem_join_anonymous treats with region pointed to by mem (which
   ...
   Other code in the thread group can at_shmem_join( name, ... ) as
   though the at_shmem_join_anonymous was done for the mapping join for
   ...
   (not AT_SHMEM_JOIN_MODE_{READ_ONLY,READ_WRITE}.

   This will shadow any named shared memory region in the calling thread
   group (but not other thread groups).

   at_shmem_leave_anonymous is just the inverse of this.  Returns 0 on
   ...
   at_shmem_join_anonymous should done only by at_shmem_leave_anonymous.
   Conversely, at_shmem_leave_anonymous should only be used for the
   final leave of any anonymous join. */

int
at_shmem_join_anonymous( char const * name,
                         int          mode,
                         void *       join,
                         void *       mem,
                         ulong        page_sz,
...
int
at_shmem_leave_anonymous( void *                 join,
                          at_shmem_join_info_t * opt_info );
...
/* at_shmem_{numa,cpu}_cnt returns the number of numa nodes / logical
   cpus configured in system.  numa nodes are indexed in
   [0,at_shmem_numa_cnt()) where at_shmem_numa_cnt() is in
   [1,AT_SHMEM_NUMA_MAX] and similarly for logical cpus.  This value is
   determined at thread group boot.  cpu_cnt>=numa_cnt. */

AT_FN_PURE ulong at_shmem_numa_cnt( void );
AT_FN_PURE ulong at_shmem_cpu_cnt ( void );

/* at_shmem_numa_idx returns the closest numa node to the given logical
   cpu_idx.  Given a cpu_idx in [0,at_shmem_cpu_cnt()), returns a value
   in [0,at_shmem_numa_cnt()).  Returns ULONG_MAX otherwise.  The cpu ->
   numa mapping is determined at thread group boot. */

AT_FN_PURE ulong at_shmem_numa_idx( ulong cpu_idx );

/* at_shmem_cpu_idx returns the smallest cpu_idx of a cpu close to
   numa_idx.  Given a numa_idx in [0,at_shmem_numa_cnt()), returns a
   value in [0,at_shmem_cpu_cnt()).  Returns ULONG_MAX otherwise.  The
   numa -> cpu mapping is determined at thread group boot. */

AT_FN_PURE ulong at_shmem_cpu_idx( ulong numa_idx );

/* at_shmem_numa_validate returns 0 if all the pages in the page_cnt
...
int
at_shmem_numa_validate( void const * mem,
                        ulong        page_sz,
                        ulong        page_cnt,
                        ulong        cpu_idx );
...
/* at_shmem_create_multi creates a shared memory region whose name is
   ...
   at_shmem_create is a simple wrapper around at_shmem_create_multi for
...
at_shmem_create_multi( char const *  name,         /* Should point to cstr with a valid name for a shared memory region */
                       ulong         page_sz,      /* Should be a AT_SHMEM_{NORMAL,HUGE,GIGANTIC}_PAGE_SZ */
                       ulong         sub_cnt,      /* Should be positive */
                       ulong const * sub_page_cnt, /* Indexed [0,sub_cnt), 0 < sum(page_cnt)*page_sz <= ULONG_MAX */
                       ulong const * sub_cpu_idx,  /* Indexed [0,sub_cnt), each should be in [0,at_shmem_cpu_cnt()) */
                       ulong         mode );       /* E.g. 0660 for user rw, group rw, world none */

/* at_shmem_update_multi updates a shared memory region created by
   at_shmem_create_multi in place, to be as-if it was created with
...
int
at_shmem_update_multi( char const *  name,
                       ulong         page_sz,
                       ulong         sub_cnt,
...
static inline int
at_shmem_create( char const * name,
                 ulong        page_sz,
                 ulong        page_cnt,
                 ulong        cpu_idx,
                 ulong        mode ) {
  return at_shmem_create_multi( name, page_sz, 1UL, &page_cnt, &cpu_idx, mode );
}

/* at_shmem_unlink removes the name of the page_sz backed shared memory
...
int
at_shmem_unlink( char const * name,
                 ulong        page_sz );

/* at_shmem_info returns info about the given page_sz backed shared
...
int
at_shmem_info( char const *      name,
               ulong             page_sz,
               at_shmem_info_t * opt_info );

/* Raw page allocation */

/* at_shmem_acquire_multi acquires the page_sz pages to create a memory
   ...
   at_shmem_acquire is a simple wrapper around at_shmem_acquire_multi
   for applications that just want to a create a shared memory region
   that contains only 1 subregion. */

void *
at_shmem_acquire_multi( ulong         page_sz,       /* Should be a AT_SHMEM_{NORMAL,HUGE,GIGANTIC}_PAGE_SZ */
                        ulong         sub_cnt,       /* Should be positive */
                        ulong const * sub_page_cnt,  /* Indexed [0,sub_cnt), 0 < sum(page_cnt)*page_sz <= ULONG_MAX */
                        ulong const * sub_cpu_idx ); /* Indexed [0,sub_cnt), each should be in [0,at_shmem_cpu_cnt()) */

static inline void *
at_shmem_acquire( ulong page_sz,
                  ulong page_cnt,
                  ulong cpu_idx ) {
  return at_shmem_acquire_multi( page_sz, 1UL, &page_cnt, &cpu_idx );
}

/* at_shmem_release releases page_cnt page_sz pages of memory allocated
   by at_shmem_acquire.  This always succeeds from the caller's POV but
...
int
at_shmem_release( void * mem,
                  ulong  page_sz,
                  ulong  page_cnt );

/* Parsing APIs */

/* at_shmem_name_len:  If name points at a cstr holding a valid name,
   returns at_strlen( name ) (which is guaranteed to be in
   [1,AT_SHMEM_NAME_MAX)).  Returns 0 otherwise (e.g. name is NULL, name
   is too short, name is too long, name contains characters other than
   [0-9,A-Z,a-z,'_','-','.'], name doesn't start with a [0-9,A-Z,a-z],
   etc). */

AT_FN_PURE ulong at_shmem_name_len( char const * name );

/* at_shmem_page_sz_valid:  Returns 1 if page_sz is a valid page size
   or 0 otherwise. */

AT_FN_CONST static inline int
at_shmem_is_page_sz( ulong page_sz ) {
  return (page_sz==AT_SHMEM_NORMAL_PAGE_SZ) | (page_sz==AT_SHMEM_HUGE_PAGE_SZ) | (page_sz==AT_SHMEM_GIGANTIC_PAGE_SZ);
}

/* at_cstr_to_shmem_lg_page_sz:  Convert a cstr pointed to by cstr to
   a shmem log2 page size (guaranteed to be one of
   AT_SHMEM_*_LG_PAGE_SZ) via case insensitive comparison with various
   tokens and (if none match) at_cstr_to_int.  Returns
   AT_SHMEM_UNKNOWN_LG_PAGE_SZ (-1 ... the only negative return
   possible) if it can't figure this out. */

AT_FN_PURE int
at_cstr_to_shmem_lg_page_sz( char const * cstr );

/* at_shmem_lg_page_sz_to_cstr:  Return a pointer to a cstr
   ...
   at_cstr_to_shmem_lg_page_sz / at_cstr_to_shmem_page_sz.  strlen of
   the returned result will be in in [1,AT_SHMEM_PAGE_SZ_CSTR_MAX]. */

AT_FN_CONST char const *
at_shmem_lg_page_sz_to_cstr( int lg_page_sz );

/* at_cstr_to_shmem_page_sz:  Convert a cstr pointed to by cstr to a
   shmem page size (guaranteed to be one of the AT_SHMEM_*_PAGE_SZ
   values) via case insensitive comparison with various token and (if
   non match) via at_cstr_to_ulong.  Returns AT_SHMEM_UNKNOWN_PAGE_SZ
   (0UL, the only non-integral power of 2 return possible) if it can't
   figure this out. */

AT_FN_PURE ulong
at_cstr_to_shmem_page_sz( char const * cstr );

/* at_shmem_page_sz_to_cstr:  Return a pointer to a cstr corresponding
   ...
   to be compatible with at_cstr_to_shmem_lg_page_sz /
   at_cstr_to_shmem_page_sz.  strlen of the returned result in
   [1,AT_SHMEM_PAGE_SZ_CSTR_MAX].  */

AT_FN_CONST char const *
at_shmem_page_sz_to_cstr( ulong page_sz );

/* at_shmem_iter_begin returns a pointer to the first join info in the
   process. Returns NULL if there are no more joins. */

at_shmem_join_info_t const * at_shmem_iter_begin( void );

/* at_shmem_iter_next returns the next join info in the process.
   Returns NULL if there are no more joins */

at_shmem_join_info_t const * at_shmem_iter_next( at_shmem_join_info_t const * iter );

static inline int at_shmem_iter_done( at_shmem_join_info_t const * iter ) {
  return (iter == NULL);
}

/* These functions are for at_shmem internal use only. */

void
at_shmem_private_boot( int *    pargc,
                       char *** pargv );

void
at_shmem_private_halt( void );

AT_PROTOTYPES_END

#endif /* HEADER_at_src_util_shmem_at_shmem_h */