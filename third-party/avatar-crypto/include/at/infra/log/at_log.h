#ifndef HEADER_at_src_util_log_at_log_h
#define HEADER_at_src_util_log_at_log_h

/* Note: fd must be booted to use the APIs in this module */

/* The at_log conceptually produces up to two log message streams for
   ...
   - AT_LOG_DEBUG messages are not written to either stream (the
     argument list is still processed though so that any side effects of
     the argument list are not lost).

   - AT_LOG_INFO messages are written in detailed form to the log file
     (if the at_log log file is setup).

   - AT_LOG_NOTICE is AT_LOG_INFO + messages are written in summary
     form to stderr.

   - AT_LOG_WARNING is AT_LOG_NOTICE + the log file and stderr are
     flushed to minimize the risk of this message and any preceding not
     making it out before thread resumption.

   - AT_LOG_ERR is AT_LOG_WARNING + the program will be exited with
     an error code of 1.

   - AT_LOG_CRIT and above are AT_LOG_WARNING + the program will
     do a backtrace if possible to the log file and stderr and, after a
     brief delay to let any pending at_log writes complete, aborts the
...
#include "../env/at_env.h"
#include "../io/at_io.h"

/* AT_LOG_NOTICE(( ... printf style arguments ... )) will send a message
   at the NOTICE level to the logger.  E.g. for a typical at_log
   configuration:

     AT_LOG_NOTICE(( "%lu is the loneliest number", 1UL ));
   ...
   AT_LOG_NOTICE has a hexdump counterpart that essentially behaves
   like:

     void
     AT_LOG_HEXDUMP_NOTICE(( char const * tag,
                             void const * mem,
                             ulong        sz ));
   ...
   (e.g. at_strlen(tag) >> 32) due to internal implementation limitations.
   ...
   E.g. for a typical at_log configuration:

     AT_LOG_HEXDUMP_WARNING(( "bad_pkt", pkt, pkt_sz ));
   ...
   Note: at_log_wallclock called outside the arg list to give it a
...
#define AT_LOG_DEBUG(a)           do { long _at_log_msg_now = at_log_wallclock(); at_log_private_1( 0, _at_log_msg_now, __FILE__, __LINE__, __func__, at_log_private_0           a ); } while(0)
#define AT_LOG_INFO(a)            do { long _at_log_msg_now = at_log_wallclock(); at_log_private_1( 1, _at_log_msg_now, __FILE__, __LINE__, __func__, at_log_private_0           a ); } while(0)
#define AT_LOG_NOTICE(a)          do { long _at_log_msg_now = at_log_wallclock(); at_log_private_1( 2, _at_log_msg_now, __FILE__, __LINE__, __func__, at_log_private_0           a ); } while(0)
#define AT_LOG_WARNING(a)         do { long _at_log_msg_now = at_log_wallclock(); at_log_private_1( 3, _at_log_msg_now, __FILE__, __LINE__, __func__, at_log_private_0           a ); } while(0)
#define AT_LOG_ERR(a)             do { long _at_log_msg_now = at_log_wallclock(); at_log_private_2( 4, _at_log_msg_now, __FILE__, __LINE__, __func__, at_log_private_0           a ); } while(0)
#define AT_LOG_CRIT(a)            do { long _at_log_msg_now = at_log_wallclock(); at_log_private_2( 5, _at_log_msg_now, __FILE__, __LINE__, __func__, at_log_private_0           a ); } while(0)
#define AT_LOG_ALERT(a)           do { long _at_log_msg_now = at_log_wallclock(); at_log_private_2( 6, _at_log_msg_now, __FILE__, __LINE__, __func__, at_log_private_0           a ); } while(0)
#define AT_LOG_EMERG(a)           do { long _at_log_msg_now = at_log_wallclock(); at_log_private_2( 7, _at_log_msg_now, __FILE__, __LINE__, __func__, at_log_private_0           a ); } while(0)

#define AT_LOG_HEXDUMP_DEBUG(a)   do { long _at_log_msg_now = at_log_wallclock(); at_log_private_1( 0, _at_log_msg_now, __FILE__, __LINE__, __func__, at_log_private_hexdump_msg a ); } while(0)
#define AT_LOG_HEXDUMP_INFO(a)    do { long _at_log_msg_now = at_log_wallclock(); at_log_private_1( 1, _at_log_msg_now, __FILE__, __LINE__, __func__, at_log_private_hexdump_msg a ); } while(0)
#define AT_LOG_HEXDUMP_NOTICE(a)  do { long _at_log_msg_now = at_log_wallclock(); at_log_private_1( 2, _at_log_msg_now, __FILE__, __LINE__, __func__, at_log_private_hexdump_msg a ); } while(0)
#define AT_LOG_HEXDUMP_WARNING(a) do { long _at_log_msg_now = at_log_wallclock(); at_log_private_1( 3, _at_log_msg_now, __FILE__, __LINE__, __func__, at_log_private_hexdump_msg a ); } while(0)
#define AT_LOG_HEXDUMP_ERR(a)     do { long _at_log_msg_now = at_log_wallclock(); at_log_private_2( 4, _at_log_msg_now, __FILE__, __LINE__, __func__, at_log_private_hexdump_msg a ); } while(0)
#define AT_LOG_HEXDUMP_CRIT(a)    do { long _at_log_msg_now = at_log_wallclock(); at_log_private_2( 5, _at_log_msg_now, __FILE__, __LINE__, __func__, at_log_private_hexdump_msg a ); } while(0)
#define AT_LOG_HEXDUMP_ALERT(a)   do { long _at_log_msg_now = at_log_wallclock(); at_log_private_2( 6, _at_log_msg_now, __FILE__, __LINE__, __func__, at_log_private_hexdump_msg a ); } while(0)
#define AT_LOG_HEXDUMP_EMERG(a)   do { long _at_log_msg_now = at_log_wallclock(); at_log_private_2( 7, _at_log_msg_now, __FILE__, __LINE__, __func__, at_log_private_hexdump_msg a ); } while(0)

/* AT_LOG_STDOUT(()) is used for writing formatted messages to STDOUT, it does not
   take a lock and might interleave with other messages to the same pipe.  It
   should only be used for command output. */
#define AT_LOG_STDOUT(a) do { at_log_private_fprintf_nolock_0( STDOUT_FILENO, "%s", at_log_private_0 a ); } while(0)

/* AT_TEST is a single statement that evaluates condition c and, if c
   evaluates to false, will AT_LOG_ERR that the condition failed.  It is
   ...
   limitations, c cannot contain things like double quotes, etc.  E.g.
     ...
     AT_TEST( broken_func_that_should_return_zero( arg1, arg2 )!=0 );
...
#define AT_TEST(c) do { if( AT_UNLIKELY( !(c) ) ) AT_LOG_ERR(( "FAIL: %s", #c )); } while(0)

/* AT_TEST_CUSTOM is like AT_TEST but with a custom error msg err. */

#define AT_TEST_CUSTOM(c,err) do { if( AT_UNLIKELY( !(c) ) ) AT_LOG_ERR(( "FAIL: %s", (err) )); } while(0)

/* AT_PARANOID / AT_CRIT / AT_ALERT:

   AT_PARANOID configures the AT_CRIT / AT_ALERT runtime checks.

   If AT_PARANOID is set: AT_CRIT / AT_ALERT will AT_LOG_CRIT /
   AT_LOG_ALERT the application if c evaluates to false with a
   descriptive error that includes the user message m (m should evaluate
   to a cstr when c is false).

   If not set: AT_CRIT will evaluate c (such that any side effects of c
   will still happen), the false code path will be marked as unreachable
   (such that the optimizer will treat the code following the AT_CRIT
   the same as when paranoid was set) and m will not be evaluated.
   AT_ALERT will not evaluate c or m.

   In short, use AT_ALERT when c is expensive to evalute but has no side
   effects.  Use AT_CRIT for all other cases.

   FIXME: probably should rename AT_TEST_CUSTOM to AT_ERR. */

#ifndef AT_PARANOID
#define AT_PARANOID 0
#endif

#if AT_PARANOID
#define AT_CRIT( c,m) do { if( AT_UNLIKELY( !(c) ) ) AT_LOG_CRIT (( "FAIL: %s (%s)", #c, (m) )); } while(0)
#define AT_ALERT(c,m) do { if( AT_UNLIKELY( !(c) ) ) AT_LOG_ALERT(( "FAIL: %s (%s)", #c, (m) )); } while(0)
#else
#define AT_CRIT( c,m) do { (void)(c); } while(0)
#define AT_ALERT(c,m) do {            } while(0)
#endif

/* Macros for doing hexedit / tcpdump-like logging of memory regions.
   E.g.

     AT_LOG_NOTICE(( "cache line %016lx\n\t"
                     "%02x: " AT_LOG_HEX16_FMT "\n\t"
                     "%02x: " AT_LOG_HEX16_FMT "\n\t"
                     "%02x: " AT_LOG_HEX16_FMT "\n\t"
                     "%02x: " AT_LOG_HEX16_FMT,
                     (ulong)mem,
                      0U, AT_LOG_HEX16_FMT_ARGS( mem    ),
                     16U, AT_LOG_HEX16_FMT_ARGS( mem+16 ),
                     32U, AT_LOG_HEX16_FMT_ARGS( mem+32 ),
                     48U, AT_LOG_HEX16_FMT_ARGS( mem+48 ) ));
...
#define AT_LOG_HEX16_FMT "%02x %02x %02x %02x %02x %02x %02x %02x  %02x %02x %02x %02x %02x %02x %02x %02x"
#define AT_LOG_HEX16_FMT_ARGS(b)                                      \
...
#define AT_LOG_HEX20_FMT "%02x%02x%02x%02x %02x%02x%02x%02x %02x%02x%02x%02x %02x%02x%02x%02x %02x%02x%02x%02x"
#define AT_LOG_HEX20_FMT_ARGS(b)                                      \
  AT_LOG_HEX16_FMT_ARGS(b),                                           \
  (uint)(((uchar const *)(b))[16]), (uint)(((uchar const *)(b))[17]), \
  (uint)(((uchar const *)(b))[18]), (uint)(((uchar const *)(b))[19])

#define AT_LOG_NAME_MAX (40UL)

AT_PROTOTYPES_BEGIN

/* APPLICATION LOGICAL IDENTIFIERS ************************************/

/* at_log_app_id() returns an integer application id of the application
...
AT_FN_PURE ulong at_log_app_id( void );

/* at_log_app() returns a non-NULL pointer to a cstr describing the
   ...
   from the caller's point of view.  at_strlen(at_log_app()) is in
   [1,AT_LOG_NAME_MAX). */

AT_FN_CONST char const * at_log_app( void ); /* Pointer is CONST, cstr pointed at is PURE */

/* at_log_thread_id() returns the caller's integer thread id.  A thread
...
ulong at_log_thread_id( void );

/* at_log_thread() returns a non-NULL pointer to a cstr describing the
   ...
   set or the caller terminates.  at_strlen(at_log_thread()) is in
   [1,AT_LOG_NAME_MAX). */

char const * at_log_thread( void );

/* at_log_thread_set() sets the caller's description to the cstr
   ...
   the function and the at_log does not retain any interest in name
   after return.  The actual resulting description will be truncated to
   a strlen of AT_LOG_NAME_MAX-1 if name is longer and potentially
   sanitized in other ways as necessary for the log. */

void
at_log_thread_set( char const * name );

/* APPLICATION PHYSICAL IDENTIFIERS ***********************************/

/* at_log_host_id() returns an integer host id of the host on which the
...
AT_FN_PURE ulong at_log_host_id( void );

/* at_log_host() returns a non-NULL pointer to a cstr describing the
   ...
   point of view.  at_strlen(at_log_host()) is in [1,AT_LOG_NAME_MAX). */

AT_FN_CONST char const * at_log_host( void ); /* ptr is CONST, cstr pointed at is PURE */

/* at_log_cpu_id() returns an integer cpu id of one of the cpus on
...
ulong at_log_cpu_id( void );

/* at_log_cpu() returns a non-NULL pointer to a cstr describing the cpu
   ...
   caller's point of view.  at_strlen(at_log_cpu()) is in
   [1,AT_LOG_NAME_MAX). */

char const * at_log_cpu( void );

/* at_log_cpu_set() sets the description of the cpu on which the caller
   ...
   is not changed by the function and the at_log does not retain any
   interest in name after return.  The actual resulting description will
   be truncated to a strlen of AT_LOG_NAME_MAX-1 if name is longer and
   potentially sanitized in other ways as necessary for the log. */

void
at_log_cpu_set( char const * name );

/* THREAD GROUP RELATED IDENTIFIERS ***********************************/

/* at_log_group_id() returns the thread group id of the thread group to
...
AT_FN_PURE ulong at_log_group_id( void );

/* at_log_group() returns a non-NULL pointer to a cstr describing the
...
AT_FN_CONST char const * at_log_group( void ); /* ptr is CONST, cstr pointed at is PURE */

/* at_log_tid() returns the caller's thread group thread id.  A thread
...
ulong at_log_tid( void );

/* at_log_user_id() returns the user id of the thread group to which the
...
AT_FN_PURE ulong at_log_user_id( void );

/* at_log_user() returns a non-NULL pointer to a cstr describing the
   ...
   the first call and the lifetime of the returned string is infinite
   from the caller's point of view.  at_strlen(at_log_user()) is in
   [1,AT_LOG_NAME_MAX). */

AT_FN_CONST char const * at_log_user( void ); /* ptr is CONST, cstr pointed at is PURE */

/* at_log_group_id_query() returns the status of group_id.  Will be a
   AT_LOG_GROUP_ID_QUERY_* code.  Positive indicates live, zero
   indicates dead, negative indicates failure reason. */

#define AT_LOG_GROUP_ID_QUERY_LIVE  (1)  /* group_id is live */
#define AT_LOG_GROUP_ID_QUERY_DEAD  (0)  /* group_id is not live */
#define AT_LOG_GROUP_ID_QUERY_INVAL (-1) /* query failed because invalid group_id (e.g. group_id does to map to a host pid) */
#define AT_LOG_GROUP_ID_QUERY_PERM  (-2) /* query failed because caller lacks permissions */
#define AT_LOG_GROUP_ID_QUERY_FAIL  (-3) /* query failed for unknown reason (should not happen) */

int at_log_group_id_query( ulong group_id );

/* FIXME: TID DESC? */

/* Build info APIs ****************************************************/

/* at_log_build_info points in the caller's address space to the first
   byte of a memory region of size at_log_build_info_sz containing a
...
extern char const  at_log_build_info[] __attribute__((aligned(1)));
extern ulong const at_log_build_info_sz; /* == at_strlen( at_log_build_info ) + 1UL */

/* Logging helper APIs ************************************************/

/* at_log_wallclock_host( NULL ) reads the host's wallclock as ns since
...
long at_log_wallclock_host( void const * _ ); /* at_clock_func_t compat */

/* at_log_wallclock reads the log's timesource to get the ns since the
   UNIX epoch GMT.  By default, this is at_log_wallclock_host but the
   thread group can be configures this to use an alternative time source
   if desired. */

long at_log_wallclock( void ); /* FIXME: Make at_clock_func_t compat */

/* at_log_wallclock_set configures the log to use "clock( args )" as its
...
void
at_log_wallclock_set( at_clock_func_t clock,
                      void const *    args );

/* at_log_wallclock_cstr( t, buf ) pretty prints the wallclock
   measurement t as:
   ...
   AT_LOG_WALLCLOCK_CSTR_BUF_SZ bytes.  Returns buf and buf will be
   populated with the desired cstr on return. */

#define AT_LOG_WALLCLOCK_CSTR_BUF_SZ (37UL)

char *
at_log_wallclock_cstr( long   t,
                       char * buf );

/* at_log_sleep puts the calling thread to sleep for dt ns.  dt<=0 is
   assumed to be a sched_yield request.  Returns the amount of sleep
   remaining if the sleep was interrupted. */

long
at_log_sleep( long dt );

/* at_log_wait_until waits until at_log_wallclock() is at least then.
   ...
   at_log_wallclock).  That is, as the time remaining to wait decreases,
   ...
   If (as is usually the case) at_log_sleep precision is much better
   than <<~100 ms accurate, AT_YIELD() delays take <<~100ms and
   AT_SPIN_PAUSE() << 1 us, the return value will be an accurate read of
   the at_log_wallclock at the time of return and within the overhead of
   at_log_wallclock. */

long
at_log_wait_until( long then );

/* at_log_flush() manually flushes the log (e.g. log a bunch of low
...
int at_log_colorize( void );
int at_log_level_logfile ( void );
int at_log_level_stderr  ( void );
int at_log_level_flush   ( void );
int at_log_level_core    ( void );

void at_log_colorize_set     ( int mode  );
void at_log_level_logfile_set( int level );
void at_log_level_stderr_set ( int level );
void at_log_level_flush_set  ( int level );
void at_log_level_core_set   ( int level );

void at_log_enable_signal_handler( void );
void at_log_enable_unclean_exit( void );

/* These functions are for at_log internal use only. */

void
at_log_private_fprintf_0( int fd, char const * fmt, ... ) __attribute__((format(printf,2,3))); /* Type check the fmt string at compile time */

void
at_log_private_fprintf_nolock_0( int fd, char const * fmt, ... ) __attribute__((format(printf,2,3))); /* Type check the fmt string at compile time */

char const *
at_log_private_0( char const * fmt, ... ) __attribute__((format(printf,1,2))); /* Type check the fmt string at compile time */

void
at_log_private_1( int          level,
                  long         now,
                  char const * file,
                  int          line,
                  char const * func,
...
void
at_log_private_raw_2( char const * file,
                      int          line,
                      char const * func,
                      char const * msg ) __attribute__((noreturn)); /* Let compiler know this will not be returning */

char const *
at_log_private_hexdump_msg( char const * tag,
                            void const * mem,
                            ulong        sz );

void
at_log_private_boot( int *    pargc,
                     char *** pargv );

void
at_log_private_boot_custom( int *        lock,
                            ulong        app_id,
                            char const * app,
                            ulong        thread_id,
                            char const * thread,
...
void
at_log_private_halt( void );

/* Log lock memfd for multi-process log synchronization (Reference alignment) */
int  at_log_create_lock_memfd( void );
void at_log_attach_lock_memfd( int log_lock_fd );

ulong at_log_private_main_stack_sz( void ); /* Returns ulimit -s (if reasonable) on success, 0 on failure (logs details) */

ulong
at_log_private_tid_default( void );

ulong
at_log_private_cpu_id_default( void );

void
at_log_private_stack_discover( ulong   stack_sz,  /* Size the stack is expected to be */
...
void at_log_private_app_id_set   ( ulong app_id    );
void at_log_private_thread_id_set( ulong thread_id );
void at_log_private_host_id_set  ( ulong host_id   );
void at_log_private_cpu_id_set   ( ulong cpu_id    );
void at_log_private_group_id_set ( ulong group_id  );
void at_log_private_tid_set      ( ulong tid       );
void at_log_private_user_id_set  ( ulong user_id   );

void at_log_private_app_set  ( char const * app   ); /* Not thread safe */
void at_log_private_host_set ( char const * host  ); /* Not thread safe */
void at_log_private_group_set( char const * group ); /* Not thread safe */
void at_log_private_user_set ( char const * user  ); /* Not thread safe */
...
/* This is exposed to allow the user to know the expected file descriptor
   ...
   logs and that should be done by the functions in at_log.h */
int at_log_private_logfile_fd( void );

AT_PROTOTYPES_END

#endif /* HEADER_at_src_util_log_at_log_h */
