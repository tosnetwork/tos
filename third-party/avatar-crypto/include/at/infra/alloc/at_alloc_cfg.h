#ifndef HEADER_at_src_util_alloc_at_alloc_cfg_h
#define HEADER_at_src_util_alloc_at_alloc_cfg_h
...
/* The below table was autogenerated.
   ...
   should be passed to at_wksp_alloc to create a "huge" superblock).
   Below is such that at_alloc_malloc will request arbitrarily aligned
   512KiB regions.  This was picked to match the maximum size supported
   by the 19-bit at_alloc_hdr_t offset (used to find in an address space
   ...
   H=512KiB-19B=524269B (where the 19B accounts for a 4B at_alloc_hdr_t
   and a 15B worst case alignment padding since at_alloc_malloc
   ...
   at_alloc_hdr_t sizeclass field).  We chose block counts for these 33
   ...
   byte at_alloc_hdr_t associated with it).  This yields the 126 size
   classes below.
...
/* Requests with worst case footprint greater than
   AT_ALLOC_FOOTPRINT_SMALL_THRESH will be considered as large and done
   directly through the at_wksp_alloc allocator.  Smaller footprints
...
#define AT_ALLOC_FOOTPRINT_SMALL_THRESH (65531UL)

/* AT_ALLOC_SIZECLASS_CNT is the number of sizeclasses supported by this
   implementation.  This should be positive integer of at most 127. */

#define AT_ALLOC_SIZECLASS_CNT (126UL)

/* AT_ALLOC_SIZECLASS_LARGE is a sentinel value used to indicate that
   an allocation is "large" (i.e. that is was done by the at_wksp_alloc
   allocator). */

#define AT_ALLOC_SIZECLASS_LARGE (127UL)

struct __attribute__((aligned(8))) at_alloc_sizeclass_cfg {
...
typedef struct at_alloc_sizeclass_cfg at_alloc_sizeclass_cfg_t;

static at_alloc_sizeclass_cfg_t const at_alloc_sizeclass_cfg[126] = {
...
#endif /* HEADER_at_src_util_alloc_at_alloc_cfg_h */
