#ifndef HEADER_at_src_util_alloc_at_alloc_h
#define HEADER_at_src_util_alloc_at_alloc_h

/* at_alloc is a high performance lockfree fast O(1) (typically)
   allocator.
   ...
   A at_alloc stores its state in a wksp in a persistent way and backs
   ...
   at_alloc_malloc will not stall your program behind your back, calling
   ...
   the underlying wksp.  And, if at_alloc_malloc succeeds, the returned
   memory is real and is ready for use.

   Obligatory dynamic allocation rant *********************************

   That is, at_alloc is not the absolute unforgivable garbage of
   ...
   If you truly need malloc-free semantics, use at_alloc.  This at least
   ...
   at_alloc to go far beyond the capabilities of a typical allocator
   Allocations done by at_alloc can be shared between processes (can
   ...
   O(1) and only "quasi"-lockfree in the sense described in at_wksp.h).
...
#include "../wksp/at_wksp.h"

/* AT_ALLOC_{ALIGN,FOOTPRINT} give the required alignment and footprint
   needed for a wksp allocation to be suitable as a at_alloc.  ALIGN is
   ...
   the minimum alignment of a at_wksp_alloc. */

#define AT_ALLOC_ALIGN     (4096UL)
#define AT_ALLOC_FOOTPRINT (20480UL)

/* AT_ALLOC_MALLOC_ALIGN_DEFAULT gives the alignment that will be used
   ...
   platforms with AT_HAS_INT128.) */

#define AT_ALLOC_MALLOC_ALIGN_DEFAULT (16UL)

/* AT_ALLOC_JOIN_CGROUP_HINT_MAX is maximum value for a cgroup hint.
   This is an integer power of 2 minus 1 of at most AT_ALLOC_ALIGN. */

#define AT_ALLOC_JOIN_CGROUP_HINT_MAX (15UL)

/* A "at_alloc_t *" is an opaque handle of an at_alloc. */
...
/* at_alloc_{align,footprint} return AT_ALLOC_{ALIGN,FOOTPRINT}. */

AT_FN_CONST ulong
at_alloc_align( void );

AT_FN_CONST ulong
at_alloc_footprint( void );

/* at_alloc_new formats an unused wksp allocation with the appropriate
   alignment and footprint as a at_alloc.  Caller is not joined on
   ...
   workspace can have multiple at_alloc created for it.  They will
   dynamically share the underlying workspace along with any other
   non-at_alloc usage but will otherwise act as completely separate
...
void *
at_alloc_new( void * shmem,
              ulong  tag );

/* at_alloc_join joins the caller to a at_alloc.  shalloc points to the
   ...
   wide (note that joins are practically free).  And if using a at_alloc
   ...
   cgroup_hint is not in [0,AT_ALLOC_JOIN_CGROUP_HINT_MAX], it will be
   wrapped to be in that range.

   TL;DR A cgroup_hint of 0 is often a practical choice single threaded.
   A cgroup_hint of at_tile_idx() or just uniform random 64-bit value
   choice in more general situations. */

at_alloc_t *
at_alloc_join( void * shalloc,
               ulong  cgroup_hint );

/* at_alloc_leave leaves an existing join.  Returns the underlying
...
void *
at_alloc_leave( at_alloc_t * join );

/* at_alloc_delete unformats a wksp allocation used as a at_alloc.
   Assumes nobody is or will be joined to the at_alloc.  The caller
   ...
   at_alloc, bad magic, etc. */

void *
at_alloc_delete( void * shalloc );

/* at_alloc_join_cgroup_hint returns the cgroup_hint of the current
   join.  Assumes join is a current local join.  The return will be in
   [0,AT_ALLOC_JOIN_CGROUP_HINT_MAX].

   at_alloc_join_cgroup_hint_set returns join with the cgroup_hint
   updated to provided cgroup_hint.  If cgroup hint is not in
   [0,AT_ALLOC_JOIN_CGROUP_HINT_MAX], it will be wrapped into this
   range.  Assumes join is a current local join.  The return value is
   not a new join. */

AT_FN_CONST static inline ulong
at_alloc_join_cgroup_hint( at_alloc_t * join ) {
  return ((ulong)join) & AT_ALLOC_JOIN_CGROUP_HINT_MAX;
}

AT_FN_CONST static inline at_alloc_t *
at_alloc_join_cgroup_hint_set( at_alloc_t * join,
                               ulong        cgroup_hint ) {
  return (at_alloc_t *)((((ulong)join) & (~AT_ALLOC_JOIN_CGROUP_HINT_MAX)) | (cgroup_hint & AT_ALLOC_JOIN_CGROUP_HINT_MAX));
}

/* at_alloc_wksp returns a pointer to a local wksp join of the wksp
   backing the at_alloc with the current local join.  Caller should not
   call at_alloc_leave on the returned value.  Lifetime of the returned
   wksp handle is as long as the shalloc used on the at_alloc_join is
   still mapped into the caller's address space.

   at_alloc_tag returns the tag that will be used for allocations from
   this workspace. */

AT_FN_PURE at_wksp_t * at_alloc_wksp( at_alloc_t * join ); // NULL indicates NULL join
AT_FN_PURE ulong       at_alloc_tag ( at_alloc_t * join ); // Positive, 0 indicates NULL join

/* at_alloc_malloc_at_least allocates at least sz bytes with alignment
   of at least align from the wksp backing the at_alloc.  join is a
   current local join to the at_alloc.  align should be an integer power
   of 2 or 0.

   An align of 0 indicates to use AT_ALLOC_MALLOC_DEFAULT_ALIGN for the
   request alignment.  This will be large enough such that
   at_alloc_malloc is conformant with C/C++ alignment specifications
   (i.e. can trivially wrap at_alloc_malloc to use as a drop in
   ...
   allocation).  at_alloc is not particularly optimized when align>~sz
   and/or large alignments (>~4096B).  While large values for align are
   supported by at_alloc_malloc, directly using at_wksp_alloc is
   recommended in such cases.

   If an allocation is "large" (align + sz >~ 64KiB for the current
   implementation), it will be handled by at_wksp_alloc under the hood.
   Otherwise, it will be handled by at_alloc_malloc algorithms (which
   are ultimately backed by at_wksp_alloc).  As such, if a small
   ...
   around that sz, etc), this might also fallback on at_wksp_alloc.
   Typically though, after initial allocation and/or program warmup,
   at_alloc_malloc calls will be a reasonably fast O(1) lockfree.
   ...
   via a join to the at_alloc in that thread group, persisted beyond the
   ...
   *max will be zero.
   ...
   at_alloc_malloc is a simple wrapper around at_alloc_malloc_at_least
   for use when applications do not care about the actual size of their
   allocation. */

void *
at_alloc_malloc_at_least( at_alloc_t * join,
                          ulong        align,
                          ulong        sz,
                          ulong *      max );

static inline void *
at_alloc_malloc( at_alloc_t * join,
                 ulong        align,
                 ulong        sz ) {
  ulong max[1];
  return at_alloc_malloc_at_least( join, align, sz, max );
}

/* at_alloc_free frees the outstanding allocation whose first byte is
   pointed to by laddr in the caller's local address space.  join is a
   current local join to the at_alloc.  The caller promises laddr was
   allocated by the underlying at_alloc (but not necessarily on the
   ...
   Like at_alloc_malloc, if the allocation was large, this will be
   handled by at_wksp_free under the hood, which is neither lockfree nor
   ...
   allocations has reduced significantly, at_alloc_free on a small
   allocation might trigger a at_wksp_free to free up wksp space for
   other usage (including uses not through this at_alloc).
...
void
at_alloc_free( at_alloc_t * join,
               void *       laddr );

/* at_alloc_compact frees all wksp allocations that are not required
   for any outstanding user mallocs (note that at_alloc_free lazily
   ...
   at_alloc_compact has the property that it minimizes the amount of
   ...
   Also note that this function is not O(1) and the at_alloc_free lazy
...
void
at_alloc_compact( at_alloc_t * join );

/* at_alloc_is_empty returns 1 if the alloc has no outstanding mallocs
   ...
   at_alloc_compact under the hood.  It assumes the user provided memory
   region holding the alloc state is contained within a region returned
   by a single at_wksp_alloc call (it would be hard to create an alloc
...
int
at_alloc_is_empty( at_alloc_t * join );

/* at_alloc_max_expand computes a recommended value to use for max when
   ...
   we can use something like:
     ...
     foo_max = at_ulong_max( foo_max*gamma, foo_max + foo_delta );
   ...
   silver yields the very practical:
     ...
     new_max = at_ulong_max( max + (max>>2) + (max>>4), max + delta );
   ...
   allocation error trapping):
     ...
     while( ... still appending ... ) {
       ...
       if( AT_UNLIKELY( foo_cnt==foo_max ) ) {
         foo_max = at_alloc_max_expand( foo_max, foo_delta, foo_cnt + foo_delta );
         foo     = (foo_t *)realloc( foo, foo_max*sizeof(foo_t) );
       }
...
AT_FN_CONST static inline ulong       /* new_max, new_max>=max(needed,max), if max<ULONG_MAX, will be new_max>max */
at_alloc_max_expand( ulong max,
                     ulong delta,     /* Assumed > 0 */
                     ulong needed ) {
  ulong t0 = max + delta;               t0 = at_ulong_if( t0<max, ULONG_MAX, t0 ); /* Handle overflow */
  ulong t1 = max + (max>>2) + (max>>4); t1 = at_ulong_if( t1<max, ULONG_MAX, t1 ); /* Handle overflow */
  return at_ulong_max( at_ulong_max( t0, t1 ), needed );
}

AT_PROTOTYPES_END

#endif /* HEADER_at_src_util_alloc_at_alloc_h */