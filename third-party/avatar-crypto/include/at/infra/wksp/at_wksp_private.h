#ifndef HEADER_at_src_util_wksp_at_wksp_private_h
#define HEADER_at_src_util_wksp_at_wksp_private_h

#include "at_wksp.h"

/* If AT_WKSP_LOCK_RECLAIM==0, do not try to recover the lock from
...
#ifndef AT_WKSP_LOCK_RECLAIM
#define AT_WKSP_LOCK_RECLAIM 0
#endif

/* AT_WKSP_PRIVATE_PINFO_IDX_NULL is the pinfo index value used to
   indicate NULL */

#define AT_WKSP_PRIVATE_PINFO_IDX_NULL ((ulong)UINT_MAX)

/* A at_wksp_private_pinfo_t specifies details about a partition in a
   workspace and its relationship to other partitions in that workspace.

   AT_WKSP_PRIVATE_PINFO_ALIGN is an integer power of 2 and
   AT_WKSP_PRIVATE_PINFO_FOOTPRINT will be a multiple of align.
   ...
   - prev_cidx, next_cidx give the index (in compressed form) of the
     ...
     partition idx where idx is in [0,wksp->part_max):
       ...
       ulong prev_idx = at_wksp_private_pinfo_idx( pinfo[ idx ].prev_cidx );
       if( at_wksp_private_pinfo_idx_is_null( prev_idx ) ) {
         ... at this point:
         ...   idx == at_wksp_private_pinfo_idx( wksp->part_head_cidx ) );
         ...   pinfo[ idx ].gaddr_lo == wksp->gaddr_lo );
       } else {
         ... at this point:
         ...   idx ==_at_wksp_private_pinfo_idx( pinfo[ prev_idx ].next_cidx );
         ...   pinfo[ idx ].gaddr_lo == pinfo[ prev_idx ].gaddr_hi;
       }

     and:

       ulong next_idx = at_wksp_private_pinfo_idx( pinfo[ idx ].next_cidx );
       if( at_wksp_private_pinfo_idx_is_null( next_idx ) ) {
         ... at this point:
         ...   idx == at_wksp_private_pinfo_idx( wksp->part_tail_cidx ) );
         ...   pinfo[ idx ].gaddr_hi == wksp->gaddr_hi );
       } else {
         ... at this point:
         ...   idx ==_at_wksp_private_pinfo_idx( pinfo[ next_idx ].prev_cidx );
...
#define AT_WKSP_PRIVATE_PINFO_ALIGN     (64UL) /* At most AT_WKSP_ALIGN */
#define AT_WKSP_PRIVATE_PINFO_FOOTPRINT (64UL)

struct __attribute__((aligned(AT_WKSP_PRIVATE_PINFO_ALIGN))) at_wksp_private_pinfo {
  ...
  uint  prev_cidx;      /* ",                at_wksp_private_pinfo_cidx( AT_WKSP_INFO_IDX_NULL ) */
  uint  next_cidx;      /* ",                at_wksp_private_pinfo_cidx( AT_WKSP_INFO_IDX_NULL ) */
  uint  left_cidx;      /* ",                at_wksp_private_pinfo_cidx( AT_WKSP_INFO_IDX_NULL ) */
  uint  right_cidx;     /* ",                at_wksp_private_pinfo_cidx( AT_WKSP_INFO_IDX_NULL ) */
  uint  parent_cidx;    /* ",                cidx of next idle or at_wksp_private_pinfo_cidx( AT_WKSP_INFO_IDX_NULL ) if no more */
  uint  same_cidx;      /* ",                at_wksp_private_pinfo_cidx( AT_WKSP_INFO_IDX_NULL ) */
  uint  stack_cidx;     /* internal use */
  ulong cycle_tag;      /* internal use */
};

typedef struct at_wksp_private_pinfo at_wksp_private_pinfo_t;

/* AT_WKSP_MAGIC is an ideally unique number that specifies the precise
   memory layout of a at_wksp. */

#define AT_WKSP_MAGIC (0xF17EDA2C3731C591UL) /* F17E=FIRE,DA2C/3R<>DANCER,31/C59<>WKSP,0<>0 --> FIRE DANCER WKSP VERSION 1 */

/* at_wksp_private specifies the detailed layout of the internals of a
   at_wksp_t */

struct at_wksp_private {

  /* This point is AT_WKSP_ALIGN aligned */

  /* This fields are static and mostly in the first cache line */

  ulong magic;                     /* ==AT_WKSP_MAGIC */
  ulong part_max;                  /* Max wksp partitions */
  ulong data_max;                  /* Data region */
  ulong gaddr_lo;                  /* ==at_wksp_private_data_off( part_max ), data region covers offsets [gaddr_lo,gaddr_hi) */
  ulong gaddr_hi;                  /* ==gaddr_lo + data_max,                  offset gaddr_hi is to 1 byte footer */
  char  name[ AT_SHMEM_NAME_MAX ]; /* (Convenience) backing at_shmem region cstr name */
  ...
  /* Padding to AT_WKSP_PRIVATE_PINFO_ALIGN here */

  /* part_max pinfo here */
  /* data_max byte data region here */
  /* 1 footer byte here */
  /* Padding to AT_WKSP_ALIGN here */
};

AT_PROTOTYPES_BEGIN

/* at_wksp_private_pinfo_sz returns the size of a partition in bytes.
   Assumes pinfo points to the pinfo of a partition in a current local
   join.  Will be positive. */

AT_FN_PURE static inline ulong
at_wksp_private_pinfo_sz( at_wksp_private_pinfo_t const * pinfo ) {
  return pinfo->gaddr_hi - pinfo->gaddr_lo;
}

/* at_wksp_private_{part,data}_off return the wksp offset of the
...
at_wksp_private_pinfo_off( void ) {
  return 128UL; /* at_ulong_align_up( sizeof(at_wksp_t), AT_WKSP_PRIVATE_PINFO_ALIGN ); */
}

AT_FN_CONST static inline ulong
at_wksp_private_data_off( ulong part_max ) {
  return at_wksp_private_pinfo_off() + part_max*sizeof(at_wksp_private_pinfo_t);
}

/* at_wksp_private_pinfo returns the location of wksp pinfo array in the
   caller's address space.  Assumes wksp is a current local join.
   at_wksp_private_pinfo_const is a const-correct version. */

AT_FN_CONST static inline at_wksp_private_pinfo_t *
at_wksp_private_pinfo( at_wksp_t * wksp ) {
  return (at_wksp_private_pinfo_t *)(((ulong)wksp) + at_wksp_private_pinfo_off());
}

AT_FN_CONST static inline at_wksp_private_pinfo_t const *
at_wksp_private_pinfo_const( at_wksp_t const * wksp ) {
  return (at_wksp_private_pinfo_t const *)(((ulong)wksp) + at_wksp_private_pinfo_off());
}

/* at_wksp_private_pinfo_{cidx,idx} compresses / uncompresses a pinfo index */

static inline uint  at_wksp_private_pinfo_cidx( ulong idx  ) { return (uint) idx;  }
static inline ulong at_wksp_private_pinfo_idx ( uint  cidx ) { return (ulong)cidx; }

/* at_wksp_private_pinfo_idx_is_null returns 1 if idx is
   AT_WKSP_PRIVATE_PINFO_IDX_NULL and 0 otherwise */

static inline int at_wksp_private_pinfo_idx_is_null( ulong idx ) { return idx==AT_WKSP_PRIVATE_PINFO_IDX_NULL; }

/* pinfo idle stack APIs **********************************************/

/* at_wksp_private_idle_stack_is_empty returns 1 if there are no idle
   partitions and 0 otherwise.  Also returns 1 if corruption is
   detected.  Assumes wksp is a current local join. */

static inline int
at_wksp_private_idle_stack_is_empty( at_wksp_t * wksp ) {
  return at_wksp_private_pinfo_idx( wksp->idle_top_cidx ) >= wksp->part_max;
}

/* at_wksp_private_idle_stack_pop pops an idle partition off wksp's idle
...
at_wksp_private_idle_stack_pop( at_wksp_t *               wksp,     /* Assumes current local join */
                                at_wksp_private_pinfo_t * pinfo ) { /* == at_wksp_private_pinfo( wksp ) */
  ulong i = at_wksp_private_pinfo_idx( wksp->idle_top_cidx );
  wksp->idle_top_cidx = pinfo[ i ].parent_cidx;
  pinfo[ i ].parent_cidx = at_wksp_private_pinfo_cidx( AT_WKSP_PRIVATE_PINFO_IDX_NULL );
  return i;
}

/* at_wksp_private_idle_stack_push pushes partition i onto the idle
   stack.  Assumes the caller knows i is not currently in the idle
   stack, partitioning, used treap or free treap. */

static inline void
at_wksp_private_idle_stack_push( ulong                     i,        /* Assumes in [0,part_max) */
                                 at_wksp_t *               wksp,     /* Assumes current local join */
                                 at_wksp_private_pinfo_t * pinfo ) { /* == at_wksp_private_pinfo( wksp ) */
  pinfo[ i ].gaddr_lo    = 0UL;
  pinfo[ i ].gaddr_hi    = 0UL;
  pinfo[ i ].tag         = 0U;
  pinfo[ i ].in_same     = 0U;
  pinfo[ i ].prev_cidx   = at_wksp_private_pinfo_cidx( AT_WKSP_PRIVATE_PINFO_IDX_NULL );
  pinfo[ i ].next_cidx   = at_wksp_private_pinfo_cidx( AT_WKSP_PRIVATE_PINFO_IDX_NULL );
  pinfo[ i ].left_cidx   = at_wksp_private_pinfo_cidx( AT_WKSP_PRIVATE_PINFO_IDX_NULL );
  pinfo[ i ].right_cidx  = at_wksp_private_pinfo_cidx( AT_WKSP_PRIVATE_PINFO_IDX_NULL );
  pinfo[ i ].same_cidx   = at_wksp_private_pinfo_cidx( AT_WKSP_PRIVATE_PINFO_IDX_NULL );
  pinfo[ i ].parent_cidx = wksp->idle_top_cidx;
  wksp->idle_top_cidx = at_wksp_private_pinfo_cidx( i );
}

/* pinfo used treap APIs **********************************************/

/* at_wksp_private_used_treap_query queries wksp's used treap for the
...
ulong
at_wksp_private_used_treap_query( ulong                     gaddr,
                                  at_wksp_t *               wksp,
                                  at_wksp_private_pinfo_t * pinfo );

/* at_wksp_private_used_treap_insert inserts partition n into wksp's
   ...
   Returns AT_WKSP_SUCCESS (zero) on success and a AT_WKSP_ERR_*
...
int
at_wksp_private_used_treap_insert( ulong                     n,
                                   at_wksp_t *               wksp,    /* Assumes current local join */
                                   at_wksp_private_pinfo_t * pinfo ); /* == at_wksp_private_pinfo( wksp ) */

/* at_wksp_private_used_treap_remove removes partition d from wksp's
...
int
at_wksp_private_used_treap_remove( ulong                     d,
                                   at_wksp_t *               wksp,    /* Assumes current local join */
                                   at_wksp_private_pinfo_t * pinfo ); /* == at_wksp_private_pinfo( wksp ) */

/* pinfo free treap APIs **********************************************/

/* at_wksp_private_free_treap_query queries wksp's free treap for the
...
ulong
at_wksp_private_free_treap_query( ulong                     sz,
                                  at_wksp_t *               wksp,    /* Assumes current local join */
                                  at_wksp_private_pinfo_t * pinfo ); /* == at_wksp_private_pinfo( wksp ) */

/* at_wksp_private_free_treap_insert inserts partition n into wksp's
...
int
at_wksp_private_free_treap_insert( ulong                     n,
                                   at_wksp_t *               wksp,    /* Assumes current local join */
                                   at_wksp_private_pinfo_t * pinfo ); /* == at_wksp_private_pinfo( wksp ) */

/* at_wksp_private_free_treap_same_is_empty returns 1 if the same list
   for d is empty and 0 if not.  Returns 1 if corruption in detected.
   Assumes d is in the free treap. */

static inline int
at_wksp_private_free_treap_same_is_empty( ulong                     d,
                                          at_wksp_t *               wksp,     /* Assumes current local join */
                                          at_wksp_private_pinfo_t * pinfo ) { /* == at_wksp_private_pinfo( wksp ) */
  ulong part_max = wksp->part_max;
  return at_wksp_private_pinfo_idx( pinfo[ d ].same_cidx )>=part_max;
}

/* at_wksp_private_free_treap_same_remove removes the first partition
...
static inline ulong
at_wksp_private_free_treap_same_remove( ulong                     d,
                                        at_wksp_t *               wksp,     /* Assumes current local join */
                                        at_wksp_private_pinfo_t * pinfo ) { /* == at_wksp_private_pinfo( wksp ) */
  ulong part_max = wksp->part_max;
  ulong i = at_wksp_private_pinfo_idx( pinfo[ d ].same_cidx );
  ulong j = at_wksp_private_pinfo_idx( pinfo[ i ].same_cidx );
  /**/             pinfo[ d ].same_cidx = at_wksp_private_pinfo_cidx( j );
  if( j<part_max ) pinfo[ j ].parent_cidx = at_wksp_private_pinfo_cidx( d );
  pinfo[ i ].in_same     = 0U;
  pinfo[ i ].same_cidx   = at_wksp_private_pinfo_cidx( AT_WKSP_PRIVATE_PINFO_IDX_NULL );
  pinfo[ i ].parent_cidx = at_wksp_private_pinfo_cidx( AT_WKSP_PRIVATE_PINFO_IDX_NULL );
  return i;
...
/* at_wksp_private_free_treap_remove removes partition d from wksp's
...
int
at_wksp_private_free_treap_remove( ulong                     d,
                                   at_wksp_t *               wksp,    /* Assumes current local join */
                                   at_wksp_private_pinfo_t * pinfo ); /* == at_wksp_private_pinfo( wksp ) */

/* private admin APIs *************************************************/

/* at_wksp_private_lock locks wksp.  Assumes wksp is a current local
   ...
   operation the caller was doing.  Returns AT_WKSP_SUCCESS (0) if the
   lock was acquired or AT_WKSP_ERR_CORRUPT if the lock could not be
   obtained because memory corruption was detected while trying to
   recover from a dead caller that corrupted the wksp memory. */

int
at_wksp_private_lock( at_wksp_t * wksp );

/* at_wksp_private_unlock unlocks a locked wksp.  Assumes wksp is a
   current local join and the caller has the lock */

static inline void
at_wksp_private_unlock( at_wksp_t * wksp ) {
  AT_COMPILER_MFENCE();
  AT_VOLATILE( wksp->owner ) = ULONG_MAX;
  AT_COMPILER_MFENCE();
}
...
/* AT_WKSP_CHECKPT_{V1,V2}_{BINFO,UINFO}_MAX give the maximum byte size
   (including the terminating '\0') of a decompressed {v1,v2} checkpt
   {build,user} info cstr. */

#define AT_WKSP_CHECKPT_V1_BINFO_MAX (16384UL)
#define AT_WKSP_CHECKPT_V1_UINFO_MAX (16384UL)

#define AT_WKSP_CHECKPT_V2_BINFO_MAX (16384UL)
#define AT_WKSP_CHECKPT_V2_UINFO_MAX (16384UL)

/* A at_wksp_checkpt_v2_hdr_t gives the byte layout of frame 0 of a wksp
   v2 checkpt.  This frame contains the style, compression algo used for
   the info, cgroup and appendix frames and at_wksp_preview information
   uncompressed. */

struct at_wksp_checkpt_v2_hdr {
  ulong magic;                     /* Must be first, ==AT_WKSP_MAGIC */
  ...
  char  name[ AT_SHMEM_NAME_MAX ]; /* cstr holding the original wksp name (note: AT_SHMEM_NAME_MAX==AT_LOG_NAME_MAX==40) */
...
typedef struct at_wksp_checkpt_v2_hdr at_wksp_checkpt_v2_hdr_t;

/* A at_wksp_checkpt_v2_info_t gives the byte layout of frame 1 of a
...
struct at_wksp_checkpt_v2_info {
  ulong mode;
  long  wallclock;
  ulong app_id;
  ulong thread_id;
  ...
  ulong sz_app;    /* in [1,AT_LOG_NAME_MAX ~ 40B] */
  ulong sz_thread; /* " */
  ulong sz_host;   /* " */
  ulong sz_cpu;    /* " */
  ulong sz_group;  /* " */
  ...
  ulong sz_binfo;  /* in [1,AT_WKSP_CHECKPT_V2_BINFO_MAX ~ 16KiB] */
  ulong sz_uinfo;  /* in [1,AT_WKSP_CHECKPT_V2_UINFO_MAX ~ 16KiB] */
};

typedef struct at_wksp_checkpt_v2_info at_wksp_checkpt_v2_info_t;
...
/* A v2 info frame is followed by zero or more volumes.  A volume
   ...
   A at_wksp_checkpt_v2_cmd_t supports writing an arbitrarily large
...
typedef union at_wksp_checkpt_v2_cmd at_wksp_checkpt_v2_cmd_t;

AT_FN_PURE static inline int
at_wksp_checkpt_v2_cmd_is_meta( at_wksp_checkpt_v2_cmd_t const * cmd ) {
  return cmd->meta.tag > 0UL;
}

AT_FN_PURE static inline int
at_wksp_checkpt_v2_cmd_is_data( at_wksp_checkpt_v2_cmd_t const * cmd ) {
  return (cmd->data.tag==0UL) & (cmd->data.cgroup_cnt==ULONG_MAX) & (cmd->data.frame_off==ULONG_MAX);
}

AT_FN_PURE static inline int
at_wksp_checkpt_v2_cmd_is_appendix( at_wksp_checkpt_v2_cmd_t const * cmd ) {
  return (cmd->appendix.tag==0UL) & (cmd->appendix.cgroup_cnt<ULONG_MAX) & (cmd->appendix.frame_off<ULONG_MAX);
}

AT_FN_PURE static inline int
at_wksp_checkpt_v2_cmd_is_volumes( at_wksp_checkpt_v2_cmd_t const * cmd ) {
  return (cmd->volumes.tag==0UL) & (cmd->volumes.cgroup_cnt==ULONG_MAX) & (cmd->volumes.frame_off<ULONG_MAX);
}

/* A at_wksp_checkpt_v2_ftr_t gives the byte layout of the final frame
...
struct at_wksp_checkpt_v2_ftr {
  ...
  int   style;                     /* " */
  ulong unmagic;                   /* ==~AT_WKSP_MAGIC */
};

typedef struct at_wksp_checkpt_v2_ftr at_wksp_checkpt_v2_ftr_t;

/* at_wksp_private_{checkpt,restore,printf}_v1 provide the v1
...
int
at_wksp_private_checkpt_v1( at_tpool_t * tpool,
                            ulong        t0,
                            ulong        t1,
                            at_wksp_t *  wksp,
                            char const * path,
...
int
at_wksp_private_restore_v1( at_tpool_t * tpool,
                            ulong        t0,
                            ulong        t1,
                            at_wksp_t *  wksp,
                            char const * path,
...
int
at_wksp_private_printf_v1( int          fd,
                           char const * path,
                           int          verbose );

/* Similarly for v2.  Note that style==AT_WKSP_CHECKPT_STYLE_V3 in the
   at_wksp_checkpt function becomes a AT_WKSP_CHECKPT_STYLE_V2 with a
   AT_CHECKPT_FRAME_STYLE_LZ4 cgroup frames in the checkpt itself. */

int
at_wksp_private_checkpt_v2( at_tpool_t * tpool,
                            ulong        t0,
                            ulong        t1,
                            at_wksp_t *  wksp,
                            char const * path,
...
int
at_wksp_private_restore_v2( at_tpool_t * tpool,
                            ulong        t0,
                            ulong        t1,
                            at_wksp_t *  wksp,
                            char const * path,
...
int
at_wksp_private_printf_v2( int          fd,
                           char const * path,
                           int          verbose );

AT_PROTOTYPES_END

#endif /* HEADER_at_src_util_wksp_at_wksp_private_h */