/* Declare a bunch of functions for fast manipulation of index sets that
   ...
   Example:
     ...
     #include "at/infra/tmpl/at_set.c"
...
AT_STATIC_ASSERT( 1<=SET_MAX && SET_MAX<=2147483584 /* 2^31-64 */, invalid_set_max );
...
#define SET_(x) AT_EXPAND_THEN_CONCAT3(SET_NAME,_,x)
...
SET_(private_full_last_word)( void ) {
  return at_ulong_mask_lsb( SET_MAX - ((SET_(word_cnt)-1)<<6) );
}

/* Public APIs ********************************************************/

AT_FN_CONST static inline ulong SET_(align)    ( void ) { return alignof(ulong); }
AT_FN_CONST static inline ulong SET_(footprint)( void ) { return 8UL*(ulong)SET_(word_cnt); }

static inline void *
SET_(new)( void * shmem ) {
# if AT_TMPL_USE_HANDHOLDING
  if( AT_UNLIKELY( !at_ulong_is_aligned( (ulong)shmem, SET_(align)() ) ) ) AT_LOG_CRIT(( "unaligned shmem" ));
# endif
  return at_memset( shmem, 0, SET_(footprint)() );
}
...
AT_FN_CONST static inline ulong SET_(max)( SET_(t) const * set ) { (void)set; return (ulong)SET_MAX; }

AT_FN_PURE static inline int
SET_(valid)( SET_(t) const * set ) {
  if( AT_UNLIKELY( !set | !at_ulong_is_aligned( (ulong)set, SET_(align)() ) ) ) return 0;
...
SET_(cnt)( SET_(t) const * set ) {
  ...
  ulong cnt = 0UL;
  for( ulong i=0UL; i<word_cnt; i++ ) cnt += (ulong)at_ulong_popcnt( set[i] );
  return cnt;
}
...
SET_(first)( SET_(t) const * set ) {
  ...
  for( ulong i=0UL; i<word_cnt; i++ ) {
    ulong w = set[i];
    if( w ) return (i<<6) + (ulong)at_ulong_find_lsb( w );
  }
  return ~0UL;
}
...
SET_(last)( SET_(t) const * set ) {
  ...
  for( ulong i=word_cnt; i>0UL; i-- ) {
    ulong w = set[i-1];
    if( w ) return ((i-1)<<6) + (ulong)at_ulong_find_msb( w );
  }
  return ~0UL;
}

__attribute__((warn_unused_result)) AT_FN_UNUSED static ulong /* Work around -Winline */
SET_(iter_next)( SET_(t) * set,
  ...
  for( ulong i=(j>>6); i<word_cnt; i++ ) {           /* For all words with bits we haven't considered */
    ...
    if( w ) {                                        /* If any bits are set in this word */
      set[i] = at_ulong_pop_lsb( w );                /* Clear the lsb */
      return (i<<6) + (ulong)at_ulong_find_lsb( w ); /* And return the index */
    }
  }
  return ~0UL;                                       /* No more bits to consider */
}
static inline ulong SET_(iter_init)( SET_(t) * set ) { return SET_(iter_next)( set, ~0UL ); }
AT_FN_CONST static inline ulong SET_(iter_done)( ulong j ) { return !~j; }

__attribute__((warn_unused_result)) AT_FN_PURE AT_FN_UNUSED static ulong /* Work around -Winline */
SET_(const_iter_next)( SET_(t) const * set,
  ...
  for( ulong i=(j>>6); i<word_cnt; i++ ) {                 /* For all words with bits we haven't considered */
    ...
    if( w ) return (i<<6) + (ulong)at_ulong_find_lsb( w ); /* If any bit is set in this word, return its index */
    m = 0UL;                                               /* Otherwise, continue to next word (haven't considered any bits) */
  }
  return ~0UL;                                             /* No more bits to consider */
}
AT_FN_PURE  static inline ulong SET_(const_iter_init)( SET_(t) const * set ) { return SET_(const_iter_next)( set, ~0UL ); }
AT_FN_CONST static inline ulong SET_(const_iter_done)( ulong j             ) { return !~j; }

static inline SET_(t) *
SET_(insert)( SET_(t) * set,
              ulong     idx ) {
# if AT_TMPL_USE_HANDHOLDING
  if( AT_UNLIKELY( idx>=(ulong)(SET_MAX) ) ) AT_LOG_CRIT(( "idx out of bounds" ));
# endif
...
SET_(insert_if)( SET_(t) * set,
                 ...
                 ulong     idx ) {
# if AT_TMPL_USE_HANDHOLDING
  if( AT_UNLIKELY( c && idx>=(ulong)(SET_MAX) ) ) AT_LOG_CRIT(( "idx out of bounds" ));
# endif
...
SET_(range)( SET_(t) * set,
             ...
             ulong     h ) {
# if AT_TMPL_USE_HANDHOLDING
  if( AT_UNLIKELY( !( (l<=h) & (h<=(ulong)(SET_MAX)) ) ) ) AT_LOG_CRIT(( "invalid range" ));
# endif
  ...
  if( AT_LIKELY( zcnt ) ) set[ word_idx++ ] = ((1UL << at_ulong_min( 64UL-zcnt, h-l ))-1UL) << zcnt; /* opt large range */
  ...
  ulong ocnt = h - at_ulong_min( word_idx<<6, h ); /* in [0,63] */
  if( AT_LIKELY( ocnt ) ) set[ word_idx++ ] = ((1UL << ocnt)-1UL); /* opt large range */
...
# endif
  ...
  word_idx = at_ulong_max( word_idx, h>>6 );
...
# endif
  ...
  ulong zcnt = l & 63UL; // == l - (word_idx<<6); /* In [0,63] */
  if( AT_LIKELY( zcnt ) ) cnt += (ulong)at_ulong_popcnt( set[ word_idx++ ] & (((1UL << at_ulong_min( 64UL-zcnt, h-l ))-1UL) << zcnt) ); /* opt large range */

  /* Handle any complete ones words.  Need to be careful as 64 word_idx
     might already be past h if the range ended in the mixed leading
     word. */

  for( ulong stop_idx=h>>6; word_idx<stop_idx; word_idx++ ) cnt += (ulong)at_ulong_popcnt( set[ word_idx ] );

  /* Handle any mixed trailing word.  Like the above, 64 word_idx might
     already be past h at this point. */

  ulong ocnt = h - at_ulong_min( word_idx<<6, h ); /* in [0,63] */
  if( AT_LIKELY( ocnt ) ) cnt += (ulong)at_ulong_popcnt( set[ word_idx++ ] & ((1UL << ocnt)-1UL) ); /* opt large range */
