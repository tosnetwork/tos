/* Generate prototypes, inlines and/or implementations for
   ...
   at_map_slot_para with an API more like at_map_dynamic.  Namely, like
   at_map_slot_para and beyond at_map_dynamic, this has:

   - First class support for seeded hashing (this has also been
     backported to at_map_dynamic).
   ...
   current translation unit:
     ...
     //     ... insert it
     //
     //   } else if( AT_UNLIKELY( mymap_ele_is_free( ele ) ) ) {
     //
...
#ifndef MAP_KEY_HASH
#define MAP_KEY_HASH(key,seed) at_ulong_hash( (*(key)) ^ (seed) )
#endif
...
#define MAP_STATIC AT_FN_UNUSED static
#else /* library header and/or implementation */
#define MAP_STATIC
#endif

#define MAP_(n) AT_EXPAND_THEN_CONCAT3(MAP_NAME,_,n)
...
AT_FN_PURE MAP_STATIC MAP_ELE_T *
...
AT_FN_CONST static inline ulong MAP_(probe_max_est)( ulong ele_max ) { return ele_max; }

AT_FN_CONST static inline ulong MAP_(align)( void ) { return alignof(MAP_ELE_T); }

AT_FN_CONST static inline ulong
MAP_(footprint)( ulong ele_max ) {
  if( !((ele_max<=(ULONG_MAX / sizeof(MAP_ELE_T))) & at_ulong_is_pow2( ele_max )) ) return 0UL;
...
AT_FN_PURE  static inline MAP_ELE_T *       MAP_(ele0)      ( MAP_(t) *       join ) { return join->ele0;              }
AT_FN_PURE  static inline MAP_ELE_T const * MAP_(ele0_const)( MAP_(t) const * join ) { return join->ele0;              }
AT_FN_PURE  static inline ulong             MAP_(ele_max)   ( MAP_(t) const * join ) { return join->ele_mask + 1UL;    }
AT_FN_PURE  static inline ulong             MAP_(probe_max) ( MAP_(t) const * join ) { return join->probe_rem + 1UL;   }
AT_FN_PURE  static inline ulong             MAP_(seed)      ( MAP_(t) const * join ) { return join->seed;              }
AT_FN_CONST static inline void *            MAP_(ctx)       ( MAP_(t) *       join ) { return join->ctx;               }
AT_FN_CONST static inline void const *      MAP_(ctx_const) ( MAP_(t) const * join ) { return join->ctx;               }
AT_FN_CONST static inline ulong             MAP_(ctx_max)   ( MAP_(t) const * join ) { (void)join; return MAP_CTX_MAX; }
...
AT_FN_PURE static inline MAP_ELE_T *
MAP_(update_fast)( MAP_(t) *         join,
                   MAP_KEY_T const * key,
                   ulong             memo,
                   MAP_ELE_T *       sentinel ) {
# if AT_TMPL_USE_HANDHOLDING
  AT_TEST( memo==MAP_(key_hash)( key, join->seed ) );
# endif
  ...
  if( AT_UNLIKELY( MAP_(ele_is_free)( ele ) ) ) return sentinel;

# if MAP_MEMOIZE && MAP_KEY_EQ_IS_SLOW
  if( AT_LIKELY( ele->MAP_MEMO==memo ) && AT_LIKELY( MAP_(key_eq)( &ele->MAP_KEY, key ) ) ) return ele;
# else
  if( AT_LIKELY( MAP_(key_eq)( &ele->MAP_KEY, key ) ) ) return ele;
# endif
...
# endif
  ...
  if( AT_LIKELY( MAP_(ele_is_free)( ele ) ) ) {
#   if MAP_MEMOIZE
    ele->MAP_MEMO = memo;
#   endif
    ele->MAP_KEY  = *key;
...
# if MAP_MEMOIZE && MAP_KEY_EQ_IS_SLOW
  if( AT_UNLIKELY( ele->MAP_MEMO==memo ) && AT_UNLIKELY( MAP_(key_eq)( &ele->MAP_KEY, key ) ) ) return sentinel;
# else
  if( AT_UNLIKELY( MAP_(key_eq)( &ele->MAP_KEY, key ) ) ) return sentinel;
# endif
...
# endif
  ...
  if( AT_UNLIKELY( MAP_(ele_is_free)( ele ) ) ) {
#   if MAP_MEMOIZE
    ele->MAP_MEMO = memo;
#   endif
    ele->MAP_KEY  = *key;
...
AT_FN_PURE static inline MAP_ELE_T const *
...
# if AT_TMPL_USE_HANDHOLDING
  AT_TEST( at_ulong_is_aligned( (ulong)ele0, alignof(MAP_ELE_T) ) );
  AT_TEST( ele0<=ele );
  AT_TEST( ele<=(ele0+ele_mask) );
  AT_TEST( !MAP_(ele_is_free( ele ) ) );
# endif

  MAP_(private_ele_free)( join->ctx, ele );

  if( AT_UNLIKELY( !MAP_(ele_is_free)( &ele0[ (hole_idx+1UL) & ele_mask ] ) ) ) MAP_(private_remove)( join, hole_idx );
}
...
__attribute__((warn_unused_result)) AT_FN_PURE static inline MAP_(iter_t)
MAP_(iter_next)( MAP_(t) const * join,
  ...
  for(;;) {
    iter++;
    if( AT_UNLIKELY( iter>probe_rem ) ) break;
...
#   if MAP_MEMOIZE
    if( AT_LIKELY( ele->MAP_MEMO==memo ) ) break;
#   else
    if( AT_LIKELY( MAP_(key_hash)( &ele->MAP_KEY, seed )==memo ) ) break;
#   endif
  }
...
AT_FN_PURE static inline MAP_(iter_t)
...
#include "../log/at_log.h" /* Used by constructors and verify (FIXME: Consider making a compile time option) */
...
MAP_(new)( void * shmem,
  ...
  if( AT_UNLIKELY( !at_ulong_is_aligned( (ulong)shmem, MAP_(align)() ) ) ) {
    AT_LOG_WARNING(( "misaligned shmem" ));
    return NULL;
  }

  ulong footprint = MAP_(footprint)( ele_max );
  if( AT_UNLIKELY( !footprint ) ) {
    AT_LOG_WARNING(( "bad ele_max" ));
    return NULL;
  }

  MAP_ELE_T * ele0 = (MAP_ELE_T *)shmem;

  if( reset ) at_memset( ele0, 0, footprint );
...
MAP_(join)( void *      lmem,
  ...
  if( AT_UNLIKELY( !lmem ) ) {
    AT_LOG_WARNING(( "NULL lmem" ));
    return NULL;
  }

  if( AT_UNLIKELY( !at_ulong_is_aligned( (ulong)lmem, alignof(MAP_(t)) ) ) ) {
    AT_LOG_WARNING(( "misaligned lmem" ));
    return NULL;
  }

  if( AT_UNLIKELY( !ele0 ) ) {
    AT_LOG_WARNING(( "NULL ele0" ));
    return NULL;
  }

  if( AT_UNLIKELY( !at_ulong_is_aligned( (ulong)ele0, MAP_(align)() ) ) ) {
    AT_LOG_WARNING(( "misaligned ele0" ));
    return NULL;
  }
  ...
  if( AT_UNLIKELY( !((1UL<=probe_max) & (probe_max<=ele_max)) ) ) {
    AT_LOG_WARNING(( "bad probe_max" ));
    return NULL;
  }
  ...
  at_memset( join, 0, sizeof(MAP_(t)) ); /* note: clears user context */
...
MAP_(delete)( MAP_ELE_T * ele0 ) {
  ...
  if( AT_UNLIKELY( !at_ulong_is_aligned( (ulong)ele0, alignof(MAP_ELE_T) ) ) ) {
    AT_LOG_WARNING(( "misaligned ele0" ));
    return NULL;
  }
...
MAP_(private_update)( MAP_(t) *         join,
  ...
  for( ulong rem=join->probe_rem; rem; rem-- ) {
    ...
    if( AT_UNLIKELY( MAP_(ele_is_free)( ele ) ) ) break;
...
# define MAP_TEST(c) do {                                                        \
    if( AT_UNLIKELY( !(c) ) ) { AT_LOG_WARNING(( "FAIL: %s", #c )); return -1; } \
  } while(0)

  /* Validate join */

  MAP_TEST( join );
  MAP_TEST( at_ulong_is_aligned( (ulong)join, alignof(MAP_(t)) ) );
  ...
  MAP_TEST( ele0                                                   );
  MAP_TEST( at_ulong_is_aligned( (ulong)ele0, alignof(MAP_ELE_T) ) );
  MAP_TEST( ele_max <= (ULONG_MAX / sizeof(MAP_ELE_T))             );
  MAP_TEST( at_ulong_is_pow2( ele_max )                            );
  ...
  for( ulong ele_idx=0UL; ele_idx<ele_max; ele_idx++ ) {
    ...
    if( AT_LIKELY( MAP_(ele_is_free)( ele ) ) ) continue; /* opt for sparse */
...
#   endif
    ...
    ulong probe_cnt = at_ulong_if( ele_idx>=probe_idx, ele_idx - probe_idx, ele_max + ele_idx - probe_idx ) + 1UL;
    ...
    for( ulong probe_rem=probe_cnt; probe_rem; probe_rem-- ) {
      ...
      int found =
#       if MAP_MEMOIZE && MAP_KEY_EQ_IS_SLOW
        AT_LIKELY( probe->MAP_MEMO == ele->MAP_MEMO ) &&
#       endif
