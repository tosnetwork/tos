/* Declares a family of functions useful for single threaded sorting of
   POD types in high performance contexts.  Example usage:
     ...
     #include "at/infra/tmpl/at_sort.c"
   ...
   will create the following API for use in the local compile unit:
     ...
     double *
     sort_double_descend_stable_fast_para( at_tpool_t * tpool, ulong t0, ulong t1,
                                           double * key,
                                           ulong    cnt,
                                           void *   scratch );

     double *
     sort_double_descend_stable_para( at_tpool_t * tpool, ulong t0, ulong t1,
                                      double * key,
                                      ulong    cnt,
                                      void *   scratch );

     double *
     sort_double_descend_inplace_para( at_tpool_t * tpool, ulong t0, ulong t1,
                                       double * key,
                                       ulong    cnt );
     ...
     // to use (e.g. at_tickcount()).  If stable is non-zero, a stable
     ...
     // requested and the target has AT_HAS_ALLOCA.

     double *
     sort_double_descend_fast_para( at_tpool_t * tpool, ulong t0, ulong t1,
                                    double * key,
                                    ulong    cnt,
                                    void *   scratch,
                                    ulong    seed,
...
/* SORT_OVERSAMPLE_RATIO indicates the amount of oversampling fast
   ...
   SORT_PARALLEL is true and AT_HAS_ALLOCA. */
...
#ifndef SORT_IDX_IF
#define SORT_IDX_IF(c,t,f) ((SORT_IDX_T)at_ulong_if( (c), (ulong)(t), (ulong)(f) ))
#endif
...
#define SORT_STATIC AT_FN_UNUSED static
#else /* library header and/or implementation */
#define SORT_STATIC
#endif

#define SORT_(x)AT_EXPAND_THEN_CONCAT3(SORT_NAME,_,x)
...
SORT_(cnt_valid)( SORT_IDX_T cnt ) {
  ...
  SORT_IDX_T max = (SORT_IDX_T)at_ulong_min( (-alignof(SORT_KEY_T))/sizeof(SORT_KEY_T), /* ==floor( (2^64-align-1)/sz ) */
                                             (ulong)(SORT_IDX_T)((1UL<<57)-1UL) );      /* ==SORT_IDX_MAX as ulong */
  return (!cnt) | ((((SORT_IDX_T)0)<cnt) & (cnt<max)) | (cnt==max);
}

AT_FN_CONST static inline ulong SORT_(stable_scratch_align)    ( void )           { return alignof(SORT_KEY_T); }
AT_FN_CONST static inline ulong SORT_(stable_scratch_footprint)( SORT_IDX_T cnt ) { return sizeof (SORT_KEY_T)*(ulong)cnt; }
...
SORT_(stable_fast)( SORT_KEY_T * key,
                    SORT_IDX_T   cnt,
                    void *       scratch ) {
  if( AT_UNLIKELY( cnt<(SORT_IDX_T)2 ) ) return key;
...
SORT_(stable)( SORT_KEY_T * key,
  ...
  if( SORT_(private_merge)( key, (long)cnt, tmp )==tmp ) /* 50/50 branch prob */
    at_memcpy( key, tmp, sizeof(SORT_KEY_T)*(ulong)cnt );
  return key;
}
...
SORT_(split)( SORT_KEY_T const * sorted,
  ...
  for(;;) {
    SORT_IDX_T n = k-j;
    if( AT_UNLIKELY( n<(SORT_IDX_T)1 ) ) break;
...
SORT_STATIC AT_MAP_REDUCE_PROTO( SORT_(private_merge_para)   );
SORT_STATIC AT_FOR_ALL_PROTO   ( SORT_(private_memcpy_para)  );

static inline SORT_KEY_T *
SORT_(stable_fast_para)( at_tpool_t * tpool,
                         ulong        t0,
                         ulong        t1,
                         SORT_KEY_T * key,
                         SORT_IDX_T   cnt,
  ...
  if( AT_UNLIKELY( cnt<2L ) ) return key;

  static ulong const thresh = (4096UL + sizeof(SORT_KEY_T)-1UL) / sizeof(SORT_KEY_T); /* ceil(page_sz/key_sz) */
  ulong t_cnt = at_ulong_min( t1 - t0, ((ulong)cnt) / thresh );
  if( AT_UNLIKELY( t_cnt<2UL ) ) return SORT_(stable_fast)( key, cnt, scratch );
  t1 = t0 + t_cnt;

  SORT_KEY_T * out[1];
  AT_MAP_REDUCE( SORT_(private_merge_para), tpool,t0,t1, 0L,(long)cnt, out, key, scratch );
  return out[0];
}

static inline SORT_KEY_T *
SORT_(stable_para)( at_tpool_t * tpool,
                    ulong        t0,
                    ulong        t1,
                    SORT_KEY_T * key,
                    SORT_IDX_T   cnt,
  ...
  static ulong const thresh = (4096UL + sizeof(SORT_KEY_T)-1UL) / sizeof(SORT_KEY_T); /* ceil(page_sz/key_sz) */
  ulong t_cnt = at_ulong_min( t1 - t0, ((ulong)cnt) / thresh );
  if( AT_UNLIKELY( t_cnt<2UL ) ) return SORT_(stable)( key, cnt, scratch );
  t1 = t0 + t_cnt;

  SORT_KEY_T * out[1];
  AT_MAP_REDUCE( SORT_(private_merge_para), tpool,t0,t1, 0L,(long)cnt, out, key, scratch );
  if( out[0]!=key ) AT_FOR_ALL( SORT_(private_memcpy_para), tpool,t0,t1, 0L,(long)cnt, key, scratch ); /* 50/50 branch prob */
  return key;
}
...
static inline SORT_KEY_T *
SORT_(inplace_para)( at_tpool_t * tpool,
                     ulong        t0,
                     ulong        t1,
                     SORT_KEY_T * key,
                     SORT_IDX_T   cnt ) {
...
SORT_FN_ATTR SORT_STATIC SORT_KEY_T *
SORT_(fast_para)( at_tpool_t * tpool, ulong t0, ulong t1,
                  SORT_KEY_T * key,
                  SORT_IDX_T   cnt,
                  void *       scratch,
                  ulong        seed,
...
# if 1 /* Minimal C language operations */
  for(;;) { /* Note that cnt_left>0 and cnt_right>0 as cnt > SORT_MERGE_THRESH >= 1 at this point */
    if( SORT_BEFORE( key_r[j], key_l[i] ) ) {
      key_m[k++] = key_r[j++];
      if( j>=cnt_r ) {
        at_memcpy( key_m + k, key_l + i, sizeof(SORT_KEY_T)*(ulong)(cnt_l-i) ); /* append left  stragglers (at least one) */
        break;
      }
    } else {
      key_m[k++] = key_l[i++];
      if( i>=cnt_l ) {
        at_memcpy( key_m + k, key_r + j, sizeof(SORT_KEY_T)*(ulong)(cnt_r-j) ); /* append right stragglers (at least one) */
        break;
      }
    }
  }
# else /* Branch free variant (Pyth investigations suggested the above is better) */
  ...
  if(      i<cnt_l ) at_memcpy( key_m + k, key_l + i, sizeof(SORT_KEY_T)*(ulong)(cnt_l-i) );
  else if( j<cnt_r ) at_memcpy( key_m + k, key_r + j, sizeof(SORT_KEY_T)*(ulong)(cnt_r-j) );
# endif
}
...
SORT_(private_quick)( SORT_KEY_T * key,
  ...
  for(;;) {
    ...
    SORT_IDX_T n = h-l;
    if( AT_LIKELY( n <= ((SORT_IDX_T)(SORT_QUICK_THRESH)) ) ) {
...
#     if SORT_QUICK_SWAP_MINIMIZE
      if( AT_LIKELY( j!=h ) ) { key[j] = key[m]; key[m] = kj; } /* ~2/3 prob */
#     else
...
#     endif
    ...
    if( AT_UNLIKELY( (j-i)==n ) ) {
      ...
      if( AT_UNLIKELY( (SORT_BEFORE( p1, p2 )) ) ) { p1 = p2; goto retry; }
      ...
      ulong push_smaller = (ulong)(l_smaller<h_smaller); AT_COMPILER_FORGET( push_smaller );

      /* Immediately process partitions where all keys are the same */
      ulong push_center  = (ulong)(SORT_BEFORE( p1, p2 )); AT_COMPILER_FORGET( push_center );
...
SORT_(private_select)( SORT_KEY_T * key,
  ...
  for(;;) {
    ...
    SORT_IDX_T n = h-l;
    if( AT_LIKELY( n <= ((SORT_IDX_T)(SORT_QUICK_THRESH)) ) ) { SORT_(insert)( key+l, n ); return key; }
...
#     if SORT_QUICK_SWAP_MINIMIZE
      if( AT_LIKELY( j!=h ) ) { key[j] = key[m]; key[m] = kj; }
#     else
      /**/                      key[j] = key[m]; key[m] = kj;
#     endif
    ...
    if( AT_UNLIKELY( rnk< i   ) ) l_next = l, h_next = i;
    if( AT_UNLIKELY( j  <=rnk ) ) l_next = j, h_next = h;
    l = l_next; h = h_next;
  }
  /* never get here */
}
...
SORT_(private_quick_node)( void * _tpool,
  ...
  at_tpool_t * tpool = (at_tpool_t *)     _tpool;
...
#     define WORK_APPROX(cnt) (((ulong)(cnt))*(ulong)((5*at_ulong_find_msb_w_default( (ulong)(cnt), 0 )+7) >> 3))

      struct { SORT_IDX_T l; SORT_IDX_T h; ulong work; } part[3];

      part[0].l = l; part[0].h = i; part[0].work =                                     WORK_APPROX( i-l );
      part[1].l = i; part[1].h = j; part[1].work = at_ulong_if( SORT_BEFORE( p1, p2 ), WORK_APPROX( j-i ), 0UL );
...
#     define ORDER(i,j) at_swap_if( part[j].work > part[i].work, part[i], part[j] )
...
#     undef ORDER
      ...
      for( ulong idx=0UL; idx<3UL; idx++ ) {
        ...
        if( AT_UNLIKELY( !p_work ) ) break; /* no work need for this partition and remainin partitions */

        SORT_IDX_T p_l = part[idx].l;
        SORT_IDX_T p_h = part[idx].h;

        ulong t_cnt = t1 - t0;
        if( AT_UNLIKELY( (t_cnt>1UL) & (p_work<work_remain) ) ) {
          ...
          /* At this point, we have at least two partitions that need
             ...
             Practically, thread_cnt<=AT_TILE_MAX.  For
             AT_TILE_MAX==1024, 14 is sufficient and 82 is sufficient
             for any thread_cnt representable by a ulong. */

          ulong ts = t1 - at_ulong_min( at_ulong_max( (t_cnt*p_work + (work_remain>>1)) / work_remain, 1UL ), t_cnt-1UL );

          at_tpool_exec( tpool, ts, SORT_(private_quick_node), tpool, ts, t1, key + p_l, (void *)(ulong)(p_h - p_l),
  ...
  while( wait_stack_cnt ) at_tpool_wait( tpool, wait_stack[ --wait_stack_cnt ] );
}

AT_MAP_REDUCE_BEGIN( SORT_(private_merge_para), 1L, 0UL, sizeof(SORT_KEY_T *), 1L ) {
...
AT_FOR_ALL_BEGIN( SORT_(private_memcpy_para), 1L ) {

  SORT_KEY_T       * dst = (SORT_KEY_T       *)arg[0];
  SORT_KEY_T const * src = (SORT_KEY_T const *)arg[1];

  if( AT_LIKELY( block_cnt ) ) at_memcpy( dst + block_i0, src + block_i0, sizeof(SORT_KEY_T)*(ulong)block_cnt );

} AT_FOR_ALL_END

#if AT_HAS_ALLOCA

static AT_FOR_ALL_BEGIN( SORT_(private_cntcpy_para), 1L ) {
  ...
  ulong * key_cnt = at_alloca( alignof(ulong), sizeof(ulong)*t_cnt );

  at_memset( key_cnt, 0, sizeof(ulong)*t_cnt );
  ...
  at_memcpy( _key_cnt + (tpool_t0-tpool_base)*t_cnt, key_cnt, sizeof(ulong)*t_cnt );

} AT_FOR_ALL_END

static AT_FOR_ALL_BEGIN( SORT_(private_scatter_para), 1L ) {
  ulong              tpool_base = (ulong             )arg[0];
  ulong              t_cnt      = (ulong             )arg[1];
  ...
  ulong * part = at_alloca( alignof(ulong), sizeof(ulong)*t_cnt );

  at_memcpy( part, _part + (tpool_t0-tpool_base)*t_cnt, sizeof(ulong)*t_cnt );
...
static AT_FOR_ALL_BEGIN( SORT_(private_subsort_para), 1L ) {
...
SORT_(fast_para)( at_tpool_t * tpool, ulong t0, ulong t1,
  ...
  if( AT_UNLIKELY( cnt<(SORT_IDX_T)2 ) ) return key; /* nothing to do */
  ...
  ulong t_cnt  = at_ulong_min( t1 - t0, (cnt*(ulong)at_ulong_find_msb( cnt ) + ((cnt+thresh)>>1)) / thresh );
  if( AT_UNLIKELY( t_cnt<2UL ) ) return stable ? SORT_(stable)( key, cnt, scratch ) : SORT_(inplace)( key, cnt );
  t1 = t0 + t_cnt;
  ...
  /* At this point, we have at least 2 threads available and at least 2
     ...
     - The sampling below is practically uniform IID but it could be
       made more robust with a rejection method (ala at_rng_ulong_roll).
  ...
  SORT_KEY_T * pivot      = at_alloca( alignof(ulong), sizeof(ulong)*sample_cnt );

  for( ulong i=0UL; i<sample_cnt; i++ ) pivot[i] = key[ at_ulong_hash( seed ^ i ) % cnt ];
...
# define part(i,j) part[ (i) + t_cnt*(j) ]
  ...
  ulong * part = at_alloca( alignof(ulong), sizeof(ulong)*t_cnt*t_cnt );

  SORT_KEY_T * tmp = (SORT_KEY_T *)scratch;

  AT_FOR_ALL( SORT_(private_cntcpy_para), tpool,t0,t1, 0L,(long)cnt, t0, t_cnt, part, key, tmp, pivot );
  ...
  AT_FOR_ALL( SORT_(private_scatter_para), tpool,t0,t1, 0L,(long)cnt, t0, t_cnt, part, key, tmp, pivot );
  ...
  AT_FOR_ALL( SORT_(private_subsort_para), tpool,t0,t1, 0L,(long)t_cnt, part, key, tmp, stable );
