#ifndef HEADER_at_src_util_rng_at_rng_h
#define HEADER_at_src_util_rng_at_rng_h
...
/* Simple fast high quality non-cryptographic pseudo random number
   ...
   Assumes at_bits provides a high quality 64<>64-bit integer hash
   functions (i.e. full avalanche) with the property
   at_ulong_hash(0)==0, at_ulong_hash(i) for i in [0,2^64) yields a
   permutation of [0,2^64) and also provides reasonably efficient
   inverse of this. */

#include "../bits/at_bits.h"

/* AT_RNG_{ALIGN,FOOTPRINT} describe the alignment and footprint needed
...
#define AT_RNG_ALIGN     (16UL)
#define AT_RNG_FOOTPRINT (16UL)

/* at_rng_t should be treated as an opaque handle of a pseudo random
   number generator.  (It technically isn't here to facilitate inlining
   of at_rng operations.) */

struct __attribute__((aligned(AT_RNG_ALIGN))) at_rng_private {
  ulong seq;
  ulong idx;
};

typedef struct at_rng_private at_rng_t;

AT_PROTOTYPES_BEGIN

/* Private: at_rng_private_expand(seq) randomly expands an arbitrary
   ...
   that have the property at_ulong_hash(0)==0, the XOR 64-bit const here
   is zero in the lower 32-bits and an arbitrary non-zero in the upper
   32-bits).  For the current at_ulong_hash implementation and XOR
...
at_rng_private_expand( uint seq ) {
  return at_ulong_hash( 0x900df00d00000000UL ^ (ulong)seq );
}

/* Private: at_rng_private_contract(seq) extract the original 32-bit seq
   from its expanded value */

AT_FN_CONST static inline uint
at_rng_private_contract( ulong eseq ) {
  return (uint)at_ulong_hash_inverse( eseq );
}

/* at_rng_{align,footprint} give the needed alignment and footprint
   for a memory region suitable to hold a at_rng's state.  Declaration /
   aligned_alloc / at_alloca friendly (e.g. a memory region declared as
   "at_rng_t _rng[1];", or created by
   "aligned_alloc(alignof(at_rng_t),sizeof(at_rng_t))" or created by
   "at_alloca(alignof(at_rng_t),sizeof(at_rng_t))" will all
   automatically have the needed alignment and footprint).
   at_rng_{align,footprint} return the same value as
   AT_RNG_{ALIGN,FOOTPRINT}.

   at_rng_new takes ownership of the memory region pointed to by mem
   (which is assumed to be non-NULL with the appropriate alignment and
   footprint) and formats it as a at_rng.  The random number generator
   ...
   at_rng_join joins the caller to a memory region holding the state of
   a at_rng.  _rng points to a memory region in the local address space
   that holds a at_rng.  Returns an opaque handle of the local join in
   the local address space to the at_rng (which might not be the same
   ...
   at_rng_leave leaves the current rng join.  Returns a pointer in the
   local address space to the memory region holding the state of the
   at_rng.  The join should not be used afterward.

   at_rng_delete unformats the memory region currently used to hold the
   state of a _rng and returns ownership of the underlying memory region
   to the caller.  There should be no joins in the system on the at_rng.
   Returns a pointer to the underlying memory region. */

AT_FN_CONST static inline ulong at_rng_align    ( void ) { return alignof( at_rng_t ); }
AT_FN_CONST static inline ulong at_rng_footprint( void ) { return sizeof ( at_rng_t ); }

static inline void *
at_rng_new( void * mem,
            uint   seq,
            ulong  idx ) {
  at_rng_t * rng = (at_rng_t *)mem;
  rng->seq = at_rng_private_expand( seq );
  rng->idx = idx;
  return (void *)rng;
}

static inline at_rng_t * at_rng_join  ( void     * _rng ) { return (at_rng_t *)_rng; }
static inline void     * at_rng_leave ( at_rng_t *  rng ) { return (void     *) rng; }
static inline void     * at_rng_delete( void     * _rng ) { return (void     *)_rng; }

/* at_rng_seq returns the sequence used by the rng.  at_rng_idx returns
   the next slot that will be consumed by the rng. */

static inline uint  at_rng_seq( at_rng_t * rng ) { return at_rng_private_contract( rng->seq ); }
static inline ulong at_rng_idx( at_rng_t * rng ) { return rng->idx;                            }

/* at_rng_seq_set sets the sequence to be used by rng and returns
   the replaced value.  at_rng_idx_set sets the next slot that will be
   consumed next by rng and returns the replaced value. */

static inline uint
at_rng_seq_set( at_rng_t * rng,
                uint       seq ) {
  uint old = at_rng_seq( rng );
  rng->seq = at_rng_private_expand( seq );
  return old;
}

static inline ulong
at_rng_idx_set( at_rng_t * rng,
                ulong      idx ) {
  ulong old = at_rng_idx( rng );
  rng->idx = idx;
  return old;
}

/* at_rng_{uchar,ushort,uint,ulong} returns the next integer in the PRNG
   sequence in [0,2^N) for N in {8,16,32,64} respectively with uniform
   probability with a period of 2^64 (at_rng_ulong has a period of 2^63
   as it consumes two slots).  Passes various strict PRNG tests (e.g.
   diehard, dieharder, testu01, etc).  Assumes rng is a current join.
   at_rng_{schar,short,int,long} are the same but return a value in
   ...
   The theory for this that at_ulong_hash(i) for i in [0,2^64) specifies
   ...
   - Is "unpredictable".  That is, the internal state of the generator
     ...
     at_rng_uint could be have been generated from 2^32 internal states
   ...
   at_ulong_hash( perm_j( i ) ) where j is some parameterized family of
   ...
   at_ulong_hash permutation, creating a large number of parallel
...
static inline uchar  at_rng_uchar ( at_rng_t * rng ) { return (uchar )at_ulong_hash( rng->seq ^ (rng->idx++) ); }
static inline ushort at_rng_ushort( at_rng_t * rng ) { return (ushort)at_ulong_hash( rng->seq ^ (rng->idx++) ); }
static inline uint   at_rng_uint  ( at_rng_t * rng ) { return (uint  )at_ulong_hash( rng->seq ^ (rng->idx++) ); }

AT_FN_UNUSED static ulong /* Work around -Winline */
at_rng_ulong( at_rng_t * rng ) {
  ulong hi = (ulong)at_rng_uint( rng );
  return (hi<<32) | (ulong)at_rng_uint( rng );
}

static inline schar at_rng_schar( at_rng_t * rng ) { return (schar)( at_rng_uchar ( rng ) >> 1 ); }
static inline short at_rng_short( at_rng_t * rng ) { return (short)( at_rng_ushort( rng ) >> 1 ); }
static inline int   at_rng_int  ( at_rng_t * rng ) { return (int  )( at_rng_uint  ( rng ) >> 1 ); }
static inline long  at_rng_long ( at_rng_t * rng ) { return (long )( at_rng_ulong ( rng ) >> 1 ); }

#if AT_HAS_INT128
AT_FN_UNUSED static uint128 /* Work around -Winline */
at_rng_uint128( at_rng_t * rng ) {
  return (((uint128)at_rng_ulong( rng ))<<64) | ((uint128)at_rng_ulong( rng ));
}

/* FIXME: MIGHT BE BETTER TO MASK OR (hi<<63) ^ lo */
static inline int128 at_rng_int128( at_rng_t * rng ) { return (int128)( at_rng_uint128( rng ) >> 1 ); }
#endif

/* at_rng_{uint_to_float,ulong_to_double}_{c0,c1,c,o}( u ):  These
   ...
   at_rng_uint_to_float_c0 converter output is as though a continuous
   ...
   Similarly for at_rng_uint_to_float_c1 converter rounds up to the end
   ...
   The at_rng_uint_to_float_c converter rounds toward nearest even
   ...
   The at_rng_uint_to_float_o converter rounds toward the middle of
...
AT_FN_CONST static inline float  at_rng_uint_to_float_c0  ( uint  u ) { return (1.f/(float )(1 <<24))*(float )(int )( u>>(32-24)         ); }
AT_FN_CONST static inline float  at_rng_uint_to_float_c1  ( uint  u ) { return (1.f/(float )(1 <<24))*(float )(int )((u>>(32-24))+   1U  ); }
AT_FN_CONST static inline float  at_rng_uint_to_float_c   ( uint  u ) { return (1.f/(float )(1 <<24))*(float )(int )((u>>(32-24))+(u&1U )); }
AT_FN_CONST static inline float  at_rng_uint_to_float_o   ( uint  u ) { return (1.f/(float )(1 <<24))*(float )(int )((u>>(32-24))|   1U  ); }

#if AT_HAS_DOUBLE
AT_FN_CONST static inline double at_rng_ulong_to_double_c0( ulong u ) { return (1. /(double)(1L<<53))*(double)(long)( u>>(64-53)         ); }
AT_FN_CONST static inline double at_rng_ulong_to_double_c1( ulong u ) { return (1. /(double)(1L<<53))*(double)(long)((u>>(64-53))+   1UL ); }
AT_FN_CONST static inline double at_rng_ulong_to_double_c ( ulong u ) { return (1. /(double)(1L<<53))*(double)(long)((u>>(64-53))+(u&1UL)); }
AT_FN_CONST static inline double at_rng_ulong_to_double_o ( ulong u ) { return (1. /(double)(1L<<53))*(double)(long)((u>>(64-53))|   1UL ); }
#endif

/* at_rng_{float,double}_{c0,c1,c,o} are basic uniform generators on the
...
static inline float  at_rng_float_c0 ( at_rng_t * rng ) { return at_rng_uint_to_float_c0  ( at_rng_uint ( rng ) ); }
static inline float  at_rng_float_c1 ( at_rng_t * rng ) { return at_rng_uint_to_float_c1  ( at_rng_uint ( rng ) ); }
static inline float  at_rng_float_c  ( at_rng_t * rng ) { return at_rng_uint_to_float_c   ( at_rng_uint ( rng ) ); }
static inline float  at_rng_float_o  ( at_rng_t * rng ) { return at_rng_uint_to_float_o   ( at_rng_uint ( rng ) ); }

#if AT_HAS_DOUBLE
static inline double at_rng_double_c0( at_rng_t * rng ) { return at_rng_ulong_to_double_c0( at_rng_ulong( rng ) ); }
static inline double at_rng_double_c1( at_rng_t * rng ) { return at_rng_ulong_to_double_c1( at_rng_ulong( rng ) ); }
static inline double at_rng_double_c ( at_rng_t * rng ) { return at_rng_ulong_to_double_c ( at_rng_ulong( rng ) ); }
static inline double at_rng_double_o ( at_rng_t * rng ) { return at_rng_ulong_to_double_o ( at_rng_ulong( rng ) ); }
#endif

/* at_rng_int_roll uses the given rng to roll an n-sided die where n is
...
static inline uint
at_rng_private_roll32( at_rng_t * rng,
                       uint       n ) {
  uint r = (-n) % n; /* Compute 2^32 mod n = (2^32 - n) mod n = (-n) mod n, compile time for compile time n */
  uint u; do u = at_rng_uint( rng ); while( AT_UNLIKELY( u<r ) ); /* Rejection unlikely (highly unlikely for n<<<2^32) */
...
static inline ulong
at_rng_private_roll64( at_rng_t * rng,
                       ulong      n ) {
  ulong r = (-n) % n; /* Compute 2^64 mod n = (2^64 - n) mod n = (-n) mod n, compile time for compile time n */
  ulong u; do u = at_rng_ulong( rng ); while( AT_UNLIKELY( u<r ) ); /* Rejection unlikely (highly unlikely for n<<<2^64) */
  /* At this point, u is uniform in [r,2^64) which has an integer
     multiple of n elements (thus u % n is in [0,n) uniform) */
  return u % n;
}

static inline uchar  at_rng_uchar_roll ( at_rng_t * rng, uchar  n ) { return (uchar )at_rng_private_roll32( rng, (uint )n ); }
static inline ushort at_rng_ushort_roll( at_rng_t * rng, ushort n ) { return (ushort)at_rng_private_roll32( rng, (uint )n ); }
static inline uint   at_rng_uint_roll  ( at_rng_t * rng, uint   n ) { return (uint  )at_rng_private_roll32( rng, (uint )n ); }
static inline ulong  at_rng_ulong_roll ( at_rng_t * rng, ulong  n ) { return (ulong )at_rng_private_roll64( rng, (ulong)n ); }

static inline schar  at_rng_schar_roll ( at_rng_t * rng, schar  n ) { return (schar )at_rng_private_roll32( rng, (uint )n ); }
static inline short  at_rng_short_roll ( at_rng_t * rng, short  n ) { return (short )at_rng_private_roll32( rng, (uint )n ); }
static inline int    at_rng_int_roll   ( at_rng_t * rng, int    n ) { return (int   )at_rng_private_roll32( rng, (uint )n ); }
static inline long   at_rng_long_roll  ( at_rng_t * rng, long   n ) { return (long  )at_rng_private_roll64( rng, (ulong)n ); }

/* at_rng_coin_tosses tosses a fair coin until it comes up tails and
...
AT_FN_UNUSED static ulong /* Work around -Winline */
at_rng_coin_tosses( at_rng_t * rng ) {
  ulong cnt = 1UL;
  ulong u;
  for(;;) {
    u = (ulong)at_rng_uint( rng );
    if( AT_LIKELY( u ) ) break;
    cnt += 32UL;
  }
  cnt += (ulong)at_ulong_find_lsb( u );
  return cnt;
}

/* at_rng_float_robust generates a uniform random number in [0,1) and
   ...
   at_rng_float_exp generates a random number with an exponential
   distribution.
   ...
   at_rng_float_norm generates a random number with a normal
   distribution.
...
float at_rng_float_robust( at_rng_t * rng );
float at_rng_float_exp   ( at_rng_t * rng );
float at_rng_float_norm  ( at_rng_t * rng );

#if AT_HAS_DOUBLE
double at_rng_double_robust( at_rng_t * rng );
double at_rng_double_exp   ( at_rng_t * rng );
double at_rng_double_norm  ( at_rng_t * rng );
#endif
...
/* at_rng_secure reads random bytes from a cryptographically secure
...
AT_FN_SENSITIVE __attribute__((warn_unused_result))
void *
at_rng_secure( void * d,
               ulong  sz );

AT_PROTOTYPES_END

#endif /* HEADER_at_src_util_rng_at_rng_h */