#ifndef HEADER_at_src_util_tpool_at_tpool_h
#define HEADER_at_src_util_tpool_at_tpool_h
...
/* tpool provides APIs to group sets of tiles together for ultra low
   ...
   explicitly or under the hood and ignoring error trapping):
     ...
     Note: This is example code for illustration of pthread API.
     Production tile code should use at_alloc_malloc() instead of malloc().
   ...
   Together, these turn the above into something like:
     ...
     task_block( void * _args,
       ...
       ulong n0;
       ulong n1;
       AT_TPOOL_PARTITION( 0,N,1, p,P, n0,n1 );
   ...
   The actual function signature used for a at_tpool_task_t below was
   ...
   a powerful use case for scratch memory too):
     ...
     void
     atb_node( at_tpool_t * tpool,       // tpool to use
       ...
       if( t_cnt>1UL ) {  // ... and optionally the task big enough to be worth parallelizing
         ...
         if( AT_UNLIKELY( (m_cnt>l_cnt) & (m_cnt>n_cnt) ) ) { // Split m range
           ...
           float * CR = (float *)at_scratch_alloc( 0UL, l_cnt*n_cnt*sizeof(float) );

           at_tpool_exec( tpool,ts, atb_node,tpool,ts,t1, args, CR,l_cnt, l0,l1, ms,m1, n0,n1 );
           atb_node( tpool,t0,ts, args, C,sC, l0,l1, m0,ms, n0,n1 );
           at_tpool_wait( tpool,ts );

           // Do C([0,l_cnt),[0,n_cnt)) += CR([0,l_cnt),[0,n_cnt)) on threads [t0,t1) here

           at_scratch_pop();

         } else if( AT_UNLIKELY( n_cnt>l_cnt ) ) { // Split n range
           ...
           at_tpool_exec( tpool,ts, atb_node,tpool,ts,t1, args, CR,sC, l0,l1, m0,m1, ns,n1 );
           ...
           at_tpool_exec( tpool,ts, atb_node,tpool,ts,t1, args, CR,sC, ls,l1, m0,m1, n0,n1 );
...
#include "../scratch/at_scratch.h"

/* AT_TPOOL_OPT_SLEEP specifies that tpool threads should sleep from the
...
#define AT_TPOOL_OPT_SLEEP (1UL)

/* AT_TPOOL_TASK_ARG_MAX specifies raw number of number of arguments
   that can be passed to the task.  Different styles of task scheduling
   may impose additional constraints on this. */

#define AT_TPOOL_TASK_ARG_MAX (43UL)

/* AT_TPOOL_PARTITION partitions tasks indexed [task0,task1) over
...
#define AT_TPOOL_PARTITION( task0, task1, lane_cnt, worker_idx, worker_cnt, worker_task0, worker_task1 ) do {                 \
    ...
    ulong _ftp_worker_task0     = _ftp_task0                                                                                  \
       + _ftp_lane_cnt*(_ftp_worker_block_min*_ftp_worker_idx + at_ulong_min(_ftp_worker_idx,_ftp_worker_extra_cnt));         \
    ulong _ftp_worker_task1     = _ftp_worker_task0                                                                           \
       + _ftp_lane_cnt*(_ftp_worker_block_min + ((ulong)(_ftp_worker_idx<_ftp_worker_extra_cnt)))                             \
       + at_ulong_if( _ftp_worker_idx==(_ftp_worker_cnt-1UL), _ftp_block_rem, 0UL );                                          \
    (worker_task0)              = _ftp_worker_task0;                                                                          \
    (worker_task1)              = _ftp_worker_task1;                                                                          \
  } while(0)

/* A at_tpool_t is an opaque handle of a thread pool */

struct at_tpool_private;
typedef struct at_tpool_private at_tpool_t;

/* A at_tpool_task_t is the function signature used for the entry
...
/* A at_tpool_task_v2_t is the function signature used for entry
   point of a v2 task.  This allows passing a lot more args than the
   original at_tpool_task_t.  tpool on entry will be the calling tpool
   and worker_idx will be the tpool worker running the task.  arg_cnt is
   the number of user provided args (in [0,AT_TPOOL_TASK_ARG_MAX]) and
   arg[i] for i in [0,arg_cnt) are the i-th user argument. */

typedef void
(*at_tpool_task_v2_t)( at_tpool_t *  tpool,
                       ulong         worker_idx,
                       ulong         arg_cnt,
                       ulong const * arg );
...
struct __attribute__((aligned(128))) at_tpool_private_worker {
  uint  seq0;                         /* Dispatch rd/wr (ideally wr-only), worker rd-only */
  uint  arg_cnt;                      /* UINT_MAX for a v1 task, or arg count or a v2 task (in [0,AT_TPOOL_TASK_ARG_MAX]) */
  ulong task;                         /* 0 to halt worker, at_tpool_task_t if arg_cnt==UINT_MAX, at_tpool_task_v2_t otherwise */
  ulong arg[ AT_TPOOL_TASK_ARG_MAX ]; /* task arguments, indexed [0,arg_cnt) */
...
typedef struct at_tpool_private_worker at_tpool_private_worker_t;

struct at_tpool_private {

  /* This point is 128 aligned and preceded by the worker0 sentinel */

  ulong opt;        /* Bit-or of AT_TPOOL_OPTs */
  uint  worker_max; /* Positive */
  uint  worker_cnt; /* in [1,worker_max] */

  /* worker_max element at_tpool_private_worker_t * array here, indexed
     ...
     it and we use C++ in at_tpool.cxx to handle people using C++
...
AT_STATIC_ASSERT( AT_TILE_MAX<2048UL, update_implementation );

/* at_tpool_private_worker0 returns a pointer in the local address space
   to the worker0 sentinel */

AT_FN_CONST static inline at_tpool_private_worker_t *
at_tpool_private_worker0( at_tpool_t const * tpool ) {
  return ((at_tpool_private_worker_t *)tpool)-1;
}

/* at_tpool_private_worker returns a pointer in the local address space
   of the first element of the tpool's worker array. */

AT_FN_CONST static inline at_tpool_private_worker_t **
at_tpool_private_worker( at_tpool_t const * tpool ) {
  return (at_tpool_private_worker_t **)(tpool+1);
}

AT_FN_CONST static inline ulong     /* Returns number of elements to left side */
at_tpool_private_split( ulong n ) { /* Assumes n>1 */
...
# else /* NUMA aware splitting */
  ...
  int   b = at_ulong_find_msb( n );
  ulong m = 1UL << (b-1);
  return at_ulong_if( !(n & m), n-m, m<<1 );
# endif
}

#if AT_HAS_THREADS

/* at_tpool_private_wake will wake worker.  Assumes worker is an idle
   sleeper.  This implies among other things worker's tpool was created
   with AT_TPOOL_OPT_SLEEP, the caller is not the worker and the worker
   is not tpool worker 0. */

void
at_tpool_private_wake( at_tpool_private_worker_t * worker );
...
/* AT_TPOOL_{ALIGN,FOOTPRINT} return the alignment and footprint
   ...
   [1,AT_TILE_MAX].  These are provided to facilitate compile time
   construction and for consistency with other constructors.  (FIXME:
   consider using AT_LAYOUT here.) */

#define AT_TPOOL_ALIGN                   (128UL)
#define AT_TPOOL_FOOTPRINT( worker_max ) ( ( sizeof(at_tpool_private_worker_t)                         /* worker0 sentinel */ \
                                           + sizeof(at_tpool_t) +                                      /* tpool header     */ \
                                           + ((ulong)(worker_max))*sizeof(at_tpool_private_worker_t *) /* worker array     */ \
                                           + AT_TPOOL_ALIGN-1UL ) & (~(AT_TPOOL_ALIGN-1UL)) )

AT_PROTOTYPES_BEGIN

/* at_tpool_align returns AT_TPOOL_ALIGN.  at_tpool_footprint returns
   AT_TPOOL_FOOTPRINT( worker_max ) if worker_max is in [1,AT_TILE_MAX]
   or 0 otherwise. */

AT_FN_CONST ulong at_tpool_align( void );
AT_FN_CONST ulong at_tpool_footprint( ulong worker_max );

/* at_tpool_init formats a memory region mem with the appropriate
   alignment and footprint as a thread pool that can support up to
   worker_max worker threads.  worker max must be in [1,AT_TILE_MAX].
   opt is a bit or of AT_TPOOL_OPT specifying additional behaviors of
...
at_tpool_t *
at_tpool_init( void * mem,
               ulong  worker_max,
               ulong  opt );

/* at_tpool_fini pops all worker threads pushed into the tpool and
...
void *
at_tpool_fini( at_tpool_t * tpool );

/* at_tpool_worker_push pushes tile tile_idx into the tpool.  tile_idx
...
at_tpool_t *
at_tpool_worker_push( at_tpool_t * tpool,
                      ulong        tile_idx );

/* at_tpool_worker_pop pops the most recently pushed tpool worker
   thread.
...
at_tpool_t *
at_tpool_worker_pop( at_tpool_t * tpool );
...
AT_FN_PURE static inline ulong at_tpool_opt       ( at_tpool_t const * tpool ) { return tpool->opt;               }
AT_FN_PURE static inline ulong at_tpool_worker_cnt( at_tpool_t const * tpool ) { return (ulong)tpool->worker_cnt; }
AT_FN_PURE static inline ulong at_tpool_worker_max( at_tpool_t const * tpool ) { return (ulong)tpool->worker_max; }

AT_FN_PURE static inline ulong
at_tpool_worker_tile_idx( at_tpool_t const * tpool,
                          ulong              worker_idx ) {
  return (ulong)at_tpool_private_worker( tpool )[ worker_idx ]->tile_idx;
}

/* at_tpool_worker_idle atomically observes the state of tpool thread
...
static inline int
at_tpool_worker_idle( at_tpool_t const * tpool,
                      ulong              worker_idx ) {
  at_tpool_private_worker_t * worker = at_tpool_private_worker( tpool )[ worker_idx ];
...
/* at_tpool_exec calls
   ...
   be usable for additional at_tpool_exec).  As this is used in high
...
static inline void
at_tpool_exec( at_tpool_t *    tpool,       ulong worker_idx,
               at_tpool_task_t task,
               void *          task_tpool,
  ...
  worker->arg_cnt = UINT_MAX; /* Signal at_tpool_exec dispatch */
...
/* at_tpool_wait waits for the tpool thread worker_idx to leave the
...
static inline void
at_tpool_wait( at_tpool_t const * tpool,
               ulong              worker_idx ) {
  at_tpool_private_worker_t * worker = at_tpool_private_worker( tpool )[ worker_idx ];
  uint const * _seq1 = &worker->seq1;
  ...
  for(;;) {
    ...
    uint seq1 = *_seq1;
    AT_COMPILER_MFENCE();
    if( AT_LIKELY( seq0==seq1 ) ) break;
    AT_SPIN_PAUSE();
  }
}

/* Assuming the tasks can be executed safely in any order and/or
   concurrently, at_tpool_exec_all_rrobin, at_tpool_exec_all_block and
   at_tpool_exec_all_taskq are functionally equivalent to:
   ...
   The taskq variant requires AT_HAS_ATOMIC support and assigns tasks to
   ...
   at_tpool_exec_all_batch is functionally equivalent to:

     for( ulong t=t0; t<t1; t++ ) {
       ulong batch_task_l0;
       ulong batch_task_l1;
       AT_TPOOL_PARTITION( task_l0,task_l1,1, t-t0,t1-t0, batch_task_l0,batch_task_l1 );
   ...
   assigned to a thread by at_tpool_exec_all_block.  The difference is
   that at_tpool_exec_all_batch effectively merges all the calls to task
   that at_tpool_exec_all_block would make in a block into a single call
   per thread.

   at_tpool_exec_all_raw is functionally equivalent to:
...
#define AT_TPOOL_EXEC_ALL_DECL(style)                                                                          \
void                                                                                                           \
at_tpool_private_exec_all_##style##_node( void * _node_tpool,                                                  \
                                          ...
                                          ulong  t0,      ulong t1 );                                          \
                                                                                                               \
static inline void                                                                                             \
at_tpool_exec_all_##style( at_tpool_t *    tpool,                                                              \
                           ulong           t0,          ulong t1,                                              \
                           at_tpool_task_t task,                                                               \
                           void *          task_tpool,                                                         \
                           void *          task_args,                                                          \
                           ...
                           ulong           task_l0,     ulong task_l1 ) {                                      \
  AT_COMPILER_MFENCE();                                                                                        \
  at_tpool_private_exec_all_##style##_node( tpool, t0,t1, task_args, task_reduce,task_stride, task_l0,task_l1, \
                                            (ulong)task,(ulong)task_tpool, t0,t1 );                            \
}

AT_TPOOL_EXEC_ALL_DECL(rrobin)
AT_TPOOL_EXEC_ALL_DECL(block)
AT_TPOOL_EXEC_ALL_DECL(batch)
AT_TPOOL_EXEC_ALL_DECL(raw)

#if AT_HAS_ATOMIC
void
at_tpool_private_exec_all_taskq_node( void * _node_tpool,
                                      ulong  node_t0, ulong node_t1,
                                      void * args,
                                      void * reduce,  ulong stride,
                                      ulong  l0,      ulong l1,
...
static inline void
at_tpool_exec_all_taskq( at_tpool_t *    tpool,
                         ulong           t0,          ulong t1,
                         at_tpool_task_t task,
                         void *          task_tpool,
                         void *          task_args,
  ...
  AT_COMPILER_MFENCE();
  at_tpool_private_exec_all_taskq_node( tpool, t0,t1, task_args, task_reduce,task_stride, task_l0,task_l1,
                                        (ulong)task,(ulong)l_next, t0,t1 );
}
#endif

#undef AT_TPOOL_EXEC_ALL_DECL

#include "at_map_reduce.h"

AT_PROTOTYPES_END

#endif /* HEADER_at_src_util_tpool_at_tpool_h */