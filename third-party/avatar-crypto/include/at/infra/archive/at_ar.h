#ifndef HEADER_at_src_util_archive_at_ar_h
#define HEADER_at_src_util_archive_at_ar_h
...
/* AR(5) is a simple archive format combining multiple files into one.
   ...
   Basic usage:
     ...
     at_ar_read_init( stream );
     for(;;) {
       at_ar_meta_t meta[1];
       if( at_ar_read_next( stream, meta ) ) break;
...
/* See note in at_ar.c for details on use of long for these fields. */

#define AT_AR_META_IDENT_SZ (17UL) /* 16 + 1 for '\0' termination */
...
struct at_ar_meta {
  ...
  char ident[ AT_AR_META_IDENT_SZ ]; /* Guaranteed '\0' terminated */
};

typedef struct at_ar_meta at_ar_meta_t;

AT_PROTOTYPES_BEGIN

/* at_ar_read_init starts reading an ar archive in the given stream.  On
   entry, assumes stream is positioned on the first byte of the ar
   magic.  If AT_HAS_HOSTED, stream is FILE * pointer.  Otherwise stream
...
int
at_ar_read_init( void * stream );

/* at_ar_read_next starts reading the archive file in the given stream.
   ...
   processed archive file content.  If AT_HAS_HOSTED, stream should
   ...
   Before at_ar_read_next is called on the get the next file, it should
...
int
at_ar_read_next( void *         stream,
                 at_ar_meta_t * opt_meta );

/* FIXME: CONSIDER AN AT_AR_READ_FINI THAT WOULD MOVE THE STREAM POINTER
   TO THE END OF THE AR FILE? */

AT_PROTOTYPES_END

#endif /* HEADER_at_src_util_archive_at_ar_h */