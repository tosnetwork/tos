#ifndef HEADER_at_src_util_io_at_io_h
#define HEADER_at_src_util_io_at_io_h
...
/* API for platform agnostic high performance stream I/O.  Summary:
   ...
   buf (1<=min<=max):
     ...
     ulong rsz; int err = at_io_read( fd, buf, min, max, &rsz );
     if     ( AT_LIKELY( err==0 ) ) ... success, rsz in [min,max], buf updated
     else if( AT_LIKELY( err< 0 ) ) ... EOF encountered, rsz is [0,min), buf updated
   ...
   min==0:
     ...
     ulong rsz; int err = at_io_read( fd, buf, 0UL, max, &rsz );
     if     ( AT_LIKELY( err==0      ) ) ... success, rsz in [1,max], buf updated
     else if( AT_LIKELY( err==EAGAIN ) ) ... try again later, rsz is zero, buf unchanged
     else if( AT_LIKELY( err< 0      ) ) ... EOF encountered, rsz is zero, buf unchanged
     else                                ... I/O error, rsz is zero, buf clobbered
                                         ... err is strerror compat, err is neither EAGAIN nor EWOULDBLOCK
                                         ... fd should be considered failed
   ...
   (1<=min<=max):
     ...
     ulong wsz; int err = at_io_write( fd, buf, min, max, &wsz );
     if( AT_LIKELY( err==0 ) ) ... success, wsz in [min,max]
   ...
   min==0:
     ...
     ulong wsz; int err = at_io_write( fd, buf, 0UL, max, &wsz );
     if     ( AT_LIKELY( err==0      ) ) ... success, wsz in [1,max]
     else if( AT_LIKELY( err==EAGAIN ) ) ... try again later, wsz is zero
     else                                ... I/O error, wsz is zero, how much of buf was streamed before the error is unknown
                                         ... err is strerror compat, err is neither EAGAIN nor EWOULDBLOCK
                                         ... fd should be considered failed
   ...
   Buffered reads:
     ...
     at_io_buffered_istream_t in[1];
     at_io_buffered_istream_init( in, fd, rbuf, rbuf_sz );
     ... in is initialized and has ownership of fd and rbuf

     ... accessors (these return the values used to init in)

     int    fd      = at_io_buffered_istream_fd     ( in );
     void * rbuf    = at_io_buffered_istream_rbuf   ( in );
     ulong  rbuf_sz = at_io_buffered_istream_rbuf_sz( in );

     ... read sz bytes from a buffered stream

     int err = at_io_buffered_istream_read( in, buf, sz );
     if     ( AT_LIKELY( err==0 ) ) ... success, buf holds the next sz bytes of stream
     else if( AT_LIKELY( err< 0 ) ) ... EOF before sz bytes could be read, buf clobbered
     ...
     int err = at_io_buffered_istream_skip( in, sz );
     if     ( AT_LIKELY( err==0 ) ) ... success, sz bytes skipped
     else if( AT_LIKELY( err< 0 ) ) ... EOF before sz bytes could be skipped
     else                           ... I/O error
                                    ... err is strerror compat, err is neither EAGAIN nor EWOULDBLOCK
                                    ... in and fd should be considered failed
     ...
     ulong        peek_sz = at_io_buffered_istream_peek_sz( in ); ... returns number of bytes currently buffered
     void const * peek    = at_io_buffered_istream_peek   ( in ); ... returns location of current buffered bytes

     at_io_buffered_istream_seek( in, sz ); ... consume sz currently buffered bytes, sz in [0,peek_sz]

     ... read buffering control

     int err = at_io_buffered_istream_fetch( in );
     if     ( AT_LIKELY( err==0      ) ) ... success,         peek_sz updated to at most rbuf_sz
     else if( AT_LIKELY( err< 0      ) ) ... end-of-file,     peek_sz updated to unconsumed bytes remaining (at most rbuf_sz)
     else if( AT_LIKELY( err==EAGAIN ) ) ... try again later, peek_sz unchanged
     ...
     at_io_buffered_istream_fini( in );
   ...
   There are nearly symmetric APIs for buffered writes.
     ...
     at_io_buffered_ostream_t out[1];
     at_io_buffered_ostream_init( out, fd, wbuf, wbuf_sz );
     ... out is initialized and has ownership of fd and wbuf

     ... accessors (these return the values used to init in)

     int    fd      = at_io_buffered_ostream_fd     ( out );
     void * wbuf    = at_io_buffered_ostream_wbuf   ( out );
     ulong  wbuf_sz = at_io_buffered_ostream_wbuf_sz( out );

     ... write sz bytes to a buffered stream

     int err = at_io_buffered_ostream_write( out, buf, sz );
     if( AT_LIKELY( err==0 ) ) ... success, sz bytes from have been written from the caller's POV
     ...
     ulong  peek_sz = at_io_buffered_ostream_peek_sz( out ); ... returns amount of unused write buffer space, in [0,wbuf_sz]
     void * peek    = at_io_buffered_ostream_peek   ( out ); ... returns location of unused write buffer space

     at_io_buffered_ostream_seek( in, sz ); ... commit sz unused bytes of write buffer, sz in [0,peek_sz]

     ... write buffer control

     int err = at_io_buffered_ostream_flush( out );
     if( AT_LIKELY( err==0 ) ) ... success, all buffered bytes have been drained to fd
     else                      ... I/O error, err is strerror compat, err is neither EAGAIN nor EWOULDBLOCK
                               ... out and fd should be considered failed
     ...
     at_io_buffered_ostream_fini( out );
...
/* AT_IO_SEEK_TYPE_{SET,CUR,END} give supported seek types.  They have
   the same meaning as the corresponding lseek SEEK_{SET,CUR_END}. */

#define AT_IO_SEEK_TYPE_SET (0)
#define AT_IO_SEEK_TYPE_CUR (1)
#define AT_IO_SEEK_TYPE_END (2)

/* at_io_buffered_{istream,ostream}_t is an opaque handle of an
   ...
   "at_io_buffered_istream_t in[1];" on the stack to get a suitable
   memory region for the stream state).  These are not meant to be
   persistent or shared IPC. */

struct at_io_buffered_istream_private {
...
typedef struct at_io_buffered_istream_private at_io_buffered_istream_t;

struct at_io_buffered_ostream_private {
  int     fd;        /* Open normal-ish file descriptor of stream */
...
typedef struct at_io_buffered_ostream_private at_io_buffered_ostream_t;

/* AT_IO_MMIO_MODE_* give supported modes for memory mapped I/O */

#define AT_IO_MMIO_MODE_READ_ONLY  (0)
#define AT_IO_MMIO_MODE_READ_WRITE (1)

AT_PROTOTYPES_BEGIN

/* at_io_read streams at least dst_min bytes from the given file
   ...
   - dst_min is positive:
       ...
       ulong dst_sz; int err = at_io_read( fd, dst, dst_min, dst_max, &dst_sz );
       if     ( AT_LIKELY( err==0 ) ) ... success, dst_sz in [dst_min,dst_max], dst updated
       else if( AT_LIKELY( err< 0 ) ) ... EOF, dst_sz in [0,dst_min), dst updated
   ...
   - dst_min is zero and fd is blocking:
       ...
       ulong dst_sz; int err = at_io_read( fd, dst, dst_min, dst_max, &dst_sz );
       if     ( AT_LIKELY( err==0 ) ) ... success, dst_sz in [1,dst_max], dst updated
       else if( AT_LIKELY( err< 0 ) ) ... EOF, dst_sz is zero, dst unchanged
       else                           ... I/O error, dst_sz is zero, dst clobbered
                                      ... err is strerror compat, err is neither EAGAIN nor EWOULDBLOCK
                                      ... fd should be considered failed
   ...
   - dst_min is zero and fd is non-blocking:
       ...
       ulong dst_sz; int err = at_io_read( fd, dst, dst_min, dst_max, &dst_sz );
       if     ( AT_LIKELY( err==0      ) ) ... success, dst_sz in [1,dst_max], dst updated
       else if( AT_LIKELY( err==EAGAIN ) ) ... no data available now, try again later, dst_sz is zero, dst unchanged
       else if( AT_LIKELY( err< 0      ) ) ... EOF, dst_sz is zero, dst unchanged
       else                                ... I/O error, dst_sz is zero, dst clobbered
                                           ... err is strerror compat, err is neither EAGAIN nor EWOULDBLOCK
                                           ... fd should be considered failed
...
/* at_io_write behaves virtually identical to at_io_read but the
   ...
   - src_min is positive:
       ...
       ulong src_sz; int err = at_io_write( fd, src, src_min, src_max, &src_sz );
       if( AT_LIKELY( err==0 ) ) ... success, src_sz in [src_min,src_max]
   ...
   - src_min is zero and fd is blocking:
       ...
       ulong src_sz; int err = at_io_write( fd, src, src_min, src_max, &src_sz );
       if( AT_LIKELY( err==0 ) ) ... success, src_sz in [1,src_max]
       else                      ... I/O error, src_sz is zero
                                 ... err is strerror compat, err is neither EAGAIN nor EWOULDBLOCK
                                 ... fd should be considered failed
   ...
   - src_min is zero and fd is non-blocking:
       ...
       ulong src_sz; int err = at_io_write( fd, src, src_min, src_max, &src_sz );
       if     ( AT_LIKELY( err==0      ) ) ... success, src_sz in [1,src_max]
       else if( AT_LIKELY( err==EAGAIN ) ) ... no bytes written, try again later, src_sz is zero
       else                                ... I/O error, src_sz is zero
                                           ... err is strerror compat, err is neither EAGAIN nor EWOULDBLOCK
                                           ... fd should be considered failed
...
/* at_io_sz returns the current byte size of the file underlying the
   ...
   the current size (in [0,LONG_MAX], not ULONG_MAX for at_io_seek
...
/* at_io_truncate truncates the file underlying the given file descriptor
...
int
at_io_truncate( int   fd,
                ulong sz );

/* at_io_seek seeks the byte index of the given file descriptor
   ...
   defined behaviors but portable code should not assume them).
     ...
     type==AT_IO_SEEK_TYPE_SET: the index will be seeked rel_off bytes
     from SOF / the first byte of the file.  Supported rel_off are in
     [0,sz].

     type==AT_IO_SEEK_TYPE_CUR: the index will be seeked rel_off bytes
     from the current byte index.  Supported rel_off are in
     [-idx,sz-idx].

     type==AT_IO_SEEK_TYPE_END: the index will be seeked rel_off bytes
  ...
  Note that, if the file descriptor supports it:
    ...
    at_io_seek( fd, 0L, AT_IO_SEEK_TYPE_CUR, &idx );
...
/* at_io_buffered_read is like at_io_read but can consolidate many
   tiny reads into a larger at_io_read via the given buffer.  Unlike
   at_io_read, dst NULL is okay if dst_sz is 0 (dst_sz 0 is a no-op that
   ...
   *_rbuf_lo and *_rbuf_ready will still hold.
   ...
   for implementing higher level APIs like at_io_buffered_istream below. */

int
at_io_buffered_read( int     fd,
                     void *  dst,
                     ulong   dst_sz,
                     void *  rbuf,
                     ulong   rbuf_sz,
...
/* at_io_buffered_skip is like at_io_buffered_read but will skip over
...
/* at_io_buffered_write is like at_io_write but can consolidate many
   tiny writes into a larger at_io_write via the given buffer.  Unlike
   at_io_write, src NULL is okay if src_sz is 0 (src_sz 0 is a no-op
   ...
   This API usually should not be used directly.  It is mostly useful
   for implementing higher level APIs like at_io_buffered_ostream below. */

int
at_io_buffered_write( int          fd,
                      void const * src,
                      ulong        src_sz,
                      void *       wbuf,
                      ulong        wbuf_sz,
...
/* at_io_buffered_istream_init initializes in to do buffered reads from
...
static inline at_io_buffered_istream_t *
at_io_buffered_istream_init( at_io_buffered_istream_t * in,
                             int                        fd,
                             void *                     rbuf,
                             ulong                      rbuf_sz ) {
  in->fd         = fd;
...
/* at_io_buffered_istream_{fd,rbuf,rbuf_sz} return the corresponding
   value used to initialize in.  Assumes in is initialized. */

AT_FN_PURE static inline int    at_io_buffered_istream_fd     ( at_io_buffered_istream_t const * in ) { return in->fd;      }
AT_FN_PURE static inline void * at_io_buffered_istream_rbuf   ( at_io_buffered_istream_t const * in ) { return in->rbuf;    }
AT_FN_PURE static inline ulong  at_io_buffered_istream_rbuf_sz( at_io_buffered_istream_t const * in ) { return in->rbuf_sz; }

/* at_io_buffered_istream_fini finalizes a buffered input stream.
...
static inline void
at_io_buffered_istream_fini( at_io_buffered_istream_t * in ) {
  (void)in;
}

/* at_io_buffered_istream_read reads dst_sz bytes from in to dst,
   reading ahead as convenient.  Assumes in is initialized.  dst /
   dst_sz have the same meaning / restrictions as at_io_buffered_read.
   Returns 0 on success and non-zero on failure.  Failure interpretation
   is the same as at_io_buffered_read.  On failure, in and the
   ...
   IMPORTANT!  If at_io_buffered_istream_{fetch,skip} below are never
...
AT_FN_UNUSED static int /* Work around -Winline */
at_io_buffered_istream_read( at_io_buffered_istream_t * in,
                             void *                     dst,
                             ulong                      dst_sz ) {
  ...
  int err = at_io_buffered_read( in->fd, dst, dst_sz, in->rbuf, in->rbuf_sz, &rbuf_lo, &rbuf_ready );
  in->rbuf_lo    = rbuf_lo;
  in->rbuf_ready = rbuf_ready;
  return err;
}

/* at_io_buffered_istream_skip skips skip_sz bytes from in.  Assumes in
   is initialized.  Returns 0 on success and non-zero on failure.
   Failure interpretation is the same as at_io_buffered_read.  On a
   ...
   IMPORTANT!  See note in at_io_buffered_istream_read above about the
   impact of this on file pointer alignment. */

static inline int
at_io_buffered_istream_skip( at_io_buffered_istream_t * in,
                             ulong                      skip_sz ) {
  /* We destructure in to avoid pointer escapes that might inhibit
     optimizations of other in inlines. */
  ulong rbuf_lo    = in->rbuf_lo;
  ulong rbuf_ready = in->rbuf_ready;
  int err = at_io_buffered_skip( in->fd, skip_sz, in->rbuf, in->rbuf_sz, &rbuf_lo, &rbuf_ready );
  in->rbuf_lo    = rbuf_lo;
  in->rbuf_ready = rbuf_ready;
  return err;
}

/* at_io_buffered_istream_peek returns a pointer in the caller's address
...
at_io_buffered_istream_peek( at_io_buffered_istream_t * in ) {
  return in->rbuf + in->rbuf_lo;
}

/* at_io_buffered_istream_peek_sz returns the number of bytes that have
...
at_io_buffered_istream_peek_sz( at_io_buffered_istream_t * in ) {
  return in->rbuf_ready;
}

/* at_io_buffered_istream_seek consumes sz buffered bytes from in.
   Assumes in is initialized and that sz is at most peek_sz. */

static inline void
at_io_buffered_istream_seek( at_io_buffered_istream_t * in,
                             ulong                      sz ) {
  in->rbuf_lo    += sz;
  in->rbuf_ready -= sz;
}

/* at_io_buffered_istream_fetch tries to fill up the stream's read
   ...
   close).  That is:
     ...
     int err = at_io_buffered_istream_fetch( in );
     if(      AT_LIKELY( err==0      ) ) ... success,     peek_sz() updated to at most rbuf_sz
     else if( AT_LIKELY( err< 0      ) ) ... end-of-file, peek_sz() updated to at most rbuf_sz and is num unconsumed bytes to EOF
     else if( AT_LIKELY( err==EAGAIN ) ) ... try again,   peek_sz() unchanged, only possible if fd is non-blocking
     else                                ... I/O error,   peek_sz() unchanged,
                                         ... err is strerror compat, err is neither EAGAIN nor EWOULDBLOCK
                                         ... in and fd should be considered failed
...
AT_FN_UNUSED static int /* Work around -Winline */
at_io_buffered_istream_fetch( at_io_buffered_istream_t * in ) {
  uchar * rbuf       = in->rbuf;
  ulong   rbuf_sz    = in->rbuf_sz;
  ulong   rbuf_lo    = in->rbuf_lo;
  ulong   rbuf_ready = in->rbuf_ready;
  if( AT_UNLIKELY( rbuf_ready>=rbuf_sz ) ) return 0; /* buffer already full */
  if( AT_LIKELY( (!!rbuf_ready) & (!!rbuf_lo) ) ) at_memmove( rbuf, rbuf+rbuf_lo, rbuf_ready ); /* Move unconsumed to beginning */
  ulong   rsz;
  int     err = at_io_read( in->fd, rbuf+rbuf_ready, 0UL, rbuf_sz-rbuf_ready, &rsz );
  in->rbuf_lo    = 0UL;
  in->rbuf_ready = rbuf_ready + rsz;
  return err;
}

/* at_io_buffered_ostream_init initializes out to do buffered writes to
...
static inline at_io_buffered_ostream_t *
at_io_buffered_ostream_init( at_io_buffered_ostream_t * out,
                             int                        fd,
                             void *                     wbuf,
                             ulong                      wbuf_sz ) {
  out->fd        = fd;
...
/* at_io_buffered_ostream_{fd,wbuf,wbuf_sz} return the corresponding
   value used to initialize out.  Assumes out is initialized. */

AT_FN_PURE static inline int    at_io_buffered_ostream_fd     ( at_io_buffered_ostream_t const * out ) { return out->fd;      }
AT_FN_PURE static inline void * at_io_buffered_ostream_wbuf   ( at_io_buffered_ostream_t const * out ) { return out->wbuf;    }
AT_FN_PURE static inline ulong  at_io_buffered_ostream_wbuf_sz( at_io_buffered_ostream_t const * out ) { return out->wbuf_sz; }

/* at_io_buffered_ostream_fini finalizes a buffered output stream.
...
static inline void
at_io_buffered_ostream_fini( at_io_buffered_ostream_t * out ) {
  (void)out;
}

/* at_io_buffered_ostream_write writes src_sz bytes from src to the
   stream, temporarily buffering zero or more bytes as convenient.
   Assume out is initialized.  src / src_sz have the same meaning /
   restrictions as at_io_buffered_write.  Returns 0 on success and
   non-zero on failure.  Failure interpretation is the same as
   at_io_buffered_write.  On failure, out and the underlying file
   ...
   IMPORTANT!  If at_io_buffered_ostream_flush is only used to do a
...
static inline int
at_io_buffered_ostream_write( at_io_buffered_ostream_t * out,
                              void const *               src,
                              ulong                      src_sz ) {
  ...
  int   err = at_io_buffered_write( out->fd, src, src_sz, out->wbuf, out->wbuf_sz, &wsz );
  out->wbuf_used = wsz;
  return err;
}

/* at_io_buffered_ostream_peek returns a pointer in the caller's address
...
at_io_buffered_ostream_peek( at_io_buffered_ostream_t * out ) {
  return out->wbuf + out->wbuf_used;
}

/* at_io_buffered_istream_peek_sz returns the number of bytes available
...
AT_FN_PURE static inline ulong
at_io_buffered_ostream_peek_sz( at_io_buffered_ostream_t * out ) {
  return out->wbuf_sz - out->wbuf_used;
}

/* at_io_buffered_istream_seek commits the next sz unused write buffer
   bytes to be streamed out.  Assumes out is initialized and that sz is
   at most peek_sz. */

static inline void
at_io_buffered_ostream_seek( at_io_buffered_ostream_t * out,
                             ulong                      sz ) {
  out->wbuf_used += sz;
}

/* at_io_buffered_ostream_flush writes any buffered bytes in the stream's
   ...
   valid thing to do to fd is close).
     ...
     int err = at_io_buffered_ostream_flush( out );
     if( AT_LIKELY( err==0 ) ) ... success,   write buffer empty
     else                      ... I/O error, write buffer empty
                               ... err is strerror compat, err is neither EAGAIN nor EWOULDBLOCK
                               ... in and fd should be considered failed

   IMPORTANT!  See note in at_io_buffered_ostream_write below about the
   impact of doing this outside a final flush. */

AT_FN_UNUSED static int /* Work around -Winline */
at_io_buffered_ostream_flush( at_io_buffered_ostream_t * out ) {
  ulong wbuf_used = out->wbuf_used;
  if( AT_UNLIKELY( !wbuf_used ) ) return 0; /* optimize for lots of tiny writes */
  out->wbuf_used = 0UL;
  ulong wsz;
  return at_io_write( out->fd, out->wbuf, wbuf_used, wbuf_used, &wsz );
}

/* Memory mapped I/O APIs */

/* at_io_mmio_init starts memory mapped I/O on the file underlying
   the given file descriptor.  Specifically, it maps the file into the
   caller's address space according to the given AT_IO_MMIO_MODE.
   ...
   failure include all usual the at_io_sz reasons and mmap reasons (e.g.
...
int
at_io_mmio_init( int     fd,
                 int     mode,
                 void *  _mmio,      /* Psychologically, a "caller_mmio_t **" */
                 ulong * _mmio_sz );

/* at_io_mmio_fini finishes memory mapped I/O on a file.  mmio and
   ...
   to fail from the caller's point of view.  See at_shmem_mmio_init for
   more details. */

void
at_io_mmio_fini( void const * mmio,
                 ulong        mmio_sz );

/* TODO: consider a at_io_mmio_sync API to allow changes to an
   in-progress mmio to be made visible? */

/* Misc APIs */

/* at_io_strerror converts an at_io error code (i.e. negative ->
...
at_io_strerror( int err );

/* at_io_strsignal converts a signal code (like returned by WTERMSIG)
...
AT_FN_CONST char const *
at_io_strsignal( int err );

/* TODO: ASYNC IO APIS */

AT_PROTOTYPES_END

#endif /* HEADER_at_src_util_io_at_io_h */