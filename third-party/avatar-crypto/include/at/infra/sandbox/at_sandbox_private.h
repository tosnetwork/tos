#ifndef HEADER_at_src_util_sandbox_at_sandbox_private_h
#define HEADER_at_src_util_sandbox_at_sandbox_private_h

#include "at_sandbox.h"
...
/* Clears the environment variables of the process by explicitly zeroing
   ...
   Reference does not use environment variables to pass information,
...
at_sandbox_private_explicit_clear_environment_variables( void );
...
/* Checks that the open file descriptors in the current process exactly
   ...
   spawned the Reference process with unexpected file descriptors still
   open.
...
void
at_sandbox_private_check_exact_file_descriptors( ulong       allowed_file_descriptor_cnt /* Must be in [0, 256] */,
...
void
at_sandbox_private_switch_uid_gid( uint desired_uid,
                                   uint desired_gid );
...
void
at_sandbox_private_write_userns_uid_gid_maps( uint uid_in_parent,   /* The UID in the parent to map UID 1 in the namespace to */
...
void
at_sandbox_private_deny_namespaces( void );
...
void
at_sandbox_private_pivot_root( void );
...
void
at_sandbox_private_set_rlimits( ulong rlimit_file_cnt,
...
ulong
at_sandbox_private_read_cap_last_cap( void );
...
void
at_sandbox_private_drop_caps( ulong cap_last_cap );
...
void
at_sandbox_private_landlock_restrict_self( int allow_connect,
                                           int allow_renameat );
...
void
at_sandbox_private_set_seccomp_filter( ushort               seccomp_filter_cnt,
                                       struct sock_filter * seccomp_filter );

/* The same as at_sandbox_enter, enters the full sandbox with one
...
void
at_sandbox_private_enter_no_seccomp( uint        desired_uid,
                                     uint        desired_gid,
...
#endif /* HEADER_at_src_util_sandbox_at_sandbox_private_h */