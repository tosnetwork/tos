#ifndef HEADER_at_src_util_sandbox_at_sandbox_h
#define HEADER_at_src_util_sandbox_at_sandbox_h
...
/* at_sandbox_requires_cap_sys_admin checks if the current environment
...
int
at_sandbox_requires_cap_sys_admin( uint desired_uid,
                                   uint desired_gid );

/* at_sandbox_enter takes various steps to enter the process into a
   ...
   Calling at_sandbox_enter may require capabilities,
   ...
   Calling at_sandbox_enter alone is not enough to sandbox a process, as
...
void
at_sandbox_enter( uint                 desired_uid,                  /* User ID to switch the process to inside the sandbox */
                  ...
                  ulong                rlimit_data,                  /* Maximum data segment size to provide to setrlimit(RLIMIT_DATA) */
                  ulong                rlimit_nproc,                 /* Maximum number of processes to provide to setrlimit(RLIMIT_NPROC) */
...
/* at_sandbox_switch_uid_gid switches the calling process effective,
   ...
   process, unlike at_sandbox_enter.

   Calling at_sandbox_switch_uid_gid may require CAP_SETGID and
...
void
at_sandbox_switch_uid_gid( uint desired_uid,   /* User ID to switch the process to */
                           uint desired_gid ); /* Group ID to switch the process to */

/* at_sandbox_getpid returns the true PID of the current process as it
...
ulong
at_sandbox_getpid( void );

/* at_sandbox_gettid returns the true TID of the current process as it
   appears in the root PID namespace of the system.
...
ulong
at_sandbox_gettid( void );

AT_PROTOTYPES_END

#endif /* defined(__linux__) */

#endif /* HEADER_at_src_util_sandbox_at_sandbox_h */