#ifndef HEADER_at_src_util_bits_at_uwide_h
#define HEADER_at_src_util_bits_at_uwide_h
...
/* at_uwide_add computes co 2^128 + <zh,zl> = <xh,xl> + <yh,yl> + ci
...
static inline ulong
at_uwide_add( ulong * AT_RESTRICT _zh, ulong * AT_RESTRICT _zl,
              ulong               xh,  ulong               xl,
              ulong               yh,  ulong               yl,
              ulong               ci ) {
...
/* at_uwide_inc computes <zh,zl> = (<xh,xl> + y) mod 2^128 exactly (a
   common use of the above) */

static inline void
at_uwide_inc( ulong * AT_RESTRICT _zh, ulong * AT_RESTRICT _zl,
              ulong               xh,  ulong               xl,
              ulong               y ) {
  ulong zl = xl + y;
  ulong zh = xh + (zl<xl);
...
/* at_uwide_sub compute <zh,zl> = bo 2^128 + <xh,xl> - <yh,yl> - bi
...
static inline ulong
at_uwide_sub( ulong * AT_RESTRICT _zh, ulong * AT_RESTRICT _zl,
              ulong               xh,  ulong               xl,
              ulong               yh,  ulong               yl,
              ulong               bi ) {
  ulong zh = xh; ulong zl = xl;
...
/* at_uwide_dec computes <zh,zl> = (<xh,xl> - y) mod 2^128 exactly (a
   common use of the above) */

static inline void
at_uwide_dec( ulong * AT_RESTRICT _zh, ulong * AT_RESTRICT _zl,
              ulong               xh,  ulong               xl,
              ulong               y ) {
...
/* at_uwide_mul computes <zh,zl> = x*y exactly, will be in
...
static inline void
at_uwide_mul( ulong * AT_RESTRICT _zh, ulong * AT_RESTRICT _zl,
              ulong               x,
              ulong               y ) {
#if AT_HAS_INT128
...
/* at_uwide_find_msb returns floor( log2 <xh,xl> ) exactly.  Assumes
   <xh,xl> is not 0. */

static inline int
at_uwide_find_msb( ulong xh,
                   ulong xl ) {
  int off = 0;
  if( xh ) { off = 64; xl = xh; } /* FIXME: BRANCHLESS? */
  return off + at_ulong_find_msb( xl );
}

/* at_uwide_find_msb_def same as the at_uwide_find_msb but returns def
   is <xh,xl> is 0.  FIXME: BRANCHLESS? */

static inline int at_uwide_find_msb_def( ulong xh, ulong xl, int def ) { return (xh|xl) ? at_uwide_find_msb(xh,xl) : def; }

/* at_uwide_sl computes <zh,zl> = <xh,xl> << s.  Assumes _zh and _zl are
...
static inline int
at_uwide_sl( ulong * AT_RESTRICT _zh, ulong * AT_RESTRICT _zl,
             ulong               xh,  ulong               xl,
             int                 s ) {
...
/* at_uwide_sr compute <zh,zl> = <xh,xl> >> s.  Assumes _zh and _zl are
   valid (e.g. non-NULL and non-overlapping) and s is non-negative.
   Large values of s are fine (shifts to zero).  Returns the inexact
   flag (will be 0 or 1) which indicates if any non-zero bits of <xh,xl>
   were lost in the process.  Note that inexact handling and various
...
static inline int
at_uwide_sr( ulong * AT_RESTRICT _zh, ulong * AT_RESTRICT _zl,
             ulong               xh,  ulong                xl,
             int                 s ) {
  if( s>=128 ) {                        *_zh = 0UL;   *_zl = 0UL;             return !!( xh    |xl); }
...
/* at_uwide_private_div_approx_init/at_uwide_private_div_approx computes
...
at_uwide_private_div_approx_init( ulong d ) { /* d in [2^63,2^64) */
...
at_uwide_private_div_approx( ulong n,    /* In [0,d) */
                             ulong m ) { /* Output of at_uwide_private_div_approx_init for the desired d */
  ulong nh = n>>32;
  ulong nl = (ulong)(uint)n;
  return n + nh*m + ((nl*m)>>32);
}

/* at_uwide_div computes zh 2^64 + zl = floor( (xh 2^64 + xl) / y ).
...
at_uwide_div( ulong * AT_RESTRICT _zh, ulong * AT_RESTRICT _zl,
              ulong               xh,  ulong               xl,
              ulong               y ) {

  /* Simple cases (y==0, x==0 and y==2^n) */

  if( AT_UNLIKELY( !y  ) ) { *_zh = 0UL; *_zl = 0UL;    return 1; }
  if( AT_UNLIKELY( !xh ) ) { *_zh = 0UL; *_zl = xl / y; return 0; }

  int n = at_ulong_find_msb( y );
  if( AT_UNLIKELY( !at_ulong_pop_lsb(y) ) ) { at_uwide_sr( _zh,_zl, xh,xl, n ); return 0; }
  ...
  if( AT_UNLIKELY( !rh ) ) { *_zh = qh; *_zl = xl / y; return 0; }
  ...
  do {
    ...
    ql += at_uwide_private_div_approx( eh, m ); /* Guaranteed to make progress if eh > 0 */
    at_uwide_mul( &eh,&el, ql, d );             /* <eh,el> = ql*d */
    at_uwide_sub( &eh,&el, nh,nl, eh,el, 0UL ); /* <eh,el> = <nh,nl> - ql d */
  } while( eh );
...
/* at_uwide_divrem is same as the above but returns the value of the
...
static inline ulong
at_uwide_divrem( ulong * AT_RESTRICT _zh, ulong * AT_RESTRICT _zl,
                 ulong               xh,  ulong               xl,
                 ulong               y ) {

  if( AT_UNLIKELY( !y  ) ) { *_zh = 0UL; *_zl = 0UL; return ULONG_MAX; }
  if( AT_UNLIKELY( !xh ) ) { ulong ql = xl / y; ulong r = xl - ql*y; *_zh = 0UL; *_zl = ql; return r; }

  int n = at_ulong_find_msb( y );
  if( AT_UNLIKELY( !at_ulong_pop_lsb(y) ) ) { int s = 64-n; at_uwide_sr( _zh,_zl, xh,xl, n ); return n ? ((xl << s) >> s) : 0UL; }

  ulong qh = xh / y;
  ulong rh = xh - qh*y;

  if( AT_UNLIKELY( !rh ) ) { ulong ql = xl / y; ulong r = xl - ql*y; *_zh = qh; *_zl = ql; return r; }
  ...
  ulong ql = 0UL;
  do {
    ql += at_uwide_private_div_approx( eh, m );
    at_uwide_mul( &eh,&el, ql, d );
    at_uwide_sub( &eh,&el, nh,nl, eh,el, 0UL );
  } while( eh );
...
#endif /* HEADER_at_src_util_bits_at_uwide_h */
