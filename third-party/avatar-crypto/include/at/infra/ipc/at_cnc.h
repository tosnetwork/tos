#ifndef HEADER_at_src_tango_cnc_at_cnc_h
#define HEADER_at_src_tango_cnc_at_cnc_h

/* A at_cnc_t provides APIs for out-of-band low bandwidth
   ...
   housekeeping, it uses at_cnc_t object dedicated to it to send and
   receive information from command and control threads and/or monitoring
   threads.  The basic template for an at_cnc_t state machine is:
...
/* AT_CNC_{ALIGN,FOOTPRINT} describe the alignment and footprint of a
   at_cnc_t.  ALIGN is a positive integer power of 2.  FOOTPRINT is a
...
#define AT_CNC_ALIGN (128UL)
#define AT_CNC_FOOTPRINT( app_sz )                                    \
  AT_LAYOUT_FINI( AT_LAYOUT_APPEND( AT_LAYOUT_APPEND( AT_LAYOUT_INIT, \
    AT_CNC_ALIGN,     64UL     ),                                     \
    AT_CNC_APP_ALIGN, (app_sz) ),                                     \
    AT_CNC_ALIGN )

/* AT_CNC_APP_ALIGN describes the alignment and footprint of a at_cnc_t's
   application region.  This is a power of 2 of the minimal malloc
   alignment (typically 8) and at most AT_CNC_ALIGN. */

#define AT_CNC_APP_ALIGN (64UL)

/* AT_CNC_SIGNAL_* are the standard cnc signals.  All remaining values
   ([4,ULONG_MAX]) are available to implement user defined signals.
   Details of the standard signals are provided above. */

#define AT_CNC_SIGNAL_RUN  (0UL)
#define AT_CNC_SIGNAL_BOOT (1UL)
#define AT_CNC_SIGNAL_FAIL (2UL)
#define AT_CNC_SIGNAL_HALT (3UL)

/* AT_CNC_SUCCESS, AT_CNC_ERR_* are error code return values used by
   cnc APIs.  SUCCESS must be zero, ERR_* are negative and distinct. */

#define AT_CNC_SUCCESS   (0)  /* success */
#define AT_CNC_ERR_UNSUP (-1) /* unsupported on this caller */
#define AT_CNC_ERR_INVAL (-2) /* bad inputs */
#define AT_CNC_ERR_AGAIN (-3) /* potentially transient failure */
#define AT_CNC_ERR_FAIL  (-4) /* permanent failure */

/* at_cnc_t is an opaque handle of a command-and-control object.
   Details are exposed here to facilitate inlining of many cnc
   operations in performance critical app thread paths. */

#define AT_CNC_MAGIC (0xf17eda2c37c2c000UL) /* cnc ver 0 (binary compatible) */

struct __attribute__((aligned(AT_CNC_ALIGN))) at_cnc_private {
  ulong magic;     /* ==AT_CNC_MAGIC */
  ulong app_sz;
  ulong type;
  long  heartbeat0;
  long  heartbeat;
  ulong lock;
  ulong signal;
  /* Padding to AT_CNC_APP_ALIGN here */
  /* app_sz bytes here */
  /* Padding to AT_CNC_ALIGN here */
};

typedef struct at_cnc_private at_cnc_t;

AT_PROTOTYPES_BEGIN

/* at_cnc_{align,footprint} return the required alignment and footprint
   of a memory region suitable for use as a cnc.  at_cnc_align returns
   AT_CNC_ALIGN.  If footprint is larger than ULONG_MAX, footprint will
...
AT_FN_CONST ulong
at_cnc_footprint( ulong app_sz );

/* at_cnc_new formats an unused memory region for use as a cnc.  Assumes
...
void *
at_cnc_new( void * shmem,
            ulong  app_sz,
            ulong  type,
            long   now );

/* at_cnc_join joins the caller to the cnc.  shcnc points to the first
...
at_cnc_t *
at_cnc_join( void * shcnc );

/* at_cnc_leave leaves a current local join.  Returns a pointer to the
...
void *
at_cnc_leave( at_cnc_t const * cnc );

/* at_cnc_delete unformats a memory region used as a cnc.  Assumes
...
void *
at_cnc_delete( void * shcnc );

/* at_cnc_shmem returns a pointer in the caller's address space to the
   underlying shared memory region used by a cnc.  Assumes cnc is a
   current local join.  at_cnc_shmem_const is a const-correct version.  */

AT_FN_CONST static inline void       * at_cnc_shmem      ( at_cnc_t       * cnc ) { return (void       *)cnc; }
AT_FN_CONST static inline void const * at_cnc_shmem_const( at_cnc_t const * cnc ) { return (void const *)cnc; }

/* at_cnc_app_sz returns the size of a the cnc's application region.
   Assumes cnc is a current local join. */

AT_FN_PURE static inline ulong at_cnc_app_sz( at_cnc_t const * cnc ) { return cnc->app_sz; }

/* at_cnc_app_laddr returns local address of the cnc's application
   region.  This will have AT_CNC_APP_ALIGN alignment and room for at
   least at_cnc_app_sz( cnc ) bytes.  Assumes cnc is a current local
   join.  at_cnc_app_laddr_const is for const correctness.  The return
   values are valid for the lifetime of the local join. */

AT_FN_CONST static inline void *       at_cnc_app_laddr      ( at_cnc_t *       cnc ) { return (void *      )(((ulong)cnc)+64UL); }
AT_FN_CONST static inline void const * at_cnc_app_laddr_const( at_cnc_t const * cnc ) { return (void const *)(((ulong)cnc)+64UL); }

/* at_cnc_type returns the application defined type of a cnc.  Assumes
   cnc is a current local join. */

AT_FN_PURE static inline ulong at_cnc_type( at_cnc_t const * cnc ) { return cnc->type; }

/* at_cnc_heartbeat0 returns the heartbeat assigned when the cnc was
   created.  Assumes cnc is a current local join. */

AT_FN_PURE static inline long at_cnc_heartbeat0( at_cnc_t const * cnc ) { return cnc->heartbeat0; }

/* at_cnc_heartbeat_query returns the value of the cnc's heartbeat
...
static inline long
at_cnc_heartbeat_query( at_cnc_t const * cnc ) {
  AT_COMPILER_MFENCE();
  long then = AT_VOLATILE_CONST( cnc->heartbeat );
  AT_COMPILER_MFENCE();
  return then;
}

/* at_cnc_heartbeat is used by an app thread to update the cnc's
...
static inline void
at_cnc_heartbeat( at_cnc_t * cnc,
                  long       now ) {
  AT_COMPILER_MFENCE();
  AT_VOLATILE( cnc->heartbeat ) = now;
  AT_COMPILER_MFENCE();
}

/* at_cnc_signal query observes the current signal posted to the cnc.
...
static inline ulong
at_cnc_signal_query( at_cnc_t const * cnc ) {
  AT_COMPILER_MFENCE();
  ulong s = AT_VOLATILE_CONST( cnc->signal );
  AT_COMPILER_MFENCE();
  return s;
}

/* at_cnc_signal atomically transitions the cnc to signal s.  Assumes
...
static inline void
at_cnc_signal( at_cnc_t * cnc,
               ulong      s ) {
  AT_COMPILER_MFENCE();
  AT_VOLATILE( cnc->signal ) = s;
  AT_COMPILER_MFENCE();
}

/* at_cnc_open opens a new command session to an app thread.  Returns 0
   (AT_CNC_SUCCESS) on success and a negative (AT_CNC_ERR_*) on failure
   ...
   Reasons for AT_CNC_ERR_UNSUP include not running on a hosted target,
   not running on an atomic capable target and strange thread group id;
   this is a permanent failure.  Reasons for AT_CNC_ERR_INVAL include
   NULL cur; this is a permanent failure.  Reasons for AT_CNC_ERR_FAIL
   include app thread is not running and cannot be restarted cleanly;
   this is a permanent failure.  Reasons for AT_CNC_ERR_AGAIN include
...
int
at_cnc_open( at_cnc_t * cnc );

/* at_cnc_wait waits up to dt ns for the cnc to transition to something
...
ulong
at_cnc_wait( at_cnc_t const * cnc,
             ulong            test,
             long             dt,
             long *           _opt_now );

/* at_cnc_close ends the current command session on cnc.  Assumes caller
...
static inline void
at_cnc_close( at_cnc_t * cnc ) {
  AT_COMPILER_MFENCE();
  AT_VOLATILE( cnc->lock ) = 0UL;
  AT_COMPILER_MFENCE();
}

/* at_cnc_strerror converts a AT_CNC_SUCCESS / AT_CNC_ERR_* code into
...
at_cnc_strerror( int err );

/* at_cstr_to_cnc_signal converts the cstr pointed to by into a cnc
   signal value.  Return value undefined if cstr does not point to a cnc
   signal cstr. */

AT_FN_PURE ulong
at_cstr_to_cnc_signal( char const * cstr );

/* at_cnc_signal_cstr pretty prints the cnc signal value into buf.  buf
   must point to a character buffer with at least
   AT_CNC_SIGNAL_CSTR_BUF_MAX bytes.  Always returns buf.  If buf is
   non-NULL, the buffer pointed at will be populated with a proper '\0'
   terminated cstr on return (and one that at_cstr_to_cnc_signal
   properly convert back to signal). */

#define AT_CNC_SIGNAL_CSTR_BUF_MAX (21UL)

char *
at_cnc_signal_cstr( ulong  signal,
                    char * buf );

AT_PROTOTYPES_END

#endif /* HEADER_at_src_tango_cnc_at_cnc_h */