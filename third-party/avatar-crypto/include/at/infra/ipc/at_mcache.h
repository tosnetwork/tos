#ifndef HEADER_at_src_tango_mcache_at_mcache_h
#define HEADER_at_src_tango_mcache_at_mcache_h

#include "at_tango_base.h"

/* AT_MCACHE_{ALIGN,FOOTPRINT} specify the alignment and footprint
   needed for a mcache with depth entries and an application region of
   size app_sz.  ALIGN is at least AT_FRAG_META_ALIGN and recommended to
   ...
   integer power of 2 of at least AT_MCACHE_BLOCK and the combination
   will not require a footprint larger than ULONG_MAX).  These are
   provided to facilitate compile time mcache declarations. */

#define AT_MCACHE_ALIGN (128UL)
#define AT_MCACHE_FOOTPRINT( depth, app_sz )                                                              \
  AT_LAYOUT_FINI( AT_LAYOUT_APPEND( AT_LAYOUT_APPEND( AT_LAYOUT_APPEND( AT_LAYOUT_APPEND( AT_LAYOUT_INIT, \
    AT_MCACHE_ALIGN, 128UL                           ), /* hdr  */                                        \
    AT_MCACHE_ALIGN, AT_MCACHE_SEQ_CNT*sizeof(ulong) ), /* seq  */                                        \
    AT_MCACHE_ALIGN, (depth)*sizeof(at_frag_meta_t)  ), /* meta */                                        \
    AT_MCACHE_ALIGN, (app_sz)                        ), /* app  */                                        \
    AT_MCACHE_ALIGN )

/* AT_MCACHE_SEQ_CNT specifies the number of entries in the mcache's seq
   storage region.  It is aligned AT_MCACHE_ALIGN.  Multiples of 16 have
   good Feng Shui.  seq[0] has special meaning; see below for details. */

#define AT_MCACHE_SEQ_CNT (16UL)

/* AT_MCACHE_{LG_BLOCK,LG_INTERLEAVE,BLOCK} specifies how recent
   fragment meta data should be packed into mcaches.  LG_BLOCK should be
   in [1,64).  LG_INTERLEAVE should be in [0,AT_MCACHE_BLOCK).  BLOCK ==
   2^LG_BLOCK.  See below for more details. */

#define AT_MCACHE_LG_BLOCK      (0)
#define AT_MCACHE_LG_INTERLEAVE (0)
#define AT_MCACHE_BLOCK         (1UL) /* == 2^AT_MCACHE_LG_BLOCK, explicit to workaround compiler limitations */

AT_PROTOTYPES_BEGIN

/* Construction API */

/* at_mcache_{align,footprint} return the required alignment and
   footprint of a memory region suitable for use as mcache with depth
   entries.  align returns AT_MCACHE_ALIGN.  If depth is invalid (e.g.
   not an integer power-of-2 >= AT_MCACHE_BLOCK or the footprint is
...
AT_FN_CONST ulong
at_mcache_align( void );

AT_FN_CONST ulong
at_mcache_footprint( ulong depth,
                     ulong app_sz );

/* at_mcache_new formats an unused memory region for use as a mcache.
   ...
   cache entries (should be an integer power of 2 >= AT_MCACHE_BLOCK).
...
void *
at_mcache_new( void * shmem,
               ulong  depth,
               ulong  app_sz,
               ulong  seq0 );

/* at_mcache_join joins the caller to the mcache.  shmcache points to
...
at_frag_meta_t *
at_mcache_join( void * shmcache );

/* at_mcache_leave leaves a current local join.  Returns a pointer to
...
void *
at_mcache_leave( at_frag_meta_t const * mcache );

/* at_mcache_delete unformats a memory region used as a mcache.  Assumes
...
void *
at_mcache_delete( void * shmcache );

/* Accessor API */

/* at_mcache_{depth,seq0} return the values corresponding to those use
   at the mcache's construction.  Assume mcache is a current local join. */

AT_FN_PURE ulong at_mcache_depth ( at_frag_meta_t const * mcache );
AT_FN_PURE ulong at_mcache_app_sz( at_frag_meta_t const * mcache );
AT_FN_PURE ulong at_mcache_seq0  ( at_frag_meta_t const * mcache );

/* at_mcache_seq_laddr returns location in the caller's local address
   space of mcache's sequence array.  This array is indexed
   [0,AT_MCACHE_SEQ_CNT) with AT_MCACHE_ALIGN alignment (double cache
...
AT_FN_CONST ulong const * at_mcache_seq_laddr_const( at_frag_meta_t const * mcache );
AT_FN_CONST ulong *       at_mcache_seq_laddr      ( at_frag_meta_t *       mcache );

/* at_mcache_app_laddr returns location in the caller's local address
   ...
   is the same as the underlying join.  This region has AT_MCACHE_ALIGN
   alignment (double cache line) and is at_mcache_app_sz( mcache ) in
   size.  laddr_const is a const-correct version. */

AT_FN_PURE uchar const * at_mcache_app_laddr_const( at_frag_meta_t const * mcache );
AT_FN_PURE uchar *       at_mcache_app_laddr      ( at_frag_meta_t *       mcache );

/* at_mcache_seq_query atomically reads the mcache's seq[0] (e.g. from
   at_mcache_seq_laddr_const) to get a lower bound of where the producer
...
static inline ulong
at_mcache_seq_query( ulong const * _seq ) {
  AT_COMPILER_MFENCE();
  ulong seq = AT_VOLATILE_CONST( *_seq );
  AT_COMPILER_MFENCE();
  return seq;
}

/* at_mcache_seq_update updates the mcache's seq[0] (e.g. from
   at_mcache_seq_laddr) above where the producer a lower bound of where
...
static inline void
at_mcache_seq_update( ulong * _seq,
                      ulong   seq ) {
  AT_COMPILER_MFENCE();
  AT_VOLATILE( *_seq ) = seq;
  AT_COMPILER_MFENCE();
}

/* at_mcache_line_idx returns the index of the cache line in a depth
   ...
   AT_MCACHE_LG_INTERLEAVE is in [0,AT_MCACHE_LG_BLOCK) and controls the
   ...
   Using 0 / B for AT_MCACHE_LG_INTERLEAVE / LG_BLOCK will disable meta
   ...
   2 / 7 (with a 32-byte size 32-byte aligned at_frag_meta_t and a
...
#if AT_MCACHE_LG_INTERLEAVE==0

AT_FN_CONST static inline ulong /* Will be in [0,depth) */
at_mcache_line_idx( ulong seq,
...
at_mcache_line_idx( ulong seq,
                    ulong depth ) { /* Assumed power of 2 >= BLOCK */
  ulong block_mask = AT_MCACHE_BLOCK - 1UL; /* Compile time */
  ulong page_mask  = (depth-1UL) & (~block_mask);    /* Typically compile time or loop invariant */
  ulong page = seq & page_mask;
  ulong bank = (seq << AT_MCACHE_LG_INTERLEAVE) & block_mask;
  ulong idx  = (seq & block_mask) >> (AT_MCACHE_LG_BLOCK-AT_MCACHE_LG_INTERLEAVE);
  return page | bank | idx;
}

#endif

/* at_mcache_publish inserts the metadata for frag seq into the given
   depth entry mcache in a way compatible with AT_MCACHE_WAIT and
   AT_MCACHE_WAIT_SSE (but not AT_MCACHE_WAIT_AVX ... see AT_MCACHE_WAIT
   for more details).  This implicitly evicts the metadata for the
   sequence number currently stored at at_mcache_line_idx( seq, depth ).
...
static inline void
at_mcache_publish( at_frag_meta_t * mcache,   /* Assumed a current local join */
  ...
  at_frag_meta_t * meta = mcache + at_mcache_line_idx( seq, depth );
  AT_COMPILER_MFENCE();
  meta->seq    = at_seq_dec( seq, 1UL );
  AT_COMPILER_MFENCE();
  meta->sig    =         sig;
...
/* at_mcache_publish_sse is a SSE implementation of at_mcache_publish.
   It is compatible with AT_MCACHE_WAIT and AT_MCACHE_WAIT_SSE. */

static inline void
at_mcache_publish_sse( at_frag_meta_t * mcache,   /* Assumed a current local join */
                       ulong            depth,    /* Assumed an integer power-of-2 >= BLOCK */
                       ulong            seq,
                       ulong            sig,
                       ulong            chunk,    /* Assumed in [0,UINT_MAX] */
                       ...
                       ulong            tspub ) { /* Assumed in [0,UINT_MAX] */
  at_frag_meta_t * meta = mcache + at_mcache_line_idx( seq, depth );
  __m128i meta_sse0 = at_frag_meta_sse0( at_seq_dec( seq, 1UL ), sig );
  __m128i meta_sse1 = at_frag_meta_sse1( chunk, sz, ctl, tsorig, tspub );
  AT_COMPILER_MFENCE();
  AT_VOLATILE( meta->sse0 ) = meta_sse0;
  AT_COMPILER_MFENCE();
  AT_VOLATILE( meta->sse1 ) = meta_sse1;
  AT_COMPILER_MFENCE();
  meta->seq = seq;
  AT_COMPILER_MFENCE();
}
...
/* at_mcache_publish_avx is an AVX implementation of at_mcache_publish.
   It is compatible with AT_MCACHE_WAIT, AT_MCACHE_WAIT_SSE and
   AT_MCACHE_WAIT_AVX.  It requires a target for which aligned AVX
   stores are guaranteed atomic under the hood (see below for more
   details). */

static inline void
at_mcache_publish_avx( at_frag_meta_t * mcache,   /* Assumed a current local join */
                       ulong            depth,    /* Assumed an integer power-of-2 >= BLOCK */
                       ulong            seq,
                       ulong            sig,
                       ulong            chunk,    /* Assumed in [0,UINT_MAX] */
                       ...
                       ulong            tspub ) { /* Assumed in [0,UINT_MAX] */
  at_frag_meta_t * meta = mcache + at_mcache_line_idx( seq, depth );
  __m256i meta_avx = at_frag_meta_avx( seq, sig, chunk, sz, ctl, tsorig, tspub );
  AT_COMPILER_MFENCE();
  /* _mm256_store_si256( &meta->avx, meta_avx );
     ...
     as a workaround. */
  AT_VOLATILE( meta->avx ) = meta_avx;
  AT_COMPILER_MFENCE();
}

#endif

/* AT_MCACHE_WAIT does a bounded wait for a producer to transmit a
   particular frag.

   meta (at_frag_meta_t * compatible) is the location on the caller
   where the wait should save the found metadata.  This typically
   points to a stack temporary.

   mline (at_frag_meta_t const * compatible) will be
     mcache + at_mcache_line_idx( seq_expected, depth )
   ...
   of seq_expected when the wait does not time out
     at_seq_diff( seq_found, seq_expected )
   ...
   poll_max (ulong compatible) is the number of times AT_MCACHE_WAIT
   ...
   mcache (at_frag_meta_t const * compatible) is a current local join to
   the mcache the producer uses to cache metadata for the frags it is
   producing.

   depth (a ulong compatible power of two of at least AT_MCACHE_BLOCK)
   ...
   TL;DR  Typical usage:
     ...
     at_frag_meta_t const * mcache   = ... local join to producer's mcache
     ...
     for(;;) {
       ...
       at_frag_meta_t         meta[1];
       at_frag_meta_t const * mline;
       ulong                  tx_seq;
       long                   seq_diff;
       AT_MCACHE_WAIT( meta, mline, tx_seq, seq_diff, poll_max, mcache, depth, rx_seq );
       ...
       ... metadata.  poll_max>0 and seq_diff==at_seq_diff(tx_seq,rx_seq).

       if( AT_UNLIKELY( seq_diff ) ) {
       ...
       tx_seq = at_frag_meta_seq_query( mline );
       if( AT_UNLIKELY( at_seq_ne( tx_seq, rx_seq ) ) ) {
   ...
   atomic) or writes the metadata cache line in a particular order:
     ...
     AT_COMPILER_MFENCE();
     mcache_line->seq = at_seq_dec( seq, 1UL ); // atomically marks cache line as in the process of writing seq
   ...
   aligned SSE writes are guaranteed to be atomic) as:
     ...
     AT_COMPILER_MFENCE();
     _mm_store_si128( &mcache_line->sse0, at_frag_meta_sse0( at_seq_dec( seq, 1UL ), sig );
     AT_COMPILER_MFENCE();
     _mm_store_si128( &mcache_line->sse1, at_frag_meta_sse1( chunk, sz, ctl, tsorig, tspub );
     AT_COMPILER_MFENCE();
     mcache_line->seq = seq;
     AT_COMPILER_MFENCE();
...
#define AT_MCACHE_WAIT( meta, mline, seq_found, seq_diff, poll_max, mcache, depth, seq_expected ) do {                    \
    ulong                  _at_mcache_wait_seq_expected = (seq_expected);                                                 \
    at_frag_meta_t const * _at_mcache_wait_mline        = (mcache)                                                        \
                                                        + at_mcache_line_idx( _at_mcache_wait_seq_expected, (depth) );    \
    at_frag_meta_t *       _at_mcache_wait_meta         = (meta);                                                         \
    ulong                  _at_mcache_wait_seq_found;                                                                     \
    long                   _at_mcache_wait_seq_diff;                                                                      \
    ulong                  _at_mcache_wait_poll_max     = (poll_max);                                                     \
    for(;;) {                                                                                                             \
      AT_COMPILER_MFENCE();                                                                                               \
      _at_mcache_wait_seq_found = _at_mcache_wait_mline->seq; /* atomic */                                                \
      AT_COMPILER_MFENCE();                                                                                               \
      *_at_mcache_wait_meta = *_at_mcache_wait_mline; /* probably non-atomic, typically fast L1 cache hit */              \
      AT_COMPILER_MFENCE();                                                                                               \
      ulong _at_mcache_wait_seq_test = _at_mcache_wait_mline->seq; /* atomic, typically fast L1 cache hit */              \
      AT_COMPILER_MFENCE();                                                                                               \
      _at_mcache_wait_seq_diff = at_seq_diff( _at_mcache_wait_seq_found, _at_mcache_wait_seq_expected );                  \
      int _at_mcache_wait_done = ((_at_mcache_wait_seq_found==_at_mcache_wait_seq_test) & (_at_mcache_wait_seq_diff>=0L)) \
                               | (!--_at_mcache_wait_poll_max);                                                           \
      AT_COMPILER_FORGET( _at_mcache_wait_done ); /* inhibit compiler from turning this into branch nest */               \
      if( AT_LIKELY( _at_mcache_wait_done ) ) break; /* opt for exit, single exit to help spin_pause cpu hinting */       \
      AT_SPIN_PAUSE();                                                                                                    \
    }                                                                                                                     \
    (mline)     = _at_mcache_wait_mline;                                                                                  \
    (seq_found) = _at_mcache_wait_seq_found;                                                                              \
    (seq_diff)  = _at_mcache_wait_seq_diff;                                                                               \
    (poll_max)  = _at_mcache_wait_poll_max;                                                                               \
  } while(0)

/* AT_MCACHE_WAIT_REG: similar to AT_MCACHE_WAIT but uses (nominally)
   registers to hold the metadata instead of a local buffer. */

#define AT_MCACHE_WAIT_REG( _sig, _chunk, _sz, _ctl, _tsorig, _tspub, mline, seq_found, seq_diff, poll_max,                     \
                            mcache, depth, seq_expected ) do {                                                            \
    ulong                  _at_mcache_wait_seq_expected = (seq_expected);                                                 \
    at_frag_meta_t const * _at_mcache_wait_mline        = (mcache)                                                        \
                                                        + at_mcache_line_idx( _at_mcache_wait_seq_expected, (depth) );    \
    ulong                  _at_mcache_wait_poll_max     = (poll_max);                                                     \
    ulong                  _at_mcache_wait_sig;                                                                           \
    ulong                  _at_mcache_wait_chunk;                                                                         \
    ulong                  _at_mcache_wait_sz;                                                                            \
    ulong                  _at_mcache_wait_ctl;                                                                           \
    ulong                  _at_mcache_wait_tsorig;                                                                        \
    ulong                  _at_mcache_wait_tspub;                                                                         \
    ulong                  _at_mcache_wait_seq_found;                                                                     \
    long                   _at_mcache_wait_seq_diff;                                                                      \
    for(;;) {                                                                                                             \
      AT_COMPILER_MFENCE();                                                                                               \
      _at_mcache_wait_seq_found = _at_mcache_wait_mline->seq; /* atomic */                                                \
      AT_COMPILER_MFENCE();                                                                                               \
      _at_mcache_wait_sig       =        _at_mcache_wait_mline->sig;                                                      \
      _at_mcache_wait_chunk     = (ulong)_at_mcache_wait_mline->chunk;                                                    \
      _at_mcache_wait_sz        = (ulong)_at_mcache_wait_mline->sz;                                                       \
      _at_mcache_wait_ctl       = (ulong)_at_mcache_wait_mline->ctl;                                                      \
      _at_mcache_wait_tsorig    = (ulong)_at_mcache_wait_mline->tsorig;                                                   \
      _at_mcache_wait_tspub     = (ulong)_at_mcache_wait_mline->tspub;                                                    \
      AT_COMPILER_MFENCE();                                                                                               \
      ulong _at_mcache_wait_seq_test = _at_mcache_wait_mline->seq; /* atomic, typically fast L1 cache hit */              \
      AT_COMPILER_MFENCE();                                                                                               \
      _at_mcache_wait_seq_diff = at_seq_diff( _at_mcache_wait_seq_found, _at_mcache_wait_seq_expected );                  \
    ...
    (_sig)       = _at_mcache_wait_sig;                                                                                    \
    (_chunk)     = _at_mcache_wait_chunk;                                                                                  \
    (_sz)        = _at_mcache_wait_sz;                                                                                     \
    (_ctl)       = _at_mcache_wait_ctl;                                                                                    \
    (_tsorig)    = _at_mcache_wait_tsorig;                                                                                 \
    (_tspub)     = _at_mcache_wait_tspub;                                                                                  \
    (mline)     = _at_mcache_wait_mline;                                                                                  \
    (seq_found) = _at_mcache_wait_seq_found;                                                                              \
    (seq_diff)  = _at_mcache_wait_seq_diff;                                                                               \
    (poll_max)  = _at_mcache_wait_poll_max;                                                                               \
...
/* AT_MCACHE_WAIT_SSE: similar to AT_MCACHE_WAIT but uses a pair of SSE
   registers to hold the metadata instead of a local buffer.  This is
   only valid on targets with the AT_HAS_AVX capability (see
   at_tango_base.h for details on Intel's atomicity guarantees). */

#define AT_MCACHE_WAIT_SSE( meta_sse0, meta_sse1, mline, seq_found, seq_diff, poll_max, mcache, depth, seq_expected ) do { \
    ulong                  _at_mcache_wait_seq_expected = (seq_expected);                                                  \
    at_frag_meta_t const * _at_mcache_wait_mline        = (mcache)                                                         \
                                                        + at_mcache_line_idx( _at_mcache_wait_seq_expected, (depth) );     \
    __m128i                _at_mcache_wait_meta_sse0;                                                                      \
    __m128i                _at_mcache_wait_meta_sse1;                                                                      \
    ulong                  _at_mcache_wait_seq_found;                                                                      \
    long                   _at_mcache_wait_seq_diff;                                                                       \
    ulong                  _at_mcache_wait_poll_max     = (poll_max);                                                      \
    for(;;) {                                                                                                              \
      AT_COMPILER_MFENCE();                                                                                                \
      _at_mcache_wait_meta_sse0 = _mm_load_si128( &_at_mcache_wait_mline->sse0 ); /* atomic */                             \
      AT_COMPILER_MFENCE();                                                                                                \
      _at_mcache_wait_meta_sse1 = _mm_load_si128( &_at_mcache_wait_mline->sse1 ); /* atomic, typ fast L1 hit */            \
      AT_COMPILER_MFENCE();                                                                                                \
      ulong _at_mcache_wait_seq_test = _at_mcache_wait_mline->seq; /* atomic, typically fast L1 cache hit */               \
      AT_COMPILER_MFENCE();                                                                                                \
      _at_mcache_wait_seq_found = at_frag_meta_sse0_seq( _at_mcache_wait_meta_sse0 );                                      \
      _at_mcache_wait_seq_diff  = at_seq_diff( _at_mcache_wait_seq_found, _at_mcache_wait_seq_expected );                  \
      int _at_mcache_wait_done  = ((_at_mcache_wait_seq_found==_at_mcache_wait_seq_test) & (_at_mcache_wait_seq_diff>=0L)) \
                                | (!--_at_mcache_wait_poll_max);                                                           \
      AT_COMPILER_FORGET( _at_mcache_wait_done ); /* inhibit compiler from turning this into branch nest */                \
    ...
    (meta_sse0) = _at_mcache_wait_meta_sse0;                                                                               \
    (meta_sse1) = _at_mcache_wait_meta_sse1;                                                                               \
    (mline)     = _at_mcache_wait_mline;                                                                                   \
    (seq_found) = _at_mcache_wait_seq_found;                                                                               \
    (seq_diff)  = _at_mcache_wait_seq_diff;                                                                                \
    (poll_max)  = _at_mcache_wait_poll_max;                                                                                \
...
/* AT_MCACHE_WAIT_AVX: similar to AT_MCACHE_WAIT_SSE but uses a single
...
#define AT_MCACHE_WAIT_AVX( meta_avx, mline, seq_found, seq_diff, poll_max, mcache, depth, seq_expected ) do {         \
    ulong                  _at_mcache_wait_seq_expected = (seq_expected);                                              \
    at_frag_meta_t const * _at_mcache_wait_mline        = (mcache)                                                     \
                                                        + at_mcache_line_idx( _at_mcache_wait_seq_expected, (depth) ); \
    __m256i                _at_mcache_wait_meta_avx;                                                                   \
    ulong                  _at_mcache_wait_seq_found;                                                                  \
    long                   _at_mcache_wait_seq_diff;                                                                   \
    ulong                  _at_mcache_wait_poll_max     = (poll_max);                                                  \
    for(;;) {                                                                                                          \
      AT_COMPILER_MFENCE();                                                                                            \
      _at_mcache_wait_meta_avx  = _mm256_load_si256( &_at_mcache_wait_mline->avx ); /* atomic */                       \
      AT_COMPILER_MFENCE();                                                                                            \
      _at_mcache_wait_seq_found = at_frag_meta_avx_seq( _at_mcache_wait_meta_avx );                                    \
      _at_mcache_wait_seq_diff  = at_seq_diff( _at_mcache_wait_seq_found, _at_mcache_wait_seq_expected );              \
      int _at_mcache_wait_done  = (_at_mcache_wait_seq_diff>=0L) | (!--_at_mcache_wait_poll_max);                      \
      AT_COMPILER_FORGET( _at_mcache_wait_done ); /* inhibit compiler from turning this into branch nest */            \
      if( AT_LIKELY( _at_mcache_wait_done ) ) break; /* opt for exit, single exit to help spin_pause cpu hinting */    \
      AT_SPIN_PAUSE();                                                                                                 \
    }                                                                                                                  \
    (meta_avx)  = _at_mcache_wait_meta_avx;                                                                            \
    (mline)     = _at_mcache_wait_mline;                                                                               \
    (seq_found) = _at_mcache_wait_seq_found;                                                                           \
    (seq_diff)  = _at_mcache_wait_seq_diff;                                                                            \
    (poll_max)  = _at_mcache_wait_poll_max;                                                                            \
...
/* at_mcache_query returns seq_query if seq_query is still in the mcache
   (assumed to be a current local mcache join) with depth entries (depth
   is assumed to be an integer power of two of at least
   AT_MCACHE_BLOCK).  It will return a sequence number before seq_query
...
static inline ulong
at_mcache_query( at_frag_meta_t const * mcache,
                 ulong                  depth,
                 ulong                  seq_query ) {
  return at_frag_meta_seq_query( mcache + at_mcache_line_idx( seq_query, depth ) );
}

AT_PROTOTYPES_END

#endif /* HEADER_at_src_tango_mcache_at_mcache_h */
