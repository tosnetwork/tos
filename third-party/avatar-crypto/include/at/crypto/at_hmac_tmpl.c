/* Defines a family of functions implementing HMAC over a single hash
   function.  See at_hmac.c for example usage.
   ...
   following at_sha256 conventions (at_{HASH_ALG}_{init,append,fini}).
...
#define HMAC_FN  AT_EXPAND_THEN_CONCAT2(at_hmac_,HASH_ALG)
#define HASH_(x) AT_EXPAND_THEN_CONCAT4(at_,HASH_ALG,_,x)
...
HMAC_FN( void const * data,
  ...
  at_memset( key_ipad, 0x36, HASH_BLOCK_SZ );
  at_memset( key_opad, 0x5c, HASH_BLOCK_SZ );
