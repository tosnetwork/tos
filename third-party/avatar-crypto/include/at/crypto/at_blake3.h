#ifndef HEADER_at_src_ballet_blake3_at_blake3_h
#define HEADER_at_src_ballet_blake3_at_blake3_h
...
/* at_blake3 provides APIs for BLAKE3 hashing of messages.
   ...
   at_blake3 provides the "hash" mode of BLAKE3 with variable size
   ...
   ### Usage (simple)
     ...
     at_blake3_t hasher[1];
     at_blake3_init( hasher );
     at_blake3_append( hasher, data, sz );
     uchar hash[ 32 ];
     at_blake3_fini( hasher, hash );
...
/* ### Implementation
   ...
   at_blake3 consists of three major parts:
   ...
   (width*AT_CHUNK_SZ), i.e. >=8192 bytes of input for AVX2.
   ...
   bytes of data)  at_blake3 therefore has an internal clock that ticks
...
/* AT_BLAKE3_BLOCK_SZ is the byte size of the inputs to the internal
   compression function.  This is a protocol constant. */

#define AT_BLAKE3_BLOCK_LG_SZ (6)
#define AT_BLAKE3_BLOCK_SZ    (64UL)

/* AT_BLAKE3_OUTCHAIN_SZ is the byte size of an "output chaining
   value".  This is a protocol constant. */

#define AT_BLAKE3_OUTCHAIN_LG_SZ (5)
#define AT_BLAKE3_OUTCHAIN_SZ    (32UL)

/* AT_BLAKE3_CHUNK_SZ is the max number of input bytes of a leaf node.
   This is a protocol constant.
   (1<<AT_BLAKE3_CHUNK_LG_SZ)==AT_BLAKE3_CHUNK_SZ */

#define AT_BLAKE3_CHUNK_LG_SZ (10)
#define AT_BLAKE3_CHUNK_SZ    (1024UL)

/* AT_BLAKE3_KEY_SZ is the byte size of the optional key in expanded
   form.  This is a protocol constant. */

#define AT_BLAKE3_KEY_SZ (32UL)

/* Implementation constants *******************************************/

/* AT_BLAKE3_ROW_CNT is the max supported tree height of at_blake3. */

#define AT_BLAKE3_ROW_CNT (32UL)

/* AT_BLAKE3_INPUT_MAX_SZ is the max supported message size of
   at_blake3, derived by AT_BLAKE3_ROW_CNT.  (About 4.40 terabytes) */

#define AT_BLAKE3_INPUT_MAX_SZ ((1UL<<AT_BLAKE3_ROW_CNT)<<AT_BLAKE3_CHUNK_LG_SZ)

/* AT_BLAKE3_COL_CNT is the max number of adjacent tree nodes to be
   buffered per hash state.  Used for parallel processing.
   (1<<AT_BLAKE3_COL_LG_CNT) == AT_BLAKE3_COL_CNT */

#if AT_HAS_AVX512
#define AT_BLAKE3_COL_LG_CNT ( 5UL)
#define AT_BLAKE3_COL_CNT    (32UL)
#else
#define AT_BLAKE3_COL_LG_CNT ( 4UL)
#define AT_BLAKE3_COL_CNT    (16UL)
#endif

/* AT_BLAKE3_{ALIGN,FOOTPRINT} describe the alignment and footprint needed
   for a memory region to hold a at_blake3_t.  ALIGN is a positive
...
#define AT_BLAKE3_ALIGN (128UL)

/* A at_blake3_t should be treated as an opaque handle of a blake3
   calculation state.  (It technically isn't here facilitate compile
   time declarations of at_blake3_t memory.) */

#define AT_BLAKE3_MAGIC (0xF17EDA2CEB1A4E30) /* FIREDANCE BLAKE3 V0 */

/* Hash state machine *************************************************/

/* at_blake3_pos_t is a hash state machine.  The user should consider
   ...
   those calls.  For every at_blake3_pos_t, there is a at_blake3_buf_t.
   Depending on input size, it may be able to prepare multiple ops that
   can be worked on in parallel. */

struct __attribute__((aligned(AT_BLAKE3_ALIGN))) at_blake3_pos {
...
# endif
  ...
  /* leaf_idx is the number of leaf chunks processed so far.  All but
     the last leaf chunk are of size AT_CHUNK_SZs.  live_cnt is the
  ...
  /* magic==AT_BLAKE3_MAGIC (useful for debugging and detecting memory
     corruption) */
...
typedef struct at_blake3_pos at_blake3_pos_t;

/* at_blake3_buf_t contains intermediate results of hash tree
...
union __attribute__((aligned(AT_BLAKE3_ALIGN))) at_blake3_buf {

  uchar slots[ AT_BLAKE3_ROW_CNT ][ AT_BLAKE3_COL_CNT ][ AT_BLAKE3_OUTCHAIN_SZ ];
  uchar rows [ AT_BLAKE3_ROW_CNT ][ AT_BLAKE3_COL_CNT *  AT_BLAKE3_OUTCHAIN_SZ ];

};

typedef union at_blake3_buf at_blake3_buf_t;

/* Simple API *********************************************************/

#if AT_HAS_AVX512
#define AT_BLAKE3_PARA_LG_MAX (4UL)
#elif AT_HAS_AVX
#define AT_BLAKE3_PARA_LG_MAX (3UL)
#else
#define AT_BLAKE3_PARA_LG_MAX (0UL)
#endif
#define AT_BLAKE3_PARA_MAX (1<<AT_BLAKE3_PARA_LG_MAX)

#define AT_BLAKE3_PRIVATE_LG_BUF_MAX (AT_BLAKE3_PARA_LG_MAX+AT_BLAKE3_CHUNK_LG_SZ)
#define AT_BLAKE3_PRIVATE_BUF_MAX    (1UL<<AT_BLAKE3_PRIVATE_LG_BUF_MAX)

struct at_blake3 {
  at_blake3_buf_t buf;
  uchar           block[ AT_BLAKE3_PRIVATE_BUF_MAX ];
  at_blake3_pos_t pos;
  ulong           block_sz;
};

typedef struct at_blake3 at_blake3_t;

#define AT_BLAKE3_FOOTPRINT (sizeof(at_blake3_t))

AT_PROTOTYPES_BEGIN

/* at_blake3_{align,footprint,new,join,leave,delete} usage is identical to
...
/* at_blake3_init starts a blake3 calculation.  sha is assumed to be a
...
/* at_blake3_append adds sz bytes locally pointed to by data an
...
/* at_blake3_fini finishes a a blake3 calculation.  sha and hash are
...
/* at_blake3_lthash_batch{n} calculate a batch of n independent BLAKE3
...
#endif /* HEADER_at_src_ballet_blake3_at_blake3_h */