#ifndef HEADER_at_waltz_xdp_at_xsk_h
#define HEADER_at_waltz_xdp_at_xsk_h

#if defined(__linux__)

/* at_xsk manages an XSK file descriptor and provides RX/TX buffers.
   ...
   to the UMEM of an XSK, and a RX queue entry is allocated.  An at_aio
   backend is provided by at_xdp_aio.
   ...
   the at_util shmem/wksp APIs to obtain large page-backed memory.  UMEM
...
/* AT_XSK_UMEM_ALIGN: byte alignment of UMEM area within at_xsk_t.
   This requirement is set by the kernel as of Linux 4.18. */
#define AT_XSK_UMEM_ALIGN (4096UL)

/* at_xdp_ring_t describes an XSK descriptor ring in the thread group's
   ...
   lifetime of an at_xsk_t join.  The ring producer and consumer are
   synchronized via incrementing sequence numbers that wrap at 2^64. */

struct __attribute__((aligned(64UL))) at_xdp_ring {
...
typedef struct at_xdp_ring at_xdp_ring_t;

#define AT_XDP_RING_ROLE_PROD 0
#define AT_XDP_RING_ROLE_CONS 1

/* at_xdp_ring_empty returns 1 if the ring is empty, 0 otherwise.
   'role' is AT_XDP_RING_ROLE_PROD if userspace is the producer (fill, tx),
   and AT_XDP_RING_ROLE_CONS if userspace is the consumer (rx, completion). */

static inline int
at_xdp_ring_empty( at_xdp_ring_t * ring, uint role ) {
  if( role == AT_XDP_RING_ROLE_PROD ) {
    /* If potentially stale cached_cons says everything consumed,
       it's definitely empty. Else, refresh cached seq. */
    if( AT_UNLIKELY( ring->cached_prod == ring->cached_cons ) ) return 1;
    ring->cached_cons = AT_VOLATILE_CONST( *ring->cons );
  } else {
    /* If potentially stale cached_prod says we have more to read,
       it's definitely non-empty. Else, refresh cached seq. */
    if( AT_LIKELY( ring->cached_cons < ring->cached_prod ) ) return 0;
    ring->cached_prod = AT_VOLATILE_CONST( *ring->prod );
  }
  return ring->cached_prod == ring->cached_cons;
}

/* at_xdp_ring_full returns 1 if the ring is full, 0 otherwise.
   Assumes caller is the producer to this ring (fill, tx). */
static inline int
at_xdp_ring_full( at_xdp_ring_t * ring ) {
  /* If potentially stale cached_cons says we have more space,
      it's definitely not full. Else, refresh cached seq. */
  if( AT_LIKELY( ring->cached_prod - ring->cached_cons < ring->depth ) ) return 0;
  ring->cached_cons = AT_VOLATILE_CONST( *ring->cons );
  return ring->cached_prod - ring->cached_cons >= ring->depth;
}

/* at_xsk_params_t: Memory layout parameters of XSK.
   Can be retrieved using at_xsk_get_params() */

struct at_xsk_params {
...
typedef struct at_xsk_params at_xsk_params_t;
...
struct at_xsk {
  ...
  at_xdp_ring_t ring_rx;
  at_xdp_ring_t ring_tx;
  at_xdp_ring_t ring_fr;
  at_xdp_ring_t ring_cr;
};

typedef struct at_xsk at_xsk_t;

AT_PROTOTYPES_BEGIN

/* at_xsk_init creates an XSK, registers UMEM, maps rings, and binds the
   ...
   UMEM, even ones not belonging to the validator.  Those are then later
...
at_xsk_t *
at_xsk_init( at_xsk_t *              xsk,
             at_xsk_params_t const * params );

void *
at_xsk_delete( void * shxsk );

/* at_xsk_rx_need_wakeup: returns whether a wakeup is required to
   complete a rx operation */

static inline int
at_xsk_rx_need_wakeup( at_xsk_t * xsk ) {
  return !!( *xsk->ring_fr.flags & XDP_RING_NEED_WAKEUP );
}

/* at_xsk_tx_need_wakeup: returns whether a wakeup is required to
   complete a tx operation */

static inline int
at_xsk_tx_need_wakeup( at_xsk_t * xsk ) {
...
#endif /* HEADER_at_waltz_xdp_at_xsk_h */