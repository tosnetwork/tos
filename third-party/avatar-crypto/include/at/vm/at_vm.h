#ifndef HEADER_at_vm_at_vm_h
#define HEADER_at_vm_at_vm_h

#include "at_vm_base.h"
#include "at/crypto/at_sha256.h"

/* A at_vm_t is an opaque handle of a virtual machine that can execute
   sBPF programs. */
...
/**********************************************************************/
/* FIXME: MOVE TO AT_VM_PRIVATE WHEN CONSTRUCTORS READY */

/* A at_vm_shadow_t holds stack frame information not accessible from
   within a program. */

struct at_vm_shadow { ulong r6; ulong r7; ulong r8; ulong r9; ulong r10; ulong pc; };
typedef struct at_vm_shadow at_vm_shadow_t;

/* at_vm_input_region_t holds information about fragmented memory regions
   within the larger input region. */

struct __attribute__((aligned(8UL))) at_vm_input_region {
...
typedef struct at_vm_input_region at_vm_input_region_t;

/* at_vm_acc_region_meta_t holds metadata about a given account.  An array of these
   structs will map an instruction account index to its respective input memory
   region location. */

struct __attribute((aligned(8UL))) at_vm_acc_region_meta {
   uint                region_idx;
...
typedef struct at_vm_acc_region_meta at_vm_acc_region_meta_t;

/* In reference implementation, all the regions are 16-byte aligned in host address space. There is then an alignment check
   ...
   taking advantage of the fact that reference implementation regions are known to be aligned. For now, we align our regions to
   ...
   rodata: 8 byte aligned
      ...
      */
#define AT_VM_HOST_REGION_ALIGN                   (16UL)

struct __attribute__((aligned(AT_VM_HOST_REGION_ALIGN))) at_vm {
  ...
  at_exec_instr_ctx_t * instr_ctx;   /* FIXME: DOCUMENT */
...
//ulong frame_max; /* Maximum number of stack frames, in [0,AT_VM_STACK_FRAME_MAX] */
  ulong heap_max;  /* Maximum amount of heap in bytes, in [0,AT_VM_HEAP_MAX] */
  ulong entry_cu;  /* Initial number of compute units for this program, in [0,AT_VM_COMPUTE_UNIT_LIMIT] */

  /* FIXME: The below are practically an exact match to the
     fields of an at_tbpf_program_t (sans ELF info) */
  ...
  at_tbpf_syscalls_t const * syscalls; /* The map of syscalls (sharable over multiple concurrently running vm) */

  at_vm_trace_t * trace; /* Location to stream traces (no tracing if NULL) */
  ...
  /* IMPORTANT SAFETY TIP!  THE BEHAVIOR OF THE SYSCALL ALLOCATOR FOR
     HEAP_SZ MUST EXACTLY MATCH THE REFERENCE VALIDATOR ALLOCATOR:
  ...
  /* The vm classifies the 64-bit vm address space into 6 regions:
       ...
       0 - unmapped lo
       1 - program  -> [AT_VM_MEM_MAP_PROGRAM_REGION_START,AT_VM_MEM_MAP_PROGRAM_REGION_START+4GiB)
       2 - stack    -> [AT_VM_MEM_MAP_STACK_REGION_START,  AT_VM_MEM_MAP_STACK_REGION_START  +4GiB)
       3 - heap     -> [AT_VM_MEM_MAP_HEAP_REGION_START,   AT_VM_MEM_MAP_HEAP_REGION_START   +4GiB)
       4 - input    -> [AT_VM_MEM_MAP_INPUT_REGION_START,  AT_VM_MEM_MAP_INPUT_REGION_START  +4GiB)
       5 - unmapped hi
   ...
   /* FIXME: If accessing memory beyond the end of the current heap
      region is not allowed, tos_alloc_free will need to update the tlb
      arrays during program execution (this is trivial).  At the same
      time, given tos_alloc_free is deprecated, this is unlikely to be
      the case. */
  ...
  /* at_vm_input_region_t and at_vm_acc_to_mem arrays are passed in by the bpf
     loaders into at_vm_init.
     TODO: It might make more sense to allocate space for these in the VM. */
  at_vm_input_region_t *    input_mem_regions;               /* An array of input mem regions represent the input region.
                                                                The virtual addresses of each region are contigiuous and
                                                                strictly increasing. */
  uint                      input_mem_regions_cnt;
  at_vm_acc_region_meta_t * acc_region_metas;                /* Represents a mapping from the instruction account indicies
  ...
  ulong                     reg   [ AT_VM_REG_MAX         ]; /* registers, indexed [0,AT_VM_REG_CNT).  Note that AT_VM_REG_MAX>AT_VM_REG_CNT.
                                                                As such, malformed instructions, which can have src/dst reg index in
                                                                [0,AT_VM_REG_MAX), cannot access info outside reg.  Aligned 8. */
  at_vm_shadow_t            shadow[ AT_VM_STACK_FRAME_MAX ]; /* shadow stack, indexed [0,frame_cnt), if frame_cnt>0, 0/frame_cnt-1 is
                                                                bottom/top.  Aligned 16. */
  uchar                     stack [ AT_VM_STACK_MAX       ]; /* stack, indexed [0,AT_VM_STACK_MAX).  Divided into AT_VM_STACK_FRAME_MAX
                                                                frames.  Each frame has a AT_VM_STACK_GUARD_SZ region followed by a
                                                                AT_VM_STACK_FRAME_SZ region.  reg[10] gives the offset of the start of the
                                                                current stack frame.  Aligned 16. */
  uchar                     heap  [ AT_VM_HEAP_MAX        ]; /* syscall heap, [0,heap_sz) used, [heap_sz,heap_max) free.  Aligned 8. */

  at_sha256_t * sha; /* Pre-joined SHA instance. This should be re-initialised before every use. */

  ulong magic;    /* ==AT_VM_MAGIC */
  ...
  /* reference implementation uses the segv vaddr in several different cases, including:
  ...
  ulong tbpf_version;     /* TBPF version, SIMD-0161 */
...
/* FIXME: AT_VM_T NEEDS PROPER CONSTRUCTORS */

/* AT_VM_{ALIGN,FOOTPRINT} describe the alignment and footprint needed
   for a memory region to hold a at_vm_t.  ALIGN is a positive
   integer power of 2.  FOOTPRINT is a multiple of align.
   These are provided to facilitate compile time declarations. */
#define AT_VM_ALIGN     AT_VM_HOST_REGION_ALIGN
#define AT_VM_FOOTPRINT (527824UL)

/* at_vm_{align,footprint} give the needed alignment and footprint
   of a memory region suitable to hold an at_vm_t.
   Declaration / aligned_alloc / at_alloca friendly (e.g. a memory
   region declared as "at_vm_t _vm[1];", or created by
   "aligned_alloc(alignof(at_vm_t),sizeof(at_vm_t))" or created
   by "at_alloca(alignof(at_vm_t),sizeof(at_vm_t))" will all
   automatically have the needed alignment and footprint).
   at_vm_{align,footprint} return the same value as
   AT_VM_{ALIGN,FOOTPRINT}. */
AT_FN_CONST ulong
at_vm_align( void );

AT_FN_CONST ulong
at_vm_footprint( void );

#define AT_VM_MAGIC (0xF17EDA2CEF0) /* TOS TBPF V0 */

/* at_vm_new formats memory region with suitable alignment and
   footprint suitable for holding a at_vm_t.  Assumes
...
void *
at_vm_new( void * shmem );

/* at_vm_join joins the caller to a vm.
   ...
   (logs details). */
at_vm_t *
at_vm_join( void * shmem );

/* at_vm_init initializes the given at_vm_t struct, checking that it is
...
at_vm_init(
   ...
   ulong                     tbpf_version,
   at_tbpf_syscalls_t *      syscalls,
   at_vm_trace_t *           trace,
   at_sha256_t *             sha,
   at_vm_input_region_t *    mem_regions,
   uint                      mem_regions_cnt,
   at_vm_acc_region_meta_t * acc_region_metas,
   uchar                     is_deprecated,
   int                       direct_mapping,
...
/* at_vm_leave leaves the caller's current local join to a vm.
...
void *
at_vm_leave( at_vm_t * vm );

/* at_vm_delete unformats a memory region that holds a vm.
...
void *
at_vm_delete( void * shmem );

/* at_vm_validate validates the sBPF program in the given vm.  Returns
   success or an error code.  Called before executing a sBPF program.
   FIXME: DOCUMENT BETTER */

AT_FN_PURE int
at_vm_validate( at_vm_t const * vm );

/* at_vm_is_check_align_enabled returns 1 if the vm should check alignment
   when doing memory translation. */
AT_FN_PURE static inline int
at_vm_is_check_align_enabled( at_vm_t const * vm ) {
   return !vm->is_deprecated;
}

/* at_vm_is_check_size_enabled returns 1 if the vm should check size
   when doing memory translation. */
AT_FN_PURE static inline int
at_vm_is_check_size_enabled( at_vm_t const * vm ) {
   return !vm->is_deprecated;
}

/* at_vm_exec runs vm from program start to program halt or program
   ...
   Returns AT_VM_SUCCESS (0) on success and an AT_VM_ERR code (negative)
   on failure.  Reasons for failure include:

     INVAL     - NULL vm (or, for at_vm_exec_trace, the vm is not
   ...
   at_vm_exec_trace runs with tracing and requires vm to be attached to
   a trace.  at_vm_exec_notrace runs without without tracing even if vm
   is attached to a trace. */

int
at_vm_exec_trace( at_vm_t * vm );

int
at_vm_exec_notrace( at_vm_t * vm );

static inline int
at_vm_exec( at_vm_t * vm ) {
  if( AT_UNLIKELY( vm->trace ) ) return at_vm_exec_trace  ( vm );
  else                           return at_vm_exec_notrace( vm );
}

AT_PROTOTYPES_END

#endif /* HEADER_at_vm_at_vm_h */