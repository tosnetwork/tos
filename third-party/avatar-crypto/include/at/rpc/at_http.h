#ifndef HEADER_at_src_waltz_http_at_http_h
#define HEADER_at_src_waltz_http_at_http_h

/* An at_http_server is a WebSocket capable HTTP server designed to
   ...
   Using the outgoing ring has two steps,
     ...
     (1) Stage data into the ring with at_http_server_printf and
         at_http_server_memcpy functions.
     (2) Send the staged data to clients with at_http_server_send and
         at_http_server_ws_broadcast.

   The server is designed to be used in a single threaded event loop and
   run within a tile.  The caller should call at_http_server_poll as
   frequently as possible to service connections and make forward
   progress. */

#include "at/infra/at_util_base.h"

#define AT_HTTP_SERVER_ALIGN       (128UL)

#define AT_HTTP_SERVER_METHOD_GET     (0)
#define AT_HTTP_SERVER_METHOD_POST    (1)
#define AT_HTTP_SERVER_METHOD_OPTIONS (2)
#define AT_HTTP_SERVER_METHOD_PUT     (3)

#define AT_HTTP_SERVER_CONNECTION_CLOSE_OK                           ( -1)
#define AT_HTTP_SERVER_CONNECTION_CLOSE_EVICTED                      ( -2)
#define AT_HTTP_SERVER_CONNECTION_CLOSE_TOO_SLOW                     ( -3)
#define AT_HTTP_SERVER_CONNECTION_CLOSE_EXPECTED_EOF                 ( -4)
#define AT_HTTP_SERVER_CONNECTION_CLOSE_PEER_RESET                   ( -5)
#define AT_HTTP_SERVER_CONNECTION_CLOSE_LARGE_REQUEST                ( -6)
#define AT_HTTP_SERVER_CONNECTION_CLOSE_BAD_REQUEST                  ( -7)
#define AT_HTTP_SERVER_CONNECTION_CLOSE_MISSING_CONENT_LENGTH_HEADER ( -8)
#define AT_HTTP_SERVER_CONNECTION_CLOSE_UNKNOWN_METHOD               ( -9)
#define AT_HTTP_SERVER_CONNECTION_CLOSE_PATH_TOO_LONG                (-10)
#define AT_HTTP_SERVER_CONNECTION_CLOSE_WS_BAD_KEY                   (-11)
#define AT_HTTP_SERVER_CONNECTION_CLOSE_WS_UNEXPECTED_VERSION        (-12)
#define AT_HTTP_SERVER_CONNECTION_CLOSE_WS_MISSING_KEY_HEADER        (-13)
#define AT_HTTP_SERVER_CONNECTION_CLOSE_WS_MISSING_VERSION_HEADER    (-14)
#define AT_HTTP_SERVER_CONNECTION_CLOSE_WS_BAD_MASK                  (-15)
#define AT_HTTP_SERVER_CONNECTION_CLOSE_WS_UNKNOWN_OPCODE            (-16)
#define AT_HTTP_SERVER_CONNECTION_CLOSE_WS_OVERSIZE_FRAME            (-17)
#define AT_HTTP_SERVER_CONNECTION_CLOSE_WS_CLIENT_TOO_SLOW           (-18)
#define AT_HTTP_SERVER_CONNECTION_CLOSE_WS_MISSING_UPGRADE           (-19)
#define AT_HTTP_SERVER_CONNECTION_CLOSE_WS_EXPECTED_CONT_OPCODE      (-20)
#define AT_HTTP_SERVER_CONNECTION_CLOSE_WS_EXPECTED_TEXT_OPCODE      (-21)
#define AT_HTTP_SERVER_CONNECTION_CLOSE_WS_CONTROL_FRAME_TOO_LARGE   (-22)
#define AT_HTTP_SERVER_CONNECTION_CLOSE_WS_CHANGED_OPCODE            (-23)

/* Given a AT_HTTP_SERVER_CONNECTION_CLOSE_* reason code, a reason that
   a HTTP connection a client was closed, produce a human readable
   string describing the reason. */

AT_FN_CONST char const *
at_http_server_connection_close_reason_str( int reason );

/* Given a AT_HTTP_SERVER_METHOD_* code, produce the string for that
   method. */

AT_FN_CONST char const *
at_http_server_method_str( uchar method );

/* Parameters needed for constructing an HTTP server.  */

struct at_http_server_params {
...
typedef struct at_http_server_params at_http_server_params_t;

struct at_http_server_request {
  ...
  uint         client_ip4;    /* Client IPv4 address in host byte order */

  uchar        method;        /* One of AT_HTTP_SERVER_METHOD_* indicating what the method of the request is */
  ...
  void *       connection;    /* Opaque pointer to underlying HTTP connection (for mining info storage) */
...
typedef struct at_http_server_request at_http_server_request_t;
...
/* A response issued by the server handler function to an HTTP request.
   ...
   server functions like at_http_server_printf.  This allows the server
   ...
   static_body_len, and should instead use at_http_server_stage_body
...
struct at_http_server_response {
...
typedef struct at_http_server_response at_http_server_response_t;

struct at_http_server_callbacks {
  ...
  at_http_server_response_t ( * request     )( at_http_server_request_t const * request );
  ...
  /* Close an HTTP request.  This is called back once all the data has
     ...
     AT_HTTP_SERVER_CONNECTION_CLOSE_* indicating why the connection is
  ...
  void                      ( * ws_open     )( ulong ws_conn_id, void * ws_connection, void * ctx );
  ...
  /* Called when a WebSocket connection is closed.  reason is one of
     AT_HTTP_SERVER_CONNECTION_CLOSE_* indicating why the connection was
...
typedef struct at_http_server_callbacks at_http_server_callbacks_t;
...
/* at_http_server_{align,footprint} give the needed alignment and
   footprint of a memory region suitable to hold an http server.

   at_http_server_new formats memory region with suitable alignment and
   ...
   at_http_server_join joins the caller to a http server state. Assumes
   ...
   at_http_server_leave leaves the caller's current local join to a http
   ...
   at_http_server_delete unformats a memory region that holds a http
...
AT_FN_CONST ulong
at_http_server_align( void );

AT_FN_CONST ulong
at_http_server_footprint( at_http_server_params_t params );

void *
at_http_server_new( void *                     shmem,
                    at_http_server_params_t    params,
                    at_http_server_callbacks_t callbacks,
                    void *                     callback_ctx );

at_http_server_t *
at_http_server_join( void * shhttp );

void
at_http_server_set_callback_ctx( at_http_server_t * server, void * ctx );

void *
at_http_server_leave( at_http_server_t * http );

void *
at_http_server_delete( void * shhttp );

/* at_http_server_fd returns the file descriptor of the server.  The
   file descriptor is used to poll for incoming connections and data
   on the server. */

int
at_http_server_fd( at_http_server_t * http );

at_http_server_t *
at_http_server_listen( at_http_server_t * http,
                       uint               address,
                       ushort             port );
...
void
at_http_server_close( at_http_server_t * http,
                      ulong              conn_id,
                      int                reason );
...
void
at_http_server_ws_close( at_http_server_t * http,
                         ulong              ws_conn_id,
                         int                reason );

/* at_http_server_buffer_trunc truncates the pending message to the given length. */

void
at_http_server_stage_trunc( at_http_server_t * http,
                             ulong len );

/* at_http_server_buffer_len returns the length of the pending message. */

ulong
at_http_server_stage_len( at_http_server_t * http );

/* at_http_server_printf appends the rendered format string fmt into the
   ...
   the HTTP server using at_http_server_broadcast.  This will end the
...
void
at_http_server_printf( at_http_server_t * http,
                       char const *       fmt,
                       ... )  __attribute__((format (printf, 2, 3)));

/* at_http_server_memcpy appends the data provided to the end of the
   staging area of the outgoing ring buffer.  Assumes http is a current
   local join.
...
void
at_http_server_memcpy( at_http_server_t * http,
                       uchar const *      data,
                       ulong              data_len );

/* at_http_server_append_start starts an in-place append operation.
...
uchar *
at_http_server_append_start( at_http_server_t * http,
                             ulong              len );

/* at_http_server_append_end finishes an earlier started in-place
   append.  len is the number of bytes that were acutally written. */

void
at_http_server_append_end( at_http_server_t * http,
                           ulong              len );

/* at_http_server_unstage unstages any data written into the staging
   buffer, clearing its contents.  It does not advance the ring buffer
   usage, and no clients will be evicted. */

void
at_http_server_unstage( at_http_server_t * http );

/* at_http_server_stage_body marks the current contents of the staging
...
int
at_http_server_stage_body( at_http_server_t *          http,
...
int
at_http_server_ws_send( at_http_server_t * http,
...
int
at_http_server_ws_broadcast( at_http_server_t * http );

/* at_http_server_poll needs to be continuously called in a spin loop to
...
int
at_http_server_poll( at_http_server_t * http,
                     int                poll_timeout );

/* Mining WebSocket support - store miner info during /getwork upgrade

   These functions allow the request handler to store mining connection
   info (miner pubkey and worker name) in the HTTP connection, which will
   be transferred to the WebSocket connection during upgrade. */

void
at_http_server_set_miner_info( void *         connection,
                                uchar const    miner_pubkey[32],
                                char const *   worker_name );

int
at_http_server_ws_is_mining( void * ws_connection );

void
at_http_server_ws_get_miner_info( void *  ws_connection,
                                   uchar   out_miner_pubkey[32],
                                   char    out_worker_name[64] );

AT_PROTOTYPES_END

#endif /* HEADER_at_src_waltz_http_at_http_h */