...
//! ABI/transcript v2 additionally absorbs the host-rebuilt canonical replay
//! context. Block commitment is not a substitute for prover transcript binding.
//! There remain two unreviewed independent M3 entries, now with v2 content.
use bulletproofs::PedersenGens;
use curve25519_dalek::{ristretto::CompressedRistretto, Scalar, traits::IsIdentity};
use sha2::{Digest, Sha512};
use crate::ffi::{AbiStatus as Error, ClosurePossessionRequestV2};

fn challenge(r: &ClosurePossessionRequestV2, r1: &[u8;32], r2: &[u8;32]) -> Scalar {
    let mut h = Sha512::new();
    h.update(b"TOS/UNO/CLOSE/KEY-POSSESSION/v2");
    h.update(r.context);
    h.update(r.domain);
...
pub(crate) fn verify(r: &ClosurePossessionRequestV2) -> Result<(), Error> {
...
mod tests {
    use super::*;
    use crate::ffi::uno_crypto_verify_closure_possession_v2;
    fn proof(value: u64) -> ClosurePossessionRequestV2 {
        ...
        let mut r = ClosurePossessionRequestV2 { abi_version:2, context:[9;426], domain:[7;80], global_id:-23903,
    ...
    fn randomized_zero_and_nonzero() {
        ...
        assert_eq!(unsafe { uno_crypto_verify_closure_possession_v2(&r) },0);
        assert_eq!(unsafe { uno_crypto_verify_closure_possession_v2(&proof(1)) },Error::UNO_CRYPTO_VERIFY as u32);
    }
    #[test]
    fn state_binding_and_degenerate_inputs() {
        let original = proof(0);
        let changes: &[fn(&mut ClosurePossessionRequestV2)] = &[
        ...
        for change in changes { let mut r=original; change(&mut r);
            assert_ne!(unsafe { uno_crypto_verify_closure_possession_v2(&r) },0); }
        let mut r=original; r.handle=[0;32];
        assert_eq!(unsafe { uno_crypto_verify_closure_possession_v2(&r) },Error::UNO_CRYPTO_DECODE as u32);
        r=original; r.public_key=[0;32];
        assert_eq!(unsafe { uno_crypto_verify_closure_possession_v2(&r) },Error::UNO_CRYPTO_DECODE as u32);
        r=original; r.proof[64..].fill(255);
        assert_eq!(unsafe { uno_crypto_verify_closure_possession_v2(&r) },Error::UNO_CRYPTO_DECODE as u32);
        assert_eq!(unsafe { uno_crypto_verify_closure_possession_v2(std::ptr::null()) },Error::UNO_CRYPTO_ARGUMENTS as u32);
        r=original; r.abi_version=1;
        assert_eq!(unsafe { uno_crypto_verify_closure_possession_v2(&r) },Error::UNO_CRYPTO_ARGUMENTS as u32);
        assert_eq!(crate::ffi::uno_crypto_verify_closure_possession_v1(std::ptr::null()),Error::UNO_CRYPTO_ARGUMENTS as u32);
        assert_eq!(crate::ffi::uno_crypto_verify_closure_possession_v1(
            (&r as *const ClosurePossessionRequestV2).cast()),Error::UNO_CRYPTO_ARGUMENTS as u32);
    }
}
