...
std::unique_ptr<tos::adnl::AdnlExtClient::Callback> ValidatorEngineConsole::make_callback() {
  class Callback : public tos::adnl::AdnlExtClient::Callback {
   public:
...
void ValidatorEngineConsole::run() {
  ...
  client_ = tos::adnl::AdnlExtClient::create(tos::adnl::AdnlNodeIdFull{server_public_key_}, private_key_, remote_addr_,
                                             make_callback());
...
bool ValidatorEngineConsole::envelope_send_query(td::BufferSlice query, td::Promise<td::BufferSlice> promise) {
  if (!ready_ || client_.empty()) {
    promise.set_error(td::Status::Error(tos::ErrorCode::notready, "failed to send query to server: not ready"));
    return false;
  }
  auto P = td::PromiseCreator::lambda([promise = std::move(promise)](td::Result<td::BufferSlice> R) mutable {
    ...
    auto F = tos::fetch_tl_object<tos::tos_api::engine_validator_controlQueryError>(data.clone(), true);
    if (F.is_ok()) {
      auto f = F.move_as_ok();
  ...
  td::BufferSlice b = tos::serialize_tl_object(
      tos::create_tl_object<tos::tos_api::engine_validator_controlQuery>(std::move(query)), true);
  td::actor::send_closure(client_, &tos::adnl::AdnlExtClient::send_query, "query", std::move(b),
...
void ValidatorEngineConsole::set_private_key(td::BufferSlice file_name) {
  auto R = [&]() -> td::Result<tos::PrivateKey> {
    TRY_RESULT_PREFIX(conf_data, td::read_file(file_name.as_slice().str()), "failed to read: ");

    return tos::PrivateKey::import(conf_data.as_slice());
  }();
...
int main(int argc, char* argv[]) {
  ...
  td::OptionParser p;
  p.set_description("console for validator for TOS Blockchain");
