...
template <>
inline td::Result<tos::PublicKeyHash> Tokenizer::get_token() {
...
template <>
inline td::Result<tos::ShardIdFull> Tokenizer::get_token() {
  TRY_RESULT(word, get_raw_token());
  auto r_wc = td::to_integer_safe<tos::WorkchainId>(word);
  if (r_wc.is_ok()) {
    TRY_RESULT_ASSIGN(word, get_raw_token());
    TRY_RESULT(shard, td::to_integer_safe<td::int64>(word));
    return tos::ShardIdFull{r_wc.move_as_ok(), (tos::ShardId)shard};
  }
  return tos::ShardIdFull::parse(word);
}
...
class AddValidatorTempKeyQuery : public Query {
 ...
 private:
  tos::PublicKeyHash perm_key_hash_;
  tos::PublicKeyHash key_hash_;
  td::uint32 expire_at_;
};
...
class SignCertificateQuery : public Query {
 ...
 private:
  ...
  td::uint32 max_size_;
  std::string out_file_;
  tos::PublicKeyHash signer_;
  td::BufferSlice signature_;
  std::unique_ptr<tos::tos_api::PublicKey> pubkey_;
...
class GetShardOutQueueSizeQuery : public Query {
 ...
 private:
  tos::BlockId block_id_;
  tos::ShardIdFull dest_ = tos::ShardIdFull{tos::workchainInvalid};
};
