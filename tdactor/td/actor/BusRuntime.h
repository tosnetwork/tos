...
class Runtime : public std::enable_shared_from_this<Runtime> {
 ...
 private:
  ...
  void wire_bus(std::shared_ptr<BusTreeNode> node) {
    ...
    // First, run providers from the base bus to the most-derived bus.
    ...
    // Next, spawn only the actors enabled for this bus instance.
    for (auto bus_type : bus_inheritance_chain) {
...
#define TOS_RUNTIME_DEFINE_EVENT_HANDLER()                                                         \
