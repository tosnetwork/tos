...
class QuicHttpServer : public td::actor::Actor {
 public:
  class ServerCallback final : public tos::quic::QuicServer::Callback {
   public:
    ...
    td::Status on_stream(tos::quic::QuicConnectionId cid, tos::quic::QuicStreamID sid, td::BufferSlice data,
    ...
    void on_closed(tos::quic::QuicConnectionId cid) override {
 ...
 private:
  void on_connected(tos::quic::QuicConnectionId cid, td::SecureString public_key) {
    auto public_key_b64 = td::base64_encode(public_key.as_slice());
    LOG(INFO) << "connected: CID, peer public key: " << public_key_b64;
  }

  void on_closed(tos::quic::QuicConnectionId cid) {
    request_buf_.erase(cid);
    LOG(INFO) << "connection closed";
  }

  void on_stream_data(tos::quic::QuicConnectionId cid, tos::quic::QuicStreamID sid, td::BufferSlice data) {
    auto &buf = request_buf_[cid][sid];
    auto s = data.as_slice();
    buf.append(s.data(), s.size());
  }

  void on_stream_end(tos::quic::QuicConnectionId cid, tos::quic::QuicStreamID sid) {
    ...
    td::actor::send_closure(server_.get(), &tos::quic::QuicServer::send_stream_data, cid, sid, td::BufferSlice(stored));
    td::actor::send_closure(server_.get(), &tos::quic::QuicServer::send_stream_end, cid, sid);
  ...
  td::actor::ActorOwn<tos::quic::QuicServer> server_;

  std::map<tos::quic::QuicConnectionId, std::map<tos::quic::QuicStreamID, std::string>> request_buf_;
