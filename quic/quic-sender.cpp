...
class QuicSender::ServerCallback final : public QuicServer::Callback {
 public:
  ...
  td::Status on_connected(QuicConnectionId cid, td::SecureString local_public_key,
    ...
    auto local_id = local_id_;
    if (!local_public_key.empty()) {
      TRY_RESULT(parsed_local_id, parse_peer_id(local_public_key));
  ...
  td::Status on_stream(QuicConnectionId cid, QuicStreamID sid, td::BufferSlice data, bool is_end) override {
    ...
    if (inserted) {
      td::uint64 mtu = get_peer_mtu_(local_id_, peer_id);
...
void QuicSender::on_mtu_updated(td::optional<adnl::AdnlNodeIdShort> local_id,
  ...
  if (!peer_id) {
    td::actor::send_closure(it->second, &QuicServer::set_default_mtu, local_id.value(), get_local_id_mtu(local_id.value()));
    return;
  }
...
td::actor::Task<td::BufferSlice> QuicSender::send_query_coro(adnl::AdnlNodeIdShort src, adnl::AdnlNodeIdShort dst,
  ...
  td::BufferSlice wire_data = create_serialize_tl_object<tos_api::quic_query>(std::move(data));
  auto cid = conn->cid;
...
td::actor::Task<> QuicSender::add_local_id_coro(adnl::AdnlNodeIdShort local_id) {
  ...
  auto server_it = servers_by_port_.find(port);
  td::actor::ActorId<QuicServer> server;
  if (server_it != servers_by_port_.end()) {
    server = server_it->second.get();
    ...
    auto owned = co_await QuicServer::create(port, std::make_unique<ServerCallback>(local_id, actor_id(this)),
...
td::actor::Task<td::Unit> QuicSender::init_connection_inner(AdnlPath path, std::shared_ptr<Connection> conn) {
  ...
  auto connection_id =
      co_await ask(server, &QuicServer::connect_with_sni, peer_host, peer_port, std::move(client_key), td::Slice("tos"),
                   td::Slice(ServerIdentity::sni(path.second)))
          .trace("connect");
...
void QuicSender::on_stream_complete(QuicConnectionId cid, QuicStreamID stream_id, td::Result<td::BufferSlice> r_data) {
  ...
  if (!connection->is_outbound) {
    ...
    auto answer_R = fetch_tl_object<tos_api::quic_answer>(data.clone(), true);
    if (answer_R.is_ok()) {
...
void QuicSender::on_answer(Connection &connection, QuicStreamID stream_id, tos_api::quic_answer &answer) {
