...
class AdnlNode : public td::actor::Actor {
 private:
  ...
  struct NodeInfo {
    ...
    double sum_time = 0.0;
    explicit NodeInfo(tos::adnl::AdnlNodeIdShort id) : id(id) {
    }
  };
 ...
 public:
  ...
  void run() {
    ...
    for (const auto &node : dht->nodes().list()) {
      ...
      static_nodes.push(tos::adnl::AdnlNode(node.adnl_id(), node.addr_list()));
    }
    td::actor::send_closure(adnl_, &tos::adnl::Adnl::add_static_nodes_from_config, std::move(static_nodes));

    tos::delay_action([SelfId = actor_id(this)]() { td::actor::send_closure(SelfId, &AdnlNode::send_pings); },
                      td::Timestamp::in(1.0));
  }
  ...
  void send_pings() {
    ...
    for (size_t i = 0; i < nodes_.size(); ++i) {
      ...
      td::actor::send_closure(
          adnl_, &tos::adnl::Adnl::send_query, local_id_, id, "ping",
          ...
          td::Timestamp::in(5.0), tos::create_serialize_tl_object<tos::tos_api::dht_ping>(td::Random::fast_uint64()));
    }
