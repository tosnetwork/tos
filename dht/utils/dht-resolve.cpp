...
class Resolver : public td::actor::Actor {
 private:
  td::actor::ActorOwn<tos::adnl::AdnlNetworkManager> network_manager_;
 ...
 public:
  Resolver(std::string global_config, int server_idx, td::uint16 port, tos::dht::DhtKey key, double timeout,
  ...
  void run() {
    network_manager_ = tos::adnl::AdnlNetworkManager::create(port_);
    keyring_ = tos::keyring::Keyring::create("");
    adnl_ = tos::adnl::Adnl::create("", keyring_.get());
    td::actor::send_closure(adnl_, &tos::adnl::Adnl::register_network_manager, network_manager_.get());

    td::IPAddress addr;
    addr.init_host_port(host_, port_).ensure();
    tos::adnl::AdnlCategoryMask mask;
    mask[0] = true;
    td::actor::send_closure(network_manager_, &tos::adnl::AdnlNetworkManager::add_self_addr, addr, mask, 0);
    auto pk = tos::privkeys::Ed25519::random();
    td::actor::send_closure(keyring_, &tos::keyring::Keyring::add_key, pk, true, [](td::Result<td::Unit>) {});
    tos::adnl::AdnlNodeIdFull local_id_full(pk.pub());
    ...
    td::actor::send_closure(adnl_, &tos::adnl::Adnl::add_id, local_id_full, std::move(addr_list), (td::uint8)0);
    ...
    auto D = tos::dht::Dht::create_client(local_id_, "", dht_config.move_as_ok(), keyring_.get(), adnl_.get());
    if (D.is_error()) {
  ...
  void send_query() {
    td::actor::send_closure(dht_, &tos::dht::Dht::get_value, key_,
                            [SelfId = actor_id(this)](td::Result<tos::dht::DhtValue> R) {
                              td::actor::send_closure(SelfId, &Resolver::got_result, std::move(R));
                            });
  }

  void got_result(td::Result<tos::dht::DhtValue> R) {
    if (R.is_error()) {
      LOG(WARNING) << "Failed to get value, retrying: " << R.move_as_error();
      tos::delay_action([SelfId = actor_id(this)]() { td::actor::send_closure(SelfId, &Resolver::send_query); },
                        td::Timestamp::in(0.25));
      return;
    }
    auto r = R.move_as_ok();
    LOG(INFO) << "Got result";
    td::TerminalIO::out() << "KEY: " << td::base64_encode(tos::serialize_tl_object(r.key().public_key().tl(), true))
                          << "\n";
    td::TerminalIO::out() << "VALUE: " << td::base64_encode(r.value().as_slice()) << "\n";
    if (!save_json_to_.empty()) {
      auto obj = tos::fetch_tl_object<tos::tos_api::Object>(r.value().as_slice(), true).ensure().move_as_ok();
  ...
  td::Result<std::shared_ptr<tos::dht::DhtGlobalConfig>> get_dht_config() {
    ...
    tos::tos_api::dht_nodes *static_nodes = nullptr;
    tos::tos_api::downcast_call(*conf.dht_, [&](auto &f) { static_nodes = f.static_nodes_.get(); });
...
int main(int argc, char *argv[]) {
  ...
  scheduler.run_in_context([&] {
    ...
    x = td::actor::create_actor<Resolver>(
        ...
        tos::dht::DhtKey{tos::PublicKeyHash(key_id.value()), key_name.value(), key_idx}, timeout, save_json_to);
  });
