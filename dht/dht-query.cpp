...
void DhtQueryFindNodes::send_one_query(adnl::AdnlNodeIdShort id) {
  auto P = create_serialize_tl_object<tos_api::dht_findNode>(get_key().tl(), get_k());
  td::BufferSlice B;
  if (client_only_) {
    B = std::move(P);
  } else {
    B = create_serialize_tl_object_suffix<tos_api::dht_query>(P.as_slice(), self_.tl());
  }
...
void DhtQueryFindNodes::on_result(td::Result<td::BufferSlice> R, adnl::AdnlNodeIdShort dst) {
  ...
  auto Res = fetch_tl_object<tos_api::dht_nodes>(R.move_as_ok(), true);
  if (Res.is_error()) {
...
void DhtQueryFindValue::send_one_query(adnl::AdnlNodeIdShort id) {
  auto P = create_serialize_tl_object<tos_api::dht_findValue>(get_key().tl(), get_k());
  td::BufferSlice B;
  if (client_only_) {
    B = std::move(P);
  } else {
...
void DhtQueryFindValue::on_result(td::Result<td::BufferSlice> R, adnl::AdnlNodeIdShort dst) {
  ...
  auto Res = fetch_tl_object<tos_api::dht_ValueResult>(R.move_as_ok(), true);
  if (Res.is_error()) {
  ...
  tos_api::downcast_call(
      *A, td::overloaded(
              ...
              [&](tos_api::dht_valueNotFound &v) { add_nodes(DhtNodesList{std::move(v.nodes_), our_network_id()}); }));
  if (need_stop) {
    stop();
...
void DhtQueryFindValue::on_result_nodes(td::Result<td::BufferSlice> R, adnl::AdnlNodeIdShort dst) {
  ...
  add_nodes(DhtNodesList{create_tl_object<tos_api::dht_nodes>(std::move(r->nodes_)), our_network_id()});
  finish_query(dst);
}
...
void DhtQueryStore::send_stores(td::Result<DhtNodesList> R) {
  ...
  for (auto &node : list.list()) {
    ...
    auto M = create_serialize_tl_object<tos_api::dht_store>(value_.tl());
...
void DhtQueryStore::store_ready(td::Result<td::BufferSlice> R) {
  if (R.is_error()) {
    ...
    auto R2 = fetch_tl_object<tos_api::dht_stored>(R.move_as_ok(), true);
    if (R2.is_error()) {
      fail_++;
...
DhtQueryRegisterReverseConnection::DhtQueryRegisterReverseConnection(
  ...
  adnl_ = adnl;
  src_ = src;
  query_ = create_serialize_tl_object<tos_api::dht_registerReverseConnection>(client.tl(), ttl, std::move(signature));
}
...
void DhtQueryRequestReversePing::send_one_query(adnl::AdnlNodeIdShort id) {
  ...
  if (client_only_) {
    ...
    B = create_serialize_tl_object_suffix<tos_api::dht_query>(query_.as_slice(), self_.tl());
  }
...
void DhtQueryRequestReversePing::on_result(td::Result<td::BufferSlice> R, adnl::AdnlNodeIdShort dst) {
  ...
  auto Res = fetch_tl_object<tos_api::dht_ReversePingResult>(R.move_as_ok(), true);
  if (Res.is_error()) {
  ...
  auto A = Res.move_as_ok();
  tos_api::downcast_call(*A, td::overloaded(
                                 [&](tos_api::dht_reversePingOk &v) {
                                   promise_.set_value(td::Unit());
                                   stop();
                                 },
                                 [&](tos_api::dht_clientNotFound &v) {
