...
void DhtMemberImpl::start_up() {
  std::vector<td::int32> methods = {tos_api::dht_getSignedAddressList::ID,
                                    tos_api::dht_findNode::ID,
                                    tos_api::dht_findValue::ID,
                                    tos_api::dht_store::ID,
                                    tos_api::dht_ping::ID,
                                    tos_api::dht_registerReverseConnection::ID,
                                    tos_api::dht_requestReversePing::ID,
                                    tos_api::dht_query::ID,
                                    tos_api::dht_message::ID,
                                    tos_api::dht_requestReversePingCont::ID};
  ...
  if (!db_root_.empty()) {
    ...
    for (td::uint32 bit = 0; bit < 256; bit++) {
      auto key = create_hash_tl_object<tos_api::dht_db_key_bucket>(bit);
      std::string value;
      auto R = kv->get(key.as_slice(), value);
      R.ensure();
      if (R.move_as_ok() == td::KeyValue::GetStatus::Ok) {
        auto V = fetch_tl_object<tos_api::dht_db_bucket>(td::BufferSlice{value}, true);
        V.ensure();
...
void DhtMemberImpl::save_to_db() {
  ...
  if (list.size() > 0) {
    auto key = create_hash_tl_object<tos_api::dht_db_key_bucket>(bit);
    auto value = create_serialize_tl_object<tos_api::dht_db_bucket>(list.tl());
...
void DhtMemberImpl::process_query(adnl::AdnlNodeIdShort src, tos_api::dht_ping &query,
                                  td::Promise<td::BufferSlice> promise) {
  ping_queries_++;
  promise.set_value(create_serialize_tl_object<tos_api::dht_pong>(query.random_id_));
}

void DhtMemberImpl::process_query(adnl::AdnlNodeIdShort src, tos_api::dht_findNode &query,
                                  td::Promise<td::BufferSlice> promise) {
  find_node_queries_++;
...
void DhtMemberImpl::process_query(adnl::AdnlNodeIdShort src, tos_api::dht_findValue &query,
                                  td::Promise<td::BufferSlice> promise) {
  find_value_queries_++;
  auto it = values_.find(DhtKeyId{query.key_});
  if (it != values_.end() && !it->second.expired()) {
    promise.set_value(create_serialize_tl_object<tos_api::dht_valueFound>(it->second.tl()));
    return;
  }
  ...
  promise.set_value(create_serialize_tl_object<tos_api::dht_valueNotFound>(R.tl()));
}
...
void DhtMemberImpl::process_query(adnl::AdnlNodeIdShort src, tos_api::dht_store &query,
                                  td::Promise<td::BufferSlice> promise) {
  store_queries_++;
  ...
  if (b.is_ok()) {
    promise.set_value(create_serialize_tl_object<tos_api::dht_stored>());
  } else {
...
void DhtMemberImpl::process_query(adnl::AdnlNodeIdShort src, tos_api::dht_getSignedAddressList &query,
                                  td::Promise<td::BufferSlice> promise) {
  get_addr_list_queries_++;

  auto P = td::PromiseCreator::lambda([promise = std::move(promise)](td::Result<DhtNode> R) mutable {
    TRY_RESULT_PROMISE_PREFIX(promise, self_node, std::move(R), "Failed to get self DHT node: ");
    promise.set_value(serialize_tl_object(self_node.tl(), true));
  });
...
void DhtMemberImpl::process_query(adnl::AdnlNodeIdShort src, tos_api::dht_registerReverseConnection &query,
                                  td::Promise<td::BufferSlice> promise) {
...
void DhtMemberImpl::process_query(adnl::AdnlNodeIdShort src, tos_api::dht_requestReversePing &query,
                                  td::Promise<td::BufferSlice> promise) {
  ...
  if (it != reverse_connections_.end() && !it->second.ttl_.is_in_past()) {
    ...
    promise.set_result(create_serialize_tl_object<tos_api::dht_reversePingOk>());
    return;
  }
  auto k = static_cast<td::uint32>(query.k_);
  k = std::min(k, max_k());
  auto R = get_nearest_nodes(get_reverse_connection_key(client).compute_key_id(), k);
  promise.set_value(create_serialize_tl_object<tos_api::dht_clientNotFound>(R.tl()));
}
...
void DhtMemberImpl::receive_query(adnl::AdnlNodeIdShort src, td::BufferSlice data,
  ...
  {
    auto R = fetch_tl_prefix<tos_api::dht_query>(data, true);
    if (R.is_ok()) {
  ...
  auto R = fetch_tl_object<tos_api::Function>(std::move(data), true);
  ...
  if (td::Random::fast(0, 127) == 0) {
    ...
    VLOG(DHT_DEBUG) << this << ": query to DHT from " << src << ": " << tos_api::to_string(Q);
  }

  VLOG(DHT_EXTRA_DEBUG) << this << ": query to DHT from " << src << ": " << tos_api::to_string(Q);

  tos_api::downcast_call(*Q, [&](auto &object) { this->process_query(src, object, std::move(promise)); });
}
...
void DhtMemberImpl::receive_message(adnl::AdnlNodeIdShort src, td::BufferSlice data) {
  auto F = fetch_tl_object<tos_api::dht_requestReversePingCont>(data, true);
  if (F.is_ok()) {
    auto S = [&]() -> td::Status {
      auto f = F.move_as_ok();
...
void DhtMemberImpl::get_value_in(DhtKeyId key, td::Promise<DhtValue> result) {
  auto P = td::PromiseCreator::lambda([key, promise = std::move(result), SelfId = actor_id(this), print_id = print_id(),
    ...
    td::actor::create_actor<DhtQueryFindValueSingle>("FindValueQuery", key, print_id, id, std::move(list), k, a,
                                                     network_id, std::move(self_node), client_only, SelfId, adnl,
                                                     std::move(promise))
        .release();
  });
                            ...
                            td::actor::send_closure(SelfId, &DhtMemberImpl::get_self_node,
                                                    [=, list = std::move(list), signature = std::move(signature),
                                                     promise = std::move(promise)](td::Result<DhtNode> R) mutable {
                                                      TRY_RESULT_PROMISE_PREFIX(promise, self_node, std::move(R),
                                                                                "Failed to get self DHT node: ");
...
void DhtMemberImpl::request_reverse_ping_cont(adnl::AdnlNode target, td::BufferSlice signature,
  ...
  get_self_node([=, target = std::move(target), signature = std::move(signature), promise = std::move(promise),
    ...
    td::actor::create_actor<DhtQueryRequestReversePing>(
        ...
        network_id, std::move(self_node), client_only, SelfId, adnl, std::move(promise))
        .release();
  });
}
  ...
  if (fill_att_.is_in_past()) {
    ...
    auto P = td::PromiseCreator::lambda([key, promise = std::move(promise), SelfId = actor_id(this),
      ...
      td::actor::create_actor<DhtQueryFindNodes>("FindNodesQuery", key, print_id, id, std::move(list), k, a, network_id,
                                                 std::move(self_node), client_only, SelfId, adnl, std::move(promise))
          .release();
    });
...
td::Result<std::shared_ptr<DhtGlobalConfig>> Dht::create_global_config(tl_object_ptr<tos_api::dht_config_Global> conf) {
  ...
  tl_object_ptr<tos_api::dht_nodes> static_nodes;
  tos_api::downcast_call(*conf, td::overloaded(
                                    [&](tos_api::dht_config_global &f) {
                                      k = f.k_;
                                      a = f.a_;
                                      network_id = -1;
                                      static_nodes = std::move(f.static_nodes_);
                                    },
                                    [&](tos_api::dht_config_global_v2 &f) {
                                      k = f.k_;
                                      a = f.a_;
