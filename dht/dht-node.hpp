...
class DhtNode {
 ...
 public:
  ...
  static td::Result<DhtNode> create(tl_object_ptr<tos_api::dht_node> obj, td::int32 our_network_id) {
    if (obj->version_ == 0) {
  ...
  td::Status update(tl_object_ptr<tos_api::dht_node> obj, td::int32 our_network_id);
  ...
  tl_object_ptr<tos_api::dht_node> tl() const {
    ...
    return create_tl_object<tos_api::dht_node>(id_.tl(), addr_list_.tl(), version_, std::move(signature_ext));
  }
  DhtNode clone() const {
...
class DhtNodesList {
 public:
  ...
  DhtNodesList(tl_object_ptr<tos_api::dht_nodes> R, td::int32 our_network_id) {
    for (auto &n : R->nodes_) {
  ...
  tl_object_ptr<tos_api::dht_nodes> tl() const;
