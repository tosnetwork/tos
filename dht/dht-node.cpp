...
td::Status DhtNode::update(tl_object_ptr<tos_api::dht_node> obj, td::int32 our_network_id) {
...
tl_object_ptr<tos_api::dht_nodes> DhtNodesList::tl() const {
  std::vector<tl_object_ptr<tos_api::dht_node>> L;
  for (auto &n : list_) {
    L.emplace_back(n.tl());
  }
  return create_tl_object<tos_api::dht_nodes>(std::move(L));
}
