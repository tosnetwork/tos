...
- ✅ Lifecycle mutation surfaces: 6 RPC handlers with real account-model detection; `contract.pool.nominator` returns deposit/withdraw `lifecycle.mutationResult` with `mutation_intent`; revoke returns a canonical bound `affected_object_preview`, while grant returns an explicitly `projected` preview until on-chain state materializes a stable permission object; immutable models return `LIFECYCLE_IMMUTABLE`; unsupported models return `PERMISSION_SOURCE_UNSUPPORTED`
