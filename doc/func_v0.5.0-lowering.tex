...
the VM level. The canonical lowering rewrites the surface method call into an
internal helper call that returns both the updated receiver and the value
result. The caller-side rebind must respect the error-path rule from the
main spec: \emph{on \texttt{Err}, the receiver is restored to its pre-call
state}. So for a \verb|mut self| method returning \verb|Result|, the
desugaring is:
...
match __r1 {
    Ok(v)  => { cs = __cs1; v }        // commit new receiver, yield value
    Err(e) => { /* cs unchanged */
                return Err(e.into()) }  // error path preserves receiver
};
\end{verbatim}

For \verb|mut self| methods that return a non-\verb|Result| type, the
writeback is unconditional (there is no failure channel, so nothing to
roll back):

\begin{verbatim}
let (__cs1, __r1) = __Slice_some_op(cs, ...);
cs = __cs1;
let v = __r1;
\end{verbatim}

\verb|__Slice_load_uint| and its siblings lower to the same \verb|LDUX|-based
helper family already used by current \verb|~load_uint| built-ins; the only
new obligation is the tag-check before writeback in the \verb|Result|
variant.
