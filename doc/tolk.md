# Tolk Execution Roadmap for TOS

## 0. Scope and references

This document specifies the concrete execution path for Tolk as
...
The document does not relitigate the choice between Tolk and
external high-level languages; that choice has been made on the
grounds of **language sovereignty**, **TVM co-evolution**, and the
**static-analysis foundation** that Tolk already provides.

## 1. Premise

Tolk is the TOS-controlled compiler from a high-level smart-contract
language directly to TVM bytecode. It lives in the same repository
as the validator, the TVM, and the system contracts:

```
/home/tomi/tos/tolk/                       — Tolk compiler (C++)
/home/tomi/tos/tolk-tester/                — language test suite
/home/tomi/tos/crypto/smartcont/tolk-stdlib/ — current stdlib (low-level only)
```
...
Tolk co-evolves with TVM. When TVM changes
(e.g. v12 full bounces — see [`Changelog.md`](../Changelog.md)),
Tolk's codegen and the language surface adapt in the same release
cycle, in the same repository, with no third-party dependency on the
critical path.

## 2. Current Tolk surface
...
- **Types.** `int`, `slice`, `cell`, `tuple`, `bytesN`, structs,
  ...
  (`tolk-tester/tests/generics-*.tolk`).
- **Functions and methods.** `fun`, methods bound to any type
  ...
  (`tolk/pipe-mini-borrow-checker.cpp`), pure/impure
...
The `actor.md` strengthening roadmap requires Tolk to grow
...
3. **No domain stdlib.** Tolk has low-level utilities only. There
...
- A canonical message-header layout: opcode, `query_id`,
  ...
  as a Tolk struct with auto-derived pack/unpack.
...
implementation is fully independent: Tolk continues to compile
...
- A second-tier stdlib aimed at common contract patterns:
  - `tolk-stdlib/jetton` — Jetton master + Jetton wallet
    templates conforming to the relevant TEP standard.
  - `tolk-stdlib/nft` — NFT collection + NFT item templates.
  - `tolk-stdlib/ownable` — owner / two-step ownership transfer
    pattern.
  - `tolk-stdlib/wallet` — wallet-vN reference implementation.
  - `tolk-stdlib/multisig` — basic multisig pattern.
- The official TOS reference contracts (wallet, Jetton wallet,
  NFT item, multisig) **rewritten in Tolk using the new syntax
...
contracts in Tolk also surfaces every rough edge in `contract` /
...
- **Scaffolding CLI.**
  `tolk new --pattern jetton-wallet my_token` generates a
  ready-to-build skeleton with the new syntax, stdlib imports,
  tests, and deploy script.
- **Documentation.**
  - Tolk language reference, current and complete.
  - "Writing TOS contracts in Tolk" — task-oriented guide that
...
- **Test harness.** A property-based / replay test framework
  similar in ergonomics to Foundry, targeting Tolk contracts.
...
- **§5.9 bounded postponement.** A language and protocol
  ...
  format (Q1).
- **§6.5 behaviour patterns formalized as Tolk traits.**
...
- **Cross-language ABI freeze.** A documented binary interface so
  that legacy FunC contracts and new Tolk contracts can call
...
These items are deliberately not on the Tolk roadmap. Listing
...
- **No premature multi-language behaviour story.**
  Until Tolk has shipped traits with conformance checking
...
| Multi-language fragmentation. | Tolk-only investment. FunC stays for low-level. No third-language commitments. |
| Tolk syntax decisions made in isolation from contract authors. | Every Q2 and Q3 deliverable goes through external dogfooding by 2–3 contract teams before stabilization. |
| Drift between TVM evolution and language surface. | Tolk and TVM ship from the same repository; every TVM-changing PR includes a Tolk-impact note. |
...
| End of Year-1 Q3 | The official wallet, Jetton master, Jetton wallet, NFT item, and multisig reference contracts all use the new Tolk syntax and the new stdlib. The old hand-written FunC versions remain only as system contracts. |
| End of Year-1 Q4 | A new contract author can produce a working, audited-pattern Jetton or NFT in under one hour using `tolk new`, the stdlib, and the documentation. |
| End of Year 2 | At least three external teams ship production contracts in Tolk using behaviour traits. The first-wave stdlib has been used by enough projects that breaking changes require a deprecation cycle. |
| End of Year 3 | Tolk + behaviour stdlib covers the patterns of at least 80% of contracts deployed in the previous quarter. Supervision, scheduled messages, and structured errors are in production. |

## 10. Closing framing

Tolk is the only TOS-controlled point in the contract-author
...
messages, capability handles — has to land in Tolk to reach
