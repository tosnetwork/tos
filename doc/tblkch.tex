...
All data in the blocks and state of the TOS Blockchain is represented as a collection of {\em cells\/} (cf.~\cite[2.5]{TON}). Therefore, this chapter begins with a general discussion of cells.
...
\nxsubpoint\label{sp:lt.ton.blkch}\emb{Logical time in the TOS Blockchain}
...
Recall that the TOS Blockchain has several so-called ``configurable parameters'' (cf.~\cite{TON}), which are either certain values or certain smart contracts residing in the masterchain. This section discusses the storage of and access to these configurable parameters.
...
\nxsubpoint\label{sp:ton.curve25519}\emb{Public and private keys for TOS Curve25519 cryptography}
...
Notice that the asymmetric Curve25519 cryptography described in~\ptref{sp:ton.curve25519} is extensively used by the TOS Network, especially the ADNL (Abstract Datagram Network Layer) protocol. However, TOS Blockchain needs elliptic curve cryptography mostly for signatures. For this purpose, Ed25519 signatures described in the next section are used.
