...
The skip certificate proves that at least $2/3+1$ of validator weight has agreed to advance beyond this slot. It does not prove that no candidate for the slot exists: a notarization certificate may also exist before finality. The protocol then advances to the next slot.
...
\item A validator may cast both a notarize vote and a skip vote for the same slot, provided it has not finalized that slot. This permits a validator that validated a candidate to participate in liveness after a timeout.
\end{enumerate}
...
Notarization and skip certificates may coexist for one slot: an honest validator may sign both votes before finalization. The safety-critical exclusion is between a finalization certificate and a skip certificate. Their supermajority signer sets intersect in more than one third of total validator weight, whose honest members cannot sign both votes by Invariant~5. Under $f<n/3$, the two certificates cannot coexist.

Two finalization certificates for different candidates in the same slot also cannot coexist under $f<n/3$. A dual-certified notarize/skip slot may cause nodes to construct different pre-finality branches. Once a candidate has signed an exact parent CandidateId, state resolution must follow that explicit chain; local skip evidence cannot erase its ancestor.
...
For a given slot, the following certificates are mutually exclusive under $f<n/3$:
\begin{itemize}
...
A slot may have both $\NotarCert(c)$ and $\SkipCert(s)$. This is a pre-finality ambiguity, not a finalized-state ambiguity. A later candidate commits to its signed parent CandidateId, and state resolution follows that chain. A slot may also have both $\NotarCert(c)$ and $\FinalCert(c)$ for the {\em same\/} candidate; this is the normal progression for a successfully finalized block.
...
The available base used to construct a future candidate and the parent of an already-signed candidate are distinct. A skip certificate may advance the available base for a new proposal. It must not substitute a different ancestor for an exact CandidateId already embedded in a signed candidate chain.
