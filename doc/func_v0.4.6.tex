...
An integer literal has the shape: an optional leading \verb|-|, followed by
either

\begin{itemize}
\item decimal digits, as in \verb|0|, \verb|17|, or \verb|-3|; or
\item the prefix \verb|0x| and hexadecimal digits (case-insensitive in a-f),
as in \verb|0xff|, \verb|0xDEAD|, or \verb|-0xff|.
\end{itemize}

The prefix \verb|0x| is recognized only in lowercase. The parser converts
integer literals into signed 257-bit values and rejects anything that does
not fit.
...
The \verb|->| operator is used in function-type expressions (see
Section~\ref{sec:types}) and as the terminator of a \verb|forall| clause.
...
The parser recognizes the following atomic types in type-expression position
(that is, in any context where \verb|parse_type| is invoked --- global variable
declarations, constant declarations, formal parameter types, return types,
type ascriptions inside expressions):
...
The keyword \verb|type| is \emph{not} accepted as a type expression. It is
reserved by the lexer and used by the parser in only two places:

\begin{itemize}
\item as an optional prefix before each bound variable in a \verb|forall|
clause (see Section~\ref{sec:types-forall});
\item as a primary expression in \verb|parse_expr100|, where it denotes an
atomic type value of kind \verb|type|. This usage is rarely written by hand in
ordinary contract source code.
\end{itemize}

Writing \verb|type| in a type-expression position, such as
\verb|global type x;| or \verb|type f();|, is a parse error.
...
The parser imposes the following static restrictions on \verb|asm| functions:

\begin{itemize}
\item at most 16 formal arguments;
\item the return type must have a well-defined fixed width, at most 16 stack
values wide;
\item each formal parameter's type must have a well-defined fixed width, at
most 16 stack values wide.
\end{itemize}

All three bounds are enforced independently. A tensor-valued argument or
return type counts as its total width in stack values.
...
If a \verb|.name| or \verb|~name| symbol is not defined but the corresponding
plain \verb|name| symbol exists, the parser falls back to the plain symbol.
The fallback applies uniformly to both dot-identifiers and tilde-identifiers.
