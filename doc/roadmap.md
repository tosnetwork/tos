...
5. ⬜ **Stage 4 — behaviour manifest foundation.**
   ...
   - ⬜ Add `request_server` behaviour manifest
     (`doc/` or `examples/slice4/`).
   - ⬜ Add `state_machine` behaviour manifest.
   - ⬜ Add `postponing_state_machine` behaviour manifest.
   - ⬜ Add compiler check (or validation script) that validates an
     ordinary Tol contract against a selected manifest without
     changing generated bytecode.
   - ⬜ Warning mode for raw code contracts; error mode for
     stdlib/generated contracts.
   - ⬜ Tests: manifest check is purely compile-time — no
     bytecode-visible trait objects, no dynamic dispatch, no vtables.
   - ⬜ Tests: a conforming contract passes; a non-conforming contract
     triggers the expected warning/error.
   - ⬜ Exit criterion: compiler proves callback/message/error shape
     for a small generated contract while preserving bytecode.
