...
run three distinct execution domains in parallel under one
...
smart contracts, and the Uno privacy workchain with PQ-native
shielded transfers. One node binary (\texttt{validator-engine})
serves these domains, and one canonical machine-facing surface
(\texttt{tos\_*}, \texttt{eth\_*}, \texttt{uno\_*}) together with one
operator path (\texttt{tosctl}) is intended to expose them.
...
is organized around three concrete execution domains:
\begin{itemize}
...
(Workchain Two). All three are economically independent — there
...
These three domains share masterchain coordination, validator
