...
\author{TOMI SETSU\\Based on the original TON paper by Dr.\ Nikolai Durov}
\begin{document}
...
\begin{abstract}
  ...
  original TON paper, preserving its architecture and broad
...
multichain network project. It keeps the original TON
...
\clearpage
\mysection{Brief Description of TOS Components}\label{sect:ton.components}
...
\item A distributed file storage technology {\em (TOS Storage;}
  cf.~\ptref{sp:ex.ton.storage}), accessible through {\em TOS
...
\item {\em TOS DNS\/} (cf.~\ptref{sp:ton.dns}), a service for
...
\nxsubpoint\label{sp:tonvm} \embt(TOS Virtual Machine.)  The {\em TOS
...
components of the TOS Project (cf.~\ptref{sect:ton.components}), its
...
{\em TOS Storage\/} (cf.~\ptref{sp:ex.ton.storage}) uses this
...
\nxsubpoint\label{sp:ton.overlays} \embt(Overlay networks in TOS.)
...
implemented as a ``tos-service''; cf.~\ptref{sp:telegram.ton.serv}),
...
\nxsubpoint\label{sp:telegram.ton.serv} \embt(A messaging application
...
\nxsubpoint\label{sp:ex.ton.proxy} \embt(Example: TOS Proxy is a fog
...
\nxsubpoint\label{sp:ton.dns} \embt(TOS DNS: a mostly on-chain
...
and resources by including TOS URIs (cf.~\ptref{sp:ton.uri}) in
...
\nxsubpoint\label{sp:ton.www} \embt(TOS WWW.)  All of the above will
