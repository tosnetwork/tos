...
run four distinct execution domains in parallel under one
masterchain-rooted consensus: the asynchronous TVM-based base
workchain, a synchronous EVM-oriented workchain for Ethereum-style
smart contracts, the Uno privacy workchain with PQ-native
shielded transfers, and the Avata JVM workchain for deterministic
Java-bytecode contracts. The first three are live in the network's
zerostate; the JVM domain is implemented and wired end-to-end, with
its on-chain activation gated behind a governance flag (see
...
\item {\em Workchain Three\/} ($\workchainid=3$): {\em Avata JVM\/},
  ...
  interpreter. It is implemented and wired end-to-end into the node;
  its on-chain activation is gated behind a governance flag (and the
  pinning of its runtime hash and genesis set), so it is not yet
