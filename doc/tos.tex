...
asset of the base chain (Workchain Zero) {\em and} the
EVM-oriented workchain (Workchain One) — a single fungible token
shared across both VMs via cross-workchain message routing — while
{\em UNO\/} is the native asset of the privacy workchain
(Workchain Two), independent of TOS by design (privacy preservation
forbids a wc=2~$\leftrightarrow$~wc=0/wc=1 bridge).
...
workchain) uses {\em the same TOS asset\/} as the base chain — TOS
flows between the TVM workchain and the EVM workchain via masterchain-
routed cross-workchain messages, with a fixed 9-decimal $\leftrightarrow$
18-decimal conversion (1 nano-tomi = $10^9$ wei). Workchain Two (Uno)
uses {\em UNO\/} as its native asset, independent of TOS: privacy
preservation precludes a wc=2 $\leftrightarrow$ wc=0/wc=1 value bridge,
so UNO is fully contained within wc=2.
