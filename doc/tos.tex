...
JVM workchain for deterministic Java-bytecode contracts. The native,
EVM and Uno engines are compiled into the default node binary; the
JVM engine is a build-time option (it requires a Java toolchain to
produce its runtime archive). In all cases a domain is \emph{active}
...
\end{itemize}
The native, EVM and Uno engines are compiled into the default node
binary (the JVM engine is an optional build); each is dormant until a
corresponding workchain descriptor appears in the on-chain
configuration. A network may therefore launch with only Workchain Zero
and bring the others online one at a time, by governance, as each
matures; for the engines present in the binary an activation is a
configuration change rather than a software upgrade, while enabling
the JVM workchain additionally requires validators to run a
JVM-enabled build.
