...
In its current product form, TOS uses this multichain architecture
with one active execution domain under masterchain-rooted consensus:
the asynchronous TVM-based base workchain. Additional workchains
remain an architectural extension point, but no custom execution
domain is registered or built into the current product configuration
(see \ptref{sp:cur.exec.domains}).
One node binary (\texttt{validator-engine}) serves the native domain,
and one canonical machine-facing surface (\texttt{tos\_*}) together
with one operator path (\texttt{tosctl}) exposes it.
...
\item A flexible multichain platform ({\em TOS Blockchain};
  ...
  formal upgradable parameters and a native execution domain.
  TOS preserves the ``self-healing'' vertical block\-chain mechanism
  (cf.~\ptref{sp:inv.sh.blk.corr}) and Instant Hypercube Routing
  (cf.~\ptref{sp:instant.hypercube}), while retaining the workchain
  model as a future extension point alongside the TVM-based base
  workchain.
...
\item Each workchain is in turn subdivided into up to $2^{60}$ {\em
  ...
  similarly to what has been discussed in one of a smart-contract platform scaling
  proposals.\footnote{\url{https://github.com/smart-contract-platform/wiki/wiki/Sharding-FAQ}}
...
heterogeneous execution domains to TOS while keeping chain
coordination, finality and workchain routing under TOS rules.
...
is organized around one concrete execution domain:
\begin{itemize}
...
\end{itemize}
The default node binary only includes this native execution path, and
the on-chain workchain configuration registers only Workchain Zero.
Future workchains, if introduced, would require their own descriptors,
software support and validator activation process.
...
In the current TOS product structure there is only one native asset:
{\em TOS\/} on Workchain Zero.
...
code in the masterchain and in the basic workchain. Other future
workchains may use other virtual machines alongside or instead of the
TVM.
...
customary stack-oriented virtual machine designs: it
...
(e.g., Bitcoin and a smart-contract platform).
...
coins or ERC-20 tokens defined in the Ethe\-reum blockchain, and
...
a smart-contract platform, and most other cryptocurrencies. This means that one cannot
...
In the Bitcoin, a smart-contract platform and other Proof-of-Work cryptocurrency mining
...
conventional Proof-of-Work blockchains like Bitcoin or a smart-contract platform, where
...
Therefore, as of 2017, more than 75\% of new a smart-contract platform or Bitcoin
blocks are produced by less than ten miners. In fact, the two largest
a smart-contract platform mining pools produce together more than half of all new
...
become a successful stand-alone Bitcoin or a smart-contract platform miner. In fact,
...
of ten (Bitcoin) to less than one hundred\footnote{More like 15, for
  ...
  a smart-contract platform transaction throughput several times larger.}  (a smart-contract platform)
...
the oldest ones (such as Bitcoin and at least the original a smart-contract platform),
use Proof-of-Work.
...
principal reason why a new blockchain project, a smart-contract platform, had to be
created.
...
agnostic of it. For example, one can try to use the a smart-contract platform
blockchain as a masterchain for an external project, and publish
special smart contracts into the a smart-contract platform blockchain for this purpose
...
as a smart-contract platform) when the load becomes too high.
...
blockchain projects (e.g., Bitcoin and a smart-contract platform; imagine two parties
want to exchange some Bitcoins, kept in the Bitcoin blockchain, into
coins, kept in the a smart-contract platform blockchain). In other words, one must
...
\item Second generation: Single-chain, PoW, {\bf smart-contract
  support}. Example: a smart-contract platform (2013; deployed in 2015), at least in
  its original form.
\item Third generation: Single-chain, {\bf PoS}, smart-contract
  support. Example: {\em future a smart-contract platform} (2018 or later).
...
masterchain for an essentially new and separate project.\footnote{For
  example, the Plasma project plans to use the a smart-contract platform blockchain as
  its (external) masterchain; it does not interact much with a smart-contract platform
  otherwise, and it could have been suggested and implemented by a
  team unrelated to the a smart-contract platform project.}
...
replace it with PoS in the future is quite complicated.\footnote{As of
  2017, a smart-contract platform is still struggling to transition from PoW to a
...
support for them seems almost impossible.\footnote{There are sharding
  proposals for a smart-contract platform dating back to 2015; it is unclear how they
  might be implemented and deployed without disrupting a smart-contract platform or
...
project, a smart-contract platform.
...
\begin{table}
  ...
  \begin{tabular}{|c|cc|ccc|ccc|}
    ...
    Int. \\ \hline Bitcoin & 2009 & 1 & PoW & no & 1 \\ a smart-contract platform &
...
\nxsubpoint \embt(a smart-contract platform \cite{SmartContractWP}.)
            \emph{Website:} \url{https://smart-contract-platform.org/}.
            {\em a smart-contract platform\/} (2015) is the first blockchain with
...
\nxsubpoint
\embt(Casper.)%
\footnote{\href{https://blog.smart-contract-platform.org/2015/08/01/introducing-casper-friendly-ghost/}{a smart-contract platform blog post on Casper FFG}}
{\em Casper\/} is an upcoming PoS algorithm for a smart-contract platform; its gradual
deployment in 2017 (or 2018), if successful, will change a smart-contract platform into
a single-chain PoS or mixed PoW+PoS system with smart contract
support, transforming a smart-contract platform into a {\em third-generation\/}
project.
...
a smart-contract platform co-founders. This project is one of the closest projects to
...
a smart-contract platform blockchain). It also uses hypercube routing, somewhat like
...
co-founder of a smart-contract platform. It is supposed to mitigate some limitations of
a smart-contract platform without introducing sharding. In essence, it is a separate
project from a smart-contract platform, introducing a hierarchy of (heterogeneous)
workchains, bound to the a smart-contract platform blockchain (to be used as an
external masterchain) at the top level. Funds can be transferred from
any blockchain up in the hierarchy (starting from the a smart-contract platform
...
computations bound to the a smart-contract platform blockchain, something like a
...
Bitcoin or a smart-contract platform, can be met quite easily (one essentially needs to
...
\texttt{validator-engine} embeds the canonical JSON-RPC namespace for
the native workchain: \texttt{tos\_*}. The same embedded server also
exposes structured health, readiness and metrics surfaces
...
need to interact with native TVM contracts in Workchain Zero. A
standardized connection layer avoids bespoke integration logic for
each application.
...
surface and the possibility of future heterogeneous workchains. A
brief comparison of the TOS
...
\bibitem{SmartContractWP}
  {\sc V.~Buterin}, {\sl a smart-contract platform: A next-generation smart contract and decentralized application platform}, \url{https://github.com/smart-contract-platform/wiki/wiki/White-Paper}, 2013.
