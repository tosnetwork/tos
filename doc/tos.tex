...
asset of the base chain (Workchain Zero), {\em EMO\/} is the native
asset of the EVM-oriented workchain (Workchain One), and {\em UNO\/}
is the native asset of the privacy workchain (Workchain Two).
...
In the current three-domain TOS product structure, Workchain One uses
{\em EMO\/} as its native asset, while Workchain Two (Uno) uses {\em
UNO\/} as its native asset. These workchain-native assets are
distinct from the base-chain asset TOS even though all three
workchains remain under one masterchain and one validator economy.
