...
Patch provenance was checked without touching the working tree: each patch was
applied separately to an isolated Git index loaded from pushed source
`0f104caee` (`git read-tree`, then `git apply --cached`, with
`--unidiff-zero` for the Rust patch). `git show :<mutated-path> | sha256sum`
reconstructed, respectively, the exact original Fift file
`7828df82…`, the red C++ source `52f1a50d…`, and the compiled red Rust source
`39497cd4…` recorded above. Thus a merely applicable but differently placed
patch is not being counted as mutation proof.
