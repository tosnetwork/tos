...
| --- | --- | --- |
| `crypto/smartcont/single-nominator-pool/validator-elect-signed.fif` | Retired at T05: no operator script remains at this path. Its exact historical Ed25519 codec moved to `crypto/test/fift/fixtures/single-nominator-legacy-elect-signed.fif` solely for the C++ byte-regression fixture. | The supported operator path is `tosctl config wallet stake`, which obtains node authorization, verifies the imported controller birth artifact and live ConfigParam 47, and uses the production pool builder. Its exact first stake was accepted and entered live ConfigParam 34 at `ca842b0f1`. The PQ pool sandbox separately refuses the classical body before relay. |
| `scripts/nominator-pool-lifecycle-e2e.py` | Deploys `nominator-pool/pool.fc`, **not** `single-nominator-pool/single-nominator-code.fc`. It now routes PQ stake orders through admitted controllers and the production pool builder. Its exact `1af9df5f1` ordinary lifecycle passed 27/27, with the separately recorded second-stake evidence limit. | Keep this multi-nominator result separate from the single-pool operator command's `ca842b0f1` product evidence. |
| `crypto/test/test-smartcont.cpp::run_validator_fift_script_regression` | Runs both base Fift tools and the relocated test-only single-pool legacy fixture with a fixed Ed25519 key, then checks bytes/BOC deserialization. It never sends to the current PQ pool, controller or Elector. | This is explicitly classical codec/parity only; its green result is not PQ stake acceptance. T07 handles the remaining base-tool dependency. |
...
| `crypto/test/fift/fixtures/liquid-controller-legacy-elect-signed.fif` | T06 retired the distributed classical operator command and retained its bytes only for `test-smartcont.cpp` parity. The current liquid controller parses PQ-shaped terms and forwards them to a named Validator Controller; the compiled-contract sandbox separately rejects the old body before relay. | This is not a proved liquid-staking product route: deployment, admission, authorization, live stake acceptance and ConfigParam 34 pairing remain untested. Keep it out of launch claims. |
