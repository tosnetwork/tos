...
This is **only T08's executable test dependency**. At T08, the distributed `crypto/smartcont/validator-elect-req.fif`, `crypto/smartcont/validator-elect-signed.fif` and `crypto/fift/lib/Validator.fif` remained installed, and T09 still included `Validator.fif`. Subsequent correction: T09 is now test-local and T10 retired the installed request script; the signed script and library remain for T11–T12. The local ZIP check does not inspect an already published release, and a green proposal smoke is not a PQ vote or stake test.
