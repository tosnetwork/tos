...
**Integration status (2026-04-20, updated)** — The v1 build surface is structurally complete end-to-end. 6 parallel build agents (A1–A6) + 3 sequential integration agents (I-A, I-B, I-C) + 3 M-batch agents (M-codec, M-liboqs, M-P2) + 3 N-batch agents (N-P2-real-poseidon2, N-P5, N-P6) + 7 K-batch agents (K-AIR, K-e2e-aead, K-fixtures, K-P6-wire, K-FRI-production, K-witness, K-p7-skip-lift, K-poseidon2-ffi, K-corrosion, K-p7-fixtures) have landed incremental slices on the `uno` branch. `cargo test` in `uno/plonky3-ffi/` passes 43 tests; `cargo test` in `tosctl/uno/` passes 49 tests; all §12 C++ test binaries pass with **zero skips** (primitive-parity 7/0, mandatory-negatives 7/0, end-to-end 0/0, parallel-verify 27/27). `validator-engine` links cleanly against the real `libuno_plonky3_ffi.a` via vendored corrosion-rs. All 45 design decisions in §16 are locked. Remaining production work is narrowly scoped: P.2 AIR column-count collapse (to shrink MVP-AIR proofs from ~33 MB at 4/4 to the §3.4 envelope of ~100 KB worst), P.3 4-core scaling to ≥3.5× (tied to P.2), and P.7 external crypto audit + 60-day testnet.
...
| **P.2** Transfer AIR + prover (Rust) | 🟡 | Hand-written Plonky3 AIR implementing claims 1–8 of §4.2: spends 1..4, outputs 1..4, in-circuit balance, Merkle paths, nullifier derivation, hash-chain key identity. Reference prover CLI in `tosctl`. | 🟡 All 8 AIR claims live over the full §4.1 envelope (1..4 × 1..4) with real Poseidon2 compression (M-P2 + K-AIR): 32-level Merkle paths (§2.3), wide-sponge Poseidon2-16 for the 15-fe `cm` absorb (§3.2), explicit u64 range-check constraints on `value_i` / `value_j` (claims 5/7), full-width nullifier PI binding (claim 4), and the single-equality in-circuit balance constraint (claim 8). Production §2.1 FRI parameters pinned (K-FRI-production: `log_blowup=2, num_queries=128, query_pow_bits=16`). **Remaining**: collapse MVP-AIR column count (currently 27,837 cols at 4/4 → ~33 MB proofs, ~790 ms verify) down to the §3.4 envelope (~52 KB typical / ~100 KB worst, ≤20 ms single-thread verify). Pure AIR-design work; all primitives and prover plumbing are in place. |
...
| **P.7** Conformance + audit + docs | 🟡 | Full test matrix green; one external crypto audit (6–10 weeks, extended scope for first-application-of-Plonky3-to-shielded-pool) covering the Transfer AIR, key derivation, transcript composition, hybrid-KEM combiner, verifier FFI, deterministic-reject ordering. Upstream primitives (Plonky3 crates, ML-KEM reference, Ristretto255 impl) out of scope. | 🟡 §12 test matrix green with **zero skips**: `test-uno-primitive-parity` 7/0 (Poseidon2 + nullifier derivation + hybrid-KEM + ML-KEM-768 round-trip all asserting real values), `test-uno-mandatory-negatives` 7/0 (replay + cross-chain replay + stale anchor + inflation + sender-linkage + receiver-linkage + HNDL-simulation all exercised against real reject paths; valid-Transfer fixture extracted via K-p7-fixtures), `test-uno-end-to-end` pass-all, `test-uno-parallel-verify` 27/27, `test-uno-state-transition-golden` pass-all with reject-path records populated (K-fixtures). **Remaining**: external crypto audit (vendor selection + 6–10 weeks window; audit scope must cover the three new constructions per §0.2 — no cv/binding_sig, fresh per-spend `rk`, `ivk`-commitment hash-chain binding), 60-day 5-validator testnet run. |

v1 release gate: all of P.0–P.7 ✅ + 60 days of 5-validator testnet stability.

**Remaining critical path to mainnet**: P.2 column-count collapse (largest; 3–5 months of focused AIR-design work to shrink MVP-AIR's 27,837 cols at 4/4 down to the §3.4 envelope) → automatic P.3 ≥3.5× 4-core scaling recovery once verify work shrinks → P.7 external audit + 60-day 5-validator testnet (4–6 months including audit window). P.5 and P.6 are ✅ and no longer on the critical path. Realistic v1 ship: **9–13 months** from the updated integration milestone above, assuming 3–4 senior engineers (down from the 5–6 estimate before P.5/P.6 landed) as the off-doc engineering plan.
