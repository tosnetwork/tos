...
**UNO's product is the intersection of the last two** — a Private L1 that is also a Post-Quantum L1, delivered as a single product. Fast L1 is inherited from TOS's 1-second global block rate. Gigagas and Teragas are not pursued: v1 accepts **4 TPS sustained per wc=2 shardchain** (horizontally scalable via additional wc=2a, wc=2b shards) as the intrinsic cost of terminal privacy + PQ-native proving on commodity validator hardware. Raw-throughput optimization at the cost of privacy is explicitly rejected (§15). Privacy and PQ are the two axes this chain optimizes; every other parameter follows.

**v1 rationale** for `BLOCK_TX_CAP = 4` per shardchain: each Transfer ships its own ~520 KB Plonky3 per-Tx STARK proof on-chain; at 4 Tx/s × ~520 KB ≈ 2 MB/s = ~16 Mbps validator bandwidth, which is consumer-broadband territory. Pushing to 30 TPS per shardchain on a per-Tx wire shape would demand 120 Mbps peak and force validators into datacenter hardware — rejected as inconsistent with the 2026 decentralization goal. Block-level proof aggregation, which would collapse 30 Tx worth of proofs into one ~420 KB block proof, is available in-tree but gated behind v2 triggers (WHIR/BaseFold maturity + specialized prover ecosystem).
...
7. **Sustained throughput: 4 TPS per wc=2 shardchain at 1 s global block time**, bandwidth-bound by the per-Tx STARK proof (~520 KB). Parallel Plonky3 verify on 4-core validator hardware has ample headroom (~30 TPS verify capacity; not the binding constraint). **Higher aggregate throughput across UNO = horizontal scale via additional wc=2a, wc=2b shardchains**, each 4 TPS. Block-level proof aggregation (which would allow ~30 TPS per shard at the cost of datacenter-grade prover hardware) is deferred to v2. This ceiling is a deliberate v1 choice for consumer-broadband validator participation; we do not fight it.
...
> under FRI Option B). §16 decision #37 (fee schedule) closed with
> a middle-path landing: original fee numbers retained at genesis,
> ConfigParam-11 escalation proposal template pre-agreed to fire if
> rolling-quarter saturation exceeds 80 % (symmetric de-escalation
> below 20 %). The legacy 30 TPS / 20 TPS targets from the pre-pivot
> block-aggregation design appear only in historical / v2-return
> contexts (§14 aggregation deferral, §15 TPS row alternative
> column). A v2 return to 30 TPS/shard via block aggregation +
> WHIR/BaseFold is preserved as an explicit future option.
...
- **Residential-fiber-feasible**: any connection ≥ 100 Mbps symmetric clears the bandwidth profile comfortably. The pre-pivot 200 Mbps sustained figure was sized for a 30-TPS block-aggregated target; per-Tx @ 4 TPS cuts it by ~8×, explicitly bringing the v1 profile into consumer-broadband territory. This is the intended design corollary of the 2026-04-21 pivot (§15 / §16 decision #21).
...
- **Proof system**: **Plonky3 STARK** with FRI polynomial commitments. No trusted setup. Transparent, post-quantum-native (hash-based). Under the pinned FRI parameters below, v1 Transfer proofs are ~520 KB for the typical 1-spend/2-output shape and ~915 KB for the 4-spend/4-output worst case; verify is ~7–30 ms on a modern validator core.
...
bytes versus Option B. After a 15-config sweep, Option B
...
- `zk_proof` is a ref to a canonical §4.1a chunk tree — Plonky3 STARK proof under the pinned §2.1 Option B FRI parameters. **Measured v1 envelope (2026-04-20, FRI Option B)**: ~520 KB for a 1-spend/2-output Transfer, ~915 KB worst-case 4/4; the original design-phase target of ~52 KB typical / ~100 KB worst-case is deferred to a post-v1 AIR architectural pass (`uni-stark → batch-stark`). **Bandwidth implication at v1 `BLOCK_TX_CAP = 4`**: typical 1/2 mix gives ~520 KB × 4 TPS ≈ **~2 MB/s (~16 Mbps)** per-validator outbound; worst-case 4/4 mix gives ~915 KB × 4 ≈ **~3.6 MB/s (~29 Mbps)**. Both well inside the residential-fiber profile (§1.4a, 25–50 Mbps sustained), which is the key design consequence of the 2026-04-21 per-Tx pivot. A v2 return to 30 TPS/shard via block aggregation (§14) would collapse block-proof to ~420 KB/block regardless of 1/2 vs 4/4 mix, yielding ~420 KB/s — a ~5× bandwidth improvement at the cost of specialized prover hardware. See §17.1 for the full cell-tree cost analysis.
- Transfer structural refs (`spends_root`, `outputs_root`, per-item cells, and inline-continuation cells) are ordinary cells only. Special cells (`PrunedBranch`, `MerkleProof`, `MerkleUpdate`, `Library`) at these positions are deterministic decode rejects, never VM exceptions.
...
Larger than a Halo2-based shielded tx (~15 KB) due to the STARK proof size; larger than the pre-pivot design-phase target (~68 KB / ~135 KB) by an order of magnitude. This is the intrinsic cost of PQ-native proving at 180-bit conjectured soundness without a post-v1 `uni-stark → batch-stark` AIR rearchitecture. Bandwidth impact analyzed in §5.9 and §7.4; the 2026-04-21 per-Tx pivot (`BLOCK_TX_CAP = 4`) keeps per-validator bandwidth in residential-fiber territory despite the ~10× per-Tx proof bloat. A conscious trade against Phase 2 proof-system migration debt.
...
`enc_ciphertext`, `mlkem_ct`, and `zk_proof` are over-sized byte blobs (up to ~915 KB for `zk_proof` worst case) that do not fit in a single cell (max 127 B per §17). They are encoded as a **balanced 4-ary chunk tree** rooted at the `^Cell` ref. This is the canonical realization of §17.3's 4-ref fan-out doctrine for all Uno oversized-blob fields; v1 proof blobs need 6-7 tree levels depending on shape.
...
**Canonicality and bounds check** (decoder): every decoder MUST reject special cells, MUST enforce `total_leaves ≤ kChunkChainMaxChunks = 8192` during the DFS walk, MUST cap total visited cells (`kChunkTreeMaxCells = 16384` in the reference implementations), and MUST reject a tree whose root hash differs from re-encoding the recovered byte stream with the canonical construction above. The leaf bound caps any single chunk-tree payload at `8192 × 127 = 1,040,384` bytes (~1.04 MB); the total-cell bound and canonical root-hash check prevent malformed non-canonical trees from hiding excessive internal-node work behind a small leaf count. This is above the ~915 KB 4/4 `zk_proof` worst case; the full external-message cap is separately governed by ConfigParam 43 (`max_ext_msg_size = 2 MiB`).
...
Both implementations are covered by the V1-3c cross-language parity test `tosctl/uno/tests/boc_parity.rs` at realistic v1 sizes (`zk_proof = 520 KB`, `mlkem_ct = 1088 B`, `enc_ciphertext = 579 B`) and by special-cell rejection regressions for C++ decode bridge paths.
...
2. **Nullifier not-spent**: for each spend, `nf ∉ nullifier_set` (LRU hit is sufficient for positive-lookup reject; negative LRU must be followed by a cell-dict lookup per §5.3). Under §13 P.3 batch verification, this check is performed speculatively against the block pre-state; before applying each speculative `Ok`, the collator/validator MUST re-check the live nullifier set after all earlier accepted txs in the same batch, and reject same-batch conflicts as `NullifierAlreadySpent`.
...
4. **Plonky3 proof** verifies (covers claims 1–8 from §4.2). Public inputs are assembled into a canonical Goldilocks field-element vector. Verifier and prover must agree **bit-identically**; the encoding below is consensus-binding and pinned for `scheme_id = 0x01`.
   ...
   **R9 audit status — public-input parity + field-material AIR (2026-04-23, updated post-Phase-4b-step3)**. The encoding above is the consensus target. Implementation closure was split into two tracks; **both now closed**:

   - **Tier 1 / launch blocker: dynamic public inputs — CLOSED (M-P2, 2026-04-22).** The shipped `uno_plonky3_prove` / `uno_plonky3_verify` FFI runs `MvpBatchProver::prove_with_range_check` / `MvpBatchVerifier::verify_with_range_check` (commit `df42d6e87`). `MvpWitness::public_inputs_bytes()` is byte-identical to C++ `build_plonky3_public_inputs(tx).to_bytes()` for every slot.
   - **Tier 2 / P.2 proper: true field-material AIR — CLOSED (Phase 4b-step3, 2026-04-23).** All in-circuit Poseidon2 / Merkle-walk constraints now run over real 32-byte material with ≥256-bit binding. Commit trail on branch `uno`:
     - Step 0 (`42c6550ef`) — widened `MvpWitness` u64 proxy fields to `[u8; 32]` on the wire.
     - Step 1 (12 sub-commits `50045938f` → `8ce78a572` + `172e079e9` for 1.3-fields) — **cm derivation via the 15-fe iterated Poseidon2-w=16 sponge under `"uno-cm-v1"` tag block**; byte-identical to `uno/crypto/poseidon2.cpp::compute_note_commitment`. PI[pi_cm(j)+k] driven by the AIR-ratified sponge output.
     - Step 2 (`b41af594e`, `9a5c93861`, `690fa6492`, `9add1ad0f`) — **nf derivation via the 9-fe iterated Poseidon2-w=16 sponge under `"uno-nf-v1"` tag block**; byte-identical to C++ `derive_nullifier`. Step 2b-AIR-v2 (`9add1ad0f`) is the spec-compliant final form — bank-1 on row 16+i / bank-2 on row 20+i with `S_NF_CARRY_{CAP,RATE}[0..8]` proxy cols for the 4-row cross-perm carry.
     - Step 3 (`36beca92a` → `a0ff246ae`) — **4-fe Merkle walk**: `SpendWitness.merkle_path` widened from `[u64; 32]` to `[[u8; 32]; 32]` (step 3c); then `S_CURRENT` widened from 1 fe to 4 fe and every per-level Poseidon2-w=8 compresses `(left[4] ‖ right[4]) → out[4]` (step 3a, `a0ff246ae`). Last-row `S_CURRENT_FE[0..4] = PI[PI_ANCHOR + 0..4]`. The Phase 4b-step1/step2b `G_ANCHOR_*` global cols were retired in the same commit.
     - Step 4 (`8d6ad205b`) — retired `MvpWitness.anchor_proxy` wire field (AIR no longer reads it); `TAIL` 53 → 45 B.

     After Phase 4b-step3, the STARK proves:
     - **Output-side `cm[4]_j = Poseidon2-hash_tagged("uno-cm-v1", real d_j, real pk_d_j, real ivk_commitment_j, value_j, real rcm_j)`** — 15-fe iterated sponge over real 32-byte material (step 1).
     - `anchor = Poseidon2-w=8-merkle-walk-4fe(real leaf[32B], [real siblings[32B]])`, 32 levels (step 3a).
     - `nf[4]_i = Poseidon2-hash_tagged("uno-nf-v1", real nk_i, real leaf_i, pos_i)` — 9-fe iterated sponge (step 2b-AIR-v2); `leaf_i` here is the 32-byte Merkle-tree leaf the spender claims to own, bound by step 3a to be the root walk's initial state.
     - `PI[cm+k] = cm[4]` / `PI[anchor+k] = anchor[4]` / `PI[nf+k] = nf[4]` all driven from AIR-ratified derivations, not witness-bytes copy-constraints.

     - Step 5 (`1156cd82b` → current — 4 sub-commits 5a-wire / 5b-decomp / 5c-sponge / 5d-close) — **spend-side claim 2 closure**: mirrored output-side step 1.2 + step 1.3-fields on the spend side. `SpendWitness.d` widened from `[u8; 8]` to `[u8; 32]` (step 5a-wire), a `SpendWitness.ivk_commitment: [u8; 32]` field added; 11 fe-limb proxy cols + 56 u16 decomposition cols per spend for d / pk_d / ivk_commitment / rcm (step 5b-decomp); and the spend-row single-perm claim-2 block replaced with a full 15-fe iterated Poseidon2-w=16 sponge — bank-1 on row i, bank-2 on row 24+i, with `S_CM_CARRY_{CAP,RATE}[0..8]` proxy cols for the cross-perm carry (step 5c-sponge). Bank-2 output binds `shared_cm_out[0..4] == (S_LEAF, S_LEAF_FE1..3) == pack_32b_as_4fe(&s.leaf)` — the real 4-fe note commitment digest, byte-identical to `compute_note_commitment` on the C++ side.

     After step 5, the STARK additionally proves:
     - **Spend-side `leaf_i = Poseidon2-hash_tagged("uno-cm-v1", real d_i, real pk_d_i, real ivk_commitment_i, value_i, real rcm_i)`** — the full 15-fe iterated sponge over real 32-byte material, byte-identical to the output-side sponge.

     Cross-crate byte-parity (both sides) locked in by `tosctl/uno/tests/phase4b_step3_sponge_parity.rs`. Rust prover + Rust FFI verifier + C++ validator all agree byte-identically on every per-slot PI derivation and on every internal hash claim. The Uno PQ-native positioning is no longer contradicted by ANY internal 64-bit proxy binding; every claim in the AIR is backed by ≥256-bit real-material cryptography.
...
| **Plonky3 proof verify** (`num_queries=52`; FRI Merkle paths + PoW check) | **7–20 ms** |
...
| Plonky3 STARK proving (`log_blowup=3, num_queries=52, query_pow_bits=24`; FRI + Poseidon2-Goldilocks) | the dominant cost | **15–40 s** |
...
| **Plonky3 proof verify (52 FRI queries + PoW check + Merkle paths)** | **7–20 ms** |
...
|---|---|---|---|---|
| Prove | Sender device | 1 | 15–40 s laptop (UX) | Plonky3 FRI (`log_blowup=3, num_queries=52, query_pow_bits=24`) + Poseidon2 on user hardware |
...
3. **Proof bandwidth** — an inter-validator cost borne `V − 1` times per tx during catchain propagation, plus once per replay. Scales with proof size (~520 KB Plonky3 typical / ~915 KB worst-case under FRI Option B — an order of magnitude above the pre-pivot ~52 KB design target; see §17.1 for the architectural trace). At v1 `BLOCK_TX_CAP = 4`, per-validator outbound ~2 MB/s ≈ 16 Mbps, consumer-broadband. The PQ-native-with-payment-grade-security trade: Plonky3 proofs at 180-bit conjectured / 102-bit proven soundness are an order of magnitude larger than Halo2 would have produced; we accepted this cost (and chose `BLOCK_TX_CAP = 4` for the per-Tx pivot) in exchange for no Phase 2 migration debt and stronger soundness margin while staying on residential-fiber-grade validator links.

An optimization that improves one axis at the expense of another is a real trade-off and must be evaluated against which axis is the currently binding constraint, not against an aggregate "performance" metric. Example: choosing Plonky3 over Halo2 costs prove time (~4–6× slower) and proof bandwidth (~5× larger), but it delivers native PQ and removes the Phase 2 migration debt (§6). Likewise, the Option B pin (`log_blowup=3, num_queries=52, query_pow_bits=24`) keeps 180-bit conjectured / 102-bit proven classical soundness while fitting the ~1 MB v1 proof envelope — a rational trade for a native-value L1.
...
**Decided — middle-path fee schedule (2026-04-21, §16 decision #37 closed).** The v1 per-Tx pivot cuts per-shard throughput ~5× (20 → 4 TPS), which cuts annual burn ~5× at the same fee schedule. Two endpoints were considered:

1. **Raise fees 5× immediately** to hold the ~1 % annual-burn target at launch. Typical tx cost 0.000255 → 0.001275 UNO (~$0.01 at $10/UNO). Rejected: a pre-traction 5× fee bump prices out early adopters before saturation is ever near, and any future reversal is a noisier ConfigParam 11 event than starting low.
2. **Accept 0.153 %/shard/year as a launch posture**, relying on horizontal shard scaling (N shards × 0.153 % → ~1 % at N ≈ 6–7) to re-aggregate the burn target as adoption grows. Rejected as a final answer: passive scaling doesn't self-correct if one shard sustains high saturation alone.

**Middle path — launch with option 2 + a governance-automatic escalation trigger**:

- **Launch values unchanged**: `min_fee_nano = 100,000`, `fee_per_byte_nano = 10`, `fee_per_spend_nano = 50,000`, `fee_per_output_nano = 50,000`. Typical tx stays at 0.000255 UNO.
- **Escalation trigger**: a ConfigParam-14-adjacent observability signal tracks rolling-quarter per-shard saturation (`shard-hours at ≥80 % of BLOCK_TX_CAP`). When aggregate cross-shard saturation exceeds 80 % over a rolling quarter, the network governance path (ConfigParam 11 voting) SHALL consider a 5× fee escalation; the specific ConfigParam update template is pre-agreed so the vote is a single up/down on a fixed proposal, not a drafting exercise under pressure.
- **Observability pre-requisite**: the saturation signal itself is computed off-chain from aggregate `uno_block_tx_count` telemetry; it is NOT consensus state and does NOT auto-mutate fees without a governance vote. "Auto-escalate" is shorthand for "template proposal is ready to activate"; the human governance loop remains explicit.
- **De-escalation is symmetric**: if saturation later drops below 20 % over a rolling quarter, the reverse-escalation proposal is equally pre-agreed. This protects against fee-freeze lock-in.

This preserves the original economics intent (1 % burn asymptote) as an *emergent* property under healthy adoption without fronting a speculative price for pre-launch demand. Early-phase per-shard TPS is under 4 anyway, so real-world launch burn is a fraction of either endpoint column; the middle-path choice costs nothing in the low-demand regime and self-adjusts once demand materializes.
...
**Legend**: ✅ complete · 🟡 scaffolded (skeleton shipped, production work remaining) · ⬜ not started.

**Integration status (2026-04-20, refreshed)** — The v1 build surface is structurally complete end-to-end and the in-house §12 test matrix is fully green. 6 parallel build agents (A1–A6) + 3 sequential integration agents (I-A, I-B, I-C) + 3 M-batch agents (M-codec, M-liboqs, M-P2) + 3 N-batch agents (N-P2-real-poseidon2, N-P5, N-P6) + 30 K-batch agents with landed commits (K-AIR, K-e2e-aead, K-fixtures, K-P6-wire, K-FRI-production, K-witness, K-p7-skip-lift, K-poseidon2-ffi, K-corrosion, K-p7-fixtures, K-restart-survival-drive, K-state-golden-unskip, K-determinism-v2, K-blake3-cell-state, K-genesis-loader, K-air-col-share, K-nullifier-warm-lru, K-tx-boc, K-bech32m, K-air-col-step2, K-openapi-uno, K-audit-scope, K-testnet-runbook, K-subs-test, K-genesis-distribution, K-ci-setup, K-codec-fuzzer, K-uno-metrics, K-air-bench, K-fri-analysis) have landed incremental slices on the `uno` branch. `cargo test` in `uno/plonky3-ffi/` passes 43 tests; `cargo test` in `tosctl/uno/` passes 49 tests; **all seven §12 C++ test binaries pass with zero unintended skips**:
...
| `test-uno-parallel-verify`          | 36 / 0 / 0                |
...
| `test-uno-subscriptions`            | 4 / 0 / 0 (K-subs-test)            |
| `test-uno-codec-fuzz`               | PASSED 1 M iters (K-codec-fuzzer)  |
| `test-uno-metrics`                  | 53 / 0 / 0 (K-uno-metrics)         |
...
1/2 common case (14 ms). R9 corrected that interpretation by splitting
P.2 into two tracks:

- **Tier 1 dynamic public inputs — CLOSED (2026-04-22, M-P2 series).**
  The `uni-stark → batch-stark` migration + the Phase 3b cross-AIR u16
  range-check LogUp + the Phase 4 byte-parity closure + the Phase 5
  FFI switch all landed in one 17-commit series on 2026-04-22.
  `MvpWitness::public_inputs_bytes()` is now byte-identical to the
  validator-rebuilt `PublicInputs(tx)` on every slot. Post-M-P2
  shape_matrix: 4/4 at 1 658 cols / 885 KB / 21 ms verify.
- **Tier 2 true field-material AIR (task #131) — post-v1.** Even
  after M-P2, the AIR still runs Poseidon2 / Merkle constraints over
  u64 PROXIES of 32-byte address material. Phase 4b-step2a/b
  specifically decoupled those proxy-derivation constraints from the
  PI wire to achieve byte parity; the cryptographic binding between
  the real cm / anchor and the STARK witness now rests on §4.1
  tx_hash + consensus-level validator checks rather than on the
  STARK itself. Tier 2 closes this gap by lifting
  `SpendWitness::{d, pk_d, ivk, rcm}` / `OutputWitness::{d, pk_d,
  ivk_commitment, rcm}` from u64 proxies to `[u8; 32]` and
  reconstructing the in-circuit derivation over real 32-byte inputs.
  Breaking witness wire format; tracked as task #131.

The performance gap is now smaller: 4/4 sits at 885 KB / 21 ms after
M-P2 (vs. the old §3.4 ~100 KB target, 8.9× gap remaining). Closing
the last gap requires a separate AIR optimization pass — **not a
proof-system change**, since batch-stark is already the default
shipped prover. P.3 remains at 95 % because the verify-time drop
makes the 4-core ≥ 3.5× scaling target effectively achievable
(verify is now small enough that Amdahl serial overhead is a smaller
fraction).

P.7's 70 % = 40 % §12 test matrix green (includes the **17-binary** set with K-codec-fuzzer closing the §12 P.2 "1 M iterations of random-bytes fuzz" line) + 5 % audit-scope preparation + 5 % 60-day testnet runbook (`doc/uno-testnet-runbook.md`, K-testnet-runbook) + 10 % CI gate (`.github/workflows/uno-ci.yml`, K-ci-setup) + 5 % Prometheus metrics endpoint for the testnet's §5 monitoring signals (`uno/rpc/metrics.{h,cpp}` + `uno_getMetrics`, K-uno-metrics) + 5 % genesis distribution builder and `tosctl uno genesis build` CLI with cross-impl parity fixture (K-genesis-distribution, which also closed a pre-existing §3.1 rcm-tag bug). Remaining 20 % external audit execution + 10 % 60-day testnet run.

The remaining ~17 % is concentrated in two blocks:

- **P.2 AIR architectural pass** (~2–4 months of focused cryptographer work). This now has two meanings that must not be conflated: (a) true field-material AIR soundness closure (replace u64 proxy constraints with real 4-limb `anchor`/`cm`/`nf` material and settle `rk`/`epk`/`filter_tag` linkage), and (b) proof-size/performance rearchitecture. Column-sharing and row-cycling have been exhausted on the current AIR structure; the viable performance paths forward are:
    ...
    - K-air-scheduler (step 4, no-commit): cross-instance Poseidon2 row-doubling proven net-negative under the earlier 128-query pin.
...
| **P.2** Transfer AIR + prover (Rust) | ✅ (byte-parity tier) / ✅ (strong-soundness tier) | Hand-written Plonky3 AIR implementing claims 1–8 of §4.2: spends 1..4, outputs 1..4, in-circuit balance, Merkle paths, nullifier derivation, hash-chain key identity. Reference prover CLI in `tosctl`. | ✅ K-AIR implements the 8 claims over the full §4.1 envelope (1..4 × 1..4). **Tier 1 (dynamic PI / byte parity) — CLOSED** by the M-P2 batch-stark migration (2026-04-22, 17 commits, shipped in `df42d6e87`). **Tier 2 (real 32-byte field-material AIR) — CLOSED** by Phase 4b-step3 (2026-04-23, ~25 commits, task #131). The AIR now proves every cryptographic claim over real 32-byte inputs with ≥256-bit binding: `cm = Poseidon2-hash_tagged("uno-cm-v1", real 15 fes)` (step 1, `50045938f` → `172e079e9`), `anchor = Poseidon2-w=8-merkle-walk-4fe(real leaf, real siblings, 32 levels)` (step 3, `36beca92a` → `a0ff246ae`), `nf = Poseidon2-hash_tagged("uno-nf-v1", real 9 fes)` (step 2, `9add1ad0f` spec-compliant v2). Cross-crate byte-parity with tosctl + C++ locked in by `tosctl/uno/tests/phase4b_step3_sponge_parity.rs` (5 tests). PI copy-constraints rewired to AIR-derived cols (step 1.3-pi `016b2b5bf`; step 3a last-row anchor bind). Legacy `anchor_proxy` / `O_CM_LIMB*` wire fields and cols retired (step 1.3-cleanup `8ce78a572`, step 3a `a0ff246ae`, step 4a `8d6ad205b`). **FRI Option B pinned 2026-04-20**: `(log_blowup=3, num_queries=52, query_pow_bits=24)` — 180 conjectured / 102 proven bits. **shape_matrix (2026-04-23, 192-core host, post-Phase-4b-step3)**:

| shape | cols | PI bytes | prove ms | verify ms | proof bytes |
|---|---:|---:|---:|---:|---:|
| 1/1 | 1 045 | 200 |   1 818 | 14.3 |   561 553 |
| 1/2 | 1 146 | 272 |   2 530 | 15.6 |   588 154 |
| 2/2 | 1 561 | 336 |  10 834 | 20.3 |   731 808 |
| 2/3 | 1 662 | 408 |   6 023 | 20.7 |   759 176 |
| 3/3 | 2 077 | 472 |     516 | 25.3 |   906 229 |
| 4/4 | 2 593 | 608 |     509 | 30.8 | 1 072 674 |

Deltas vs. pre-step3 baseline (2026-04-22, 1 658 cols / 885 KB / 21 ms verify at 4/4): +935 cols (+56 %), +188 KB proof (+21 %), +9.8 ms verify (+47 %). Width growth breakdown per spend: +96 cols for 4-fe sibling block (step 3a, 32 → 128 cols), +3 cols for 4-fe `S_CURRENT_FE` (step 3a), +38 cols for nk+leaf u16 decomp (step 2b-decomp), +16 cols for nf sponge carry (step 2b-AIR-v2). Verify time still well below the §1.4 ≤100 ms gate; proof size above the pre-Phase-4b-step3 baseline but not above the pre-FRI-Option-B historical max (2.3 MB). **Cross-AIR LogUp range-check in production**: u64 limb range on value/d/pk_d/ivk_cm/rcm/nk/leaf fe-limb cols enforced by `Kind::Global("u16_range")` from `MvpTransferAir` into `Range16Air` (height 2^16, preprocessed range-table) via `4·(n_s+n_o) + 56·n_o + 32·n_s` single-tuple lookups (56 for output + 32 for spend fe-limb decomps landed in Phase 4b-step3). |
| **P.3** Parallel verifier in C++ (FFI) | 🟡 | Link `uno_plonky3_ffi` into validator via minimal C ABI. **Must support parallel verification across N worker threads (N = num_cores) with deterministic output ordering.** Under the original pre-pivot 30 TPS target this was an activation prerequisite; under v1 `BLOCK_TX_CAP = 4`, serial verify already fits the compute budget, so the v1 posture is strong recommendation / required-for-burst-and-v2-headroom. Per-tx signatures and proof verifies run concurrently in the pool; state mutations remain serialized by tx order. | 🟡 `ParallelVerifyPool` lands with process-singleton lifecycle, per-worker verifier handle, and input-order-stable `VerifyResult` vector. Batch apply re-checks the live nullifier set in declared tx order before each speculative `Ok` is applied, so same-batch double spends reject identically on serial and parallel paths. `test-uno-parallel-verify` passes 36/36 (determinism across 1/2/4/8 workers, byte-identical state root vs serial reference, and duplicate-nullifier same-batch regression coverage). Corrosion-rs v0.5.2 vendored at `third-party/corrosion/` and wired through the top-level CMake (K-corrosion): `libuno_plonky3_ffi.a` now links into `uno_workchain` PUBLIC, which in turn propagates to `validator-engine` / `create-hardfork` / `test-tos-collator`. Measured 4-core speedup 2.81×–3.01× on MVP AIR (8-core 3.61×–6.23×); the §1.4 ≥3.5× 4-core target is tied to P.2's column-count collapse — shrinking per-tx verify work lifts scaling into the target band. **Tracked for v1 headroom and v2-return readiness**, not a hard 4 TPS activation blocker. |
...
| **P.6** Wallet (CLI via tosctl) | ✅ | Key gen from main TOS seed (§2.6), address derivation (including `pk_mlkem` + `ivk_commitment`), compact-filter scan, hybrid-KEM decrypt, build+prove+send, balance from scan. | ✅ `tosctl uno` CLI shipped (N-P6): keygen from main TOS seed, address derivation, chain-info, scan, balance. `tosctl uno send` (M-send + K-P6-wire) builds a well-formed Transfer, hybrid-KEM encrypts outputs, signs with fresh per-spend Schnorr on Ristretto255, and invokes the real Plonky3 reference prover. Wallet witness construction rebuilt against K-AIR's tightened AIR shape (K-witness): 32-level Merkle path walking, width-16 Poseidon2 `cm` absorb, full nullifier-PI hash chain. `cargo test --release` in `tosctl/uno/` passes **49 tests** (44 lib + 4 send-roundtrip + 1 derive-keys), 0 ignored, 0 failing. |
| **P.7** Conformance + audit + docs | 🟡 | Full test matrix green; one external crypto audit (6–10 weeks, extended scope for first-application-of-Plonky3-to-shielded-pool) covering the Transfer AIR, key derivation, transcript composition, hybrid-KEM combiner, verifier FFI, deterministic-reject ordering. Upstream primitives (Plonky3 crates, ML-KEM reference, Ristretto255 impl) out of scope. | 🟡 17-binary §12 test matrix green with **zero unintended skips** (see integration-status table above) including the 1 M-iteration random-bytes codec fuzzer (K-codec-fuzzer) that closes the §12 P.2 fuzz line. **Operational prep for P.7 complete**: audit-scope preparation ready for vendor handoff, 60-day testnet runbook `doc/uno-testnet-runbook.md` ready for operator bring-up (K-testnet-runbook), GitHub Actions CI gate running all 17 §12 binaries + Rust lib tests + doc-lint (K-ci-setup), Prometheus metrics exposed via `uno_getMetrics` to make the runbook's §5 monitoring signals measurable (K-uno-metrics), genesis distribution builder + `tosctl uno genesis build` CLI with Rust↔C++ cross-impl parity fixture (K-genesis-distribution, which also closed a pre-existing §3.1 rcm-tag bug). **Remaining**: external crypto audit execution (vendor selection + 6–10 weeks window; audit scope must cover the three new constructions per §0.2 — no cv/binding_sig, fresh per-spend `rk`, `ivk`-commitment hash-chain binding), 60-day 5-validator testnet run. |

v1 release gate: all of P.0–P.7 ✅ + 60 days of 5-validator testnet stability.

**Remaining critical path to mainnet**: Tier 1 dynamic public-input closure ✅ (M-P2 landed 2026-04-22 in `df42d6e87`) + Tier 2 field-material AIR closure ✅ (Phase 4b-step3 landed 2026-04-23; every cryptographic claim now proves over real 32-byte material) + post-step3 shape_matrix measured ✅ (4/4 at 2 593 cols / 1 073 KB proof / 30.8 ms verify; verify-time under §1.4 ≤100 ms gate with 3× margin) → P.7 external audit + 60-day 5-validator testnet (4–6 months including audit window; operational prep — audit-scope doc, testnet runbook, CI gate, metrics endpoint — all landed). P.3 still needs a ≥3.5× 4-core speedup re-measure for headroom/v2 readiness (tracked, shipped but measuring below target on pre-step3 AIR; post-step3 wider trace means per-tx verify work went from 21 → 31 ms so the absolute ceiling is higher but the scaling ratio should not have regressed materially). P.5 and P.6 are ✅. Realistic v1 ship: **4–6 months** from 2026-04-23, dominated by the audit window; the cryptographic closure is no longer gating.
...
| TPS posture | **4 TPS sustained per wc=2 shardchain** at 1 s global block time; scale horizontally via additional wc=2a/b shards. | v1 binding constraint is per-Tx proof bandwidth (~520 KB × 4 ≈ 2 MB/s ≈ 16 Mbps validator). Aggregation would enable 30 TPS per shard but requires datacenter hardware — deferred to v2. Verify-capacity ceiling is ~30 TPS, not binding in v1. |
...
21. **Throughput posture — decided: accept low TPS, scale horizontally.** TOS simplex consensus uses a global block rate (ConfigParam 30); per-workchain block intervals are not configurable in v1 and we do not propose extending the consensus protocol to support them. **Proof aggregation defers to v2** after the 2026-04-21 pivot; v1 ships with per-Tx proofs and `BLOCK_TX_CAP = 4` to keep validator bandwidth in consumer-broadband range (~16 Mbps × 1 s). v1 within-block optimization: parallel verify as a **strong recommendation / required-for-burst-and-v2-headroom** (§13 P.3), nullifier LRU cache (§5.3), verifier pre-load (§8.3), compact filters to offload wallet work (§2.8), and one reserved ref slot on the root state cell (§5.1). Expected per-shard ceiling: **4 TPS sustained** (bandwidth-bound, not verify-bound). Beyond 4 TPS: horizontal scaling via additional wc=2a, wc=2b shardchains — TOS supports this natively. v2 aggregation revives 30 TPS/shard when WHIR/BaseFold mature + specialized prover ecosystem emerges.
...
23. **Validator compensation — decided: masterchain in native TOS, not UNO.** wc=2 is a TOS shardchain from the consensus reward perspective; its block producers are compensated out of masterchain validator revenue in native TOS, exactly as wc=0 and wc=1 producers are. **Concrete mechanism**: ConfigParam 14 `BlockCreateFees.basechain_block_fee` (currently 1.0 TOS/block) applies uniformly to every non-masterchain shard including wc=2; no wc=2-specific premium is introduced. UNO is never minted as a validator reward. This closes the economic model cleanly without requiring a UNO inflation path, preserving decision #16's bridgeless invariant and decision #19's monotonic-non-increasing supply property. See decision #40 for the uniform-rate rationale.
...
28. **Parallel verify — decided as a v1 strong recommendation; v2-return prerequisite at 30 TPS.** Original rationale: at the pre-pivot 30 TPS target, the chain was structurally unable to produce 1 s blocks without parallel Plonky3 verify across `num_cores` workers (§13 P.3), since 30 × 25 ms = 750 ms > 400 ms compute budget. After the 2026-04-21 per-Tx pivot, 4 × 25 ms = 100 ms serial fits comfortably inside the 400 ms budget, so parallel verify becomes a **strong recommendation / required-for-burst-and-v2-headroom** rather than a structural precondition. §13 P.3 remains ✅ landed (`ParallelVerifyPool` shipped); for v1 at `BLOCK_TX_CAP = 4`, the roadmap classification should be read as a headroom requirement rather than a hard activation blocker. A v2 return to 30 TPS/shard (§14 aggregation) would immediately re-elevate this to a hard activation gate; the code path is therefore retained at full strength and the relaxation is purely a goals-statement / release-gate bookkeeping change. See §5.9 and §7.4 for the updated compute-phase math.
...
33. **FRI security parameters — decided (amended 2026-04-20): `log_blowup = 3`, `num_queries = 52`, `query_proof_of_work_bits = 24`.** §2.1 pins these as consensus-binding ("Option B"). Gives **180-bit conjectured / 102-bit proven classical soundness**, 90-bit conjectured / 51-bit proven quantum. The design-target bar is 128-bit conjectured classical; the pin sits 40 % above that bar. Above Plonky3/SP1/AggLayer defaults (`num_queries=84-100`, ~100-bit conjectured) because a soundness break on a privacy L1 with fixed-supply native asset enables unauthorized value creation, not just cross-chain bridge inconsistency.
    ...
    **Amendment history**: the original v1 decision pinned `(log_blowup = 2, num_queries = 128, pow_bits = 16)` which gave 272 conjectured / 144 proven bits — more than 2× the design goal, paying ~57 % extra proof bytes for zero realistic-adversary security gain. A 15-config parameter sweep identified Option B as the best trade-off at the same soundness tier (180 bits, well above 128). Measured delta at 4/4 worst case: proof 2.22 MB → 984 KB (−57 %); verify 55.7 ms → 24.7 ms (−56 %); prove 124 ms → 354 ms (+185 %, acceptable because prove is client-side per §1.4a).

    **Rejected alternatives**: `(log_blowup=2, num_queries=128, pow_bits=16)` — original pin, wastes 144 bits of headroom; `(2, 84, 16)` "Option A" — defensible but −33 % vs Option B's −57 % at the same soundness tier; `(3, 28, 24)` "Option C" — 108 conjectured bits, below the design target; `log_blowup=1` — AIR-structurally incompatible (constraint C4: quotient degree ≥ 3 requires blowup ≥ 4); `query_pow_bits=0` — gives back 24 bits for no proof-size savings.
34. **ConfigParam slot for UnoConfig — decided: `ConfigParam 84`.** §10.2 slot allocation follows the TOS convention of placing workchain-specific / bridge-adjacent protocol parameters in the 70s-80s cluster (existing usage: 71-73 oracle bridges, 79/81/82 jetton bridges). 84 is the first free slot after the cluster. Rejected: `26` / `27` (core-band gaps that TOS upstream may backfill with future low-numbered core-protocol extensions — clash risk); `100+` (arbitrary, breaks spatial locality with 71-82). Canonical registry entry added to `doc/ConfigParam.md`.
...
37. **Fee schedule at launch — decided (middle-path, 2026-04-21 v1-2b close): `min_fee_nano=100,000`, `fee_per_byte_nano=10`, `fee_per_spend_nano=50,000`, `fee_per_output_nano=50,000` at genesis, plus a pre-agreed ConfigParam-11 escalation proposal pinned in the launch governance template.** The v1 per-Tx pivot (decision #21) cut per-shard TPS 20 → 4, which at the original fee schedule cuts annual burn ~5× (~0.76 %/year → ~0.153 %/year per shard; see §8 economics). Two endpoints were considered: (a) raise fees 5× immediately to hold the original ~1 % burn target — rejected as front-loading a fee bump on pre-traction users who don't justify it; (b) accept the 0.153 %/shard/year as a permanent v1 posture — rejected as passively blind to sustained per-shard saturation. **Middle path (landed):** launch with the original fee numbers (option b cost posture), accompanied by a ConfigParam-11-ready escalation proposal template: if rolling-quarter per-shard saturation exceeds 80 % of `BLOCK_TX_CAP`, the governance vote to 5× the schedule is a pre-agreed single up/down rather than a drafting exercise; symmetric de-escalation if sustained saturation drops below 20 %. The saturation signal is off-chain telemetry over `uno_block_tx_count` (not consensus state); "auto-escalate" means "template is ready to activate", not "fees auto-mutate without a vote". This preserves the ~1 % burn target as an emergent property under healthy adoption while costing nothing in the low-demand launch regime. Rejected (unchanged): higher `min_fee` to $0.01-level (creates friction for legitimate users); zero-fee (insufficient DoS floor). `nullifier_lru_capacity = 1,000,000` pinned at the same time as an advisory (non-consensus) parameter and is unaffected by the v1-2b pivot.
...
--- _Tier 3 engineering-cleanup batch_ (decisions #41–45 executed during agent-branch merge; no independent design deliberation):

41. **BLAKE3 backend — decided: `UNO_BLAKE3_AVATAR` (SIMD).** A3's `crypto/internal/blake3_adapter.{h,cpp}` selects the avatar SIMD impl via `-DUNO_BLAKE3_AVATAR=ON`; `UNO_BLAKE3_REFERENCE` remains as fallback for platforms without avatar build. Picked avatar because `~/avatar` ships a production SIMD BLAKE3 we already have; no reason to add a second implementation surface.
42. **Poseidon2 round constants — decided: Polygon audited constants at P.2.** A4's MVP AIR uses `SmallRng::seed_from_u64(0x554E4F5F5743_3D32)` as a placeholder Poseidon2-8 seed; this is **only** for P.0 toolchain bring-up and **must** be replaced with `p3_goldilocks::poseidon2::GOLDILOCKS_POSEIDON2_RC_*` (Polygon's audited round constants) before the P.2 AIR is frozen. Non-negotiable consensus-binding substitution; audit surface depends on using the standard constants.
43. **Plonky3 version / vendor strategy — decided: fork into `third-party/plonky3-uno/`, pin at A4's commit `6374a36ff50fc641821513852263cc61ca7a1278` (v0.5.1 of upstream).** We only hand-cherry-pick upstream security fixes; we do **not** auto-track upstream breaking changes. Rationale: Plonky3 is not yet published to crates.io (git-rev pin requires network at first build); vendor-fork isolates us from upstream API churn; auditors have a stable artifact. Upgrade to crates.io-published 0.6.x when available and stable.
44. **A1 ↔ A2 interface style — decided: A2's method-based API wins.** A1's worktree expected free functions (`serialize_commitment_tree(tree, cb)`, `commitment_tree_append(tree, cm)`); A2 delivered methods (`tree.serialize_to_cell()`, `tree.append(cm)`). The method form is C++-idiomatic, keeps invariants on the owning type, and is already tested. During integration (Tier 4), A1's `uno/core/cell-state.cpp` and `uno/core/genesis.cpp` call sites are rewritten to use the method API (~50 lines of mechanical change).
45. **A5 weak-symbol → A3 header swap — decided: use A3's `uno_crypto::*` namespace directly.** A5 used `uno_crypto_fwd::*` weak-linked placeholder symbols to let its module compile before A3 landed. A3 delivered headers under `uno_crypto::` namespace. During integration (Tier 4), A5's `uno/core/compute-phase.cpp` `#include`s A3's real headers and swaps `uno_crypto_fwd::...` calls for `uno_crypto::...` calls; simultaneously flip `-DUNO_BLAKE3_AVAILABLE=ON` so A5's `tx_hash` path uses A3's `uno_crypto::blake3_256` instead of the `td::sha256` fallback.
