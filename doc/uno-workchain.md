...
**Integration status (2026-04-20, refreshed)** — The v1 build surface is structurally complete end-to-end and the in-house §12 test matrix is fully green. 6 parallel build agents (A1–A6) + 3 sequential integration agents (I-A, I-B, I-C) + 3 M-batch agents (M-codec, M-liboqs, M-P2) + 3 N-batch agents (N-P2-real-poseidon2, N-P5, N-P6) + 20 K-batch agents (K-AIR, K-e2e-aead, K-fixtures, K-P6-wire, K-FRI-production, K-witness, K-p7-skip-lift, K-poseidon2-ffi, K-corrosion, K-p7-fixtures, K-restart-survival-drive, K-state-golden-unskip, K-determinism-v2, K-blake3-cell-state, K-genesis-loader, K-air-col-share, K-nullifier-warm-lru, K-tx-boc, K-bech32m, K-air-col-step2) have landed incremental slices on the `uno` branch. `cargo test` in `uno/plonky3-ffi/` passes 43 tests; `cargo test` in `tosctl/uno/` passes 49 tests; **all seven §12 C++ test binaries pass with zero unintended skips**:
...
- **P.2 AIR column-count continued collapse** (~1–3 months of further AIR-design work). Two column-sharing passes have landed in sequence:
    - K-air-col-share (strategy a): Merkle-level row loop across 32 rows.
    - K-air-col-step2 (strategies b + c): wide-Poseidon2 Cm/OutCm reuse + narrow-Poseidon2 IvkCm/Nf reuse, both riding the existing row-selector bank.
  
  Combined at 4/4 worst case: cols **27,837 → 2,081 (−92.5 %)**, proof **33 MB → 2.22 MB**, verify **790 ms → 111 ms**. Remaining to reach the §3.4 envelope (~52 KB typical / ~100 KB worst, ≤ 20 ms single-thread verify): per-spend / per-output folding for sub-4/4 shapes, FRI-parameter-level bandwidth reduction, and whatever Plonky3 trace-height optimizations apply. P.3 ≥ 3.5× 4-core scaling recovers automatically once verify cost drops below ~20 ms.
...
| **P.2** Transfer AIR + prover (Rust) | 🟡 | Hand-written Plonky3 AIR implementing claims 1–8 of §4.2: spends 1..4, outputs 1..4, in-circuit balance, Merkle paths, nullifier derivation, hash-chain key identity. Reference prover CLI in `tosctl`. | 🟡 All 8 AIR claims live over the full §4.1 envelope (1..4 × 1..4) with real Poseidon2 compression (M-P2 + K-AIR). Production §2.1 FRI parameters pinned (K-FRI-production). **Two column-sharing passes landed** (steps 1 + 2 of P.2): step 1 K-air-col-share folds the 32-level Merkle compressions into a single shared w=8 column block across 32 rows; step 2 K-air-col-step2 folds the wide-Poseidon2 Cm/OutCm (claims 2 + 6) into one shared w=16 block and the narrow-Poseidon2 IvkCm/Nf (claims 3 + 4) into one shared w=8 block, both riding the same row-selector bank. Cumulative at 4/4 worst case: cols **27,837 → 2,081 (−92.5 %)**, proof **33 MB → 2.22 MB**, verify **790 ms → 111 ms** single-thread. At 1/1: cols 6,960 → 917 (−87 %). All 43 Rust FFI tests + C++ public-input golden stay byte-identical. **Remaining to reach §3.4 envelope** (~100 KB worst, ≤ 20 ms verify): per-spend / per-output folding for sub-4/4 shapes, FRI-parameter-level bandwidth tuning, trace-height shrinkage where claim overlap allows. |
...
**Remaining critical path to mainnet**: P.2 column-count continued collapse (1–3 months; two passes already landed bringing the MVP AIR to 2,081 cols / 2.22 MB proof / 111 ms verify at 4/4, cumulative −92.5 % — remaining ~22× shrink to reach §3.4 ~100 KB worst envelope via per-spend folding, FRI-parameter tuning, trace-height shrinkage) → automatic P.3 ≥ 3.5× 4-core scaling recovery once verify work shrinks below ~20 ms → P.7 external audit + 60-day 5-validator testnet (4–6 months including audit window). P.5 and P.6 are ✅ and no longer on the critical path. Realistic v1 ship: **6–10 months** from the updated integration milestone above, assuming 3–4 senior engineers.
