...
4. **Plonky3 proof** verifies (covers claims 1–8 from §4.2). Public inputs are assembled into a canonical Goldilocks field-element vector. Verifier and prover must agree **bit-identically**; the encoding below is consensus-binding and pinned for `scheme_id = 0x01`.
   ...
   **R9 audit status — public-input parity vs. true field-material AIR (2026-04-22, updated post-M-P2)**. The encoding above is the consensus target. Implementation closure is split into two deliberately separate tracks:

   - **Tier 1 / launch blocker: dynamic public inputs — CLOSED.** Closed by the M-P2 Phase 3b–5 commit series (2026-04-22). The shipped `uno_plonky3_prove` / `uno_plonky3_verify` FFI now runs `MvpBatchProver::prove_with_range_check` / `MvpBatchVerifier::verify_with_range_check` (commit `df42d6e87`). `MvpWitness::public_inputs_bytes()` is byte-identical to C++ `build_plonky3_public_inputs(tx).to_bytes()` for every slot — real `scheme_id`, `chain_id`, `expiry_block`, `fee`, 4-limb `anchor` (Phase 4b commits `720481e0e` / `8d9637d61`), per-spend `rk` (Phase 4a `e83c2cce4`), per-output `epk`, `cm[4]` (Phase 4b-step2a `b508b3213` + step1 `720481e0e`), and `filter_tag` (Phase 4a). All 391 lib / 5 integration / 66 tosctl tests green through the FFI boundary.
   - **Tier 2 / P.2 proper: true field-material AIR — tracked as Phase 4b-step3 (task #131), post-v1.** The M-P2 byte-parity closure achieves PI equality without upgrading the in-circuit derivation: Phase 4b-step2a/b specifically **decouples** the Poseidon2-w=16 / Merkle-walk constraints from the PI wire. After M-P2, the STARK proves:
     - trace-internal claim 2/6: `O_CM_CLAIM = Poseidon2-w=16(TAG_CM, d_proxy, pk_d_proxy, ivk_commitment_proxy, value, rcm_proxy)` — still over u64 proxies of address bytes, and the output no longer drives any PI slot
     - trace-internal claim 1: `S_CURRENT_last_row = poseidon2_merkle_path(proxies)` — similar
     - copy-constraint: `PI[cm+k] = witness.cm_bytes[k*8..(k+1)*8]`, `PI[anchor+k] = witness.anchor_bytes[k*8..(k+1)*8]` for k ∈ 0..4
     The "real cm = Poseidon2 over real 32-byte inputs" / "real anchor = 32-level Merkle-walk over real 32-byte siblings" cryptographic bindings are therefore NOT asserted by the STARK; they rest on §4.1 tx_hash + consensus-level validator checks (the validator's `build_plonky3_public_inputs` reads the Transfer BoC directly and verifies the same bytes against block-committed state).

   Tier 2 (Phase 4b-step3 / task #131) TODOs — close in one breaking-witness-format commit, multi-week cryptographer work per `doc/uno-p2-path-research.md` Path (iii)-step-2:
   - Migrate `SpendWitness::{d, pk_d, ivk, rcm}` and `OutputWitness::{d, pk_d, ivk_commitment, rcm}` from u64 proxies to `[u8; 32]`. Already-real `witness.cm_bytes` / `anchor_bytes` / `rk_bytes` / `epk_bytes` stay pass-through. (Audit: tosctl has every real 32-byte preimage available at witness build time; currently loses it through 11 `reduce_digest_to_proxy` calls.)
   - Bind the depth-32 Merkle path to real 4-fe sibling material. `S_CURRENT` column vector expands from 1 fe to 4 fe per spend. Each level's Poseidon2-w=8 compresses `(4-fe left ‖ 4-fe right) → 4-fe output` via `TruncatedPermutation<Poseidon2Goldilocks<8>, 2, 4, 8>` (primitive already used in `MvpCompress`; Merkle AIR itself must be hand-written — no ready-made in `plonky3-uno`).
   - Compute/output `cm[4]` in-circuit from the full §3.2 15-fe Poseidon2-16 absorb over real 32-byte inputs. Expose all 4 output limbs instead of `state[0]`. Remove Phase 4b-step2a's `O_CM_LIMB0_REAL` decoupling — limb 0 goes back to being the Poseidon2 output, but this time over real 32-byte inputs so it's both crypto-sound AND byte-equal to C++.
   - Compute/output `nf[4]` from the real nullifier material (`nk`, real `cm[4]`, `pos`); current AIR already computes nf in-circuit but over proxy leaf, so this is a minor tightening once cm is real.
   - Wire the existing-but-unused pre-check helpers `witness_anchor_bytes_consistent` / `witness_cm_bytes_consistent` in `transfer_air.rs` — they enforce `anchor_bytes[0..8] as u64 == anchor_proxy` and `cm_bytes[0..8] as u64 == poseidon2_cm_fe(...)`, checks that are currently FALSE for real tosctl witnesses (because the proxies aren't over real 32-byte material) but will be TRUE after the refactor above.
   - Regenerate golden fixtures — ONCE, after step3 lands. Phase 5 already confirmed no golden embeds proof bytes (the PI format is stable through M-P2); step3 only changes the witness byte layout.

   **Under the M-P2-closed Tier 1**: the v1 proof path is **byte-parity-complete + range-check-sound + copy-constraint-bound to consensus material**. It is NOT a complete in-circuit derivation of cm/anchor from real 32-byte inputs; that binding is offloaded to consensus. Shipping v1 without Tier 2 is a product/security-policy decision — the documented residual surface is clear and auditable.
...
| **P.2** Transfer AIR + prover (Rust) | ✅ (byte-parity tier) / 🟡 (strong-soundness tier) | Hand-written Plonky3 AIR implementing claims 1–8 of §4.2: spends 1..4, outputs 1..4, in-circuit balance, Merkle paths, nullifier derivation, hash-chain key identity. Reference prover CLI in `tosctl`. | ✅ K-AIR implements the 8 claims over the full §4.1 envelope (1..4 × 1..4). **Tier 1 (dynamic PI / byte parity) — CLOSED** by the M-P2 batch-stark migration (2026-04-22, 17 commits). The shipped `uno_plonky3_prove` / `uno_plonky3_verify` FFI now runs `MvpBatchProver::prove_with_range_check` / `MvpBatchVerifier::verify_with_range_check` (commit `df42d6e87`, Phase 5). All 6 256-bit PI fields (anchor / cm / nf / rk / epk) + `filter_tag` are byte-identical between Rust `MvpWitness::public_inputs_bytes()` and C++ `build_plonky3_public_inputs(tx).to_bytes()`: real `scheme_id`, `chain_id`, `expiry_block`, `fee` (Phase 4a scalar headers), `rk` / `epk` / `filter_tag` (Phase 4a copy-constraint, commit `e83c2cce4`), anchor limbs 1..3 / cm limbs 1..3 (Phase 4b-step1, commit `720481e0e`), and limb 0 of anchor + cm (Phase 4b-step2a/b, commits `b508b3213` / `8d9637d61`) — all routed through witness bytes and AIR-bound by row-0 copy-constraint. **Tier 2 (real 32-byte field-material AIR) — tracked as task #131** and remains post-v1 unless launch policy requires in-circuit derivation of cm/anchor over real 32-byte inputs before activation. **FRI Option B pinned 2026-04-20**: `(log_blowup=3, num_queries=52, query_pow_bits=24)` — 180 conjectured / 102 proven bits. **Post-M-P2 shape_matrix (2026-04-22, 64-core host)**: 4/4 at **1 658 cols / 885 KB / 21 ms verify** (vs. pre-Phase-3b baseline ~915 KB / 26 ms); +57 cols / +6 KB delta from the Phase 4 byte-parity closure, below the ≈−156 KB gross delta the range-check LogUp migration unlocks. **Cross-AIR LogUp range-check in production**: u64 limb range on `value_i` / `value_j` enforced by `Kind::Global("u16_range")` from `MvpTransferAir` limb cols into the new `Range16Air` (height 2^16, preprocessed range-table) via 4·(n_s+n_o) single-tuple lookups (the N-bundled-tuple bug + fix history is recorded in `doc/uno-p2-path-research.md §Path (iii) Progress log`). Column-sharing steps 1 + 2 reduced cols 27 837 → 2 081 (−92.5 %) prior to M-P2; M-P2 adds +6 cols of LogUp aux at 1/1 and ~36 aux cols at 4/4 for the range-check, offset by the 480-column savings from swapping 64 bit-decomp cols → 4 u16 limb cols (Phase 3b-step2). Measured 4/4 proof **33 MB → 915 KB → 885 KB**, verify **790 ms → 26 ms → 21 ms** across the three-way optimization series (original → FRI Option B + column-sharing → M-P2 Phase 3b/4). |
...
**Remaining critical path to mainnet**: Tier 1 dynamic public-input closure ✅ (M-P2 landed 2026-04-22, Phase 5 shipped in `df42d6e87`) → explicit launch-policy decision on Tier 2 (ship with the current AIR's documented soundness trade-off — Phase 4b-step2a/b decouples Poseidon2-w=16 / Merkle-walk from the PI wire, so the STARK proves trace well-formedness + copy-constraint PI bindings but not "real cm = Poseidon2 over real 32-byte inputs"; real cryptographic derivation rests on §4.1 tx_hash + consensus-level validator checks — OR require full field-material AIR via task #131 before activation) → P.7 external audit + 60-day 5-validator testnet (4–6 months including audit window; operational prep — audit-scope doc, testnet runbook, CI gate, metrics endpoint — all landed). If Tier 2 is required before mainnet, add the Phase 4b-step3 AIR refactor (1–2 cryptographer-weeks per `doc/uno-p2-path-research.md` Path (iii)-step-2 estimate; primitives `TruncatedPermutation<Poseidon2Goldilocks<8>, 2, 4, 8>` are upstream-ready, Merkle AIR itself must be hand-written). P.5 and P.6 are ✅ and no longer on the critical path. Realistic v1 ship remains policy-dependent: **4–6 months** under the M-P2-closed Tier-1 scope, add ~1 cryptographer-month if Tier 2 is made a pre-mainnet gate.
