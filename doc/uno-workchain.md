...
**Overall v1 completion: approximately 82 %** by work-weighted effort. The remaining 18 % is concentrated in two blocks:

- **P.2 AIR column-count continued collapse** (still the largest remaining item; ~2–4 months of focused AIR-design work). The first column-sharing pass (K-air-col-share: Merkle-level row loop, strategy (a)) already landed a **−80 % column reduction** at 4/4 (27,837 → 5,553) with proof bytes 33 MB → 6.46 MB and verify 790 ms → 148 ms. Further sharing strategies (claim-2 / claim-6 wide-Poseidon2 reuse, claim-3 / claim-4 narrow-Poseidon2 reuse, per-spend column folding) still need to land to reach the §3.4 envelope (~52 KB typical / ~100 KB worst, ≤ 20 ms single-thread verify). P.3 ≥ 3.5× 4-core scaling recovers automatically once verify cost drops.
...
| **P.2** Transfer AIR + prover (Rust) | 🟡 | Hand-written Plonky3 AIR implementing claims 1–8 of §4.2: spends 1..4, outputs 1..4, in-circuit balance, Merkle paths, nullifier derivation, hash-chain key identity. Reference prover CLI in `tosctl`. | 🟡 All 8 AIR claims live over the full §4.1 envelope (1..4 × 1..4) with real Poseidon2 compression (M-P2 + K-AIR). Production §2.1 FRI parameters pinned (K-FRI-production). **First column-sharing pass landed** (K-air-col-share, step 1 of P.2): Merkle-level row loop folds the 32 per-spend Poseidon2 compressions into a single shared column block iterated across 32 rows with one-hot row selectors and a running-digest column. Measured at 4/4 worst case: cols 27,837 → **5,553** (−80 %), proof 33 MB → **6.46 MB**, verify 790 ms → **148 ms** single-thread. At 1/1: cols 6,960 → 1,413; proof ≈ 1.8 MB; verify ≈ 42 ms. All 43 Rust FFI tests + the C++ public-input golden pass byte-identically. **Remaining to reach §3.4 envelope** (~100 KB worst, ≤ 20 ms verify): claim-2 / claim-6 wide-Poseidon2 column reuse, claim-3 / claim-4 narrow-Poseidon2 reuse, per-spend column folding for sub-4-spend shapes. |
...
| **P.7** Conformance + audit + docs | 🟡 | Full test matrix green; one external crypto audit (6–10 weeks, extended scope for first-application-of-Plonky3-to-shielded-pool) covering the Transfer AIR, key derivation, transcript composition, hybrid-KEM combiner, verifier FFI, deterministic-reject ordering. Upstream primitives (Plonky3 crates, ML-KEM reference, Ristretto255 impl) out of scope. | 🟡 Entire §12 test matrix green with **zero unintended skips**: `test-uno-primitive-parity` 7/0, `test-uno-mandatory-negatives` 7/0 (replay + cross-chain replay + stale anchor + inflation + sender-linkage + receiver-linkage + HNDL-simulation all exercised against real reject paths via the extracted valid-Transfer fixture, K-p7-fixtures), `test-uno-end-to-end` pass-all, `test-uno-parallel-verify` 27/27, `test-uno-restart-survival` 2/0 (K-restart-survival-drive: full apply/serialize/deserialize cycle), `test-uno-determinism` 3/0 (K-determinism-v2: 4-validator permuted-mempool replay yields byte-identical state fingerprints), `test-uno-state-transition-golden` 13/0/2 (K-state-golden-unskip; the 2 remaining skips are the intentional `UNO_RUN_PROVE_FIXTURES=1` opt-in for slow prove-heavy records). **Remaining**: external crypto audit (vendor selection + 6–10 weeks window; audit scope must cover the three new constructions per §0.2 — no cv/binding_sig, fresh per-spend `rk`, `ivk`-commitment hash-chain binding), 60-day 5-validator testnet run. |

v1 release gate: all of P.0–P.7 ✅ + 60 days of 5-validator testnet stability.

**Remaining critical path to mainnet**: P.2 column-count continued collapse (2–4 months; first pass K-air-col-share already landed −80 % at 4/4, bringing the MVP AIR to 5,553 cols / 6.46 MB proof / 148 ms verify — remaining ~64× shrink to reach §3.4 ~100 KB worst envelope via claim-2/6 + claim-3/4 column reuse and per-spend folding) → automatic P.3 ≥ 3.5× 4-core scaling recovery once verify work shrinks below ~20 ms → P.7 external audit + 60-day 5-validator testnet (4–6 months including audit window). P.5 and P.6 are ✅ and no longer on the critical path. Realistic v1 ship: **7–11 months** from the updated integration milestone above, assuming 3–4 senior engineers.
