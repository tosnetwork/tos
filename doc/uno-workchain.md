...
**Integration status (2026-04-20)** — 6 parallel build agents (A1–A6) + 3 sequential integration agents (I-A, I-B, I-C) have landed the v1 skeleton. `uno_workchain` library compiles cleanly. `cargo test` in `uno/plonky3-ffi/` passes 24 tests. `uno/test/test-uno-public-input-fixture` passes byte-identical Rust↔C++ cross-check. All 45 design decisions in §16 are locked via commit chain on the `uno` branch. Remaining production work is in P.2 (full Transfer AIR), P.3 (parallel verify pool), P.6 (wallet CLI), and P.7 (test matrix + external audit).
...
| **P.1** Crypto scaffolding + public-input golden fixture | ✅ | Goldilocks field ops, Poseidon2 over Goldilocks, Ristretto255, Schnorr-on-Ristretto255, ML-KEM-768, hybrid-KEM combiner, stealth-address derivation, note encryption. **Cross-agent consensus-binding test**: `uno/test/golden/public-inputs-v1.hex` — fixed Transfer inputs produce byte-identical serialized `PublicInputs` vectors (§4.3 step 4) from both A4's Rust encoder and A5's C++ encoder. Test vectors pass. | ✅ All 8 primitives landed by A3 (libsodium for Ristretto/ChaCha20/BLAKE2b, liboqs for ML-KEM-768 with `UNO_MLKEM_STUB` fallback, A4 FFI for Poseidon2-Goldilocks, in-tree C++ for Goldilocks). Public-input fixture cross-checked Rust↔C++ (I-B) — 272 B + 608 B records both match byte-for-byte. |
| **P.2** Transfer AIR + prover (Rust) | 🟡 | Hand-written Plonky3 AIR implementing claims 1–8 of §4.2: spends 1..4, outputs 1..4, in-circuit balance, Merkle paths, nullifier derivation, hash-chain key identity. Reference prover CLI in `tosctl`. | 🟡 MVP AIR scaffold in place: single Merkle-step + range-check + **ivk-commitment binding (claim 3 landed by I-B)**. Remaining: claims 1, 2, 4–8 with real Poseidon2 compression (decision #42 constants); 4–6 months of focused cryptographer work. |
| **P.3** Parallel verifier in C++ (FFI) | 🟡 | Link `uno_plonky3_ffi` into validator via minimal C ABI. **Must support parallel verification across N worker threads (N = num_cores) with deterministic output ordering — this is an ACTIVATION PREREQUISITE, not a later release gate or optimization.** Per-tx signatures and proof verifies run concurrently in the pool; state mutations remain serialized by tx order. | 🟡 FFI round-trip works (A4 + I-A); C++ `Plonky3Verifier` bridge compiles and links. Remaining: parallel pool with deterministic ordering, verify ≤ 15 ms / ≥ 3.5× 4-core scaling, byte-identical state root across parallel and serial execution. **Blocking for mainnet activation**, not for testnet. |
...
| **P.5** End-to-end compute + RPC | 🟡 | verify_transfer, apply_transfer, block-commit path. `uno_sendTransfer`, `uno_getAnchor`, `uno_getBlockFilter`, `uno_getOutputsAtBlock`, `uno_estimateFee`, `uno_getNullifierStatus`. | 🟡 A5 verify/apply skeleton + A6 all 11 `uno_*` RPC methods + 3 subscription methods landed, compiles. `validator-engine/json-rpc-server.cpp` surgical edit in place. Remaining: setter-DI bindings in `init.cpp` (A1 state reader → A6 setters, A2 filter backend → A6 setters, A5 admission → A6 setters); end-of-block subscription `notify_*` hook; two-wallet demo. |
| **P.6** Wallet (CLI via tosctl) | ⬜ | Key gen from main TOS seed (§2.6), address derivation (including `pk_mlkem` + `ivk_commitment`), compact-filter scan, hybrid-KEM decrypt, build+prove+send, balance from scan. | Not started. Future dedicated wallet agent. Can proceed in parallel with P.2 AIR work once A3's stealth-address + note-encryption API is finalized. |
| **P.7** Conformance + audit + docs | ⬜ | Full test matrix green; one external crypto audit (6–10 weeks, extended scope for first-application-of-Plonky3-to-shielded-pool) covering the Transfer AIR, key derivation, transcript composition, hybrid-KEM combiner, verifier FFI, deterministic-reject ordering. Upstream primitives (Plonky3 crates, ML-KEM reference, Ristretto255 impl) out of scope. | Not started. Audit vendor selection needs 3–6 month lead time; book after P.2 passes internal review. 60-day 5-validator testnet run is the final gate before mainnet. |

v1 release gate: all of P.0–P.7 ✅ + 60 days of 5-validator testnet stability.

**Remaining critical path to mainnet**: P.2 (largest; 4–6 months) → P.3 parallel pool (4–6 weeks) → P.5 end-to-end wiring (2–4 weeks) → P.6 wallet (8–12 weeks, can parallelize with P.2) → P.7 audit + testnet (4–6 months including audit window). Realistic v1 ship: **18–22 months** from the integration-complete milestone recorded above, assuming 5–6 senior engineers as §"How many person-months" in the off-doc engineering plan.
