...
**Remaining critical path to mainnet**: Tier 1 dynamic public-input closure ✅ (M-P2 landed 2026-04-22 in `df42d6e87`) + Tier 2 field-material AIR closure ✅ (Phase 4b-step3 landed 2026-04-23; every cryptographic claim now proves over real 32-byte material) → re-measure 4/4 shape_matrix bench under the wider post-step3 trace (estimated +~150 KB proof / +~5 ms verify vs. the 2026-04-22 baseline; re-run before activation) → P.7 external audit + 60-day 5-validator testnet (4–6 months including audit window; operational prep — audit-scope doc, testnet runbook, CI gate, metrics endpoint — all landed). The previous policy option "ship with decoupled proxy constraints" is no longer relevant since Tier 2 is closed. P.3 still needs the ≥3.5× 4-core speedup target (tracked, shipped but measuring below target on pre-step3 AIR — wider trace may affect the ratio). P.5 and P.6 are ✅. Realistic v1 ship: **4–6 months** from 2026-04-23, dominated by the audit window; the cryptographic closure is no longer gating.
