...
> WHIR/BaseFold is preserved as an explicit future option; see
> `doc/uno-aggregation-design.md` §-1 for the v1 pivot decision
> record and v2 trigger checklist.
...
4. **Plonky3 proof** verifies (covers claims 1–8 from §4.2). Public inputs are assembled into a canonical Goldilocks field-element vector. Verifier and prover must agree **bit-identically**; the encoding below is consensus-binding and pinned for `scheme_id = 0x01`.
   ...
   **R9 audit status — public-input parity + field-material AIR (2026-04-23, updated post-Phase-4b-step3)**. The encoding above is the consensus target. Implementation closure was split into two tracks; **both now closed**:
   ...
   - **Tier 2 / P.2 proper: true field-material AIR — CLOSED (Phase 4b-step3, 2026-04-23).** All in-circuit Poseidon2 / Merkle-walk constraints now run over real 32-byte material with ≥256-bit binding. Commit trail on branch `uno`:
     ...
     - **Spend-side `leaf_i = Poseidon2-hash_tagged("uno-cm-v1", real d_i, real pk_d_i, real ivk_commitment_i, value_i, real rcm_i)`** — the full 15-fe iterated sponge over real 32-byte material, byte-identical to the output-side sponge. **CLOSES Codex audit finding 1** (`doc/uno-phase4b-step3-codex-audit.md`).
...
- **Tier 1 dynamic public inputs — CLOSED (2026-04-22, M-P2 series).**
  The `uni-stark → batch-stark` migration (Path iii per
  `doc/uno-p2-path-research.md`) + the Phase 3b cross-AIR u16
...
- **Tier 2 true field-material AIR (task #131) — post-v1.** Even
  ...
  Breaking witness wire format; 1–2 cryptographer-weeks per
  `doc/uno-p2-path-research.md`. Tracked as task #131.
