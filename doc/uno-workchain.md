...
**Integration status (2026-04-20, refreshed)** — The v1 build surface is structurally complete end-to-end and the in-house §12 test matrix is fully green. 6 parallel build agents (A1–A6) + 3 sequential integration agents (I-A, I-B, I-C) + 3 M-batch agents (M-codec, M-liboqs, M-P2) + 3 N-batch agents (N-P2-real-poseidon2, N-P5, N-P6) + 27 K-batch agents (K-AIR, K-e2e-aead, K-fixtures, K-P6-wire, K-FRI-production, K-witness, K-p7-skip-lift, K-poseidon2-ffi, K-corrosion, K-p7-fixtures, K-restart-survival-drive, K-state-golden-unskip, K-determinism-v2, K-blake3-cell-state, K-genesis-loader, K-air-col-share, K-nullifier-warm-lru, K-tx-boc, K-bech32m, K-air-col-step2, K-openapi-uno, K-audit-scope, K-testnet-runbook, K-subs-test, K-genesis-distribution, K-ci-setup, K-codec-fuzzer, K-uno-metrics) have landed incremental slices on the `uno` branch. `cargo test` in `uno/plonky3-ffi/` passes 43 tests; `cargo test` in `tosctl/uno/` passes 49 tests; **all seven §12 C++ test binaries pass with zero unintended skips**:
...
- **P.2 AIR architectural pass** (~2–4 months of focused cryptographer work). Column-sharing has been pushed to its limit on the current AIR structure:
    ...
    - K-air-col-step2 (strategies b + c): wide-Poseidon2 Cm/OutCm reuse + narrow-Poseidon2 IvkCm/Nf reuse, both riding the same row-selector bank.
  ...
  Combined at 4/4 worst case: cols **27,837 → 2,081 (−92.5 %)**, proof **33 MB → 2.22 MB**, verify **790 ms → 111 ms**.
  
  **Load-bearing constraint identified by K-air-scheduler** (step 4, negative result): at the current FRI parameterization, the 4/4 proof is **leaf-bound** — per-query leaf values account for ~2.12 MB of the 2.22 MB (`128 queries × 2,081 cols × 8 bytes`) while Merkle auth paths contribute only ~73 KB. Halving trace height (64 → 32) saves only ~12 KB on the path term. Adding any column to a shared block costs ~128 × 8 bytes per added column on the leaf term — i.e. adding a whole Poseidon2 block (~180 cols) grows the proof by ~184 KB. This rules out cross-instance Poseidon2 scheduling as a proof-size win at `num_queries=128`.
  
  The ~22× gap to the §3.4 envelope (~100 KB worst, ≤ 20 ms single-thread verify) therefore requires one of: (i) **FRI parameter retuning** — the dominant lever, since cutting `num_queries` from 128 → e.g. 52 cuts the leaf term ~2.5× directly (a decision analysis is landing via K-fri-analysis), (ii) **structural AIR rewrite** that eliminates the `SPEND_PROXY_COLS`-style replication across spends (~70+ cols per spend dominates per-spend width; would need a fundamentally different claim layout), or (iii) **proof-system change** (out of scope for v1). P.3 ≥ 3.5× 4-core scaling recovers automatically once verify cost drops below ~20 ms via whichever path is taken.
