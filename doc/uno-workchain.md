...
4. **Plonky3 proof** verifies (covers claims 1–8 from §4.2). Public inputs are assembled into a canonical Goldilocks field-element vector. Verifier and prover must agree **bit-identically**; the encoding below is consensus-binding and pinned for `scheme_id = 0x01`.
   ...
   **R9 audit status — public-input parity + field-material AIR (2026-04-23, updated post-Phase-4b-step3)**. The encoding above is the consensus target. Implementation closure was split into two tracks; **both now closed**:
   ...
   - **Tier 2 / P.2 proper: true field-material AIR — CLOSED (Phase 4b-step3, 2026-04-23).** All in-circuit Poseidon2 / Merkle-walk constraints now run over real 32-byte material with ≥256-bit binding. Commit trail on branch `uno`:
     ...
     **Out-of-scope (known gap, Codex audit finding 1 / `doc/uno-phase4b-step3-codex-audit.md`):** the **spend-side claim 2** `leaf_i = Poseidon2(real d_i, real pk_d_i, real ivk_commitment_i, value_i, real rcm_i)` was NOT migrated in Phase 4b-step3. The AIR's claim-2 constraint on trace row `i` (spend `i`) still binds `shared_cm.inputs = (TAG_CM, d_u64_proxy, pk_d_u64_proxy, ivkcm_u64_proxy, value, rcm_u64_proxy, 0…)` via the pre-step3 single-permutation u64-proxy path, and `SpendWitness.d` is still `[u8; 8]`. This is a well-formedness claim (does the spender own a correctly-constructed note?) rather than a consensus binding — the two hard consensus bindings on the spend side are (a) Merkle inclusion of `leaf` under `anchor` (✅ real 4-fe, step 3a) and (b) nullifier derivation `nf = Poseidon2("uno-nf-v1", nk, leaf, pos)` (✅ real 9-fe sponge, step 2b-AIR-v2). Both are closed over real 32-byte material. The spender could, in principle, construct a synthetic leaf that is in the anchor tree without going through the canonical note construction; Phase 4b-step3 does not prevent this in the STARK, but the protocol design relies on the spender's own key material + hybrid-KEM ciphertext binding to prevent it in practice (an attacker who doesn't know the real `(d, pk_d, ivk_commitment, rcm)` for a leaf in the tree cannot decrypt the note's `enc_ciphertext`). Closing this gap is tracked as a follow-up task; scope is ~250 LOC mirroring step 1.2 on the spend side (bank-1 sponge rows 0..3 under the wide block + a per-spend carry bank).

     Cross-crate byte-parity (output side) locked in by `tosctl/uno/tests/phase4b_step3_sponge_parity.rs` (5 tests: 3 cm + 2 nf). Rust prover + Rust FFI verifier + C++ validator all agree byte-identically on every per-slot output-side PI derivation and on nf. The Uno PQ-native positioning on the output-side cryptographic surface is no longer contradicted by an internal 64-bit proxy binding; the residual spend-side well-formedness claim is explicitly scoped as above.
