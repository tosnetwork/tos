...
| Native configuration account context | Independent C++/Rust binding of actual ShardAccounts code/data/library, Config0, owned config dictionary, complete checkpoint and parent committee | 22 shared cases, 24 compiled guards and full-dependency sanitizer parity | Native commit of the account the contract produced. The contract now carries its checkpoint through every store and replaces it from the state instruction on a registry update, so parameter 46 and the account's checkpoint commit together, and a design-active chain missing either is refused rather than migrated; first-update migration is deferred and is not a P0 path. The refusal is in the contract and proven; the seeding it presumes is not written yet -- no genesis template installs parameter 46, activates the capability or stores the account checkpoint, and the rehearsal builds a validator-set descriptor rather than a whole zero state. Until one does, a genesis that activated the design would refuse its own configuration account on that account's first tick-tock |
