...
| Privileged native VM host | C++/Rust VAUTH_STATE/VAUTH_APPLY/VAUTH_BIND, transaction-scoped host ownership, no nested VM inheritance and explicit charge callback | 39 exact outcome/gas/host-call comparisons, 12 compiled guards and full-dependency sanitizer parity | Deterministic native gas and real action/commit wiring. The authority is now assembled per transaction by collation, validation and message-pool admission through one assembler, so no block-scoped installation remains |
| Native configuration account context | Independent C++/Rust binding of actual ShardAccounts code/data/library, Config0, owned config dictionary, complete checkpoint and parent committee | 22 shared cases, 24 compiled guards and full-dependency sanitizer parity | Actual c4 installation and native commit; the transaction factory is the one assembler all three execution paths already use |
