...
| P-6 | Third-party indexer (Blockscout or similar) syncs the chain without warnings | Manual run | Recommended | 🚧 — **Blockscout 9.0.2 stack landed** at `test/conformance/blockscout/` (compose + minimal env + normalising proxy); against the live 4-validator testnet (1-block idle chain) reaches `finished_indexing:true, indexed_blocks_ratio:1` in <90 s with **0 backend restarts**, but only via a `normalize-proxy.py` shim that papers over **2 hard-blocking RPC bugs** (BUG #1: `error` is a top-level string instead of `{code,message}` object — crashes `Indexer.Block.Catchup.MissingRangesCollector` GenServer; BUG #2: batch JSON-RPC rejected outright — every catchup request is batched). Plus 1 method-not-implemented (`txpool_content`, ~5400 noisy log lines/h, non-blocking) and 1 known SHAPE issue (placeholder block returned for unknown numbers, already mitigated for hive). Without BUG #1 + #2 fixed in the TOS RPC server itself, P-6 cannot be marked ✅: any third-party indexer / wallet / explorer that doesn't ship a TOS-specific shim will hit the same crash. ETA: ~1-2 engineer-days for both bug fixes (single error-serialiser + batch dispatcher in `crypto/block/evm-workchain/json-rpc/`). Once shipped, re-run after testnet has user txs to also exercise `eth_getBlockReceipts` + `eth_getLogs` + `eth_getProof` paths under indexing load. Full issue catalogue + how-to-reproduce in `test/conformance/blockscout/README.md`. |
