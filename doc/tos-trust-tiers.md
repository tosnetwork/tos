...
An Agent Wallet can use proof-backed remote verification when it cannot run a full node.
...
- Tier 2 for Agent Wallets and lightweight agents that need proof-backed reads
...
- Agent Wallets that cannot run a full node
...
| Agent Wallet / agent runner | Tier 1 or Tier 2 | Use Tier 1 when controlling material funds; Tier 2 for proof-backed lightweight operation |
...
1. **Light-client library**: When will a published verification library make Tier 2 practical for Agent Wallet and agent-runner integrators?
