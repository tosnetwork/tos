...
out=Path('/tmp/uno-d40-postzero/candidate-as-local');out.mkdir(exist_ok=False)
...
frm=b'return reject_query("invalid workchain instance ledger transition", delta.move_as_error());'
to=b'return fatal_error(delta.move_as_error_prefix("candidate delta incorrectly treated as local: "));'
...
try:
 ...
 assert report['replacement']['corrupt']['stdout']=='final_typed_kind=2\n'
 assert all(row['exit_status']==0 for key,row in report['replacement'].items() if key!='corrupt')
...
(out/'report.json').write_text(json.dumps(report,indent=2)+'\n')
print('PASS: candidate-as-local; only 931 fails; restored')
