...
out=Path('/tmp/uno-d40-postzero/local-as-candidate');out.mkdir(exist_ok=False)
...
frm=b'return fatal_error(previous.move_as_error_prefix("authenticated instance ledger: "));'
to=b'return reject_query("local ledger incorrectly treated as candidate", previous.move_as_error());'
...
def cases(label):
 result={}
 for mode in ['unchanged','corrupt','local-fault']:
...
try:
 ...
 assert report['replacement']['local-fault']['exit_status']==1
 assert report['replacement']['local-fault']['stdout']=='final_typed_kind=1\n'
 assert all(row['exit_status']==0 for key,row in report['replacement'].items() if key!='local-fault')
 assert b'failure_identity=932\n' in (out/('replacement-'+'local-fault'+'.stderr.log')).read_bytes()
finally:
...
(out/'report.json').write_text(json.dumps(report,indent=2)+'\n')
print('PASS: local-as-candidate; only 932 fails; restored')
