...
root=Path('/home/tomi/tos-m2');build=Path('/tmp/uno-publication-build');out=root/'doc/measurements/uno-m1-cadence-7008e8a40-decode';out.mkdir(exist_ok=False)
...
for name,before,after,identity in controls[1:]:
 ...
 try:
  ...
  record['run']=run(name+'-run',[str(build/'test-workchain-block'),'--filter','EngineConfigurationAcceptedCadence','--verbosity','0']);combined=(out/record['run']['stdout']).read_bytes()+(out/record['run']['stderr']).read_bytes();assert record['run']['exit_code']!=0;assert ('('+str(identity)+' != 0)').encode() in combined
 finally:
