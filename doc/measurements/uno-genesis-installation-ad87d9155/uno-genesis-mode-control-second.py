...
repo=Path('/home/tomi/tos-m2');build=Path('/tmp/uno-publication-build');out=Path('/tmp/uno-genesis-mode-control-2');out.mkdir(exist_ok=False)
...
with tempfile.TemporaryDirectory(prefix='uno-mode-copy-') as temporary:
 ...
 entry=next(e for e in json.loads((build/'compile_commands.json').read_text()) if Path(e['file']).resolve()==repo/source)
 cmd=shlex.split(entry['command']);oldobj=cmd[cmd.index('-o')+1];obj=temp/'probe.cpp.o';cmd[cmd.index('-o')+1]=str(obj);cmd=[str(copy) if Path(x).resolve()==repo/source else x for x in cmd];cmd+=['-I',str((repo/source).parent)]
