...
static td::Ref<vm::Cell> read_boc(const std::string& name) {
  std::ifstream in("/tmp/uno-m3-live-8x3sz5uk/"+name,std::ios::binary);CHECK(in.good());
...
TEST(BReview, FailedActualComponents) {
 ...
 auto record=w0.control.withdrawals.front();auto receipt=w1.origin_pending.front();auto z=y+td::make_refint(record.costs.original_reserve)-td::make_refint(h);CHECK(td::cmp(z,td::make_refint(receipt.amount))==0);
 ...
 for(auto key:{*ingress.custody_address,ingress.executor_address}){
  ...
  unsigned count=0;CHECK(txs.check_for_each([&](td::Ref<vm::CellSlice> v,td::ConstBitPtr,int){++count;gen::Transaction::Record tx;CHECK(::tlb::unpack_cell(v->prefetch_ref(),tx));CurrencyCollection paid;CHECK(paid.unpack(tx.total_fees));
   ...
   else {gen::TransactionDescr::Record_trans_workchain_entry_v3 entry;CHECK(::tlb::unpack_cell(tx.description,entry));gen::UnoV2HostEffects::Record eff;CHECK(::tlb::unpack_cell(entry.effects,eff));auto native=decode_workchain_native_effects(eff.native).move_as_ok();CHECK(native.fees);CHECK(td::cmp(native.fees->compute_fee,td::make_refint(g))==0);CHECK(td::cmp(native.fees->state_fee,td::make_refint(policy.deposit->slot_fee))==0);}
   return true;}));CHECK(count==1);
 }
 auto archive=tos::fetch_tl_object<tos::tos_api::db_candidate>(td::read_file("/tmp/uno-m3-live-8x3sz5uk/payout-recipient.candidate").move_as_ok(),true).move_as_ok();
