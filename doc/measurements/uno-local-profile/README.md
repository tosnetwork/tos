...
**Superseded by the bounded resource-policy migration:** the current default test now reads
`crypto/test/workchain-bounded-zerostate.boc`, regenerated by
`test/workchain-resource-fixture.py`. The old file below remains historical
acceptance evidence and is not rewritten. Original dependency notice follows.
