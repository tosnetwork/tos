...
class CatChainReceivedBlock {
 public:
  ...
  virtual tl_object_ptr<tos_api::catchain_block> export_tl() const = 0;
  virtual tl_object_ptr<tos_api::catchain_block_dep> export_tl_dep() const = 0;
 ...
 public:
  //change state
  virtual void initialize(tl_object_ptr<tos_api::catchain_block> block, td::SharedSlice payload) = 0;
  virtual void set_ill() = 0;
  virtual void written() = 0;
  virtual void run() = 0;

 public:
  static std::unique_ptr<CatChainReceivedBlock> create(tl_object_ptr<tos_api::catchain_block> block,
                                                       td::SharedSlice payload, CatChainReceiver *chain);
  static std::unique_ptr<CatChainReceivedBlock> create(tl_object_ptr<tos_api::catchain_block_dep> block,
  ...
  static tl_object_ptr<tos_api::catchain_block_id> block_id(const CatChainReceiver *chain,
  ...
  static CatChainBlockHash block_hash(const CatChainReceiver *chain,
                                      const tl_object_ptr<tos_api::catchain_block> &block, const td::Slice &payload);
  static CatChainBlockHash block_hash(const CatChainReceiver *chain,
                                      const tl_object_ptr<tos_api::catchain_block_dep> &block);
  ...
  static CatChainBlockPayloadHash data_payload_hash(const CatChainReceiver *chain,
                                                    const tl_object_ptr<tos_api::catchain_block_data> &data,
                                                    const td::Slice &payload);
...
inline td::StringBuilder &operator<<(td::StringBuilder &sb, const tos::catchain::CatChainReceivedBlock &block) {
