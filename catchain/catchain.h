...
inline td::StringBuilder &operator<<(td::StringBuilder &sb, const tos::catchain::CatChain::PrintId &print_id) {
  sb << "[catchain " << print_id.instance_ << "@" << print_id.local_id_ << "]";
  return sb;
}

inline td::StringBuilder &operator<<(td::StringBuilder &sb, const tos::catchain::CatChain *catchain) {
