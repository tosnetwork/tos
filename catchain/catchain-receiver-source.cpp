...
void CatChainReceiverSourceImpl::on_new_block(CatChainReceivedBlock *block) {
  ...
  if (it != blocks_.end()) {
    ...
    if (!fork_is_found()) {
      on_found_fork_proof(create_serialize_tl_object<tos_api::catchain_block_data_fork>(block->export_tl_dep(),
...
void CatChainReceiverSourceImpl::on_found_fork_proof(const td::Slice &proof) {
  if (!fork_is_found()) {
    fetch_tl_object<tos_api::catchain_block_data_fork>(proof, true).ensure();
