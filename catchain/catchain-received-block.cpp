...
void CatChainReceivedBlockImpl::initialize(tl_object_ptr<tos_api::catchain_block> block, td::SharedSlice payload) {
  if (state_ != bs_none) {
    return;
  }
...
void CatChainReceivedBlockImpl::pre_deliver(tos_api::catchain_block_data_fork &b) {
  {
    td::Status S;
  ...
  S->on_found_fork_proof(
      create_serialize_tl_object<tos_api::catchain_block_data_fork>(std::move(b.left_), std::move(b.right_)));
  S->blame(fork_id_, height_);
}

void CatChainReceivedBlockImpl::pre_deliver(tos_api::catchain_block_data_badBlock &b) {
}

void CatChainReceivedBlockImpl::pre_deliver(tos_api::catchain_block_data_nop &b) {
}
...
void CatChainReceivedBlockImpl::pre_deliver() {
  ...
  auto X = fetch_tl_object<tos_api::catchain_block_inner_Data>(payload_.as_slice(), true);
  if (X.is_error()) {
    is_custom_ = true;
  } else {
    tos_api::downcast_call(*X.move_as_ok(), [Self = this](auto &obj) { Self->pre_deliver(obj); });
  }
}
...
tl_object_ptr<tos_api::catchain_block_id> CatChainReceivedBlock::block_id(
    const CatChainReceiver *chain, const tl_object_ptr<tos_api::catchain_block> &block, const td::Slice &payload) {
  td::Bits256 hash = data_payload_hash(chain, block->data_, payload);
  return create_tl_object<tos_api::catchain_block_id>(block->incarnation_, chain->get_source_hash(block->src_).tl(),
                                                      block->height_, hash);
}
tl_object_ptr<tos_api::catchain_block_id> CatChainReceivedBlock::block_id(
    const CatChainReceiver *chain, const tl_object_ptr<tos_api::catchain_block_dep> &block) {
  return create_tl_object<tos_api::catchain_block_id>(
...
CatChainBlockHash CatChainReceivedBlock::block_hash(const CatChainReceiver *chain,
                                                    const tl_object_ptr<tos_api::catchain_block_dep> &block) {
...
td::Status CatChainReceivedBlock::pre_validate_block(const CatChainReceiver *chain,
                                                     const tl_object_ptr<tos_api::catchain_block> &block,
                                                     const td::Slice &payload) {
  // The previous CHECK aborted the validator when a Byzantine catchain peer
  // returned a fork-proof or a
...
tl_object_ptr<tos_api::catchain_block> CatChainReceivedBlockImpl::export_tl() const {
  CHECK(initialized());
  CHECK(height_ > 0);
  std::vector<tl_object_ptr<tos_api::catchain_block_dep>> deps;
  ...
  return create_tl_object<tos_api::catchain_block>(
      chain_->get_incarnation(), source_id_, height_,
      create_tl_object<tos_api::catchain_block_data>(prev_->export_tl_dep(), std::move(deps)),
      signature_.clone_as_buffer_slice());
}

tl_object_ptr<tos_api::catchain_block_dep> CatChainReceivedBlockImpl::export_tl_dep() const {
  return create_tl_object<tos_api::catchain_block_dep>(source_id_, height_, data_payload_hash_,
                                                       signature_.clone_as_buffer_slice());
}
...
CatChainReceivedBlockImpl::CatChainReceivedBlockImpl(td::uint32 source_id, const CatChainBlockPayloadHash &hash,
  ...
  block_id_hash_ = get_tl_object_sha_bits256(create_tl_object<tos_api::catchain_block_id>(
      chain->get_incarnation(), chain->get_incarnation(), height_, data_payload_hash_));
}

CatChainReceivedBlockImpl::CatChainReceivedBlockImpl(tl_object_ptr<tos_api::catchain_block> block,
...
CatChainReceivedBlockImpl::CatChainReceivedBlockImpl(tl_object_ptr<tos_api::catchain_block_dep> block,
                                                     CatChainReceiver *chain) {
  chain_ = chain;
...
std::unique_ptr<CatChainReceivedBlock> CatChainReceivedBlock::create(tl_object_ptr<tos_api::catchain_block> block,
                                                                     td::SharedSlice payload, CatChainReceiver *chain) {
  return std::make_unique<CatChainReceivedBlockImpl>(std::move(block), std::move(payload), chain);
}

std::unique_ptr<CatChainReceivedBlock> CatChainReceivedBlock::create(tl_object_ptr<tos_api::catchain_block_dep> block,
                                                                     CatChainReceiver *chain) {
...
CatChainBlockPayloadHash CatChainReceivedBlock::data_payload_hash(
    const CatChainReceiver *chain, const tl_object_ptr<tos_api::catchain_block_data> &data, const td::Slice &payload) {
