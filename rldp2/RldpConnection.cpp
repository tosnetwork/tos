...
void RldpConnection::receive_raw(td::BufferSlice packet) {
  auto F = tos::fetch_tl_object<tos::tos_api::rldp2_MessagePart>(std::move(packet), true);
  if (F.is_error()) {
    return;
  }
      ...
      inbound.receiver.next_action(td::Timestamp::now())
          .visit(td::overloaded([&](const RldpReceiver::ActionWait &wait) { wakeup_at.relax(wait.wait_till); },
                                [&](const RldpReceiver::ActionSendAck &send) {
                                  send_packet(tos::create_serialize_tl_object<tos::tos_api::rldp2_confirm>(
  ...
  for (auto &it : outbound.parts(RldpSender::Config{})) {
    ...
    action.visit(td::overloaded(
        [&](const RldpSender::ActionSend &send) {
          ...
          send_packet(tos::create_serialize_tl_object<tos::tos_api::rldp2_messagePart>(
...
void RldpConnection::receive_raw_obj(tos::tos_api::rldp2_messagePart &part) {
  if (completed_set_.count(part.transfer_id_) > 0) {
    send_packet(tos::create_serialize_tl_object<tos::tos_api::rldp2_complete>(part.transfer_id_, part.part_));
    return;
  }
  ...
  auto r_fec_type = tos::fec::FecType::create(std::move(part.fec_type_));
    ...
    if (in_part->receiver.on_received(seqno + 1, td::Timestamp::now())) {
      ...
      if (in_part->decoder->may_try_decode()) {
        ...
        if (r_data.is_ok()) {
          ...
          inbound.finish_part(part_idx, std::move(decoded));
          to_part_completed_.emplace_back(transfer_id, part_idx, decoded_bytes);
        }
      }
    }
...
void RldpConnection::receive_raw_obj(tos::tos_api::rldp2_complete &complete) {
...
void RldpConnection::receive_raw_obj(tos::tos_api::rldp2_confirm &confirm) {
