...
// which guarantees that the strict-2/3 quorum check
//   sig_weight * 3 > total_weight * 2
// and the threshold expression
//   floor(total_weight * 2 / 3) + 1
...
// Returns true iff signed_weight strictly exceeds the 2/3 quorum of
// total_weight. Computed in 128-bit so the result is correct even if a
// future loader change ever lets total_weight exceed
...
// Returns floor(total_weight * 2 / 3) + 1: the smallest weight that
// passes has_quorum() above. Computed in 128-bit; throws if the
// 64-bit cap is somehow exceeded (should be impossible while the loader
// enforces kMaxTotalValidatorWeight).
inline ValidatorWeight two_thirds_plus_one(ValidatorWeight total_weight) {
  __uint128_t t = static_cast<__uint128_t>(total_weight);
  __uint128_t q = (t * 2) / 3 + 1;
  if (q > UINT64_MAX) {
