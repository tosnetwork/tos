...
describe("Bridge contract", () => {
  ...
  const tosAddressHash = TOS_ADDRESS_HASH;
  ...
  it("Cant lock if no allowLock ", async () => {
    ...
    try {
      ...
      await bridge.lock(token.address, bridgeAllowance, tosAddressHash);
  ...
  it("Cant lock wrapped jetton", async () => {
    ...
    try {
      await bridge.lock(wrappedJetton.address, bridgeAllowance, tosAddressHash);
      expect.fail()
    } catch (err: any) {
  ...
  it("lock big supply 2**120 - 1", async () => {
    ...
    await bridge.lock(tokenBigSupply.address, bridgeAllowance, tosAddressHash);
  ...
  it("Should unlock token", async () => {
    ...
    await expect(bridge.unlock(data, signatures))
        ...
        .withArgs(token.address, tosAddressHash.toLowerCase(), TOS_TX_HASH.toLowerCase(), TOS_TX_LT, owner.address, amount, parseUnits("3"));
  ...
  it("Should disable and enable token", async () => {
    ...
    try {
      await bridge.lock(tokenWithoutMetadata.address, bridgeAllowance, tosAddressHash);
      expect.fail()
    } catch (err: any) {
