...
3. `jetton-minter.fc`: the wrapped-token metadata URI prefix is now `https://tos.network/bridge/token/`. This literal is compiled into the minter code, so **the minter code hash and every state-init-derived minter address differ from the upstream artifacts by design**. Deployment tooling must derive addresses from this build, never from upstream expectations.
