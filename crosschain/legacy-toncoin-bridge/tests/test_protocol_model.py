#!/usr/bin/env python3
"""Dependency-free protocol invariants for the legacy oracle Toncoin bridge.
...
def vote_digest(contract: str, kind: str, payload: bytes) -> bytes:
    ...
    return hashlib.sha3_256(
        b"TOS_LEGACY_TONCOIN" + contract.encode() + kind.encode() + payload
    ).digest()


@dataclass
class WrappedTonBridgeModel:
    """Models Bridge.sol + WrappedTON.sol from the pinned Solidity plane."""
...
class TvmBridgeModel:
    """Models the locked-Toncoin side of bridge_code.fc."""
...
class MintVoteTest(unittest.TestCase):
    def setUp(self) -> None:
        self.bridge = WrappedTonBridgeModel(ORACLES)
...
class SupplyConservationTest(unittest.TestCase):
    def test_locked_equals_minted_over_round_trip(self) -> None:
        tvm = TvmBridgeModel()
        evm = WrappedTonBridgeModel(ORACLES)
