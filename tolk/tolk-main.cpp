...
static std::string auto_discover_stdlib_folder() {
...
#ifdef TD_DARWIN
  std::string def_location = join_path(executable_dir, "../share/tos/tos/smartcont/tolk-stdlib");
#elif TD_WINDOWS
  std::string def_location = join_path(executable_dir, "smartcont/tolk-stdlib");
#else  // linux
  std::string def_location = join_path(executable_dir, "../share/tos/smartcont/tolk-stdlib");
#endif
  ...
  // maybe it's just built from sources? e.g. ~/tos/cmake-build-debug/tolk/tolk
