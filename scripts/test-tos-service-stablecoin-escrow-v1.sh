...
"$REPO_ROOT/scripts/build-tos-service-stablecoin-escrow-v1.sh" "$TEST_DIR/first.boc" >/dev/null
"$REPO_ROOT/scripts/build-tos-service-stablecoin-escrow-v1.sh" "$TEST_DIR/second.boc" >/dev/null
cmp "$TEST_DIR/first.boc" "$TEST_DIR/second.boc"
python3 - "$REPO_ROOT/crypto/smartcont/tos-service-stablecoin-escrow-v1.boc.base64" "$TEST_DIR/frozen.boc" <<'PY'
import base64
import pathlib
import sys
...
python3 - "$REPO_ROOT/crypto/smartcont/tos-service-stablecoin-escrow-v1.release.json" <<'PY'
import json
import pathlib
import sys
...
assert manifest["protocol"] == "tos_service_stablecoin_escrow_v1"
