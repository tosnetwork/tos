#!/usr/bin/env python3
"""Opt-in real-manager cold-join baseline; not Counter/UNO sync acceptance.

Uses independent process databases and only a shared trusted genesis. Keeps
diagnostics, refuses more than three retained runs, and never targets other
...
async def exercise(root, build, port, join_timeout):
    ...
    async with Network(install, network_dir, base_port=port) as network:
        ...
        return {"scope": "native/masterchain real-manager cold-join baseline only",
                "counter_workchain_tested": False, "uno_sync_accepted": False,
...
def main():
    ...
    with (build / "m1-real-manager-runs.lock").open("a") as lock:
        ...
        if len(list(build.glob("m1-real-manager-run-*"))) >= 3:
            parser.error("three retained M1 network runs exist; inspect before starting another")
        root = Path(tempfile.mkdtemp(prefix="m1-real-manager-run-", dir=build))
    ...
    try:
        report.update(asyncio.run(exercise(root, build, args.base_port, args.join_timeout)))
        report["passed"] = True
