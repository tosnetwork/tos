#!/usr/bin/env python3
"""Synchronize the Rust Agent Account BOC with CMake's FunC output."""
...
RUST_PATTERN = re.compile(
    r'pub const AGENT_ACCOUNT_CODE_B64: &str = "[A-Za-z0-9+/=]+";'
)
...
def main() -> None:
    ...
    replacement = f'pub const AGENT_ACCOUNT_CODE_B64: &str = "{match.group("boc")}";'

    source = args.rust_source.read_text(encoding="utf-8")
    updated, count = RUST_PATTERN.subn(replacement, source)
    if count != 1:
        raise SystemExit(f"expected exactly one Rust Agent Account BOC constant, found {count}")
    if args.check:
        if updated != source:
            raise SystemExit("embedded Agent Account BOC is stale")
        return
