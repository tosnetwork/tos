...
void RldpIn::receive_message_part(adnl::AdnlNodeIdShort source, adnl::AdnlNodeIdShort local_id, td::BufferSlice data) {
  auto F = fetch_tl_object<tos_api::rldp_MessagePart>(std::move(data), true);
  if (F.is_error()) {
    VLOG(RLDP_INFO) << "failed to parse rldp packet [" << source << "->" << local_id << "]: " << F.move_as_error();
    return;
  }

  tos_api::downcast_call(*F.move_as_ok().get(), [&](auto &obj) { this->process_message_part(source, local_id, obj); });
}

void RldpIn::process_message_part(adnl::AdnlNodeIdShort source, adnl::AdnlNodeIdShort local_id,
                                  tos_api::rldp_messagePart &part) {
  ...
  if (it == receivers_.end()) {
    ...
    if (lru_set_.count(part.transfer_id_) == 1) {
      auto obj = create_tl_object<tos_api::rldp_complete>(part.transfer_id_, part.part_);
...
void RldpIn::add_id(adnl::AdnlNodeIdShort local_id) {
  ...
  std::vector<std::string> X{adnl::Adnl::int_to_bytestring(tos_api::rldp_messagePart::ID),
                             adnl::Adnl::int_to_bytestring(tos_api::rldp_confirm::ID),
                             adnl::Adnl::int_to_bytestring(tos_api::rldp_complete::ID)};
  for (auto &x : X) {
