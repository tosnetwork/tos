...
void TestNode::run() {
  ...
  if (!single_remote_public_key_.empty()) {  // Use single provided liteserver
    servers.push_back(
        liteclient::LiteServerConfig{tos::adnl::AdnlNodeIdFull{single_remote_public_key_}, single_remote_addr_});
    ...
    if (gc.validator_ && gc.validator_->zero_state_) {
      ...
      if (zstate_id_.workchain != tos::workchainInvalid) {
...
bool TestNode::envelope_send_query(td::BufferSlice query, td::Promise<td::BufferSlice> promise) {
  ...
  td::BufferSlice b =
      tos::serialize_tl_object(tos::create_tl_object<tos::lite_api::liteServer_query>(std::move(query)), true);
...
bool TestNode::register_blkid(const tos::BlockIdExt& blkid) {
...
bool TestNode::complete_blkid(tos::BlockId partial_blkid, tos::BlockIdExt& complete_blkid) const {
  ...
  if (partial_blkid.is_masterchain() && partial_blkid.seqno == ~0U) {
    complete_blkid.id = tos::BlockId{tos::masterchainId, tos::shardIdAll, ~0U};
...
bool TestNode::get_server_time() {
  auto b = tos::serialize_tl_object(tos::create_tl_object<tos::lite_api::liteServer_getTime>(), true);
  return envelope_send_query(std::move(b), [&, Self = actor_id(this)](td::Result<td::BufferSlice> res) -> void {
    if (res.is_error()) {
      ...
      auto F = tos::fetch_tl_object<tos::lite_api::liteServer_currentTime>(res.move_as_ok(), true);
      if (F.is_error()) {
...
bool TestNode::get_server_version(int mode) {
  auto b = tos::serialize_tl_object(tos::create_tl_object<tos::lite_api::liteServer_getVersion>(), true);
...
void TestNode::got_server_version(td::Result<td::BufferSlice> res, int mode) {
  ...
  if (res.is_error()) {
    ...
    auto F = tos::fetch_tl_object<tos::lite_api::liteServer_version>(res.move_as_ok(), true);
    if (F.is_error()) {
...
bool TestNode::get_server_mc_block_id() {
  ...
  if (mode < 0) {
    auto b = tos::serialize_tl_object(tos::create_tl_object<tos::lite_api::liteServer_getMasterchainInfo>(), true);
    return envelope_send_query(std::move(b), [Self = actor_id(this)](td::Result<td::BufferSlice> res) -> void {
      if (res.is_error()) {
        LOG(ERROR) << "cannot get masterchain info from server";
        return;
      } else {
        auto F = tos::fetch_tl_object<tos::lite_api::liteServer_masterchainInfo>(res.move_as_ok(), true);
        if (F.is_error()) {
    ...
    auto b =
        tos::serialize_tl_object(tos::create_tl_object<tos::lite_api::liteServer_getMasterchainInfoExt>(mode), true);
    return envelope_send_query(std::move(b), [Self = actor_id(this), mode](td::Result<td::BufferSlice> res) -> void {
      if (res.is_error()) {
        ...
        auto F = tos::fetch_tl_object<tos::lite_api::liteServer_masterchainInfoExt>(res.move_as_ok(), true);
        if (F.is_error()) {
...
void TestNode::got_server_mc_block_id(tos::BlockIdExt blkid, tos::ZeroStateIdExt zstateid, int created) {
  ...
  register_blkid(tos::BlockIdExt{tos::masterchainId, tos::shardIdAll, 0, zstateid.root_hash, zstateid.file_hash});
  ...
  if (created > 0) {
    auto time = now();
    if (time >= static_cast<tos::UnixTime>(created)) {
...
void TestNode::got_server_mc_block_id_ext(tos::BlockIdExt blkid, tos::ZeroStateIdExt zstateid, int mode, int version,
...
bool TestNode::request_block(tos::BlockIdExt blkid) {
  auto b = tos::serialize_tl_object(
      tos::create_tl_object<tos::lite_api::liteServer_getBlock>(tos::create_tl_lite_block_id(blkid)), true);
  return envelope_send_query(std::move(b), [Self = actor_id(this), blkid](td::Result<td::BufferSlice> res) -> void {
    if (res.is_error()) {
      ...
      auto F = tos::fetch_tl_object<tos::lite_api::liteServer_blockData>(res.move_as_ok(), true);
      if (F.is_error()) {
...
bool TestNode::request_state(tos::BlockIdExt blkid) {
  auto b = tos::serialize_tl_object(
      tos::create_tl_object<tos::lite_api::liteServer_getState>(tos::create_tl_lite_block_id(blkid)), true);
  return envelope_send_query(std::move(b), [Self = actor_id(this), blkid](td::Result<td::BufferSlice> res) -> void {
    if (res.is_error()) {
      LOG(ERROR) << "cannot obtain state " << blkid.to_str() << " from server";
      return;
    } else {
      auto F = tos::fetch_tl_object<tos::lite_api::liteServer_blockState>(res.move_as_ok(), true);
      if (F.is_error()) {
...
void TestNode::got_mc_block(tos::BlockIdExt blkid, td::BufferSlice data) {
...
void TestNode::got_mc_state(tos::BlockIdExt blkid, tos::RootHash root_hash, tos::FileHash file_hash,
                            td::BufferSlice data) {
...
td::Status TestNode::save_db_file(tos::FileHash file_hash, td::BufferSlice data) {
...
bool TestNode::parse_account_addr(tos::WorkchainId& wc, tos::StdSmcAddress& addr, bool allow_none) {
  auto word = get_word();
...
bool TestNode::parse_account_addr_ext(tos::WorkchainId& wc, tos::StdSmcAddress& addr, int& addr_ext, bool allow_none) {
  addr_ext = 0;
  auto word = get_word();
  if (allow_none && (word == "none" || word == "root")) {
    wc = tos::workchainInvalid;
...
bool TestNode::parse_lt(tos::LogicalTime& lt) {
...
bool TestNode::parse_hash(td::Slice str, tos::Bits256& hash) {
  return str.size() == 64 && parse_hash(str.data(), hash);
}

bool TestNode::parse_hash(const char* str, tos::Bits256& hash) {
...
bool TestNode::parse_block_id_ext(std::string blkid_str, tos::BlockIdExt& blkid, bool allow_incomplete) const {
  if (blkid_str.empty()) {
    return false;
  }
  auto fc = blkid_str[0];
...
bool TestNode::parse_block_id_ext(tos::BlockIdExt& blk, bool allow_incomplete) {
  return parse_block_id_ext(get_word().str(), blk, allow_incomplete) || set_error("cannot parse BlockIdExt");
}

bool TestNode::parse_hash(tos::Bits256& hash) {
  auto word = get_word();
  return parse_hash(word, hash) || set_error("cannot parse hash");
}

bool TestNode::convert_shard_id(td::Slice str, tos::ShardIdFull& shard) {
  shard.workchain = tos::workchainInvalid;
  shard.shard = 0;
  auto pos = str.find(':');
...
bool TestNode::parse_shard_id(tos::ShardIdFull& shard) {
...
bool TestNode::do_parse_line() {
  tos::WorkchainId workchain = tos::masterchainId;  // change to basechain later
  int addr_ext = 0;
  tos::StdSmcAddress addr = tos::StdSmcAddress::zero();
...
td::Status TestNode::send_ext_msg_from_filename(std::string filename) {
  ...
  if (ready_ && !client_.empty()) {
    ...
    auto P = td::PromiseCreator::lambda([](td::Result<td::BufferSlice> R) {
      ...
      auto F = tos::fetch_tl_object<tos::lite_api::liteServer_sendMsgStatus>(R.move_as_ok(), true);
      if (F.is_error()) {
    ...
    auto b =
        tos::serialize_tl_object(tos::create_tl_object<tos::lite_api::liteServer_sendMessage>(F.move_as_ok()), true);
...
bool TestNode::get_account_state(tos::WorkchainId workchain, tos::StdSmcAddress addr, tos::BlockIdExt ref_blkid,
  ...
  if (addr_ext) {
    return get_special_smc_addr(
        addr_ext, [this, ref_blkid, filename, mode, prunned](td::Result<tos::StdSmcAddress> res) {
          if (res.is_error()) {
            LOG(ERROR) << "cannot resolve special smart contract address: " << res.move_as_error();
          } else {
            get_account_state(tos::masterchainId, res.move_as_ok(), ref_blkid, 0, filename, mode, prunned);
          }
        });
  }
  auto a = tos::create_tl_object<tos::lite_api::liteServer_accountId>(workchain, addr);
  td::BufferSlice b;
  if (prunned) {
    b = tos::serialize_tl_object(tos::create_tl_object<tos::lite_api::liteServer_getAccountStatePrunned>(
                                     tos::create_tl_lite_block_id(ref_blkid), std::move(a)),
                                 true);
  } else {
    b = tos::serialize_tl_object(tos::create_tl_object<tos::lite_api::liteServer_getAccountState>(
                                     tos::create_tl_lite_block_id(ref_blkid), std::move(a)),
                                 true);
  }
  ...
  return envelope_send_query(std::move(b), [Self = actor_id(this), workchain, addr, ref_blkid, filename, mode,
    ...
    auto F = tos::fetch_tl_object<tos::lite_api::liteServer_accountState>(R.move_as_ok(), true);
    if (F.is_error()) {
      LOG(ERROR) << "cannot parse answer to liteServer.getAccountState";
    } else {
      auto f = F.move_as_ok();
      td::actor::send_closure_later(Self, &TestNode::got_account_state, ref_blkid, tos::create_block_id(f->id_),
                                    tos::create_block_id(f->shardblk_), std::move(f->shard_proof_),
...
bool TestNode::parse_run_method(tos::WorkchainId workchain, tos::StdSmcAddress addr, tos::BlockIdExt ref_blkid,
  ...
  if (addr_ext) {
    return get_special_smc_addr(addr_ext, [this, ref_blkid, method_name, ext_mode,
                                           args = R.move_as_ok()](td::Result<tos::StdSmcAddress> res) mutable {
      if (res.is_error()) {
        LOG(ERROR) << "cannot resolve special smart contract address: " << res.move_as_error();
      } else {
        after_parse_run_method(tos::masterchainId, res.move_as_ok(), ref_blkid, method_name, std::move(args), ext_mode);
      }
    });
  }
...
bool TestNode::after_parse_run_method(tos::WorkchainId workchain, tos::StdSmcAddress addr, tos::BlockIdExt ref_blkid,
...
bool TestNode::start_run_method(tos::WorkchainId workchain, tos::StdSmcAddress addr, tos::BlockIdExt ref_blkid,
  ...
  auto a = tos::create_tl_object<tos::lite_api::liteServer_accountId>(workchain, addr);
  if (!mode) {
    auto b = tos::serialize_tl_object(tos::create_tl_object<tos::lite_api::liteServer_getAccountState>(
                                          tos::create_tl_lite_block_id(ref_blkid), std::move(a)),
                                      true);
    ...
    return envelope_send_query(
        std::move(b), [Self = actor_id(this), workchain, addr, ref_blkid, method_name, params = std::move(params),
          ...
          if (F.is_error()) {
            ...
            auto f = F.move_as_ok();
            td::actor::send_closure_later(Self, &TestNode::run_smc_method, 0, ref_blkid, tos::create_block_id(f->id_),
                                          tos::create_block_id(f->shardblk_), std::move(f->shard_proof_),
    ...
    auto b = tos::serialize_tl_object(
        tos::create_tl_object<tos::lite_api::liteServer_runSmcMethod>(mode, tos::create_tl_lite_block_id(ref_blkid),
    ...
    return envelope_send_query(std::move(b), [Self = actor_id(this), workchain, addr, ref_blkid, method_name, mode,
      ...
      if (F.is_error()) {
        ...
        auto f = F.move_as_ok();
        td::actor::send_closure_later(Self, &TestNode::run_smc_method, mode, ref_blkid, tos::create_block_id(f->id_),
                                      tos::create_block_id(f->shardblk_), std::move(f->shard_proof_),
...
bool TestNode::get_config_addr(td::Promise<tos::StdSmcAddress> promise) {
...
bool TestNode::get_elector_addr(td::Promise<tos::StdSmcAddress> promise) {
...
bool TestNode::get_dns_root(td::Promise<tos::StdSmcAddress> promise) {
...
bool TestNode::get_special_smc_addr(int addr_ext, td::Promise<tos::StdSmcAddress> promise) {
  switch (addr_ext) {
    case 1:
...
bool TestNode::get_past_validator_sets() {
  return get_elector_addr([this](td::Result<tos::StdSmcAddress> res) {
    if (res.is_error()) {
...
bool TestNode::send_past_vset_query(tos::StdSmcAddress elector_addr) {
  ...
  return start_run_method(tos::masterchainId, elector_addr, mc_last_id_, "past_elections_list", std::move(params), 0x17,
                          std::move(P));
}
...
bool TestNode::get_complaints(unsigned elect_id, std::string file_pfx) {
  return get_elector_addr([this, elect_id, file_pfx](td::Result<tos::StdSmcAddress> res) {
    if (res.is_error()) {
      LOG(ERROR) << res.move_as_error();
    } else {
...
void TestNode::send_get_complaints_query(unsigned elect_id, tos::StdSmcAddress elector_addr, std::string file_pfx) {
  std::vector<vm::StackEntry> params;
  ...
  start_run_method(tos::masterchainId, elector_addr, mc_last_id_, "get_past_complaints", std::move(params), 0x17,
                   std::move(P));
}
...
td::Status TestNode::get_complaint_price(unsigned expires_in, unsigned bits, unsigned refs, td::Bits256 chash,
  ...
  return get_elector_addr([this, filename, expires_in, bits, refs, chash](td::Result<tos::StdSmcAddress> res) {
    if (res.is_error()) {
      LOG(ERROR) << res.move_as_error();
    } else {
...
void TestNode::send_compute_complaint_price_query(tos::StdSmcAddress elector_addr, unsigned expires_in, unsigned bits,
  ...
  start_run_method(tos::masterchainId, elector_addr, mc_last_id_, "complaint_storage_price", std::move(params), 0x17,
                   std::move(P));
}

bool TestNode::get_msg_queue_sizes() {
  tos::BlockIdExt blkid = mc_last_id_;
  ...
  auto b =
      tos::create_serialize_tl_object<tos::lite_api::liteServer_getAllShardsInfo>(tos::create_tl_lite_block_id(blkid));
  ...
  return envelope_send_query(std::move(b), [Self = actor_id(this), blkid](td::Result<td::BufferSlice> R) -> void {
    ...
    auto F = tos::fetch_tl_object<tos::lite_api::liteServer_allShardsInfo>(R.move_as_ok(), true);
    if (F.is_error()) {
...
void TestNode::get_msg_queue_sizes_cont(tos::BlockIdExt mc_blkid, td::BufferSlice data) {
  ...
  for (size_t i = 0; i < info->blocks.size(); ++i) {
    tos::BlockIdExt block_id = info->blocks[i];
    auto b = tos::create_serialize_tl_object<tos::lite_api::liteServer_getBlockOutMsgQueueSize>(
        0, tos::create_tl_lite_block_id(block_id), false);
    ...
    envelope_send_query(std::move(b), [=, this](td::Result<td::BufferSlice> R) -> void {
      ...
      auto F = tos::fetch_tl_object<tos::lite_api::liteServer_blockOutMsgQueueSize>(R.move_as_ok(), true);
      if (F.is_error()) {
...
void TestNode::get_msg_queue_sizes_finish(std::vector<tos::BlockIdExt> blocks, std::vector<td::uint64> sizes) {
...
bool TestNode::get_dispatch_queue_info(tos::BlockIdExt block_id) {
...
bool TestNode::get_dispatch_queue_info_cont(tos::BlockIdExt block_id, bool first, td::Bits256 after_addr) {
  auto q = tos::create_serialize_tl_object<tos::lite_api::liteServer_getDispatchQueueInfo>(
      first ? 0 : 2, tos::create_tl_lite_block_id(block_id), after_addr, 32, false);
  return envelope_send_query(std::move(q), [=, Self = actor_id(this)](td::Result<td::BufferSlice> res) -> void {
    ...
    auto F = tos::fetch_tl_object<tos::lite_api::liteServer_dispatchQueueInfo>(res.move_as_ok(), true);
    if (F.is_error()) {
...
void TestNode::got_dispatch_queue_info(tos::BlockIdExt block_id,
...
bool TestNode::get_dispatch_queue_messages(tos::BlockIdExt block_id, tos::WorkchainId wc, tos::StdSmcAddress addr,
                                           tos::LogicalTime lt, bool one_account) {
  if (wc != block_id.id.workchain) {
    return set_error("workchain mismatch");
  }
  auto q = tos::create_serialize_tl_object<tos::lite_api::liteServer_getDispatchQueueMessages>(
      one_account ? 2 : 0, tos::create_tl_lite_block_id(block_id), addr, lt, 64, false, one_account, false);
  return envelope_send_query(std::move(q), [=, Self = actor_id(this)](td::Result<td::BufferSlice> res) -> void {
    if (res.is_error()) {
      LOG(ERROR) << "liteServer.getDispatchQueueMessages error: " << res.move_as_error();
      return;
    }
    auto F = tos::fetch_tl_object<tos::lite_api::liteServer_dispatchQueueMessages>(res.move_as_ok(), true);
    if (F.is_error()) {
...
void TestNode::got_dispatch_queue_messages(tos::tl_object_ptr<tos::lite_api::liteServer_dispatchQueueMessages> msgs) {
  ...
  for (auto& m : msgs->messages_) {
    ...
    td::TerminalIO::out() << "Msg #" << ++count << ": " << msgs->id_->workchain_ << ":" << m->addr_.to_hex() << " "
                          << m->lt_ << " : "
                          << (meta->initiator_->workchain_ == tos::workchainInvalid
                                  ? "[ no metadata ]"
                                  : block::MsgMetadata{(td::uint32)meta->depth_, meta->initiator_->workchain_,
                                                       meta->initiator_->id_, (tos::LogicalTime)meta->initiator_lt_}
                                        .to_str())
                          << "\n";
  }
  if (!msgs->complete_) {
...
bool TestNode::dns_resolve_start(tos::WorkchainId workchain, tos::StdSmcAddress addr, tos::BlockIdExt blkid,
  ...
  if (workchain == tos::workchainInvalid) {
    if (dns_root_queried_) {
      workchain = tos::masterchainId;
      addr = dns_root_;
    } else {
      auto P =
          td::PromiseCreator::lambda([this, blkid, domain, cat, mode](td::Result<std::unique_ptr<block::Config>> R) {
            if (R.is_error()) {
              ...
              dns_resolve_start(tos::masterchainId, dns_root_, blkid, domain, cat, mode);
            } else {
...
void TestNode::dns_resolve_finish(tos::WorkchainId workchain, tos::StdSmcAddress addr, tos::BlockIdExt blkid,
  ...
  if (!end) {
    ...
    tos::WorkchainId nx_wc;
    tos::StdSmcAddress nx_addr;
...
bool TestNode::get_one_transaction(tos::BlockIdExt blkid, tos::WorkchainId workchain, tos::StdSmcAddress addr,
                                   tos::LogicalTime lt, bool dump) {
  if (!blkid.is_valid_full()) {
    return set_error("invalid block id");
  }
  if (!tos::shard_contains(blkid.shard_full(), tos::extract_addr_prefix(workchain, addr))) {
  ...
  auto a = tos::create_tl_object<tos::lite_api::liteServer_accountId>(workchain, addr);
  auto b = tos::serialize_tl_object(tos::create_tl_object<tos::lite_api::liteServer_getOneTransaction>(
                                        tos::create_tl_lite_block_id(blkid), std::move(a), lt),
                                    true);
  ...
  return envelope_send_query(
      std::move(b), [Self = actor_id(this), workchain, addr, lt, blkid, dump](td::Result<td::BufferSlice> R) -> void {
        ...
        auto F = tos::fetch_tl_object<tos::lite_api::liteServer_transactionInfo>(R.move_as_ok(), true);
        if (F.is_error()) {
          LOG(ERROR) << "cannot parse answer to liteServer.getOneTransaction";
        } else {
          auto f = F.move_as_ok();
          td::actor::send_closure_later(Self, &TestNode::got_one_transaction, blkid, tos::create_block_id(f->id_),
                                        std::move(f->proof_), std::move(f->transaction_), workchain, addr, lt, dump);
        }
      });
}

bool TestNode::get_last_transactions(tos::WorkchainId workchain, tos::StdSmcAddress addr, tos::LogicalTime lt,
  ...
  auto a = tos::create_tl_object<tos::lite_api::liteServer_accountId>(workchain, addr);
  auto b = tos::serialize_tl_object(
      tos::create_tl_object<tos::lite_api::liteServer_getTransactions>(count, std::move(a), lt, hash), true);
  ...
  return envelope_send_query(
      std::move(b), [Self = actor_id(this), workchain, addr, lt, hash, count, dump](td::Result<td::BufferSlice> R) {
        ...
        auto F = tos::fetch_tl_object<tos::lite_api::liteServer_transactionList>(R.move_as_ok(), true);
        if (F.is_error()) {
...
void TestNode::got_account_state(tos::BlockIdExt ref_blk, tos::BlockIdExt blk, tos::BlockIdExt shard_blk,
...
void TestNode::run_smc_method(int mode, tos::BlockIdExt ref_blk, tos::BlockIdExt blk, tos::BlockIdExt shard_blk,
                              td::BufferSlice shard_proof, td::BufferSlice proof, td::BufferSlice state,
  ...
  try {
    ...
    vm::VmState vm{code, tos::SUPPORTED_VERSION, std::move(stack), gas, 1, data, vm::VmLog()};
    ...
    if (!(mode & 4)) {
      ...
      if (remote_exit_code != 0) {
        ...
        bool special = false;
        auto cs = vm::load_cell_slice_special(res.move_as_ok(), special);
        if (special) {
          promise.set_error(td::Status::Error("remote VM result boc has a special root"));
          return;
        }
...
void TestNode::got_one_transaction(tos::BlockIdExt req_blkid, tos::BlockIdExt blkid, td::BufferSlice proof,
...
void TestNode::got_last_transactions(std::vector<tos::BlockIdExt> blkids, td::BufferSlice transactions_boc,
...
bool TestNode::get_block_transactions(tos::BlockIdExt blkid, int mode, unsigned count, tos::Bits256 acc_addr,
                                      tos::LogicalTime lt) {
  if (!(ready_ && !client_.empty())) {
    return set_error("server connection not ready");
  }
  auto a = tos::create_tl_object<tos::lite_api::liteServer_transactionId3>(acc_addr, lt);
  auto b = tos::serialize_tl_object(tos::create_tl_object<tos::lite_api::liteServer_listBlockTransactions>(
                                        tos::create_tl_lite_block_id(blkid), mode, count, std::move(a), false, false),
                                    true);
  ...
  return envelope_send_query(std::move(b), [Self = actor_id(this), mode](td::Result<td::BufferSlice> R) {
    ...
    auto F = tos::fetch_tl_object<tos::lite_api::liteServer_blockTransactions>(R.move_as_ok(), true);
    if (F.is_error()) {
      ...
      td::actor::send_closure_later(Self, &TestNode::got_block_transactions, tos::create_block_id(f->id_), mode,
...
void TestNode::got_block_transactions(
    tos::BlockIdExt blkid, int mode, unsigned req_count, bool incomplete, std::vector<TestNode::TransId> trans,
    std::vector<tos::tl_object_ptr<tos::lite_api::liteServer_transactionMetadata>> metadata, td::BufferSlice proof) {
      ...
      if (meta == nullptr) {
        ...
        out << "    metadata: "
            << block::MsgMetadata{(td::uint32)meta->depth_, meta->initiator_->workchain_, meta->initiator_->id_,
                                  (tos::LogicalTime)meta->initiator_lt_}
                   .to_str()
            << std::endl;
      }
    }
...
bool TestNode::get_all_shards(std::string filename, bool use_last, tos::BlockIdExt blkid) {
  if (use_last) {
    blkid = mc_last_id_;
  }
  if (!blkid.is_valid_full()) {
  ...
  auto b = tos::serialize_tl_object(
      tos::create_tl_object<tos::lite_api::liteServer_getAllShardsInfo>(tos::create_tl_lite_block_id(blkid)), true);
  LOG(INFO) << "requesting recent shard configuration";
  return envelope_send_query(std::move(b), [Self = actor_id(this), filename](td::Result<td::BufferSlice> R) -> void {
    ...
    if (F.is_error()) {
      ...
      auto f = F.move_as_ok();
      td::actor::send_closure_later(Self, &TestNode::got_all_shards, tos::create_block_id(f->id_), std::move(f->proof_),
                                    std::move(f->data_), filename);
    }
  });
}

void TestNode::got_all_shards(tos::BlockIdExt blk, td::BufferSlice proof, td::BufferSlice data, std::string filename) {
...
bool TestNode::parse_get_config_params(tos::BlockIdExt blkid, int mode, std::string filename, std::vector<int> params) {
  if (mode < 0) {
    mode = 0x80000;
  }
...
bool TestNode::get_config_params(tos::BlockIdExt blkid, td::Promise<std::unique_ptr<block::Config>> promise, int mode,
...
bool TestNode::get_config_params_ext(tos::BlockIdExt blkid, td::Promise<ConfigInfo> promise, int mode,
                                     std::string filename, std::vector<int> params) {
  if (!(ready_ && !client_.empty())) {
  ...
  auto b = (mode & 0x1000) ? tos::serialize_tl_object(
                                 tos::create_tl_object<tos::lite_api::liteServer_getConfigParams>(
                                     mode & 0x8fff, tos::create_tl_lite_block_id(blkid), std::move(params_copy)),
                                 true)
                           : tos::serialize_tl_object(tos::create_tl_object<tos::lite_api::liteServer_getConfigAll>(
                                                          mode & 0x8fff, tos::create_tl_lite_block_id(blkid)),
                                                      true);
...
void TestNode::got_config_params(tos::BlockIdExt req_blkid, int mode, std::string filename, std::vector<int> params,
  ...
  TRY_RESULT_PROMISE_PREFIX(promise, f,
                            tos::fetch_tl_object<tos::lite_api::liteServer_configInfo>(std::move(res), true),
                            "cannot parse answer to liteServer.getConfigParams");
  auto blkid = tos::create_block_id(f->id_);
...
bool TestNode::get_block(tos::BlockIdExt blkid, bool dump) {
...
bool TestNode::get_state(tos::BlockIdExt blkid, bool dump) {
...
void TestNode::got_block(tos::BlockIdExt blkid, td::BufferSlice data, bool dump) {
  LOG(INFO) << "obtained " << data.size() << " data bytes for block " << blkid.to_str();
  tos::FileHash fhash;
  td::sha256(data.as_slice(), fhash.as_slice());
  if (fhash != blkid.file_hash) {
  ...
  if (dump) {
    ...
    tos::RootHash rhash{root->get_hash().bits()};
...
void TestNode::got_state(tos::BlockIdExt blkid, tos::RootHash root_hash, tos::FileHash file_hash, td::BufferSlice data,
                         bool dump) {
  LOG(INFO) << "obtained " << data.size() << " state bytes for block " << blkid.to_str();
  tos::FileHash fhash;
  td::sha256(data.as_slice(), fhash.as_slice());
...
bool TestNode::get_show_block_header(tos::BlockIdExt blkid, int mode) {
...
bool TestNode::get_block_header(tos::BlockIdExt blkid, int mode, td::Promise<TestNode::BlockHdrInfo> promise) {
  LOG(INFO) << "got block header request for " << blkid.to_str() << " with mode " << mode;
  auto b = tos::serialize_tl_object(
      tos::create_tl_object<tos::lite_api::liteServer_getBlockHeader>(tos::create_tl_lite_block_id(blkid), mode), true);
  return envelope_send_query(
...
bool TestNode::lookup_show_block(tos::ShardIdFull shard, int mode, td::uint64 arg) {
...
bool TestNode::lookup_block(tos::ShardIdFull shard, int mode, td::uint64 arg,
                            td::Promise<TestNode::BlockHdrInfo> promise) {
  tos::BlockId id{shard, mode & 1 ? (td::uint32)arg : 0};
  LOG(INFO) << "got block lookup request for " << id.to_str() << " with mode " << mode << " and argument " << arg;
  auto b = tos::serialize_tl_object(tos::create_tl_object<tos::lite_api::liteServer_lookupBlock>(
                                        mode, tos::create_tl_lite_block_id_simple(id), arg, (td::uint32)arg),
                                    true);
  return envelope_send_query(
...
void TestNode::got_block_header_raw(td::BufferSlice res, td::Promise<TestNode::BlockHdrInfo> promise,
                                    tos::BlockIdExt req_blkid) {
  TRY_RESULT_PROMISE_PREFIX(promise, f,
                            tos::fetch_tl_object<tos::lite_api::liteServer_blockHeader>(std::move(res), true),
...
bool TestNode::show_block_header(tos::BlockIdExt blkid, Ref<vm::Cell> root, int mode) {
  tos::RootHash vhash{root->get_hash().bits()};
...
bool TestNode::show_state_header(tos::BlockIdExt blkid, Ref<vm::Cell> root, int mode) {
  return true;
}

void TestNode::got_block_header(tos::BlockIdExt blkid, td::BufferSlice data, int mode) {
...
void TestNode::got_block_proof(tos::BlockIdExt from, tos::BlockIdExt to, int mode, td::BufferSlice pchain) {
  LOG(INFO) << "got block proof from " << from.to_str() << " to "
            ...
            << " bytes)";
  auto r_f = tos::fetch_tl_object<tos::lite_api::liteServer_partialBlockProof>(std::move(pchain), true);
  if (r_f.is_error()) {
...
bool TestNode::get_creator_stats(tos::BlockIdExt blkid, int mode, unsigned req_count, tos::Bits256 start_after,
...
bool TestNode::get_creator_stats(tos::BlockIdExt blkid, int mode, unsigned req_count, tos::Bits256 start_after,
                                 tos::UnixTime min_utime, TestNode::creator_stats_func_t func,
...
bool TestNode::get_creator_stats(tos::BlockIdExt blkid, unsigned req_count, tos::UnixTime min_utime,
  ...
  auto b = tos::serialize_tl_object(
      tos::create_tl_object<tos::lite_api::liteServer_getValidatorStats>(
          state->mode & 0xff, tos::create_tl_lite_block_id(blkid), req_count, state->last_key, min_utime),
      true);
  ...
  return envelope_send_query(
      std::move(b), [this, blkid, req_count, state = std::move(state), min_utime, func = std::move(func),
        ...
        TRY_RESULT_PROMISE(promise, res, std::move(R));
        TRY_RESULT_PROMISE_PREFIX(promise, f,
                                  tos::fetch_tl_object<tos::lite_api::liteServer_validatorStats>(std::move(res), true),
                                  "cannot parse answer to liteServer.getValidatorStats");
        got_creator_stats(blkid, tos::create_block_id(f->id_), f->mode_, min_utime, std::move(f->state_proof_),
                          std::move(f->data_proof_), f->count_, req_count, f->complete_, std::move(func),
                          std::move(state), std::move(promise));
      });
}

void TestNode::got_creator_stats(tos::BlockIdExt req_blkid, tos::BlockIdExt blkid, int mode, tos::UnixTime min_utime,
  ...
  tos::Bits256 key{status->last_key};
  std::ostringstream os;
  try {
...
bool TestNode::check_validator_load(int start_time, int end_time, int mode, std::string file_pfx) {
  ...
  lookup_block(tos::ShardIdFull(tos::masterchainId), 4, start_time, std::move(P.first));
  return lookup_block(tos::ShardIdFull(tos::masterchainId), 4, end_time, std::move(P.second));
}

void TestNode::continue_check_validator_load(tos::BlockIdExt blkid1, Ref<vm::Cell> root1, tos::BlockIdExt blkid2,
...
bool TestNode::load_creator_stats(std::unique_ptr<TestNode::ValidatorLoadInfo> load_to,
  ...
  tos::UnixTime min_utime = info.valid_since - 1000;
  return get_creator_stats(
...
void TestNode::continue_check_validator_load3(std::unique_ptr<TestNode::ValidatorLoadInfo> info1,
  ...
  if (mode & 4) {
    tos::BlockSeqno start_seqno = info1->blk_id.seqno();
    tos::BlockSeqno end_seqno = info2->blk_id.seqno();
...
void TestNode::load_validator_shard_shares(tos::BlockSeqno start_seqno, tos::BlockSeqno end_seqno,
...
void TestNode::load_validator_shard_shares_cont(std::shared_ptr<LoadValidatorShardSharesState> state) {
  ...
  try {
    for (size_t idx = 0; idx + 1 < state->shard_configs.size(); ++idx) {
      ...
      auto process_shard = [&](tos::ShardIdFull shard, tos::BlockSeqno first_seqno) {
        ...
        tos::CatchainSeqno cc_seqno = shards1.get_shard_cc_seqno(shard);
        auto val_set =
      ...
      for (const tos::BlockId& id : shards1.get_shard_hash_ids()) {
        tos::ShardIdFull shard = id.shard_full();
        auto desc = shards1.get_shard_hash(shard);
        CHECK(desc.not_null());
        if (desc->before_split()) {
          tos::ShardIdFull l_shard = shard_child(shard, true);
          tos::ShardIdFull r_shard = shard_child(shard, false);
          ...
          tos::ShardIdFull sibling_shard = shard_sibling(shard);
          auto sibling_desc = shards1.get_shard_hash(sibling_shard);
          CHECK(sibling_desc.not_null());
          tos::ShardIdFull p_shard = shard_parent(shard);
...
void TestNode::load_block_shard_configuration(tos::BlockSeqno seqno, td::Promise<block::ShardConfig> promise) {
  lookup_block(
      tos::ShardIdFull{tos::masterchainId}, 1, seqno,
      [this, promise = std::move(promise)](td::Result<BlockHdrInfo> R) mutable {
        TRY_RESULT_PROMISE(promise, res, std::move(R));
        auto b = tos::serialize_tl_object(
            tos::create_tl_object<tos::lite_api::liteServer_getAllShardsInfo>(tos::create_tl_lite_block_id(res.blk_id)),
            true);
        envelope_send_query(std::move(b), [promise = std::move(promise)](td::Result<td::BufferSlice> R) mutable {
          ...
          TRY_RESULT_PROMISE(promise, f, tos::fetch_tl_object<tos::lite_api::liteServer_allShardsInfo>(data, true));
...
td::Status TestNode::ValidatorLoadInfo::check_header_proof(tos::UnixTime* save_utime, tos::LogicalTime* save_lt) const {
...
int main(int argc, char* argv[]) {
  ...
  td::OptionParser p;
  p.set_description("Test Lite Client for TOS Blockchain");
