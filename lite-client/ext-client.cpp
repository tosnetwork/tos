 ...
 private:
  void send_query_internal(std::string name, td::BufferSlice data, QueryInfo query_info, size_t server_idx,
    ...
    td::Promise<td::BufferSlice> P = [SelfId = actor_id(this), server_idx,
      ...
      if (R.is_error() &&
          (R.error().code() == tos::ErrorCode::timeout || R.error().code() == tos::ErrorCode::cancelled)) {
    ...
    send_closure(server.client, &tos::adnl::AdnlExtClient::send_query, std::move(name), std::move(data), timeout,
                 std::move(P));
  }
  ...
  void prepare_server(size_t server_idx, const QueryInfo* query_info) {
    ...
    server.client = tos::adnl::AdnlExtClient::create(server.config.adnl_id, server.config.hostname,
  ...
  struct Server {
    ...
    size_t idx = 0;
    td::actor::ActorOwn<tos::adnl::AdnlExtClient> client;
    bool alive = false;
...
td::actor::ActorOwn<ExtClient> ExtClient::create(tos::adnl::AdnlNodeIdFull dst, td::IPAddress dst_addr,
