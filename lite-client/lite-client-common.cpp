...
td::Result<std::unique_ptr<block::BlockProofChain>> deserialize_proof_chain(
  ...
  auto chain = std::make_unique<block::BlockProofChain>(tos::create_block_id(f->from_), tos::create_block_id(f->to_));
  ...
  for (auto& s : f->steps_) {
    ...
    tos::lite_api::downcast_call(*s, td::overloaded(
                                         [&](tos::lite_api::liteServer_blockLinkBack& s) {
                                           auto& link = chain->new_link(tos::create_block_id(s.from_),
                                                                        tos::create_block_id(s.to_), s.to_key_block_);
                                           link.is_fwd = false;
                                         ...
                                         [&](tos::lite_api::liteServer_blockLinkForward& s) {
                                           auto& link = chain->new_link(tos::create_block_id(s.from_),
                                                                        tos::create_block_id(s.to_), s.to_key_block_);
                                           link.is_fwd = true;
