...
td::Result<std::vector<LiteServerConfig>> LiteServerConfig::parse_global_config(
    const tos_api::liteclient_config_global& config) {
  ...
  for (const auto& f : config.liteservers_v2_) {
    ...
    for (const auto& slice_obj : f->slices_) {
      ...
      downcast_call(*slice_obj,
                    td::overloaded(
                        [&](const tos_api::liteserver_descV2_sliceSimple& s) {
                        ...
                        [&](const tos_api::liteserver_descV2_sliceTimed& s) {
                          auto parse_shards =
                              [](const std::vector<tl_object_ptr<tos_api::liteserver_descV2_shardInfo>>& shard_objs,
