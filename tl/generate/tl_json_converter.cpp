...
void gen_to_json(StringBuilder &sb, const tl::simple::Schema &schema, bool is_header) {
  ...
  if (is_header) {
    sb << "inline void to_json(JsonValueScope &jv, const tos::" << tl_name
       << "::Object &object) {\n"
          "  tos::"
       << tl_name << "::downcast_call(const_cast<tos::" << tl_name
       << "::Object &>(object),[&jv](const auto &object) { "
          "to_json(jv, object); });\n"
       << "}\n";
    sb << "inline void to_json(JsonValueScope &jv, const tos::" << tl_name << "::Function &object) {\n"
       << "  tos::" << tl_name << "::downcast_call(const_cast<tos::" << tl_name
