...
template <class T>
void to_json(JsonValueScope &jv, const tos::tl_object_ptr<T> &value) {
  if (value) {
    to_json(jv, *value);
  } else {
    jv << JsonNull();
...
template <class T>
std::enable_if_t<!std::is_constructible<T>::value, Status> from_json(tos::tl_object_ptr<T> &to, JsonValue from) {
