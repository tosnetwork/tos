...
td::Result<ResolvedUnoRpcWorkchain> resolve_uno_rpc_workchain_from_masterchain_state(
  ...
  for (const auto& [workchain_id, info] : config.get_workchain_list()) {
    ...
    if (!resolved.descriptor.accept_msgs) {
      return td::Status::Error("active Uno workchain does not accept external messages");
    }
...
void JsonRpcServer::handle_uno_sendTransfer(td::JsonValue& params_val,
  ...
  // Hard size cap before hex decode. Audit #9 (2026-04-26): raised from
  // 1 MiB to 3 MiB to match the documented post-V1-3c-gamma envelope:
  // ~655 KB typical / ~1.15 MB worst-case 4/4 binary, ~2.30 MB hex+JSON
