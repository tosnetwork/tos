...
td::BufferSlice create_query_error(td::CSlice message) {
  return create_serialize_tl_object<tos_api::storage_daemon_queryError>(message.str());
}
...
class StorageDaemon : public td::actor::Actor {
 public:
  ...
  td::Status load_global_config() {
    ...
    tos_api::config_global conf;
    TRY_STATUS_PREFIX(tos_api::from_json(conf, conf_json.get_object()), "json does not fit TL scheme: ");
    if (!conf.dht_) {
  ...
  td::Status load_daemon_config() {
    daemon_config_ = create_tl_object<tos_api::storage_daemon_config>();
    auto r_conf_data = td::read_file(daemon_config_file());
    if (r_conf_data.is_ok()) {
      auto conf_data = r_conf_data.move_as_ok();
      TRY_RESULT_PREFIX(conf_json, td::json_decode(conf_data.as_slice()), "failed to parse json: ");
      TRY_STATUS_PREFIX(tos_api::from_json(*daemon_config_, conf_json.get_object()), "json does not fit TL scheme: ");
      return td::Status::OK();
    }
    ...
    if (enable_storage_provider_) {
      if (!daemon_config_->provider_address_.empty()) {
        ...
        provider_ = td::actor::create_actor<StorageProvider>("provider", provider_account, db_root_ + "/provider",
                                                             toslib_client_.get(), manager_.get(), keyring_.get());
      } else {
  ...
  void init_control_interface() {
    ...
    class Callback : public adnl::Adnl::Callback {
     public:
      ...
      void receive_message(adnl::AdnlNodeIdShort src, tos::adnl::AdnlNodeIdShort dst, td::BufferSlice data) override {
      }
  ...
  void process_control_query(adnl::AdnlNodeIdShort src, td::BufferSlice data, td::Promise<td::BufferSlice> promise) {
    ...
    auto F = fetch_tl_object<tos_api::Function>(data, true);
    if (F.is_error()) {
    ...
    tos_api::downcast_call(*f, [&](auto &obj) { run_control_query(obj, std::move(promise)); });
  }

  void run_control_query(tos_api::storage_daemon_setVerbosity &query, td::Promise<td::BufferSlice> promise) {
    ...
    promise.set_result(create_serialize_tl_object<tos_api::storage_daemon_success>());
  }

  void run_control_query(tos_api::storage_daemon_createTorrent &query, td::Promise<td::BufferSlice> promise) {
  ...
  void run_control_query(tos_api::storage_daemon_addByHash &query, td::Promise<td::BufferSlice> promise) {
  ...
  void run_control_query(tos_api::storage_daemon_addByMeta &query, td::Promise<td::BufferSlice> promise) {
  ...
  td::Promise<td::Unit> query_add_torrent_cont(td::Bits256 hash, bool start_download,
                                               std::vector<tl_object_ptr<tos_api::storage_PriorityAction>> priorities,
  ...
  void run_control_query(tos_api::storage_daemon_setActiveDownload &query, td::Promise<td::BufferSlice> promise) {
    td::actor::send_closure(
        manager_, &StorageManager::set_active_download, query.hash_, query.active_,
        promise.wrap([](td::Unit &&) { return create_serialize_tl_object<tos_api::storage_daemon_success>(); }));
  }

  void run_control_query(tos_api::storage_daemon_setActiveUpload &query, td::Promise<td::BufferSlice> promise) {
    td::actor::send_closure(
        manager_, &StorageManager::set_active_upload, query.hash_, query.active_,
        promise.wrap([](td::Unit &&) { return create_serialize_tl_object<tos_api::storage_daemon_success>(); }));
  }

  void run_control_query(tos_api::storage_daemon_getTorrents &query, td::Promise<td::BufferSlice> promise) {
    td::actor::send_closure(
        ...
        [manager = manager_.get(), promise = std::move(promise)](td::Result<std::vector<td::Bits256>> R) mutable {
          ...
          auto result = std::make_shared<std::vector<tl_object_ptr<tos_api::storage_daemon_torrent>>>(torrents.size());
          td::MultiPromise mp;
          auto ig = mp.init_guard();
          for (size_t i = 0; i < torrents.size(); ++i) {
            get_torrent_info_short(manager, torrents[i],
                                   [i, result, promise = ig.get_promise()](
                                       td::Result<tl_object_ptr<tos_api::storage_daemon_torrent>> R) mutable {
                                     if (R.is_ok()) {
          ...
          ig.add_promise([promise = std::move(promise), result](td::Result<td::Unit> R) mutable {
            ...
            promise.set_result(create_serialize_tl_object<tos_api::storage_daemon_torrentList>(std::move(v)));
          });
        });
  }

  void run_control_query(tos_api::storage_daemon_getTorrentFull &query, td::Promise<td::BufferSlice> promise) {
    get_torrent_info_full_serialized(manager_.get(), query.hash_, std::move(promise));
  }

  void run_control_query(tos_api::storage_daemon_getTorrentMeta &query, td::Promise<td::BufferSlice> promise) {
    td::actor::send_closure(
        ...
        promise.wrap([](NodeActor::NodeState state) -> td::Result<td::BufferSlice> {
          ...
          return create_serialize_tl_object<tos_api::storage_daemon_torrentMeta>(td::BufferSlice(meta_str));
        }));
  }

  void run_control_query(tos_api::storage_daemon_getTorrentPeers &query, td::Promise<td::BufferSlice> promise) {
    td::actor::send_closure(manager_, &StorageManager::get_peers_info, query.hash_,
                            promise.wrap([](tl_object_ptr<tos_api::storage_daemon_peerList> obj) -> td::BufferSlice {
                              return serialize_tl_object(obj, true);
                            }));
  }

  void run_control_query(tos_api::storage_daemon_getTorrentPiecesInfo &query, td::Promise<td::BufferSlice> promise) {
    td::Bits256 hash = query.hash_;
    td::actor::send_closure(
        ...
        promise.wrap([query = std::move(query)](NodeActor::NodeState state) -> td::Result<td::BufferSlice> {
          ...
          auto result = create_tl_object<tos_api::storage_daemon_torrentPiecesInfo>();
          ...
          if ((query.flags_ & 1) && torrent.inited_header()) {
            ...
            result->files_.push_back(
                create_tl_object<tos_api::storage_daemon_filePiecesInfo>("", range.begin, range.end));
            for (size_t i = 0; i < torrent.get_files_count().value(); ++i) {
              auto range = torrent.get_file_parts_range(i);
              result->files_.push_back(create_tl_object<tos_api::storage_daemon_filePiecesInfo>(
  ...
  void run_control_query(tos_api::storage_daemon_setFilePriorityAll &query, td::Promise<td::BufferSlice> promise) {
    ...
    td::actor::send_closure(manager_, &StorageManager::set_all_files_priority, query.hash_, priority,
                            promise.wrap([](bool done) -> td::Result<td::BufferSlice> {
                              if (done) {
                                return create_serialize_tl_object<tos_api::storage_daemon_prioritySet>();
                              } else {
                                return create_serialize_tl_object<tos_api::storage_daemon_priorityPending>();
                              }
                            }));
  }

  void run_control_query(tos_api::storage_daemon_setFilePriorityByIdx &query, td::Promise<td::BufferSlice> promise) {
    TRY_RESULT_PROMISE(promise, priority, td::narrow_cast_safe<td::uint8>(query.priority_));
  ...
  void run_control_query(tos_api::storage_daemon_setFilePriorityByName &query, td::Promise<td::BufferSlice> promise) {
    TRY_RESULT_PROMISE(promise, priority, td::narrow_cast_safe<td::uint8>(query.priority_));
  ...
  void run_control_query(tos_api::storage_daemon_removeTorrent &query, td::Promise<td::BufferSlice> promise) {
    td::actor::send_closure(
        manager_, &StorageManager::remove_torrent, query.hash_, query.remove_files_,
        promise.wrap([](td::Unit &&) { return create_serialize_tl_object<tos_api::storage_daemon_success>(); }));
  }

  void run_control_query(tos_api::storage_daemon_loadFrom &query, td::Promise<td::BufferSlice> promise) {
    ...
    td::actor::send_closure(
        ...
        [manager = manager_.get(), hash = query.hash_, promise = std::move(promise)](td::Result<td::Unit> R) mutable {
          if (R.is_error()) {
            ...
            get_torrent_info_short(manager, hash, promise.wrap([](tl_object_ptr<tos_api::storage_daemon_torrent> obj) {
              return serialize_tl_object(obj, true);
            }));
          }
        });
  ...
  void run_control_query(tos_api::storage_daemon_getSpeedLimits &query, td::Promise<td::BufferSlice> promise) {
    td::actor::send_closure(manager_, &StorageManager::get_speed_limits,
                            promise.wrap([](std::pair<double, double> limits) -> td::BufferSlice {
                              return create_serialize_tl_object<tos_api::storage_daemon_speedLimits>(limits.first,
                                                                                                     limits.second);
                            }));
  }

  void run_control_query(tos_api::storage_daemon_setSpeedLimits &query, td::Promise<td::BufferSlice> promise) {
    if (query.flags_ & 1) {
  ...
  void run_control_query(tos_api::storage_daemon_getNewContractMessage &query, td::Promise<td::BufferSlice> promise) {
         ...
         manager = manager_.get()](td::Result<std::pair<td::RefInt256, td::uint32>> R) mutable {
          ...
          td::actor::send_closure(
              ...
              promise.wrap([r = std::move(r), query_id](NodeActor::NodeState state) -> td::Result<td::BufferSlice> {
                ...
                return create_serialize_tl_object<tos_api::storage_daemon_newContractMessage>(
                    vm::std_boc_serialize(msg).move_as_ok(), r.first->to_dec_string(), r.second);
              }));
        };

    tos_api::downcast_call(*query.params_,
                           td::overloaded(
                               [&](tos_api::storage_daemon_newContractParams &obj) {
                               ...
                               [&](tos_api::storage_daemon_newContractParamsAuto &obj) {
                                 TRY_RESULT_PROMISE(P, address, ContractAddress::parse(obj.provider_address_));
                                 init_toslib_client();
                                 StorageProvider::get_provider_params(
                                     toslib_client_.get(), address, P.wrap([](ProviderParams params) {
                                       return std::make_pair(std::move(params.rate_per_mb_day), params.max_span);
                                     }));
                               }));
  }

  void run_control_query(tos_api::storage_daemon_importPrivateKey &query, td::Promise<td::BufferSlice> promise) {
    ...
    td::actor::send_closure(keyring_, &tos::keyring::Keyring::add_key, std::move(pk), false,
                            promise.wrap([hash = pk.compute_short_id()](td::Unit) mutable {
                              return create_serialize_tl_object<tos_api::storage_daemon_keyHash>(hash.bits256_value());
                            }));
  }

  void run_control_query(tos_api::storage_daemon_deployProvider &query, td::Promise<td::BufferSlice> promise) {
    if (!enable_storage_provider_) {
      promise.set_error(
    ...
    promise.set_result(create_serialize_tl_object<tos_api::storage_daemon_providerAddress>(
        deploying_provider_.value().address.to_string()));
    do_deploy_provider();
  }

  void run_control_query(tos_api::storage_daemon_initProvider &query, td::Promise<td::BufferSlice> promise) {
    if (!enable_storage_provider_) {
      promise.set_error(
          td::Status::Error("Storage provider is not enabled, run daemon with --storage-provider to enable it"));
      return;
    ...
    do_init_provider(
        address, promise.wrap([](td::Unit) { return create_serialize_tl_object<tos_api::storage_daemon_success>(); }));
  }
  ...
  void do_deploy_provider() {
    ...
    check_contract_exists(
        deploying_provider_.value().address, toslib_client_.get(),
        [SelfId = actor_id(this), client = toslib_client_.get(),
         init = deploying_provider_.value()](td::Result<bool> R) mutable {
          ...
          auto query = create_tl_object<toslib_api::raw_createAndSendMessage>(
              create_tl_object<toslib_api::accountAddress>(address.to_string()), state_init_boc.as_slice().str(),
              body_boc.as_slice().str());
          td::actor::send_closure(
              client, &toslib::ToslibClientWrapper::send_request<toslib_api::raw_createAndSendMessage>,
              std::move(query), [=](td::Result<tl_object_ptr<toslib_api::ok>> R) {
                if (R.is_error()) {
                  LOG(INFO) << "Deploying storage contract: " << R.move_as_error();
                }
                delay_action([=]() { td::actor::send_closure(SelfId, &StorageDaemon::do_deploy_provider); },
  ...
  void run_control_query(tos_api::storage_daemon_removeStorageProvider &query, td::Promise<td::BufferSlice> promise) {
    if (!enable_storage_provider_) {
  ...
  void run_control_query(tos_api::storage_daemon_getProviderParams &query, td::Promise<td::BufferSlice> promise) {
  ...
  void run_control_query(tos_api::storage_daemon_setProviderParams &query, td::Promise<td::BufferSlice> promise) {
    if (provider_.empty()) {
      promise.set_error(td::Status::Error("No storage provider"));
      return;
    }
    ...
    td::actor::send_closure(
        ...
        promise.wrap([](td::Unit) mutable { return create_serialize_tl_object<tos_api::storage_daemon_success>(); }));
  }
  ...
  void run_control_query(tos_api::storage_daemon_getProviderInfo &query, td::Promise<td::BufferSlice> promise) {
    if (provider_.empty()) {
      promise.set_error(td::Status::Error("No storage provider"));
      return;
    }
    td::actor::send_closure(provider_, &StorageProvider::get_provider_info, query.with_balances_, query.with_contracts_,
                            promise.wrap([](tl_object_ptr<tos_api::storage_daemon_providerInfo> info) {
                              return serialize_tl_object(info, true);
                            }));
  }

  void run_control_query(tos_api::storage_daemon_setProviderConfig &query, td::Promise<td::BufferSlice> promise) {
    if (provider_.empty()) {
      promise.set_error(td::Status::Error("No storage provider"));
      return;
    }
  ...
  void run_control_query(tos_api::storage_daemon_withdraw &query, td::Promise<td::BufferSlice> promise) {
    if (provider_.empty()) {
      promise.set_error(td::Status::Error("No storage provider"));
      return;
    }
    ...
    td::actor::send_closure(provider_, &StorageProvider::withdraw, address, promise.wrap([](td::Unit) {
      return create_serialize_tl_object<tos_api::storage_daemon_success>();
    }));
  }

  void run_control_query(tos_api::storage_daemon_sendCoins &query, td::Promise<td::BufferSlice> promise) {
    if (provider_.empty()) {
      promise.set_error(td::Status::Error("No storage provider"));
      return;
    }
  ...
  void run_control_query(tos_api::storage_daemon_closeStorageContract &query, td::Promise<td::BufferSlice> promise) {
    if (provider_.empty()) {
      promise.set_error(td::Status::Error("No storage provider"));
      return;
    }
 ...
 private:
  static void fill_torrent_info_short(Torrent &torrent, tos_api::storage_daemon_torrent &obj) {
  ...
  static void fill_torrent_info_full(Torrent &torrent, tos_api::storage_daemon_torrentFull &obj) {
    if (!obj.torrent_) {
      obj.torrent_ = create_tl_object<tos_api::storage_daemon_torrent>();
    }
    ...
    for (size_t i = 0; i < count.value(); ++i) {
      auto file = create_tl_object<tos_api::storage_daemon_fileInfo>();
  ...
  static void get_torrent_info_short(td::actor::ActorId<StorageManager> manager, td::Bits256 hash,
                                     td::Promise<tl_object_ptr<tos_api::storage_daemon_torrent>> promise) {
    td::actor::send_closure(manager, &StorageManager::with_torrent, hash,
                            [promise = std::move(promise)](td::Result<NodeActor::NodeState> R) mutable {
                              ...
                              auto obj = create_tl_object<tos_api::storage_daemon_torrent>();
    ...
    td::actor::send_closure(manager, &StorageManager::with_torrent, hash,
                            [promise = std::move(promise)](td::Result<NodeActor::NodeState> R) mutable {
                              if (R.is_error()) {
                                ...
                                auto state = R.move_as_ok();
                                auto obj = create_tl_object<tos_api::storage_daemon_torrentFull>();
  ...
  tl_object_ptr<tos_api::storage_daemon_config> daemon_config_;
  ...
  void init_toslib_client() {
    if (!toslib_client_.empty()) {
      return;
    }
    auto r_conf_data = td::read_file(global_config_);
    r_conf_data.ensure();
    std::string key_store = db_root_ + "/toslib/";
    td::mkpath(key_store).ensure();
    auto toslib_options = toslib_api::make_object<toslib_api::options>(
        toslib_api::make_object<toslib_api::config>(r_conf_data.move_as_ok().as_slice().str(), "", false, false),
        toslib_api::make_object<toslib_api::keyStoreTypeDirectory>(key_store));
...
int main(int argc, char *argv[]) {
  ...
  scheduler.run_in_context([&] {
    ...
    td::actor::create_actor<tos::StorageDaemon>("storage-daemon", ip_addr, client_mode, global_config, db_root,
