...
class FabricContractWrapper : public td::actor::Actor {
 public:
  class Callback {
   public:
    ...
    virtual void on_transaction(tl_object_ptr<toslib_api::raw_transaction> transaction) = 0;
  };

  explicit FabricContractWrapper(ContractAddress address, td::actor::ActorId<toslib::ToslibClientWrapper> client,
  ...
  void run_get_method(std::string method, std::vector<tl_object_ptr<toslib_api::tvm_StackEntry>> args,
                      td::Promise<std::vector<tl_object_ptr<toslib_api::tvm_StackEntry>>> promise);
 ...
 private:
  ...
  td::actor::ActorId<toslib::ToslibClientWrapper> client_;
  ...
  void load_last_transactions(std::vector<tl_object_ptr<toslib_api::raw_transaction>> transactions,
                              tl_object_ptr<toslib_api::internal_transactionId> next_id, td::uint32 utime);
  ...
  void do_send_external_message_finish(td::Result<const std::vector<tl_object_ptr<toslib_api::raw_message>>*> R);
};

template <typename T>
inline td::Result<T> entry_to_int(const tl_object_ptr<toslib_api::tvm_StackEntry>& entry) {
...
template <>
inline td::Result<td::RefInt256> entry_to_int<td::RefInt256>(const tl_object_ptr<toslib_api::tvm_StackEntry>& entry) {
  auto num = dynamic_cast<toslib_api::tvm_stackEntryNumber*>(entry.get());
  if (num == nullptr) {
    return td::Status::Error("Unexpected value type");
  }
...
inline td::Result<td::Bits256> entry_to_bits256(const tl_object_ptr<toslib_api::tvm_StackEntry>& entry) {
