...
td::Result<ProviderParams> ProviderParams::create(const tl_object_ptr<tos_api::storage_daemon_provider_params>& obj) {
  ProviderParams p;
...
tl_object_ptr<tos_api::storage_daemon_provider_params> ProviderParams::tl() const {
  return create_tl_object<tos_api::storage_daemon_provider_params>(
...
StorageProvider::StorageProvider(ContractAddress account_address, std::string db_root,
                                 td::actor::ActorId<toslib::ToslibClientWrapper> toslib_client,
...
void StorageProvider::start_up() {
  ...
  auto r_state = db::db_get<tos_api::storage_provider_db_state>(
      *db_, create_hash_tl_object<tos_api::storage_provider_db_key_state>(), true);
  r_state.ensure();
  ...
  class Callback : public FabricContractWrapper::Callback {
   public:
    ...
    void on_transaction(tl_object_ptr<toslib_api::raw_transaction> transaction) override {
  ...
  contract_wrapper_ =
      td::actor::create_actor<FabricContractWrapper>("ContractWrapper", main_address_, toslib_client_, keyring_,
                                                     td::make_unique<Callback>(actor_id(this)), last_processed_lt_);

  auto r_config = db::db_get<tos_api::storage_daemon_providerConfig>(
      *db_, create_hash_tl_object<tos_api::storage_provider_db_key_providerConfig>(), true);
  r_config.ensure();
  ...
  auto r_contract_list = db::db_get<tos_api::storage_provider_db_contractList>(
      *db_, create_hash_tl_object<tos_api::storage_provider_db_key_contractList>(), true);
  ...
  if (contract_list) {
    ...
    for (auto& c : contract_list->contracts_) {
      ...
      auto r_contract = db::db_get<tos_api::storage_provider_db_storageContract>(
          *db_, create_hash_tl_object<tos_api::storage_provider_db_key_storageContract>(address.wc, address.addr),
          true);
      r_contract.ensure();
      ...
      auto r_tree = db::db_get<tos_api::storage_provider_db_microchunkTree>(
          *db_, create_hash_tl_object<tos_api::storage_provider_db_key_microchunkTree>(address.wc, address.addr), true);
      r_tree.ensure();
...
void StorageProvider::get_params(td::Promise<ProviderParams> promise) {
  return get_provider_params(toslib_client_, main_address_, std::move(promise));
}

void StorageProvider::get_provider_params(td::actor::ActorId<toslib::ToslibClientWrapper> client,
                                          ContractAddress address, td::Promise<ProviderParams> promise) {
  run_get_method(
      address, client, "get_storage_params", std::vector<tl_object_ptr<toslib_api::tvm_StackEntry>>(),
      promise.wrap([](std::vector<tl_object_ptr<toslib_api::tvm_StackEntry>> stack) -> td::Result<ProviderParams> {
        if (stack.size() != 5) {
...
void StorageProvider::db_store_state() {
  ...
  db_->set(create_hash_tl_object<tos_api::storage_provider_db_key_state>().as_slice(),
           create_serialize_tl_object<tos_api::storage_provider_db_state>(last_processed_lt_))
      .ensure();
...
void StorageProvider::db_store_config() {
  ...
  db_->begin_transaction().ensure();
  db_->set(create_hash_tl_object<tos_api::storage_provider_db_key_providerConfig>().as_slice(),
...
void StorageProvider::process_transaction(tl_object_ptr<toslib_api::raw_transaction> transaction) {
  std::string new_contract_address;
  for (auto& message : transaction->out_msgs_) {
    auto data = dynamic_cast<toslib_api::msg_dataRaw*>(message->msg_data_.get());
    if (data == nullptr) {
      continue;
    }
    auto r_body = vm::std_boc_deserialize(data->body_);
    if (r_body.is_error()) {
      LOG(ERROR) << "Invalid message body in toslib response: " << r_body.move_as_error();
      continue;
    }
    td::Ref<vm::Cell> body = r_body.move_as_ok();
    vm::CellSlice cs = vm::load_cell_slice_quiet(body);
    if (!cs.is_valid()) {
      LOG(ERROR) << "Invalid message body in toslib response";
      continue;
    }
    if (cs.size() >= 32) {
...
void StorageProvider::db_update_storage_contract(const ContractAddress& address, bool update_list) {
  ...
  db_->begin_transaction().ensure();
  if (update_list) {
    std::vector<tl_object_ptr<tos_api::storage_provider_db_contractAddress>> list;
    for (const auto& t : contracts_) {
      list.push_back(create_tl_object<tos_api::storage_provider_db_contractAddress>(t.first.wc, t.first.addr));
    }
    db_->set(create_hash_tl_object<tos_api::storage_provider_db_key_contractList>().as_slice(),
             create_serialize_tl_object<tos_api::storage_provider_db_contractList>(std::move(list)))
        .ensure();
  }
  auto key = create_hash_tl_object<tos_api::storage_provider_db_key_storageContract>(address.wc, address.addr);
  ...
  if (it == contracts_.end()) {
    ...
    db_->set(key.as_slice(),
             create_serialize_tl_object<tos_api::storage_provider_db_storageContract>(
...
void StorageProvider::db_update_microchunk_tree(const ContractAddress& address) {
  ...
  db_->begin_transaction().ensure();
  auto key = create_hash_tl_object<tos_api::storage_provider_db_key_microchunkTree>(address.wc, address.addr);
  auto it = contracts_.find(address);
  if (it == contracts_.end() || it->second.microchunk_tree == nullptr) {
    db_->erase(key.as_slice()).ensure();
  } else {
    db_->set(key.as_slice(), create_serialize_tl_object<tos_api::storage_provider_db_microchunkTree>(
...
void StorageProvider::after_contract_downloaded(ContractAddress address, td::Timestamp retry_until,
  ...
  get_storage_contract_data(address, toslib_client_,
...
void StorageProvider::check_storage_contract_deleted(ContractAddress address, td::Timestamp retry_false_until) {
  check_contract_exists(address, toslib_client_, [=, SelfId = actor_id(this)](td::Result<bool> R) {
    if (R.is_error()) {
      delay_action(
          [=]() {
            td::actor::send_closure(SelfId, &StorageProvider::check_storage_contract_deleted, address,
...
void StorageProvider::check_next_proof(ContractAddress address, StorageContract& contract) {
  ...
  get_storage_contract_data(
      address, toslib_client_, [SelfId = actor_id(this), address](td::Result<StorageContractData> R) {
...
void StorageProvider::got_next_proof_info(ContractAddress address, td::Result<StorageContractData> R) {
  ...
  if (R.is_error()) {
    ...
    check_contract_exists(address, toslib_client_, [SelfId = actor_id(this), address](td::Result<bool> R) {
...
void StorageProvider::get_provider_info(bool with_balances, bool with_contracts,
                                        td::Promise<tl_object_ptr<tos_api::storage_daemon_providerInfo>> promise) {
  auto result = std::make_shared<tos_api::storage_daemon_providerInfo>();
  td::MultiPromise mp;
  auto ig = mp.init_guard();
  ig.add_promise(promise.wrap(
      [result](td::Unit) { return create_tl_object<tos_api::storage_daemon_providerInfo>(std::move(*result)); }));
  ...
  if (with_balances) {
    get_contract_balance(main_address_, toslib_client_, ig.get_promise().wrap([result](td::RefInt256 balance) {
  ...
  if (with_contracts) {
    for (const auto& p : contracts_) {
      auto obj = create_tl_object<tos_api::storage_daemon_contractInfo>();
    ...
    for (const auto& p : contracts_) {
      ...
      if (with_balances) {
        get_contract_balance(p.first, toslib_client_,
        ...
        get_storage_contract_data(p.first, toslib_client_,
...
StorageProvider::Config::Config(const tl_object_ptr<tos_api::storage_daemon_providerConfig>& obj)
    : max_contracts(obj->max_contracts_), max_total_size(obj->max_total_size_) {
}

tl_object_ptr<tos_api::storage_daemon_providerConfig> StorageProvider::Config::tl() const {
  return create_tl_object<tos_api::storage_daemon_providerConfig>(max_contracts, max_total_size);
}
