...
void StorageManager::start_up() {
  ...
  db::db_get<tos_api::storage_db_config>(
      *db_, create_hash_tl_object<tos_api::storage_db_key_config>(), true,
      [SelfId = actor_id(this)](td::Result<tl_object_ptr<tos_api::storage_db_config>> R) {
        if (R.is_error()) {
...
void StorageManager::loaded_config_from_db(tl_object_ptr<tos_api::storage_db_config> config) {
  if (config) {
  ...
  db::db_get<tos_api::storage_db_torrentList>(
      *db_, create_hash_tl_object<tos_api::storage_db_key_torrentList>(), true,
      [SelfId = actor_id(this)](td::Result<tl_object_ptr<tos_api::storage_db_torrentList>> R) {
...
void StorageManager::db_store_config() {
  auto config = create_tl_object<tos_api::storage_db_config>();
  config->download_speed_limit_ = download_speed_limit_;
  config->upload_speed_limit_ = upload_speed_limit_;
  db_->set(create_hash_tl_object<tos_api::storage_db_key_config>(), serialize_tl_object(config, true),
           [](td::Result<td::Unit> R) {
             if (R.is_error()) {
...
void StorageManager::db_store_torrent_list() {
  ...
  db_->set(create_hash_tl_object<tos_api::storage_db_key_torrentList>(),
           create_serialize_tl_object<tos_api::storage_db_torrentList>(std::move(torrents)),
           [](td::Result<td::Unit> R) {
             if (R.is_error()) {
...
void StorageManager::get_peers_info(td::Bits256 hash,
                                    td::Promise<tl_object_ptr<tos_api::storage_daemon_peerList>> promise) {
  TRY_RESULT_PROMISE(promise, entry, get_torrent(hash));
