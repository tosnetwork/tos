...
TEST(MerkleTree, Manual) {
  ...
  size_t i = 0;
  tos::MerkleTree tree(td::transform(hashes, [&i](auto &x) { return tos::MerkleTree::Piece{i++, x}; }));
  LOG(INFO) << timer;
  ...
  for (size_t stride : {1 << 6, 1}) {
    ...
    {
      tos::MerkleTree new_tree(chunks_count, root_hash);
      tos::MerkleTree other_new_tree(chunks_count, root_hash);
      for (size_t i = 0; i < chunks_count; i += stride) {
        ...
        std::vector<tos::MerkleTree::Piece> chunks;
...
TEST(MerkleTree, Stress) {
  ...
  for (int t = 0; t < 100; t++) {
    ...
    for (int t2 = 0; t2 < 1000; t2++) {
      ...
      td::Bitset bitmask;
      tos::MerkleTree new_tree(chunks_count, tree.get_root(rnd.fast(1, 5)));
...
struct TorrentMetas {
  td::optional<tos::Torrent> torrent;
  struct File {
    std::string name;
    td::BlobView buffer;
  };
...
TorrentMetas create_random_torrent(td::Random::Xorshift128plus &rnd, td::int64 total_size = 0,
  ...
  tos::Torrent::Creator::Options options;
  if (piece_size == 0) {
  ...
  tos::Torrent::Creator creator{options};
  ...
  tos::Torrent::GetMetaOptions opt;
  LOG(INFO) << "Meta size (full): " << res.torrent.value().get_meta_str(tos::Torrent::GetMetaOptions()).size();
  LOG(INFO) << "Meta size (only proof): "
            << res.torrent.value().get_meta_str(tos::Torrent::GetMetaOptions().without_header()).size();
  LOG(INFO) << "Meta size (only small proof): "
            << res.torrent.value()
                   .get_meta_str(tos::Torrent::GetMetaOptions().without_header().with_proof_depth_limit(10))
                   .size();
  LOG(INFO) << "Meta size (only header): "
            << res.torrent.value().get_meta_str(tos::Torrent::GetMetaOptions().without_proof()).size();
  LOG(INFO) << "Meta size (min): "
            << res.torrent.value().get_meta_str(tos::Torrent::GetMetaOptions().without_proof().without_header()).size();
  return res;
}
...
TEST(Torrent, Meta) {
  ...
  for (int test_i = 0; test_i < 100; test_i++) {
    ...
    auto torrent_file = tos::TorrentMeta::deserialize(torrent_str).move_as_ok();
    CHECK(torrent_file.serialize() == torrent_str);
    torrent_str.back()++;
    tos::TorrentMeta::deserialize(torrent_str).ensure_error();
    CHECK(torrent.get_info().get_hash() == torrent_file.info.get_hash());

    tos::Torrent::Options options;
    options.in_memory = true;
    torrent_file.header = {};
    torrent_file.root_proof = {};
    auto new_torrent = tos::Torrent::open(options, torrent_file).move_as_ok();
...
TEST(Torrent, OneFile) {
  ...
  auto torrent = tos::Torrent::Creator::create_from_path(options, "first/hello.txt").move_as_ok();
  auto meta = tos::TorrentMeta::deserialize(torrent.get_meta().serialize()).move_as_ok();
  CHECK(torrent.is_completed());

  {
    tos::Torrent::Options options;
    options.root_dir = "first/";
    auto other_torrent = tos::Torrent::open(options, meta).move_as_ok();
...
TEST(Torrent, PartsHelper) {
  int parts_count = 100;
  tos::PartsHelper parts(parts_count);
...
void print_debug(tos::Torrent *torrent) {
...
TEST(Torrent, Peer) {
  class PeerManager : public td::actor::Actor {
   public:
    ...
    void do_send_query(tos::PeerId src, tos::PeerId dst, td::BufferSlice query, td::Result<td::Unit> res,
    ...
    void execute_query(tos::PeerId src, tos::PeerId dst, td::BufferSlice query, td::Result<td::Unit> res,
                       td::Promise<td::BufferSlice> promise) {
      TRY_RESULT_PROMISE(promise, x, std::move(res));
      (void)x;
      ...
      if (it == peers_.end()) {
        ...
        send_closure(node_it->second, &tos::NodeActor::start_peer, src,
                     [promise = std::move(promise),
                      query = std::move(query)](td::Result<td::actor::ActorId<tos::PeerActor>> r_peer) mutable {
                       TRY_RESULT_PROMISE(promise, peer, std::move(r_peer));
                       send_closure(peer, &tos::PeerActor::execute_query, std::move(query), std::move(promise));
                     });
        return;
      }
      send_closure(it->second, &tos::PeerActor::execute_query, std::move(query), std::move(promise));
    }

    void send_response(tos::PeerId src, tos::PeerId dst, td::Result<td::BufferSlice> r_response,
                       td::Promise<td::BufferSlice> promise) {
    ...
    void do_send_response(tos::PeerId src, tos::PeerId dst, td::BufferSlice response, td::Result<td::Unit> res,
                          td::Promise<td::BufferSlice> promise) {
      TRY_RESULT_PROMISE(promise, x, std::move(res));
      (void)x;
      send_closure(
    ...
    void do_execute_response(tos::PeerId src, tos::PeerId dst, td::BufferSlice response, td::Result<td::Unit> res,
                             td::Promise<td::BufferSlice> promise) {
      TRY_RESULT_PROMISE(promise, x, std::move(res));
      (void)x;
   ...
   private:
    ...
    std::map<tos::PeerId, td::actor::ActorOwn<NetChannel>> inbound_channel_;
    std::map<tos::PeerId, td::actor::ActorOwn<NetChannel>> outbound_channel_;
    ...
    td::actor::ActorId<NetChannel> get_outbound_channel(tos::PeerId peer_id) {
    ...
    td::actor::ActorId<NetChannel> get_inbound_channel(tos::PeerId peer_id) {
  ...
  class PeerCreator : public tos::NodeActor::NodeCallback {
   public:
    PeerCreator(td::actor::ActorId<PeerManager> peer_manager, tos::PeerId self_id, std::vector<tos::PeerId> peers)
    ...
    td::actor::ActorOwn<tos::PeerActor> create_peer(tos::PeerId self_id, tos::PeerId peer_id,
          ...
          send_closure(
              ...
              [self = self_, query_id,
               tmp = td::actor::create_actor<X>(PSLICE() << self_id_ << "->" << peer_id_ << " : " << query_id)](
                  auto x) { promise_send_closure(self, &tos::PeerActor::on_query_result, query_id)(std::move(x)); });
        }
      ...
      return td::actor::create_actor<tos::PeerActor>(PSLICE() << "tos::PeerActor " << self_id << "->" << peer_id,
  ...
  class TorrentCallback : public tos::NodeActor::Callback {
   public:
    ...
    void on_closed(tos::Torrent torrent) override {
      CHECK(torrent.is_completed());
  ...
  struct StatsActor : public td::actor::Actor {
   public:
    StatsActor(td::actor::ActorId<tos::NodeActor> node_actor) : node_actor_(node_actor) {
    }

   private:
    td::actor::ActorId<tos::NodeActor> node_actor_;
    void start_up() override {
      alarm_timestamp() = td::Timestamp::in(1);
    }
    void alarm() override {
      send_closure(node_actor_, &tos::NodeActor::with_torrent, [](td::Result<tos::NodeActor::NodeState> r_state) {
        if (r_state.is_error()) {
          return;
        }
  ...
  scheduler.run_in_context([&] {
    ...
    guard->push_back(td::actor::create_actor<tos::NodeActor>(
        "Node#1", 1, std::move(torrent), td::make_unique<TorrentCallback>(stop_watcher, complete_watcher),
        td::make_unique<PeerCreator>(peer_manager.get(), 1, gen_peers(1, 2)), nullptr, tos::SpeedLimiters{}));
    for (size_t i = 2; i <= peers_n; i++) {
      tos::Torrent::Options options;
      options.in_memory = true;
      auto other_torrent = tos::Torrent::open(options, tos::TorrentMeta(info)).move_as_ok();
      auto node_actor = td::actor::create_actor<tos::NodeActor>(
          PSLICE() << "Node#" << i, i, std::move(other_torrent),
          td::make_unique<TorrentCallback>(stop_watcher, complete_watcher),
          td::make_unique<PeerCreator>(peer_manager.get(), i, gen_peers(i, 2)), nullptr, tos::SpeedLimiters{});
