...
NodeActor::NodeActor(PeerId self_id, tos::Torrent torrent, td::unique_ptr<Callback> callback,
                     td::unique_ptr<NodeCallback> node_callback, std::shared_ptr<db::DbType> db,
                     SpeedLimiters speed_limiters, bool should_download, bool should_upload,
...
void NodeActor::db_store_torrent() {
  ...
  auto obj = create_tl_object<tos_api::storage_db_torrentV2>();
  ...
  db_->set(create_hash_tl_object<tos_api::storage_db_key_torrent>(torrent_.get_hash()), serialize_tl_object(obj, true),
           [](td::Result<td::Unit> R) {
             if (R.is_error()) {
...
void NodeActor::db_store_priorities() {
  ...
  auto obj = create_tl_object<tos_api::storage_db_priorities>();
  if (file_priority_.empty()) {
    for (auto &s : pending_set_file_priority_) {
      s.file.visit(td::overloaded(
          [&](const PendingSetFilePriority::All &) {
            obj->actions_.push_back(create_tl_object<tos_api::storage_priorityAction_all>(s.priority));
          },
          [&](const size_t &i) {
            obj->actions_.push_back(create_tl_object<tos_api::storage_priorityAction_idx>(i, s.priority));
          },
          [&](const std::string &name) {
            obj->actions_.push_back(create_tl_object<tos_api::storage_priorityAction_name>(name, s.priority));
          }));
    }
  } else {
    ...
    obj->actions_.push_back(create_tl_object<tos_api::storage_priorityAction_all>(base_priority));
    for (size_t i = 0; i < file_priority_.size(); ++i) {
      if (file_priority_[i] != base_priority) {
        obj->actions_.push_back(create_tl_object<tos_api::storage_priorityAction_idx>(i, file_priority_[i]));
      }
    }
  }
  db_->set(create_hash_tl_object<tos_api::storage_db_key_priorities>(torrent_.get_hash()),
  ...
  delay_action(
      [SelfId = actor_id(this), meta = std::move(meta), db = db_, hash = torrent_.get_hash(),
       new_count = (td::int64)torrent_.get_ready_parts_count()]() {
        ...
        db->set(create_hash_tl_object<tos_api::storage_db_key_torrentMeta>(hash), td::BufferSlice(meta_str),
                [=](td::Result<td::Unit> R) {
                  if (R.is_error()) {
...
void NodeActor::db_store_piece(td::uint64 i, std::string s) {
  ...
  db_->set(create_hash_tl_object<tos_api::storage_db_key_pieceInDb>(torrent_.get_hash(), i), td::BufferSlice(s),
           [](td::Result<td::Unit> R) {
             if (R.is_error()) {
...
void NodeActor::db_erase_piece(td::uint64 i) {
  ...
  db_->erase(create_hash_tl_object<tos_api::storage_db_key_pieceInDb>(torrent_.get_hash(), i),
             [](td::Result<td::Unit> R) {
               if (R.is_error()) {
                 LOG(ERROR) << "Failed to store piece to db: " << R.move_as_error();
               }
...
void NodeActor::db_update_pieces_list() {
  ...
  auto obj = create_tl_object<tos_api::storage_db_piecesInDb>();
  for (td::uint64 p : pieces_in_db_) {
    obj->pieces_.push_back(p);
  }
  db_->set(create_hash_tl_object<tos_api::storage_db_key_piecesInDb>(torrent_.get_hash()),
           serialize_tl_object(obj, true), [](td::Result<td::Unit> R) {
             if (R.is_error()) {
  ...
  class Loader : public td::actor::Actor {
   public:
    ...
    void start_up() override {
      db::db_get<tos_api::storage_db_TorrentShort>(
          *db_, create_hash_tl_object<tos_api::storage_db_key_torrent>(hash_), false,
          [SelfId = actor_id(this)](td::Result<tl_object_ptr<tos_api::storage_db_TorrentShort>> R) {
            if (R.is_error()) {
    ...
    void got_torrent(tl_object_ptr<tos_api::storage_db_TorrentShort> obj) {
      tos_api::downcast_call(*obj, td::overloaded(
                                       [&](tos_api::storage_db_torrent &t) {
                                         root_dir_ = std::move(t.root_dir_);
                                         active_download_ = t.active_download_;
                                         active_upload_ = t.active_upload_;
                                         added_at_ = (td::uint32)td::Clocks::system();
                                       },
                                       [&](tos_api::storage_db_torrentV2 &t) {
                                         root_dir_ = std::move(t.root_dir_);
                                         active_download_ = t.active_download_;
                                         active_upload_ = t.active_upload_;
                                         added_at_ = t.added_at_;
                                       }));
      db_->get(create_hash_tl_object<tos_api::storage_db_key_torrentMeta>(hash_),
    ...
    void got_meta_str(td::optional<td::BufferSlice> meta_str) {
      ...
      db::db_get<tos_api::storage_db_priorities>(
          *db_, create_hash_tl_object<tos_api::storage_db_key_priorities>(hash_), true,
          [SelfId = actor_id(this)](td::Result<tl_object_ptr<tos_api::storage_db_priorities>> R) {
            if (R.is_error()) {
    ...
    void got_priorities(tl_object_ptr<tos_api::storage_db_priorities> priorities) {
      if (priorities != nullptr) {
        for (auto &p : priorities->actions_) {
          td::Variant<PendingSetFilePriority::All, size_t, std::string> file;
          int priority = 0;
          tos_api::downcast_call(*p, td::overloaded(
      ...
      db::db_get<tos_api::storage_db_piecesInDb>(
          *db_, create_hash_tl_object<tos_api::storage_db_key_piecesInDb>(hash_), true,
          [SelfId = actor_id(this)](td::Result<tl_object_ptr<tos_api::storage_db_piecesInDb>> R) {
            if (R.is_error()) {
    ...
    void got_pieces_in_db(tl_object_ptr<tos_api::storage_db_piecesInDb> list) {
      for (auto idx : list == nullptr ? std::vector<td::int64>() : list->pieces_) {
        ++remaining_pieces_in_db_;
        db_->get(create_hash_tl_object<tos_api::storage_db_key_pieceInDb>(hash_, idx),
...
void NodeActor::cleanup_db(std::shared_ptr<db::DbType> db, td::Bits256 hash, td::Promise<td::Unit> promise) {
  ...
  db->erase(create_hash_tl_object<tos_api::storage_db_key_torrent>(hash), ig.get_promise());
  db->erase(create_hash_tl_object<tos_api::storage_db_key_torrentMeta>(hash), ig.get_promise());
  db->erase(create_hash_tl_object<tos_api::storage_db_key_priorities>(hash), ig.get_promise());
  db::db_get<tos_api::storage_db_piecesInDb>(
      *db, create_hash_tl_object<tos_api::storage_db_key_piecesInDb>(hash), true,
      [db, promise = ig.get_promise(), hash](td::Result<tl_object_ptr<tos_api::storage_db_piecesInDb>> R) mutable {
        if (R.is_error()) {
        ...
        auto ig = mp.init_guard();
        ig.add_promise(std::move(promise));
        db->erase(create_hash_tl_object<tos_api::storage_db_key_piecesInDb>(hash), ig.get_promise());
        for (auto idx : pieces->pieces_) {
          db->erase(create_hash_tl_object<tos_api::storage_db_key_pieceInDb>(hash, idx), ig.get_promise());
        }
      });
}

void NodeActor::get_peers_info(td::Promise<tl_object_ptr<tos_api::storage_daemon_peerList>> promise) {
  auto result = std::make_shared<std::vector<tl_object_ptr<tos_api::storage_daemon_peer>>>();
  td::MultiPromise mp;
  auto ig = mp.init_guard();
  ig.add_promise([result, promise = std::move(promise), download_speed = download_speed_.speed(),
    ...
    promise.set_result(
        create_tl_object<tos_api::storage_daemon_peerList>(std::move(*result), download_speed, upload_speed, parts));
  });
  ...
  for (auto &peer : peers_) {
    ...
    result->push_back(create_tl_object<tos_api::storage_daemon_peer>());
