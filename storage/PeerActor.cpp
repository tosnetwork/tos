...
td::uint64 PeerActor::create_and_send_query(ArgsT &&...args) {
  return send_query(tos::create_serialize_tl_object<T>(std::forward<ArgsT>(args)...));
}
...
void PeerActor::execute_query(td::BufferSlice query, td::Promise<td::BufferSlice> promise) {
  on_pong();
  TRY_RESULT_PROMISE(promise, f, tos::fetch_tl_object<tos::tos_api::Function>(std::move(query), true));
  tos::tos_api::downcast_call(
      *f, td::overloaded(
              [&](tos::tos_api::storage_ping &ping) {
                execute_ping(static_cast<td::uint64>(ping.session_id_), std::move(promise));
              },
              [&](tos::tos_api::storage_addUpdate &add_update) { execute_add_update(add_update, std::move(promise)); },
              [&](tos::tos_api::storage_getPiece &get_piece) { execute_get_piece(get_piece, std::move(promise)); },
              [&](tos::tos_api::storage_getTorrentInfo &) { execute_get_torrent_info(std::move(promise)); },
...
void PeerActor::on_get_piece_result(PartId piece_id, td::Result<td::BufferSlice> r_answer) {
  ...
  auto res = [&]() -> td::Result<PeerState::Part> {
    ...
    TRY_RESULT(piece, tos::fetch_result<tos::tos_api::storage_getPiece>(slice.as_slice()));
    PeerState::Part res;
...
void PeerActor::on_get_info_result(td::Result<td::BufferSlice> r_answer) {
  ...
  auto R = fetch_tl_object<tos::tos_api::storage_torrentInfo>(r_answer.move_as_ok(), true);
  if (R.is_error()) {
...
void PeerActor::loop_ping() {
  ...
  alarm_timestamp().relax(next_ping_at_);
  ping_query_id_ = create_and_send_query<tos::tos_api::storage_ping>(node_session_id_);
}

td::BufferSlice PeerActor::create_update_query(tos::tl_object_ptr<tos::tos_api::storage_Update> update) {
  auto session_id = static_cast<td::int64>(peer_session_id_.value());
  auto seqno = static_cast<td::int32>(++node_seqno_);
  return tos::create_serialize_tl_object<tos::tos_api::storage_addUpdate>(session_id, seqno, std::move(update));
}
...
void PeerActor::loop_update_pieces() {
  ...
  if (!have_pieces_list_.empty()) {
    ...
    auto query = create_update_query(tos::create_tl_object<tos::tos_api::storage_updateHavePieces>(
...
void PeerActor::loop_get_torrent_info() {
  ...
  get_info_query_id_ = create_and_send_query<tos::tos_api::storage_getTorrentInfo>();
}
...
void PeerActor::loop_node_get_piece() {
  ...
  for (auto &query_it : node_get_piece_) {
    ...
    query_it.second.query_id =
        create_and_send_query<tos::tos_api::storage_getPiece>(static_cast<td::int32>(query_it.first));
  }
}
    ...
    td::Promise<PeerState::Part> promise =
        [i = p.first, promise = std::move(promise_it->second.promise)](td::Result<PeerState::Part> R) mutable {
          ...
          promise.set_result(R.move_map([](PeerState::Part part) {
            return create_serialize_tl_object<tos_api::storage_piece>(std::move(part.proof), std::move(part.data));
          }));
        };
...
void PeerActor::execute_ping(td::uint64 session_id, td::Promise<td::BufferSlice> promise) {
  ...
  promise.set_value(tos::create_serialize_tl_object<tos::tos_api::storage_pong>());
}

void PeerActor::execute_add_update(tos::tos_api::storage_addUpdate &add_update, td::Promise<td::BufferSlice> promise) {
  ...
  promise.set_value(tos::create_serialize_tl_object<tos::tos_api::storage_ok>());
...
void PeerActor::process_update_peer_parts(const tl_object_ptr<tos_api::storage_Update> &update) {
  CHECK(torrent_info_);
  ...
  downcast_call(*update, td::overloaded(
                             ...
                             [&](const tos::tos_api::storage_updateState &state) {},
                             [&](const tos::tos_api::storage_updateInit &init) {
...
void PeerActor::execute_get_piece(tos::tos_api::storage_getPiece &get_piece, td::Promise<td::BufferSlice> promise) {
...
void PeerActor::execute_get_torrent_info(td::Promise<td::BufferSlice> promise) {
  td::BufferSlice result = create_serialize_tl_object<tos_api::storage_torrentInfo>(
