...
class StorageCli : public td::actor::Actor {
 ...
 private:
  ...
  void init_network() {
    ...
    keyring_ = tos::keyring::Keyring::create(options_.db_root + "/keyring");
    ...
    if (r_public_key.is_error()) {
      ...
      td::actor::send_closure(keyring_, &tos::keyring::Keyring::add_key, std::move(private_key), false,
    ...
    td::actor::send_closure(adnl_network_manager_, &tos::adnl::AdnlNetworkManager::add_self_addr, options_.addr,
    ...
    dht_ =
        tos::dht::Dht::create(tos::adnl::AdnlNodeIdShort{short_id}, "" /*NO db for dht! No wrong cache - no problems*/,
                              dht_config_, keyring_.get(), adnl_.get())
            .move_as_ok();

    send_closure(adnl_, &tos::Adnl::register_dht_node, dht_.get());
    overlays_ = tos::overlay::Overlays::create(options_.db_root, keyring_.get(), adnl_.get(), dht_.get());
  }
  ...
  void parse_line(td::BufferSlice line) {
    ...
    if (cmd == "help") {
      ...
      auto key = tos::dht::DhtKey(public_key_.compute_short_id(), name, 0);
      send_closure(dht_, &tos::dht::Dht::get_value, std::move(key), cmd_promise.wrap([](auto &&res) {
      ...
      tos::dht::DhtKeyDescription dht_key_description{key.clone(), public_key_, dht_update_rule, td::BufferSlice()};

      auto to_sing = dht_key_description.to_sign();
      send_closure(keyring_, &tos::keyring::Keyring::sign_message, public_key_.compute_short_id(), std::move(to_sing),
  ...
  void dht_set1(tos::dht::DhtKeyDescription dht_key_description, td::BufferSlice value,
    ...
    tos::dht::DhtValue dht_value{dht_key_description.clone(), std::move(value), ttl, td::BufferSlice("")};
    auto to_sign = dht_value.to_sign();
    send_closure(keyring_, &tos::keyring::Keyring::sign_message, public_key_.compute_short_id(), std::move(to_sign),
                 promise.send_closure(actor_id(this), &StorageCli::dht_set2, std::move(dht_value)));
  }

  void dht_set2(tos::dht::DhtValue dht_value, td::Result<td::BufferSlice> r_signature, td::Promise<td::Unit> promise) {
    TRY_RESULT_PROMISE(promise, signature, std::move(r_signature));
    dht_value.update_signature(std::move(signature));
    dht_value.check().ensure();

    send_closure(dht_, &tos::dht::Dht::set_value, std::move(dht_value), promise.wrap([](auto &&res) {
      LOG(ERROR) << "OK";
      return td::Unit();
    }));
  }
  ...
  struct Info {
    ...
    td::actor::ActorOwn<tos::NodeActor> node;
  };
  ...
  void torrent_create(td::Slice path_raw, td::Promise<td::Unit> promise) {
    ...
    TRY_RESULT_PROMISE(promise, torrent, tos::Torrent::Creator::create_from_path(options, path));
    ...
    infos_.emplace(torrent_id_, Info{torrent_id_, hash, std::move(torrent), td::actor::ActorOwn<PeerManager>(),
                                     td::actor::ActorOwn<tos::NodeActor>()});
    torrent_id_++;

    promise.set_value(td::Unit());
  }

  td::Result<tos::Torrent *> to_torrent(td::Slice id_raw) {
  ...
  void torrent_info(td::Slice id_raw, td::Promise<td::Unit> promise) {
    ...
    if (info->torrent) {
      ...
      send_closure(info->node, &tos::NodeActor::get_stats_str, promise.wrap([](std::string stats) {
  ...
  td::actor::ActorOwn<PeerManager> create_peer_manager(td::Bits256 hash) {
    ...
    tos::overlay::OverlayIdFull overlay_id(std::move(hash_str));
    auto adnl_id = tos::adnl::AdnlNodeIdShort{public_key_.compute_short_id()};
  ...
  void torrent_start(td::Slice id_raw, bool wait_download, bool should_download, td::Promise<td::Unit> promise) {
    ...
    tos::PeerId self_id = 1;

    class Callback : public tos::NodeActor::Callback {
     public:
    ...
    ptr->node = td::actor::create_actor<tos::NodeActor>(PSLICE() << "Node#" << self_id, self_id, ptr->torrent.unwrap(),
                                                        std::move(callback), std::move(context), nullptr,
                                                        tos::SpeedLimiters{}, should_download);
  ...
  void got_torrent(td::uint32 torrent_id, tos::Torrent &&torrent) {
  ...
  void torrent_set_should_download(td::Slice id_raw, bool should_download, td::Promise<td::Unit> promise) {
    ...
    send_closure(ptr->node, &tos::NodeActor::set_should_download, should_download);
    promise.set_value(td::Unit());
  }
  ...
  void torrent_set_priority(td::ConstParser &parser, td::Promise<td::Unit> promise) {
    ...
    if (all) {
      send_closure(ptr->node, &tos::NodeActor::set_all_files_priority, priority,
                   promise.wrap([](bool) { return td::Unit(); }));
    } else {
      send_closure(ptr->node, &tos::NodeActor::set_file_priority_by_idx, file_id, priority,
                   promise.wrap([](bool) { return td::Unit(); }));
    }
    promise.set_value(td::Unit());
  }
  ...
  void torrent_save(td::Slice id_raw, td::Slice path, td::Promise<td::Unit> promise) {
    ...
    auto meta = ptr->get_meta(tos::Torrent::GetMetaOptions().with_proof_depth_limit(10));
  ...
  td::Result<Info *> torrent_load(td::Slice path) {
    ...
    TRY_RESULT(meta, tos::TorrentMeta::deserialize(data));
    tos::Torrent::Options options;
    options.in_memory = false;
    options.root_dir = ".";
    options.validate = true;

    TRY_RESULT(torrent, tos::Torrent::open(options, data));
  ...
  td::Result<Info *> torrent_add_by_hash(td::Slice hash_hex) {
    ...
    TRY_RESULT(torrent, tos::Torrent::open(options, hash));
