...
class Receiver : public td::actor::Actor {
 ...
 private:
  td::uint16 in_port_;
  td::uint16 out_port_;
  std::shared_ptr<tos::adnl::AdnlProxy> proxy_;
  td::IPAddress addr_;
...
void Receiver::receive_from_client(td::IPAddress addr, td::BufferSlice data) {
  ...
  if (f.flags & (1 << 17)) {
    auto F = fetch_tl_object<tos_api::adnl_ProxyControlPacket>(std::move(f.data), true);
    if (F.is_error()) {
    ...
    tos_api::downcast_call(*F.move_as_ok().get(),
                           td::overloaded(
                               [&](const tos_api::adnl_proxyControlPacketPing &f) {
                                 auto data = create_serialize_tl_object<tos_api::adnl_proxyControlPacketPong>(f.id_);
                                 AdnlProxy::Packet p;
                                 p.flags = 6 | (1 << 16) | (1 << 17);
                               ...
                               [&](const tos_api::adnl_proxyControlPacketPong &f) {},
                               [&](const tos_api::adnl_proxyControlPacketRegister &f) {
...
int main(int argc, char *argv[]) {
  ...
  std::vector<td::actor::ActorOwn<tos::adnl::Receiver>> x;
  ...
  auto R = [&]() -> td::Status {
    ...
    tos::tos_api::engine_adnlProxy_config conf;
    ...
    for (auto &y : conf.ports_) {
      ...
      TRY_RESULT(proxy, tos::adnl::AdnlProxy::create(*y->proxy_type_.get()));
      td::IPAddress a;
      ...
      scheduler.run_in_context([&] {
        x.push_back(td::actor::create_actor<tos::adnl::Receiver>("adnl-proxy", in_port, out_port, std::move(proxy), a));
      });
    }
