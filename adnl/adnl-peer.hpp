...
class AdnlPeerPairImpl : public AdnlPeerPair {
 public:
  ...
  void get_stats(bool all, td::Promise<tl_object_ptr<tos_api::adnl_stats_peerPair>> promise) override;
 ...
 private:
  ...
  struct PacketStats {
    ...
    tl_object_ptr<tos_api::adnl_stats_packets> tl() const;
...
inline td::StringBuilder &operator<<(td::StringBuilder &sb, const tos::adnl::AdnlPeerPairImpl::PrintId &id) {
  sb << "[peerpair " << id.peer_id << "-" << id.local_id << "]";
  return sb;
}

inline td::StringBuilder &operator<<(td::StringBuilder &sb, const tos::adnl::AdnlPeerPairImpl &peer) {
  sb << peer.print_id();
  return sb;
}
