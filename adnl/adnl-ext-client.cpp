...
td::Status AdnlOutboundConnection::process_custom_packet(td::BufferSlice &data, bool &processed) {
  if (data.size() == 12) {
    auto F = fetch_tl_object<tos_api::tcp_pong>(data.clone(), true);
    if (F.is_ok()) {
      processed = true;
      return td::Status::OK();
    }
  ...
  if (!local_id_.empty() && nonce_.size() != 0) {
    auto F = fetch_tl_object<tos_api::tcp_authentificationNonce>(data.clone(), true);
    if (F.is_ok()) {
      auto f = F.move_as_ok();
      ...
      auto obj =
          create_tl_object<tos_api::tcp_authentificationComplete>(local_id_.compute_public_key().tl(), std::move(B));
...
void AdnlOutboundConnection::start_up() {
  ...
  if (!local_id_.empty()) {
    ...
    auto obj = create_tl_object<tos_api::tcp_authentificate>(td::BufferSlice{nonce_.as_slice()});
    send(serialize_tl_object(obj, true));
  }
}
