...
void AdnlDbImpl::update(AdnlNodeIdShort local_id, AdnlNodeIdShort peer_id, AdnlDbItem item,
  ...
  auto obj = create_tl_object<tos_api::adnl_db_node_value>(static_cast<td::int32>(td::Clocks::system()), item.id.tl(),
...
void AdnlDbImpl::get(AdnlNodeIdShort local_id, AdnlNodeIdShort peer_id, td::Promise<AdnlDbItem> promise) {
  ...
  auto F = fetch_tl_object<tos_api::adnl_db_node_value>(td::BufferSlice{value}, true);
  F.ensure();
  auto f = F.move_as_ok();
  AdnlDbItem n;
