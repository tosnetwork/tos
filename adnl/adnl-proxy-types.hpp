...
class AdnlProxyNone : public AdnlProxy {
 public:
  ...
  tl_object_ptr<tos_api::adnl_Proxy> tl() const override {
    return create_tl_object<tos_api::adnl_proxy_none>(id_);
  }
...
class AdnlProxyFast : public AdnlProxy {
 public:
  ...
  td::Result<Packet> decrypt(td::BufferSlice packet) const override;
  tl_object_ptr<tos_api::adnl_Proxy> tl() const override {
    return create_tl_object<tos_api::adnl_proxy_fast>(id_, shared_secret_raw_.clone_as_buffer_slice());
  }
  const td::Bits256 &id() const override {
    return id_;
  }
