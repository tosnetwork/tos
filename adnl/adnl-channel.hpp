...
inline td::StringBuilder &operator<<(td::StringBuilder &sb, const tos::adnl::AdnlChannelImpl::AdnlChannelPrintId &id) {
  sb << "[channel " << id.peer_id_ << "-" << id.local_id_ << " " << id.channel_out_id_ << "-" << id.channel_in_id_
     << "]";
  return sb;
}

inline td::StringBuilder &operator<<(td::StringBuilder &sb, const tos::adnl::AdnlChannelImpl &channel) {
  sb << channel.print_id();
  return sb;
}
