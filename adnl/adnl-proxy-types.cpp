...
td::BufferSlice AdnlProxyFast::encrypt(Packet packet) const {
  ...
  auto obj = create_tl_object<tos_api::adnl_proxyPacketHeader>(id_, packet.flags, packet.ip, packet.port,
...
td::Result<AdnlProxy::Packet> AdnlProxyFast::decrypt(td::BufferSlice packet) const {
  TRY_RESULT(obj, fetch_tl_prefix<tos_api::adnl_proxyPacketHeader>(packet, false));
...
td::Result<std::shared_ptr<AdnlProxy>> AdnlProxy::create(const tos_api::adnl_Proxy &proxy_type) {
  std::shared_ptr<AdnlProxy> R;
  tos_api::downcast_call(
      const_cast<tos_api::adnl_Proxy &>(proxy_type),
      td::overloaded([&](const tos_api::adnl_proxy_none &x) { R = std::make_shared<AdnlProxyNone>(x.id_); },
                     [&](const tos_api::adnl_proxy_fast &x) {
