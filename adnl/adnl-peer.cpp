...
void AdnlPeerPairImpl::discover() {
  ...
  auto P = td::PromiseCreator::lambda([SelfId = actor_id(this), id = print_id(),
    ...
    auto addr_list = fetch_tl_object<tos_api::adnl_addressList>(k.value().clone(), true);
...
void AdnlPeerPairImpl::process_message(const adnlmessage::AdnlMessagePart &message) {
  ...
  if (message.offset() == huge_message_offset_) {
    ...
    if (huge_message_offset_ == huge_message_.size()) {
      ...
      huge_message_hash_.set_zero();
      huge_message_offset_ = 0;
      auto MR = fetch_tl_object<tos_api::adnl_Message>(std::move(huge_message_), true);
      if (MR.is_error()) {
...
void AdnlPeerPairImpl::get_stats(bool all, td::Promise<tl_object_ptr<tos_api::adnl_stats_peerPair>> promise) {
  if (!all) {
  ...
  auto stats = create_tl_object<tos_api::adnl_stats_peerPair>();
  ...
  for (const AdnlAddress &addr : addr_list_.adnl_addrs()) {
    tos_api::downcast_call(*addr->tl(), td::overloaded(
                                            [&](const tos_api::adnl_address_udp &obj) {
...
tl_object_ptr<tos_api::adnl_stats_packets> AdnlPeerPairImpl::PacketStats::tl() const {
  return create_tl_object<tos_api::adnl_stats_packets>(ts_start, ts_end, in_packets, in_bytes, in_packets_channel,
