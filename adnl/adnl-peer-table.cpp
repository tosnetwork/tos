...
void AdnlPeerTableImpl::get_stats_peer(AdnlNodeIdShort peer_id, AdnlPeerTableImpl::PeerInfo &peer_info, bool all,
  ...
  class Cb : public td::actor::Actor {
   public:
    explicit Cb(td::Promise<std::vector<tl_object_ptr<tos_api::adnl_stats_peerPair>>> promise)
        : promise_(std::move(promise)) {
    }

    void got_peer_pair_stats(tl_object_ptr<tos_api::adnl_stats_peerPair> peer_pair) {
      if (peer_pair) {
   ...
   private:
    td::Promise<std::vector<tl_object_ptr<tos_api::adnl_stats_peerPair>>> promise_;
    size_t pending_ = 1;
    std::vector<tl_object_ptr<tos_api::adnl_stats_peerPair>> result_;
  };
  ...
  for (auto &[local_id, peer_pair] : peer_info.peers) {
    ...
    td::actor::send_closure(
        ...
        [local_id = local_id, peer_id = peer_id, callback](td::Result<tl_object_ptr<tos_api::adnl_stats_peerPair>> R) {
          if (R.is_error()) {
...
void AdnlPeerTableImpl::get_stats(bool all, td::Promise<tl_object_ptr<tos_api::adnl_stats>> promise) {
  class Cb : public td::actor::Actor {
   public:
    explicit Cb(td::Promise<tl_object_ptr<tos_api::adnl_stats>> promise) : promise_(std::move(promise)) {
    }

    void got_local_id_stats(tl_object_ptr<tos_api::adnl_stats_localId> local_id) {
    ...
    void got_peer_stats(std::vector<tl_object_ptr<tos_api::adnl_stats_peerPair>> peer_pairs) {
      for (auto &peer_pair : peer_pairs) {
        auto &local_id_stats = local_id_stats_[peer_pair->local_id_];
        if (local_id_stats == nullptr) {
          local_id_stats = create_tl_object<tos_api::adnl_stats_localId>();
    ...
    void dec_pending() {
      ...
      --pending_;
      if (pending_ == 0) {
        auto stats = create_tl_object<tos_api::adnl_stats>();
   ...
   private:
    td::Promise<tl_object_ptr<tos_api::adnl_stats>> promise_;
    size_t pending_ = 1;

    std::map<td::Bits256, tl_object_ptr<tos_api::adnl_stats_localId>> local_id_stats_;
  };
  ...
  for (auto &[id, local_id] : local_ids_) {
    ...
    td::actor::send_closure(local_id.local_id, &AdnlLocalId::get_stats, all,
                            [id = id, callback](td::Result<tl_object_ptr<tos_api::adnl_stats_localId>> R) {
                              if (R.is_error()) {
                                VLOG(ADNL_NOTICE)
  ...
  for (auto &[id, peer] : peers_) {
    ...
    get_stats_peer(id, peer, all,
                   [id = id, callback](td::Result<std::vector<tl_object_ptr<tos_api::adnl_stats_peerPair>>> R) {
                     if (R.is_error()) {
