  ...
  if (socket.allow_proxy) {
    ...
    if (it != proxy_addrs_.end()) {
      ...
      if (packet.flags & (1 << 17)) {
        auto F = fetch_tl_object<tos_api::adnl_ProxyControlPacket>(std::move(packet.data), true);
        if (F.is_error()) {
        ...
        tos_api::downcast_call(*F.move_as_ok().get(),
                               td::overloaded(
                                   [&](const tos_api::adnl_proxyControlPacketPing &f) {
                                     ...
                                     auto data =
                                         create_serialize_tl_object<tos_api::adnl_proxyControlPacketPong>(f.id_);
                                     AdnlProxy::Packet p;
                                     p.flags = 6 | (1 << 17);
                                     p.ip = 0;
                                     p.port = 0;
...
void AdnlNetworkManagerImpl::proxy_register(OutDesc &desc) {
  auto data = create_serialize_tl_object<tos_api::adnl_proxyControlPacketRegister>(0, 0);
  AdnlProxy::Packet p;
  p.flags = 6 | (1 << 17);
  p.ip = 0;
  p.port = 0;
