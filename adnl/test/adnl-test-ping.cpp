...
class AdnlNode : public td::actor::Actor {
 private:
  ...
  td::actor::ActorOwn<tos::AdnlNetworkManager> network_manager_;
  td::actor::ActorOwn<tos::AdnlPeerTable> peer_table_;
  ...
  void receive_query(td::UInt256 src, td::UInt256 dst, td::uint64 query_id, td::BufferSlice data) {
    ...
    td::actor::send_closure(peer_table_, &tos::AdnlPeerTable::answer_query, dst, src, query_id,
                            tos::create_tl_object<tos::tos_api::testObject>());
  }

  std::unique_ptr<tos::AdnlPeerTable::Callback> make_callback() {
    class Callback : public tos::AdnlPeerTable::Callback {
     public:
 ...
 public:
  ...
  AdnlNode() {
    network_manager_ = tos::AdnlNetworkManager::create();
    peer_table_ = tos::AdnlPeerTable::create();
    td::actor::send_closure(network_manager_, &tos::AdnlNetworkManager::register_peer_table, peer_table_.get());
    td::actor::send_closure(peer_table_, &tos::AdnlPeerTable::register_network_manager, network_manager_.get());
  }
  void listen_udp(td::uint16 port) {
    td::actor::send_closure(network_manager_, &tos::AdnlNetworkManager::add_listening_udp_port, "0.0.0.0", port);
    port_ = port;
  }
  ...
  void add_local_id(tos::tl_object_ptr<tos::tos_api::adnl_id_Pk> pk_) {
    auto pub_ = tos::get_public_key(pk_);
    local_id_ = tos::adnl_short_id(pub_);
    std::cout << "local_id = '" << local_id_ << "'\n";
    auto x = tos::create_tl_object<tos::tos_api::adnl_address_udp>(ip_, port_);
    auto v = std::vector<tos::tl_object_ptr<tos::tos_api::adnl_Address>>();
    v.push_back(tos::move_tl_object_as<tos::tos_api::adnl_Address>(x));
    auto y =
        tos::create_tl_object<tos::tos_api::adnl_addressList>(std::move(v), static_cast<td::int32>(td::Time::now()));

    LOG(INFO) << "local_addr_list: " << tos::tos_api::to_string(y);
    td::actor::send_closure(peer_table_, &tos::AdnlPeerTable::add_id, std::move(pk_), std::move(y));
    td::actor::send_closure(peer_table_, &tos::AdnlPeerTable::subscribe, local_id_, "", make_callback());
    local_id_set_ = true;
  }

  void add_foreign(tos::tl_object_ptr<tos::tos_api::adnl_id_Full> id,
                   tos::tl_object_ptr<tos::tos_api::adnl_addressList> addr_list) {
    std::cout << tos::adnl_short_id(id) << "\n";
    td::actor::send_closure(peer_table_, &tos::AdnlPeerTable::add_peer, std::move(id), std::move(addr_list));
  }
  ...
  void alarm() override {
    ...
    if (local_id_set_) {
      for (auto it = ping_ids_.begin(); it != ping_ids_.end(); it++) {
        ...
        td::actor::send_closure(peer_table_, &tos::AdnlPeerTable::send_query, local_id_, *it, std::move(P),
                                td::Timestamp::in(5), tos::create_tl_object<tos::tos_api::getTestObject>());
      }
    }
...
int main(int argc, char *argv[]) {
  ...
  p.add_option('i', "id", "sets local id", [&](td::Slice id) {
    td::actor::send_closure(x, &AdnlNode::add_local_id,
                            tos::create_tl_object<tos::tos_api::adnl_id_pk_unenc>(id.str()));
    return td::Status::OK();
  });
  p.add_option('P', "peer", "adds peer id@host:port", [&](td::Slice id) {
    ...
    auto f_id = tos::create_tl_object<tos::tos_api::adnl_id_unenc>(s1.str());
    td::IPAddress addr;
    ...
    auto f_addr = tos::create_tl_object<tos::tos_api::adnl_address_udp>(addr.get_ipv4(), addr.get_port());
    std::vector<tos::tl_object_ptr<tos::tos_api::adnl_Address>> vv;
    vv.push_back(tos::move_tl_object_as<tos::tos_api::adnl_Address>(f_addr));

    auto f_addr_list =
        tos::create_tl_object<tos::tos_api::adnl_addressList>(std::move(vv), static_cast<int>(td::Time::now()));

    td::actor::send_closure(x, &AdnlNode::add_foreign, tos::move_tl_object_as<tos::tos_api::adnl_id_Full>(f_id),
