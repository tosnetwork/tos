...
class AdnlMessageCreateChannel {
 public:
  ...
  tl_object_ptr<tos_api::adnl_Message> tl() const {
    return create_tl_object<tos_api::adnl_message_createChannel>(key_.raw(), date_);
  }
...
class AdnlMessageConfirmChannel {
 public:
  ...
  tl_object_ptr<tos_api::adnl_Message> tl() const {
    return create_tl_object<tos_api::adnl_message_confirmChannel>(key_.raw(), peer_key_.raw(), date_);
  }
...
class AdnlMessageNop {
 public:
  ...
  tl_object_ptr<tos_api::adnl_Message> tl() const {
    return create_tl_object<tos_api::adnl_message_nop>();
  }
...
class AdnlMessageReinit {
 public:
  ...
  tl_object_ptr<tos_api::adnl_Message> tl() const {
    return create_tl_object<tos_api::adnl_message_reinit>(date_);
  }
...
class AdnlMessage {
 ...
 public:
  explicit AdnlMessage(tl_object_ptr<tos_api::adnl_Message> message);
  template <class T>
  AdnlMessage(T m) : message_(std::move(m)) {
  }

  tl_object_ptr<tos_api::adnl_Message> tl() const {
    tl_object_ptr<tos_api::adnl_Message> res;
...
class AdnlMessageList {
 public:
  ...
  AdnlMessageList(tl_object_ptr<tos_api::adnl_Message> message) {
    auto msg = AdnlMessage{std::move(message)};
    messages_.emplace_back(std::move(msg));
  }
  AdnlMessageList(std::vector<tl_object_ptr<tos_api::adnl_Message>> messages) {
  ...
  tl_object_ptr<tos_api::adnl_Message> one_message() const {
    CHECK(size() == 1);
    return messages_[0].tl();
  }
  std::vector<tl_object_ptr<tos_api::adnl_Message>> mult_messages() const {
    std::vector<tl_object_ptr<tos_api::adnl_Message>> vec;
    for (auto &m : messages_) {
      vec.emplace_back(m.tl());
    }
    return vec;
  }
  static std::vector<tl_object_ptr<tos_api::adnl_Message>> empty_vector() {
    return std::vector<tl_object_ptr<tos_api::adnl_Message>>{};
  }
  auto &vector() {
    return messages_;
  }
...
class OutboundAdnlMessageList {
 public:
  ...
  OutboundAdnlMessageList(tl_object_ptr<tos_api::adnl_Message> message, td::uint32 flags) {
    auto msg = OutboundAdnlMessage{std::move(message), flags};
    messages_.emplace_back(std::move(msg));
  }
  OutboundAdnlMessageList(std::vector<tl_object_ptr<tos_api::adnl_Message>> messages, td::uint32 flags) {
    for (auto &message : messages) {
