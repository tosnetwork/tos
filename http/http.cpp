...
tl_object_ptr<tos_api::http_header> HttpHeader::store_tl() {
  return create_tl_object<tos_api::http_header>(name, value);
}
...
tl_object_ptr<tos_api::http_request> HttpRequest::store_tl(td::Bits256 req_id) {
  std::vector<tl_object_ptr<tos_api::http_header>> headers;
...
tl_object_ptr<tos_api::http_payloadPart> HttpPayload::store_tl(size_t max_size) {
  auto b = ready_bytes();
  if (b > max_size) {
    b = max_size;
  }
  ...
  td::BufferSlice x{b};
  auto S = x.as_slice();
  auto obj = create_tl_object<tos_api::http_payloadPart>(std::move(x),
                                                         std::vector<tl_object_ptr<tos_api::http_header>>(), false);
...
/*tl_object_ptr<tos_api::http_payloadPart> HttpPayload::store_tl(size_t max_size) {
  auto obj = create_tl_object<tos_api::http_payloadPart>(std::vector<td::BufferSlice>(),
                                                         std::vector<tl_object_ptr<tos_api::http_header>>(), false);
...
tl_object_ptr<tos_api::http_PayloadInfo> HttpPayload::store_info(size_t max_size) {
  if (type_ == PayloadType::pt_empty) {
    return create_tl_object<tos_api::http_payloadInfo>(create_tl_object<tos_api::http_payloadEmpty>());
  }
  if (!parse_completed()) {
    return create_tl_object<tos_api::http_payloadSizeUnknown>();
  }
  if (ready_bytes_ > max_size) {
    return create_tl_object<tos_api::http_payloadBig>(ready_bytes_);
  }
  auto obj = store_tl(max_size);
  CHECK(obj->last_);
  return create_tl_object<tos_api::http_payloadInfo>(
      create_tl_object<tos_api::http_payload>(std::move(obj->data_), std::move(obj->trailer_)));
}*/
...
tl_object_ptr<tos_api::http_response> HttpResponse::store_tl() {
  std::vector<tl_object_ptr<tos_api::http_header>> headers;
  headers.reserve(options_.size());
  for (auto &h : options_) {
    headers.push_back(h.store_tl());
  ...
  return create_tl_object<tos_api::http_response>(proto_version_, code_, reason_, std::move(headers), false);
}
