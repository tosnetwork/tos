  ...
  void set_ready(bool ready) {
    ...
    if (!ready) {
      ...
      while (list_.size() > 0) {
        ...
        list_.pop_front();
        td::actor::send_closure(client_, &tos::http::HttpClient::send_request, std::move(q.request),
  ...
  void receive_request(
    ...
    auto P = td::PromiseCreator::lambda(
        [promise = std::move(promise),
         keep](td::Result<std::pair<std::unique_ptr<tos::http::HttpResponse>, std::shared_ptr<tos::http::HttpPayload>>>
                   R) mutable {
          if (R.is_error()) {
