...
HttpAnswer& HttpAnswer::operator<<(AddressCell addr_c) {
  ...
  *this << "<a href=\"" << AccountLink{caddr, tos::BlockIdExt{}} << "\">" << caddr.rserialize(true) << "</a>";
  return *this;
}
...
HttpAnswer& HttpAnswer::operator<<(tos::BlockIdExt block_id) {
  return *this << "<a href=\"" << BlockLink{block_id} << "\">" << block_id.id.to_str() << "</a>";
}

HttpAnswer& HttpAnswer::operator<<(tos::BlockId block_id) {
  return *this << "<a href=\"" << prefix_ << "search?workchain=" << block_id.workchain
               << "&shard=" << tos::shard_to_str(block_id.shard) << "&seqno=" << block_id.seqno << "\">"
               << block_id.to_str() << "</a>";
}
...
HttpAnswer& HttpAnswer::operator<<(BlockSearch bs) {
  *this << "<form class=\"container\" action=\"" << prefix_ << "search\" method=\"get\">"
        ...
        << (bs.block_id.is_valid() ? tos::shard_to_str(bs.block_id.id.shard) : "") << "\"></div>"
        << "<div class=\"form-group col-lg-3 col-md-4\">"
        << "<label>seqno</label>"
...
HttpAnswer& HttpAnswer::operator<<(AccountCell acc_c) {
  *this << "<div>";
  ...
  if (!tos::shard_contains(block_id.shard_full(), tos::extract_addr_prefix(acc_c.addr.workchain, acc_c.addr.addr))) {
  ...
  *this << "<form class=\"container\" action=\"" << prefix_ << "runmethod\" method=\"get\">"
        ...
        << "<input type=\"hidden\" name=\"shard\" value=\"" << tos::shard_to_str(block_id.id.shard) << "\">"
...
HttpAnswer& HttpAnswer::operator<<(BlockHeaderCell head_c) {
  *this << "<div>";
  ...
  try {
    ...
    tos::RootHash vhash{virt_root->get_hash().bits()};
    ...
    *this << "<div class=\"table-responsive my-3\">\n"
          ...
          << tos::BlockId{tos::masterchainId, tos::shardIdAll, info.prev_key_block_seqno} << "</td></tr>\n";
    for (auto id : prev) {
      *this << "<tr><th>prev block</th><td>" << id << "</td></tr>\n";
    }
    if (!before_split) {
      *this << "<tr><th>next block</th><td>"
            << tos::BlockId{block_id.id.workchain, block_id.id.shard, block_id.id.seqno + 1} << "</td></tr>\n";
    } else {
      *this << "<tr><th>next block</th><td>"
            << tos::BlockId{block_id.id.workchain, tos::shard_child(block_id.id.shard, true), block_id.id.seqno + 1}
            << "</td></tr>\n";
      *this << "<tr><th>next block</th><td>"
            << tos::BlockId{block_id.id.workchain, tos::shard_child(block_id.id.shard, false), block_id.id.seqno + 1}
            << "</td></tr>\n";
    }
...
HttpAnswer& HttpAnswer::operator<<(BlockShardsCell shards_c) {
  ...
  if (!sh_conf.unpack(vm::load_cell_slice_ref(shards_c.root))) {
    ...
    auto workchain = tos::masterchainId;
    *this << "<div class=\"table-responsive my-3\">\n"
          << "<table class=\"table\">\n<tbody>\n"
          << "<thead>\n"
          << "<tr>\n"
    ...
    for (auto id : ids) {
      ...
      *this << "<tr>";
      tos::ShardIdFull shard{id.workchain, id.shard};
      if (ref.not_null()) {
...
void HttpAnswer::block_id_link(tos::BlockIdExt block_id) {
  *this << "workchain=" << block_id.id.workchain << "&shard=" << tos::shard_to_str(block_id.id.shard)
...
std::string HttpAnswer::header() {
  ...
  *this << "<div class=\"collapse\" data-parent=\"#searchgroup\" id=\"transactionsearch\">\n"
        << "<div class=\"card card-body\">\n"
        << TransactionSearch{block_id_, account_id_, 0, tos::Bits256::zero()} << "</div></div></div>\n";
