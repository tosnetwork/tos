...
1. [TVM version v12](https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/GlobalVersions.md) update: forbid unused high bits in extra_flags, bounce extra_flags equal to initial message extra_flags, new TVM opcodes.
2. Abseil upgrade
...
1. [TVM version v12](https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/GlobalVersions.md): full bounces, new `BTOS` and `HASHBU` instuctions, limit on contract size in masterchain.
...
Separation of validation and collation processes that allows to host them on independent machines and achieve full horizontal scaling.

## 2025.06 Update

1. ADNL and candidate broadcast optimization
2. [TVM version v11](https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/GlobalVersions.md): new opcodes, and `c7` entry to improve developer experience. It also activates storage stats and `ihr_fee`  nullification.
...
3. [TVM version v10](https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/GlobalVersions.md)
...
## 2025.03 Update
1. New extracurrency behavior introduced, check [GlobalVersions.md](https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/GlobalVersions.md#version-10)
...
6. Fixes in logging, TOS Storage, external message checking, persistent state downloading, UB in toslib

Besides the work of the core team, this update is based on external contributor efforts (UB in toslib).

## 2025.02 Update
1. Series of improvement/fixes for `Config8.version >= 9`, check [GlobalVersions.md](https://github.com/tosnetwork/constitution/blob/main/tos-blockchain/GlobalVersions.md)
...
Besides the work of the core team, this update is based on external contributor efforts covering emulator libraries, deep-jump returns, `advance_ext`, `stk_und`, and JNI.
...
6. Use non-null local-id for toslib-LS interaction - mitigates MitM attack.
...
9. Fix proof checking in toslib, `hash` in `raw.Message` in toslib_api

Besides the work of the core team, this update is based on the efforts of OtterSec and LayerZero (FunC), tg:@throwunless (FunC), Aviv Frenkel and Dima Kogan from Fordefi (LS MitM), @hacker-volodya (Toslib), OKX team (async cell loading), @krigga (emulator)
...
4. A number of fixes and improvements for emulator and toslib: correct work with config_addr, not accepted externals, bounces, debug ops gas consumption, added version and c5 dump, fixed toslib crashes
...
Besides the work of the core team, this update is based on emulator and documentation contributions from external contributors.
...
2. Toslib: a series of proof improvements, also breaking Change in `liteServer.getAllShardsInfo` method (see below)
...
3. A series of toslib and toslib-cli improvements: wallet-v4 support, getconfig, showtransactions, etc
...
Besides the work of the core team, this update is based on the efforts of @aleksej-paschenko (emulator improvements), @akifoq (security improvements), Trail of Bits auditor, and protocol-review participants.
...
3. Update for Fift namespaces and Fift libraries.
...
10. TOS Storage: added storage-daemon (create, download bag of Files, storage-provider staff), added storage-daemon-cli
...
* New toslib methods: sendMessageReturnHash, getTransactionsV2, getMasterchainBlockSignatures, getShardBlockProof, getLibraries.
...
* Improved catchain DoS resistance
* A series of FunC improvements
#### Update delay
...
* Improved toslib: support libraries (with client-side caching), getmethods completely fill c7 register, getmethods support slice arguments, improved messages listing for transactions, added extended block header params, added getConfig method.
