...
report={'commit':commit,'source':source,'create_state_sha256':sha(a.create_state.read_bytes()),'controls':[],'scope':'Single Fift source substitutions. create-state interprets library on each invocation; no compiled binary contains the Fift mutation.'}
for name,before,after,expected in specs:
 ...
 with tempfile.TemporaryDirectory(prefix='uno-descriptor-copy-') as directory:
  ...
  decoded=json.loads((a.out/name/'report.json').read_text());assert decoded['failure_identity']==1221
