...
def reweight_first_validator(cell):
    ...
    for index, value in entries.items():
        ...
        measured += old_weight
        new_weight = old_weight + int(index == 0)
        if new_weight >= 1 << 64:
    ...
    return (Builder().store_uint(0x12, 8).store_uint(since, 32).store_uint(until, 32)
            .store_uint(total, 16).store_uint(main, 16).store_uint(weight + 1, 64)
...
async def submit_reweight(install, state_dir, client, block):
    original, seqno = await read_committee(client, block)
    updated = reweight_first_validator(original)
...
async def wait_reweighted(client, expected):
    while True:
