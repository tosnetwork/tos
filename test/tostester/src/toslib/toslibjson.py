...
class ToslibError(Exception):
    def __init__(self, result: toslib_api.Error):
        super().__init__()
        self.result: toslib_api.Error = result
...
def _parse_toslib_error(result: dict[str, JSONSerializable]):
    if result.get("@type") == "error":
        er = toslib_api.Error.from_dict(result)
        return ToslibError(er)
    return None
...
class TonLib:
    def __init__(
        ...
        self._client: int = toslib.client_json_create()
    ...
    def _send(self, query: JSONSerializable):
        ...
        self._toslib.client_json_send(self._client, q)
    ...
    def _receive(self) -> _Payload[JSONSerializable] | None:
        ...
        result = self._toslib.client_json_receive(self._client, 60)
        if result is None:
            return None
    ...
    async def aclose(self):
        try:
            self._toslib.client_json_cancel_requests(self._client)
            ...
            self._toslib.client_json_destroy(self._client)
            self._client = 0
        ...
        try:
            while True:
                ...
                if "@extra" not in result:
                    continue  # support callback on update from toslib
                ...
                try:
                    toslib_error = _parse_toslib_error(result)
                    if toslib_error is not None:
                        future.set_exception(toslib_error)
                    else:
