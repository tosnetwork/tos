...
class CommonMsgInfo(TlbScheme):
    ...
    value:CurrencyCollection ihr_fee:Tomis fwd_fee:Tomis
    created_lt:uint64 created_at:uint32 = CommonMsgInfo;

    ext_in_msg_info$10 src:MsgAddressExt dest:MsgAddressInt
    import_fee:Tomis = CommonMsgInfo;
...
class InternalMsgInfo(CommonMsgInfo):
    ...
    def __init__(self, ihr_disabled: bool, bounce: bool, bounced: bool, src: Address, dest: Address,
        ...
        self.dest = dest
        self.value = value
        self.value_coins: int = value.tomis
...
class TrCreditPhase(TlbScheme):
    """
    tr_phase_credit$_ due_fees_collected:(Maybe Tomis)
...
class TrComputePhase(TlbScheme):
    ...
    account_activated:Bool gas_fees:Tomis
    ^[
...
class TrActionPhase(TlbScheme):
    ...
    status_change:AccStatusChange
    total_fwd_fees:(Maybe Tomis) total_action_fees:(Maybe Tomis)
...
class TrBouncePhase(TlbScheme):
    ...
    req_fwd_fees:Tomis = TrBouncePhase;

    tr_phase_bounce_ok$1 msg_size:StorageUsedShort
    msg_fees:Tomis fwd_fees:Tomis = TrBouncePhase;
    """
    def __init__(self,
...
class InMsg(TlbScheme):
    ...
    msg_import_ihr$010 msg:^(Message Any) transaction:^Transaction
        ihr_fee:Tomis proof_created:^Cell = InMsg;

    msg_import_imm$011 in_msg:^MsgEnvelope
        transaction:^Transaction fwd_fee:Tomis = InMsg;

    msg_import_fin$100 in_msg:^MsgEnvelope
        transaction:^Transaction fwd_fee:Tomis = InMsg;

    msg_import_tr$101  in_msg:^MsgEnvelope out_msg:^MsgEnvelope
        transit_fee:Tomis = InMsg;

    msg_discard_fin$110 in_msg:^MsgEnvelope transaction_id:uint64
        fwd_fee:Tomis = InMsg;

    msg_discard_tr$111 in_msg:^MsgEnvelope transaction_id:uint64
        fwd_fee:Tomis proof_delivered:^Cell = InMsg;
...
class ImportFees(TlbScheme):
    """
    import_fees$_ fees_collected:Tomis
