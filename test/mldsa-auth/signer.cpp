...
int main(int argc, char** argv) {
  try {
    ...
    if (MLD_API_NAMESPACE(keypair_internal)(pk.data(), sk.data(), seed.data()) != 0) {
    ...
    const int rc = MLD_API_NAMESPACE(signature_internal)(sig.data(), message.data(), message.size(),
        prefix.data(), prefix.size(), rnd.data(), sk.data(), 0);
    volatile uint8_t* wipe = sk.data();
    for (std::size_t i = 0; i < sk.size(); ++i) wipe[i] = 0;
    if (rc != 0 || MLD_API_NAMESPACE(verify)(sig.data(), message.data(), message.size(),
