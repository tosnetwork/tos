...
struct SimNode {
  tos::PrivateKey validator_key;
  tos::PublicKeyHash validator_id;
  tos::PublicKey validator_id_full;
...
DeliveryDiversity summarize_full_node_diversity(const Graph &graph, const std::vector<bool> &delivered,
                                                ...
                                                const std::vector<tos::PublicKeyHash> &sources) {
  std::set<td::Bits256> hashes;
  std::set<tos::PublicKeyHash> source_hashes;
  for (std::size_t i = 0;
...
int main(int argc, char **argv) {
  ...
  scheduler.run_in_context([&] {
    tos::errorlog::ErrorLog::create(db_root);
    keyring = tos::keyring::Keyring::create(db_root);
    network_manager = td::actor::create_actor<tos::adnl::TestLoopbackNetworkManager>("plumtree sim net");
    ...
    overlay_manager =
        tos::overlay::Overlays::create(db_root, keyring.get(), adnl.get(), td::actor::ActorId<tos::dht::Dht>{});
    network->overlay_manager = td::actor::actor_dynamic_cast<tos::overlay::OverlayManager>(overlay_manager.get());
  ...
  std::vector<tos::adnl::AdnlNodeIdShort> adnl_ids;
  std::vector<tos::PublicKeyHash> validator_ids;
  adnl_ids.reserve(graph.nodes.size());
  validator_ids.reserve(validators.size());

  auto overlay_id_full =
      tos::create_serialize_tl_object<tos::tos_api::pub_overlay>(td::BufferSlice("PLUMTREE-GRAPH-SIM"));
  ...
  scheduler.run_in_context([&] {
    ...
    for (std::size_t i = 0; i < nodes.size(); ++i) {
      ...
      nodes[i].adnl_id_full = tos::adnl::AdnlNodeIdFull{adnl_public};
      nodes[i].adnl_id = tos::adnl::AdnlNodeIdShort{adnl_public.compute_short_id()};
      network->node_by_adnl.emplace(nodes[i].adnl_id, i);
      td::actor::send_closure(keyring, &tos::keyring::Keyring::add_key, std::move(adnl_key), true, [](td::Result<>) {});
      td::actor::send_closure(adnl, &tos::adnl::Adnl::add_id, tos::adnl::AdnlNodeIdFull{adnl_public}, addr,
                              static_cast<td::uint8>(0));
      td::actor::send_closure(network_manager, &tos::adnl::TestLoopbackNetworkManager::add_node_id, nodes[i].adnl_id,
                              true, true);
      ...
      td::actor::send_closure(keyring, &tos::keyring::Keyring::add_key, std::move(validator_key), true,
                              [](td::Result<>) {});
  ...
  std::map<tos::PublicKeyHash, td::uint32> authorized_keys;
  for (const auto &key : validator_ids) {
    authorized_keys.emplace(key, tos::overlay::Overlays::max_fec_broadcast_size());
  }
  tos::overlay::OverlayPrivacyRules rules{tos::overlay::Overlays::max_fec_broadcast_size(),
                                          tos::overlay::CertificateFlags::AllowFec, std::move(authorized_keys)};

  scheduler.run_in_context([&] {
    for (std::size_t i = 0; i < nodes.size(); ++i) {
      std::vector<tos::adnl::AdnlNodeIdShort> peer_ids;
      for (auto peer_index : known_peers[i]) {
        peer_ids.push_back(nodes[peer_index].adnl_id);
      }
      std::vector<tos::adnl::AdnlNodeIdShort> plumtree_peer_ids;
      for (auto peer_index : out_neighbours[i]) {
        plumtree_peer_ids.push_back(nodes[peer_index].adnl_id);
      }
      td::actor::send_closure(overlay_manager, &tos::overlay::Overlays::create_private_overlay_ex, nodes[i].adnl_id,
                              tos::overlay::OverlayIdFull(overlay_id_full.clone()), std::move(peer_ids),
                              std::make_unique<DeliveryCallback>(delivery, i), rules, "", opts);
      td::actor::send_closure(network->overlay_manager, &tos::overlay::OverlayManager::set_test_plumtree_neighbours,
    ...
    scheduler.run_in_context([&] {
      for (std::size_t local_index = 0; local_index < validators.size(); ++local_index) {
        ...
        if (settings.broadcast_mode == BroadcastMode::Fec) {
          td::actor::send_closure(overlay_manager, &tos::overlay::Overlays::send_broadcast_plumtree_fec,
                                  nodes[node_index].adnl_id, overlay_id_short, nodes[node_index].validator_id,
                                  tos::overlay::Overlays::BroadcastFlagAnySender(), std::move(payload_for_sender));
        } else {
          td::actor::send_closure(overlay_manager, &tos::overlay::Overlays::send_broadcast_plumtree,
                                  nodes[node_index].adnl_id, overlay_id_short, nodes[node_index].validator_id,
                                  tos::overlay::Overlays::BroadcastFlagAnySender(), broadcast_id,
          ...
          for (auto &event : due_events) {
            switch (event.kind) {
              case SimEventKind::Message:
                td::actor::send_closure(network->overlay_manager, &tos::overlay::OverlayManager::receive_message,
                                        event.src, event.dst, std::move(event.data));
                break;
              case SimEventKind::Query:
                td::actor::send_closure(network->overlay_manager, &tos::overlay::OverlayManager::receive_query,
