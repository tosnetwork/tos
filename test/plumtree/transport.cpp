...
namespace tos::overlay::plumtree_sim {
namespace {
...
std::int32_t overlay_payload_constructor_id(td::Slice data) {
  ...
  if (fetch_tl_prefix<tos_api::overlay_messageWithExtra>(payload, true).is_ok()) {
    return tl_constructor_id(payload);
  }
  payload = data;
  if (fetch_tl_prefix<tos_api::overlay_message>(payload, true).is_ok()) {
    return tl_constructor_id(payload);
  }
...
void SimNetwork::enqueue_event(adnl::AdnlNodeIdShort src, adnl::AdnlNodeIdShort dst, td::BufferSlice data,
  ...
  auto now = now_s();
  sent_bytes += bytes;
  if (type == tos_api::overlay_broadcastPlumtreePrune::ID) {
    ++prune_messages;
  }
  auto src_it = node_by_adnl.find(src);
...
std::vector<SimEvent> SimNetwork::pop_due_events(double time_s) {
  ...
  while (!events.empty() && events.begin()->first.first <= time_s + 1e-12) {
    ...
    if (payload_type == tos_api::overlay_broadcastPlumtreeFec::ID ||
        payload_type == tos_api::overlay_broadcastPlumtreeSimple::ID) {
      ++payload_deliveries;
    }
    auto dst_it = node_by_adnl.find(event.dst);
