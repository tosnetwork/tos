...
class CatChainInst : public td::actor::Actor {
 public:
  class PayloadExtra : public tos::catchain::CatChainBlock::Extra {
   public:
    PayloadExtra(td::uint64 sum) : sum(sum) {
    }
    td::uint64 sum;
  };
  void process_blocks(std::vector<tos::catchain::CatChainBlock *> blocks) {
    td::uint64 sum = sum_;
    for (auto &B : blocks) {
    ...
    td::actor::send_closure(catchain_, &tos::catchain::CatChain::processed_block,
  ...
  void preprocess_block(tos::catchain::CatChainBlock *block) {
    td::uint64 sum = 0;
  ...
  void alarm() override {
    td::actor::send_closure(catchain_, &tos::catchain::CatChain::need_new_block, td::Timestamp::in(0.1));
  }

  void start_up() override {
    alarm_timestamp() = td::Timestamp::in(0.1);
    tos::CatChainOptions opts;
    opts.debug_disable_db = true;
    //opts.block_hash_covers_data = true;

    std::vector<tos::catchain::CatChainNode> nodes;
    for (auto &n : nodes_) {
      nodes.push_back(tos::catchain::CatChainNode{n.adnl_id, n.id_full});
    }
    catchain_ =
        tos::catchain::CatChain::create(make_callback(), opts, keyring_, adnl_, {}, overlay_manager_, std::move(nodes),
                                        nodes_[idx_].id, unique_hash_, std::string(""), "", false);
  }

  CatChainInst(td::actor::ActorId<tos::keyring::Keyring> keyring, td::actor::ActorId<tos::adnl::Adnl> adnl,
               td::actor::ActorId<tos::overlay::Overlays> overlay_manager, std::vector<Node> nodes, td::uint32 idx,
               tos::catchain::CatChainSessionId unique_hash)
      : keyring_(keyring)
      , adnl_(adnl)
  ...
  std::unique_ptr<tos::catchain::CatChain::Callback> make_callback() {
    class Callback : public tos::catchain::CatChain::Callback {
     public:
      void process_blocks(std::vector<tos::catchain::CatChainBlock *> blocks) override {
      ...
      void preprocess_block(tos::catchain::CatChainBlock *block) override {
        td::actor::send_closure(id_, &CatChainInst::preprocess_block, std::move(block));
      }
      void process_broadcast(const tos::PublicKeyHash &src, td::BufferSlice data) override {
        UNREACHABLE();
      }
      void process_message(const tos::PublicKeyHash &src, td::BufferSlice data) override {
        UNREACHABLE();
      }
      void process_query(const tos::PublicKeyHash &src, td::BufferSlice data,
  ...
  void create_fork() {
    ...
    td::actor::send_closure(catchain_, &tos::catchain::CatChain::debug_add_fork,
 ...
 private:
  ...
  td::actor::ActorId<tos::overlay::Overlays> overlay_manager_;

  std::vector<Node> nodes_;
  td::uint32 idx_;

  tos::catchain::CatChainSessionId unique_hash_;

  td::actor::ActorOwn<tos::catchain::CatChain> catchain_;
  td::uint64 sum_ = 0;
  td::uint32 height_ = 0;
...
int main(int argc, char *argv[]) {
  ...
  for (td::uint32 att = 0; att < 20; att++) {
    ...
    tos::catchain::CatChainSessionId unique_id;
