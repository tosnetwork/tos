...
CPP=[
 ('response-body-bound','policy','http_transport_max_body_bytes = 4194304','http_transport_max_body_bytes = 67108864'),
 ('response-header-bound','policy','http_transport_max_header_bytes = 8192','http_transport_max_header_bytes = 65536'),
 ('response-duplicate-header','policy','if (fields.contains(name))','if (false && fields.contains(name))'),
 ('response-compression','policy','name == "content-encoding"','false'),
 ('response-transfer-encoding','policy','name == "transfer-encoding"','false'),
 ('response-upgrade','policy','name == "upgrade"','false'),
 ('response-header-count','policy','fields.size() >= http_transport_max_headers','false'),
 ('response-length-leading-zero','policy',"(value.size() > 1 && value.front() == '0')",'false'),
 ('response-redirect','policy','!(status >= 300 && status < 400)','true'),
 ('response-version','local','h.line.substr(0, 9) != "HTTP/1.1 "','false'),
 ('response-peer-uid','local','credentials.value() != expected','false'),
 ('request-version','local','head.line.substr(second + 1) != "HTTP/1.1"','false'),
 ('request-get-body','local','request.verb == "GET" && head.length != 0','false'),
 ('request-host','local','!head.fields.contains("host") || head.fields.at("host").empty()','false'),
]
RUST=[
...
def main(args):
 ...
 with tempfile.TemporaryDirectory(prefix='p0-http-mut-') as tmp:
  ...
  if args.language=='rust':
   ...
   sources={
    'local':folder/'mutated-local-channel.cpp',
    'policy':folder/'http-mutant-root/validator/auth/http-transport-policy.h',
   }
   binary=folder/'test-p0-http-mutant'
   command=['cmake','--build',str(args.build),'--target','test-p0-http-mutant','-j2']
   mutations=CPP
   originals={name:path.read_text() for name,path in sources.items()}
   def build(texts):
    for name,path in sources.items():path.write_text(texts[name])
    result=subprocess.run(command,capture_output=True,text=True)
    if result.returncode:raise RuntimeError('mutation build failed: '+result.stdout+result.stderr)
  def test(case=None):
  ...
  if args.language=='rust':
   ...
   build(originals);base=test();assert base.returncode==0,base.stderr
   try:
    for case,source_key,before,after in mutations:
     mutated=dict(originals);mutated[source_key]=replace_once(originals[source_key],before,after)
     build(mutated);result=test(case)
     assert result.returncode==1 and 'AssertionError:' in result.stderr and repr(case) in result.stderr and 'RuntimeError' not in result.stderr,(case,'invalid kill',result.stderr)
     report.append(dict(language=args.language,guard=case,compiled=True,assertion_failed=True));print('KILLED:',args.language,case,flush=True)
   finally:
    build(originals);base=test();assert base.returncode==0,base.stderr
