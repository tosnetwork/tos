...
tos::PrivateKey make_private_key(td::uint8 seed) {
  td::uint8 data[32];
  std::memset(data, seed, 32);
  return tos::PrivateKey{tos::privkeys::Ed25519{td::Slice(data, 32)}};
}

// Fixed keys for benchmarking (lazy initialization)
const tos::PrivateKey& server_private_key() {
  static auto key = make_private_key(1);
  return key;
}
const tos::PublicKey& server_public_key() {
  static auto key = server_private_key().compute_public_key();
  return key;
}
tos::PrivateKey client_private_key(td::uint32 client_id = 0) {
  return make_private_key(static_cast<td::uint8>(2 + client_id));
}
tos::PublicKey client_public_key(td::uint32 client_id = 0) {
...
struct Config {
  ...
  tos::quic::CongestionControlAlgo cc_algo = tos::quic::CongestionControlAlgo::Bbr;
...
class Server : public tos::adnl::Adnl::Callback {
 public:
  Server(td::uint32 response_size) : response_size_(response_size) {
  }

  void receive_message(tos::adnl::AdnlNodeIdShort src, tos::adnl::AdnlNodeIdShort dst, td::BufferSlice data) override {
  }
...
class BenchmarkRunner : public td::actor::Actor {
 public:
  BenchmarkRunner(Config config, td::actor::ActorId<tos::adnl::AdnlSenderInterface> rldp,
                  tos::adnl::AdnlNodeIdShort src, tos::adnl::AdnlNodeIdShort dst)
 ...
 private:
  ...
  void send_queries() {
    ...
    while (sent_ < config_.num_queries && inflight_ < max_inflight) {
      ...
      td::actor::send_closure(rldp_, &tos::adnl::AdnlSenderInterface::send_query_ex, src_, dst_, std::string("bench"),
  ...
  void finish() {
    ...
    if (config_.protocol == Protocol::quic) {
      auto quic_sender = td::actor::actor_dynamic_cast<tos::quic::QuicSender>(rldp_);
      td::actor::send_closure(quic_sender, &tos::quic::QuicSender::log_stats, "bench-complete");
      exit_pending_ = true;
...
class StatsReporter : public td::actor::Actor {
 public:
  StatsReporter(td::actor::ActorId<tos::quic::QuicSender> quic_sender, std::string reason, bool enabled,
                double interval_sec)
  ...
  void alarm() override {
    if (enabled_ && !quic_sender_.empty()) {
      td::actor::send_closure(quic_sender_, &tos::quic::QuicSender::log_stats, reason_);
    }
    alarm_timestamp() = td::Timestamp::in(interval_sec_);
  }

 private:
  td::actor::ActorId<tos::quic::QuicSender> quic_sender_;
  std::string reason_;
  bool enabled_{false};
...
void run_loopback(Config config) {
  ...
  scheduler.run_in_context([&] {
    ...
    network_manager = td::actor::create_actor<tos::adnl::TestLoopbackNetworkManager>("net");
    ...
    quic_sender = td::actor::create_actor<tos::quic::QuicSender>(
        "quic", td::actor::actor_dynamic_cast<tos::adnl::AdnlPeerTable>(adnl.get()), keyring.get());
    td::actor::send_closure(quic_sender, &tos::quic::QuicSender::set_quic_options,
                            tos::quic::QuicServer::Options{.enable_gso = config.enable_gso,
    ...
    td::actor::send_closure(quic_sender, &tos::quic::QuicSender::add_id, dst);
    ...
    td::actor::send_closure(adnl, &tos::adnl::Adnl::subscribe, dst, "B",
...
void run_server(Config config) {
  ...
  scheduler.run_in_context([&] {
    ...
    td::actor::send_closure(network_manager, &tos::adnl::AdnlNetworkManager::add_self_addr, self_addr,
                            std::move(cat_mask), 0);

    auto local_id = tos::adnl::AdnlNodeIdShort{server_public_key().compute_short_id()};
    td::actor::send_closure(keyring, &tos::keyring::Keyring::add_key, server_private_key(), true, [](td::Result<>) {});
    ...
    td::actor::send_closure(adnl, &tos::adnl::Adnl::add_id, tos::adnl::AdnlNodeIdFull{server_public_key()}, addr_list,
                            td::uint8(0));
...
void run_client(Config config) {
  ...
  scheduler.run_in_context([&] {
    ...
    src = tos::adnl::AdnlNodeIdShort{client_priv_key.compute_public_key().compute_short_id()};
    td::actor::send_closure(keyring, &tos::keyring::Keyring::add_key, std::move(client_priv_key), true,
                            [](td::Result<>) {});

    tos::adnl::AdnlAddressList local_addr_list;
    local_addr_list.add_udp_adnl_address(self_addr).ensure();
    local_addr_list.set_version(static_cast<td::int32>(td::Clocks::system()));
    local_addr_list.set_reinit_date(tos::adnl::Adnl::adnl_start_time());

    td::actor::send_closure(adnl, &tos::adnl::Adnl::add_id,
                            tos::adnl::AdnlNodeIdFull{client_public_key(config.client_id)}, local_addr_list,
                            td::uint8(0));
    ...
    dst = tos::adnl::AdnlNodeIdShort{server_public_key().compute_short_id()};
    tos::adnl::AdnlAddressList server_addr_list;
    ...
    tos::adnl::AdnlNodesList static_nodes;
    static_nodes.push(tos::adnl::AdnlNode{tos::adnl::AdnlNodeIdFull{server_public_key()}, server_addr_list});
    td::actor::send_closure(adnl, &tos::adnl::Adnl::add_static_nodes_from_config, std::move(static_nodes));
...
int main(int argc, char* argv[]) {
  ...
  p.add_checked_option('\0', "cc", "congestion control algorithm: cubic, reno, bbr (default: bbr)", [&](td::Slice arg) {
    if (arg == "cubic") {
      config.cc_algo = tos::quic::CongestionControlAlgo::Cubic;
    } else if (arg == "reno") {
      config.cc_algo = tos::quic::CongestionControlAlgo::Reno;
    } else if (arg == "bbr") {
      config.cc_algo = tos::quic::CongestionControlAlgo::Bbr;
    } else {
