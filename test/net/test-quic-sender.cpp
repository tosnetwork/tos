...
tos::PrivateKey make_key(int seed) {
  td::Bits256 hash;
  auto seed_str = std::to_string(seed);
  td::sha256(td::Slice(seed_str), hash.as_slice());
  return tos::PrivateKey{tos::privkeys::Ed25519{hash}};
}

tos::adnl::AdnlAddressList make_addr_list(td::Slice ip_str, int port) {
  td::IPAddress ip;
  ip.init_host_port(PSTRING() << ip_str << ":" << port).ensure();
  tos::adnl::AdnlAddressList list;
  list.add_udp_adnl_address(ip).ensure();
  list.set_version(static_cast<td::int32>(td::Clocks::system()));
  list.set_reinit_date(tos::adnl::Adnl::adnl_start_time());
  return list;
}
...
class EchoCallback : public tos::adnl::Adnl::Callback {
 public:
  ...
  void receive_message(tos::adnl::AdnlNodeIdShort, tos::adnl::AdnlNodeIdShort, td::BufferSlice data) override {
...
class SlowEchoCallback : public tos::adnl::Adnl::Callback {
 public:
...
class LimitedEchoCallback : public tos::adnl::Adnl::Callback {
 public:
...
class NeverRespondCallback : public tos::adnl::Adnl::Callback {
 public:
...
class StaticResponseCallback : public tos::adnl::Adnl::Callback {
 public:
...
class TestRunner : public td::actor::Actor {
 public:
  ...
  td::actor::Task<TestNode> create_node(std::string name, int port, std::optional<tos::PrivateKey> key = std::nullopt,
                                        std::string ip = "127.0.0.1") {
    TestNode node;
    node.ip = ip;
    node.port = port;
    ...
    td::actor::send_closure(node.adnl, &tos::adnl::Adnl::subscribe, node.id, "M",
  ...
  void add_peer(TestNode& from, const TestNode& to) {
    ...
    td::actor::send_closure(from.adnl, &tos::adnl::Adnl::add_peer, from.id,
                            tos::adnl::AdnlNodeIdFull{to.key.compute_public_key()}, addr_list);
  }

  void set_slow_echo(TestNode& node, double delay) {
    td::actor::send_closure(node.adnl, &tos::adnl::Adnl::subscribe, node.id, "S",
                            std::make_unique<SlowEchoCallback>(delay));
  }

  void set_never_respond(TestNode& node) {
    td::actor::send_closure(node.adnl, &tos::adnl::Adnl::subscribe, node.id, "X",
                            std::make_unique<NeverRespondCallback>());
  }

  void set_static_response(TestNode& node, std::string prefix, std::string response) {
    td::actor::send_closure(node.adnl, &tos::adnl::Adnl::subscribe, node.id, std::move(prefix),
  ...
  td::actor::Task<td::BufferSlice> send_never_respond_query(TestNode& from, TestNode& to, td::Slice data,
    ...
    td::actor::send_closure(from.quic_sender, &tos::quic::QuicSender::send_query_ex, from.id, to.id, std::string("X"),
  ...
  td::actor::Task<td::BufferSlice> send_query(TestNode& from, TestNode& to, td::Slice data) {
    ...
    auto [future, promise] = td::actor::StartedTask<td::BufferSlice>::make_bridge();
    td::actor::send_closure(from.quic_sender, &tos::quic::QuicSender::send_query, from.id, to.id, std::string("Q"),
  ...
  td::actor::Task<td::BufferSlice> send_query_ex(TestNode& from, TestNode& to, td::Slice data, double timeout,
    ...
    auto [future, promise] = td::actor::StartedTask<td::BufferSlice>::make_bridge();
    td::actor::send_closure(from.quic_sender, &tos::quic::QuicSender::send_query_ex, from.id, to.id, std::string("Q"),
                            std::move(promise), td::Timestamp::in(timeout), std::move(query), max_answer_size);
    co_return co_await std::move(future);
  }
  ...
  td::actor::Task<td::BufferSlice> send_slow_query_ex(TestNode& from, TestNode& to, td::Slice data, double timeout,
    ...
    auto [future, promise] = td::actor::StartedTask<td::BufferSlice>::make_bridge();
    td::actor::send_closure(from.quic_sender, &tos::quic::QuicSender::send_query_ex, from.id, to.id, std::string("S"),
                            std::move(promise), td::Timestamp::in(timeout), std::move(query), max_answer_size);
    co_return co_await std::move(future);
  }
  ...
  td::actor::Task<td::BufferSlice> send_prefixed_query_ex(TestNode& from, TestNode& to, td::Slice prefix,
    ...
    auto [future, promise] = td::actor::StartedTask<td::BufferSlice>::make_bridge();
    td::actor::send_closure(from.quic_sender, &tos::quic::QuicSender::send_query_ex, from.id, to.id, prefix.str(),
                            std::move(promise), td::Timestamp::in(timeout), std::move(query), max_answer_size);
    co_return co_await std::move(future);
  }
  ...
  void send_message(TestNode& from, TestNode& to, td::Slice data) {
    ...
    td::actor::send_closure(from.quic_sender, &tos::quic::QuicSender::send_message, from.id, to.id, std::move(msg));
  }
...
tos::quic::QuicServer::Options small_stream_limit_options(size_t max_streams_bidi) {
...
struct RawQuicEndpointState {
  void remember_connection(tos::quic::QuicConnectionId cid, td::SecureString local_public_key,
                            td::SecureString peer_public_key, bool is_outbound) {
    std::lock_guard guard(mutex);
    if (is_outbound) {
      outbound_cid = cid;
      outbound_local_public_key = local_public_key.as_slice().str();
    } else {
      inbound_cid = cid;
      inbound_local_public_key = local_public_key.as_slice().str();
    }
  }

  void remember_local_stream(tos::quic::QuicStreamID sid) {
    std::lock_guard guard(mutex);
    locally_opened_streams.insert(sid);
  }

  bool is_local_stream(tos::quic::QuicStreamID sid) const {
    std::lock_guard guard(mutex);
    return locally_opened_streams.contains(sid);
  }

  void remember_closed_stream(tos::quic::QuicStreamID sid) {
    std::lock_guard guard(mutex);
  ...
  bool has_closed_stream(tos::quic::QuicStreamID sid) const {
    std::lock_guard guard(mutex);
  ...
  std::optional<tos::quic::QuicConnectionId> get_outbound_cid() const {
    std::lock_guard guard(mutex);
    return outbound_cid;
  }

  std::optional<tos::quic::QuicConnectionId> get_inbound_cid() const {
    std::lock_guard guard(mutex);
    return inbound_cid;
  }
  ...
  std::optional<tos::quic::QuicConnectionId> outbound_cid;
  std::optional<tos::quic::QuicConnectionId> inbound_cid;
  std::optional<std::string> outbound_local_public_key;
  std::optional<std::string> inbound_local_public_key;
  std::unordered_set<tos::quic::QuicStreamID> locally_opened_streams;
  std::unordered_set<tos::quic::QuicStreamID> closed_streams;
  size_t closed_stream_count = 0;
  size_t local_closed_stream_count = 0;
};

class RawQuicCallback final : public tos::quic::QuicServer::Callback {
 public:
  ...
  td::Status on_stream(tos::quic::QuicConnectionId cid, tos::quic::QuicStreamID sid, td::BufferSlice,
    ...
    if (is_end && !state_->is_local_stream(sid)) {
      td::actor::send_closure(server_, &tos::quic::QuicServer::send_stream_end, cid, sid);
    }
    return td::Status::OK();
  }
...
struct RawQuicEndpoint {
  ...
  td::actor::ActorOwn<tos::quic::QuicServer> server;
...
class RawQuicTestRunner final : public td::actor::Actor {
 public:
  ...
  td::actor::Task<RawQuicEndpoint> create_endpoint(tos::quic::QuicServer::Options options) {
    auto port = next_port();
  ...
  td::actor::Task<tos::quic::QuicStreamID> open_stream_with_retry(RawQuicEndpoint& endpoint,
                                                                  tos::quic::QuicConnectionId cid,
                                                                  double timeout = 5.0) {
    auto deadline = td::Timestamp::in(timeout);
    while (true) {
      auto sid_result =
          co_await td::actor::ask(endpoint.server, &tos::quic::QuicServer::open_stream, cid, tos::quic::StreamOptions{})
              .wrap();
      if (sid_result.is_ok()) {
  ...
  td::actor::Task<tos::quic::QuicStreamID> send_and_fin_stream(RawQuicEndpoint& endpoint,
  ...
  td::actor::Task<size_t> open_streams_until_blocked(RawQuicEndpoint& endpoint, tos::quic::QuicConnectionId cid,
                                                     size_t max_attempts) {
    size_t opened = 0;
  ...
  td::actor::Task<td::Unit> wait_for_stream_close(const RawQuicEndpoint& endpoint, tos::quic::QuicStreamID sid,
                                                  double timeout = 5.0) {
    ...
    for (int i = 0; i < n; i++) {
      for (int j = 0; j < n; j++) {
        if (i != j) {
          ...
          auto [future, promise] = td::actor::StartedTask<td::BufferSlice>::make_bridge();
          td::actor::send_closure(nodes[i].quic_sender, &tos::quic::QuicSender::send_query, nodes[i].id, nodes[j].id,
...
tos::adnl::AdnlNodeIdShort short_id_from_key(const td::Ed25519::PrivateKey& key) {
  return tos::adnl::AdnlNodeIdFull(tos::PublicKey(tos::pubkeys::Ed25519(key.get_public_key().move_as_ok())))
      .compute_short_id();
}
...
TEST(QuicSniDispatch, ClientSniSelectsAdditionalIdentity) {
  run_raw_quic_test([](RawQuicTestRunner& t) -> td::actor::Task<td::Unit> {
    ...
    auto server = co_await t.create_endpoint(quic_test_options());
    auto extra_key = make_quic_key(server.port * 31 + 1);
    auto extra_id = short_id_from_key(extra_key);
    td::actor::send_closure(server.server, &tos::quic::QuicServer::add_identity, extra_id,
                            clone_quic_key(extra_key));
    co_await td::actor::Yield{};
    co_await t.connect(client, server, tos::quic::ServerIdentity::sni(extra_id));
    ASSERT_TRUE(server.state->get_inbound_local_public_key().has_value());
    ASSERT_EQ(*server.state->get_inbound_local_public_key(), pubkey_bytes(extra_key));
    co_return td::Unit{};
  });
}
...
TEST(QuicSniDispatch, NoSniFallsBackToDefaultIdentity) {
  run_raw_quic_test([](RawQuicTestRunner& t) -> td::actor::Task<td::Unit> {
    ...
    td::actor::send_closure(server.server, &tos::quic::QuicServer::add_identity, short_id_from_key(extra_key),
                            clone_quic_key(extra_key));
    co_await td::actor::Yield{};
    co_await t.connect(client, server);
    ASSERT_TRUE(server.state->get_inbound_local_public_key().has_value());
    ASSERT_EQ(*server.state->get_inbound_local_public_key(), pubkey_bytes(server.key));
    co_return td::Unit{};
  });
}
...
TEST(QuicSniDispatch, UnknownSniFailsHandshake) {
  run_raw_quic_test([](RawQuicTestRunner& t) -> td::actor::Task<td::Unit> {
    auto client = co_await t.create_endpoint(quic_test_options());
    auto server = co_await t.create_endpoint(quic_test_options());
    auto bogus_id = short_id_from_key(make_quic_key(server.port * 31 + 3));
    auto bogus_sni = tos::quic::ServerIdentity::sni(bogus_id);
    auto result = co_await td::actor::ask(client.server, &tos::quic::QuicServer::connect, td::Slice("127.0.0.1"),
                                          server.port, clone_quic_key(client.key), td::Slice("tos"),
                                          td::Slice(bogus_sni))
                       .wrap();
    LOG_CHECK(result.is_ok()) << "connect failed before handshake: " << result.error();
    static_cast<void>(result.move_as_ok());
    co_await td::actor::coro_sleep(td::Timestamp::in(0.2));
    ASSERT_TRUE(!server.state->get_inbound_local_public_key().has_value());
    co_return td::Unit{};
  });
...
TEST(QuicSender, WrongPublicKey) {
  run_test([](TestRunner& t) -> td::actor::Task<td::Unit> {
    ...
    td::actor::send_closure(a.adnl, &tos::adnl::Adnl::add_peer, a.id,
                            tos::adnl::AdnlNodeIdFull{wrong_key.compute_public_key()}, wrong_addr_list);

    t.add_peer(b, a);  // B knows correct A

    // Try to send query - A thinks it's talking to wrong_key but B has different key
    auto wrong_id = tos::adnl::AdnlNodeIdShort{wrong_key.compute_public_key().compute_short_id()};
    ...
    td::actor::send_closure(a.quic_sender, &tos::quic::QuicSender::send_query, a.id, wrong_id, std::string("Q"),
...
TEST(QuicSender, TlsKeyMismatchDoesNotPoisonOutboundPath) {
  run_test([](TestRunner& t) -> td::actor::Task<td::Unit> {
    ...
    td::actor::send_closure(a.adnl, &tos::adnl::Adnl::add_peer, a.id,
                            tos::adnl::AdnlNodeIdFull{b.key.compute_public_key()}, wrong_addr_list);
    t.add_peer(b, a);
    t.add_peer(c, a);
    ...
    td::actor::send_closure(a.adnl, &tos::adnl::Adnl::add_peer, a.id,
                            tos::adnl::AdnlNodeIdFull{b.key.compute_public_key()}, correct_addr_list);
    co_await td::actor::ask(a.adnl, &tos::adnl::Adnl::get_peer_node, a.id, b.id);
...
TEST(QuicSender, LargeScale) {
  ...
  run_test([](TestRunner& t) -> td::actor::Task<td::Unit> {
    ...
    for (int i = 0; i < num_nodes; i++) {
      ...
      td::actor::send_closure(nodes[i].quic_sender, &tos::quic::QuicSender::set_default_mtu, 2 * query_size);
    }
    ...
    for (int dest = 0; dest < num_nodes; dest++) {
      ...
      for (int q = 0; q < queries_per_dest; q++) {
        ...
        auto [future, promise] = td::actor::StartedTask<td::BufferSlice>::make_bridge();
        td::actor::send_closure(nodes[sender_idx].quic_sender, &tos::quic::QuicSender::send_query, nodes[sender_idx].id,
...
TEST(QuicSender, ResponseSizeLimitDoesNotWaitForTimeout) {
  run_test([](TestRunner& t) -> td::actor::Task<td::Unit> {
    ...
    td::actor::send_closure(
        a.quic_sender, &tos::quic::QuicSender::send_query_ex, a.id, b.id, std::string("Q"),
...
TEST(QuicSender, ResponseSizeLimitByOneDoesNotWaitForTimeout) {
  run_test([](TestRunner& t) -> td::actor::Task<td::Unit> {
    ...
    td::actor::send_closure(a.quic_sender, &tos::quic::QuicSender::set_default_mtu, 1 << 20);
    td::actor::send_closure(b.quic_sender, &tos::quic::QuicSender::set_default_mtu, 1 << 20);
    ...
    auto wire_answer = tos::create_serialize_tl_object<tos::tos_api::quic_answer>(answer_data.clone());
...
TEST(QuicFairness, TwoConnectionsFairBandwidth) {
  run_test([](TestRunner& t) -> td::actor::Task<td::Unit> {
    ...
    td::actor::send_closure(hub.quic_sender, &tos::quic::QuicSender::set_default_mtu, 2 * query_size);

    // Create two sender nodes
    auto sender1 = co_await t.create_node("s1", next_port());
    auto sender2 = co_await t.create_node("s2", next_port());
    td::actor::send_closure(sender1.quic_sender, &tos::quic::QuicSender::set_default_mtu, 2 * query_size);
    td::actor::send_closure(sender2.quic_sender, &tos::quic::QuicSender::set_default_mtu, 2 * query_size);
    ...
    for (int i = 0; i < queries_per_sender; i++) {
      ...
      td::Random::secure_bytes(data.as_slice().substr(1));
      auto [future, promise] = td::actor::StartedTask<td::BufferSlice>::make_bridge();
      td::actor::send_closure(sender1.quic_sender, &tos::quic::QuicSender::send_query, sender1.id, hub.id,
    ...
    for (int i = 0; i < queries_per_sender; i++) {
      ...
      td::Random::secure_bytes(data.as_slice().substr(1));
      auto [future, promise] = td::actor::StartedTask<td::BufferSlice>::make_bridge();
      td::actor::send_closure(sender2.quic_sender, &tos::quic::QuicSender::send_query, sender2.id, hub.id,
                              std::string("Q"), std::move(promise), td::Timestamp::in(60.0), std::move(data));
...
TEST(QuicFairness, MultipleStreamsFairBandwidth) {
  run_test([](TestRunner& t) -> td::actor::Task<td::Unit> {
    ...
    td::actor::send_closure(server.quic_sender, &tos::quic::QuicSender::set_default_mtu, 2 * query_size);
    td::actor::send_closure(client.quic_sender, &tos::quic::QuicSender::set_default_mtu, 2 * query_size);
    ...
    for (int i = 0; i < num_streams; i++) {
      ...
      td::Random::secure_bytes(data.as_slice().substr(1));
      auto [future, promise] = td::actor::StartedTask<td::BufferSlice>::make_bridge();
      td::actor::send_closure(client.quic_sender, &tos::quic::QuicSender::send_query, client.id, server.id,
                              std::string("Q"), std::move(promise), td::Timestamp::in(30.0), std::move(data));
      tasks.push_back(std::move(future));
    }
...
TEST(QuicFairness, ManyConnectionsNoStarvation) {
  run_test([](TestRunner& t) -> td::actor::Task<td::Unit> {
    ...
    for (int i = 0; i < num_senders; i++) {
      ...
      td::actor::send_closure(senders.back().quic_sender, &tos::quic::QuicSender::set_default_mtu, 2 * query_size);
    ...
    for (int s = 0; s < num_senders; s++) {
      for (int q = 0; q < queries_per_sender; q++) {
        ...
        td::Random::secure_bytes(data.as_slice().substr(1));
        auto [future, promise] = td::actor::StartedTask<td::BufferSlice>::make_bridge();
        td::actor::send_closure(senders[s].quic_sender, &tos::quic::QuicSender::send_query, senders[s].id, hub.id,
                                std::string("Q"), std::move(promise), td::Timestamp::in(60.0), std::move(data));
...
TEST(QuicRateLimiter, BurstAndRefillSemantics) {
  tos::adnl::RateLimiter limiter(3, 1.0);
...
TEST(QuicRateLimiter, CapacityOneDoesNotAllowExtraBurst) {
  tos::adnl::RateLimiter limiter(1, 1.0);
