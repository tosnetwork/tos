...
class TestNode : public td::actor::Actor {
 private:
  ...
  tos::BlockIdExt shard_top_block_id_;
 ...
 public:
  ...
  void set_shard(tos::ShardIdFull shard) {
    LOG(DEBUG) << "setting shard to " << shard.to_str();
    shard_ = shard;
  }
  void set_shard_top_block(tos::BlockIdExt block_id) {
  ...
  void set_zero_file(std::string filename) {
    try {
      ...
      bs_ = res1.move_as_ok();
      auto res = tos::validator::create_shard_state(
          tos::BlockIdExt{tos::BlockId{shard_.workchain, tos::shardIdAll, 0}, zero_id_.root_hash, zero_id_.file_hash},
          bs_.clone());
      if (res.is_error()) {
        throw IntError{res.move_as_error()};
      }
      auto state = res.move_as_ok();
      tos::FileHash fhash = block::compute_file_hash(bs_.as_slice());
      tos::RootHash rhash = state->root_hash();
      CHECK(!fhash.is_zero());
      CHECK(!rhash.is_zero());
  ...
  td::Status create_validator_options() {
    if (!global_config_.length()) {
      ...
      opts_ = tos::validator::ValidatorManagerOptions::create(
    ...
    opts_ = tos::validator::ValidatorManagerOptions::create(zero_state, init_block);
  ...
  void run() {
    ...
    for (auto &msg : ext_msgs_) {
      td::actor::ask(validator_manager_, &tos::validator::ValidatorManager::new_external_message_broadcast,
                     std::move(msg), 0,
                     tos::validator::ExtMessageIngressSource{tos::validator::LocalOrigin{}})
          .detach();
    }
    for (auto &topmsg : top_shard_descrs_) {
      td::actor::send_closure(validator_manager_,
                              &tos::validator::ValidatorManager::new_shard_block_description_broadcast,
                              tos::BlockIdExt{}, 0, std::move(topmsg));
    }
    class Callback : public tos::validator::ValidatorManagerInterface::Callback {
     private:
      td::actor::ActorId<tos::validator::ValidatorManagerInterface> id_;
      bool tdescr_save_;
      std::string tdescr_pfx_;
      int tdescr_cnt_ = 0;

     public:
      Callback(td::actor::ActorId<tos::validator::ValidatorManagerInterface> id, bool tdescr_save = false,
               std::string tdescr_pfx = "")
          : id_(id), tdescr_save_(tdescr_save), tdescr_pfx_(tdescr_pfx) {
      }

      void initial_read_complete(tos::validator::BlockHandle handle) override {
        td::actor::send_closure(id_, &tos::validator::ValidatorManager::sync_complete,
                                td::PromiseCreator::lambda([](td::Result<>) {}));
      }
      void send_shard_block_info(tos::BlockIdExt block_id, tos::CatchainSeqno cc_seqno, td::BufferSlice data) override {
        ++tdescr_cnt_;
        if (!tdescr_save_) {
    ...
    td::actor::send_closure(validator_manager_, &tos::validator::ValidatorManagerInterface::install_callback,
...
int main(int argc, char *argv[]) {
  ...
  p.add_checked_option('w', "workchain", "<workchain>[:<shard>]\tcollate block in this workchain", [&](td::Slice arg) {
    ...
    while (++pos < arg.size()) {
      ...
      shard |= (tos::ShardId(x) << s);
      s -= 4;
    }
    td::actor::send_closure(x, &TestNode::set_shard, tos::ShardIdFull{workchain, shard ? shard : tos::shardIdAll});
    return td::Status::OK();
  });
