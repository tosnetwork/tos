...
int main() {
...

  {
    ...
    auto id = tos::adnl::AdnlNodeIdShort::parse(id_str).move_as_ok();
...

  {
    ...
    auto obj = tos::create_tl_object<tos::tos_api::adnl_proxy_fast>(proxy_id, td::BufferSlice{"1234"});
    auto R = tos::adnl::AdnlProxy::create(*obj.get());
    R.ensure();
    auto P = R.move_as_ok();
    td::BufferSlice z{64};
    td::Random::secure_bytes(z.as_slice());
    auto packet = P->encrypt(tos::adnl::AdnlProxy::Packet{15, 2, 3, 4, 5, 6, z.clone()});
    ...
    class Callback : public tos::adnl::Adnl::Callback {
     public:
      void receive_message(tos::adnl::AdnlNodeIdShort src, tos::adnl::AdnlNodeIdShort dst,
        ...
        CHECK(data.size() <= tos::adnl::Adnl::huge_packet_max_size());
        CHECK(src == src);
        CHECK(dst == dst);
        if (data.size() >= 5) {
  ...
  scheduler.run_in_context([&] {
    ...
    for (td::uint32 i : {1, 2, 3, 4, 100, 500, 900}) {
      remaining++;
      td::actor::send_closure(adnl, &tos::adnl::Adnl::send_message, src, dst, send_packet(i));
    }
    for (td::uint32 i = 1024; i <= tos::adnl::Adnl::huge_packet_max_size() /* 1024 * 8 */; i += 1024) {
      remaining++;
      td::actor::send_closure(adnl, &tos::adnl::Adnl::send_message, src, dst, send_packet(i));
    }
  });
  ...
  scheduler.run_in_context(
      [&] { td::actor::send_closure(adnl, &tos::adnl::Adnl::send_message, dst, src, send_packet(1)); });
  ...
  f = td::Clocks::system();
  scheduler.run_in_context([&] {
    for (td::uint32 i = 1; i <= tos::adnl::Adnl::huge_packet_max_size(); i++) {
      remaining++;
      td::actor::send_closure(adnl, &tos::adnl::Adnl::send_message, src, dst, send_packet(i));
    }
  });
  ...
  scheduler.run_in_context([&] {
    ...
    td::actor::send_closure(adnl, &tos::adnl::Adnl::subscribe, dst, "2", std::make_unique<Callback>());
  });
  ...
  for (td::uint32 i = 1; i <= tos::adnl::Adnl::huge_packet_max_size(); i++) {
    ...
    scheduler.run_in_context([&] {
      td::actor::send_closure(adnl, &tos::adnl::Adnl::send_query, src, dst, PSTRING() << "query" << i, std::move(P),
  ...
  // too big answer
  scheduler.run_in_context([&] {
    auto x = tos::adnl::Adnl::huge_packet_max_size() + 1;
    td::BufferSlice d{5};
    d.as_slice()[0] = '2';
    ...
    td::actor::send_closure(adnl, &tos::adnl::Adnl::send_query, src, dst, PSTRING() << "query" << x, std::move(P),
                            td::Timestamp::in(320.0), std::move(d));
  });
  // too big packet
  scheduler.run_in_context([&] {
    auto x = tos::adnl::Adnl::huge_packet_max_size() + 1;
    td::actor::send_closure(adnl, &tos::adnl::Adnl::send_message, src, dst, send_packet(x));
  });
  // no callbacks
  scheduler.run_in_context([&] {
    td::BufferSlice d{1};
    d.as_slice()[0] = '3';
    td::actor::send_closure(adnl, &tos::adnl::Adnl::send_message, src, dst, std::move(d));
  });
  // no callbacks 2
  scheduler.run_in_context([&] {
    td::BufferSlice d{};
  ...
  if (true) {
    auto a = tos::adnl::Adnl::adnl_start_time();
    std::this_thread::sleep_for(std::chrono::milliseconds(10000));
    CHECK(a == tos::adnl::Adnl::adnl_start_time());
  }
