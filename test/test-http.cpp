...
int main(int argc, char *argv[]) {
...

  {
    ...
    std::string cur_line = "";
    auto R = tos::http::HttpRequest::parse(nullptr, cur_line, exit_loop, r);
    R.ensure();
...

  {
    ...
    std::unique_ptr<tos::http::HttpRequest> req;
    ...
    for (size_t i = 0; i < l; i++) {
      ...
      if (!req || !req->check_parse_header_completed()) {
        req = tos::http::HttpRequest::parse(std::move(req), cur_line, exit_loop, r).move_as_ok();
      } else {
        if (!payload) {
...

  {
    ...
    bool exit_loop = false;
    std::string cur_line = "";
    auto R = tos::http::HttpResponse::parse(nullptr, cur_line, false, false, exit_loop, r);
    R.ensure();
