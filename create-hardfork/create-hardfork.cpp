...
class HardforkCreator : public td::actor::Actor {
 private:
  ...
  tos::ShardIdFull shard_{tos::masterchainId};
 ...
 public:
  ...
  void run() {
    ...
    validator_manager_ =
        tos::validator::ValidatorManagerHardforkFactory::create(opts, shard_, shard_top_block_id_, db_root_);
...
int main(int argc, char *argv[]) {
  ...
  p.add_checked_option('w', "workchain", "<workchain>[:<shard>]\tcollate block in this workchain", [&](td::Slice arg) {
    ...
    td::actor::send_closure(x, &HardforkCreator::set_shard,
                            tos::ShardIdFull{workchain, shard ? shard : tos::shardIdAll});
    return td::Status::OK();
  });
