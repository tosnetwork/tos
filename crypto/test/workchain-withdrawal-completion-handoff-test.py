...
path = Path(__file__).with_name('workchain-withdrawal-completion-handoff.py')
...
class Readiness(unittest.TestCase):
    ...
    def test_each_bucket_obligation_is_independently_required(self):
        expected = ('row6', 'row4', 'row5', 'bucket-small', 'bucket-full',
                    'bucket-closed', 'sweep-atomic', 'oracle-control')
        for suffix in expected:
            name = 'test-workchain-withdrawal-completion-' + suffix
