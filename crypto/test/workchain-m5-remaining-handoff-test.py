...
path = Path(__file__).with_name('workchain-m5-remaining-handoff.py')
...
class Readiness(unittest.TestCase):
    ...
    def test_each_bucket_obligation_is_independently_required(self):
        expected = ('phase-transition', 'paid-lazy', 'late-return', 'validator-unknown', 'source-classification', 'integrated-regression', 'activation-freeze')
        for suffix in expected:
            name = 'test-workchain-m5-remaining-' + suffix
