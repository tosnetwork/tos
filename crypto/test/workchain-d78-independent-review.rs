// D78 version of the pinned review; pre-D78 source remains unchanged.
...
fn amounts()->WithdrawalAmounts { WithdrawalAmounts {
    principal:100, outward_fee:17, operation_fee:11 } }
#[test]
...
fn independent_d64_points_total_and_fee_separate() {
    ...
    assert_eq!(a.total(),Ok(117));assert_eq!(s.fee(),11);
    assert_eq!(s.points()[0],q[0]);assert_eq!(s.points()[1],q[0]);
    assert_eq!(s.points()[7],s.points()[8]);
    let r=crate::withdrawal_statement::public_opening(&[1;80],&[2;32],&[3;32],&q[0],117).unwrap();
    let pc=PedersenGens::default();
    assert_eq!(s.points()[6],(Scalar::from(117u64)*pc.B+r*pc.B_blinding).compress().to_bytes());
    ...
    for a in [WithdrawalAmounts{principal:u64::MAX,outward_fee:1,operation_fee:0},
        WithdrawalAmounts{principal:u64::MAX-1,outward_fee:2,operation_fee:0}] {
