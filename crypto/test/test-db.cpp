...
TEST(TosDb, BocDeserializeTruncated) {
  td::Random::Xorshift128plus rnd{123};
...
TEST(TosDb, InMemoryDynamicBocSimple) {
...
TEST(TosDb, BocDeserializerBaseline) {
  test_boc_deserializer<StaticBagOfCellsDbBaseline>();
}

TEST(TosDb, BocDeserializerSimple) {
...
TEST(TosDb, BocDeserializerSimpleThreads) {
...
TEST(TosDb, BenchDynamicBocGetAndSet) {
  with_all_boc_options(bench_dboc_get_and_set, 1);
}

TEST(TosDb, DynamicBocIncSimple) {
    ...
    if (config.blob_type == BenchBocDeserializerConfig::RocksDb) {
      {
        ...
        auto db = vm::TosDbImpl::open(td::Slice(db_path)).move_as_ok();
      ...
      db_ = vm::TosDbImpl::open(td::Slice(db_path)).move_as_ok();
    } else {
...
TEST(TosDb, BenchBocSerializerImport) {
  if (0) {
...
TEST(TosDb, BenchBocSerializerSerialize) {
...
TEST(TosDb, BenchBocThreadsDeserializerSimple) {
...
TEST(TosDb, BenchBocDeserializerSimple) {
  //td::bench_n(BenchBocDeserializer<vm::StaticBagOfCellsDbLazy>("simple", BenchBocDeserializerConfig()), 1000000);
  //std::exit(0);
  bench_deserializer<vm::StaticBagOfCellsDbLazy>("simple", false);
}
TEST(TosDb, BenchBocDeserializerBaseline) {
...
TEST(TosDb, BenchBocDeserializerRocksDb) {
  //td::bench_n(BenchBocDeserializer<vm::StaticBagOfCellsDbBaseline>("baseline", BenchBocDeserializerConfig()), 1000000);
  //std::exit(0);
...
TEST(TosDb, CompactArray) {
  ...
  auto db = vm::TosDbImpl::open(db_path).move_as_ok();
  auto txn = db->begin_transaction();
  auto smt = txn->begin_smartcontract();
  SCOPE_EXIT {
    db->commit_transaction(std::move(txn));
  ...
  auto flush_to_db = [&] {
    ...
    if (restart_db) {
      db->clear_cache();
      //db.reset();
      //db = vm::TosDbImpl::open(db_path).move_as_ok();
    }
...
TEST(TosDb, CompactArrayOld) {
  SET_VERBOSITY_LEVEL(VERBOSITY_NAME(ERROR));
  using namespace vm;
  //auto kv = std::make_unique<MemoryKeyValue>();
  td::RocksDb::destroy("ttt").ensure();

  auto tos_db = vm::TosDbImpl::open("ttt").move_as_ok();
  ...
  {
    auto txn = tos_db->begin_transaction();
    SCOPE_EXIT {
      tos_db->commit_transaction(std::move(txn));
    };
  ...
  for (int i = 0; i < 100; i++) {
    if (i % 10 == 9) {
      //LOG(ERROR) << tos_db->stat();
      tos_db.reset();
      tos_db = vm::TosDbImpl::open("ttt").move_as_ok();
    }
    auto txn = tos_db->begin_transaction();
    SCOPE_EXIT {
      tos_db->commit_transaction(std::move(txn));
  ...
  }
  {
    auto txn = tos_db->begin_transaction();
    SCOPE_EXIT {
      tos_db->abort_transaction(std::move(txn));
    };
    auto smart = txn->begin_smartcontract();
    SCOPE_EXIT {
...
TEST(TosDb, BocRespectsUsageCell) {
  td::Random::Xorshift128plus rnd(123);
...
TEST(TosDb, DoNotMakeListsPrunned) {
