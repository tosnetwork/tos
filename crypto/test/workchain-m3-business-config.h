...
struct M3TestBusinessParameters {
  ...
  // Version 4: explicit test-profile inputs, not frozen initial values. Neither
...
inline td::Result<td::Ref<vm::Cell>> encode_m3_test_business_parameters(const M3TestBusinessParameters& value) {
  ...
  if (!root.store_long_bool(tag, 32) || !root.store_long_bool(value.failed ? 4 : value.operation_tariff ? 3 : value.deposit ? 2 : version, 16) ||
  ...
  if (value.failed && (!root.store_long_bool(value.failed->withdrawal_limit, 32) ||
      !root.store_long_bool(value.failed->issuance_billing_units, 64))) return malformed();
...
inline td::Result<M3TestBusinessParameters> decode_m3_test_business_parameters(const td::Ref<vm::Cell>& cell) {
  ...
  if (special || root.size_refs() != 4 || (root.size() != 256 && root.size() != 448 && root.size() != 640 && root.size() != 736)) return malformed();
  if (root.fetch_ulong(32) != tag) return td::Status::Error("unknown M3 test business tag");
  auto wire_version = root.fetch_ulong(16);
  if (wire_version != version && wire_version != 2 && wire_version != 3 && wire_version != 4)
    return td::Status::Error("unsupported M3 test business version");
  if (root.size() != (wire_version == 4 ? 688u : wire_version == 3 ? 592u : wire_version == 2 ? 400u : 208u)) return malformed();
