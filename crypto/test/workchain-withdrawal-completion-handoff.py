#!/usr/bin/env python3
"""WITHDRAWAL-COMPLETION_D78 readiness runner, not a substitute for host evidence review.
...
TESTS = tuple('test-workchain-withdrawal-completion-' + suffix for suffix in (
    'row6', 'row4', 'row5', 'bucket-small', 'bucket-full', 'bucket-closed',
    'sweep-atomic', 'oracle-control',
))
MARKER = 'WITHDRAWAL-COMPLETION_D78_OBSERVED:'
...
def check(build):
    ...
    if missing or disabled or duplicated:
        print('WITHDRAWAL-COMPLETION_D78 HANDOFF_NOT_READY: missing=' + repr(missing)
    ...
    for name in TESTS:
        ...
        if (MARKER + name not in result.stdout
            ...
            print('WITHDRAWAL-COMPLETION_D78 HANDOFF_NOT_READY: missing observation or unexecuted test: ' + name)
            return 1
    print('WITHDRAWAL-COMPLETION_D78_TESTS_EXECUTED: independent observation/source review still required')
    return 0
...
def main():
    ...
    try:
        ...
        print('WITHDRAWAL-COMPLETION_D78 HANDOFF_NOT_READY: tool/test error: ' + str(error), file=sys.stderr)
        return 1
