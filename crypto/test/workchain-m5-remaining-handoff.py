#!/usr/bin/env python3
"""M5-REMAINING readiness runner, not a substitute for host evidence review.
...
TESTS = tuple('test-workchain-m5-remaining-' + suffix for suffix in (
    'phase-transition', 'paid-lazy', 'late-return', 'validator-unknown', 'source-classification', 'integrated-regression', 'activation-freeze',
))
MARKER = 'M5-REMAINING_OBSERVED:'
...
def check(build):
    ...
    if missing or disabled or duplicated:
        print('M5-REMAINING HANDOFF_NOT_READY: missing=' + repr(missing)
    ...
    for name in TESTS:
        ...
        if (MARKER + name not in result.stdout
            ...
            print('M5-REMAINING HANDOFF_NOT_READY: missing observation or unexecuted test: ' + name)
            return 1
    print('M5-REMAINING_TESTS_EXECUTED: independent observation/source review still required')
    return 0
...
def main():
    ...
    try:
        ...
        print('M5-REMAINING HANDOFF_NOT_READY: tool/test error: ' + str(error), file=sys.stderr)
        return 1
