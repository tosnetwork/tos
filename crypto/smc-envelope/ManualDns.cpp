...
td::Ref<ManualDns> ManualDns::create(td::Ref<vm::Cell> data, int revision) {
  return td::Ref<ManualDns>(
      true, State{tos::SmartContractCode::get_code(tos::SmartContractCode::ManualDns, revision), std::move(data)});
}
...
td::optional<td::int32> ManualDns::guess_revision(const vm::Cell::Hash& code_hash) {
  for (auto i : tos::SmartContractCode::get_revisions(tos::SmartContractCode::ManualDns)) {
    if (tos::SmartContractCode::get_code(tos::SmartContractCode::ManualDns, i)->get_hash() == code_hash) {
      return i;
    }
  }
  return {};
...
std::string ManualDns::serialize_data(const EntryData& data) {
  std::string res;
  data.data.visit(
      td::overloaded([&](const tos::ManualDns::EntryDataText& text) { res = "UNSUPPORTED"; },
                     [&](const tos::ManualDns::EntryDataNextResolver& resolver) { res = "UNSUPPORTED"; },
                     [&](const tos::ManualDns::EntryDataAdnlAddress& adnl_address) { res = "UNSUPPORTED"; },
                     [&](const tos::ManualDns::EntryDataSmcAddress& text) { res = "UNSUPPORTED"; },
                     [&](const tos::ManualDns::EntryDataStorageAddress& storage_address) { res = "UNSUPPORTED"; }));
  return res;
}
