...
td::Ref<vm::Cell> Wallet::get_init_message_new(const td::Ed25519::PrivateKey& private_key) noexcept {
  ...
  auto r_signature = private_key.sign(
      vm::CellBuilder().store_long(global_id_, 32).store_long(seqno, 32).store_long(valid_until, 32).finalize()->get_hash().as_slice());
...
td::Ref<vm::Cell> Wallet::get_init_code(td::int32 revision) noexcept {
  return SmartContractCode::get_code(tos::SmartContractCode::WalletV2, revision);
}
