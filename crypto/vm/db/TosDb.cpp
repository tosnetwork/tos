...
//
// TosDbTransactionImpl
//
SmartContractDb TosDbTransactionImpl::begin_smartcontract(td::Slice hash) {
  SmartContractDb res;
...
void TosDbTransactionImpl::commit_smartcontract(SmartContractDb txn) {
  commit_smartcontract(SmartContractDiff(std::move(txn)));
}
void TosDbTransactionImpl::commit_smartcontract(SmartContractDiff txn) {
  {
...
void TosDbTransactionImpl::abort_smartcontract(SmartContractDb txn) {
  end_smartcontract(std::move(txn));
}
void TosDbTransactionImpl::abort_smartcontract(SmartContractDiff txn) {
  end_smartcontract(txn.extract_smartcontract());
}

TosDbTransactionImpl::TosDbTransactionImpl(std::shared_ptr<KeyValue> kv) : kv_(std::move(kv)) {
  CHECK(kv_ != nullptr);
  reader_.reset(kv_->snapshot().release());
}

void TosDbTransactionImpl::begin() {
  kv_->begin_write_batch();
  generation_++;
}
void TosDbTransactionImpl::commit() {
  kv_->commit_write_batch();
  reader_.reset(kv_->snapshot().release());
}
void TosDbTransactionImpl::abort() {
  kv_->abort_write_batch();
}
void TosDbTransactionImpl::clear_cache() {
  contracts_ = {};
}

void TosDbTransactionImpl::end_smartcontract(SmartContractDb smart_contract) {
...
// TosDbImpl
//
TosDbImpl::TosDbImpl(std::unique_ptr<KeyValue> kv)
    : kv_(std::move(kv)), transaction_(std::make_unique<TosDbTransactionImpl>(kv_)) {
}
TosDbImpl::~TosDbImpl() {
  CHECK(transaction_);
  kv_->flush();
}
TosDbTransaction TosDbImpl::begin_transaction() {
  CHECK(transaction_);
  transaction_->begin();
  return std::move(transaction_);
}
void TosDbImpl::commit_transaction(TosDbTransaction transaction) {
  CHECK(!transaction_);
  CHECK(&transaction->kv() == kv_.get());
  transaction_ = std::move(transaction);
  transaction_->commit();
}
void TosDbImpl::abort_transaction(TosDbTransaction transaction) {
  CHECK(!transaction_);
  CHECK(&transaction->kv() == kv_.get());
  transaction_ = std::move(transaction);
  transaction_->abort();
}
void TosDbImpl::clear_cache() {
  CHECK(transaction_);
  transaction_->clear_cache();
}

std::string TosDbImpl::stats() const {
  return kv_->stats();
}

td::Result<TosDb> TosDbImpl::open(td::Slice path) {
#if TDDB_USE_ROCKSDB
  TRY_RESULT(rocksdb, td::RocksDb::open(path.str()));
  return std::make_unique<TosDbImpl>(std::make_unique<td::RocksDb>(std::move(rocksdb)));
#else
  return td::Status::Error("TosDb is not supported in this build");
#endif
}
