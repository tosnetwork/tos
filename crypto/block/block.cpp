...
bool StdAddress::parse_addr(td::Slice acc_string) {
  ...
  if (pos != std::string::npos) {
    ...
    auto r_wc = td::to_integer_safe<tos::WorkchainId>(tmp);
    if (r_wc.is_error()) {
      return invalidate();
    }
...
bool parse_std_account_addr(td::Slice acc_string, tos::WorkchainId& wc, tos::StdSmcAddress& addr, bool* bounceable,
                            bool* testnet_only) {
  StdAddress a;
...
ShardId::ShardId(tos::WorkchainId wc_id, unsigned long long sh_pfx) : workchain_id(wc_id), shard_pfx(sh_pfx) {
  init();
}
...
ShardId::ShardId(tos::WorkchainId wc_id, unsigned long long sh_pfx, int sh_pfx_len)
...
void ShardId::show(std::ostream& os) const {
  if (workchain_id == tos::workchainInvalid) {
    os << '?';
    return;
  }
...
bool ShardId::serialize(vm::CellBuilder& cb) const {
  if (workchain_id == tos::workchainInvalid || cb.remaining_bits() < 104) {
    return false;
  }
...
bool ShardId::deserialize(vm::CellSlice& cs) {
  if (cs.fetch_ulong(2) == 0 && cs.fetch_uint_to(6, shard_pfx_len) && cs.fetch_int_to(32, workchain_id) &&
      workchain_id != tos::workchainInvalid && cs.fetch_uint_to(64, shard_pfx)) {
...
MsgProcessedUptoCollection::MsgProcessedUptoCollection(tos::ShardIdFull _owner, Ref<vm::CellSlice> cs_ref)
    : owner(_owner) {
  ...
  valid = dict.check_for_each([&](Ref<vm::CellSlice> value, td::ConstBitPtr key, int n) -> bool {
    ...
    return value.write().fetch_bits_to(z.last_inmsg_hash) && z.shard && tos::shard_contains(owner.shard, z.shard);
  });
}

std::unique_ptr<MsgProcessedUptoCollection> MsgProcessedUptoCollection::unpack(tos::ShardIdFull _owner,
...
bool MsgProcessedUpto::contains(const MsgProcessedUpto& other) const& {
  return tos::shard_is_ancestor(shard, other.shard) && mc_seqno >= other.mc_seqno &&
         (last_inmsg_lt > other.last_inmsg_lt ||
          (last_inmsg_lt == other.last_inmsg_lt && !(last_inmsg_hash < other.last_inmsg_hash)));
}

bool MsgProcessedUpto::contains(tos::ShardId other_shard, tos::LogicalTime other_lt, td::ConstBitPtr other_hash,
                                tos::BlockSeqno other_mc_seqno) const& {
  return tos::shard_is_ancestor(shard, other_shard) && mc_seqno >= other_mc_seqno &&
         (last_inmsg_lt > other_lt || (last_inmsg_lt == other_lt && !(last_inmsg_hash < other_hash)));
}

bool MsgProcessedUptoCollection::insert(tos::BlockSeqno mc_seqno, tos::LogicalTime last_proc_lt,
...
bool MsgProcessedUptoCollection::insert_infty(tos::BlockSeqno mc_seqno, tos::LogicalTime last_proc_lt) {
...
tos::BlockSeqno MsgProcessedUptoCollection::min_mc_seqno() const {
  tos::BlockSeqno min_mc_seqno = ~0U;
  for (const auto& z : list) {
...
bool MsgProcessedUptoCollection::split(tos::ShardIdFull new_owner) {
  if (!tos::shard_is_ancestor(owner, new_owner)) {
    return false;
  }
  if (owner == new_owner) {
    return true;
  ...
  for (i = 0; i < n; i++) {
    if (tos::shard_intersects(list[i].shard, new_owner.shard)) {
      list[i].shard = tos::shard_intersection(list[i].shard, new_owner.shard);
      if (j < i) {
...
bool MsgProcessedUptoCollection::combine_with(const MsgProcessedUptoCollection& other) {
  if (!(other.owner == owner || tos::shard_is_sibling(other.owner, owner))) {
    return false;
  }
  list.insert(list.end(), other.list.begin(), other.list.end());
  if (owner != other.owner) {
    owner = tos::shard_parent(owner);
  }
  return compactify();
}
...
bool MsgProcessedUpto::already_processed(const EnqueuedMsgDescr& msg) const {
  ...
  if (!tos::shard_contains(shard, msg.next_prefix_.account_id_prefix)) {
    return false;
  }
  if (msg.lt_ == last_inmsg_lt && last_inmsg_hash < msg.hash_) {
    return false;
  }
  if (msg.same_workchain() && tos::shard_contains(shard, msg.cur_prefix_.account_id_prefix)) {
...
bool MsgProcessedUptoCollection::already_processed(const EnqueuedMsgDescr& msg) const {
  ...
  if (!tos::shard_contains(owner, msg.next_prefix_)) {
    return false;
  }
...
bool MsgProcessedUptoCollection::for_each_mcseqno(std::function<bool(tos::BlockSeqno)> func) const {
  for (const auto& entry : list) {
...
std::ostream& MsgProcessedUpto::print(std::ostream& os) const {
  return os << "[" << tos::shard_to_str(shard) << "," << mc_seqno << "," << last_inmsg_lt << ","
            << last_inmsg_hash.to_hex() << "]";
}
...
int BlockLimits::classify_lt(tos::LogicalTime lt) const {
...
int BlockLimits::classify(td::uint64 size, td::uint64 gas, tos::LogicalTime lt, td::uint64 collated_size) const {
  return std::max(
      {classify_size(size), classify_gas(gas), classify_lt(lt), classify_collated_data_size(collated_size)});
}

bool BlockLimits::fits(unsigned cls, td::uint64 size, td::uint64 gas_value, tos::LogicalTime lt,
...
bool BlockLimitStatus::would_fit(unsigned cls, tos::LogicalTime end_lt, td::uint64 more_gas,
...
// SETS: utime_, lt_
td::Status ShardState::unpack_state(tos::BlockIdExt blkid, Ref<vm::Cell> prev_state_root) {
  if (!blkid.is_valid()) {
  ...
  auto shard1 = tos::ShardIdFull(block::ShardId{state.shard_id});
...
td::Status ShardState::unpack_out_msg_queue_info(Ref<vm::Cell> out_msg_queue_info) {
  ...
  processed_upto_ = block::MsgProcessedUptoCollection::unpack(tos::ShardIdFull(id_), std::move(qinfo.proc_info));
...
bool ShardState::update_prev_utime_lt(tos::UnixTime& prev_utime, tos::LogicalTime& prev_lt) const {
  prev_utime = std::max<tos::UnixTime>(prev_utime, utime_);
  prev_lt = std::max<tos::LogicalTime>(prev_lt, lt_);
  return true;
}
...
td::Status ShardState::check_mc_blk_seqno(tos::BlockSeqno last_mc_block_seqno) const {
...
td::Status ShardState::unpack_state_ext(tos::BlockIdExt id, Ref<vm::Cell> state_root, int global_id,
                                        tos::BlockSeqno prev_mc_block_seqno, bool after_split, bool clear_history,
                                        std::function<bool(tos::BlockSeqno)> for_each_mcseqno_func) {
...
td::Status ShardState::merge_with(ShardState& sib) {
  ...
  if (!tos::shard_is_sibling(id_.shard_full(), sib.id_.shard_full())) {
    return td::Status::Error(-666, "cannot merge non-sibling states of "s + id_.to_str() + " and " + sib.id_.to_str());
  }
  tos::ShardIdFull shard = tos::shard_parent(id_.shard_full());
...
td::Result<std::unique_ptr<vm::AugmentedDictionary>> ShardState::compute_split_out_msg_queue(
  ...
  if (!tos::shard_is_parent(shard, subshard)) {
...
td::Result<std::shared_ptr<block::MsgProcessedUptoCollection>> ShardState::compute_split_processed_upto(
    tos::ShardIdFull subshard) {
  if (!tos::shard_is_parent(id_.shard_full(), subshard)) {
    return td::Status::Error(-666, "cannot split subshard "s + subshard.to_str() + " from state of " + id_.to_str() +
                                       " because it is not a parent");
  }
...
td::Status ShardState::split(tos::ShardIdFull subshard) {
  if (!tos::shard_is_parent(id_.shard_full(), subshard)) {
    return td::Status::Error(-666, "cannot split subshard "s + subshard.to_str() + " from state of " + id_.to_str() +
                                       " because it is not a parent");
  }
  ...
  auto shard1 = id_.shard_full();
  CHECK(tos::shard_is_parent(shard1, subshard));
  CHECK(out_msg_queue_);
...
int filter_out_msg_queue(vm::AugmentedDictionary& out_queue, tos::ShardIdFull old_shard, tos::ShardIdFull subshard,
  ...
  return out_queue.filter([=](vm::CellSlice& cs, td::ConstBitPtr key, int key_len) -> int {
    ...
    if (!tos::shard_contains(old_shard, cur_prefix)) {
      LOG(ERROR) << "OutMsgQueue message with key " << key.to_hex(key_len)
                 << " does not contain current address belonging to shard " << old_shard.to_str();
      return -1;
    }
    bool res = tos::shard_contains(subshard, cur_prefix);
    if (res && queue_size) {
      ++*queue_size;
    }
    return res;
...
bool DiscountedCounter::increase_by(unsigned count, tos::UnixTime now) {
  if (!validate()) {
    return false;
  }
...
tos::AccountIdPrefixFull interpolate_addr(const tos::AccountIdPrefixFull& src, const tos::AccountIdPrefixFull& dest,
  ...
  if (d <= 0) {
    ...
    return tos::AccountIdPrefixFull{dest.workchain, (dest.account_id_prefix & ~mask) | (src.account_id_prefix & mask)};
  } else {
    int mask = (int)(~0U >> d);
    return tos::AccountIdPrefixFull{(dest.workchain & ~mask) | (src.workchain & mask), src.account_id_prefix};
  }
}

bool interpolate_addr_to(const tos::AccountIdPrefixFull& src, const tos::AccountIdPrefixFull& dest, int d,
...
std::pair<int, int> perform_hypercube_routing(tos::AccountIdPrefixFull src, tos::AccountIdPrefixFull dest,
                                              tos::ShardIdFull cur, int used_dest_bits) {
  tos::AccountIdPrefixFull transit = interpolate_addr(src, dest, used_dest_bits);
  if (!tos::shard_contains(cur, transit)) {
    return {-1, -1};
  }
  if (tos::shard_contains(cur, dest)) {
    // if destination is in this shard, set cur:=next_hop:=dest
    return {96, 96};
  }
  if (transit.workchain == tos::masterchainId || dest.workchain == tos::masterchainId) {
...
bool unpack_block_prev_blk(Ref<vm::Cell> block_root, const tos::BlockIdExt& id, std::vector<tos::BlockIdExt>& prev,
                           tos::BlockIdExt& mc_blkid, bool& after_split, tos::BlockIdExt* fetch_blkid) {
...
td::Status unpack_block_prev_blk_ext(Ref<vm::Cell> block_root, const tos::BlockIdExt& id,
  ...
  if (fetch_blkid) {
    fetch_blkid->id = tos::BlockId{shard, (unsigned)info.seq_no};
    fetch_blkid->root_hash = block_root->get_hash().bits();
    fetch_blkid->file_hash.clear();
  } else {
    tos::BlockId hdr_id{shard, (unsigned)info.seq_no};
    if (id.id != hdr_id) {
  ...
  prev.clear();
  tos::BlockSeqno prev_seqno = prev1.seq_no;
  if (!info.after_merge) {
    prev.emplace_back(shard.workchain, info.after_split ? tos::shard_parent(shard.shard) : shard.shard, prev1.seq_no,
    ...
    prev.emplace_back(shard.workchain, tos::shard_child(shard.shard, true), prev1.seq_no, prev1.root_hash,
                      prev1.file_hash);
    prev.emplace_back(shard.workchain, tos::shard_child(shard.shard, false), prev2.seq_no, prev2.root_hash,
                      prev2.file_hash);
  ...
  if (shard.is_masterchain()) {
    ...
    mc_blkid = tos::BlockIdExt{tos::masterchainId, tos::shardIdAll, mcref.seq_no, mcref.root_hash, mcref.file_hash};
  }
...
td::Status check_block_header(Ref<vm::Cell> block_root, const tos::BlockIdExt& id, tos::Bits256* store_shard_hash_to) {
  block::gen::Block::Record blk;
  block::gen::BlockInfo::Record info;
  tos::ShardIdFull shard;
  if (!(tlb::unpack_cell(block_root, blk) && tlb::unpack_cell(blk.info, info) && !info.version &&
...
bool is_transaction_in_msg(Ref<vm::Cell> trans_ref, Ref<vm::Cell> msg) {
  ...
  if (block::tlb::t_TransactionDescr.get_tag(vm::load_cell_slice(transaction.description)) ==
    ...
    if (!tlb::unpack_cell(transaction.description, description)) {
      return false;
    }
    auto inbox = description.inbound_messages->prefetch_ref();
    return inbox.is_null() ? msg.is_null() : workchain_batch_inbound_contains(inbox, msg);
  }
...
bool parse_block_id_ext(const char* str, const char* end, tos::BlockIdExt& blkid) {
  ...
  if (!shard || wc == tos::workchainInvalid) {
    return false;
  }
  str += pos;
  ...
  blkid.id = tos::BlockId{wc, shard, seqno};
...
Ref<vm::CellSlice> get_dispatch_queue_min_lt_account(const vm::AugmentedDictionary& dispatch_queue,
  ...
  tos::LogicalTime min_lt = root_extra->prefetch_long(64);
  while (true) {
    td::Bits256 key;
    ...
    tos::LogicalTime cut_min_lt = root_extra->prefetch_long(64);
