...
struct OutputQueueMerger {
  struct MsgKeyValue {
   ...
   protected:
    ...
    static tos::LogicalTime get_node_lt(Ref<vm::Cell> node, int key_pfx_len);
 ...
 public:
  struct Neighbor {
    ...
    Neighbor() = default;
    Neighbor(tos::BlockIdExt block_id, td::Ref<vm::Cell> outmsg_root, bool disabled = false, td::int32 msg_limit = -1)
        : block_id_(block_id), outmsg_root_(std::move(outmsg_root)), disabled_(disabled), msg_limit_(msg_limit) {
    }
  };

  OutputQueueMerger(tos::ShardIdFull queue_for, std::vector<Neighbor> neighbors);
  bool is_eof() const {
    return eof;
  }
  MsgKeyValue* cur();
