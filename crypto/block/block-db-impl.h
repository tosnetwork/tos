...
class BlockDbImpl final : public BlockDb {
  ...
  BinlogBuffer bb;
  tos::Bits256 zstate_rhash, zstate_fhash;
  unsigned created_at;
  std::map<tos::FileHash, td::BufferSlice> file_cache;
  std::map<tos::BlockId, Ref<FileInfo>> block_info;
  std::map<tos::BlockId, Ref<FileInfo>> state_info;
  //
 ...
 private:
  ...
  void get_top_block_id(tos::ShardIdFull shard, int authority, td::Promise<tos::BlockIdExt> promise) override;
  void get_top_block_state_id(tos::ShardIdFull shard, int authority, td::Promise<tos::BlockIdExt> promise) override;
  void get_block_by_id(tos::BlockId blk_id, bool need_data, td::Promise<td::Ref<FileInfo>> promise) override;
  void get_state_by_id(tos::BlockId blk_id, bool need_data, td::Promise<td::Ref<FileInfo>> promise) override;
  void get_out_queue_info_by_id(tos::BlockId blk_id, td::Promise<td::Ref<OutputQueueInfoDescr>> promise) override;
  ...
  void get_object_by_root_hash(RootHash root_hash, bool need_data, bool force_file_load,
                               td::Promise<td::Ref<FileInfo>> promise) override;
  void save_new_block(tos::BlockIdExt blk_id, td::BufferSlice data, int authority,
                      td::Promise<td::Unit> promise) override;
  void save_new_state(tos::BlockIdExt state_id, td::BufferSlice data, int authority,
                      td::Promise<td::Unit> promise) override;
