...
td::Result<std::unique_ptr<ConfigInfo>> ConfigInfo::extract_config(Ref<vm::Cell> mc_state_root,
                                                                   tos::BlockIdExt mc_block_id, int mode) {
...
td::Status ConfigInfo::unpack() {
  ...
  block_id.id = tos::BlockId{tos::ShardIdFull(shard_id), (unsigned)root_info.seq_no};
  block_id.root_hash.set_zero();
  block_id.file_hash.set_zero();
  ...
  if (extra_info.r1.last_key_block->size() > 1) {
    ...
    last_key_block_.id = tos::BlockId{tos::masterchainId, tos::shardIdAll, ext_ref.seq_no};
  ...
  zerostate_id_.workchain = tos::masterchainId;
...
tos::ValidatorSessionConfig Config::get_consensus_config() const {
  ...
  if (c.proto_version >= tos::ValidatorSessionConfig::BLOCK_HASH_COVERS_DATA_FROM_VERSION) {
...
td::optional<tos::NewConsensusConfig> Config::get_new_consensus_config(tos::WorkchainId wc) const {
  ...
  auto c2 = (wc == tos::masterchainId ? rec.mc : rec.shard)->prefetch_ref();
  if (c2.is_null()) {
    return {};
  }
  ...
  if (gen::NewConsensusConfig::Record_simplex_config v1; gen::unpack_cell(c2, v1)) {
    ...
    using NoncriticalParams = tos::NewConsensusConfig::NoncriticalParams;
...
bool ConfigInfo::get_last_key_block(tos::BlockIdExt& blkid, tos::LogicalTime& blklt, bool strict) const {
...
td::Result<std::pair<WorkchainSet, std::unique_ptr<vm::Dictionary>>> Config::unpack_workchain_list_ext(
  ...
  if (root.is_null()) {
    ...
    if (!(wc_dict->check_for_each([&wc_list](Ref<vm::CellSlice> cs_ref, td::ConstBitPtr key, int n) -> bool {
          tos::WorkchainId wc = tos::WorkchainId(key.get_int(32));
...
bool Config::set_block_id_ext(const tos::BlockIdExt& block_id_ext) {
...
bool ConfigInfo::set_block_id_ext(const tos::BlockIdExt& block_id_ext) {
...
td::Result<tos::StdSmcAddress> Config::get_dns_root_addr() const {
...
void McShardHash::set_fsm(FsmState fsm, tos::UnixTime fsm_utime, tos::UnixTime fsm_interval) {
  fsm_ = fsm;
  fsm_utime_ = fsm_utime;
  fsm_interval_ = fsm_interval;
}

Ref<McShardHash> McShardHash::unpack(vm::CellSlice& cs, tos::ShardIdFull id) {
  ...
  auto create = [&id](auto& descr, Ref<vm::CellSlice> fees, Ref<vm::CellSlice> funds) {
    ...
    return td::make_ref<McShardHash>(tos::BlockId{id, (unsigned)descr.seq_no}, descr.start_lt, descr.end_lt,
...
Ref<McShardHash> McShardHash::from_block(Ref<vm::Cell> block_root, const tos::FileHash& fhash, bool init_fees) {
  ...
  tos::RootHash rhash = block_root->get_hash().bits();
  CurrencyCollection fees_collected, funds_created;
  if (init_fees) {
  ...
  return Ref<McShardHash>(true, tos::BlockId{tos::ShardIdFull(shard), (unsigned)info.seq_no}, info.start_lt,
...
Ref<McShardDescr> McShardDescr::from_block(Ref<vm::Cell> block_root, Ref<vm::Cell> state_root,
                                           const tos::FileHash& fhash, bool init_fees) {
  if (block_root.is_null()) {
    return {};
  }
  block::gen::Block::Record rec;
  ...
  auto res = Ref<McShardDescr>(true, tos::BlockId{tos::ShardIdFull(shard), (unsigned)info.seq_no}, info.start_lt,
                               info.end_lt, info.gen_utime, rhash, fhash, fees_collected, funds_created, ~0U,
...
Ref<McShardDescr> McShardDescr::from_state(tos::BlockIdExt blkid, Ref<vm::Cell> state_root) {
  ...
  if (tos::ShardIdFull(shard) != tos::ShardIdFull(blkid) || info.seq_no != blkid.seqno()) {
...
bool ShardConfig::get_shard_hash_raw_from(vm::Dictionary& dict, vm::CellSlice& cs, tos::ShardIdFull id,
  ...
  while (true) {
    ...
    if (t < 0) {
      ...
      true_id = tos::ShardIdFull{id.workchain, (id.shard | m) - (m >> 1)};
      if (leaf) {
        *leaf = std::move(root);
      }
      return true;
...
bool ShardConfig::get_shard_hash_raw(vm::CellSlice& cs, tos::ShardIdFull id, tos::ShardIdFull& true_id,
                                     bool exact) const {
  return shard_hashes_dict_ && get_shard_hash_raw_from(*shard_hashes_dict_, cs, id, true_id, exact);
}

Ref<McShardHash> ShardConfig::get_shard_hash(tos::ShardIdFull id, bool exact) const {
  if (id.is_masterchain()) {
    return (!exact || id.shard == tos::shardIdAll) ? get_mc_hash() : Ref<McShardHash>{};
  }
  tos::ShardIdFull true_id;
  vm::CellSlice cs;
...
bool McShardHash::extract_cc_seqno(vm::CellSlice& cs, tos::CatchainSeqno* cc) {
  auto get = [&cs, cc](auto& rec) {
    if (tlb::unpack_exact(cs, rec)) {
      *cc = rec.next_catchain_seqno;
      return true;
    } else {
      *cc = std::numeric_limits<tos::CatchainSeqno>::max();
      return false;
    }
  };
...
tos::CatchainSeqno ShardConfig::get_shard_cc_seqno(tos::ShardIdFull shard) const {
  if (shard.is_masterchain() || !shard.is_valid()) {
    return std::numeric_limits<tos::CatchainSeqno>::max();
  }
  tos::ShardIdFull true_id;
  tos::CatchainSeqno cc_seqno, cc_seqno2;
  vm::CellSlice cs;
  if (!(get_shard_hash_raw(cs, shard - 1, true_id, false) &&
        (tos::shard_is_ancestor(true_id, shard) || tos::shard_is_parent(shard, true_id)) &&
        McShardHash::extract_cc_seqno(cs, &cc_seqno))) {
    return std::numeric_limits<tos::CatchainSeqno>::max();
  }
  if (tos::shard_is_ancestor(true_id, shard)) {
    return cc_seqno;
  }
  if (!(get_shard_hash_raw(cs, shard + 1, true_id, false) && tos::shard_is_parent(shard, true_id) &&
...
tos::LogicalTime ShardConfig::get_shard_end_lt_ext(tos::AccountIdPrefixFull acc, tos::ShardIdFull& actual_shard) const {
  if (!acc.is_valid()) {
    actual_shard.workchain = tos::workchainInvalid;
    return 0;
  }
  if (acc.is_masterchain()) {
    actual_shard = tos::ShardIdFull(tos::masterchainId);
...
tos::LogicalTime ShardConfig::get_shard_end_lt(tos::AccountIdPrefixFull acc) const {
  tos::ShardIdFull tmp;
  return get_shard_end_lt_ext(acc, tmp);
}

bool ShardConfig::contains(tos::BlockIdExt blkid) const {
  auto entry = get_shard_hash(blkid.shard_full());
  return entry.not_null() && entry->blk_ == blkid;
}

static int process_workchain_shard_hashes(Ref<vm::Cell>& branch, tos::ShardIdFull shard,
  ...
  if (f == 1) {
    ...
    int r = process_workchain_shard_hashes(left, tos::shard_child(shard, true), func);
    if (r < 0) {
      return r;
    }
    r |= process_workchain_shard_hashes(right, tos::shard_child(shard, false), func);
    if (r <= 0) {
      return r;
    }
    vm::CellBuilder cb;
  ...
  shard_hashes_dict_->map(
      [&ok, &func](vm::CellBuilder& cb, Ref<vm::CellSlice> csr, td::ConstBitPtr key, int n) -> bool {
        ...
        ok = ok && (n == 32) && csr->size_ext() == 0x10000 && std::move(csr)->prefetch_ref_to(root) &&
             process_workchain_shard_hashes(root, tos::ShardIdFull{(int)key.get_int(32)}, func) >= 0 &&
...
static int process_workchain_sibling_shard_hashes(Ref<vm::Cell>& branch, Ref<vm::Cell> sibling, tos::ShardIdFull shard,
  ...
  if (f == 1) {
    ...
    auto left = cs.prefetch_ref(0), right = cs.prefetch_ref(1);
    auto orig_left = left;
    int r = process_workchain_sibling_shard_hashes(left, right, tos::shard_child(shard, true), func);
    if (r < 0) {
      return r;
    }
    r |= process_workchain_sibling_shard_hashes(right, std::move(orig_left), tos::shard_child(shard, false), func);
    if (r <= 0) {
      return r;
    }
    vm::CellBuilder cb;
    ...
    if (sibling.not_null()) {
      ...
      if (!cs2.fetch_ulong(1)) {
        sibling_info = McShardHash::unpack(cs2, tos::shard_sibling(shard));
...
bool ShardConfig::process_sibling_shard_hashes(std::function<int(McShardHash&, const McShardHash*)> func) {
  ...
  shard_hashes_dict_->map([&ok, &func](vm::CellBuilder& cb, Ref<vm::CellSlice> csr, td::ConstBitPtr key,
    ...
    Ref<vm::Cell> root;
    ok = ok && (n == 32) && csr->size_ext() == 0x10000 && std::move(csr)->prefetch_ref_to(root) &&
         process_workchain_sibling_shard_hashes(root, Ref<vm::Cell>{}, tos::ShardIdFull{(int)key.get_int(32)}, func) >=
             0;
...
std::vector<tos::BlockId> ShardConfig::get_shard_hash_ids(
    const std::function<bool(tos::ShardIdFull, bool)>& filter) const {
  if (!shard_hashes_dict_) {
    return {};
  }
  std::vector<tos::BlockId> res;
  ...
  bool ok = shard_hashes_dict_->check_for_each(
      [&res, &mcout, mc_shard_hash_ = mc_shard_hash_, &filter](Ref<vm::CellSlice> cs_ref, td::ConstBitPtr key,
        ...
        if (workchain >= 0 && !mcout) {
          if (filter(tos::ShardIdFull{tos::masterchainId}, true)) {
        ...
        stack.emplace(cs_ref->prefetch_ref(), tos::shardIdAll);
        while (!stack.empty()) {
          ...
          if (!filter(tos::ShardIdFull{workchain, shard}, !t)) {
            continue;
          }
          if (!t) {
  ...
  if (!mcout && filter(tos::ShardIdFull{tos::masterchainId}, true)) {
    res.emplace_back(mc_shard_hash_->blk_.id);
  }
  return res;
}

std::vector<tos::BlockId> ShardConfig::get_shard_hash_ids(bool skip_mc) const {
  return get_shard_hash_ids(
      [skip_mc](tos::ShardIdFull shard, bool) -> bool { return !(skip_mc && shard.is_masterchain()); });
}

std::vector<tos::BlockId> ShardConfig::get_intersecting_shard_hash_ids(tos::ShardIdFull myself) const {
  return get_shard_hash_ids(
      [myself](tos::ShardIdFull shard, bool) -> bool { return tos::shard_intersects(myself, shard); });
}

std::vector<tos::BlockId> ShardConfig::get_neighbor_shard_hash_ids(tos::ShardIdFull myself) const {
  return get_shard_hash_ids([myself](tos::ShardIdFull shard, bool) -> bool { return is_neighbor(myself, shard); });
}

std::vector<tos::BlockId> ShardConfig::get_proper_neighbor_shard_hash_ids(tos::ShardIdFull myself) const {
  return get_shard_hash_ids([myself](tos::ShardIdFull shard, bool leaf) -> bool {
    return is_neighbor(myself, shard) && !(leaf && tos::shard_intersects(myself, shard));
  });
}

bool ShardConfig::is_neighbor(tos::ShardIdFull x, tos::ShardIdFull y) {
...
bool ShardConfig::has_workchain(tos::WorkchainId workchain) const {
  return shard_hashes_dict_ && shard_hashes_dict_->key_exists(td::BitArray<32>{workchain});
}

std::vector<tos::WorkchainId> ShardConfig::get_workchains() const {
  if (!shard_hashes_dict_) {
    return {};
  }
  std::vector<tos::WorkchainId> res;
  if (!shard_hashes_dict_->check_for_each([&res](Ref<vm::CellSlice> val, td::ConstBitPtr key, int n) {
        CHECK(n == 32);
        tos::WorkchainId w = (int)key.get_int(32);
        res.push_back(w);
        return true;
      })) {
    return {};
...
bool ShardConfig::new_workchain(tos::WorkchainId workchain, tos::BlockSeqno reg_mc_seqno,
                                const tos::RootHash& zerostate_root_hash, const tos::FileHash& zerostate_file_hash) {
...
td::Result<bool> ShardConfig::may_update_shard_block_info(Ref<McShardHash> new_info,
                                                          const std::vector<tos::BlockIdExt>& old_blkids,
                                                          tos::LogicalTime lt_limit, Ref<McShardHash>* ancestor) const {
  if (!shard_hashes_dict_) {
  ...
  if (wc == tos::workchainInvalid || wc == tos::masterchainId) {
  ...
  bool before_split = tos::shard_is_parent(old_blkids[0].shard_full(), new_info->shard());
  bool before_merge = (old_blkids.size() == 2);
  if (before_merge) {
    if (!tos::shard_is_sibling(old_blkids[0].shard_full(), old_blkids[1].shard_full())) {
      return td::Status::Error(-666, "the two start blocks of a top shard block update must be siblings");
    }
    if (!tos::shard_is_parent(new_info->shard(), old_blkids[0].shard_full())) {
      return td::Status::Error(
          -666,
  ...
  tos::CatchainSeqno old_cc_seqno = 0;
...
bool ShardConfig::do_update_shard_info2(Ref<McShardHash> new_info1, Ref<McShardHash> new_info2) {
  if (new_info1.is_null() || new_info2.is_null() || !tos::shard_is_sibling(new_info1->shard(), new_info2->shard())) {
    return false;
  }
  if (new_info1->blk_.id.shard > new_info2->blk_.id.shard) {
    std::swap(new_info1, new_info2);
  ...
  return cb.store_bool_bool(true)              // bt_node$1
         ...
         && cb.finalize_to(ref) && set_shard_info(tos::shard_parent(new_info1->shard()), std::move(ref));
}

static bool btree_set(Ref<vm::Cell>& root, tos::ShardId shard, Ref<vm::Cell> value) {
  if (!shard) {
    return false;
  }
  if (shard == tos::shardIdAll) {
    root = value;
    return true;
  }
...
bool ShardConfig::set_shard_info(tos::ShardIdFull shard, Ref<vm::Cell> value) {
...
bool Config::is_special_smartcontract(const tos::StdSmcAddress& addr) const {
  CHECK(special_smc_dict);
  return special_smc_dict->lookup(addr).not_null() || addr == config_addr;
}

td::Result<std::vector<tos::StdSmcAddress>> Config::get_special_smartcontracts(bool without_config) const {
  if (!special_smc_dict) {
    return td::Status::Error(-666, "configuration loaded without fundamental smart contract list");
  }
  std::vector<tos::StdSmcAddress> res;
...
td::Result<std::vector<std::pair<tos::StdSmcAddress, int>>> ConfigInfo::get_special_ticktock_smartcontracts(
    int tick_tock) const {
  if (!special_smc_dict) {
    return td::Status::Error(-666, "configuration loaded without fundamental smart contract list");
  }
  ...
  std::vector<std::pair<tos::StdSmcAddress, int>> res;
...
tos::CatchainSeqno ConfigInfo::get_shard_cc_seqno(tos::ShardIdFull shard) const {
  return shard.is_masterchain() ? cc_seqno_ : ShardConfig::get_shard_cc_seqno(shard);
}

std::vector<tos::ValidatorDescr> Config::compute_validator_set(tos::ShardIdFull shard,
                                                               const block::TotalValidatorSet& vset, tos::UnixTime time,
                                                               tos::CatchainSeqno cc_seqno) const {
  return do_compute_validator_set(get_catchain_validators_config(), shard, vset, cc_seqno);
}

std::vector<tos::ValidatorDescr> Config::compute_validator_set(tos::ShardIdFull shard, tos::UnixTime time,
                                                               tos::CatchainSeqno cc_seqno) const {
  if (!cur_validators_) {
...
std::vector<tos::ValidatorDescr> ConfigInfo::compute_validator_set_cc(tos::ShardIdFull shard,
                                                                      const block::TotalValidatorSet& vset,
                                                                      tos::UnixTime time,
                                                                      tos::CatchainSeqno* cc_seqno_delta) const {
  if (cc_seqno_delta && (*cc_seqno_delta & -2)) {
    return {};
  }
  tos::CatchainSeqno cc_seqno = get_shard_cc_seqno(shard);
  if (cc_seqno == ~0U) {
    return {};
  }
  if (cc_seqno_delta) {
...
std::vector<tos::ValidatorDescr> ConfigInfo::compute_validator_set_cc(tos::ShardIdFull shard, tos::UnixTime time,
                                                                      tos::CatchainSeqno* cc_seqno_delta) const {
...
std::map<tos::Bits256, int> TotalValidatorSet::compute_validator_map() const {
  std::map<tos::Bits256, int> res;
...
std::vector<tos::ValidatorDescr> Config::do_compute_validator_set(const CatchainValidatorsConfig& ccv_conf,
                                                                  tos::ShardIdFull shard, const TotalValidatorSet& vset,
...
std::vector<tos::ValidatorDescr> Config::compute_total_validator_set(int next) const {
...
td::Status Config::check_aipow_config() const {
  ...
  if (reg.commitment_code_hash.is_zero()) {
...
td::Ref<vm::Tuple> Config::get_unpacked_config_tuple(tos::UnixTime now) const {
...
td::Result<std::pair<tos::UnixTime, tos::UnixTime>> Config::unpack_validator_set_start_stop(Ref<vm::Cell> vset_root) {
  if (vset_root.is_null()) {
    return td::Status::Error("validator set absent");
  }
  gen::ValidatorSet::Record_validators_ext rec;
  if (tlb::unpack_cell(vset_root, rec)) {
    return std::pair<tos::UnixTime, tos::UnixTime>(rec.utime_since, rec.utime_until);
  }
  gen::ValidatorSet::Record_validators rec0;
  if (tlb::unpack_cell(std::move(vset_root), rec0)) {
    return std::pair<tos::UnixTime, tos::UnixTime>(rec0.utime_since, rec0.utime_until);
  }
  return td::Status::Error("validator set is invalid");
}

std::pair<tos::UnixTime, tos::UnixTime> Config::get_validator_set_start_stop(int next) const {
...
bool WorkchainInfo::unpack(tos::WorkchainId wc, vm::CellSlice& cs) {
  workchain = tos::workchainInvalid;
  if (wc == tos::workchainInvalid) {
    return false;
  }
...
Ref<WorkchainInfo> Config::get_workchain_info(tos::WorkchainId workchain_id) const {
  if (!workchains_dict_) {
    return {};
  }
...
bool ConfigInfo::get_old_mc_block_id(tos::BlockSeqno seqno, tos::BlockIdExt& blkid, tos::LogicalTime* end_lt) const {
...
bool ConfigInfo::check_old_mc_block_id(const tos::BlockIdExt& blkid, bool strict) const {
...
bool ConfigInfo::get_mc_block_by_lt(tos::LogicalTime req_lt, tos::BlockIdExt& blkid, tos::LogicalTime* end_lt) const {
  if (req_lt > lt) {
    return false;
  }
  td::BitArray<32> key;
...
bool ConfigInfo::get_prev_key_block(tos::BlockSeqno req_seqno, tos::BlockIdExt& blkid, tos::LogicalTime* end_lt) const {
...
bool ConfigInfo::get_next_key_block(tos::BlockSeqno req_seqno, tos::BlockIdExt& blkid, tos::LogicalTime* end_lt) const {
  td::BitArray<32> key;
  auto found = prev_blocks_dict_->traverse_extra(
      key.bits(), 32,
...
td::Result<Ref<vm::Tuple>> ConfigInfo::get_prev_blocks_info() const {
  ...
  auto block_id_to_tuple = [](const tos::BlockIdExt& block_id) -> vm::Ref<vm::Tuple> {
  ...
  for (tos::BlockSeqno seqno = block_id.id.seqno; seqno > 0 && last_mc_blocks.size() < 16;) {
    --seqno;
    tos::BlockIdExt id;
  ...
  tos::BlockIdExt last_key_block;
  tos::LogicalTime last_key_block_lt;
  ...
  if (get_global_version() >= 9) {
    ...
    for (tos::BlockSeqno seqno = block_id.id.seqno / 100 * 100; last_mc_blocks_100.size() < 16;) {
      tos::BlockIdExt id;
      if (!get_old_mc_block_id(seqno, id)) {
        return td::Status::Error("cannot fetch old mc block");
      }
