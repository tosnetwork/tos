...
#if defined(_INTERNAL_COMPILE) || defined(_TOSLIB_COMPILE)
#define WITH_TOSLIB
...
// Used by zerostate.py so wc=2 can route ext_in_msgs (`uno_sendMineUno`)
// from block 0 — without this the collator's ShardAccounts lookup returns
// null and every inbound MineUno tx is dropped before compute phase.
//
// Stack: ( -- accounts_cell )
void interpret_uno_zerostate_accounts_cell(vm::Stack& stack) {
  Ref<vm::Cell> cell = uno_workchain::build_uno_zerostate_accounts_cell();
  if (cell.is_null()) {
    throw fift::IntError{"could not build UNO zerostate accounts cell"};
  }
  stack.push_cell(std::move(cell));
}
...
void interpret_mnemonic_to_privkey(vm::Stack& stack, int mode) {
  ...
  auto res = toslib::Mnemonic::create(std::move(str), td::SecureString());
  if (res.is_error()) {
...
void usage(const char* progname) {
  std::cerr
      << "Creates initial state for a TOS blockchain, using configuration defined by Fift-language source files\n";
  std::cerr
      << "usage: " << progname
