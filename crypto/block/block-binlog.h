...
struct Start {
  ...
  Start(const tos::RootHash& hash, unsigned _now = 0)
      : tag_field(tag), type_field(log_type), created_at(_now ? _now : (unsigned)std::time(nullptr)) {
    td::as<tos::RootHash>(zerostate_root_hash) = hash;
  }
};
...
struct SetZeroState {
  ...
  SetZeroState(const tos::RootHash& rhash, const tos::FileHash& fhash, unsigned long long _fsize, unsigned _flags = 0)
      : tag_field(tag), flags(_flags), file_size(_fsize) {
    td::as<tos::FileHash>(file_hash) = fhash;
    td::as<tos::RootHash>(root_hash) = rhash;
  }
};
...
struct NewBlock {
  ...
  NewBlock(const tos::BlockId& block, const tos::RootHash& rhash, const tos::FileHash& fhash, unsigned long long _fsize,
           unsigned _flags)
      : tag_field(tag)
      , flags(_flags)
...
struct NewState {
  ...
  unsigned char root_hash[32];
  unsigned char last_bytes[8];
  NewState(const tos::BlockId& state, const tos::RootHash& rhash, const tos::FileHash& fhash, unsigned long long _fsize,
           unsigned _flags)
      : tag_field(tag)
      , flags(_flags)
