...
using FileHash = tos::Bits256;
using RootHash = tos::Bits256;
using td::Ref;
...
struct FileInfo : public td::CntObject {
  ...
  FileInfo(FileType _type, const tos::BlockId& _id, int _status, const FileHash& _fhash, long long _fsize = -1)
      : blk(_id, _fhash), type(_type), status(_status), file_size(_fsize) {
    blk.root_hash.set_zero();
  }
  FileInfo(FileType _type, const tos::BlockId& _id, int _status, const FileHash& _fhash, const RootHash& _rhash,
           long long _fsize = -1)
...
struct OutputQueueInfoDescr : public td::CntObject {
  ...
  OutputQueueInfoDescr(tos::BlockId _id, const RootHash& _bhash, const RootHash& _shash, Ref<vm::Cell> _qinfo)
      : id(_id)
      , block_hash(_bhash)
      , state_hash(_shash)
  ...
  OutputQueueInfoDescr(tos::BlockId _id, td::ConstBitPtr _bhash, td::ConstBitPtr _shash, Ref<vm::Cell> _qinfo)
      : id(_id)
      , block_hash(_bhash)
      , state_hash(_shash)
      , output_queue_info_hash(_qinfo->get_hash().bits())
...
class BlockDb : public td::actor::Actor {
 public:
  ...
  virtual void get_top_block_id(tos::ShardIdFull shard, int authority, td::Promise<tos::BlockIdExt> promise) = 0;
  virtual void get_top_block_state_id(tos::ShardIdFull shard, int authority, td::Promise<tos::BlockIdExt> promise) = 0;
  virtual void get_block_by_id(tos::BlockId blk_id, bool need_data, td::Promise<td::Ref<FileInfo>> promise) = 0;
  virtual void get_state_by_id(tos::BlockId blk_id, bool need_data, td::Promise<td::Ref<FileInfo>> promise) = 0;
  virtual void get_out_queue_info_by_id(tos::BlockId blk_id, td::Promise<td::Ref<OutputQueueInfoDescr>> promise) = 0;
  ...
  virtual void get_object_by_root_hash(RootHash root_hash, bool need_data, bool force_file_load,
                                       td::Promise<td::Ref<FileInfo>> promise) = 0;
  virtual void save_new_block(tos::BlockIdExt blk_id, td::BufferSlice data, int authority,
                              td::Promise<td::Unit> promise) = 0;
  virtual void save_new_state(tos::BlockIdExt state_id, td::BufferSlice data, int authority,
                              td::Promise<td::Unit> promise) = 0;
};
