...
td::Status check_block_header_proof(td::Ref<vm::Cell> root, tos::BlockIdExt blkid, tos::Bits256* store_state_hash_to,
                                    bool check_state_hash, td::uint32* save_utime, tos::LogicalTime* save_lt) {
...
td::Status check_shard_proof(tos::BlockIdExt blk, tos::BlockIdExt shard_blk, td::Slice shard_proof) {
  ...
  try {
    ...
    tos::Bits256 mc_state_hash = mc_state_root->get_hash().bits();
...
td::Status check_account_proof(td::Slice proof, tos::BlockIdExt shard_blk, const block::StdAddress& addr,
                               td::Ref<vm::Cell> root, tos::LogicalTime* last_trans_lt, tos::Bits256* last_trans_hash,
                               td::uint32* save_utime, tos::LogicalTime* save_lt) {
...
td::Result<AccountState::Info> AccountState::validate(tos::BlockIdExt ref_blk, block::StdAddress addr) const {
  ...
  if (!tos::shard_contains(shard_blk.shard_full(), tos::extract_addr_prefix(addr.workchain, addr.addr))) {
...
td::Status BlockProofLink::validate(td::uint32* save_utime) const {
  ...
  try {
    ...
    if (!is_fwd) {
      ...
      tos::ShardIdFull shard{tos::masterchainId};
