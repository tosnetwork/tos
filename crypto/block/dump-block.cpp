...
void test1() {
  block::ShardId id{tos::masterchainId}, id2{tos::basechainId, 0x11efULL << 48};
  ...
  std::cout << "Tomis.validate(cs) = " << block::tlb::t_Tomis.validate_upto(1024, *cs) << std::endl;
  std::cout << "Tomis.validate(cs2) = " << block::tlb::t_Tomis.validate_upto(1024, *cs2) << std::endl;
  //
  block::gen::SplitMergeInfo::Record data;
  block::gen::Tomis::Record data2;
  std::cout << "block::gen::Tomis.validate(cs) = " << block::gen::t_Tomis.validate_upto(1024, *cs) << std::endl;
  std::cout << "block::gen::Tomis.validate(cs2) = " << block::gen::t_Tomis.validate_upto(1024, *cs2) << std::endl;
...
td::Status test_vset() {
  ...
  tos::ShardIdFull shard{0, 0x6e80000000000000};
  tos::CatchainSeqno cc_seqno = std::max(48763, 48763) + 1 + 1;
  tos::UnixTime now = 1586169666;
