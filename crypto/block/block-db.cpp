...
int BlockBinlogCallback::replay(const log::SetZeroState& lev, unsigned long long log_pos) const {
  ...
  if (db.zstate_rhash != td::ConstBitPtr{lev.root_hash}) {
    throw BinlogBuffer::InterpretError{std::string{"SetZeroState: zerostate root hash mismatch: in binlog "} +
                                       tos::Bits256{lev.root_hash}.to_hex() + ", required " + db.zstate_rhash.to_hex()};
  }
  db.zerostate = td::Ref<FileInfo>{true,
                                   FileType::state,
                                   tos::BlockId{tos::masterchainId, 1ULL << 63, 0},
                                   0,
...
int BlockBinlogCallback::replay(const log::NewBlock& lev, unsigned long long log_pos) const {
  ...
  if (!lev.seqno || lev.workchain == tos::workchainInvalid) {
    return -1;
  }
  tos::BlockId blkid{lev.workchain, lev.shard, lev.seqno};
...
int BlockBinlogCallback::replay(const log::NewState& lev, unsigned long long log_pos) const {
  ...
  tos::BlockId id{lev.workchain, lev.shard, lev.seqno};
...
void BlockDbImpl::get_top_block_id(tos::ShardIdFull shard, int authority, td::Promise<tos::BlockIdExt> promise) {
  LOG(DEBUG) << "in BlockDb::get_top_block_id()";
  auto it = block_info.upper_bound(tos::BlockId{shard, std::numeric_limits<td::uint32>::max()});
  if (it != block_info.begin() && tos::ShardIdFull{(--it)->first} == shard) {
    promise.set_result(it->second->blk);
    return;
  }
  if (shard.is_masterchain()) {
    promise.set_result(tos::BlockIdExt{tos::BlockId{tos::masterchainId, 1ULL << 63, 0}});
    return;
  }
  promise.set_result(td::Status::Error(-666));
}

void BlockDbImpl::get_top_block_state_id(tos::ShardIdFull shard, int authority, td::Promise<tos::BlockIdExt> promise) {
  LOG(DEBUG) << "in BlockDb::get_top_block_state_id()";
  auto it = state_info.upper_bound(tos::BlockId{shard, std::numeric_limits<td::uint32>::max()});
  if (it != state_info.begin() && tos::ShardIdFull{(--it)->first} == shard) {
    promise.set_result(it->second->blk);
    return;
  }
...
void BlockDbImpl::get_block_by_id(tos::BlockId blk_id, bool need_data, td::Promise<td::Ref<FileInfo>> promise) {
...
void BlockDbImpl::get_state_by_id(tos::BlockId blk_id, bool need_data, td::Promise<td::Ref<FileInfo>> promise) {
...
void BlockDbImpl::get_out_queue_info_by_id(tos::BlockId blk_id, td::Promise<td::Ref<OutputQueueInfoDescr>> promise) {
...
void BlockDbImpl::save_new_block(tos::BlockIdExt id, td::BufferSlice data, int authority,
...
void BlockDbImpl::save_new_state(tos::BlockIdExt id, td::BufferSlice data, int authority,
                                 td::Promise<td::Unit> promise) {
  // TODO: add verification that data is a BoC with correct root hash, and that it is a Block corresponding to blk_id
  // ...
  // TODO: check whether we already have a block with blk_id
