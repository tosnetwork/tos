...
struct NewOutMsg {
  ...
  NewOutMsg(tos::LogicalTime _lt, Ref<vm::Cell> _msg, Ref<vm::Cell> _trans, unsigned _msg_idx)
...
struct StoragePhase {
  ...
  tos::UnixTime last_paid_updated;
  bool frozen{false};
  bool deleted{false};
...
struct ComputePhaseConfig {
  ...
  bool is_address_suspended(tos::WorkchainId wc, td::Bits256 addr) const;
...
struct ComputePhase {
  ...
  int exit_arg;
  int vm_steps;
  tos::Bits256 vm_init_state_hash, vm_final_state_hash;
  Ref<vm::Cell> in_msg;
...
struct Account {
  ...
  tos::StdSmcAddress addr_orig;  // address indicated in smart-contract data (must coincide with hash of StateInit)
  Ref<vm::CellSlice>
  ...
  tos::LogicalTime last_trans_end_lt_;
  tos::LogicalTime last_trans_lt_;
  tos::Bits256 last_trans_hash_;
  tos::LogicalTime block_lt;
  ...
  tos::Bits256 state_hash;         // hash of StateInit for frozen accounts
  Ref<vm::Cell> code, data, library;
  Ref<vm::Cell> orig_code, orig_data, orig_library;
  std::vector<LtCellRef> transactions;
  Account() = default;
  Account(tos::WorkchainId wc, td::ConstBitPtr _addr) : workchain(wc), addr(_addr) {
  }
  block::CurrencyCollection get_balance() const {
    return balance;
  }
  bool set_address(tos::WorkchainId wc, td::ConstBitPtr new_addr);
  bool unpack(Ref<vm::CellSlice> account, tos::UnixTime now, bool special);
  bool init_new(tos::UnixTime now);
  ...
  td::RefInt256 compute_storage_fees(tos::UnixTime now, const std::vector<block::StoragePrices>& pricing) const;
  bool is_masterchain() const {
    return workchain == tos::masterchainId;
  }
  bool belongs_to_shard(tos::ShardIdFull shard) const;
  ...
  void push_transaction(Ref<vm::Cell> trans_root, tos::LogicalTime trans_lt);
...
struct Transaction {
  ...
  tos::LogicalTime start_lt, end_lt;
 ...
 public:
  ...
  // D76: exact_principal independently binds the serialized message value to
  // the authorized operation's x, rather than to a conservation identity.
  // Principal-only callers retain fee_budget: only an exact q anchor permits
  // the private pricing scratch balance to exceed that legacy fee ceiling.
