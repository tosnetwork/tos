...
struct TotalValidatorSet {
  ...
  std::vector<tos::ValidatorDescr> export_validator_set() const;
  std::map<tos::Bits256, int> compute_validator_map() const;
...
struct validator_set_descr {
  ...
  validator_set_descr(tos::ShardIdFull shard_id, tos::CatchainSeqno cc_seqno_, bool flag)
      : shard(td::bswap64(shard_id.shard))
      , workchain(td::bswap32(shard_id.workchain))
      , cc_seqno(td::bswap32(cc_seqno_)) {
  }
  validator_set_descr(tos::ShardIdFull shard_id, tos::CatchainSeqno cc_seqno_)
      : validator_set_descr(shard_id, cc_seqno_, false) {
    std::memset(seed, 0, 32);
  }
  validator_set_descr(tos::ShardIdFull shard_id, tos::CatchainSeqno cc_seqno_, const unsigned char seed_[32])
      : validator_set_descr(shard_id, cc_seqno_, false) {
    std::memcpy(seed, seed_, 32);
  }
  validator_set_descr(tos::ShardIdFull shard_id, tos::CatchainSeqno cc_seqno_, td::ConstBitPtr seed_)
      : validator_set_descr(shard_id, cc_seqno_, false) {
    td::BitPtr{seed}.copy_from(seed_, 256);
  }
  validator_set_descr(tos::ShardIdFull shard_id, tos::CatchainSeqno cc_seqno_, const td::Bits256& seed_)
      : validator_set_descr(shard_id, cc_seqno_, false) {
...
class ValidatorSetPRNG {
 ...
 public:
  ...
  ValidatorSetPRNG(tos::ShardIdFull shard_id, tos::CatchainSeqno cc_seqno_) : data(shard_id, cc_seqno_) {
  }
  ValidatorSetPRNG(tos::ShardIdFull shard_id, tos::CatchainSeqno cc_seqno_, const unsigned char seed_[32])
      : data(shard_id, cc_seqno_, seed_) {
  }
  ValidatorSetPRNG(tos::ShardIdFull shard_id, tos::CatchainSeqno cc_seqno_, td::ConstBitPtr seed_)
      : data(shard_id, cc_seqno_, std::move(seed_)) {
  }
  ValidatorSetPRNG(tos::ShardIdFull shard_id, tos::CatchainSeqno cc_seqno_, const td::Bits256& seed_)
      : data(shard_id, cc_seqno_, seed_) {
  }
...
class McShardHashI : public td::CntObject {
 public:
  ...
  virtual tos::BlockIdExt top_block_id() const = 0;
  virtual tos::LogicalTime start_lt() const = 0;
  virtual tos::LogicalTime end_lt() const = 0;
  virtual tos::UnixTime fsm_utime() const = 0;
  virtual FsmState fsm_state() const = 0;
  virtual tos::ShardIdFull shard() const = 0;
...
struct McShardHash : public McShardHashI {
  tos::BlockIdExt blk_;
  tos::LogicalTime start_lt_, end_lt_;
  tos::UnixTime gen_utime_{0};
  tos::UnixTime fsm_utime_{0};
  tos::UnixTime fsm_interval_{0};
  tos::BlockSeqno min_ref_mc_seqno_{std::numeric_limits<tos::BlockSeqno>::max()};
  tos::BlockSeqno reg_mc_seqno_{std::numeric_limits<tos::BlockSeqno>::max()};
  FsmState fsm_{FsmState::fsm_none};
  bool disabled_{false};
  bool before_split_{false}, before_merge_{false}, want_split_{false}, want_merge_{false};
  tos::CatchainSeqno next_catchain_seqno_{std::numeric_limits<tos::CatchainSeqno>::max()};
  tos::ShardId next_validator_shard_{tos::shardIdAll};
  CurrencyCollection fees_collected_, funds_created_;
  McShardHash(const tos::BlockId& id, tos::LogicalTime start_lt, tos::LogicalTime end_lt, tos::UnixTime gen_utime,
              const tos::BlockHash& root_hash, const tos::FileHash& file_hash, CurrencyCollection fees_collected = {},
              CurrencyCollection funds_created = {},
              tos::BlockSeqno reg_mc_seqno = std::numeric_limits<tos::BlockSeqno>::max(),
              tos::BlockSeqno min_ref_mc_seqno = std::numeric_limits<tos::BlockSeqno>::max(),
              tos::CatchainSeqno cc_seqno = std::numeric_limits<tos::CatchainSeqno>::max(), tos::ShardId val_shard = 0,
  ...
  McShardHash(const tos::BlockIdExt& blk, tos::LogicalTime start_lt, tos::LogicalTime end_lt)
  ...
  tos::BlockIdExt top_block_id() const override final {
    return blk_;
  }
  //  ZeroStateIdExt zero_state() const override;
  tos::LogicalTime start_lt() const override final {
    return start_lt_;
  }
  tos::LogicalTime end_lt() const override final {
    return end_lt_;
  }
  tos::UnixTime fsm_utime() const override final {
    return fsm_utime_;
  }
  tos::UnixTime fsm_utime_end() const {
    return fsm_utime_ + fsm_interval_;
  }
  tos::UnixTime created_at() const {
    return gen_utime_;
  }
  ...
  tos::ShardIdFull shard() const override final {
    return tos::ShardIdFull(blk_);
  }
  tos::WorkchainId workchain() const {
    return blk_.id.workchain;
  }
  bool contains(const tos::AccountIdPrefixFull& pfx) const {
    return tos::shard_contains(shard(), pfx);
  }
  ...
  tos::BlockSeqno seqno() const {
    return blk_.id.seqno;
  }
  bool set_reg_mc_seqno(tos::BlockSeqno reg_mc_seqno) {
    reg_mc_seqno_ = reg_mc_seqno;
    return true;
  }
  ...
  void set_fsm(FsmState fsm, tos::UnixTime fsm_utime, tos::UnixTime fsm_interval);
  void set_fsm_split(tos::UnixTime fsm_utime, tos::UnixTime fsm_interval) {
    set_fsm(FsmState::fsm_split, fsm_utime, fsm_interval);
  }
  void set_fsm_merge(tos::UnixTime fsm_utime, tos::UnixTime fsm_interval) {
  ...
  static Ref<McShardHash> unpack(vm::CellSlice& cs, tos::ShardIdFull id);
  static Ref<McShardHash> from_block(Ref<vm::Cell> block_root, const tos::FileHash& _fhash, bool init_fees = false);
  static bool extract_cc_seqno(vm::CellSlice& cs, tos::CatchainSeqno* cc);
...
struct McShardDescr final : public McShardHash {
  ...
  McShardDescr(const tos::BlockId& id, tos::LogicalTime start_lt, tos::LogicalTime end_lt, tos::UnixTime gen_utime,
               const tos::BlockHash& root_hash, const tos::FileHash& file_hash, CurrencyCollection fees_collected = {},
               CurrencyCollection funds_created = {},
               tos::BlockSeqno reg_mc_seqno = std::numeric_limits<tos::BlockSeqno>::max(),
               tos::BlockSeqno min_ref_mc_seqno = std::numeric_limits<tos::BlockSeqno>::max(),
               tos::CatchainSeqno cc_seqno = std::numeric_limits<tos::CatchainSeqno>::max(),
               tos::ShardId val_shard = tos::shardIdAll, bool nx_cc_updated = false, bool before_split = false,
  ...
  McShardDescr(const tos::BlockIdExt& blk, tos::LogicalTime start_lt, tos::LogicalTime end_lt)
  ...
  void disable();
  static Ref<McShardDescr> from_block(Ref<vm::Cell> block_root, Ref<vm::Cell> state_root, const tos::FileHash& _fhash,
                                      bool init_fees = false);
  static Ref<McShardDescr> from_state(tos::BlockIdExt blkid, Ref<vm::Cell> state_root);
};
...
struct StoragePrices {
  ...
  StoragePrices(tos::UnixTime _valid_since, td::uint64 _bprice, td::uint64 _cprice, td::uint64 _mc_bprice,
  ...
  static td::RefInt256 compute_storage_fees(tos::UnixTime now, const std::vector<block::StoragePrices>& pricing,
                                            const StorageUsed& storage_used, tos::UnixTime last_paid, bool is_special,
                                            bool is_masterchain);
};
...
struct WorkchainInfo : public td::CntObject {
  ...
  bool unpack(tos::WorkchainId wc, vm::CellSlice& cs);
};
...
class ShardConfig {
  ...
  std::set<tos::ShardIdFull> shards_updated_;
  bool valid_{false};
 ...
 public:
  ...
  bool has_workchain(tos::WorkchainId workchain) const;
  std::vector<tos::WorkchainId> get_workchains() const;
  Ref<McShardHash> get_shard_hash(tos::ShardIdFull id, bool exact = true) const;
  bool contains(tos::BlockIdExt blkid) const;
  bool get_shard_hash_raw(vm::CellSlice& cs, tos::ShardIdFull id, tos::ShardIdFull& true_id, bool exact = true) const;
  tos::LogicalTime get_shard_end_lt(tos::AccountIdPrefixFull acc) const;
  tos::LogicalTime get_shard_end_lt_ext(tos::AccountIdPrefixFull acc, tos::ShardIdFull& actual_shard) const;
  static bool get_shard_hash_raw_from(vm::Dictionary& shard_hashes_dict, vm::CellSlice& cs, tos::ShardIdFull id,
                                      tos::ShardIdFull& true_id, bool exact = true, Ref<vm::Cell>* leaf = nullptr);
  std::vector<tos::BlockId> get_shard_hash_ids(bool skip_mc = false) const;
  std::vector<tos::BlockId> get_shard_hash_ids(const std::function<bool(tos::ShardIdFull, bool)>& filter) const;
  std::vector<tos::BlockId> get_intersecting_shard_hash_ids(tos::ShardIdFull myself) const;
  std::vector<tos::BlockId> get_neighbor_shard_hash_ids(tos::ShardIdFull myself) const;
  std::vector<tos::BlockId> get_proper_neighbor_shard_hash_ids(tos::ShardIdFull myself) const;
  ...
  static bool is_neighbor(tos::ShardIdFull x, tos::ShardIdFull y);
  ...
  tos::CatchainSeqno get_shard_cc_seqno(tos::ShardIdFull shard) const;
  ...
  bool new_workchain(tos::WorkchainId workchain, tos::BlockSeqno reg_mc_seqno, const tos::RootHash& zerostate_root_hash,
                     const tos::FileHash& zerostate_file_hash);
  td::Result<bool> update_shard_block_info(Ref<McShardHash> new_info, const std::vector<tos::BlockIdExt>& old_blkids);
  ...
  td::Result<bool> may_update_shard_block_info(Ref<McShardHash> new_info,
                                               const std::vector<tos::BlockIdExt>& old_blkids,
                                               tos::LogicalTime lt_limit = std::numeric_limits<tos::LogicalTime>::max(),
 ...
 private:
  ...
  bool set_shard_info(tos::ShardIdFull shard, Ref<vm::Cell> value);
};
...
class Config {
 ...
 public:
  static constexpr tos::LogicalTime get_lt_align() {
    return 1000000;
  }
  static constexpr tos::LogicalTime get_max_lt_growth() {
  ...
  bool ihr_enabled() const {
    return has_capability(tos::capIhrEnabled);
  }
  ...
  td::Result<tos::StdSmcAddress> get_dns_root_addr() const;
  bool set_block_id_ext(const tos::BlockIdExt& block_id_ext);
  td::Result<std::vector<tos::StdSmcAddress>> get_special_smartcontracts(bool without_config = false) const;
  bool is_special_smartcontract(const tos::StdSmcAddress& addr) const;
  ...
  std::vector<tos::ValidatorDescr> compute_validator_set(tos::ShardIdFull shard, const block::TotalValidatorSet& vset,
                                                         tos::UnixTime time, tos::CatchainSeqno cc_seqno) const;
  std::vector<tos::ValidatorDescr> compute_validator_set(tos::ShardIdFull shard, tos::UnixTime time,
  ...
  static std::vector<tos::ValidatorDescr> do_compute_validator_set(const CatchainValidatorsConfig& ccv_conf,
                                                                   tos::ShardIdFull shard,
  ...
  static td::Result<std::pair<tos::UnixTime, tos::UnixTime>> unpack_validator_set_start_stop(Ref<vm::Cell> root);
...
class ConfigInfo : public Config, public ShardConfig {
 public:
  ...
  tos::BlockSeqno min_ref_mc_seqno_{std::numeric_limits<tos::BlockSeqno>::max()};
  tos::CatchainSeqno cc_seqno_{std::numeric_limits<tos::CatchainSeqno>::max()};
 ...
 private:
  ...
  tos::LogicalTime last_key_block_lt_;
 ...
 public:
  ...
  Ref<vm::Cell> lookup_library(const tos::Bits256& root_hash) const {
  ...
  tos::BlockSeqno get_vert_seqno() const {
    return vert_seqno;
  }
  tos::CatchainSeqno get_shard_cc_seqno(tos::ShardIdFull shard) const;
  bool get_last_key_block(tos::BlockIdExt& blkid, tos::LogicalTime& blklt, bool strict = false) const;
  bool get_old_mc_block_id(tos::BlockSeqno seqno, tos::BlockIdExt& blkid, tos::LogicalTime* end_lt = nullptr) const;
  bool check_old_mc_block_id(const tos::BlockIdExt& blkid, bool strict = false) const;
  // returns block with min seqno and req_lt <= block.end_lt
  bool get_mc_block_by_lt(tos::LogicalTime lt, tos::BlockIdExt& blkid, tos::LogicalTime* end_lt = nullptr) const;
  bool get_prev_key_block(tos::BlockSeqno req_seqno, tos::BlockIdExt& blkid, tos::LogicalTime* end_lt = nullptr) const;
  bool get_next_key_block(tos::BlockSeqno req_seqno, tos::BlockIdExt& blkid, tos::LogicalTime* end_lt = nullptr) const;
  td::Result<std::vector<std::pair<tos::StdSmcAddress, int>>> get_special_ticktock_smartcontracts(
  ...
  std::vector<tos::ValidatorDescr> compute_validator_set_cc(tos::ShardIdFull shard,
                                                            const block::TotalValidatorSet& vset, tos::UnixTime time,
                                                            tos::CatchainSeqno* cc_seqno_delta = nullptr) const;
  std::vector<tos::ValidatorDescr> compute_validator_set_cc(tos::ShardIdFull shard, tos::UnixTime time,
                                                            tos::CatchainSeqno* cc_seqno_delta = nullptr) const;
  ...
  static td::Result<std::unique_ptr<ConfigInfo>> extract_config(Ref<vm::Cell> mc_state_root,
                                                                tos::BlockIdExt mc_block_id, int mode = 0);
