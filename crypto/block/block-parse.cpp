...
tos::AccountIdPrefixFull MsgAddressInt::get_prefix(vm::CellSlice&& cs) {
  if (!cs.have(3 + 8 + 64)) {
    return {};
  }
...
tos::AccountIdPrefixFull MsgAddressInt::get_prefix(const vm::CellSlice& cs) {
  return get_prefix(vm::CellSlice{cs});
}

tos::AccountIdPrefixFull MsgAddressInt::get_prefix(Ref<vm::CellSlice> cs_ref) {
  if (cs_ref->is_unique()) {
...
bool MsgAddressInt::extract_std_address(Ref<vm::CellSlice> cs_ref, tos::WorkchainId& workchain,
                                        tos::StdSmcAddress& addr, bool rewrite) const {
  if (cs_ref.is_null()) {
    return false;
...
bool MsgAddressInt::extract_std_address(vm::CellSlice& cs, tos::WorkchainId& workchain, tos::StdSmcAddress& addr,
...
bool MsgAddressInt::store_std_address(vm::CellBuilder& cb, tos::WorkchainId workchain,
                                      const tos::StdSmcAddress& addr) const {
...
Ref<vm::CellSlice> MsgAddressInt::pack_std_address(tos::WorkchainId workchain, const tos::StdSmcAddress& addr) const {
  vm::CellBuilder cb;
...
bool TransactionDescr::skip(vm::CellSlice& cs) const {
  switch (get_tag(cs)) {
    case trans_workchain_entry_v3:
      return cs.advance(4) && cs.advance_refs(3);
    ...
    case trans_workchain_batch_v2:
      return cs.advance(708) && cs.have(1) && (!cs.fetch_ulong(1) || cs.advance_refs(1));
    case trans_ord:
  ...
  switch (get_tag(cs)) {
    case trans_workchain_entry_v3:
      return cs.advance(4) && gen::t_UnoV2HostRecord.validate_ref(ops, cs.fetch_ref(), weak) &&
             gen::t_UnoV2HostInput.validate_ref(ops, cs.fetch_ref(), weak) &&
             gen::t_UnoV2HostEffects.validate_ref(ops, cs.fetch_ref(), weak);
    case trans_workchain_settlement_participant_v3:
    case trans_workchain_storage_participant_v3:
      return cs.advance(4) && gen::t_UnoV2HostRecord.validate_ref(ops, cs.fetch_ref(), weak);
    case trans_workchain_batch_v2:
      return cs.advance(708) && cs.have(1) &&
             (!cs.fetch_ulong(1) || gen::t_WorkchainBatchInbound.validate_ref(ops, cs.fetch_ref(), weak));
    case trans_ord:
...
int TransactionDescr::get_tag(const vm::CellSlice& cs) const {
  ...
  return (t >= 0 && (t < 8 || t == trans_workchain_batch_v2 || t == trans_workchain_storage_participant_v3 ||
                    t == trans_workchain_settlement_participant_v3 || t == trans_workchain_entry_v3))
             ? (t == 3 ? 2 : t) : -1;
}
...
const Aug_InMsgDescr aug_InMsgDescrDefault(tos::SUPPORTED_VERSION);
const InMsgDescr t_InMsgDescrDefault(tos::SUPPORTED_VERSION);
...
const Aug_OutMsgDescr aug_OutMsgDescrDefault(tos::SUPPORTED_VERSION);
const OutMsgDescr t_OutMsgDescrDefault(tos::SUPPORTED_VERSION);
...
bool ExtBlkRef::unpack(vm::CellSlice& cs, tos::BlockIdExt& blkid, tos::LogicalTime* end_lt) const {
  block::gen::ExtBlkRef::Record data;
  if (!tlb::unpack(cs, data)) {
    blkid.invalidate();
    return false;
  }
  blkid.id = tos::BlockId{tos::masterchainId, tos::shardIdAll, data.seq_no};
...
bool ExtBlkRef::unpack(Ref<vm::CellSlice> cs_ref, tos::BlockIdExt& blkid, tos::LogicalTime* end_lt) const {
  block::gen::ExtBlkRef::Record data;
...
bool ExtBlkRef::store(vm::CellBuilder& cb, const tos::BlockIdExt& blkid, tos::LogicalTime end_lt) const {
...
Ref<vm::Cell> ExtBlkRef::pack_cell(const tos::BlockIdExt& blkid, tos::LogicalTime end_lt) const {
  vm::CellBuilder cb;
  return store(cb, blkid, end_lt) ? cb.finalize() : Ref<vm::Cell>{};
}

bool ExtBlkRef::pack_to(Ref<vm::Cell>& cell, const tos::BlockIdExt& blkid, tos::LogicalTime end_lt) const {
  vm::CellBuilder cb;
...
bool ShardIdent::Record::check() const {
  return workchain_id != tos::workchainInvalid && !(shard_prefix & ((1ULL << (63 - shard_pfx_bits)) - 1));
}
...
bool ShardIdent::unpack(vm::CellSlice& cs, tos::WorkchainId& workchain, tos::ShardId& shard) const {
  int bits;
  ...
  return cs.fetch_ulong(2) == 0                  // shard_ident$00
         ...
         && workchain != tos::workchainInvalid && !(shard & (2 * pow2 - 1)) && assign_or(shard, pow2);
}

bool ShardIdent::unpack(vm::CellSlice& cs, tos::ShardIdFull& data) const {
  return unpack(cs, data.workchain, data.shard);
}

bool ShardIdent::pack(vm::CellBuilder& cb, tos::WorkchainId workchain, tos::ShardId shard) const {
  int bits = tos::shard_prefix_length(shard);
  return workchain != tos::workchainInvalid               // check workchain
         && shard                                         // check shard
...
bool ShardIdent::pack(vm::CellBuilder& cb, tos::ShardIdFull data) const {
...
bool BlockIdExt::unpack(vm::CellSlice& cs, tos::BlockIdExt& data) const {
...
bool BlockIdExt::pack(vm::CellBuilder& cb, const tos::BlockIdExt& data) const {
