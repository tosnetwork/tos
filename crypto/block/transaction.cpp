...
td::Result<CustomComputePlan> resolve_custom_compute_plan(const block::ComputePhaseConfig& cfg,
  ...
  if (kind == CustomComputeKind::Evm) {
    plan.evm_chain_id = resolved->descriptor.vm_mode;
  }
  return plan;
}
 ...
 */
bool Account::set_address(tos::WorkchainId wc, td::ConstBitPtr new_addr) {
  workchain = wc;
  addr = new_addr;
  return true;
}
...
bool Account::unpack_address(vm::CellSlice& addr_cs) {
  ...
  int new_wc = tos::workchainInvalid;
  switch (addr_tag) {
  ...
  addr_cs.clear();
  if (new_wc == tos::workchainInvalid) {
    return false;
  }
  if (workchain == tos::workchainInvalid) {
    ...
    tos::StdSmcAddress new_addr = addr_orig;
 ...
 * @returns True if the unpacking is successful, false otherwise.
 */
bool Account::unpack(Ref<vm::CellSlice> shard_account, tos::UnixTime now, bool special) {
 ...
 */
bool Account::init_new(tos::UnixTime now) {
 ...
 */
bool Account::belongs_to_shard(tos::ShardIdFull shard) const {
  return workchain == shard.workchain && tos::shard_is_ancestor(shard.shard, addr);
}
 ...
 */
static bool add_partial_storage_payment(td::BigInt256& payment, tos::UnixTime delta, const block::StoragePrices& prices,
 ...
 */
td::RefInt256 StoragePrices::compute_storage_fees(tos::UnixTime now, const std::vector<block::StoragePrices>& pricing,
                                                  const StorageUsed& storage_used, tos::UnixTime last_paid,
  ...
  tos::UnixTime upto = std::max(last_paid, pricing[0].valid_since);
  td::RefInt256 total{true, 0};
  for (; i < n && upto < now; i++) {
    tos::UnixTime valid_until = (i < n - 1 ? std::min(now, pricing[i + 1].valid_since) : now);
    if (upto < valid_until) {
 ...
 * @returns The computed storage fees as RefInt256.
 */
td::RefInt256 Account::compute_storage_fees(tos::UnixTime now, const std::vector<block::StoragePrices>& pricing) const {
 ...
 * @returns None
 */
Transaction::Transaction(const Account& _account, int ttype, tos::LogicalTime req_start_lt, tos::UnixTime _now,
                         Ref<vm::Cell> _inmsg)
    : trans_type(ttype)
 ...
 */
bool ComputePhaseConfig::is_address_suspended(tos::WorkchainId wc, td::Bits256 addr) const {
 ...
 */
static td::optional<td::uint64> override_gas_limit(const ComputePhaseConfig& cfg, tos::UnixTime now,
  ...
  static std::map<std::pair<tos::WorkchainId, tos::StdSmcAddress>, OverridenGasLimit> accounts = []() {
    auto parse_addr = [](const char* s) -> std::pair<tos::WorkchainId, tos::StdSmcAddress> {
      auto r_addr = StdAddress::parse(td::Slice(s));
      r_addr.ensure();
      return {r_addr.ok().workchain, r_addr.ok().addr};
    };
    std::map<std::pair<tos::WorkchainId, tos::StdSmcAddress>, OverridenGasLimit> accounts;
...
bool Transaction::prepare_compute_phase(const ComputePhaseConfig& cfg) {
  ...
  // to the EVM executor instead of TVM.
  ...
  if (custom_plan.kind == CustomComputeKind::Evm && evm_workchain_dispatch::has_evm_compute_handler()) {
    ...
    bool ok = evm_workchain_dispatch::invoke_evm_compute(
        ...
        custom_plan.evm_chain_id,                         // chain_id from WorkchainDescr.vm_mode
    ...
    if (cp.accepted && cp.new_data.not_null()) {
      ...
      // EVM commit semantics intentionally differ from TVM: a revert is an
      // engine-level failure, but nonce/gas/receipt state still commits.
  ...
  // to the shielded-pool compute phase instead of TVM.
  ...
  if (custom_plan.kind == CustomComputeKind::Uno && uno_workchain_dispatch::has_uno_compute_handler()) {
    ...
    // — making state delta part of the TOS block state_hash. Uno commits only
    // successful engine transitions; rejected proofs do not update state.
...
bool Transaction::check_rewrite_dest_addr(Ref<vm::CellSlice>& dest_addr, const ActionPhaseConfig& cfg, bool* is_mc,
  ...
  if (rec.workchain_id != tos::masterchainId) {
 ...
 */
void Account::push_transaction(Ref<vm::Cell> trans_root, tos::LogicalTime trans_lt) {
...
td::Status FetchConfigParams::fetch_config_params(
    ...
    td::RefInt256* basechain_create_fee, tos::WorkchainId wc, tos::UnixTime now) {
...
td::Status FetchConfigParams::fetch_config_params(
    ...
    tos::WorkchainId wc, tos::UnixTime now) {
  *old_mparams = config.get_config_param(9);
  ...
  TRY_RESULT(size_limits, config.get_size_limits_config());
  {
    ...
    auto cell = config.get_config_param(wc == tos::masterchainId ? 20 : 21);
    if (cell.is_null()) {
